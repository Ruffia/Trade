...
__int64 CSQLiteRecordset::AsLong(int index)
{
	ASSERT(IsOpen());
	return sqlite3_column_int64(m_pStatement, index);
}
...
__int64 CSQLiteRecordset::AsLong(LPCTSTR szName)
{
	ASSERT(IsOpen());
	return AsLong(GetFieldIndex(szName));
}
