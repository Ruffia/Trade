...
void CXTPVisualStudio2008Theme::RefreshMetrics()
{
	...
	m_clrMenuGripper.SetStandardValue(XTPColorManager()->LightColor(GetXtremeColor(COLOR_3DFACE),
	...
	switch (XTPColorManager()->GetCurrentSystemTheme())
	{
		...
		case xtpSystemThemeAero:
		{
			...
			INT nElements[]		 = { XPCOLOR_TOOLBAR_GRIPPER,
								 ...
								 XPCOLOR_PUSHED_TEXT };
			COLORREF clrValues[] = { RGB(160, 160, 180),
									 RGB(160, 160, 180),
									 RGB(128, 128, 128),
									 RGB(240, 240, 240),
									 RGB(160, 160, 160),
...
void CXTPVisualStudio2008Theme::AdjustExcludeRect(CRect& rc, CXTPControl* pControl,
												  BOOL /*bVertical*/)
{
...
void CXTPVisualStudio2008Theme::DrawPopupBarGripper(CDC* pDC, int x, int y, int cx, int cy,
													BOOL bExpanded)
{
...
{
	pDC->FillSolidRect(rc.left + XTP_DPI_X(3), rc.top, rc.Width() - XTP_DPI_X(6), XTP_DPI_Y(1),
					   clrBorder);
	pDC->FillSolidRect(rc.left + XTP_DPI_X(3), rc.bottom - XTP_DPI_Y(1), rc.Width() - XTP_DPI_X(6),
					   XTP_DPI_Y(1), clrBorder);
	pDC->FillSolidRect(rc.left, rc.top + XTP_DPI_Y(3), XTP_DPI_X(1), rc.Height() - XTP_DPI_Y(6),
					   clrBorder);
	pDC->FillSolidRect(rc.right - XTP_DPI_X(1), rc.top + XTP_DPI_Y(3), XTP_DPI_X(1),
					   rc.Height() - XTP_DPI_Y(6), clrBorder);
...
void CXTPVisualStudio2008Theme::DrawRectangle(CDC* pDC, CRect rc, BOOL bSelected, BOOL bPressed,
											  BOOL bEnabled, BOOL bChecked, BOOL bPopuped,
											  XTPBarType barType, XTPBarPosition barPosition)
{
	if (barPosition == xtpBarPopup && barType == xtpBarTypePopup && bSelected && !bChecked
		&& !bPressed)
	{
		XTPDrawHelpers()->GradientFill(pDC,
									   CRect(rc.left + XTP_DPI_X(1), rc.top + XTP_DPI_Y(1),
											 rc.right - XTP_DPI_X(1), rc.bottom - XTP_DPI_Y(1)),
									   m_clrMenuSelection, FALSE);
	...
	if (barType == xtpBarTypeMenuBar && bSelected && !bChecked && !bPressed && bEnabled)
	{
		...
		pDC->FillSolidRect(rc.left + XTP_DPI_X(1), rc.top, rc.Width() - XTP_DPI_X(2), XTP_DPI_Y(1),
						   clrShadow);
		pDC->FillSolidRect(rc.left + XTP_DPI_X(1), rc.bottom - XTP_DPI_Y(1),
						   rc.Width() - XTP_DPI_X(2), XTP_DPI_Y(1), clrShadow);
		pDC->FillSolidRect(rc.left, rc.top + XTP_DPI_Y(1), XTP_DPI_X(1), rc.Height() - XTP_DPI_Y(2),
						   clrShadow);
		pDC->FillSolidRect(rc.right - XTP_DPI_X(1), rc.top + XTP_DPI_Y(1), XTP_DPI_X(1),
						   rc.Height() - XTP_DPI_Y(2), clrShadow);
		rc.DeflateRect(1, 1);
		...
		{
			if (m_bLunaTheme)
				XTPDrawHelpers()->GradientFill(pDC, rc, m_grcLunaSelected, FALSE);
			else
	...
	CXTPVisualStudio2005Theme::DrawRectangle(pDC, rc, bSelected, bPressed, bEnabled, bChecked,
...
COLORREF CXTPVisualStudio2008Theme::GetRectangleTextColor(BOOL bSelected, BOOL bPressed,
														  BOOL bEnabled, BOOL bChecked,
														  BOOL bPopuped, XTPBarType barType,
														  XTPBarPosition barPosition)
{
	if (barPosition == xtpBarPopup && barType == xtpBarTypePopup && bSelected && !bPressed
		&& bEnabled)
	{
		return m_clrMenuSelectionText;
	}

	return CXTPVisualStudio2005Theme::GetRectangleTextColor(bSelected, bPressed, bEnabled, bChecked,
															bPopuped, barType, barPosition);
}

CSize CXTPVisualStudio2008Theme::DrawCommandBarSeparator(CDC* pDC, CXTPCommandBar* pBar,
														 CXTPControl* pControl, BOOL bDraw)
{
	...
	else
	{
		VerticalLine(pDC, rcControl.left - XTP_DPI_X(2), rcRow.top - XTP_DPI_Y(1),
					 rcRow.bottom + XTP_DPI_X(1), GetXtremeColor(XPCOLOR_SEPARATOR));
		VerticalLine(pDC, rcControl.left - XTP_DPI_X(1), rcRow.top - XTP_DPI_Y(1),
					 rcRow.bottom + XTP_DPI_X(1), GetXtremeColor(COLOR_3DHILIGHT));
	}
