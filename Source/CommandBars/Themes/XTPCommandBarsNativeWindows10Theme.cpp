// XTPCommandBarsNativeWindows10Theme.cpp : implementation of the
// CXTPCommandBarsNativeWindows10Theme class.
//
...
#include "Common/ScrollBar/Themes/XTPScrollBarThemeNativeWindows10.h"
...
#include "CommandBars/Gallery/XTPControlGalleryNativeWindows10Theme.h"
...
#include "CommandBars/Progress/XTPProgressThemeNativeWindows10.h"
...
#include "CommandBars/MessageBar/XTPMessageBarThemeNativeWindows10.h"

// Status bar
#include "CommandBars/StatusBar/XTPStatusBarPaintManager.h"
#include "CommandBars/StatusBar/XTPStatusBarThemeOffice2013.h"
#include "CommandBars/StatusBar/XTPStatusBarThemeNativeWindows10.h"
...
IMPLEMENT_DYNAMIC(CXTPCommandBarsNativeWindows10Theme, CXTPCommandBarsOffice2013Theme)

CXTPCommandBarsNativeWindows10Theme::CXTPCommandBarsNativeWindows10Theme()
{
	// Frame paint manager
	SAFE_DELETE(m_pFramePaintManager);
	m_pFramePaintManager = new CXTPFrameThemeNativeWindows10(this);

	// Progress paint manager
	SAFE_DELETE(m_pProgressPaintManager);
	m_pProgressPaintManager = new CXTPProgressThemeNativeWindows10(this);

	// Slider paint manager
	SAFE_DELETE(m_pSliderPaintManager);
	m_pSliderPaintManager = new CXTPSliderThemeNativeWindows10(this);

	// Message bar paint manager
	SAFE_DELETE(m_pMessageBarPaintManager);
	m_pMessageBarPaintManager = new CXTPMessageBarThemeNativeWindows10(this);

	// Status bar paint manager
	SAFE_DELETE(m_pStatusBarPaintManager);
	m_pStatusBarPaintManager = new CXTPStatusBarThemeNativeWindows10(this);

	// Ribbon paint manager
	SAFE_DELETE(m_pRibbonPaintManager);
	m_pRibbonPaintManager = new CXTPRibbonThemeNativeWindows10(this);

	// Gallery paint manager
	SAFE_DELETE(m_pGalleryPaintManager);
	m_pGalleryPaintManager = new CXTPControlGalleryNativeWindows10Theme(this);

	// Scrollbar paint manager
	SAFE_DELETE(m_pScrollBarPaintManager);
	m_pScrollBarPaintManager = new CXTPScrollBarThemeNativeWindows10();
...
void CXTPCommandBarsNativeWindows10Theme::SetStateColor(COLORREF clrState /*=COLORREF_NULL*/)
{
	CXTPFrameThemeNativeWindows10* pFramePaintManager = static_cast<CXTPFrameThemeNativeWindows10*>(
		m_pFramePaintManager);
	pFramePaintManager->SetStateColor(clrState);

	CXTPStatusBarThemeNativeWindows10* pStatusBarPaintManager =
		static_cast<CXTPStatusBarThemeNativeWindows10*>(m_pStatusBarPaintManager);
	pStatusBarPaintManager->SetStateColor(clrState);
}

COLORREF CXTPCommandBarsNativeWindows10Theme::GetStateColor() const
{
	const CXTPFrameThemeNativeWindows10* pFramePaintManager =
		static_cast<const CXTPFrameThemeNativeWindows10*>(m_pFramePaintManager);
	COLORREF clr1 = pFramePaintManager->GetStateColor();

	const CXTPStatusBarThemeNativeWindows10* pStatusBarPaintManager =
		static_cast<const CXTPStatusBarThemeNativeWindows10*>(m_pStatusBarPaintManager);
	COLORREF clr2 = pStatusBarPaintManager->GetStateColor();

	return clr1 == clr2 ? clr1 : COLORREF_NULL;
}

void CXTPCommandBarsNativeWindows10Theme::DrawSplitButtonFrame(CDC* pDC, CXTPControl* pButton,
															   CRect rcButton)
{
...
void CXTPCommandBarsNativeWindows10Theme::DrawCheckRect(CDC* pDC, CRect rc, COLORREF clrFore,
														COLORREF clrBack)
{
	ASSERT_VALID(pDC);
...
void CXTPCommandBarsNativeWindows10Theme::DrawGalleryItemBack(CDC* pDC, CRect rcItem, BOOL bEnabled,
...
COLORREF CXTPCommandBarsNativeWindows10Theme::GetControlTextColor(CXTPControl* pButton)
{
	ASSERT_VALID(pButton);
...
void CXTPCommandBarsNativeWindows10Theme::DrawRectangle(CDC* pDC, CRect rc, BOOL bSelected,
...
BOOL CXTPCommandBarsNativeWindows10Theme::DrawRectangle(CDC* pDC, CXTPControl* pButton)
{
	return CXTPCommandBarsOffice2013Theme::DrawRectangle(pDC, pButton);
}

BOOL AFX_CDECL XTPSetNativeWindows10StateColor(CXTPCommandBars* pCommandBars,
											   COLORREF clrState /*=COLORREF_NULL*/)
{
	if (!pCommandBars)
		return FALSE;

	CXTPCommandBarsNativeWindows10Theme* pPaintManager =
		DYNAMIC_DOWNCAST(CXTPCommandBarsNativeWindows10Theme, pCommandBars->GetPaintManager());
...
CSize CXTPCommandBarsNativeWindows10Theme::DrawCommandBarGripper(CDC* pDC, CXTPCommandBar* pBar,
																 BOOL bDraw)
{
	CSize szGripper(0, 0);
...
CSize CXTPCommandBarsNativeWindows10Theme::DrawControlComboBox(CDC* pDC,
