...
void CXTPVisualStudio2010Theme::RefreshMetrics()
{
	...
	INT nElements[]		 = { XPCOLOR_TOOLBAR_GRIPPER,
						 ...
						 XPCOLOR_PUSHED_TEXT };
	COLORREF clrValues[] = { RGB(97, 115, 140),
							 RGB(133, 145, 162),
							 RGB(128, 128, 128),
							 RGB(221, 226, 232),
							 RGB(155, 167, 183),
...
void CXTPVisualStudio2010Theme::DrawPopupBarGripper(CDC* pDC, int x, int y, int cx, int cy,
													BOOL bExpanded)
{
	CXTPOffice2003Theme::DrawPopupBarGripper(pDC, x, y, cx, cy, bExpanded);
}

void CXTPVisualStudio2010Theme::DrawRectangle(CDC* pDC, CRect rc, BOOL bSelected, BOOL bPressed,
											  ...
											  XTPBarType barType, XTPBarPosition barPosition)
{
		...
		&& !bPressed)
	{
		if (m_bLunaTheme)
		{
			XTPDrawHelpers()->GradientFill(pDC,
										   ...
										   m_grcLunaSelected, FALSE);
		}
		else
		{
			pDC->FillSolidRect(CRect(rc.left + XTP_DPI_X(1), rc.top + XTP_DPI_Y(1),
									 rc.right - XTP_DPI_X(1), rc.bottom - XTP_DPI_Y(1)),
							   GetXtremeColor(XPCOLOR_HIGHLIGHT));
		}
	...
	CXTPOffice2003Theme::DrawRectangle(pDC, rc, bSelected, bPressed, bEnabled, bChecked, bPopuped,
									   barType, barPosition);
}
...
void CXTPVisualStudio2010Theme::DrawRectangle(CDC* pDC, CRect rc, BOOL bSelected, BOOL bPressed,
											  ...
											  int nID)
{
	CXTPCommandBarsOffice2003Theme::DrawRectangle(pDC, rc, bSelected, bPressed, bEnabled, bChecked,
...
COLORREF CXTPVisualStudio2010Theme::GetRectangleTextColor(BOOL bSelected, BOOL bPressed,
...
CSize CXTPVisualStudio2010Theme::DrawCommandBarSeparator(CDC* pDC, CXTPCommandBar* pBar,
	...
	if (bDraw && pBar->GetType() == xtpBarTypePopup)
	{
		...
		if (!pControl->GetWrap())
			...
			VerticalLine(pDC, rcControl.left - XTP_DPI_X(2), rcRow.top - XTP_DPI_Y(1),
						 rcRow.bottom + XTP_DPI_Y(1), m_clrMenuSeparator);
...
void CXTPVisualStudio2010Theme::FillCommandBarEntry(CDC* pDC, CXTPCommandBar* pBar)
{
	...
	{
		if (m_bLunaTheme)
			XTPDrawHelpers()->GradientFill(pDC, rc, RGB(200, 209, 224), RGB(174, 185, 205),
	...
	else if (pBar->GetPosition() == xtpBarTop || pBar->GetPosition() == xtpBarBottom)
	{
		...
		pDC->FillSolidRect(rcFill.left + XTP_DPI_X(1), rcFill.top, rcFill.Width() - XTP_DPI_X(2),
						   XTP_DPI_Y(1), m_clrCommandBar.clrDark);
		pDC->FillSolidRect(rcFill.left, rcFill.top + XTP_DPI_Y(1), rcFill.Width(),
						   rcFill.Height() - XTP_DPI_X(2), m_clrCommandBar.clrDark);
		pDC->FillSolidRect(rcFill.left + XTP_DPI_X(1), rcFill.bottom - XTP_DPI_Y(1),
						   rcFill.Width() - XTP_DPI_X(2), XTP_DPI_Y(1), m_clrCommandBar.clrDark);
	...
	else if (pBar->GetPosition() == xtpBarLeft || pBar->GetPosition() == xtpBarRight)
	{
		...
		pDC->FillSolidRect(rcFill.left + XTP_DPI_X(1), rcFill.top, rcFill.Width() - XTP_DPI_X(2),
						   XTP_DPI_Y(1), m_clrCommandBar.clrDark);
		pDC->FillSolidRect(rcFill.left, rcFill.top + XTP_DPI_Y(1), rcFill.Width(),
						   rcFill.Height() - XTP_DPI_Y(2), m_clrCommandBar.clrDark);
		pDC->FillSolidRect(rcFill.left + XTP_DPI_X(1), rcFill.bottom - XTP_DPI_Y(1),
						   rcFill.Width() - XTP_DPI_X(2), XTP_DPI_Y(1), m_clrCommandBar.clrDark);
