...
void CXTPImageEditorPicture::Init(UINT nID, CXTPImageEditorDlg* pParentWnd)
{
	...
	SetWindowPos(&CWnd::wndTop, rc.left, rc.top, sz.cx + XTP_DPI_X(1), sz.cy + XTP_DPI_Y(1),
				 SWP_NOMOVE);
...
{
	::SetCursor(GetSelectedTool() == xtpToolFill
					? m_hCursorFillColor
					: GetSelectedTool() == xtpToolPencil
						  ? m_hCursorPencil
						  : GetSelectedTool() == xtpToolGetColor ? m_hCursorPickColor
																 : m_hCursorLine);
...
void CXTPImageEditorPicture::OnPaint()
{
	...
	if (m_pPictureBitmap)
	{
		if (!CXTPImageManagerIcon::GetBitmapBits(dc, (HBITMAP)m_pPictureBitmap->GetSafeHandle(),
												 pbmi, (LPVOID&)pBits, nSize))
			return;
	}
	...
	for (int x = 0; x < rc.Width(); x++)
	{
		for (int y = 0; y < rc.Height(); y++)
		{
			...
			{
				clr = (m_pPictureBitmap == NULL ? XTPCOLOR_ERASE
												: pBits[(pt.y * pbmi->bmiHeader.biWidth + pt.x)]);
				clr = CLR_TO_RGBQUAD(clr);

				if (clr == XTPCOLOR_ERASE)
					clr = (((x + 3) / 4) + ((y + 3) / 4)) % 2 == 0 ? RGB(255, 255, 255)
																   : RGB(235, 235, 235);
			}
...
void AFX_CDECL CXTPImageEditorPicture::CopyBitmap(CAlphaBitmap* pBitmapCopyTo,
												  CAlphaBitmap* pBitmapCopyFrom, CSize sz,
												  CPoint ptOffset, COLORREF clrTransparentFrom,
												  COLORREF clrTransparentTo, CDC* pDC)
{
	CDC dc;
...
void CXTPImageEditorPicture::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	switch (GetSelectedTool())
	{
		...
		case xtpToolLine:
		{
			...
			m_clrDraw = GetSelectedColor() != GetPicturePixel(ClientToPicture(point))
							? GetSelectedColor()
							: XTPCOLOR_ERASE;
...
void CXTPImageEditorPicture::FixAlphaLayer(CAlphaBitmap* pDest, CAlphaBitmap* pSrc)
{
	...
	if (!CXTPImageManagerIcon::GetBitmapBits(dcSrc, (HBITMAP)pDest->GetSafeHandle(), pDestBmi,
											 (LPVOID&)pDestBits, nSize))
		return;

	if (!CXTPImageManagerIcon::GetBitmapBits(dcSrc, (HBITMAP)pSrc->GetSafeHandle(), pSrcBmi,
											 (LPVOID&)pSrcBits, nSize))
	{
		free(pDestBits);
		free(pDestBmi);
		return;
	...
	HBITMAP hBitmapAlpha = CreateDIBSection(dcSrc, pDestBmi, DIB_RGB_COLORS, (void**)&pFixedBits,
											NULL, 0);
...
void CXTPImageEditorPicture::Copy()
{
	...
	CopyBitmap(&bitmapCopy, m_pPictureBitmap, m_szPicture, 0, XTPCOLOR_ERASE,
...
void CXTPImageEditorPicture::Paste()
{
	...
	bmpClip.m_bAlpha = bAlpha ? bAlpha
							  : data.IsDataAvailable(XTPImageManager()->m_nAlphaClipFormat);
	bmpClip.Attach(dataMedium.hBitmap);

	m_pCurrentBitmap = m_pPictureBitmap;

	m_pPictureBitmap = new CAlphaBitmap();
	CopyBitmap(m_pPictureBitmap, &bmpClip, m_szPicture, 0, GetXtremeColor(COLOR_3DFACE),
			   XTPCOLOR_ERASE);
...
void CXTPImageEditorPicture::Load()
{
	...
	CXTPResourceManager::AssertValid(
		XTPResourceManager()->LoadString(&strFilter, XTP_IDS_IMAGEEDITOR_FILTER));
	...
	if (fd.DoModal() == IDOK)
	{
		...
		if (strExtension == _T("ico"))
		{
			...
			if (!hBitmap)
			{
				HICON hIcon = (HICON)LoadImage(0, fd.GetPathName(), IMAGE_ICON, m_szPicture.cx,
											   m_szPicture.cy,
		...
		else if (strExtension == _T("bmp") || strExtension == _T("png"))
		{
			...
			HBITMAP hBmp = CXTPImageManagerIcon::LoadBitmapFromFile(fd.GetPathName(),
																	&bAlphaBitmap);
		...
		else
		{
			...
			if (OleLoadPicturePath((LPOLESTR)XTP_CT2CW(fd.GetPathName()), NULL, 0, 0, IID_IPicture,
								   (LPVOID*)&pPict)
				== S_OK)
			{
				m_pCurrentBitmap = m_pPictureBitmap;
				...
				pPict->Render(dc, 0, 0, m_szPicture.cx, m_szPicture.cy, 0, hmHeight - 1, hmWidth,
							  -hmHeight, 0);
...
void CXTPImageEditorPicker::OnPaint()
{
	...
	else
	{
		...
		dc.FillSolidRect(rcItem.left, rcItem.top, rcItem.Width() / 2, rcItem.Height() / 2,
						 RGB(235, 235, 235));
		dc.FillSolidRect(rcItem.left + rcItem.Width() / 2, rcItem.top + rcItem.Height() / 2,
						 rcItem.Width() - (rcItem.Width() / 2),
						 rcItem.Height() - (rcItem.Height() / 2), RGB(235, 235, 235));
	}
	if (!m_bSelected)
	{
...
void CXTPImageEditorPreview::OnPaint()
{
	...
	CRect rcButton(XTP_DPI_X(10), (rc.Height() - m_szPicture.cy) / 2,
				   XTP_DPI_X(10) + m_szPicture.cx,
				   (rc.Height() - m_szPicture.cy) / 2 + m_szPicture.cy);
	...
	if (pBitmap)
	{
		if (pBitmap->m_bAlpha)
		{
			HBITMAP hBitmap = CXTPImageManagerIcon::PreMultiplyAlphaBitmap(
				(HBITMAP)pBitmap->GetSafeHandle());
			if (hBitmap)
			{
				CXTPImageManagerIcon::DrawAlphaBitmap(&dc, hBitmap, rcButton.TopLeft(),
													  m_szPicture);
		...
		else
		{
			...
			CXTPImageEditorPicture::CopyBitmap(&bmpClient, pBitmap, m_szPicture, 0, XTPCOLOR_ERASE,
											   GetXtremeColor(COLOR_3DFACE), &dc);
...
{
	InitModalIndirect(XTPResourceManager()->LoadDialogTemplate(XTP_IDD_IMAGEEDITOR_DIALOG),
					  pParent);
...
{
	UINT nButtons[] = { XTP_ID_BUTTON_PENCIL, XTP_ID_BUTTON_FILL,	 XTP_ID_BUTTON_UNDO,
						XTP_ID_BUTTON_REDO,   XTP_ID_BUTTON_GETCOLOR, XTP_ID_BUTTON_LINE,
						XTP_ID_BUTTON_COPY,   XTP_ID_BUTTON_PASTE,	XTP_ID_BUTTON_RECTANGLE,
						XTP_ID_BUTTON_CIRCLE, XTP_ID_BUTTON_CLEAR,	XTP_ID_BUTTON_OPEN,

						XTP_IDC_BUTTON_LEFT,  XTP_IDC_BUTTON_DOWN,	XTP_IDC_BUTTON_UP,
	...
	const struct
	{
		...
		UINT nID;
		POINT ptOffset;
	} colors[] = { { RGB(0x00, 0x00, 0x00), XTP_IDS_CLR_BLACK, { 0, 0 } },
	...
	CSize sz = m_wndToolbarTools.CalcDockingLayout(XTP_DPI_X(23) * 4 + XTP_DPI_X(7),
												   LM_HORZDOCK | LM_HORZ | LM_COMMIT);
	m_wndToolbarTools.SetWindowPos(&CWnd::wndTop, rcTools.left - XTP_DPI_X(7),
								   rcTools.top - XTP_DPI_Y(4), sz.cx, sz.cy, 0);
	...
	sz = m_wndToolbarMove.CalcDockingLayout(XTP_DPI_X(23) * 4 + XTP_DPI_X(7),
											LM_HORZDOCK | LM_HORZ | LM_COMMIT);
	m_wndToolbarMove.SetWindowPos(&CWnd::wndTop, rcMove.left - XTP_DPI_X(7),
								  rcMove.top - XTP_DPI_Y(4), sz.cx, sz.cy, 0);
	...
	for (int i = 0; i < _countof(colors); i++)
	{
		...
		rcItem.OffsetRect(colors[i].ptOffset.x * XTP_DPI_X(13),
						  colors[i].ptOffset.y * XTP_DPI_Y(13));

		m_wndSelector[i].Create(NULL, WS_CHILD | WS_VISIBLE | SS_NOTIFY, rcItem, this,
								colors[i].nID);
...
{
	CXTPImageEditorPicker* pWnd = DYNAMIC_DOWNCAST(CXTPImageEditorPicker,
												   CWnd::FromHandlePermanent(pNMHDR->hwndFrom));
	ASSERT(pWnd);
	SetSelected(pWnd);
}
...
{
	m_wndPicture.MovePicture(nID == XTP_IDC_BUTTON_LEFT
								 ? CPoint(-1, 0)
								 : nID == XTP_IDC_BUTTON_RIGHT
									   ? CPoint(+1, 0)
