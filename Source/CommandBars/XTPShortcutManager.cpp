...
const CXTPShortcutManagerAccel& CXTPShortcutManagerAccel::
	operator=(const CXTPShortcutManagerAccel& accel)
{
	cmd = accel.cmd;
...
#define SAFE_DESTROY_ACCELTABLE(hAccelTable)                                                       \
	if (hAccelTable)                                                                               \
	{                                                                                              \
...
{
	if ((pAccel->key[0].fVirt & FCONTROL) == 0 && (pAccel->key[0].fVirt & FSHIFT) == 0
		&& (pAccel->key[0].fVirt & FALT) == 0 && (pAccel->key[0].fVirt & FVIRTKEY)
...
BOOL CXTPShortcutManager::IsAccelMessage(CXTPShortcutManagerAccel::SHORTCUTACCEL& accel,
										 int nKeyState, LPMSG lpMsg) const
{
	BOOL fVirt = lpMsg->message == WM_KEYDOWN || lpMsg->message == WM_SYSKEYDOWN;
	WORD flags = accel.fVirt;

	if ((DWORD)accel.key != lpMsg->wParam || ((fVirt != 0) != ((flags & FVIRTKEY) != 0)))
	{
		return FALSE;
	}
...
int CXTPShortcutManager::GetAccelKeyState() const
{
	...
	int nKeyState = 0;
	if (GetKeyState(vkCtrl) & 0x8000)
		nKeyState |= FCONTROL;
	if (GetKeyState(vkAlt) & 0x8000)
		nKeyState |= FALT;
...
BOOL CXTPShortcutManager::TranslateAccelerator(LPMSG lpMsg)
{
	...
	if (lpMsg->message != WM_KEYDOWN && lpMsg->message != WM_SYSKEYDOWN && lpMsg->message != WM_CHAR
	...
	for (int i = 0; i < nAccelSize; i++)
	{
		...
		if (IsWindowEnabled(hWnd) && (!m_bDisableOnCapture || ::GetCapture() == NULL)
			&& !IsIconic(hWnd) && accel->cmd != 0)
		{
			if (accel->key[1].key != 0)
			{
				if (m_pCommandBars->GetSite()->IsFrameWnd())
				{
					...
					CXTPResourceManager::AssertValid(
						XTPResourceManager()->LoadString(&strFormat, XTP_IDS_SHORTCUT_SECONDKEY));
				...
				while (::GetCapture() == hWnd)
				{
					...
					if (msg.message == WM_KEYDOWN || msg.message == WM_SYSKEYDOWN)
					{
						...
						for (int j = 0; j < nAccelSize; j++)
						{
							...
							if (secondAccel->key[0].fVirt == accel->key[0].fVirt
								&& secondAccel->key[0].key == accel->key[0].key)
							{
					...
					if (msg.message == WM_COMMAND || msg.message == WM_ACTIVATE
						|| msg.message == WM_LBUTTONDOWN || msg.message == WM_NCACTIVATE
						|| msg.message == WM_CLOSE || msg.message == WM_SYSCOMMAND)
						break;
				}
				::ReleaseCapture();
...
CXTPShortcutManager::CKeyHelper::CKeyHelper(const CXTPShortcutManagerAccel* lpAccel,
											CXTPShortcutManager* pManager /*= NULL*/)
	: m_lpAccel(lpAccel)
	, m_pManager(pManager)
{
...
CString CXTPShortcutManager::CKeyHelper::Format(
	const CXTPShortcutManagerAccel::SHORTCUTACCEL* pAccel) const
{
	CString str;
...
BOOL CXTPShortcutManager::CKeyAssign::PreTranslateMessage(MSG* pMsg)
{
	...
	if (pMsg->message != WM_KEYDOWN && pMsg->message != WM_SYSKEYDOWN && pMsg->message != WM_KEYUP
...
BOOL CXTPShortcutManager::CKeyAssign::TranslateDoubleKeyShortcutsMessage(MSG* pMsg)
{
	...
	if (m_keyHelper.GetShortcutManager()
		&& !m_keyHelper.GetShortcutManager()->OnPreviewEditKey(&m_accel))
	{
		ResetKey();
		return TRUE;
	}
...
BOOL CXTPShortcutManager::CKeyAssign::TranslateSingleKeyShortcutsMessage(MSG* pMsg)
{
	...
	else if (m_nKeyDefined == 0)
	{
		...
		if (pMsg->wParam == VK_SHIFT || pMsg->wParam == VK_CONTROL || pMsg->wParam == VK_MENU
			|| pMsg->wParam == VK_LWIN || pMsg->wParam == VK_RWIN)
		{
...
void CXTPShortcutManager::AddShortcut(long cmd, LPCTSTR strKey)
{
	...
	if (!ParseShortcut(strKey, &accel) && m_nEnableCustomShortcutText)
	{
		...
		for (int i = m_pCommandBars->GetCount() - 1; i >= 0; i--)
		{
			...
			if (pBar->IsVisible())
			{
				CXTPControl* pControl = pBar->GetControls()->FindControl(xtpControlError, cmd,
																		 FALSE, TRUE);
...
void CXTPShortcutManager::SaveShortcuts(LPCTSTR lpszProfileName)
{
	...
	AfxGetApp()->WriteProfileBinary(lpszProfileName, _T("Accelerators"), (LPBYTE)lpAccel,
									m_pAccelTable->GetCount() * sizeof(XTP_SHORTCUTMANAGER_ACCEL));
...
void CXTPShortcutManager::LoadShortcuts(LPCTSTR lpszProfileName)
{
	...
	if (nVersion == 0)
	{
		UINT uiSize;
		LPACCEL lpAccel = 0;
		if (AfxGetApp()->GetProfileBinary(lpszProfileName, _T("Accelerators"), (LPBYTE*)&lpAccel,
										  &uiSize))
		{
...
BOOL CXTPShortcutManager::ParseShortcut(CString strShortcutKey, BYTE& fVirt, WORD& key) const
{
	...
	if (strShortcutKey.GetLength() > 3 && strShortcutKey[0] == _T('0')
		&& strShortcutKey[1] == _T('x'))
	{
...
BOOL CXTPShortcutManager::OnBeforeAdd(CXTPShortcutManagerAccel* accel)
{
	...
	FireEvent(2, EVENT_PARAM(VTS_I4 VTS_I4 VTS_I4 VTS_PVARIANT), (long)accel->key[0].fVirt,
			  (long)accel->key[0].key, (long)accel->cmd, &vtCanceled);
...
BOOL CXTPShortcutManager::OnBeforeRemove(CXTPShortcutManagerAccel* accel)
{
	...
	FireEvent(3, EVENT_PARAM(VTS_I4 VTS_I4 VTS_I4 VTS_PVARIANT), (long)accel->key[0].fVirt,
			  (long)accel->key[0].key, (long)accel->cmd, &vtCanceled);
...
BEGIN_DISPATCH_MAP(CXTPShortcutManager, CXTPCmdTarget)
	...
	DISP_FUNCTION_ID(CXTPShortcutManager, "SetKeyNameText", 3, SetKeyNameText, VT_EMPTY,
					 VTS_I4 VTS_BSTR)
	DISP_PROPERTY_ID(CXTPShortcutManager, "UseSystemKeyNameText", 4, m_bUseSystemKeyNameText,
					 VT_BOOL)
	DISP_FUNCTION_ID(CXTPShortcutManager, "Delete", 5, OleDelete, VT_EMPTY, VTS_I4)
	DISP_PROPERTY_ID(CXTPShortcutManager, "AllowDoubleKeyShortcuts", 6, m_bAllowDoubleKeyShortcuts,
					 VT_BOOL)
	DISP_FUNCTION_ID(CXTPShortcutManager, "AddShortcut", 7, OleAddShortcut, VT_EMPTY,
					 VTS_I4 VTS_BSTR)
	...
	DISP_FUNCTION_ID(CXTPShortcutManager, "NewEnum", DISPID_NEWENUM, OleNewEnum, VT_DISPATCH,
					 VTS_NONE)
	...
	DISP_PROPERTY_EX_ID(CXTPShortcutManager, "EnableCustomShortcutText", 12,
						OleGetEnableCustomShortcutText, OleSetEnableCustomShortcutText, VT_BOOL)
END_DISPATCH_MAP()
...
BEGIN_DISPATCH_MAP(CXTPShortcutManagerAccel, CXTPCmdTarget)
	...
	DISP_PROPERTY_EX_ID(CXTPShortcutManagerAccel, "ShortcutText", 2, OleGetShortcutText,
