	...
	virtual void OnExecute()
	{
		...
		if (m_pControl)
		{
			if (m_pControl->GetHideFlags() & xtpHideCustomize)
				...
				m_pControl->SetHideFlag(xtpHideCustomize, TRUE);
...
{
	GetPaintManager()->DrawSpecialControl(pDC,
										  m_pParent->GetPosition() == xtpBarFloating
											  ? xtpButtonExpandFloating
											  : xtpButtonExpandToolbar,
										  this, m_pParent, TRUE, &m_bHiddenExists);
}
...
BOOL CXTPToolBar::CControlButtonExpand::OnSetPopup(BOOL bPopup)
{
	...
	if (bPopup)
	{
		...
		if (m_pCommandBar == NULL)
			m_pCommandBar = CXTPPopupToolBar::CreatePopupToolBar(m_pParent->GetCommandBars());
		...
		if (pCommandBars && ((CXTPToolBar*)m_pParent)->m_pDockContext)
		{
			...
			if (pCommandBars->GetCommandBarsOptions()->bShowExpandButtonAlways
				|| pCommandBars->IsCustomizeAvail())
			{
				CXTPControlPopup* pControl = (CXTPControlPopup*)m_pCommandBar->GetControls()->Add(
					xtpControlPopup, XTP_ID_CUSTOMIZE_ADDORREMOVE);
				pControl->SetFlags(xtpFlagManualUpdate);

				pControl->SetBeginGroup(m_pCommandBar->GetControlCount() > 1);
				pCommandBars->GetAddOrRemovePopup((CXTPToolBar*)m_pParent,
												  pControl->GetCommandBar());
...
CXTPToolBar::~CXTPToolBar(){ SAFE_DELETE(m_pDockContext) }
	...
	for (int i = 0; i < nCount; i++)
	{
		...
		else
		{
			...
			if (pControl == NULL)
			{
				...
				m_pControls->Add(pControl, XTPToInt(nID));
...
{
	if (GetShowGripper() && (IsDockingPosition(GetPosition())) && m_pDockContext
		&& ((m_dwFlags & (xtpFlagAlignAny | xtpFlagFloating)) != 0))
	{
...
INT_PTR CXTPCommandBar::OnToolHitTest(CPoint point, TOOLINFO* pTI) const
{
	...
	if (m_barType == xtpBarTypePopup
		&& (!pCommandBars
			|| (!pCommandBars->GetCommandBarsOptions()->bShowPopupBarToolTips
	...
	if (nHit != -1)
	{
		...
		{
			if (((XTP_TOOLTIP_TOOLINFO_EX*)pTI)->pToolInfo != NULL)
	...
	if (pCommandBars && bToolBarAccelTips)
	{
		if (strShortcut.IsEmpty())
		{
			...
			if (!pCommandBars->GetShortcutManager()->FindDefaultAccelerator(pControl->GetID(),
																			strShortcut))
				strShortcut.Empty();
		}
	...
	CXTPToolTipContext::FillInToolInfo(pTI, m_hWnd, rect, nHit, strTip, strTip,
									   pControl->GetDescription(), GetImageManager());
...
BOOL CXTPToolBar::Create(CWnd* pParentWnd, BOOL bFloat)
{
	if (!IsWindow(m_hWnd))
	{
		DWORD dwStyle = (bFloat ? WS_POPUP : WS_CHILD) | MFS_SYNCACTIVE | WS_CLIPCHILDREN
						| WS_CLIPSIBLINGS;
		...
		if (!CWnd::CreateEx(dwExStyle, _T("XTPToolBar"), m_strTitle, dwStyle,
							CRect(pt, CSize(0, 0)), pParentWnd, 0))
			return FALSE;
...
CSize CXTPToolBar::CalcDockingLayout(int nLength, DWORD dwMode, int nWidth)
{
	...
	CXTPCommandBars* pCommandBars = GetCommandBars();
	if (pCommandBars)
		bShowExpandButtonAlways = pCommandBars->GetCommandBarsOptions()->bShowExpandButtonAlways;
	...
	if (m_bShowExpandButton)
	{
		...
		if (!pPopupExp)
		{
			pPopupExp = (CXTPControlPopup*)m_pControls->Add(new CControlButtonExpand(),
															XTP_ID_TOOLBAR_EXPAND, _T(""), -1,
															TRUE);
		}
	...
	if (pPopupExp)
	{
		...
		GetPaintManager()->DrawSpecialControl(0, xtpButtonExpandToolbar, pPopupExp, this, FALSE,
											  &sz);
	}
...
CSize CXTPToolBar::CalcDynamicLayout(int nLength, DWORD nMode)
{
	...
	if (IsCloseable())
	{
		...
		if (!pHide)
		{
			pHide = (CXTPControlPopup*)m_pControls->Add(new CControlButtonHide(),
														XTP_ID_TOOLBAR_HIDE, _T(""), -1, TRUE);
		}
...
LRESULT CXTPToolBar::OnFloatStatus(WPARAM wParam, LPARAM)
{
	...
	CXTPCommandBars* pCommandBars = GetCommandBars();
	if (pCommandBars)
		bSyncActivate = pCommandBars->GetCommandBarsOptions()->bSyncFloatingBars;
...
void CXTPToolBar::OnNcLButtonDown(UINT nHitTest, CPoint point)
{
	...
	if (nHitTest >= HTSIZEFIRST && nHitTest <= HTSIZELAST)
	{
		...
		m_pCommandBars->ClosePopups();
		m_pDockContext->StartResize(XTPToInt(nHitTest), point);
		return;
	}
...
{
	return GetSafeHwnd() && IsVisible() && CWnd::IsWindowVisible()
		   && !CXTPWindowRect(this).IsRectEmpty();
}
...
void CXTPToolBar::SetVisible(BOOL bVisible)
{
	...
	if (pCommandBars)
	{
		...
		pCommandBars->GetSite()->SendMessage(WM_XTP_TOOLBARVISIBLECHANGED, (WPARAM)this,
											 (LPARAM)m_bVisible);
...
LRESULT CXTPToolBar::OnSizeParent(WPARAM, LPARAM lParam)
{
	...
	if (GetStyle() & WS_VISIBLE)
	{
		...
		if (GetFlags() & xtpFlagStretched)
			dwMode |= LM_STRETCH;
		if (GetFlags() & xtpFlagHideWrap)
			dwMode |= LM_HIDEWRAP;
...
CSize CXTPCommandBar::GetIconSize() const
{
	...
	if (!pCommandBars)
		return XTPSystemMetrics()->GetSmallIconSize();
...
CSize CXTPCommandBar::GetLargeIconSize(BOOL bAutoSize) const
{
	...
	if (!pCommandBars)
		return m_szIcons != CSize(0) ? CSize(2 * m_szIcons.cx, 2 * m_szIcons.cy)
									 : XTPSystemMetrics()->GetIconSize();

	CSize sz = m_szIcons != CSize(0) ? m_szIcons
									 : (pCommandBars->m_pOptions->szIcons != CSize(0)
											? pCommandBars->m_pOptions->szIcons
											: XTPSystemMetrics()->GetSmallIconSize());

	if (m_szButtons == CSize(0))
	{
		sz = bAutoSize ? GetAutoIconSize(TRUE)
					   : (pCommandBars->m_pOptions->szLargeIcons != CSize(0)
							  ? pCommandBars->m_pOptions->szLargeIcons
							  : CSize(sz.cx * 2, sz.cy * 2));
	}
...
int CXTPToolBar::_FindNearest(CXTPControls* pControls, CXTPControl* pFind, int nPos,
							  BOOL /*bVisible*/) const
{
	if (pFind == NULL)
		return -1;
...
void CXTPToolBar::Reset(BOOL bShowWarningMessage)
{
	if (m_bBuiltIn && m_nBarID != 0 && IsCustomizable())
	{
		if (bShowWarningMessage)
		{
			...
			if (XTPResourceManager()->ShowMessageBox(strMessage, MB_ICONWARNING | MB_OKCANCEL)
				!= IDOK)
				return;
		}
...
void CXTPToolBar::BuildCustomizePopup(CXTPCommandBar* pExpandBar)
{
	...
	CXTPControls* pOriginalControls = m_pControls->m_pOriginalControls
										  ? m_pControls->m_pOriginalControls->Duplicate(FALSE)
										  : new CXTPControls();
	...
	while (nToolBar < GetControlCount())
	{
		if ((GetControl(nToolBar)->GetHideFlags() & xtpHideCustomize) != 0)
		{
			...
			if (nPopuped != -1)
				m_nPopuped = nPopuped + (nToolBar < nPopuped ? -1 : 0);
			if (nSelected != -1)
				m_nSelected = nSelected + (nToolBar < nSelected ? -1 : 0);
		}
		else
			nToolBar++;
	}
	...
	{
		CXTPControl* pOriginal = nOriginal < pOriginalControls->GetCount()
									 ? pOriginalControls->GetAt(nOriginal)
									 : NULL;
		...
		{
			if (nNearest == nToolBar)
				nOriginal++;
			else if (nNearestOriginal != -1)
				pOriginalControls->Remove(nNearestOriginal);
			nToolBar++;
		}
		ASSERT(pControl);
		...
		if ((pControl->GetHideFlags() & (xtpHideGeneric | xtpHideDocTemplate | xtpHideRibbonTab))
			!= 0)
		{
			continue;
		}
		...
		CControlButtonCustomize* pButton = (CControlButtonCustomize*)pExpandBar->GetControls()->Add(
			new CControlButtonCustomize(), 0);
		...
		if ((controlType == xtpControlPopup) || (controlType == xtpControlButtonPopup)
			|| (controlType == xtpControlSplitButtonPopup) || (controlType == xtpControlButton))
	...
	{
		CControlButtonCustomize* pButton = (CControlButtonCustomize*)pExpandBar->GetControls()->Add(
			new CControlButtonCustomize(), XTP_ID_CUSTOMIZE_RESET);
...
void CXTPToolBar::OnSetPreviewMode(BOOL bPreview)
{
	...
	else if (m_bPreviewVisible)
	{
		...
		if (GetType() == xtpBarTypeMenuBar)
		{
			...
			if (pFrameWnd && pFrameWnd->GetSafeHwnd())
				pFrameWnd->OnUpdateFrameMenu(0);
		}
	}
}
...
{
	if (!m_hwndFocus && pOldWnd)
		m_hwndFocus = pOldWnd->GetSafeHwnd();
...
BOOL AFX_CDECL CXTPToolBar::DoUpdateCustomizeMenuControl(CXTPControl* pControl, CCmdTarget* pTarget)
{
	...
	if (0 == pControl->GetID())
	{
		...
		if (NULL != pButton)
		{
			...
			bHandled				= pTarget->OnCmdMsg(XTPToUInt(pButton->m_pControl->GetID()),
											XTP_CN_UPDATE_CUSTOMIZE_COMMAND_UI, controls, NULL);
		}
	}
