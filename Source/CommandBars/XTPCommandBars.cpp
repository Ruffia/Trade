...
const UINT CXTPCommandBars::m_nRedrawMsg = ::RegisterWindowMessage(
	_T("CXTPCommandBars::m_nRedrawMsg"));
...
CXTPCommandBar* CXTPCommandBarsContextMenus::Add(UINT nIDBar, UINT nIDMenuResource, int nSubMenu)
{
	...
	CMenu menu;
	if (!XTPResourceManager()->LoadMenu(&menu, nIDMenuResource))
		return pPopupBar;
	...
	CString strCaption;
	XTPResourceManager()->GetMenuLocaleString(&menu, XTPToUIntChecked(nSubMenu), strCaption,
											  MF_BYPOSITION);
...
CXTPCommandBars::~CXTPCommandBars()
{
	...
	if (m_pRecentFileList)
		m_pRecentFileList->InternalRelease();
#endif
...
CXTPMenuBar* CXTPCommandBars::SetMenu(LPCTSTR strName, UINT nID)
{
	...
	if (!pMenuBar->SetPosition(xtpBarTop) || !pMenuBar->LoadMenuBar(nID))
	{
		delete pMenuBar;
		return NULL;
	}
...
CXTPToolBar* CXTPCommandBars::Add(LPCTSTR strName, XTPBarPosition nBarPosition,
								  CRuntimeClass* pToolbarClass)
{
	if (!pToolbarClass)
...
CXTPToolBar* CXTPCommandBars::Add(CXTPToolBar* pCommandBar, LPCTSTR strName,
								  XTPBarPosition nBarPosition)
{
	ASSERT(pCommandBar);
	if (!pCommandBar)
		return NULL;
...
CXTPDockBar* CXTPCommandBars::CanDock(CPoint pt, CXTPDockBar* pMainDock) const
{
	...
	if (pMainDock)
	{
		...
		if (pMainDock->m_dwStyle & CBRS_ORIENT_HORZ && rc.top - XTP_DPI_Y(20) < pt.y
			&& rc.bottom + XTP_DPI_Y(20) > pt.y && rc.left - XTP_DPI_X(20) < pt.x
			&& rc.right + XTP_DPI_X(20) > pt.x)
			return pMainDock;
		if (pMainDock->m_dwStyle & CBRS_ORIENT_VERT && rc.left - XTP_DPI_X(20) < pt.x
			&& rc.right + XTP_DPI_X(20) > pt.x && rc.top - XTP_DPI_Y(20) < pt.y
			&& rc.bottom + XTP_DPI_Y(20) > pt.y)
			return pMainDock;
	}
	for (int i = 0; i < 4; i++)
	{
		m_pDocks[i]->GetWindowRect(&rc);
		if (m_pDocks[i]->m_dwStyle & CBRS_ORIENT_HORZ && rc.top - XTP_DPI_Y(20) < pt.y
			&& rc.bottom + XTP_DPI_Y(20) > pt.y && rc.left - XTP_DPI_X(20) < pt.x
			&& rc.right + XTP_DPI_X(20) > pt.x)
			return m_pDocks[i];
		if (m_pDocks[i]->m_dwStyle & CBRS_ORIENT_VERT && rc.left - XTP_DPI_X(20) < pt.x
			&& rc.right + XTP_DPI_X(20) > pt.x && rc.top - XTP_DPI_Y(20) < pt.y
			&& rc.bottom + XTP_DPI_Y(20) > pt.y)
			return m_pDocks[i];
	}
...
void CXTPCommandBars::RedrawCommandBars()
{
	...
	if (pMDIFrame)
	{
		CXTPTabClientWnd* pClient = DYNAMIC_DOWNCAST(CXTPTabClientWnd,
													 CWnd::FromHandle(pMDIFrame->m_hWndMDIClient));
		if (pClient)
			pClient->Refresh();
	}
...
CXTPCommandBarKeyboardTip::CXTPCommandBarKeyboardTip(CXTPCommandBars* pCommandBars,
													 CCmdTarget* pOwner, LPCTSTR lpszCaption,
													 CPoint pt, DWORD dwAlign, BOOL bEnabled)
{
	m_pOwner	   = pOwner;
...
{
	CXTPCommandBars* pCommandBars = GetCommandBars();
	if (pCommandBars && pCommandBars->m_keyboardTips.pBar == this
		&& pCommandBars->m_keyboardTips.nLevel > 0)
...
void CXTPCommandBar::CreateKeyboardTips()
{
	...
	if (GetParentCommandBar() == 0)
	{
		for (int j = 0; j < pCommandBars->GetCount(); j++)
		{
			...
			CXTPCommandBarKeyboardTip* pWnd = new CXTPCommandBarKeyboardTip(pCommandBars, pToolBar,
																			pToolBar->GetTitle(),
																			pt, DT_TOP | DT_LEFT,
																			TRUE);
...
void CXTPCommandBars::ShowKeyboardTips(CXTPCommandBar* pCommandBar, int nLevel)
{
	...
	for (i = (int)m_keyboardTips.arr.GetSize() - 1; i >= 0; i--)
	{
		...
		if (pWnd->m_strPrefix.IsEmpty())
		{
			...
			pWnd->m_strTip = pWnd->m_strPrefix;
	}
	...
	for (i = 0; i < nCount; i++)
	{
		...
		sz		 = CSize(max(sz.cx + XTP_DPI_X(6), XTP_DPI_X(16)),
					 max(sz.cy + XTP_DPI_Y(2), XTP_DPI_Y(15)));

		CPoint pt = pWnd->m_pt;
		if (DYNAMIC_DOWNCAST(CWnd, pWnd->m_pOwner) == 0)
			pCommandBar->ClientToScreen(&pt);

		CRect rc(CPoint(pt.x
							- (pWnd->m_dwAlign & DT_CENTER
								   ? sz.cx / 2
								   : bLayoutRTL || (pWnd->m_dwAlign & DT_RIGHT) ? sz.cx : 0),
						pt.y
							- (pWnd->m_dwAlign & DT_VCENTER
								   ? sz.cy / 2
								   : pWnd->m_dwAlign & DT_BOTTOM ? sz.cy : 0)),
				 sz);

		BOOL bLayered = pPaintManager->m_pfnSetLayeredWindowAttributes && !pWnd->m_bEnabled;

		pWnd->CreateEx(DWORD((bLayered ? WS_EX_LAYERED : 0) | WS_EX_TOOLWINDOW),
					   AfxRegisterWndClass(CS_SAVEBITS), 0, WS_POPUP, rc, pCommandBar, 0);

		pWnd->SetWindowPos(&CWnd::wndTop, rc.left, rc.top, rc.Width(), rc.Height(),
		...
		if (bLayered)
		{
			((PFNSETLAYEREDWINDOWATTRIBUTES)pPaintManager->m_pfnSetLayeredWindowAttributes)(
				pWnd->m_hWnd, 0x00, 150, LWA_ALPHA);
		}
	}
...
void CXTPCommandBars::OnTrackingModeChanged(CXTPCommandBar* pCommandBar, int bMode)
{
	...
	if (m_keyboardTips.nLevel > 0 && m_pOptions->bShowKeyboardTips)
	{
		...
		{
			CXTPCommandBar* pCommandBarTop = pMouseManager->GetTrackArray().GetAt(
				pMouseManager->GetTrackArray().GetSize() - 1);
...
CXTPControl* CXTPCommandBars::_GetNextControl(CXTPControls*& pControls, int nIndex) const
{
	...
	if (m_pOptions->keyboardCuesUse & xtpKeyboardCuesUseAll && nNextIndex <= nIndex)
	{
...
CXTPControl* CXTPCommandBars::FindAccel(CXTPCommandBar* pCommandBar, UINT chAccel,
										BOOL& bSelectOnly) const
{
	...
	do
	{
		...
		CString strCaption = pControl->IsCaptionVisible() ? pControl->GetCaption() : CString();
		if (!strCaption.IsEmpty() && pControl->GetEnabled() && pControl->GetRibbonGroup() == NULL)
		{
			...
			if (CXTPShortcutManager::CompareAccelKey(strCaption[bFound ? nAmpIndex + 1 : 0],
													 chAccel)
				&& (bFound
					|| ((m_pOptions->keyboardCuesUse & xtpKeyboardCuesUseAmpersandOnly) == 0)))
			{
				if (bFound)
				{
				...
				else if (pControl->GetParent()->GetType() == xtpBarTypeMenuBar
						 || pControl->GetParent()->IsTrackingMode())
				{
					if (pCondidate == NULL)
						pCondidate = pControl;
					else
...
BOOL CXTPCommandBars::OnFrameAccel(UINT chAccel)
{
	...
	for (int i = 0; i < GetCount(); i++)
	{
		...
		if (m_pOptions->keyboardCuesUse & xtpKeyboardCuesUseAll && pCommandBar == NULL
			&& IsToolBarVisible(pToolBar))
			pCommandBar = pToolBar;
	}
	if (!pCommandBar)
		return FALSE;
...
BOOL CXTPCommandBars::PreTranslateFrameMessage(MSG* pMsg)
{
	...
	if ((pMsg->message == WM_SYSKEYUP || pMsg->message == WM_KEYUP) && m_bUseKeyboardCues
		&& m_bKeyboardCuesVisible && !GetMouseManager()->IsTrackedLock(0))
	{
		ShowKeyboardCues(FALSE);
	}
	...
	if (pMsg->message == WM_MOUSEWHEEL)
	{
		...
		{
			return XTPKeyboardManager()->ProcessKeyboardHooks(WM_MOUSEWHEEL, pMsg->wParam,
															  pMsg->lParam);
		}
		else
		{
	...
	if (hDesktop)
	{
		...
		else
		{
			if (GetLastError() == ERROR_INSUFFICIENT_BUFFER)
			{
				...
				if (::GetUserObjectInformation(hDesktop, UOI_NAME, szNameDynamic,
											   uCharsRequired * sizeof(TCHAR), &dwLen))
				{
...
BOOL CXTPCommandBars::OnFrameWndMsg(UINT message, WPARAM wParam, LPARAM lParam, LRESULT* pResult)
{
	switch (message)
	{
		...
		case WM_SETTINGCHANGE:
		{
			...
			// Problem: when the workstation is locked a WM_SETTINGCHANGE message is sent by the
			// operating system. RedrawCommandBars is called which causes a full recalculation of
			// the layout. When the window is redrawn GetClientRect() always returns 0,0,0,0 when
			// the workstation is locked. Solution: IsWorkstationLocked function Works on Windows XP
			// SP3 x86 & Vista x86 Seems like Windows 7 is not sending WM_SETTINGCHANGE on
			// workstation lock
		...
		case WM_COMMAND:
		{
			...
			if (pWnd && pWnd->IsKindOf(RUNTIME_CLASS(CXTPCommandBarEditCtrl))
				&& ((CXTPCommandBarEditCtrl*)pWnd)->OnCommand(wParam, lParam))
			{
				if (pResult != NULL)
					*pResult = 1;
				return TRUE;
			...
			if (LOWORD(wParam) == ID_HELP && GetHelpTrackingId() > 0)
			{
				...
				{
					GetSite()->SendMessage(WM_COMMANDHELP, 0,
										   XTPToLPARAM(HID_BASE_COMMAND + GetHelpTrackingId()));
					return TRUE;
				}
			}
		...
		case WM_SYSCOMMAND:
		{
			if ((wParam == 0 || LOWORD(wParam) != 0) && (wParam & 0xFFF0) != SC_KEYMENU
				&& !IsCustomizeMode())
				ClosePopups();
		...
		case WM_ACTIVATE:
		{
			...
			if (NULL != pRibbonBar && pRibbonBar->IsBackstageViewVisible())
				return TRUE;
...
CXTPControl* CXTPCommandBars::FindControl(XTPControlType type, UINT nId, BOOL bVisible,
										  BOOL bRecursive) const
{
	for (int i = GetCount() - 1; i >= 0; i--)
	{
		...
		{
			CXTPControl* pControl = pBar->GetControls()->FindControl(type, XTPToInt(nId), bVisible,
																	 bRecursive);
			if (pControl != NULL)
				return pControl;
		}
...
BOOL CXTPCommandBars::TrackPopupMenu(CXTPPopupBar* pPopup, UINT nFlags, int x, int y, CWnd* pWnd,
									 LPCRECT rcExclude, CWnd* pWndOwner)
{
	ASSERT_VALID(pPopup);
...
BOOL CXTPCommandBars::TrackPopupMenuEx(CMenu* pMenu, UINT nFlags, int x, int y, CWnd* pWndOwner,
									   LPTPMPARAMS lptpm)
{
...
BOOL CXTPCommandBars::TrackPopupMenu(CMenu* pMenu, UINT nFlags, int x, int y, CWnd* pWnd,
									 LPCRECT rcExclude, CWnd* pWndOwner,
...
void CXTPCommandBars::GetHiddenControls(CXTPToolBar* pToolBar, CXTPCommandBar* pExpandBar)
{
	...
	for (int i = 0; i < arrBars.GetSize(); i++)
	{
		...
		if (m_pOptions->bShowPopupButtonsSelectedToolbarOnly)
		{
			if (pToolBarRow == pToolBar)
			{
...
{
	CXTPControlPopup* pButton = (CXTPControlPopup*)pExpandBar->GetControls()->Add(xtpControlPopup,
																				  0);
...
void CXTPCommandBars::GetAddOrRemovePopup(CXTPToolBar* pToolBar, CXTPCommandBar* pExpandBar)
{
	...
	{
		CXTPControl* pControlCustomize = pExpandBar->GetControls()->Add(xtpControlButton,
																		XTP_ID_CUSTOMIZE);
...
BOOL CXTPCommandBars::IsControlHidden(CXTPControl* pControl)
{
	...
	if (m_pOptions->bAutoHideUnusedPopups && !bCommandHidden
		&& IsPopupControlType(pControl->GetType()))
	{
...
void CXTPCommandBars::SetPaintManager(CXTPPaintManager* pPaintManager)
{
	...
	{
		m_pFrameHook->OnBeforeUpdatePaintManager();
	}
	...
	if (NULL != m_pFrameHook)
	{
		m_pFrameHook->OnUpdatePaintManager(m_pPaintManager);
	}
...
{
	return (NULL != m_pImageManager ? m_pImageManager : XTPImageManager());
}

void CXTPCommandBars::LoadDesignerBars(CArchive& ar, UINT nSchemaVersion /*= _XTP_SCHEMA_CURRENT*/)
{
	CXTPPropExchangeArchive px(ar);
	px.SetSchema(nSchemaVersion);
	LoadDesignerBars(&px);
}
...
{
	_XTP_USES_PROTECTED_ACCESS(CXTPCommandBars, CMDIChildWnd, HMENU, m_hMenuShared)

	CMultiDocTemplate* pTemplate = DYNAMIC_DOWNCAST(CMultiDocTemplate,
													pChild->GetActiveDocument()->GetDocTemplate());
	if (!pTemplate)
		return NULL;

	HMENU hMenuShared = _XTP_PROTECTED_ACCESS(CMDIChildWnd, pChild, m_hMenuShared);
...
#	pragma warning(disable : 4610)
	...
	_XTP_USES_PROTECTED_ACCESS_ARGS(CXTPCommandBars, CDocTemplate,
									(UINT nIDResource, CRuntimeClass * pDocClass,
									 CRuntimeClass * pFrameClass, CRuntimeClass * pViewClass),
									(nIDResource, pDocClass, pFrameClass, pViewClass), UINT,
									m_nIDResource)
	_XTP_USES_PROTECTED_ACCESS(CXTPCommandBars, CFrameWnd, UINT, m_nIDHelp)
	...
	if (pChild)
	{
		if (pChild->GetActiveDocument())
		{
			...
			if (!pDocTemplate)
				return 0;
			nIDResource = _XTP_PROTECTED_ACCESS(CDocTemplate, pDocTemplate, m_nIDResource);
		}
		else
		{
			nIDResource = _XTP_PROTECTED_ACCESS(CFrameWnd, pChild, m_nIDHelp);
		}
	}
#else
	UINT nIDResource = 1;
...
BOOL CXTPCommandBars::SetToolBarFocus(CXTPToolBar* pCommandBar, BOOL bNext /*= FALSE*/)
{
	...
	pCommandBar->SetTrackingMode(TRUE, FALSE, TRUE);
	pCommandBar->SetSelected(pCommandBar->GetControls()->GetNext(-1, +1, TRUE,
																 pCommandBar->GetType()
																	 == xtpBarTypeMenuBar),
							 TRUE_KEYBOARD_NEXT);
...
void CXTPCommandBars::EnableFrameTheme(BOOL bEnable)
{
	...
	else if (NULL != m_pFrameHook)
	{
		m_pFrameHook->EnableShadow();
	}
...
void CXTPCommandBars::SetFontHeight(int nFontHeight)
{
	GetPaintManager()->SetFontHeight(nFontHeight);
	RedrawCommandBars();
}

int CXTPCommandBars::GetFontHeight() const
{
	return GetPaintManager()->GetFontHeight();
}

void CXTPCommandBars::OnAmbientPropertyChanged(CXTPApplication* pApplication,
											   XTPApplicationAmbientProperty nProperty)
{
	UNREFERENCED_PARAMETER(pApplication);

	if (NULL != m_pFrame && xtpApplicationAccentColor == nProperty
		|| xtpApplicationStateColor == nProperty)
		m_pFrame->PostMessage(m_nRedrawMsg);
}
...
BEGIN_DISPATCH_MAP(CXTPCommandBarsContextMenus, CXTPCmdTarget)
	...
	DISP_FUNCTION_ID(CXTPCommandBarsContextMenus, "Item", DISPID_VALUE, OleItem, VT_DISPATCH,
					 VTS_I4)
	DISP_FUNCTION_ID(CXTPCommandBarsContextMenus, "_NewEnum", DISPID_NEWENUM, OleNewEnum,
					 VT_UNKNOWN, VTS_NONE)
	...
	DISP_FUNCTION_ID(CXTPCommandBarsContextMenus, "AddPopup", 6, OleAddPopup, VT_EMPTY,
					 VTS_DISPATCH)
END_DISPATCH_MAP()
