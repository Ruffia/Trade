...
void CXTPCommandBarAnimation::AlphaBlendU(PBYTE pDest, PBYTE pSrcBack, int cx, int cy, PBYTE pSrc,
										  BYTE byAlpha)
{
...
void CXTPCommandBarAnimation::AnimateRect(CClientDC& dc, CAnimateInfo* pai)
{
	...
	if (pai->m_nAnimation == MAXANIMATION)
	{
		...
		dc.BitBlt(rcPaint.left, rcPaint.top, cx, cy, &memDC, rcPaint.left, rcPaint.top, SRCCOPY);
	...
	else
	{
		...
		{
			AlphaBlendU(pDestBits, pai->pSrcBackBits, cx, cy, pai->pSrcBits,
						(BYTE)(255 * pai->m_nAnimation / MAXANIMATION));
...
void CXTPCommandBarAnimation::RedrawRect(LPCRECT lpRect, BOOL bAnimate)
{
	...
	if (!bAnimate)
	{
		...
		dcCache.BitBlt(rcPaint.left, rcPaint.top, rcPaint.Width(), rcPaint.Height(), &dcDraw,
					   rcPaint.left, rcPaint.top, SRCCOPY);

		dc.BitBlt(rcPaint.left, rcPaint.top, rcPaint.Width(), rcPaint.Height(), &dcDraw,
				  rcPaint.left, rcPaint.top, SRCCOPY);
	}
	else
	{
		...
		pai->rcPaint	  = rcPaint;
		pai->m_nAnimation = 1;
		pai->hbmSrc = CXTPImageManager::Create32BPPDIBSection(dc, rcPaint.Width(), rcPaint.Height(),
															  &pai->pSrcBits);
		pai->hbmSrcBack = CXTPImageManager::Create32BPPDIBSection(dc, rcPaint.Width(),
																  rcPaint.Height(),
																  &pai->pSrcBackBits);

		if (NULL != pai->hbmSrc && NULL != pai->hbmSrcBack && NULL != pai->pSrcBits
			&& NULL != pai->pSrcBackBits)
		{
			CDC dcSrc;
			...
			dcSrc.BitBlt(0, 0, rcPaint.Width(), rcPaint.Height(), &dcDraw, rcPaint.left,
						 rcPaint.top, SRCCOPY);

			dcSrcBack.BitBlt(0, 0, rcPaint.Width(), rcPaint.Height(), &dcCache, rcPaint.left,
							 rcPaint.top, SRCCOPY);

			dcCache.BitBlt(rcPaint.left, rcPaint.top, rcPaint.Width(), rcPaint.Height(), &dcSrc, 0,
						   0, SRCCOPY);

			::SelectObject(dcSrcBack, hOldBitmapSrcBack);
			::SelectObject(dcSrc, hOldBitmapSrc);

			if (memcmp(pai->pSrcBits, pai->pSrcBackBits,
					   XTPToSizeTChecked(rcPaint.Width() * rcPaint.Height() * 4))
				== 0)
			{
				delete pai;
			}
...
void CXTPCommandBarAnimation::OnPaint(CPaintDC& paintDC)
{
	...
	else if (m_bInvalidate || m_bmpCache.GetSafeHandle() == 0)
	{
		...
		paintDC.BitBlt(rcPaint.left, rcPaint.top, rcPaint.Width(), rcPaint.Height(), &memDC,
					   rcPaint.left, rcPaint.top, SRCCOPY);
