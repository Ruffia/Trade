...
void CXTPMiniToolBar::UpdateOpacity()
{
	...
	{
		if (nLen < XTP_DPI_X(17))
			nOpacity = XTP_DPI_X(17) + (XTP_DPI_X(16) - nLen) * 14;
	}
	else
	{
		if (nLen < XTP_DPI_X(90))
			nOpacity = MulDiv(255, XTP_DPI_X(90) - nLen, XTP_DPI_X(90));
	}
	...
	if ((m_nOpacity != nOpacity))
	{
		...
		else
		{
			SetWindowPos(0, 0, 0, 0, 0,
						 UINT(SWP_NOMOVE | SWP_NOSIZE | SWP_NOZORDER | SWP_NOOWNERZORDER
							  | (nOpacity > 20 ? SWP_SHOWWINDOW : SWP_HIDEWINDOW)));
		}
		...
		while (pos)
		{
			...
			{
				pShadow->SetWindowPos(0, 0, 0, 0, 0,
...
BOOL CXTPMiniToolBar::TrackPopupMenu(CXTPPopupBar* pPopup, UINT nFlags, int x, int y)
{
	...
	else if ((y + szPopupBar.cy)
			 >= rcWorkArea.bottom) // the menu popup goes up, we're in the bottom of the screen
	{
		nTop = rc.bottom + uMenuAndToolbarIdent + szMiniBar.cy; // minibar will be below popup

		if (nTop
			>= rcWorkArea.bottom) // the minibar is too tall to fit below, we need to place it above
		{
			nTop = y - szPopupBar.cy - uMenuAndToolbarIdent;
		}
	}
	else if ((nTop - szMiniBar.cy)
			 <= rcWorkArea.top) // the minibar is too tall to fit above, we need to place it below
	{
...
int CXTPMiniToolBar::OnHookMessage(HWND hWnd, UINT nMessage, WPARAM& wParam, LPARAM& lParam,
								   LRESULT& lResult)
{
	if (m_bTracking)
	{
		...
		if (::PeekMessage(&msg, NULL, 0, 0, PM_NOREMOVE))
		{
			...
			if (!m_pContextMenu)
			{
				...
				if ((msg.message == WM_KEYDOWN || msg.message == WM_CHAR
					 || msg.message == WM_SYSKEYDOWN || msg.message == WM_MOUSEWHEEL)
					&& !IsTrackingMode())
				{
					...
					if (IsCustomControlKeyboardInteraction())
					{
						...
						for (i = 0; i < c; i++)
						{
							CXTPControlCustom* pCustom = DYNAMIC_DOWNCAST(CXTPControlCustom,
																		  GetControl(i));
...
BEGIN_DISPATCH_MAP(CXTPMiniToolBar, CXTPPopupToolBar)
	DISP_FUNCTION_ID(CXTPMiniToolBar, "PopupContextMenu", 105, OlePopupContextMenu, VT_I4,
					 VTS_DISPATCH VTS_VARIANT VTS_VARIANT VTS_VARIANT)
	DISP_FUNCTION_ID(CXTPMiniToolBar, "PopupMiniToolBar", 106, OlePopupMiniToolBar, VT_I4,
					 VTS_VARIANT VTS_VARIANT VTS_VARIANT)

	DISP_PROPERTY_EX_ID(CXTPMiniToolBar, "CustomControlKeyboardInteraction", 302,
						IsCustomControlKeyboardInteraction, SetCustomControlKeyboardInteraction,
						VT_BOOL)
...
long CXTPMiniToolBar::OlePopupContextMenu(LPDISPATCH lpDispatch, const VARIANT& varFlags,
...
long CXTPMiniToolBar::OlePopupMiniToolBar(const VARIANT& varFlags, const VARIANT& x,
										  const VARIANT& y)
{
