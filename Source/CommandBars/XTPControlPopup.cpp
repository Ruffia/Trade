...
void CXTPControlPopup::OnLButtonUp(CPoint point)
{
	...
	if (GetEnabled() && GetType() == xtpControlSplitButtonPopup
		&& m_pParent->GetType() == xtpBarTypePopup)
	{
		if (m_rcControl.right > point.x + GetPaintManager()->m_nSplitButtonPopupWidth
			&& GetEnabled() != TRUE_SPLITDROPDOWN)
			OnExecute();
	}
}
...
BOOL CXTPControlPopup::OnSetSelected(int bSelected)
{
	...
	if (bSelected == 1 && m_pParent->GetSafeHwnd() && GetEnabled()
		&& GetType() == xtpControlSplitButtonPopup)
	{
		CPoint point;
		GetCursorPos(&point);
	...
	{
		if (GetEnabled() && bSelected && m_pParent->IsTrackingMode() == TRUE_POPUP
			&& GetParent()->GetPosition() != xtpBarPopup)
		{
			m_pParent->SetPopuped(m_nIndex);
		}
	...
	{
		if (GetEnabled() && IsKeyboardSelected(bSelected)
			&& m_pParent->IsTrackingMode() == TRUE_POPUP)
		{
			m_pParent->SetPopuped(m_nIndex);
		}
	}
...
{
	if (m_pCommandBar && m_pParent->GetPosition() != xtpBarPopup
		&& IsPopupControlType(m_controlType))
	{
		...
		if (pPaintManager && pPopupBar && pPopupBar->m_bShowShadow && m_bShowShadow
			&& pPaintManager->m_bShowShadow && !m_pParent->IsCustomizeMode())
		{
			CRect rc = GetRect();
...
{
	if (m_pCommandBar && GetEnabled()
		&& (m_pParent->IsTrackingMode() == TRUE_POPUP || GetParent()->GetPosition() == xtpBarPopup))
		GetParent()->SetPopuped(m_nIndex);
	else
		CXTPControlButton::OnMouseHover();
}
	...
	if (this != pControl)
	{
		...
		if (bRecursive && ((CXTPControlPopup*)pControl)->m_pCommandBar)
		{
			CXTPCommandBar* pCommandBar =
				((CXTPControlPopup*)pControl)->m_pCommandBar->Clone(bRecursive);
...
{
	if (!m_pCommandBar)
		m_pCommandBar = CXTPPopupBar::CreatePopupBar(m_pParent ? m_pParent->GetCommandBars()
															   : NULL);

	m_pCommandBar->LoadMenu(pMenu);
}

BOOL CXTPControlPopup::ContainsPopup(CXTPControlPopup* pControlPopup)
{
	...
	{
		CXTPControlPopup* pControlChild = DYNAMIC_DOWNCAST(CXTPControlPopup,
														   m_pCommandBar->GetControl(i));
		if (pControlChild)
		{
			if (pControlPopup->ContainsPopup(pControlChild))
				return TRUE;

			if (pControlChild->ContainsPopup(pControlPopup))
				return TRUE;
		}
	}
	return FALSE;
...
BOOL CXTPControlPopup::DoCustomDragOver(CXTPCommandBar* pCommandBar, CPoint /*point*/,
...
void CXTPControlPopup::OnCustomizeDragOver(CXTPControl* pDataObject, CPoint /*point*/,
										   DROPEFFECT& /*dropEffect*/)
{
	if (m_pCommandBar && m_pCommandBar->IsCustomizable())
	{
		...
		if (!(pControlPopup && pControlPopup->ContainsPopup(this)))
		{
			m_pParent->SetPopuped(m_nIndex);
		}
		else
...
BEGIN_DISPATCH_MAP(CXTPControlPopup, CXTPControlButton)
	DISP_PROPERTY_EX_ID(CXTPControlPopup, "CommandBar", dispidCommandBar, OleGetCommandBar,
						OleSetCommandBar, VT_DISPATCH)
END_DISPATCH_MAP()
