...
CXTPRecentFileList::CXTPRecentFileList(UINT nStart, LPCTSTR lpszSection, LPCTSTR lpszEntryFormat,
									   int nSize, int nMaxDispLen)
...
void CXTPRecentFileList::ReadList()
{
	...
	for (int iMRU = 0; iMRU < m_nSize; iMRU++)
	{
		...
		if (!m_arrNames[iMRU].IsEmpty())
		{
			...
			wsprintf(pszEntry, _T("Pinned%d"), iMRU + 1);
			m_pItems[iMRU]->m_bPinned = (0
										 != pApp->GetProfileInt(m_strSectionName, pszEntry, FALSE));

			wsprintf(pszEntry, _T("IconId%d"), iMRU + 1);
			m_pItems[iMRU]->m_nIconId = XTPToInt(
				pApp->GetProfileInt(m_strSectionName, pszEntry, -1));

			wsprintf(pszEntry, _T("Caption%d"), iMRU + 1);
			m_pItems[iMRU]->m_strCaption = pApp->GetProfileString(m_strSectionName, pszEntry,
																  _T(""));
...
BEGIN_DISPATCH_MAP(CXTPRecentFileListItem, CXTPCmdTarget)
	...
	DISP_PROPERTY_EX_ID(CXTPRecentFileListItem, "PathName", DISPID_VALUE, OleGetPathName,
...
BEGIN_DISPATCH_MAP(CXTPRecentFileList, CXTPCmdTarget)
	...
	DISP_FUNCTION_ID(CXTPRecentFileList, "_NewEnum", DISPID_NEWENUM, OleNewEnum, VT_UNKNOWN,
					 VTS_NONE)
