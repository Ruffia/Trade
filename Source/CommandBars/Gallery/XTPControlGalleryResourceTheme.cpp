...
void CXTPControlGalleryResourceTheme::FillControl(CDC* pDC, CXTPControlGallery* pGallery,
												  CRect rcControl)
{
	if (!pGallery->IsTransparent())
	{
		pDC->FillSolidRect(rcControl, (pGallery->GetParent()->GetPosition() == xtpBarPopup
									   && pGallery->GetParent()->GetType() == xtpBarTypePopup)
										  ? m_pPaintManager->GetXtremeColor(XPCOLOR_MENUBAR_FACE)
										  : (pGallery->GetSelected() && pGallery->GetEnabled())
												? m_clrControlGallerySelected
												: m_clrControlGalleryNormal);
	}
...
void CXTPControlGalleryResourceTheme::RefreshMetrics()
{
	...
	m_clrControlGallerySelected =
		pPaintManager->GetImages()->GetImageColor(_T("Toolbar"), _T("ControlGallerySelected"));
	m_clrControlGalleryNormal =
		pPaintManager->GetImages()->GetImageColor(_T("Toolbar"), _T("ControlGalleryNormal"));
	m_clrControlGalleryBorder =
		pPaintManager->GetImages()->GetImageColor(_T("Toolbar"), _T("ControlGalleryBorder"));
}

void CXTPControlGalleryResourceTheme::DrawLabel(CDC* pDC, CXTPControlGalleryItem* pLabel,
												CRect rcItem)
{
...
{
#define GETPARTSTATE3_R(ht, bEnabled)                                                              \
	(!bEnabled ? 4 : nPressedHt == ht ? 3 : nHotHt == ht ? 2 : nHotHt > 0 || nPressedHt > 0 ? 1 : 0)
	...
	CRect rcScroll(rcControl.right - XTP_DPI_X(15), rcControl.top, rcControl.right,
				   rcControl.bottom);
	...
	if (nHotHt == HTNOWHERE && nPressedHt == -1 && bControlEnabled
		&& IsKeyboardSelected(pGallery->GetSelected()) && pGallery->GetSelectedItem() == -1)
		nHotHt = XTP_HTSCROLLPOPUP;

	int nState = GETPARTSTATE3_R(XTP_HTSCROLLUP,
								 (bControlEnabled
	...
	nState = GETPARTSTATE3_R(XTP_HTSCROLLDOWN,
							 (bControlEnabled
							  && pGallery->IsScrollButtonEnabled(XTP_HTSCROLLDOWN)));
	...
	pImage->DrawImage(pDC, rcScrollPopup, pImage->GetSource(nState, 5), CRect(3, 3, 3, 3),
					  0xFF00FF);
}
