...
void CXTPCustomizeKeyboardPage::DoDataExchange(CDataExchange* pDX)
{
	...
	//}}AFX_DATA_MAP
	DDX_Control(pDX, XTP_IDC_BTN_ASSIGN, m_btnAssign);
	DDX_Control(pDX, XTP_IDC_BTN_REMOVE, m_btnRemove);
	DDX_Control(pDX, XTP_IDC_BTN_ACCEL_RESET, m_btnResetAll);
	DDX_Control(pDX, XTP_IDC_EDIT_SHORTCUTKEY, m_editShortcutKey);
#ifdef _XTP_INCLUDE_CONTROLS
	DDX_Control(pDX, XTP_IDC_LBOX_KEYS_EX, m_lboxKeys);
	DDX_Control(pDX, XTP_IDC_LBOX_ACCEL_COMMANDS_EX, m_lboxCommands);
#else
...
#ifdef _XTP_INCLUDE_CONTROLS
	ON_LBN_SELCHANGE(XTP_IDC_LBOX_ACCEL_COMMANDS_EX, OnSelchangeCommands)
	ON_LBN_SELCHANGE(XTP_IDC_LBOX_KEYS_EX, OnSelchangeCurKeys)
#else
...
BOOL CXTPCustomizeKeyboardPage::OnInitDialog()
{
	...
	m_lboxCommands.SetItemHeight(0, XTPToUInt(::GetSystemMetrics(SM_CYMENU)));
...
#ifdef _XTP_INCLUDE_CONTROLS
	SetResize(XTP_IDC_LBOX_ACCEL_COMMANDS_EX, XTP_ANCHOR_TOPLEFT, XTP_ANCHOR_BOTTOMCENTER);
	SetResize(XTP_IDC_LBOX_KEYS_EX, XTP_ANCHOR_TOPCENTER, XTP_ANCHOR_BOTTOMRIGHT);
#else
	...
	m_btnAssign.SetTheme(controlTheme);
	m_btnRemove.SetTheme(controlTheme);
	m_btnResetAll.SetTheme(controlTheme);
	m_lboxKeys.SetTheme(controlTheme);
	m_lboxCommands.SetTheme(controlTheme);
	m_comboCategory.SetTheme(controlTheme);
	m_editShortcutKey.SetTheme(controlTheme);

	m_clrText = XTPIniColor(_T("CommandBars.Toolbar"), _T("ButtonText"),
							GetXtremeColor(COLOR_WINDOWTEXT));

	XTP_SAFE_CALL1(GetDlgItem(XTP_IDC_LBOX_ACCEL_COMMANDS), ShowWindow(SW_HIDE));
	XTP_SAFE_CALL1(GetDlgItem(XTP_IDC_LBOX_KEYS), ShowWindow(SW_HIDE));
#else
	XTP_SAFE_CALL1(GetDlgItem(XTP_IDC_LBOX_ACCEL_COMMANDS_EX), ShowWindow(SW_HIDE));
	XTP_SAFE_CALL1(GetDlgItem(XTP_IDC_LBOX_KEYS_EX), ShowWindow(SW_HIDE));
#endif
...
void CXTPCustomizeKeyboardPage::OnAssign()
{
	...
	for (int i = 0; i < pAccelTable->GetCount(); i++)
	{
		...
		{
			if (XTPResourceManager()->ShowMessageBox(XTP_IDS_CONFIRM_REASSIGN,
...
void CXTPCustomizeKeyboardPage::OnResetAll()
{
	...
	CXTPResourceManager::AssertValid(
		XTPResourceManager()->LoadString(&strPrompt, XTP_IDS_CONFIRM_KEYBOARD));
...
void CXTPCustomizeKeyboardPage::OnRemove()
{
	...
	{
		if (m_pSheet->GetCommandBars()->GetShortcutManager()->OnBeforeRemove(pAccelTable->GetAt(j))
			== FALSE)
		{
...
HBRUSH CXTPCustomizeKeyboardPage::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
