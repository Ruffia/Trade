...
void CXTPCustomizeDropSource::PickTarget(CPoint point)
{
	...
	for (int i = (int)m_arrTargets.GetSize() - 1; i >= 0; i--)
	{
		...
		if (pCommandBar->IsVisible() && CXTPWindowRect(pCommandBar).PtInRect(point)
			&& (pCommandBar->GetSafeHwnd() == hWndPoint
				|| ::IsChild(pCommandBar->GetSafeHwnd(), hWndPoint)))
		{
			bFound = TRUE;
...
DROPEFFECT CXTPCustomizeDropSource::_DoDragDrop()
{
	...
	while (!bDone && ::GetCapture() == m_hwndCapture)
	{
		...
		switch (msg.message)
		{
			case WM_LBUTTONUP: bDone = TRUE; break;
			...
			case WM_RBUTTONDOWN: return DROPEFFECT_CANCEL;
...
CXTPNewToolbarDlg::CXTPNewToolbarDlg(CWnd* pParent, CXTPCommandBars* pCommandBars,
	...
	while (nIndex < nCount)
	{
		...
		if (pBar && m_pCommandBar != pBar && m_strToolbar.Compare(pBar->GetTitle()) == 0)
		{
			...
			CXTPResourceManager::AssertValid(
				XTPResourceManager()->LoadString(&strError, XTP_IDS_ERROR_EXISTS));
