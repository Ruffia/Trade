...
BOOL CXTPPopupBar::Create()
{
	...
	UINT nClassStyle			  = UINT(
		 (!pCommandBars || pCommandBars->GetCommandBarsOptions()->bUseSystemSaveBitsStyle)
	...
	if (!pCommandBars && GetImageManager()->IsDrawReverted() == 1)
		nLayoutRTL = WS_EX_LAYOUTRTL;

	if (!pCommandBars && pSite && pSite->GetExStyle() & WS_EX_LAYOUTRTL)
		nLayoutRTL = WS_EX_LAYOUTRTL;

	if (!CreateEx(XTPToDWORD(WS_EX_TOOLWINDOW | nLayoutRTL), _T("XTPPopupBar"), 0,
				  MFS_SYNCACTIVE | WS_POPUP | WS_CLIPCHILDREN | WS_CLIPSIBLINGS, CRect(0, 0, 0, 0),
				  pSite, 0))
		return FALSE;
...
void CXTPPopupBar::RecalcSizeLayout()
{
	if (!m_bDynamicLayout && GetSafeHwnd() && IsTrackingMode())
	{
		...
		CRect rc = !bRTL ? CRect(CPoint(rcWindow.left, rcWindow.top), sz)
						 : CRect(CPoint(rcWindow.right - sz.cx, rcWindow.top), sz);
		...
		if (bShowKeyboardTips && pCommandBars)
		...
		SetWindowPos(0, rc.left, rc.top, rc.Width(), rc.Height(),
					 SWP_SHOWWINDOW | SWP_NOACTIVATE | SWP_NOOWNERZORDER | SWP_NOZORDER);
	...
	if (!m_bDynamicLayout && GetSafeHwnd() && IsTrackingMode())
	{
		...
		if (m_bExpanding)
		{
			...
			if (IsAnimateType(animationType))
			{
				...
				CXTPDrawHelpers::SetContextRTL(&paintDC, DWORD(dwLayout ? LAYOUT_RTL : 0));
		...
		if (m_barPosition == xtpBarPopup && m_bShowShadow && pPaintManager->m_bShowShadow
			&& !IsCustomizeMode())
			pPaintManager->GetShadowManager()->SetShadow(this, m_rcExclude,
														 m_bRecursePopup ? 1 : 0);
...
CSize CXTPPopupBar::CalcDynamicLayout(int nLength, DWORD /*nMode*/)
{
	...
	if (m_barType == xtpBarTypeNormal)
	{
		...
		CSize sz = m_pControls->CalcDynamicSize(&dc, GetWidth(), LM_COMMIT | LM_HORZ | LM_POPUP,
												rcBorders);

		m_rcTearOffGripper.SetRect(rcBorders.left, rcBorders.top - szTearOffGripper.cy,
								   sz.cx - rcBorders.right, rcBorders.top);
		m_rcResizeGripper.SetRectEmpty();
	...
	if (m_bCollapsed)
	{
		szButtonExpand = GetPaintManager()->DrawSpecialControl(&dc, xtpButtonExpandMenu, NULL, this,
															   FALSE, NULL);
		rcBorders.bottom += szButtonExpand.cy;

		if (!pButtonExpand)
			pButtonExpand = m_pControls->Add(new CControlExpandButton(), XTP_ID_POPUPBAR_EXPAND,
											 _T(""), -1, TRUE);
		pButtonExpand->SetHideFlags(xtpHideGeneric);
	}
	else if (pButtonExpand)
		pButtonExpand->SetHideFlags(xtpHideGeneric);
	...
	if (m_bCollapsed)
	{
		...
		pButtonExpand->SetRect(CRect(rcBorders.left, sz.cy - rcBorders.bottom,
									 sz.cx - rcBorders.right,
									 sz.cy - rcBorders.bottom + szButtonExpand.cy));
	}

	m_rcTearOffGripper.SetRect(rcBorders.left, rcBorders.top - szTearOffGripper.cy,
							   sz.cx - rcBorders.right, rcBorders.top);
	m_rcResizeGripper.SetRect(rcBorders.left, sz.cy - rcBorders.bottom, sz.cx - rcBorders.right,
							  sz.cy - rcBorders.bottom + szResizeGripper.cy);
...
{
	RECT rcWork = m_rcExclude.IsRectEmpty() ? XTPMultiMonitor()->GetWorkArea(ptPopup)
											: XTPMultiMonitor()->GetWorkArea(m_rcExclude);
	RECT rcScreen = m_rcExclude.IsRectEmpty() ? XTPMultiMonitor()->GetScreenArea(ptPopup)
											  : XTPMultiMonitor()->GetScreenArea(m_rcExclude);
	...
	if (!m_rcExclude.IsRectEmpty() && !CRect().IntersectRect(&rcWork, m_rcExclude)
		&& CRect().IntersectRect(&rcScreen, m_rcExclude))
	{
		rcWork = rcScreen;
	}
	...
	BOOL bNoScroll = (pCommandars ? pCommandars->GetCommandBarsOptions()->bWrapLargePopups : FALSE)
					 || m_bMultiLine;
	...
	else if (ptPopup.y + sz.cy > rcWork.bottom)
	{
		...
		{
			ptPopup.y = (ptPopup.y < (rcWork.bottom - rcWork.top) / 2 ? rcWork.bottom : ptPopup.y)
						- sz.cy;
		}
		else
		{
			if (m_popupFlags & xtpPopupDown)
			{
				...
				if (ptPopup.y < rcWork.top)
				{
					if (bNoScroll)
					{
						...
						if (m_pControlPopup
							&& (m_pControlPopup->GetType() != xtpControlComboBox
								|| GetParentCommandBar()->GetType() == xtpBarTypePopup))
						{
	...
	m_scrollInfo.bScroll = bScroll
						   && (GetType() == xtpBarTypePopup && GetPosition() == xtpBarPopup);
...
void CXTPPopupBar::UpdateLocation(CSize sz)
{
	...
	SetWindowPos(&CWnd::wndTopMost, rc.left, rc.top, rc.Width(), rc.Height(),
				 UINT(SWP_NOACTIVATE | SWP_NOOWNERZORDER | SWP_SHOWWINDOW
					  | (IsWindowVisible() ? SWP_NOZORDER : 0)));
}
...
void CXTPPopupBar::AdjustScrolling(LPSIZE lpSize, BOOL bDown, BOOL bInvalidate)
{
	...
	if (m_bCollapsed)
	{
		szButtonExpand = GetPaintManager()->DrawSpecialControl(NULL, xtpButtonExpandMenu, NULL,
															   this, FALSE, NULL);
		rcBorders.bottom += szButtonExpand.cy;
	...
	if (!bDown)
	{
		...
		{
			nPos += nScrollHeight;
			m_scrollInfo.btnDown.m_rc.SetRect(rcBorders.left, rcBorders.top,
											  sz.cx - rcBorders.right, nPos);
		}
		...
		for (i = m_scrollInfo.nScrollFirst; i < nCount; i++)
		{
			...
			bFirst = FALSE;
			pControl->SetRect(
				CRect(rcBorders.left, nPos, sz.cx - rcBorders.right, nPos + nItemHeight));
		...
		if (i != nCount)
		{
			m_scrollInfo.btnUp.m_rc.SetRect(rcBorders.left,
											sz.cy - rcBorders.bottom - nScrollHeight,
											sz.cx - rcBorders.right, sz.cy - rcBorders.bottom);
		}
	}
	else
	{
		...
		for (i = m_scrollInfo.nScrollLast - 1; i >= 0; i--)
		{
			...
			pControl->SetRect(
				CRect(rcBorders.left, nPos - nItemHeight, sz.cx - rcBorders.right, nPos));
	...
	if (m_bResizable)
		m_rcResizeGripper.SetRect(rcBorders.left, sz.cy - rcBorders.bottom, sz.cx - rcBorders.right,
								  sz.cy - rcBorders.bottom + m_rcResizeGripper.Height());
...
void CXTPCommandBar::UpdateShortcuts()
{
	...
	for (int i = 0; i < GetControlCount(); i++)
	{
		...
		if (pControl->GetID() != 0)
		{
			pCommandBars->GetShortcutManager()->FindDefaultAccelerator(pControl->GetID(),
																	   strShortcut);
...
AFX_INLINE BOOL IsVerticalPosition(CXTPCommandBar* pCommandBar)
{
	return IsVerticalPosition(pCommandBar->GetPosition())
		   || (pCommandBar->GetType() == xtpBarTypePopup
			   && pCommandBar->GetPosition() == xtpBarFloating);
}
...
void CXTPPopupBar::UpdateFlags()
{
	if (m_pControlPopup)
	{
		...
		if (m_pControlPopup->m_dwFlags & xtpFlagLeftPopup || bLayoutRTL)
			m_popupFlags |= xtpPopupLeft;
		...
		{
			if (pParent->GetPosition() == xtpBarRight
				|| (pParent->GetType() == xtpBarTypePopup && pParent->IsPopupBar()
		...
		if (m_pControlPopup->GetType() == xtpControlComboBox
			&& pParent->GetType() != xtpBarTypePopup)
		{
			m_popupFlags |= xtpPopupDown;
		}
		else
			AdjustExcludeRect(m_rcExclude, IsVerticalPosition(pParent));
...
void CXTPPopupBar::Animate()
{
	...
	if (bAnimate)
	{
		...
		if (IsAnimateType(animationType))
		{
			...
			BOOL dwLayout = CXTPDrawHelpers::IsContextRTL(&paintDC);
			CXTPDrawHelpers::SetContextRTL(&paintDC, FALSE);
			paintDC.BitBlt(0, 0, rcClient.Width(), rcClient.Height(), pDC, rcWindow.left,
	...
	if (pParentCommandBar)
		pParentCommandBar->m_bAnimatePopup = FALSE;
	if (!bRegionChanged)
		GetPaintManager()->SetCommandBarRegion(this);
}
...
BOOL CXTPPopupBar::Popup(CXTPControlPopup* pControlPopup, BOOL bSelectFirst)
{
	...
	if (pPaintManager && m_barPosition == xtpBarPopup && m_bShowShadow
		&& pPaintManager->m_bShowShadow && !IsCustomizeMode())
	{
		pPaintManager->GetShadowManager()->RemoveShadow(this);
		...
		if (m_pTearOffBar == NULL)
		{
			...
			for (int i = 0; i < GetControlCount(); i++)
			{
				...
				if (pControl->GetID() != XTP_ID_POPUPBAR_EXPAND)
				{
					...
					pControlClone->SetHideFlags(pControlClone->GetHideFlags()
												& ~(xtpHideScroll | xtpHideExpand));
				}
			}
		}
		ASSERT(m_pTearOffBar);

		pCommandBars->GetSite()->SendMessage(WM_XTP_TEAROFFDONE, (WPARAM)m_pTearOffBar,
											 (LPARAM)this);
...
void CXTPPopupBar::TrackResize()
{
	...
	while (::GetCapture() == m_hWnd)
	{
		...
		else if (msg.message == WM_MOUSEMOVE && pt != msg.pt)
		{
			...
			if (bResizeWidth)
				pControl->SetWidth(max(szControl.cx, szControlMin.cx));
			if (bResizeHeight)
				pControl->SetHeight(max(szControl.cy, szControlMin.cy));
			RecalcSizeLayout();
		...
		else if (msg.message == WM_KEYDOWN)
		{
			...
			{
				if (bResizeWidth)
					pControl->SetWidth(szControlInit.cx);
				if (bResizeHeight)
					pControl->SetHeight(szControlInit.cy);
				RecalcSizeLayout();
				break;
			}
		}
...
void CXTPPopupBar::TrackTearOff()
{
	...
	while (CWnd::GetCapture() == this)
	{
		...
		else if (msg.message == WM_MOUSEMOVE && pt != msg.pt)
		{
			pt			= msg.pt;
			BOOL bState = !(
				pt.x > rcGripper.right + XTP_DPI_X(10) || pt.x < rcGripper.left - XTP_DPI_X(10)
	...
	if (m_bResizable)
	{
		...
		if (_MouseInResizeGripper(pt))
		{
			SetCursor(AfxGetApp()->LoadStandardCursor(
				m_bResizable == 2 ? IDC_SIZENS : IsLayoutRTL() ? IDC_SIZENESW : IDC_SIZENWSE));
			return TRUE;
		}
	}
...
void CXTPPopupBar::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	if (m_scrollInfo.bScroll)
	{
		if (m_scrollInfo.btnUp.OnMouseMove(point) || m_scrollInfo.btnDown.OnMouseMove(point))
		{
			SetPopuped(-1);
		}
	}

	if (m_bTearOffPopup && !IsCustomizeMode() && m_rcTearOffGripper.PtInRect(point)
		&& !m_bTearOffSelected)
	{
		SetSelected(-1);
		SetPopuped(-1);
...
void CXTPPopupBar::DrawCommandBar(CDC* pDC, CRect rcClipBox)
{
	...
	if (m_scrollInfo.bScroll)
	{
		if (!m_scrollInfo.btnUp.m_rc.IsRectEmpty())
		{
			...
			XTPPrimitiveDrawer()->DrawSymbol(pDC, xtpPrimitiveSymbolPlayDown, rc,
		...
		if (!m_scrollInfo.btnDown.m_rc.IsRectEmpty())
		{
			...
			rc.DeflateRect(XTP_DPI_X(2), XTP_DPI_Y(2), XTP_DPI_X(2), XTP_DPI_Y(2));
			XTPPrimitiveDrawer()->DrawSymbol(pDC, xtpPrimitiveSymbolPlayUp, rc,
											 GetXtremeColor(COLOR_GRAYTEXT));
		}
	}
...
BOOL CXTPPopupBar::SetSelected(int nSelected, BOOL bKeyboard)
{
	...
	if (m_nSelected != -1 && !bKeyboard
		&& (GetControl(m_nSelected)->GetID() == XTP_ID_POPUPBAR_EXPAND))
	{
		CXTPCommandBars* pCommandBars = GetCommandBars();
		ASSERT(pCommandBars);
...
{
	CXTPControl* pControl = fByPos ? m_pControls->GetAt(XTPToIntChecked(uItem))
								   : m_pControls->FindControl(xtpControlError,
															  XTPToIntChecked(uItem), TRUE, FALSE);
	if (pControl)
	{
...
UINT CXTPPopupBar::GetDefaultItem(UINT /*gmdiFlags*/, BOOL fByPos)
{
	for (int i = 0; i < GetControlCount(); i++)
	{
		...
		{
			return XTPToUInt(fByPos ? pControl->GetIndex() : pControl->GetID());
		}
	}
...
{
	CXTPPopupToolBar* pPopupBar = (CXTPPopupToolBar*)
									  CXTPCommandBars::m_pPopupToolBarClass->CreateObject();
	pPopupBar->SetCommandBars(pCommandBars);
	return pPopupBar;
}
