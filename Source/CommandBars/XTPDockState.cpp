...
void CXTPToolBar::CToolBarInfo::DoPropExchange(CXTPPropExchange* pPX, CXTPDockState* pDockState)
{
	...
	if (pDockState->GetVersion() > _XTP_SCHEMA_87)
	{
		...
		if (m_bDialogBar)
		{
			PX_DPI_Size(pPX, _T("DialogBarDockingClientSize"), m_szDialogBarDockingClient,
						CSize(0, 0));
			PX_DPI_Size(pPX, _T("DialogBarFloatingClientSize"), m_szDialogBarFloatingClient,
						CSize(0, 0));
		}
	}
}
...
BOOL CXTPToolBar::CToolBarInfo::SaveState(LPCTSTR lpszProfileName, int nIndex)
{
	...
	if (m_bDialogBar)
	{
		pApp->WriteProfileInt(szSection, _T("DialogBarDockingClientCx"),
							  m_szDialogBarDockingClient.cx);
		pApp->WriteProfileInt(szSection, _T("DialogBarDockingClientCy"),
							  m_szDialogBarDockingClient.cy);
		pApp->WriteProfileInt(szSection, _T("DialogBarFloatingClientCx"),
							  m_szDialogBarFloatingClient.cx);
		pApp->WriteProfileInt(szSection, _T("DialogBarFloatingClientCy"),
							  m_szDialogBarFloatingClient.cy);
	}
...
BOOL CXTPToolBar::CToolBarInfo::LoadState(LPCTSTR lpszProfileName, int nIndex,
										  CXTPDockState* pDockState)
{
	ASSERT(pDockState != NULL);
	if (!pDockState)
		return FALSE;
	...
	m_pointPos  = CPoint(XTPToInt(pApp->GetProfileInt(szSection, _xtpXPos, -1)),
	...
	m_rectMRUDockPos = CRect(XTPToInt(pApp->GetProfileInt(szSection, _xtpMRUDockLeftPos, 0)),
	...
	m_ptMRUFloatPos = CPoint(XTPToInt(pApp->GetProfileInt(szSection, _xtpMRUFloatXPos, 0)),
	...
	if (m_bDialogBar)
	{
		m_szDialogBarDockingClient.cx = XTPToLong(
			pApp->GetProfileInt(szSection, _T("DialogBarDockingClientCx"),
								m_szDialogBarDockingClient.cx));
		m_szDialogBarDockingClient.cy = XTPToLong(
			pApp->GetProfileInt(szSection, _T("DialogBarDockingClientCy"),
								m_szDialogBarDockingClient.cy));
		m_szDialogBarFloatingClient.cx = XTPToLong(
			pApp->GetProfileInt(szSection, _T("DialogBarFloatingClientCx"),
								m_szDialogBarFloatingClient.cx));
		m_szDialogBarFloatingClient.cy = XTPToLong(
			pApp->GetProfileInt(szSection, _T("DialogBarFloatingClientCy"),
								m_szDialogBarFloatingClient.cy));
	}
...
BOOL CXTPCommandBars::GetControlsFileName(CString& strFileName, LPCTSTR lpszProfileName)
{
	...
	if (g_pGlobalSettings == 0
		|| ((CCommandBarsGlobalSettings*)g_pGlobalSettings)->m_strEXEName.IsEmpty())
	{
	...
	else
	{
		strFileName = ((CCommandBarsGlobalSettings*)g_pGlobalSettings)->m_strPath + _T("\\")
					  + ((CCommandBarsGlobalSettings*)g_pGlobalSettings)->m_strEXEName + _T(".exe-")
					  + lpszProfileName;
	}
...
void CXTPCommandBars::SaveBarState(LPCTSTR lpszProfileName, BOOL bOnlyCustomized /*= TRUE*/)
{
	...
	if (m_bCustomizeAvail)
	{
		...
		if (SaveCommandBarList(&px, &lstCommandBars))
		{
			...
			if (bSaveToFile)
			{
				try
				{
					...
					if (file.Open(strFileName, CFile::modeWrite | CFile::modeCreate))
					{
						...
						AfxGetApp()->WriteProfileString(szSection, _xtpCommandBarControls,
														strFileName);
			...
			{
				AfxGetApp()->WriteProfileBinary(szSection, _xtpCommandBarControls, pControls,
												dwCount);
...
void CXTPCommandBars::DoPropExchange(CXTPPropExchange* pPX,
									 XTP_COMMANDBARS_PROPEXCHANGE_PARAM* pParam)
{
	...
	else
	{
		...
		if (pCommandBar)
		{
			if (pParam->bSerializeDesignerControls)
			{
				...
				m_pDesignerControls->Remove(m_pDesignerControls->FindControl(xtpControlError,
	...
	if (pParam->bSerializeOptions)
	{
		...
		if (pPX->IsLoading())
			SetPaintManager(CXTPPaintManager::CreateTheme(theme));
...
void CXTPCommandBars::SerializeBarState(CArchive& ar, BOOL bSerializeControls, BOOL bOnlyCustomized,
										BOOL bSilent)
{
	CXTPPropExchangeArchive px(ar);
...
void CXTPCommandBars::GenerateCommandBarList(CXTPCommandBarList* pCommandBarList,
											 XTP_COMMANDBARS_PROPEXCHANGE_PARAM* pParam)
{
	DWORD nID = 0x1000000;
	int i;
...
void CXTPCommandBar::GenerateCommandBarList(DWORD& nID, CXTPCommandBarList* pCommandBarList,
											XTP_COMMANDBARS_PROPEXCHANGE_PARAM* pParam)
{
...
void CXTPMenuBar::GenerateCommandBarList(DWORD& nID, CXTPCommandBarList* pCommandBarList,
										 XTP_COMMANDBARS_PROPEXCHANGE_PARAM* pParam)
{
	if (!pCommandBarList->Lookup(this))
	{
...
void CXTPControl::GenerateCommandBarList(DWORD& nID, CXTPCommandBarList* pCommandBarList,
										 XTP_COMMANDBARS_PROPEXCHANGE_PARAM* pParam)
{
...
void CXTPControls::GenerateCommandBarList(DWORD& nID, CXTPCommandBarList* pCommandBarList,
										  XTP_COMMANDBARS_PROPEXCHANGE_PARAM* pParam)
{
	for (int i = 0; i < GetCount(); i++)
	{
							   ...
							   int nDefaultConfirmationChoice)
{
		...
		if (pLastControls->Compare(pCommandBar->GetControls()) == FALSE)
		{
			...
			if (!bSilent && IDYES != nChoice && IDNO != nChoice)
			{
				...
				CXTPResourceManager::AssertValid(
					XTPResourceManager()->LoadString(&strUpdate, XTP_IDS_CONFIRM_UPDATE));
				strMessage.Format(strUpdate, (LPCTSTR)m_strTitle);
				nChoice = XTPResourceManager()->ShowMessageBox(strMessage,
	...
	{
		CXTPOriginalControls* pOriginalControls = pCommandBar->m_pControls->m_pOriginalControls
													  ? NULL
	...
	for (int i = 0; i < pCommandBarList->GetCount(); i++)
	{
		...
		if (pToolBar)
		{
			...
			if (pExistsBar)
			{
				...
				{
					nDefaultConfirmationChoice = (m_bDefaultResetCustomizedToolbarChoice ? IDYES
																						 : IDNO);
				}
		...
		else if (pCommandBar->IsPopupBar() && ((CXTPPopupBar*)pCommandBar)->IsContextMenu())
		{
			...
			if (pExistsBar)
			{
				...
				CXTPOriginalControls* pOriginalControls =
					pExistsBar->GetControls()->GetOriginalControls();
				CMDTARGET_ADDREF(pOriginalControls);
...
void CXTPControls::DoPropExchange(CXTPPropExchange* pPX)
{
	...
	if (pPX->IsStoring())
	{
		...
		BOOL bOriginalControls = pParam && m_pOriginalControls && pParam->bSaveOriginalControls
									 ? TRUE
									 : FALSE;
...
void CXTPControlAction::DoPropExchange(CXTPPropExchange* pPX)
{
	...
	if (pPX->GetSchema() >= _XTP_SCHEMA_2030)
	{
		PX_Bool(pPX, _T("Visible"), m_bVisible, TRUE);
...
void CXTPControl::DoPropExchange(CXTPPropExchange* pPX)
{
	...
	if (pPX->GetSchema() > _XTP_SCHEMA_85)
	{
		if (!(pPX->IsStoring() && pPX->IsAllowDefaultValues()
			  && m_mapDocTemplatesExcluded.IsEmpty()))
			PX_Serialize(pPX, _T("DocTemplatesExcluded"), &m_mapDocTemplatesExcluded);

		if (!(pPX->IsStoring() && pPX->IsAllowDefaultValues()
			  && m_mapDocTemplatesAssigned.IsEmpty()))
...
void CXTPControlButton::DoPropExchange(CXTPPropExchange* pPX)
{
	...
	if (pPX->GetSchema() >= _XTP_SCHEMA_2030)
		PX_Enum(pPX, _T("StyleEx"), m_buttonStyleEx, xtpButtonAutomatic);
}
...
void CXTPControlPopup::DoPropExchange(CXTPPropExchange* pPX)
{
	...
	if (pPX->IsStoring())
	{
		long dwCommandBarID = m_pCommandBar ? XTPToLong(m_pCommandBar->m_nBarID) : 0;
...
void CXTPControlComboBox::DoPropExchange(CXTPPropExchange* pPX)
{
	...
	// however the value still must be read out to provide format backward compatibility (ticket
	// 36079).
