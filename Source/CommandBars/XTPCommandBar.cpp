...
void CXTPCommandBar::OnMouseMove(UINT /*nFlags*/, CPoint point)
{
	...
	if (!m_hWnd || pMouseManager->IsTrackedLock(this) || (!bActive && GetPosition() != xtpBarPopup))
		return;
...
{
	if (!IsMouseLocked()
		&& !(IsTrackingMode() && (GetPosition() != xtpBarPopup || m_nPopuped != -1))
		&& (m_nLockRecurse == 0))
		SetSelected(-1);
}
...
void CXTPCommandBar::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	if (pControl)
	{
		...
		{
			CXTPCommandBars* pCommandBars = GetCommandBars();
			if (pCommandBars && pCommandBars->IsCustomizeAvail()
				&& pCommandBars->GetCommandBarsOptions()->bAltDragCustomization)
			{
...
void CXTPCommandBar::OnRButtonUp(UINT /*nFlags*/, CPoint point)
{
	...
	if (pControl)
	{
		if (GetSite()->SendMessage(WM_XTP_CONTROLRBUTTONUP, (WPARAM)(LPPOINT)&point,
								   (LPARAM)pControl)
			== TRUE)
			return;
...
BOOL CXTPCommandBar::SetSelected(int nSelected, BOOL bKeyboard)
{
	...
	if (m_nSelected != -1)
	{
		...
		if (pSelected)
			pSelected->OnMouseMove(CPoint(-1, -1));
		if (pSelected)
			pSelected->OnSetSelected(FALSE);
	}
	...
	if (m_nSelected != -1)
	{
		...
		if (IsTrackingMode())
			AccessibleNotifyWinEvent(EVENT_OBJECT_FOCUS, m_hWnd, OBJID_MENU, m_nSelected + 1);

		XTPMouseManager()->SetSelected(this);
		if (!bKeyboard && m_bTrackOnHover)
			SetTimer(XTP_TID_HOVER, XTPToUIntChecked(s_nHoverDelay), NULL);
	}
	else
	{
...
void CXTPCommandBar::OnUpdateCmdUI()
{
	...
	BOOL bDisableIfNoHndler =
		pCommandBars ? pCommandBars->GetCommandBarsOptions()->bDisableCommandIfNoHandler : FALSE;
	...
	else
	{
		...
		for (state.m_nIndex = 0; (int)state.m_nIndex < GetControlCount(); state.m_nIndex++)
		{
...
#ifdef _XTP_COMMANDBARS_ACTIVEX
			if (bCustomTarget && pCommandBars
				&& (pControl->GetID() < 9000 || pControl->GetID() > 10000))
				continue;
#endif
			...
			if (CWnd::OnCmdMsg(0, MAKELONG((int)CN_UPDATE_COMMAND_UI, WM_COMMAND + WM_REFLECT_BASE),
							   &state, NULL))
				continue;
...
BOOL CXTPControls::LoadMenu(CMenu* pMenu)
{
	...
	for (int i = 0; i < nCount; i++)
	{
		...
		else
		{
			...
			if (((info.fType & MFT_MENUBARBREAK) == MFT_MENUBARBREAK)
				|| ((info.fType & MFT_MENUBREAK) == MFT_MENUBREAK))
			{
...
BOOL CXTPCommandBar::ProcessSpecialKey(XTPSpecialKey key)
{
	...
	else if (key == xtpKeyBack && m_nPopuped == -1 && m_nSelected != -1
			 && (bPopupToolBar || !GetParentCommandBar()))
	{
		CXTPControlPopup* pControlPopup = DYNAMIC_DOWNCAST(CXTPControlPopup,
														   GetControl(m_nSelected));
		if (pControlPopup && pControlPopup->GetEnabled())
			key = xtpKeyPopup;
	}
	if (((key == xtpKeyPopup) || (key == xtpKeyBack)) && m_nPopuped == -1
		&& (bPopupToolBar || (GetPosition() == xtpBarFloating)))
	{
		int nDirection = key == xtpKeyPopup ? +1 : -1;

		if (m_nSelected == -1)
		{
			SetSelected(m_pControls->GetNext(-1, nDirection),
						nDirection ? TRUE_KEYBOARD_NEXT : TRUE_KEYBOARD_PREV);
			return TRUE;
		}
		...
		if (!pControlPopup || !pControlPopup->GetEnabled())
		{
			...
			do
			{
				...
				CRect rcNextSelected	= pCondidate->GetRect(),
					  rcNextSelectedRow = pCondidate->GetRowRect();

				if (pNextSelected && (pNextSelected->m_rcRow.top == rcNextSelectedRow.top)
					&& ((rcNextSelected.left <= rcSelected.left
						 && (rcNextSelected.left > pNextSelected->GetRect().left))
						|| (rcNextSelected.left >= rcSelected.left
							&& (pNextSelected->GetRect().left > rcNextSelected.left))))
				{
					pNextSelected = pCondidate;
				}
				else if (!pNextSelected && rcNextSelectedRow.top != rcSelectedRow.top
						 && !rcNextSelectedRow.IsRectEmpty())
				{
					pNextSelected = pCondidate;
				}
				else if (pNextSelected)
				{
					SetSelected(pNextSelected->GetIndex(),
								nDirection > 0 ? TRUE_KEYBOARD_NEXT : TRUE_KEYBOARD_PREV);
					break;
				}
	...
	switch (key)
	{
		...
		case xtpKeyHome: SetSelected(m_pControls->GetNext(-1, +1), TRUE_KEYBOARD_NEXT); return TRUE;
...
BOOL CXTPCommandBar::OnHookKeyDown(UINT nChar, LPARAM lParam)
{
	...
	switch (nChar)
	{
		case VK_TAB:
		{
			...
			if (DYNAMIC_DOWNCAST(CXTPControlCustom, pControl) && ::GetFocus()
			...
			if ((GetKeyState(VK_CONTROL) < 0) && pCommandBars)
			{
				...
				{
					if (pCommandBars->SetToolBarFocus((CXTPToolBar*)pToolBar,
	...
	BOOL bHoriz = m_barPosition == xtpBarTop || m_barPosition == xtpBarBottom
				  || m_barPosition == xtpBarFloating
	...
	{
		case VK_LEFT:
			return ProcessSpecialKey(bHoriz ? (bLayoutRTL ? xtpKeyRight : xtpKeyLeft)
											: (bLayoutRTL ? xtpKeyPopup : xtpKeyBack));
		case VK_RIGHT:
			return ProcessSpecialKey(bHoriz ? (bLayoutRTL ? xtpKeyLeft : xtpKeyRight)
											: (!bLayoutRTL ? xtpKeyPopup : xtpKeyBack));
...
void CXTPCommandBar::OnKeyboardTipChar(UINT chAccel)
{
	...
	for (int i = 0; i < (int)pCommandBars->m_keyboardTips.arr.GetSize(); i++)
	{
		...
		{
			if (CXTPShortcutManager::CompareAccelKey(
					pTip->m_strTip[pCommandBars->m_keyboardTips.nKey], chAccel))
			{
	...
	if (chAccel != VK_SHIFT && chAccel != VK_CONTROL)
		MessageBeep(0);
}
...
int CXTPCommandBar::OnHookMessage(HWND /*hWnd*/, UINT nMessage, WPARAM& wParam, LPARAM& lParam,
								  LRESULT& /*lResult*/)
{
	if (IsTrackingMode())
	{
		switch (nMessage)
		{
			case WM_KEYDOWN: return OnHookKeyDown((UINT)wParam, lParam);
...
void CXTPCommandBar::OnTimer(UINT_PTR nIDEvent)
{
	...
	if (nIDEvent == XTP_TID_SHOWKEYBOARDTIPS)
	{
		...
		CXTPCommandBar* pCommandBar = nCount > 0 ? pMouseManager->GetTrackArray().GetAt(nCount - 1)
												 : 0;

		if (pCommandBar == this && !IsKeyboardTipsVisible())
			GetCommandBars()->ShowKeyboardTips(this);
	}
...
void CXTPCommandBar::DrawCommandBar(CDC* pDC, CRect rcClipBox)
{
	...
	for (int i = 0; i < GetControlCount(); i++)
	{
		...
		if (pCommandBars && pCommandBars->IsCustomizeMode()
			&& (pCommandBars->m_pDragSelected == pControl))
		{
...
void CXTPCommandBar::CustomizeDrawMarker(CDC* pDC)
{
	...
	if (m_bVerticalMarker)
	{
		...
		pDC->PatBlt(m_rcMarker.left + XTP_DPI_X(1), m_rcMarker.top + XTP_DPI_Y(1), XTP_DPI_X(1),
					m_rcMarker.Height() - XTP_DPI_Y(2), PATINVERT);
		pDC->PatBlt(m_rcMarker.left + XTP_DPI_X(2), m_rcMarker.top + XTP_DPI_Y(2),
					m_rcMarker.Width() - XTP_DPI_X(4), XTP_DPI_Y(2), PATINVERT);
		pDC->PatBlt(m_rcMarker.right - XTP_DPI_X(1), m_rcMarker.top, XTP_DPI_X(1),
					m_rcMarker.Height(), PATINVERT);
		pDC->PatBlt(m_rcMarker.right - XTP_DPI_X(2), m_rcMarker.top + XTP_DPI_Y(1), XTP_DPI_X(1),
					m_rcMarker.Height() - XTP_DPI_Y(2), PATINVERT);
	}
	else
	{
		pDC->PatBlt(m_rcMarker.left, m_rcMarker.top, m_rcMarker.Width(), XTP_DPI_Y(1), PATINVERT);
		pDC->PatBlt(m_rcMarker.left + XTP_DPI_X(1), m_rcMarker.top + XTP_DPI_Y(1),
					m_rcMarker.Width() - XTP_DPI_X(2), XTP_DPI_Y(1), PATINVERT);
		pDC->PatBlt(m_rcMarker.left + XTP_DPI_X(2), m_rcMarker.top + XTP_DPI_Y(2), XTP_DPI_X(2),
					m_rcMarker.Height() - XTP_DPI_Y(4), PATINVERT);
		pDC->PatBlt(m_rcMarker.left, m_rcMarker.bottom - XTP_DPI_Y(1), m_rcMarker.Width(),
					XTP_DPI_Y(1), PATINVERT);
		pDC->PatBlt(m_rcMarker.left + XTP_DPI_X(1), m_rcMarker.bottom - XTP_DPI_Y(2),
					m_rcMarker.Width() - XTP_DPI_X(2), XTP_DPI_Y(1), PATINVERT);
	}
...
int CXTPCommandBar::FindAccelEx(UINT chAccel, BOOL& bSelectOnly) const
{
	...
	do
	{
		...
		if (!strCaption.IsEmpty() && pControl->GetEnabled())
		{
			...
			if (CXTPShortcutManager::CompareAccelKey(strCaption[bFound ? nAmpIndex + 1 : 0],
													 chAccel))
			{
				if (bFound)
				{
...
CWnd* CXTPCommandBar::GetSite() const
{
	...
	if (pRoot->m_pSite)
		return pRoot->m_pSite;
	if (pRoot->m_hWnd)
		return pRoot->GetOwner();
											...
											CRect& rcMarker, int& nDropIndex, BOOL& bDropAfter)
{
	...
	BOOL bVert = (m_barPosition == xtpBarLeft || m_barPosition == xtpBarRight
	...
	for (int i = 0; i < nCount; i++)
	{
		...
		if (!m_bMultiLine)
		{
			...
			{
				nOffset = -1;
				if (!bVert && rc.top > point.y)
					break;
				else if (bVert && rc.right < point.x)
					break;
			}
		}
		else if (!bVert)
...
DROPEFFECT CXTPCommandBar::OnCustomizeDragOver(CXTPControl* pDataObject, CPoint point)
{
	...
	if (!pDataObject->DoCustomDragOver(this, point, dropEffect))
		return DROPEFFECT_NONE;
...
void CXTPCommandBar::OnCustomizeDrop(CXTPControl* pDataObject, DROPEFFECT& dropEffect,
									 CPoint ptDrop, CPoint ptDrag)
{
	CXTPCommandBars* pCommandBars = GetCommandBars();
	ASSERT(pCommandBars);
	if (!pCommandBars)
...
void CXTPCommandBar::OnControlSelected(BOOL bSelected, CXTPControl* pControl)
{
	...
	SetStatusText(bSelected ? pControl->GetDescription() : CString());
#endif
}
...
void CXTPCommandBar::UpdateDocTemplateControls()
{
	...
	for (int i = 0; i < GetControlCount(); i++)
	{
		...
		if (pControl->m_mapDocTemplatesAssigned.GetCount() > 0)
		{
			...
			if (pControl->SetHideFlag(xtpHideDocTemplate, !bVisible))
				bRecalcLayout = TRUE;
		}
...
HRESULT CXTPCommandBar::GetAccessibleParent(IDispatch* FAR* ppdispParent)
{
	...
	*ppdispParent = NULL;
	...
	if (pPopupBar)
	{
		...
		{
			*ppdispParent = pPopupBar->m_pControlPopup->CreateAccProxy();
			return S_OK;
		}
	}
	else if (GetSafeHwnd())
...
HRESULT CXTPCommandBar::GetAccessibleChild(VARIANT varChild, IDispatch* FAR* ppdispChild)
{
	...
	*ppdispChild = NULL;
	...
	if (nChild > 0 && nChild <= GetControlCount())
	{
		...
		if (pControl)
		{
			*ppdispChild = pControl->CreateAccProxy();
		}
	}
...
HRESULT CXTPCommandBar::GetAccessibleRole(VARIANT varChild, VARIANT* pvarRole)
{
	...
	{
		pvarRole->vt   = VT_I4;
		pvarRole->lVal = GetType() == xtpBarTypeMenuBar
							 ? ROLE_SYSTEM_MENUBAR
							 : GetType() == xtpBarTypePopup ? ROLE_SYSTEM_MENUPOPUP
															: ROLE_SYSTEM_TOOLBAR;
		return S_OK;
	}
...
HRESULT CXTPCommandBar::AccessibleLocation(long* pxLeft, long* pyTop, long* pcxWidth,
...
HRESULT CXTPCommandBar::AccessibleHitTest(long xLeft, long yTop, VARIANT* pvarID)
{
	...
	if (pControl)
	{
		...
		pvarID->pdispVal = pControl->CreateAccProxy();
	}
...
LRESULT CXTPCommandBar::OnGetObject(WPARAM wParam, LPARAM lParam)
{
	...
	if (dwObjId > 0 && dwObjId < 0xFFFFFF)
	{
		...
		return pControl->LresultFromProxyObject(wParam);
	}
...
BEGIN_DISPATCH_MAP(CXTPCommandBar, CWnd)
	...
	DISP_PROPERTY_EX_ID(CXTPCommandBar, "Visible", dispidVisible, OleGetVisible, OleSetVisible,
						VT_BOOL)
	DISP_PROPERTY_EX_ID(CXTPCommandBar, "Position", dispidPosition, OleGetPosition, OleSetPosition,
						VT_I4)
	DISP_FUNCTION_ID(CXTPCommandBar, "ShowPopup", dispidShowPopup, OleShowPopup, VT_I4,
					 VTS_VARIANT VTS_VARIANT VTS_VARIANT)
	DISP_FUNCTION_ID(CXTPCommandBar, "SetIconSize", dispidSetIconSize, OleSetIconSize, VT_EMPTY,
					 VTS_I4 VTS_I4)
	DISP_FUNCTION_ID(CXTPCommandBar, "EnableDocking", dispidEnableDocking, OleEnableDocking,
					 VT_EMPTY, VTS_I4)
	DISP_FUNCTION_ID(CXTPCommandBar, "SetTearOffPopup", dispidSetTearOffPopup, OleSetTearOffPopup,
					 VT_EMPTY, VTS_BSTR VTS_I4 VTS_I4)
	DISP_PROPERTY_ID(CXTPCommandBar, "BarID", dispidBarID, m_nBarID, VT_I4)
	DISP_FUNCTION_ID(CXTPCommandBar, "FindControl", dispidFindControl, OleFindControl, VT_DISPATCH,
	...
	DISP_FUNCTION_ID(CXTPCommandBar, "ModifyStyle", dispidModifyStyle, OleModifyStyle, VT_EMPTY,
					 VTS_I4 VTS_I4)
	DISP_FUNCTION_ID(CXTPCommandBar, "GetWindowRect", dispidGetWindowRect, OleGetWindowRect,
					 VT_EMPTY, VTS_PI4 VTS_PI4 VTS_PI4 VTS_PI4)
	DISP_FUNCTION_ID(CXTPCommandBar, "SetFlags", dispidSetFlags, SetFlags, VT_EMPTY, VTS_I4 VTS_I4)
	DISP_PROPERTY_EX_ID(CXTPCommandBar, "ShowTextBelowIcons", 19, IsTextBelowIcons,
						ShowTextBelowIcons, VT_BOOL)
	...
	DISP_PROPERTY_EX_ID(CXTPCommandBar, "Customizable", 23, OleGetCustomizable, OleSetCustomizable,
						VT_BOOL)
	...
	DISP_PROPERTY_EX_ID(CXTPCommandBar, "SelectedControl", 34, OleGetSelectedControl,
						OleSetSelectedControl, VT_DISPATCH)
	DISP_PROPERTY_EX_ID(CXTPCommandBar, "hWnd", DISPID_HWND, OleGetHandle, SetNotSupported,
						VT_HANDLE)

	DISP_PROPERTY_EX_ID(CXTPCommandBar, "CommandBars", 45, OleGetCommandBars, SetNotSupported,
						VT_DISPATCH)

	DISP_PROPERTY_EX_ID(CXTPCommandBar, "EnableAnimation", 37, IsAnimationEnabled, EnableAnimation,
						VT_BOOL)
...
LPDISPATCH CXTPCommandBar::OleFindControl(const VARIANT& varType, const VARIANT& varId,
...
void CXTPCommandBar::OleSetSelectedControl(LPDISPATCH lpDispatch)
{
	...
	CXTPControl* pControl =
		XTP_DYNAMIC_DOWNCAST_REMOTE_(CXTPControl, CXTPControl::FromIDispatchSafe(lpDispatch));
	if (pControl)
	{
...
CXTPCommandBar* AFX_CDECL CXTPCommandBar::FromDispatch(LPDISPATCH pDisp)
{
	...
	if (NULL != pDisp)
	{
		pInst = XTP_DYNAMIC_DOWNCAST_REMOTE_(CXTPCommandBar,
											 CXTPCmdTarget::FromIDispatchSafe(pDisp));
		if (NULL == pInst)
		{
