...
protected:
	...
	virtual void OnNavigateButtonClick(
		UINT nID); // <COMBINE CWorkspace::OnNavigateButtonClick@CXTPTabManagerNavigateButton*>
...
protected:
	...
	int GetTabStickerAction(POINT pos, CWorkspace** pWorkspace);
	int GetTabStickerAction();
	int GetTabSideAction(POINT pos, CXTPTabManagerItem* pItem, CWorkspace** pWorkspace,
						 CRect& rcDragRect);
	int GetTabSideAction(CXTPTabClientWnd* pTarget, POINT pos, CRect& rcDragRect);
#	ifndef _XTP_COMMANDBARS_ACTIVEX
#		ifdef _XTP_INCLUDE_DOCKINGPANE
	int GetDockStickerAction();
	int GetDockSideAction(CXTPDockingPaneTabbedContainer* pTarget, POINT pos, CRect& rcDragRect);
#		endif
#	endif
