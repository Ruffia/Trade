...
CXTPControlListBox::CXTPControlListBox()
{
	...
	m_pListBox->CreateEx(0, _T("LISTBOX"), NULL,
						 WS_POPUP | WS_VSCROLL | WS_CLIPCHILDREN | LBS_NOTIFY, CRect(0, 0, 0, 0), 0,
						 0);
...
void CXTPControlListBox::SetMultiplSel(BOOL bMultiplSel)
{
	...
	m_pListBox->CreateEx(0, _T("LISTBOX"), NULL,
						 WS_POPUP | WS_VSCROLL | WS_CLIPCHILDREN | LBS_NOTIFY
							 | (m_bMultiplSel ? LBS_MULTIPLESEL : 0),
						 CRect(0, 0, 0, 0), 0, 0);
}
...
void CXTPControlListBox::SetRect(CRect rcControl)
{
	...
	m_pListBox->SetWindowPos(0, 0, 0, 0, 0,
							 UINT(SWP_NOMOVE | SWP_NOSIZE | SWP_NOZORDER | SWP_NOACTIVATE
								  | (!IsVisible() ? SWP_HIDEWINDOW : SWP_SHOWWINDOW)));
}
...
CSize CXTPControlListBox::GetSize(CDC* /*pDC*/)
{
	...
	int nHeight		= min(m_nListBoxLinesMax, max(m_nListBoxLinesMin, m_pListBox->GetCount()))
				  * nItemHeight;
	int nWidth = m_nWidth;
...
void CXTPControlListBox::OnClick(BOOL bKeyboard, CPoint pt)
{
	if (bKeyboard)
	{
		...
		CXTPControl::OnClick(bKeyboard, pt);
}
...
CSize CXTPControlStatic::GetSize(CDC* pDC)
{
	...
	if (bVert)
		return CSize(max(XTP_DPI_X(22), sz.cx), max(m_nWidth, sz.cy));
