...
LRESULT CXTPReBarBase::OnShowBand(WPARAM wParam, LPARAM)
{
	...
	if (lResult)
	{
		...
		CControlBar* pBar = DYNAMIC_DOWNCAST(CControlBar,
											 CWnd::FromHandlePermanent(rbBand.hwndChild));
		BOOL bWindowVisible;
		if (pBar != NULL)
...
BOOL CXTPReBarBase::AddBar(CWnd* pBar, COLORREF clrFore, COLORREF clrBack, LPCTSTR pszText,
						   DWORD dwStyle)
{
	REBARBANDINFO rbBand;
...
LRESULT CXTPReBarBase::WindowProc(UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	switch (message)
	{
		...
		case WM_SETMESSAGESTRING: return GetOwner()->SendMessage(message, wParam, lParam);
	}
...
BOOL CXTPReBar::AddToolBar(CXTPToolBar* pToolBar, DWORD dwStyle)
{
	...
	int nCount = static_cast<int>(DefWindowProc(RB_GETBANDCOUNT, 0, 0));
	REBARBANDINFO rbBand;
	rbBand.cbSize = sizeof(rbBand);
...
CSize CXTPReBar::CalcFixedLayout(BOOL bStretch, BOOL bHorz)
{
	...
	for (int nTemp = nCount; nTemp--;)
	{
		...
		if (pToolBar)
		{
			...
			CSize sz	= pToolBar->CalcDockingLayout(
				   rc.Width(),
				   DWORD(LM_HORZDOCK | LM_HORZ | LM_COMMIT
...
void CXTPReBar::LoadState(LPCTSTR lpszProfileName)
{
	...
	CString strValue = AfxGetApp()->GetProfileString(lpszProfileName, m_lpszStateInfoEntry);
	if (!strValue.IsEmpty())
	{
		REBARBANDINFO rbbi;
		rbbi.cbSize = sizeof(rbbi);
...
#pragma warning(push)
#pragma warning(                                                                                   \
	disable : 4774) // 'swscanf_s' : format string expected in argument 2 is not a string literal
...
void CXTPReBar::SaveState(LPCTSTR lpszProfileName)
{
	...
	VERIFY(AfxGetApp()->WriteProfileString(lpszProfileName, m_lpszStateInfoEntry, strValue));
}
