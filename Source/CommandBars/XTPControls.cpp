...
CXTPControl* CXTPControls::AddMenuItem(CMenu* pMenu, int nIndex)
{
	...
	CString strCaption;
	XTPResourceManager()->GetMenuLocaleString(pMenu, XTPToUIntChecked(nIndex), strCaption,
											  MF_BYPOSITION);
	...
	if (pPopupMenu)
	{
		...
		{
			nID = XTPToUInt(_ttoi(strCaption.Mid(iPos + 1)));
			if (nID > 0)
	...
	if (pControl == NULL)
	{
		switch (controlType)
		{
			case xtpControlButton:
				if (nID == XTP_ID_WINDOWLIST)
					pControl = (CXTPControl*)CXTPControlWindowList::CreateObject();
				else if (nID == XTP_ID_WORKSPACE_ACTIONS)
					pControl = (CXTPControl*)CXTPControlWorkspaceActions::CreateObject();
				else if (nID == ID_VIEW_TOOLBAR)
					pControl = (CXTPControl*)CXTPControlToolbars::CreateObject();
				else if (nID == ID_OLE_VERB_FIRST && strCaption == _T("<<OLE VERBS GO HERE>>"))
					pControl = (CXTPControl*)CXTPControlOleItems::CreateObject();
				else if (nID == ID_FILE_MRU_FILE1)
					pControl = (CXTPControl*)CXTPControlRecentFileList::CreateObject();
				else
			...
			case xtpControlEdit: pControl = (CXTPControl*)CXTPControlEdit::CreateObject(); break;
			case xtpControlLabel: pControl = (CXTPControl*)CXTPControlLabel::CreateObject(); break;
			...
			case xtpControlButtonPopup:
			{
				...
				CMenu menu;
				if (!pPopupMenu && nID > 0 && XTPResourceManager()->LoadMenu(&menu, nID))
				{
					pPopupMenu = menu.GetSubMenu(0);
				}
				if (pPopupMenu)
					pPopupBar->LoadMenu(pPopupMenu, FALSE);
	...
	{
		if (pAction == NULL)
			pAction = pCommandBars->CreateAction(XTPToInt(nID));
...
CXTPControl* CXTPControls::Add(XTPControlType controlType, int nId, LPCTSTR strParameter,
	...
	switch (controlType)
	{
		...
		case xtpControlButton:
			if (nId == XTP_ID_WINDOWLIST)
				...
				pControl = (CXTPControl*)CXTPControlToolbars::CreateObject();
			else if (nId == XTP_ID_FILE_MRU && !bTemporary)
				pControl = (CXTPControl*)CXTPControlRecentFileList::CreateObject();
			else
				pControl = (CXTPControl*)CXTPControlButton::CreateObject();
			break;
		...
		case xtpControlGallery: pControl = (CXTPControl*)CXTPControlGallery::CreateObject(); break;
		...
		case xtpControlButtonPopup:
		{
			...
			if (nId > 0)
			{
				...
				if (XTPResourceManager()->LoadMenu(&menu, XTPToUInt(nId)))
				{
					pPopupMenu = menu.GetSubMenu(0);
...
CXTPControl* CXTPControls::Add(CXTPControl* pControl, int nId, LPCTSTR strParameter, int nBefore,
							   BOOL bTemporary)
{
	ASSERT(pControl);
	if (!pControl)
...
{
	pControl = (!pControl || pControl->m_nIndex >= GetCount() - 1) ? 0
																   : GetAt(pControl->m_nIndex + 1);
}
...
CXTPControl* CXTPControls::FindControl(XTPControlType type, int nId, BOOL bVisible,
...
long CXTPControls::GetNext(long nIndex, int nDirection, BOOL bKeyboard, BOOL bSkipTemporary,
						   BOOL bSkipCollapsed) const
{
	...
	while (nNext != nIndex)
	{
		...
		if ((GetAt(nNext)->IsVisible(nSkipVisible))
			&& (!bKeyboard || !(GetAt(nNext)->m_dwFlags & xtpFlagSkipFocus)))
		{
...
void CXTPControls::_CenterControlsInRow(XTPBUTTONINFO* pData, int nFirst, int nLast, int nHeight,
										BOOL bVert, CSize sizeResult, CRect rcBorder)
{
	for (int i = nFirst; i < nLast; i++)
	{
		...
		if (bVert)
		{
			pControl->rcRow.SetRect(rc.right - nHeight, rcBorder.top, rc.right,
									sizeResult.cy + rcBorder.top);
			if (!(m_pParent->GetFlags() & xtpFlagSmartLayout))
				pControl->rcControl.OffsetRect(-(nHeight - rc.Width()) / 2, 0);
		}
		else
		{
			pControl->rcRow.SetRect(rcBorder.left, rc.top, sizeResult.cx + rcBorder.left,
									rc.top + nHeight);
...
void CXTPControls::_AdjustBorders(XTPBUTTONINFO* pData, CSize& sizeResult, DWORD dwMode,
								  CRect rcBorder)
{
	if (dwMode & LM_COMMIT)
	{
		...
		for (int i = 0; i < GetCount(); i++)
		{
			...
			if (bVert)
				pControl->rcControl.OffsetRect(sizeResult.cx, 0);
			...
			nHeight	= max(nHeight,
							 bVert ? pControl->rcControl.Width() : pControl->rcControl.Height());
		}
		_CenterControlsInRow(pData, nFirstInRow, GetCount(),
							 nFirstInRow == 0 ? nBarHeight : nHeight, bVert, sizeResult, rcBorder);
	}

	sizeResult += CSize(rcBorder.left + rcBorder.right, rcBorder.top + rcBorder.bottom);
}

void CXTPControls::_MoveRightAlligned(XTPBUTTONINFO* pData, CSize sizeResult, CRect rcBorder,
									  DWORD dwMode)
{
	BOOL bVert = (0 != (dwMode & LM_VERTDOCK));
	...
	for (i = GetCount() - 1; i >= 0; i--)
	{
		...
		if (pControl->bVisible && !pControl->bHide)
		{
			...
			{
				nStretchAvail = nRight
								- (!bVert ? pControl->rcControl.right : pControl->rcControl.bottom);
			}
		}
		...
		if ((pControl->bVisible && !pControl->bHide && pControl->bWrap) || (i == 0))
		{
			if (nStretchedCount > 0 && nStretchAvail > 0)
			{
				...
				for (int j = i; j <= nLastInRow; j++)
				{
					...
					CRect rc = pControl->rcControl;
					if (!bVert)
						pControl->rcControl.SetRect(nOffset + rc.left, rc.top, nOffset + rc.right,
													rc.bottom);
					else
						pControl->rcControl.SetRect(rc.left, nOffset + rc.top, rc.right,
													nOffset + rc.bottom);
...
CSize CXTPControls::_ReduceSmartLayoutToolBar(CDC* pDC, XTPBUTTONINFO* pData, int nWidth,
											  DWORD& dwMode)
{
	CSize sizeResult(0, 0);
	BOOL bReduced = FALSE;
			...
			for (int i = GetCount() - 1; i >= 2; i--)
			{
				...
				for (int j = i; j >= 0; j--)
				{
					...
					if ((nItems == 2 || !pControl->bBeginGroup)
						&& GetAt(j)->m_buttonStyle == xtpButtonIconAndCaptionBelow)
					{
						nButtons[nItems] = j;
						nItems += 1;
					}
		...
		if (!bReduced)
		{
			for (int i = GetCount() - 1; i >= 2; i--)
			{
				...
				for (int j = i; j >= 0; j--)
				{
					...
					if ((nItems == 2 || !pControl->bBeginGroup)
						&& GetAt(j)->m_buttonStyle == xtpButtonIconAndCaption)
					{
						nButtons[nItems] = j;
						nItems += 1;
						nLeft = nOffset;
	...
	if (dwMode & LM_HIDEWRAP)
	{
		...
		for (int i = 0; i < GetCount(); i++)
		{
			...
			if (!bFirst
				&& ((bVert ? pControl->rcControl.bottom : pControl->rcControl.right) > nWidth))
			{
...
CSize CXTPControls::_CalcSmartLayoutToolBar(CDC* pDC, XTPBUTTONINFO* pData, DWORD& dwMode)
{
	...
	int nResultHeight = szLargeIcon.cy + XTP_DPI_Y(7)
						+ m_pParent->GetPaintManager()->GetSplitDropDownHeight();
...
CSize CXTPControls::_WrapSmartLayoutToolBar(CDC* pDC, XTPBUTTONINFO* pData, int nWidth,
											DWORD& dwMode)
{
	BOOL bVert		  = (0 != (dwMode & LM_VERTDOCK));
	...
	for (int i = 0; i < GetCount(); i++)
	{
		...
		BOOL bDrawImage = GetAt(i)->GetIconId() > 0
							  ? GetAt(i)->GetImageManager()->IsPrimaryImageExists(
									XTPToUInt(GetAt(i)->GetIconId()), szLargeIcon.cx)
									!= NULL
							  : FALSE;

		pControl->bHide			= FALSE;
		pControl->bWrap			= FALSE;
		GetAt(i)->m_buttonStyle = bDrawImage ? xtpButtonIconAndCaptionBelow
...
CSize CXTPControls::CalcDynamicSize(CDC* pDC, int nLength, DWORD dwMode, const CRect& rcBorder,
									int nWidth)
{
	...
	CSize sizeResult(0, 0);
	CSize szSeparators(
		m_pParent->GetPaintManager()->DrawCommandBarSeparator(pDC, m_pParent, NULL, FALSE));
	int i;
	...
	if (GetVisibleCount() > 0)
	{
		...
		if ((m_pParent->GetFlags() & xtpFlagSmartLayout))
		{
			...
			{
				sizeResult = _WrapSmartLayoutToolBar(pDC, pData,
													 nLength - rcBorders.top - rcBorders.bottom,
													 dwMode);
			}
			else
			{
				sizeResult = _WrapSmartLayoutToolBar(pDC, pData,
													 nLength - rcBorders.right - rcBorders.left,
													 dwMode);
			}
		}
		else
		{
			...
			{
				_SizeFloatableBar(pData,
								  nLength
									  - ((dwMode & LM_LENGTHY) ? rcBorders.top + rcBorders.bottom
	...
	{
		sizeResult = XTP_DPI(CSize(23, 22))
					 + CSize(rcBorders.left + rcBorders.right, rcBorders.top + rcBorders.bottom);
		if (dwMode & LM_HORZDOCK)
			sizeResult.cy = max(nWidth, sizeResult.cy);
		else
			sizeResult.cx = max(nWidth, sizeResult.cx);
	}
...
CSize CXTPControls::CalcPopupSize(CDC* pDC, int nLength, int nMRUWidth, const CRect& rcBorder)
{
	...
	CRect rcControl;
	int yPos = rcBorder.top, nWidthColumn = 0, xPos = rcBorder.left, nFirstInColumn = 0,
		nHeight = 0;
	...
	for (i = 0; i < nCount; i++)
	{
		...
		if ((pControl->GetFlags() & xtpFlagWrapRow && !bFirst)
			|| (nLength > 0 && szControl.cy + yPos > nLength - rcBorder.bottom))
		{
		...
		if (!GetParent()->IsResizable() && CXTPPopupBar::m_dMaxWidthDivisor > 0
			&& szControl.cx > (CXTPPopupBar::m_dMaxWidthDivisor * rcWork.right))
		...
		if (nWidthColumn < szControl.cx)
			nWidthColumn = szControl.cx;
	}
	...
	{
		CXTPControl* pControl = GetAt(i);
		pControl->SetRowRect(CRect(pControl->GetRect().left, rcBorder.top,
								   pControl->GetRect().right, rcBorder.top + nHeight));
	}
...
void CXTPControls::Copy(CXTPControls* pControls, BOOL bRecursive)
{
	if (this != pControls)
	{
		...
		{
			SetOriginalControls(
				(CXTPOriginalControls*)pControls->m_pOriginalControls->Duplicate(bRecursive));
		}
		else
		{
...
CXTPControls* CXTPControls::Duplicate(BOOL bRecursive)
{
	...
	{
		pControls->SetOriginalControls(
			(CXTPOriginalControls*)m_pOriginalControls->Duplicate(bRecursive));
	}
...
void CXTPControls::Sort()
{
	...
	qsort(m_arrControls.GetData(), (size_t)m_arrControls.GetSize(), sizeof(CXTPControl*),
		  (GENERICCOMPAREFUNC)CompareByCaption);
...
BEGIN_DISPATCH_MAP(CXTPControlActions, CXTPCmdTarget)
	...
	DISP_FUNCTION_ID(CXTPControlActions, "_NewEnum", DISPID_NEWENUM, OleNewEnum, VT_UNKNOWN,
					 VTS_NONE)
	DISP_FUNCTION_ID(CXTPControlActions, "Add", dispidAdd, OleAdd, VT_DISPATCH,
	...
	DISP_FUNCTION_ID(CXTPControlActions, "Delete", 6, OleRemove, VT_EMPTY, VTS_I4)
END_DISPATCH_MAP()
...
LPDISPATCH CXTPControlActions::OleAdd(int nId, LPCTSTR lpszCaption, LPCTSTR lpszTooltipText,
...
VOID CXTPControlActions::OleRemove(int nId)
{
	Remove(nId);
}
...
BEGIN_DISPATCH_MAP(CXTPControls, CXTPCmdTarget)
	...
	DISP_FUNCTION_ID(CXTPControls, "Add", dispidAdd, OleAdd, VT_DISPATCH,
	...
	DISP_FUNCTION_ID(CXTPControls, "Find", dispidFindControl, OleFindControl, VT_DISPATCH,
					 VTS_VARIANT VTS_VARIANT VTS_VARIANT VTS_VARIANT)
	DISP_FUNCTION_ID(CXTPControls, "DeleteAll", 5, RemoveAll, VT_EMPTY, VTS_NONE)

	DISP_FUNCTION_ID(CXTPControls, "CreateOriginalControls", 6, CreateOriginalControls, VT_EMPTY,
					 VTS_NONE)
	DISP_PROPERTY_EX_ID(CXTPControls, "OriginalControls", 7, OleGetOriginalControls,
						SetNotSupported, VT_DISPATCH)

	DISP_FUNCTION_ID(CXTPControls, "AddControl", 8, OleAddControl, VT_EMPTY,
...
LPDISPATCH CXTPControls::OleAdd(long controlType, int nId, LPCTSTR lpszCaption,
...
LPDISPATCH CXTPControls::OleFindControl(const VARIANT& varType, const VARIANT& varId,
