...
public:
	...
	void OnExecute()
	{
		...
		int nId = m_nId == XTP_ID_MENUBAR_CLOSE
					  ? SC_CLOSE
...
public:
	...
	void SetMDISysMenu(HWND hWndActiveChild, HMENU hDocMenu)
	{
		...
		if (hIcon == NULL)
			hIcon = (HICON)::SendMessage(hWndActiveChild, WM_GETICON, ICON_SMALL, 0);
		if (hIcon == NULL)
			hIcon = (HICON)::SendMessage(hWndActiveChild, WM_GETICON, ICON_BIG, 0);
		if (hIcon == NULL)
			hIcon = (HICON)(ULONG_PTR)::GetClassLongPtr(hWndActiveChild, GCLP_HICONSM);
		if (hIcon == NULL)
		...
		if (hIcon)
			m_hIcon = CXTPImageManagerIcon::ScaleToFit(hIcon,
													   CXTPImageManagerIcon::GetExtent(hIcon),
													   XTP_DPI_X(16));
		...
		for (UINT i = 0; i < XTPToUInt(pMenu->GetMenuItemCount()); i++)
		{
			...
			else
			{
				...
				if (nItemID >= 0xF000 && nItemID < 0xF1F0)
				{
					...
					pControl->SetItemDefault(
						::GetMenuDefaultItem(pMenu->m_hMenu, TRUE, GMDI_USEDISABLED) == i);
	...
	void Draw(CDC* pDC)
	{
		if (m_hIcon)
		{
			...
			if (!CXTPPaintManager::IsDpiBitmapScalingEnabled(m_pCommandBar))
				size = XTP_UNDPI(size);

			pDC->DrawState(GetRect().TopLeft(), size, m_hIcon, (UINT)DSS_NORMAL, HBRUSH(0));
		}
	}
...
BOOL CXTPMenuBar::IsOleDocumentActive(HWND hWndActiveChild) const
{
	...
	COleDocument* pActiveOleDoc = DYNAMIC_DOWNCAST(COleDocument,
												   pFrame && pFrame->GetActiveView()
													   ? pFrame->GetActiveView()->GetDocument()
													   : NULL);
	...
	COleDocObjectItem* pItem = DYNAMIC_DOWNCAST(COleDocObjectItem,
												pActiveOleDoc->GetInPlaceActiveItem(
...
void CXTPMenuBar::AddSysButton(CXTPControl* pControl, int nId, LPCTSTR strParameter,
							   int nBefore /*= -1*/)
{
...
void CXTPMenuBar::RefreshSysButtons()
{
	...
	if (bMax && (m_dwFlags & xtpFlagAddMDISysPopup))
	{
		...
		if (hDocMenu && ::IsMenu(hDocMenu))
		{
			if (hDocMenuButton != hDocMenu)
			{
				...
				{
					AddSysButton(new CControlMDISysMenuPopup(hWndActiveChild, hDocMenu),
								 XTP_ID_MENUBAR_SYSMENU, _T(""), 0);
				}
			}
		}
		else
	...
	if (!pButton && bMax && (lStyle & WS_MINIMIZEBOX) && (lStyle & WS_SYSMENU)
		&& (!(m_dwFlags & xtpFlagHideMinimizeBox)))
	{
	...
	if (!pButton && bMax && (lStyle & WS_MAXIMIZEBOX) && (lStyle & WS_SYSMENU)
		&& (!(m_dwFlags & xtpFlagHideMaximizeBox)))
	{
...
CXTPMenuBarMDIMenuInfo* CXTPMenuBar::AddMDIMenu(UINT nIDResource)
{
	...
	CMenu menu;
	VERIFY(XTPResourceManager()->LoadMenu(&menu, nIDResource));
...
int CXTPMenuBar::OnHookMessage(HWND hWnd, UINT nMessage, WPARAM& wParam, LPARAM& lParam,
							   LRESULT& lResult)
{
	switch (nMessage)
	{
		case WM_SYSCOMMAND:
			if (IsWindowVisible() && !XTPMouseManager()->IsMouseLocked()
				&& (GetKeyState(VK_SHIFT) >= 0) && !IsCustomizeMode())
...
#ifndef _XTP_COMMANDBARS_ACTIVEX
				BOOL bAccepMenus = ((m_dwFlags & xtpFlagIgnoreSetMenuMessage) == 0)
								   && (wParam || m_hMenuDefault);
		...
		case WM_MDICREATE:
		case WM_MDIDESTROY:
		case WM_MDIACTIVATE: PostMessage(WM_TIMER, TID_REFRESH, 0); break;
...
void CXTPMenuBar::SetupMenu()
{
	...
	if (pCommandBars)
	{
		pCommandBars->GetImageManager()->SetIcons(XTP_IDB_MENUBAR_ICONS, nIDMenuBarIcons, 4,
												  CSize(16, 16));
	}
...
void CXTPMenuBar::Reset(BOOL bShowWarningMessage)
{
	...
	else if (((m_pMDIMenus->GetCount() > 0) || (GetControls()->GetOriginalControls() == NULL))
			 && m_nIDResourceCurrent)
	{
		...
		CMenu menu;
		VERIFY(XTPResourceManager()->LoadMenu(&menu, m_nIDResourceCurrent));
		LoadMenu(&menu);
			...
			if (pInfoOrig)
			{
					...
					&& pInfo->m_pControls && pInfo->m_pControls->m_pOriginalControls == NULL)
				{
					CXTPOriginalControls* pOriginalControls =
						pInfoOrig->m_pControls->m_pOriginalControls;
...
BEGIN_DISPATCH_MAP(CXTPMenuBarMDIMenuInfo, CXTPCmdTarget)
	...
	DISP_PROPERTY_EX_ID(CXTPMenuBarMDIMenuInfo, "Controls", 4, OleGetControls, SetNotSupported,
						VT_DISPATCH)
END_DISPATCH_MAP()
...
BEGIN_DISPATCH_MAP(CXTPMenuBarMDIMenus, CXTPCmdTarget)
	...
	DISP_FUNCTION_ID(CXTPMenuBarMDIMenus, "NewEnum", DISPID_NEWENUM, OleNewEnum, VT_DISPATCH,
					 VTS_NONE)
END_DISPATCH_MAP()
