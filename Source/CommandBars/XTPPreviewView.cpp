...
void CXTPControlPageSelector::OnDrawItem(CDC* pDC, CRect rcItem, COLORREF clrText)
{
	...
	{
		pDC->FillSolidRect(rcItem.left, y, (i + 1) % 5 == 0 ? rcItem.Width() / 2 : rcItem.Width(),
						   1, clrText);
	}
}
...
{
	XTPImageManager()->RemoveIcon(XTP_ID_PREVIEW_PRINT);
	XTPImageManager()->RemoveIcon(XTP_ID_PREVIEW_PREV);
	XTPImageManager()->RemoveIcon(XTP_ID_PREVIEW_NEXT);
	XTPImageManager()->RemoveIcon(XTP_ID_PREVIEW_ZOOMIN);
	XTPImageManager()->RemoveIcon(XTP_ID_PREVIEW_ZOOMOUT);
	XTPImageManager()->RemoveIcon(XTP_ID_PREVIEW_ONEPAGE);
	XTPImageManager()->RemoveIcon(XTP_ID_PREVIEW_TWOPAGE);
	XTPImageManager()->RemoveIcon(XTP_ID_PREVIEW_CLOSE);
...
int CXTPPreviewView::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	if (!m_pCommandBar->CreateToolBar(WS_CHILD | WS_VISIBLE | CBRS_TOP | CBRS_GRIPPER
										  | CBRS_TOOLTIPS,
									  m_pToolBar)
		|| !m_pCommandBar->LoadToolBar(XTP_IDR_PREVIEW_TOOLBAR, FALSE))
	{
	...
	m_pCommandBar->SetIconSize(XTPSystemMetrics()->GetSmallIconSize());

	CXTPImageManager* pImgMgr = m_pCommandBar->GetImageManager();
	if (pImgMgr)
	{
		CString type(_T("RT_XAML"));
		UINT nWidth = 0;

		nWidth = XTPToUInt(XTPSystemMetrics()->GetSmallIconSize().cx);
		pImgMgr->SetVectorIcon(type, XTP_XAML_SMALL_ICON_NORMAL_PRINTER, XTP_ID_PREVIEW_PRINT,
							   nWidth);
		pImgMgr->SetVectorIcon(type, XTP_XAML_SMALL_ICON_NORMAL_SYMBOL_ARROW_LEFT,
							   XTP_ID_PREVIEW_PREV, nWidth);
		pImgMgr->SetVectorIcon(type, XTP_XAML_SMALL_ICON_NORMAL_SYMBOL_ARROW_RIGHT,
							   XTP_ID_PREVIEW_NEXT, nWidth);
		pImgMgr->SetVectorIcon(type, XTP_XAML_SMALL_ICON_NORMAL_ZOOM_IN, XTP_ID_PREVIEW_ZOOMIN,
							   nWidth);
		pImgMgr->SetVectorIcon(type, XTP_XAML_SMALL_ICON_NORMAL_ZOOM_OUT, XTP_ID_PREVIEW_ZOOMOUT,
							   nWidth);
		pImgMgr->SetVectorIcon(type, XTP_XAML_SMALL_ICON_NORMAL_VIEW_PAGE_ONE,
							   XTP_ID_PREVIEW_ONEPAGE, nWidth);
		pImgMgr->SetVectorIcon(type, XTP_XAML_SMALL_ICON_NORMAL_VIEW_PAGE_FOUR,
							   XTP_ID_PREVIEW_TWOPAGE, nWidth);
		pImgMgr->SetVectorIcon(type, XTP_XAML_SMALL_ICON_NORMAL_CLOSE, XTP_ID_PREVIEW_CLOSE,
							   nWidth);

		nWidth = XTPToUInt(GetSystemMetrics(SM_CXICON));
		pImgMgr->SetVectorIcon(type, XTP_XAML_LARGE_ICON_NORMAL_PRINTER, XTP_ID_PREVIEW_PRINT,
							   nWidth);
		pImgMgr->SetVectorIcon(type, XTP_XAML_LARGE_ICON_NORMAL_SYMBOL_ARROW_LEFT,
							   XTP_ID_PREVIEW_PREV, nWidth);
		pImgMgr->SetVectorIcon(type, XTP_XAML_LARGE_ICON_NORMAL_SYMBOL_ARROW_RIGHT,
							   XTP_ID_PREVIEW_NEXT, nWidth);
		pImgMgr->SetVectorIcon(type, XTP_XAML_LARGE_ICON_NORMAL_ZOOM_IN, XTP_ID_PREVIEW_ZOOMIN,
							   nWidth);
		pImgMgr->SetVectorIcon(type, XTP_XAML_LARGE_ICON_NORMAL_ZOOM_OUT, XTP_ID_PREVIEW_ZOOMOUT,
							   nWidth);
		pImgMgr->SetVectorIcon(type, XTP_XAML_LARGE_ICON_NORMAL_VIEW_PAGE_ONE,
							   XTP_ID_PREVIEW_ONEPAGE, nWidth);
		pImgMgr->SetVectorIcon(type, XTP_XAML_LARGE_ICON_NORMAL_VIEW_PAGE_FOUR,
							   XTP_ID_PREVIEW_TWOPAGE, nWidth);
		pImgMgr->SetVectorIcon(type, XTP_XAML_LARGE_ICON_NORMAL_CLOSE, XTP_ID_PREVIEW_CLOSE,
							   nWidth);

		nWidth = XTPToUInt(GetSystemMetrics(SM_CXSMICON));
		pImgMgr->SetVectorIcon(type, XTP_XAML_SMALL_ICON_DISABLED_PRINTER, XTP_ID_PREVIEW_PRINT,
							   nWidth, xtpImageDisabled);
		pImgMgr->SetVectorIcon(type, XTP_XAML_SMALL_ICON_DISABLED_SYMBOL_ARROW_LEFT,
							   XTP_ID_PREVIEW_PREV, nWidth, xtpImageDisabled);
		pImgMgr->SetVectorIcon(type, XTP_XAML_SMALL_ICON_DISABLED_SYMBOL_ARROW_RIGHT,
							   XTP_ID_PREVIEW_NEXT, nWidth, xtpImageDisabled);
		pImgMgr->SetVectorIcon(type, XTP_XAML_SMALL_ICON_DISABLED_ZOOM_IN, XTP_ID_PREVIEW_ZOOMIN,
							   nWidth, xtpImageDisabled);
		pImgMgr->SetVectorIcon(type, XTP_XAML_SMALL_ICON_DISABLED_ZOOM_OUT, XTP_ID_PREVIEW_ZOOMOUT,
							   nWidth, xtpImageDisabled);
		pImgMgr->SetVectorIcon(type, XTP_XAML_SMALL_ICON_DISABLED_VIEW_PAGE_ONE,
							   XTP_ID_PREVIEW_ONEPAGE, nWidth, xtpImageDisabled);
		pImgMgr->SetVectorIcon(type, XTP_XAML_SMALL_ICON_DISABLED_VIEW_PAGE_FOUR,
							   XTP_ID_PREVIEW_TWOPAGE, nWidth, xtpImageDisabled);
		pImgMgr->SetVectorIcon(type, XTP_XAML_SMALL_ICON_DISABLED_CLOSE, XTP_ID_PREVIEW_CLOSE,
							   nWidth, xtpImageDisabled);

		nWidth = XTPToUInt(GetSystemMetrics(SM_CXICON));
		pImgMgr->SetVectorIcon(type, XTP_XAML_LARGE_ICON_DISABLED_PRINTER, XTP_ID_PREVIEW_PRINT,
							   nWidth, xtpImageDisabled);
		pImgMgr->SetVectorIcon(type, XTP_XAML_LARGE_ICON_DISABLED_SYMBOL_ARROW_LEFT,
							   XTP_ID_PREVIEW_PREV, nWidth, xtpImageDisabled);
		pImgMgr->SetVectorIcon(type, XTP_XAML_LARGE_ICON_DISABLED_SYMBOL_ARROW_RIGHT,
							   XTP_ID_PREVIEW_NEXT, nWidth, xtpImageDisabled);
		pImgMgr->SetVectorIcon(type, XTP_XAML_LARGE_ICON_DISABLED_ZOOM_IN, XTP_ID_PREVIEW_ZOOMIN,
							   nWidth, xtpImageDisabled);
		pImgMgr->SetVectorIcon(type, XTP_XAML_LARGE_ICON_DISABLED_ZOOM_OUT, XTP_ID_PREVIEW_ZOOMOUT,
							   nWidth, xtpImageDisabled);
		pImgMgr->SetVectorIcon(type, XTP_XAML_LARGE_ICON_DISABLED_VIEW_PAGE_ONE,
							   XTP_ID_PREVIEW_ONEPAGE, nWidth, xtpImageDisabled);
		pImgMgr->SetVectorIcon(type, XTP_XAML_LARGE_ICON_DISABLED_VIEW_PAGE_FOUR,
							   XTP_ID_PREVIEW_TWOPAGE, nWidth, xtpImageDisabled);
		pImgMgr->SetVectorIcon(type, XTP_XAML_LARGE_ICON_DISABLED_CLOSE, XTP_ID_PREVIEW_CLOSE,
							   nWidth, xtpImageDisabled);
	}
	...
	CXTPControlPopup* pTwoPages = (CXTPControlPopup*)m_pCommandBar->GetControls()->SetControlType(
		FindToolbarButton(XTP_ID_PREVIEW_TWOPAGE), xtpControlButtonPopup);
	pTwoPages->SetFlags(xtpFlagManualUpdate);

	pTwoPages->GetCommandBar()->GetControls()->Add(new CXTPControlPageSelector(),
												   XTP_ID_PREVIEW_TWOPAGE);
	pTwoPages->GetCommandBar()->SetShowGripper(FALSE);

	CXTPControlEdit* pControlGotoPage =
		(CXTPControlEdit*)m_pCommandBar->GetControls()->SetControlType(FindToolbarButton(
																		   XTP_ID_PREVIEW_GOTOPAGE),
																	   xtpControlEdit);

	pControlGotoPage->SetWidth(XTP_DPI_X(100));

	CXTPControlComboBox* pControlZoom =
		(CXTPControlComboBox*)m_pCommandBar->GetControls()->SetControlType(
			FindToolbarButton(XTP_ID_PREVIEW_ZOOMCOMBO), xtpControlComboBox);
...
void CXTPPreviewView::RecalLayout()
{
	...
	if (m_pCommandBar->IsVisible())
		size = m_pCommandBar->CalcDockingLayout(r.Width(), /*LM_HIDEWRAP| */ LM_HORZDOCK | LM_HORZ
															   | LM_COMMIT);
...
BOOL CXTPPreviewView::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message)
{
	...
	UINT nPage;
	if (m_nZoomState < 500 && FindPageRect(point, nPage))
	{ // On a page and not zoomed all the way in
		if (m_hMagnifyCursor == NULL)
		{
			HINSTANCE hInst  = AfxFindResourceHandle(MAKEINTRESOURCE(AFX_IDC_MAGNIFY),
													 RT_GROUP_CURSOR);
...
void CXTPPreviewView::SetZoomState(UINT nNewState, UINT nPage, CPoint point)
{
	...
	{
		CXTPControlComboBox* pControlCombo = (CXTPControlComboBox*)FindToolbarButton(
			XTP_ID_PREVIEW_ZOOMCOMBO);
...
void CXTPPreviewView::SetScaledSize(UINT nPage)
{
	...
	switch (m_nZoomState)
	{
		case XTP_ZOOM_WHOLEPAGE: *pRatio = *pZoomOutRatio; break;
	...
	pRect->SetRect(PREVIEW_MARGIN, PREVIEW_MARGIN, scaledSize.cx + PREVIEW_MARGIN + XTP_DPI_X(3),
	...
	{
		// set up scroll size
		SetScrollSizes(MM_TEXT, pRect->Size() + CSize(PREVIEW_MARGIN * 2, PREVIEW_MARGIN * 2),
					   windowSize);
	}
}
...
void CXTPPreviewView::OnUpdateNumPageChange(CCmdUI* /*pCmdUI*/)
{
	...
	// pCmdUI->Enable(m_nZoomState == XTP_ZOOM_WHOLEPAGE && m_nMaxPages != 1 &&
...
// return size of current page less margins
{
	...
	if (m_nZoomState == XTP_ZOOM_WHOLEPAGE && (m_pPreviewInfo->GetMaxPage() != 0xffff)
...
void CXTPPreviewView::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	...
	switch (nSBCode)
	{
		case SB_BOTTOM: SetCurrentPage(m_pPreviewInfo->GetMaxPage(), TRUE); break;

		case SB_TOP: SetCurrentPage(m_pPreviewInfo->GetMinPage(), TRUE); break;

		case SB_PAGEDOWN:
			SetCurrentPage(m_nCurrentPage
							   + (m_pPreviewInfo->GetMaxPage() - m_pPreviewInfo->GetMinPage() + 9)
									 / 10,
						   TRUE);
			break;
		...
		case SB_THUMBPOSITION: SetCurrentPage(nPos, TRUE); break;
	}
}
...
{
	CXTPControlComboBox* pControl = DYNAMIC_DOWNCAST(CXTPControlComboBox,
...
{
	CXTPControlSelector* pControl = DYNAMIC_DOWNCAST(CXTPControlSelector,
													 ((NMXTPCONTROL*)pNMHDR)->pControl);
	if (!pControl)
		return;
