...
BOOL CXTPStatusBar::PreCreateWindow(CREATESTRUCT& cs)
{
	...
	if (IsWin4() && (m_dwStyle & (CBRS_ALIGN_ANY | CBRS_BORDER_ANY)) == CBRS_BOTTOM)
	{
...
void CXTPStatusBar::RecalcLayout()
{
	...
	for (i = 0; i < GetPaneCount(); i++)
	{
		...
		if (pPane->GetBeginGroup() && !bFirstVisible)
			bSeparator = TRUE;
		...
		cxExtra -= (pPane->m_nBestFit + (pPane->HasBorders() ? CX_PANE_BORDER : 0)
					+ (bSeparator ? m_cxPaneSpacing : 0) + pPane->m_rcMargins.left
					+ pPane->m_rcMargins.right);
	...
	for (i = 0; i < GetPaneCount(); i++)
	{
		...
		pPane->m_rcPane.SetRect(left, rect.top + pPane->m_rcMargins.top, right,
								rect.bottom - pPane->m_rcMargins.bottom);
...
void CXTPStatusBar::GetPaneInfo(int nIndex, UINT& nID, UINT& nStyle, int& cxWidth) const
{
	ASSERT(this);
...
CSize CXTPStatusBar::CalcFixedLayout(BOOL, BOOL /*bHorz*/)
{
	...
	CSize size;
	size.cx = 32767;
	size.cy = szText.cy + m_cyBottomBorder + m_cyTopBorder + ::GetSystemMetrics(SM_CYBORDER) * 2
			  + pPaintManager->m_rcStatusBarBorder.top + pPaintManager->m_rcStatusBarBorder.bottom;
...
LRESULT CXTPStatusBar::OnNcHitTest(CPoint point)
{
	if (m_bShowSizeGripper && !::IsZoomed(::GetParent(m_hWnd)))
	{
		...
		int dxGripper = (m_cxRightBorder + ::GetSystemMetrics(SM_CXVSCROLL)
						 + ::GetSystemMetrics(SM_CXBORDER) * 2);
		...
		if (point.y >= rc.bottom - XTP_DPI_Y(3) && GetPaintManager()->BaseTheme() == xtpThemeRibbon
			&& CXTPCommandBarsFrameHook::m_nMsgQueryFrameHook != 0)
		{
			CXTPCommandBarsFrameHook* pFrame = (CXTPCommandBarsFrameHook*)::SendMessage(
				::GetParent(m_hWnd), CXTPCommandBarsFrameHook::m_nMsgQueryFrameHook, 0, 0);
...
{
	DWORD dwHideFlags = bVisible ? m_dwHideFlags & ~xtpHideCustomize
								 : m_dwHideFlags | xtpHideCustomize;
...
CXTPPopupBar* CXTPStatusBar::GetStatusBarPopup()
{
	...
	CXTPControl* pControl = pPopup->GetControls()->Add(xtpControlLabel,
	...
	for (int i = 0; i < GetPaneCount(); i++)
	{
		...
		pControl->SetCaption(pPane->m_strCaption.IsEmpty() ? pPane->m_strText
														   : pPane->m_strCaption);
...
void CXTPStatusBar::HandleMouseMove(CPoint point)
{
	...
	if (pPane != m_pHighlightedPane)
	{
		...
		if (pHighlightedPane)
			pHighlightedPane->Redraw();
		if (m_pHighlightedPane)
			m_pHighlightedPane->Redraw();
...
void CXTPStatusBar::OnLButtonDblClk(UINT nFlags, CPoint point)
{
	...
	nm.dwItemSpec   = (m_pHighlightedPane ? XTPToDWORDChecked(m_pHighlightedPane->GetIndex()) : ~0);
	nm.pt			= point;
...
{
	if (m_hWnd != NULL && ((dwOldStyle & CBRS_BORDER_ANY) != (dwNewStyle & CBRS_BORDER_ANY)))
	{
...
void CXTPStatusBar::OnDraw(CDC* pDC)
{
	if (m_pCommandBars)
	{
		if (m_pCommandBars->GetPaintManager()->RequiresResourceImages()
			&& !XTPResourceImages()->IsValid())
			return;
	}
...
void CXTPStatusBar::PositionControls()
{
	...
	for (int i = 0; i < GetPaneCount(); i++)
	{
		...
		else
		{
			...
			::SetWindowPos(pPane->m_hWndPane, 0, rcPane.left, rcPane.top, rcPane.Width(),
						   rcPane.Height(), SWP_NOZORDER | SWP_SHOWWINDOW);
...
void CXTPStatusBar::OnWindowPosChanged(WINDOWPOS* lpwndpos)
{
	...
	if (pPaintManager && pPaintManager->BaseTheme() == xtpThemeRibbon)
	{
		...
		{
			GetParent()->SetWindowPos(0, 0, 0, 0, 0,
...
LRESULT CXTPStatusBar::OnIdleUpdateCmdUI(WPARAM wParam, LPARAM)
{
	...
	if ((GetStyle() & WS_VISIBLE) && (m_pDockBar == NULL || (m_pDockBar->GetStyle() & WS_VISIBLE)))
	{
...
INT_PTR CXTPStatusBar::OnToolHitTest(CPoint point, TOOLINFO* pTI) const
{
	...
	if (pPane)
	{
		...
		nHit = XTPToIntPtr(pPane->GetID());

		CString strTip = pPane->GetTooltip();
		if (strTip.IsEmpty())
			return -1;

		CXTPToolTipContext::FillInToolInfo(pTI, m_hWnd, rcPane, nHit, strTip, strTip, _T(""),
										   GetImageManager());
...
void CXTPStatusBar::LoadState(LPCTSTR lpszProfileName)
{
	...
	if (m_bCustomizable)
	{
		...
		while (!strValue.IsEmpty())
		{
			...
			CXTPStatusBarPane* pPane = (nResult == 2) && (nID != 0) ? FindPane(XTPToUInt(nID))
																	: NULL;
			if (pPane)
			{
	...
	if (pPX->IsStoring())
	{
		...
		if (m_bCustomizable)
		{
			...
			for (int i = 0; i < nCount; i++)
			{
				...
				strPaneState.Format(_T("%i=%i"), pPane->GetID(),
									pPane->GetCustomizationVisible() ? 1 : 0);
...
void CXTPStatusBar::SaveState(LPCTSTR lpszProfileName)
{
	...
	if (m_bCustomizable)
	{
		...
		AfxGetApp()->WriteProfileString(lpszProfileName, _T("StatusBarPanes"),
										strValue.IsEmpty() ? (LPCTSTR)NULL : (LPCTSTR)strValue);
	}
	else
	{
...
void CXTPStatusBar::OnUpdateKeyIndicator(CCmdUI* pCmdUI)
{
	ASSERT(pCmdUI != NULL);
	int nVK   = 0;
	UINT flag = 0x0001;
	...
	switch (pCmdUI->m_nID)
	{
		case ID_INDICATOR_OVR: nVK = VK_INSERT; break;

		default:
			TRACE(_T("Warning: OnUpdateKeyIndicator - unknown indicator\n"));
			pCmdUI->ContinueRouting();
			return; // not for us
	}

	pCmdUI->SetText(_T("INS"));
	pCmdUI->Enable(0 != (::GetKeyState(nVK) & flag));
...
BEGIN_DISPATCH_MAP(CXTPStatusBar, CControlBar)
	...
	DISP_PROPERTY_NOTIFY_ID(CXTPStatusBar, "IdleText", 3, m_strIdleText, OleOnIdleTextChanged,
							VT_BSTR)
	...
	DISP_FUNCTION_ID(CXTPStatusBar, "HitTest", 14, OleHitTest, VT_DISPATCH,
					 VTS_XPOS_PIXELS VTS_YPOS_PIXELS)
	DISP_PROPERTY_EX_ID(CXTPStatusBar, "ToolTipContext", 15, OleGetToolTipContext, SetNotSupported,
						VT_DISPATCH)
	DISP_PROPERTY_EX_ID(CXTPStatusBar, "ShowSizeGripper", 16, OleGetShowSizeGripper,
						OleSetShowSizeGripper, VT_BOOL)
	...
	DISP_FUNCTION_ID(CXTPStatusBar, "EnableCustomization", 20, EnableCustomization, VT_EMPTY,
					 VTS_BOOL)
	DISP_PROPERTY_EX_ID(CXTPStatusBar, "EnableMarkup", 30, OleGetEnableMarkup, OleSetEnableMarkup,
						VT_BOOL)

	DISP_PROPERTY_EX_ID(CXTPStatusBarPane, "hWnd", DISPID_HWND, OleGetHwnd, SetNotSupported,
						VT_HANDLE)
