...
public:
	//-----------------------------------------------------------------------
	// Summary:
	//     Creates a status bar (a child window) and associates it with the
	//     CStatusBar object. Also sets the initial font and sets the
	//     status bar's height to a default value.
	// Parameters:
	//     pParentWnd - Pointer to the CWnd object whose Windows window
	//                  is the parent of the status bar.
	//     dwStyle    - The status bar window style's bits.
	//     nID        - The toolbar's child-window ID.
	// Returns:
	//     Nonzero if successful; otherwise 0.
	//-----------------------------------------------------------------------
	BOOL Create(CWnd* pParentWnd,
				DWORD dwStyle = WS_CHILD | WS_VISIBLE | CBRS_BOTTOM | WS_CLIPCHILDREN
								| WS_CLIPSIBLINGS,
...
public:
	...
	// Parameters:
	//     pWnd        - Pointer to a control window.
	...
	// Returns:
	//     CXTPStatusBarPane pointer if successful, otherwise NULL.
	//-----------------------------------------------------------------------
	CXTPStatusBarPane* AddIndicator(UINT nID, int nIndex = -1);
	CXTPStatusBarPane* AddIndicator(CXTPStatusBarPane* pPane, UINT nID,
									int nIndex = -1); // <combine
													  // CXTPStatusBar::AddIndicator@UINT@int>
	...
	//-----------------------------------------------------------------------
	// Summary:
	//     Calculates the total number of visible panes.
	// Returns:
	...
	//     nIndex - Index of the indicator pane.
	// Returns:
	//     A pointer to a CXTPStatusBarPane class.
	//-----------------------------------------------------------------------
	...
	//-----------------------------------------------------------------------
	// Summary:
	//     Determines which pane, if any, is at a specified screen position.
	// Parameters:
	//     pt     - Point to be tested.
	//     lpRect - Pointer to a CRect object to receive the item at point pt.
	// Returns:
	//     A pointer to the CXTPStatusBarPane object at the specified point, if any,
	//     or NULL otherwise.
	//-----------------------------------------------------------------------
...
public:
	//-----------------------------------------------------------------------
	// Summary:
	//     Call this method to enable/disable customization of the status bar.
	// Parameters:
...
public:
	//-----------------------------------------------------------------------
	// Summary:
	//     Call this member function to set the paint manager to draw disabled text.
	// Parameters:
	//     bDraw - TRUE to draw disabled text, otherwise FALSE.
	//-----------------------------------------------------------------------
	void SetDrawDisabledText(BOOL bDraw);

	//-----------------------------------------------------------------------
	// Summary:
	//     Call this member function to check if the paint manager will draw disabled text.
	// Returns:
	//     TRUE if the paint manager will draw disabled text, otherwise FALSE.
	//-----------------------------------------------------------------------
...
public:
	...
	// Parameters:
	//     pParentWnd - Pointer to the CWnd object whose Windows window is the
	//                  parent of the status bar.
	//     dwCtrlStyle - Additional styles for the creation of the embedded
	...
	//-----------------------------------------------------------------------
	// Summary:
	//     Copies the coordinates of the indicator specified by nIndex into
	//     the structure pointed to by lpRect. Coordinates are in pixels
	...
	//     lpRect - Pointer to a RECT structure or a CRect object that will receive
	//              the coordinates of the indicator specified by nIndex.
	// See Also:
	//     GetPaneInfo, CommandToIndex
	//-----------------------------------------------------------------------
	void GetItemRect(int nIndex, LPRECT lpRect) const;

	//-----------------------------------------------------------------------
	// Summary:
	//     Call this member function to retrieve the text that appears in a
	//     status bar pane. The second form of this member function
	//     fills a CString object with the string text.
	// Parameters:
	...
	void GetPaneText(int nIndex,
					 CString& rString) const; // <combine CXTPStatusBar::GetPaneText@int@const>
	...
	//-----------------------------------------------------------------------
	// Summary:
	//     Call this method to set the font for the status bar.
	// Parameters:
	//     pFont - Font to set.
	...
	//-----------------------------------------------------------------------
	// Summary:
	//     Call this method to get the font for the status bar.
	// Returns:
	...
	//-----------------------------------------------------------------------
	// Summary:
	//     Sets the position of the ribbon divider index. This specifies
	//     the panes that get the second color for the ribbon theme.
	// Parameters:
	//     nIndex - Position of the ribbon bar divider.
	// Remarks:
	//     RibbonDividerIndex is a zero-based index and specifies the
	...
	//-----------------------------------------------------------------------
	// Summary:
	//     Gets the position of the ribbon divider index.
	// Returns:
	//     Position of the ribbon bar divider.
	//-----------------------------------------------------------------------
	...
	//-----------------------------------------------------------------------
	// Summary:
	//     Call this method to reposition all panes.
	//-----------------------------------------------------------------------
...
public:
	...
	{
		FireEvent(DISPID_MOUSEDOWN, EVENT_PARAM(VTS_I2 VTS_I2 VTS_XPOS_PIXELS VTS_YPOS_PIXELS),
...
protected:
	...
	XTP_SUBSTITUTE_GDI_MEMBER_WITH_CACHED(CFont, m_fontStatus, m_xtpFontStatus,
										  GetStatusFontHandle);
...
{
	ASSERT(cyTop >= 2);
	CControlBar::SetBorders(cxLeft, cyTop, cxRight, cyBottom);
}
