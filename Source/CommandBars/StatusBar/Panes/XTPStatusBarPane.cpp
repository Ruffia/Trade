...
int CXTPStatusBarPane::GetBestFit()
{
	...
	{
		XTPMarkupSetDefaultFont(m_pStatusBar->m_pMarkupContext,
								(HFONT)GetTextFont()->GetSafeHandle(), COLORREF_NULL);
		cxText = XTPMarkupMeasureElement(m_pMarkupUIElement).cx + m_rcPadding.left
				 + m_rcPadding.right;
	}
	else
	{
	...
	if (pIcon)
		cxText += XTP_DPI_X(pIcon->GetWidth()) + (!m_strText.IsEmpty() ? XTP_DPI_X(2) : 0);
...
{
	ASSERT(m_pStatusBar);
	return (m_pStatusBar && m_xtpFontText.m_hObject == NULL) ? m_pStatusBar->GetFont()
															 : &m_xtpFontText;
}
...
{
	if (m_nIconIndex != -1)
		return m_pStatusBar->GetImageManager()->GetImage(XTPToUInt(m_nIconIndex), 0);
...
BEGIN_DISPATCH_MAP(CXTPStatusBarPane, CXTPCmdTarget)
	...
	DISP_PROPERTY_EX_ID(CXTPStatusBarPane, "BackgroundColor", DISPID_BACKCOLOR, GetBackgroundColor,
						SetBackgroundColor, VT_COLOR)
	DISP_PROPERTY_EX_ID(CXTPStatusBarPane, "TextColor", DISPID_FORECOLOR, GetTextColor,
						SetTextColor, VT_COLOR)
	DISP_PROPERTY_EX_ID(CXTPStatusBarPane, "TextGrayColor", 25, GetTextGrayColor, SetTextGrayColor,
						VT_COLOR)
	...
	DISP_PROPERTY_EX_ID(CXTPStatusBarPane, "Alignment", 9, GetTextAlignment, SetTextAlignment,
						VT_I4)
	...
	DISP_FUNCTION_ID(CXTPStatusBarPane, "SetPadding", 14, SetPadding, VT_EMPTY,
					 VTS_I4 VTS_I4 VTS_I4 VTS_I4)
	DISP_FUNCTION_ID(CXTPStatusBarPane, "SetMargins", 20, SetMargins, VT_EMPTY,
					 VTS_I4 VTS_I4 VTS_I4 VTS_I4)
	...
	DISP_FUNCTION_ID(CXTPStatusBarPane, "GetRect", 23, OleGetRect, VT_EMPTY,
...
{
	return m_xtpFontText.GetSafeHandle()
			   ? AxCreateOleFont(&m_xtpFontText, this,
								 (LPFNFONTCHANGED)&CXTPStatusBarPane::OleSetFont)
			   : AxCreateOleFont(m_pStatusBar->GetFont(), this,
								 (LPFNFONTCHANGED)&CXTPStatusBarPane::OleSetFont);
}
