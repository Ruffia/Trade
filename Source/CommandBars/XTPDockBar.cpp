...
{
	if (GetPosition() == xtpBarTop && m_pCommandBars->GetMenuBar() != NULL
		&& m_pCommandBars->GetMenuBar()->IsRibbonBar()
...
BOOL CXTPDockBar::Create(CWnd* pParentWnd, DWORD dwStyle, UINT nID)
{
	...
	CString strWindowName	  = barPosition == xtpBarTop
								? _T("xtpBarTop")
								: barPosition == xtpBarLeft
									  ? _T("xtpBarLeft")
									  : barPosition == xtpBarBottom ? _T("xtpBarBottom")
																	: _T("xtpBarRight");
...
CSize CXTPDockBar::CalcFixedLayout(BOOL /*bStretch*/, BOOL /*bHorz*/,
...
int CXTPDockBar::_GetMode(BOOL bHorz, CXTPToolBar* pBar)
{
	...
	if (pBar->GetFlags() & xtpFlagStretched)
		dwMode |= LM_STRETCH;
	if ((!m_pCommandBars->IsCustomizeMode() || m_pCommandBars->IsQuickCustomizeMode())
		&& pBar->GetFlags() & xtpFlagHideWrap)
		return XTPToInt(dwMode | LM_HIDEWRAP);

	return XTPToInt(dwMode);
}

int CXTPDockBar::_AdjustRow(CToolBarArray& arrRow, CPoint pt, int nLength, BOOL bHorz,
							AFX_SIZEPARENTPARAMS* lpLayout, int& nRemove)
{
	...
	if (nLen > nLength)
	{
		...
		for (nPos = (int)arrInfo.GetSize() - 1; nPos >= 0; nPos--)
		{
			...
			if (nLeft > rect.left)
				nLeft = rect.left;
			if (nLeft < nLen - rect.Width())
				nLeft = nLen - rect.Width();

			if (nLeft < arrInfo[nPos].nTotlaMinWidth)
				nLeft = arrInfo[nPos].nTotlaMinWidth;
			if (nLen - nLeft < arrInfo[nPos].nMinWidth)
				nLeft = nLen - arrInfo[nPos].nMinWidth;
	...
	for (nPos = 0; nPos < arrInfo.GetSize(); nPos++)
	{
		CXTPToolBar* pBar = arrInfo[nPos].pBar;
		CRect& rect		  = arrInfo[nPos].rcBar;
		CSize sizeBar	 = pBar->CalcDockingLayout(bHorz ? rect.Width() : rect.Height(),
												XTPToDWORD(_GetMode(bHorz, pBar)));
...
int CXTPDockBar::AdjustRow(CToolBarArray& arrRow, CPoint pt, int nLength, BOOL bHorz,
						   AFX_SIZEPARENTPARAMS* lpLayout)
{
	int nTotalWidth = 0;
...
LRESULT CXTPDockBar::OnSizeParent(WPARAM, LPARAM lParam)
{
	...
	if (dwStyle & WS_VISIBLE)
	{
		...
		int nLength = (dwStyle & CBRS_ORIENT_HORZ
						   ? sizeAvail.cx
						   : sizeAvail.cy) /*- 2 * GetSystemMetrics(SM_CXBORDER)*/;
...
int CXTPDockBar::Insert(CXTPToolBar* pBarIns, CRect /*rect*/, CPoint ptMid)
{
	...
	for (nPos = 0; nPos < m_arrBars.GetSize(); nPos++)
	{
		...
		if (pBar != NULL)
		{
			...
			nWidth = max(nWidth, bHorz ? rectBar.bottom : rectBar.right);
		}
