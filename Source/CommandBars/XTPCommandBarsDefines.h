...
// </code>
// See Also: C
//     XTPCommandBars, CXTPCommandBar, CXTPCommandBar::GetType
//
...
//-------------------------------------------------------------------------
// Summary:
//     Command bar text orientation flags.
// See Also:
//     CXTPCommandBar::m_nTextOrientation
//-------------------------------------------------------------------------
enum XTPBarTextOrientation
{
	xtpBarTextAuto,		// Lets the Commandbars decide the best orientation. Horizontal bars will
						// display text horizontally and vertical bars will display text vertically.
...
// See Also:
//     CXTPControl, CXTPControl::GetType, CXTPControls
//
...
enum XTPControlType
{
	...
	xtpControlPopup,			// Pop-up type.
	xtpControlButtonPopup,		// Button pop-up.
	xtpControlSplitButtonPopup, // Split button pop-up.
	xtpControlComboBox,			// Combobox button.
...
// See Also:
//     CXTPControl, CXTPControl::SetFlags
//
...
enum XTPControlFlags
{
	...
	xtpFlagWrapRow			   = 128, // Wrap row for toolbar or pop-up bar.
...
// See Also:
//     CXTPControl, CXTPControl::SetHideFlags
//
...
enum XTPControlHideFlags
{
	...
	xtpHideScroll = 8, // Control is hidden by scrolling.  There are too many controls in a pop-up
					   // and the control is hidden because the pop-up is scrolled.
	xtpHideCustomize = 16, // Control is hidden by customize settings.  User removed it.  This is
						   ...
						   // user removes the control from the Add and Remove Buttons pop-up menu.
...
// See Also:
//     CXTPControlButton, CXTPControl::SetStyle, CXTPControl::GetStyle
//
...
enum XTPButtonStyle
{
	...
	xtpButtonCaptionAndDescription = 5 // Button draw icon, caption and description for pop-up bar.
};

//-----------------------------------------------------------------------
// Summary:
//     Deprecated style for Combo control.
//-----------------------------------------------------------------------
...
enum XTPPaintTheme
{
	...
	xtpThemeNativeWindows10		  = 17,					// Windows 10.
	xtpThemeVisualStudio2017	  = 18,					// Visual Studio 2017 theme, uses INI file.
	xtpThemeVisualStudio2019	  = 19,					// Visual Studio 2019 theme, uses INI file.
	xtpThemeVisualStudio2022	  = 20,					// Visual Studio 2022 theme, uses INI file.
};
...
enum XTPMenuBarFlags
{
	...
	xtpFlagHideMDIButtons		= xtpFlagHideMinimizeBox | xtpFlagHideMaximizeBox
							| xtpFlagHideClose, // Specifies to hide all MDI buttons displayed on
												// the Command Bar.
	xtpFlagAddMDISysPopup = 0x2000L // To add system MDI pop-up bar.  Specifies to add system MDI
									// pop-up menu to the Menu Bar.  This will appears as an icon to
...
// </code>
// See Also:
//     CXTPToolBar, CXTPToolBar::EnableDocking
//
...
enum XTPToolBarFlags
{
	...
	xtpFlagStretchedShared = 0x100000,			// Bar is stretched, but does not fill whole row.
...
//-----------------------------------------------------------------------
// Summary:
//     Enumerated type used to determine if menu underlines execute pop-ups.
// Example:
// <code>
...
//-----------------------------------------------------------------------
// Summary:
//     Enumerated type used to determine the animation effect of pop-up bars.
// Example:
//     <code> pCommandBars->GetCommandBarsOptions()->animationType = xtpAnimateSlide; </code>
// See Also:
//     CXTPCommandBarsOptions, CXTPCommandBarsOptions::animationType
//
...
// See Also:
//     CXTPPopupBar, CXTPPopupBar::m_popupFlags
//
// <KEYWORDS xtpPopupRight, xtpPopupLeft, xtpPopupDown>
//-----------------------------------------------------------------------
enum XTPPopupDirection
{
	xtpPopupRight = 0, // Pop-up Bar will be opened right of control.
	xtpPopupLeft  = 1, // Pop-up Bar will be opened left of control.
	xtpPopupDown  = 2, // Pop-up Bar will be opened bottom of control.
	xtpPopupUp	= 4  // Pop-up Bar will be opened top of control.
};
...
// See Also:
//     CXTPCommandBar, CXTPCommandBar::ProcessSpecialKey
//
...
enum XTPSpecialKey
{
	...
	xtpKeyBack,   // Close active pop-up key.
	xtpKeyPopup,  // Open selected pop-up key.
	xtpKeyEscape, // Escape key.
	xtpKeyReturn, // Return key.
	xtpKeyHome,   // Select first key.
...
// See Also:
//     CXTPCommandBar::DelayRedraw, CXTPCommandBar::DelayLayout
//
...
enum XTPIdleFlags
{
	...
	xtpIdleRedraw = 2  // Delay redraw of command bar.
};
...
// Remarks:
//     pControl parameter is NULL when the user removes the mouse pointer from
//     the selected item.
// Example:
...
// </code>
// See Also:
//     CXTPControl, CXTPCommandBar
//-----------------------------------------------------------------------
const UINT WM_XTP_CONTROLSELECTED = (WM_XTP_COMMANDBARS_BASE + 2);

//-----------------------------------------------------------------------
// Summary:
//     The WM_XTP_DRAGCONTROLCHANGED is sent to the CXTPCommandBars site
//     when the user select a button of a toolbar in customization mode.
// Example:
...
// </code>
// See Also:
//     CXTPCommandBars::GetDragControl, CXTPCommandBars::SetCustomizeMode
//-----------------------------------------------------------------------
const UINT WM_XTP_DRAGCONTROLCHANGED = (WM_XTP_COMMANDBARS_BASE + 3);

//-----------------------------------------------------------------------
// Summary:
//     The WM_XTP_INITMENU is sent to the CXTPCommandBars site when the menu is about to
//     become active. It occurs when the user clicks an item on the menu bar or presses
...
// </code>
// See Also:
//     CXTPCommandBars, CXTPMenuBar
//-----------------------------------------------------------------------
const UINT WM_XTP_INITMENU = (WM_XTP_COMMANDBARS_BASE + 4);

//-----------------------------------------------------------------------
// Summary:
//     The WM_XTP_CONTROLRBUTTONUP is sent to the CXTPCommandBars site when the menu user
//     presses the right button in a command bar control.
// Parameters:
...
// </code>
// See Also:
//     CXTPCommandBar, CXTPControl
//-----------------------------------------------------------------------
const UINT WM_XTP_CONTROLRBUTTONUP = (WM_XTP_COMMANDBARS_BASE + 5);

//-----------------------------------------------------------------------
// Summary:
//     The WM_XTP_COMMANDBARKEYDOWN is sent to the CXTPCommandBars site when
...
// Parameters:
//     pPopupBar - Pointer to a pop-up menu.
// Example:
...
// </code>
// See Also:
//     WM_XTP_UNINITCOMMANDSPOPUP, CXTPPopupBar
//-----------------------------------------------------------------------
...
//     pPopup - (CXTPPopupBar*)lParam - Pop-up bar with context menu.
// Example:
...
//-----------------------------------------------------------------------
// Summary:
//     This structure is passed as a parameter in CBN_XTP_EXECUTE message.
// Remarks:
...
typedef struct
{
	...
	CXTPControl* pControl; // Pointer to the control that caused the event.
} NMXTPCONTROL, FAR* LPNMXTPCONTROL;

//-----------------------------------------------------------------------
// Summary:
//     The CBN_XTP_EXECUTE is sent to the CXTPCommandBars site when the user clicks
//     an item of a CXTPCommandBar object.
// Parameters:
//     id -         ID of the control that was clicked.
//     memberFxn -  Name of member function to handle the message.
// Remarks:
//     Use ON_COMMAND to map a single command to a member function if you don't need the exact
//     control that caused the event. Use ON_COMMAND_RANGE or ON_XTP_EXECUTE_RANGE to map a range of
//     command IDs to one member function. <p/> If you don't set *pResult = 1 then WM_COMMAND also
//     will be sent for this event.
// Example:
...
// Remarks:
//     This structure contains extended information about a CXTPControl object to be created.
//     CXTPCommandBars sends an XTP_COMMANDBARS_CREATECONTROL message to the owner site
//     when a new CXTPControl object is about to become created.
// See Also:
//     WM_XTP_CREATECONTROL
//-------------------------------------------------------------------------
typedef struct
{
...
//-----------------------------------------------------------------------
// Summary:
//     The WM_XTP_BEFORECREATECONTROL is sent to the CXTPCommandBars site when a
//     new CXTPControl object is created.
// Parameters:
//     lpCreateControl - Pointer to an XTP_COMMANDBARS_CREATECONTROL structure with
//                       information about the control to be created.
// Returns:
//     If an application processes this message, it should return TRUE.
// Example:
...
// </code>
// See Also:
//     XTP_COMMANDBARS_CREATECONTROL, CXTPControl, WM_XTP_AFTERCREATECONTROL
//-----------------------------------------------------------------------
...
//-----------------------------------------------------------------------
// Summary:
//     The WM_XTP_AFTERCREATECONTROL is sent to the CXTPCommandBars site after a
//     new CXTPControl object is created.
// Parameters:
//     lpCreateControl - Pointer to an XTP_COMMANDBARS_CREATECONTROL structure
//                       with information about the control to be created.
// Returns:
//     If an application processes this message, it should return TRUE.
// Example:
...
// </code>
// See Also:
//     XTP_COMMANDBARS_CREATECONTROL, CXTPControl, WM_XTP_BEFORECREATECONTROL
//-----------------------------------------------------------------------
...
//-----------------------------------------------------------------------
// Summary:
//     The WM_XTP_GETTABICON message is sent to a MDI child window to retrieve the
//     icon that will be used for the tabbed interface.
// Remarks:
//     If application returns 0, CDocument::GetTitle will be used. You can override
//     CXTPTabClientWnd::GetItemIcon instead of processing the message.
// Returns:
...
// </code>
// See Also:
//     CXTPTabClientWnd, WM_XTP_GETWINDOWTEXT, WM_XTP_GETWINDOWTOOLTIP
//-----------------------------------------------------------------------
const UINT WM_XTP_GETTABICON = (WM_XTP_COMMANDBARS_BASE + 12);

//-------------------------------------------------------------------------
// Summary:
//     Creates a bar structure.
// Remarks:
//     This structure contains extended information about a bar to be created.
//     CXTPCommandBars sends an XTP_COMMANDBARS_CREATEBAR message to the owner site
//     when a new CXTPCommandBar object is about to be created.
// See Also:
...
typedef struct
{
	...
	BOOL bPopup;				 // TRUE if it is a pop-up command bar.
	BOOL bExpandBar;			 // TRUE if it is an expanded pop-up.
	BOOL bCustomBar;			 // TRUE if it is a user-defined toolbar.
	BOOL bTearOffBar;			 // TRUE if it is a user tear-off pop-up.
...
//-----------------------------------------------------------------------
// Summary:
//     The WM_XTP_CREATECOMMANDBAR is sent to the CXTPCommandBars site when a
//     new CXTPCommandBars object is created.
// Parameters:
//     lpCreateBar - Pointer to an XTP_COMMANDBARS_CREATEBAR structure with
//                   information about a bar to be created.
// Returns:
//     If an application processes this message, it should return TRUE.
// Example:
...
// </code>
// See Also:
//     XTP_COMMANDBARS_CREATEBAR, CXTPCommandBar
//-----------------------------------------------------------------------
...
//    update a Customize menu command without updating the command itself.
// Parameters:
...
//-----------------------------------------------------------------------
// Summary:
//     This macro generates the C++ header code necessary for a CXTPControl-derived
...
// </code>
// See Also:
//     IMPLEMENT_XTP_CONTROL, DECLARE_XTP_COMMANDBAR
//-----------------------------------------------------------------------
...
//-----------------------------------------------------------------------
// Summary:
//     This macro generates the C++ code necessary for a CXTPControl-derived class
...
// </code>
// See Also:
//     DECLARE_XTP_CONTROL, DECLARE_XTP_COMMANDBAR
//-----------------------------------------------------------------------
...
//-----------------------------------------------------------------------
// Summary:
//     This macro generates the C++ header code necessary for a CXTPCommandBar-derived
//     class that can be serialized.
// Parameters:
//     class_name - The actual name of the class (not enclosed in quotation marks).
// Remarks:
...
// </code>
// See Also:
//     IMPLEMENT_XTP_COMMANDBAR, DECLARE_XTP_CONTROL
//-----------------------------------------------------------------------
...
//-----------------------------------------------------------------------
// Summary:
//     This macro generates the C++ code necessary for a CXTPCommandBar-derived
//     class that can be serialized.
// Parameters:
//     class_name      - The actual name of the class (not enclosed in quotation marks).
//     base_class_name - The name of the base class (not enclosed in quotation marks).
...
// </code>
// See Also:
//     DECLARE_XTP_COMMANDBAR, DECLARE_XTP_CONTROL
//-----------------------------------------------------------------------
...
#		undef _XTP_COMMANDBARS_ACTIVEX
#	endif
//}}AFX_CODEJOCK_PRIVATE
...
//-----------------------------------------------------------------------
// Summary:
//     Call this member to determine if the specified control type is pop-up control,
//     a button pop-up control, or a split button pop-up control.
// Parameters:
//     controlType - Control type.
// Returns:
//     TRUE if the specified control type is pop-up control, a button pop-up control,
//     or a split button pop-up control. FALSE otherwise.
//-----------------------------------------------------------------------
...
//     left, right, or is a pop-up toolbar.
// Parameters:
//     barPosition - Bar position.
// Returns:
//     TRUE if the specified bar position is left, right, or is a
//     pop-up toolbar. FALSE otherwise.
//-----------------------------------------------------------------------
