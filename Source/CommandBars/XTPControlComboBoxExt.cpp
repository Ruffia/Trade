...
void CXTPControlSizeComboBox::EnumFontSizes(CDC& dc, LPCTSTR pFontName)
{
	...
	::EnumFontFamilies(dc.m_hDC, pFontName,
					   reinterpret_cast<FONTENUMPROC>(reinterpret_cast<LPVOID>(EnumSizeCallBack)),
					   XTPToLPARAM(this));
}
																   ...
																   int FontType, LPVOID lpv)
{
	...
	if ((FontType & TRUETYPE_FONTTYPE)
		|| !((FontType & TRUETYPE_FONTTYPE)
			 || (FontType & RASTER_FONTTYPE))) // if truetype or vector font
	{
...
CXTPControlFontComboBoxList::CFontDesc::CFontDesc(LPCTSTR lpszName, LPCTSTR lpszScript,
												  BYTE nCharSet, BYTE nPitchAndFamily,
												  DWORD dwFlags)
{
	m_strName		  = lpszName;
		...
		if (bWin4)
		{
			::EnumFontFamiliesEx(dc, &lf,
								 reinterpret_cast<FONTENUMPROC>(
									 reinterpret_cast<LPVOID>(EnumFamScreenCallBackEx)),
								 XTPToLPARAM(this), NULL);
		}
		else
		{
			::EnumFontFamilies(dc, NULL,
							   reinterpret_cast<FONTENUMPROC>(
								   reinterpret_cast<LPVOID>(EnumFamScreenCallBack)),
							   XTPToLPARAM(this));
		}
	}
}
void CXTPControlFontComboBoxList::CFontDescHolder::AddFont(ENUMLOGFONT* pelf, DWORD dwType,
														   LPCTSTR lpszScript)
{
	...
	CFontDesc* pDesc = new CFontDesc(lf.lfFaceName, lpszScript, lf.lfCharSet, lf.lfPitchAndFamily,
									 dwType);
...
BOOL CALLBACK AFX_EXPORT CXTPControlFontComboBoxList::CFontDescHolder::EnumFamScreenCallBack(
	ENUMLOGFONT* pelf, NEWTEXTMETRICEX* /*lpntm*/, int FontType, LPVOID pThis)
{
	// don't put in non-printer raster fonts
...
BOOL CALLBACK AFX_EXPORT CXTPControlFontComboBoxList::CFontDescHolder::EnumFamScreenCallBackEx(
	ENUMLOGFONTEX* pelf, NEWTEXTMETRICEX* /*lpntm*/, int FontType, LPVOID pThis)
{
	...
	if (FontType & RASTER_FONTTYPE)
		return 1;
	DWORD dwData = DWORD((FontType & TRUETYPE_FONTTYPE) ? TT_FONT : 0);
	((CXTPControlFontComboBoxList::CFontDescHolder*)pThis)
		->AddFont((ENUMLOGFONT*)pelf, dwData, CString((TCHAR*)pelf->elfScript));
	return 1;
}
...
void CXTPControlFontComboBox::OnExecute()
{
	...
	if (!str.IsEmpty())
	{
		...
		if (nIndex != CB_ERR)
		{
			CXTPControlFontComboBoxList::CFontDesc* pDesc =
				(CXTPControlFontComboBoxList::CFontDesc*)GetItemData(nIndex);
			ASSERT(pDesc != NULL);
			if (!pDesc)
				return;
...
void CXTPControlFontComboBoxList::EnumFontFamiliesEx(BOOL dwStyleListBox)
{
	...
	CXTPControlFontComboBoxList::CFontDescHolder& s_fontHolder =
		CXTPSingleton<CXTPControlFontComboBoxList::CFontDescHolder>::Instance();
