...
public:
	//-----------------------------------------------------------------------
	// Summary:
	//     Either reads options from or writes options to an archive.
	// Parameters:
	...
	//-----------------------------------------------------------------------
	// Summary:
	//     Call this method to enable/disable menu underlines.
	// Parameters:
	//     keyboardCues - Determines if menu underlines are enabled.
	// Remarks:
	...
	//--------------------------------------------------------------------
	// Summary:
	//     Gets/sets DPI bitmap scaling flag. If enabled, all raster icons and bitmaps
	...
	// 	   derived from CXTPDpi::IsDpiBitmapScalingEnabled. Once changed the new value will be used.
	// Parameters:
	...
	void EnableDpiBitmapScaling(
		BOOL bEnable = TRUE); // <combine CXTPCommandBarsOptions::IsDpiBitmapScalingEnabled@const>

	//--------------------------------------------------------------------
	// Summary:
	//     Gets/sets DPI gallery item scaling flag. If enabled, the gallery
	//     items will be scaled automaticaly according to the current DPI
	//     scaling.
	// Parameters:
	// 	   bEnable - TRUE to enable gallery item scaling, FALSE to disable.
	// Returns:
	//     The current value of the DPI gallery item scaling.
	//--------------------------------------------------------------------
	BOOL IsDpiGalleryItemScalingEnabled() const;
	void EnableDpiGalleryItemScaling(
		BOOL bEnable = TRUE); // <combine
							  // CXTPCommandBarsOptions::IsDpiGalleryItemScalingEnabled@const>

	//{{AFX_CODEJOCK_PRIVATE
	_XTP_DEPRECATED_IN_FAVOR2(EnableDpiGalleryItemScaling, EnableDpiBitmapScaling)
	void SetDPIScallingOptions(BOOL bScaleGalleryItems, BOOL bScaleToolbarIcons = TRUE);

	_XTP_DEPRECATED_IN_FAVOR(IsDpiGalleryItemScalingEnabled)
...
public:
	...
	BOOL bAltDragCustomization; // Allows users to change a toolbar button's position by dragging it
								// while holding down ALT.
	BOOL bAutoHideUnusedPopups; // TRUE to hide unused pop-up controls.  This is similar to how the
								// Windows Start menu hides infrequently used pop-ups.
	BOOL bDirtyState; // TRUE if the user has done any type of customization to menus or toolbars.
					  // (I.e. Moving a toolbar or adding a new button).  After a load or save of
					  // the BarStae this is set to FALSE.
	BOOL bShowPopupBarToolTips; // TRUE to display ToolTips for controls in pop-up menus.

	BOOL bShowContextMenuAccelerators; // TRUE to show accelerators in context menus.

	XTPAnimationType animationType; // Animation type.
	BOOL bAutoUpdateShortcuts;		// TRUE to update shortcuts from accelerator table.
	DWORD keyboardCuesUse;			// Keyboard cues used.
	...
	CSize szPopupIcons; // Size of icons displayed in pop-up menus.

	BOOL bUseAltNumPadKeys;			 // TRUE to allow activate menu with Alt + num pad key.
	BOOL bUseSystemSaveBitsStyle;	// TRUE to use CS_SAVEBITS class style for pop-ups and shadows.
	BOOL bDisableCommandIfNoHandler; // TRUE to disable commands if no handler is presented.
	BOOL bWrapLargePopups;			 // Wrap controls for large pop-ups instead scroll them.
	...
	BOOL bShowMiniToolbarTips; // TRUE to show tips for controls in a mini toolbar, default is
							   // FALSE.
	BOOL bShowPopupButtonsSelectedToolbarOnly; // TRUE to show only popup with current icons only

protected:
	// DPI-scalling parameters
	BOOL m_bAutoScaleDpiGalleryItems; // TRUE by default. Enables scaling of ribbon gallery items
									  // (both inline and displayed in pop-up).
	BOOL m_bAutoScaleDpiToolbarIcons; // FALSE by default. Enables scaling of toolbar (on ribbon as
									  // well) buttons icons.
...
public:
	...
	afx_msg void OleSetShowPopupButtonsSelectedToolbarOnly(BOOL bVal);
	afx_msg BOOL OleGetShowPopupButtonsSelectedToolbarOnly();
