...
protected:
	...
	{
		CXTPTabManagerItem* pItem = m_pParent ? ((CXTPTabToolBar*)m_pParent)->GetSelectedItem()
											  : NULL;
...
CXTPTabManagerItem* CXTPTabToolBar::InsertCategory(int nItem, LPCTSTR lpszItem, UINT nIDResource,
												   BOOL bLoadIcons)
{
	...
	CXTPTabManagerItem* pItem = InsertCategory(nItem, lpszItem, pItems, pData->wItemCount,
											   bLoadIcons);
	...
	if (bLoadIcons)
	{
		...
		pImageManager->SetIcons(nIDResource, pItems, pData->wItemCount,
...
CXTPTabManagerItem* CXTPTabToolBar::InsertCategory(int nItem, LPCTSTR lpszItem, UINT* pButtons,
												   int nCount, BOOL bLoadIcons)
{
...
#ifdef _XTP_COMMANDBARS_ACTIVEX
		if (pItem)
			FireEvent(1, EVENT_PARAM(VTS_DISPATCH), XTPGetDispatch(pItem, FALSE));
#endif
	}
}
...
{
	GetPaintManager()->SetPosition(dwMode & LM_VERTDOCK ? xtpTabPositionLeft : xtpTabPositionTop);
	...
	if (pPopupExp && ((pPopupExp->GetHideFlags() & xtpHideGeneric) == 0))
	{
		...
		CXTPCommandBar::GetPaintManager()->DrawSpecialControl(0, xtpButtonExpandToolbar, pPopupExp,
															  this, FALSE, &sz);
	}
	...
	if (dwMode & LM_COMMIT)
	{
		...
		if (dwMode & LM_VERTDOCK)
			m_rcTabControl = CRect(rcBorderOriginal.left, rcBorderOriginal.top, rc.left,
								   sz.cy - nExpandBorder);
		else
			m_rcTabControl = CRect(rcBorderOriginal.left, rcBorderOriginal.top,
								   sz.cx - nExpandBorder, rc.top);
...
CSize CXTPTabToolBar::CalcDynamicLayout(int nLength, DWORD nMode)
{
	...
	CRect rcBorderOriginal(XTP_DPI_X(3), XTP_DPI_Y(3) + szBar.cy + XTP_DPI_Y(1), XTP_DPI_X(3),
						   XTP_DPI_Y(3));
	...
	{
		m_rcTabControl = CRect(rcBorderOriginal.left, rcBorderOriginal.top,
							   sz.cx - rcBorderOriginal.right, rcBorderOriginal.top + rc.top);
...
void CXTPTabToolBar::OnCustomizeDrop(CXTPControl* pDataObject, DROPEFFECT& dropEffect,
									 CPoint ptDrop, CPoint ptDrag)
{
	...
	CXTPControl* pDragControl = pCommandBars
									? DYNAMIC_DOWNCAST(CXTPControl, pCommandBars->GetDragControl())
									: NULL;
...
BOOL CXTPTabToolBar::DrawIcon(CDC* pDC, CPoint pt, CXTPTabManagerItem* pItem, BOOL bDraw,
							  CSize& szIcon) const
{
	if (!pItem)
	...
	CXTPImageManagerIcon* pImage = GetImageManager()->GetImage(XTPToUInt(pItem->GetImageIndex()),
															   szIcon.cx);
...
BEGIN_DISPATCH_MAP(CXTPTabToolBar, CXTPToolBar)
	DISP_FUNCTION_ID(CXTPTabToolBar, "InsertCategory", 100, OleInsertCategory, VT_DISPATCH,
					 VTS_I4 VTS_BSTR)
	DISP_PROPERTY_EX_ID(CXTPTabToolBar, "CategoryCount", 101, OleGetItemCount, SetNotSupported,
						VT_I4)
	...
	DISP_PROPERTY_EX_ID(CXTPTabToolBar, "TabPaintManager", 103, OleGetTabPaintManager,
