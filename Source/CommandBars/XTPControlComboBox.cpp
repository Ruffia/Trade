...
void CXTPControlComboBox::OnEditChanged()
{
	...
	if (!m_bIgnoreSelection && m_bAutoComplete && !m_bIgnoreAutoComplete && pEdit
		&& pEdit->GetSafeHwnd() && pPopupBar)
	{
		CString strWindowText = _GetEditText();
...
BOOL CXTPControlComboBox::OnHookMouseWheel(UINT /*nFlags*/, short zDelta, CPoint /*pt*/)
{
	...
	((CXTPControlComboBoxPopupBar*)m_pCommandBar)
		->ProcessHookKeyDown(this, UINT(zDelta > 0 ? VK_UP : VK_DOWN), 0);
	return TRUE;
}
...
BOOL CXTPControlComboBox::OnHookKeyDown(UINT nChar, LPARAM lParam)
{
	...
	if (nChar == VK_TAB
		&& ((m_pAutoCompleteWnd == NULL) || ((m_dwShellAutoCompleteFlags & SHACF_USETAB) == 0)))
	...
	if ((!m_pEdit
		 || (nChar == VK_UP || nChar == VK_DOWN || nChar == VK_NEXT || nChar == VK_PRIOR)
...
void CXTPControlComboBox::EnableShellAutoComplete(
	DWORD dwFlags /*= SHACF_FILESYSTEM | SHACF_URLALL*/)
{
...
void CXTPControlComboBox::SetRect(CRect rcControl)
{
	...
	if (m_pEdit)
	{
		...
		rcControl.DeflateRect(m_nLabelWidth + XTP_DPI_X(1 + 3), XTP_DPI_Y(3),
							  m_nThumbWidth + XTP_DPI_X(3), XTP_DPI_Y(3));
...
void CXTPControlComboBox::SetParent(CXTPCommandBar* pParent)
{
	if (pParent != m_pParent && pParent)
	{
		...
		if (pSite)
			SetWindowLongPtr(m_pCommandBar->GetSafeHwnd(), GWLP_HWNDPARENT,
							 (LONG_PTR)pSite->GetSafeHwnd());
	}
...
{
	return IsFocused() || (m_pCommandBar->IsTrackingMode())
		   || m_pParent->GetCommandBars()->GetMouseManager()->IsMouseLocked();
}
...
void CXTPControlComboBox::OnClick(BOOL bKeyboard, CPoint pt)
{
	...
	else if (m_bPopup && IsFocused())
	{
		...
		CXTPControlPopup::OnClick(bKeyboard, pt);
}
...
{
	return m_bPopup;
	// return m_pCommandBar->IsTrackingMode() && ((CXTPPopupBar*)m_pCommandBar)->GetControlPopup()
	// == (CXTPControlPopup*)this;
}
...
BOOL CXTPControlComboBox::OnSetPopup(BOOL bPopup)
{
	...
	if (m_pAutoCompleteWnd && bPopup)
		m_pAutoCompleteWnd->CloseWindow();

	m_bPopup = bPopup;
	if (bPopup && GetEditCtrl())
...
{
	if (m_pParent)
		GetCommandBar()->SetCommandBars(m_pParent->GetCommandBars());
...
void CXTPControlComboBox::DeflateEditRect(CRect& rcControl)
{
	if (m_nEditIconId > 0)
	{
		CXTPImageManagerIcon* pIcon =
			m_pParent->GetImageManager()->GetImage(XTPToUInt(m_nEditIconId), 0);
		if (pIcon)
...
void CXTPControlComboBox::DrawEditText(CDC* pDC, CRect rcText)
{
	...
	if (m_nEditIconId > 0)
	{
		...
		if (pIcon)
		{
			...
			pIcon->Draw(pDC, CPoint(rcText.left, rcText.CenterPoint().y - sz.cy / 2),
						GetEnabled() ? xtpImageNormal : xtpImageDisabled, sz);
	...
	{
		pDC->SetTextColor(!GetEnabled() ? clrEditTextDisabled
										: IsFocused() ? clrEditTextHighlighted : clrEditTextNormal);
...
void CXTPControlComboBox::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	...
	COLORREF crOldTextColor = pDC->SetTextColor(
		pPaintManager->GetRectangleTextColor(bSelected, FALSE, TRUE, FALSE, FALSE, xtpBarTypePopup,
											 xtpBarPopup));

	if (bSelected)
		pPaintManager->DrawRectangle(pDC, rc, TRUE, FALSE, TRUE, FALSE, FALSE, xtpBarTypePopup,
									 xtpBarPopup);
	else
	...
	if (nListIconId != -1)
	{
		...
		CXTPImageManagerIcon* pImage	= pImageManager->GetImage(nListIconId
																	  + lpDrawItemStruct->itemID,
																  0);

		if (pImage)
		{
			CSize sz = CXTPPaintManager::AdjustDpiIconSize(pImage, pImage->GetExtent(), this,
														   rc.Size());
			pImage->Draw(pDC, CPoint(rc.left + XTP_DPI_X(2), (rc.top + rc.bottom - sz.cy) / 2),
						 (bSelected ? xtpImageHot : xtpImageNormal), sz);
...
BEGIN_DISPATCH_MAP(CXTPControlComboBox, CXTPControlPopup)
	DISP_FUNCTION_ID(CXTPControlComboBox, "AddItem", dispidAddItem, OleAddItem, VT_EMPTY,
					 VTS_BSTR VTS_VARIANT)
	DISP_PROPERTY_EX_ID(CXTPControlComboBox, "DropDownWidth", dispidDropDownWidth, GetDropDownWidth,
						OleSetDropDownWidth, VT_I4)
	DISP_PROPERTY_EX_ID(CXTPControlComboBox, "DropDownListStyle", dispidDropDownListStyle,
						GetDropDownListStyle, OleSetDropDownListStyle, VT_BOOL)
	...
	DISP_FUNCTION_ID(CXTPControlComboBox, "ListCount", dispidListCount, OleGetCount, VT_I4,
					 VTS_NONE)
	DISP_FUNCTION_ID(CXTPControlComboBox, "RemoveItem", dispidRemoveItem, OleRemoveItem, VT_EMPTY,
					 VTS_I4)

	DISP_PROPERTY_EX_ID(CXTPControlComboBox, "Text", dispidText, OleGetText, OleSetText, VT_BSTR)
	DISP_PROPERTY_EX_ID(CXTPControlComboBox, "ListIndex", dispidListIndex, OleGetListIndex,
						OleSetListIndex, VT_I4)
	DISP_FUNCTION_ID(CXTPControlComboBox, "FindItem", dispidFindItem, OleFindItem, VT_I4, VTS_BSTR)
	DISP_PROPERTY_PARAM_ID(CXTPControlComboBox, "ItemData", 62, OleGetItemData, OleSetItemData,
						   VT_I4, VTS_I4)
	DISP_PROPERTY_EX_ID(CXTPControlComboBox, "EditHandle", 63, OleGetEditHandle, SetNotSupported,
						VT_HANDLE)
	...
	DISP_PROPERTY_EX_ID(CXTPControlComboBox, "ShellAutoComplete", 69, OleGetShellAutoComplete,
						OleSetShellAutoComplete, VT_I4)
	DISP_PROPERTY_EX_ID(CXTPControlComboBox, "EditIconId", 70, OleGetEditIconId, OleSetEditIconId,
						VT_I4)
	...
	DISP_PROPERTY_EX_ID(CXTPControlComboBox, "ThemedItems", 73, OleGetThemedItems,
						OleSetThemedItems, VT_BOOL)
	DISP_PROPERTY_EX_ID(CXTPControlComboBox, "ListIconId", 74, OleGetListIconId, OleSetListIconId,
						VT_I4)
...
void CXTPControlComboBox::OleAddItem(LPCTSTR strText, const VARIANT* pIndex)
{
	...
	if (pIndex
		&& (pIndex->vt == VT_I4 || pIndex->vt == VT_I2 || pIndex->vt == (VT_VARIANT | VT_BYREF)))
