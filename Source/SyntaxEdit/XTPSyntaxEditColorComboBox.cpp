...
#define COLOR_ITEM_WIDTH XTP_DPI_X(11)
...
int CXTPSyntaxEditColorComboBox::DeleteColor(COLORREF crColor)
{
	...
	if (iIndex != CB_ERR)
	{
		return DeleteString(XTPToUIntChecked(iIndex));
	}
...
bool CXTPSyntaxEditColorComboBox::Init()
{
	...
	SetItemHeight(-1, XTPToUInt(::GetSystemMetrics(SM_CYVTHUMB) - ::GetSystemMetrics(SM_CYEDGE)));
...
int CXTPSyntaxEditColorComboBox::SetUserColor(COLORREF crColor, LPCTSTR lpszUserText /*=NULL*/)
{
	...
	{
		XTPResourceManager()->LoadString(&csCustom, XTP_IDS_EDIT_CUSTOM);
	}
	else
	{
...
int CXTPSyntaxEditColorComboBox::SetAutoColor(COLORREF crColor, LPCTSTR lpszAutoText /*=NULL*/)
{
	...
	{
		XTPResourceManager()->LoadString(&csAuto, XTP_IDS_CLR_AUTOMATIC);
...
void CXTPSyntaxEditColorComboBox::DrawItem(LPDRAWITEMSTRUCT lpDIS)
{
	...
	CRect rColor(rcItem);
	rColor.DeflateRect(XTP_DPI_X(2), XTP_DPI_Y(2));
	...
	pDC->Draw3dRect(rColor, GetXtremeColor(UINT(bDisabled ? COLOR_GRAYTEXT : COLOR_WINDOWTEXT)),
					GetXtremeColor(UINT(bDisabled ? COLOR_GRAYTEXT : COLOR_WINDOWTEXT)));

	// determine the size of the text display.
	CRect rText(rColor);
	rText.top -= XTP_DPI_Y(2);
	rText.bottom = rText.top + (::GetSystemMetrics(SM_CYVTHUMB) - ::GetSystemMetrics(SM_CYEDGE));
	rText.left   = rText.right + XTP_DPI_X(4);
	rText.right  = rcItem.right;

	// draw text.
	CString csItemText;
	GetLBText(XTPToInt(itemID), csItemText);
...
void CXTPSyntaxEditColorComboBox::MeasureItem(LPMEASUREITEMSTRUCT lpMIS)
{
	...
	lpMIS->itemHeight = XTPToUInt(::GetSystemMetrics(SM_CYVTHUMB) - ::GetSystemMetrics(SM_CYEDGE));
	lpMIS->itemWidth  = 0;
}
	...
	if (GetDroppedState() == TRUE)
	{
		...
		if (::IsWindow(hWnd))
		{
			...
			::RedrawWindow(hWnd, NULL, (HRGN)NULL,
						   RDW_UPDATENOW | RDW_ALLCHILDREN | RDW_FRAME | RDW_INVALIDATE
							   | RDW_ERASE);
...
_XTP_EXT_CLASS void AFXAPI DDX_CBSyntaxColor(CDataExchange* pDX, int nIDC, COLORREF& value)
{
	...
	CXTPSyntaxEditColorComboBox* pColorCombo = (CXTPSyntaxEditColorComboBox*)CWnd::FromHandle(
		hWndCtrl);
