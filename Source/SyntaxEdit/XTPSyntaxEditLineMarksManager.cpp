...
{
	return m_eType == xtpEditLMPT_DWORD || m_eType == xtpEditLMPT_double
		   || m_eType == xtpEditLMPT_Ptr;
}
...
int CXTPSyntaxEditLineMarksManager::CLineMarksList::FindLowerIndex(const int nKey) const
{
	...
	while (nRightIndex >= nLeftIndex)
	{
		...
		XTP_EDIT_LMDATA* pMediumNextData = nMediumIndex + 1 < nCount
											   ? m_array.GetAt(nMediumIndex + 1)
											   : NULL;

		if (nKey >= pMediumData->m_nRow && (!pMediumNextData || nKey < pMediumNextData->m_nRow))
		{
			return nMediumIndex;
		}
		else if (nKey > pMediumData->m_nRow)
...
int CXTPSyntaxEditLineMarksManager::CLineMarksList::FindUpperIndex(const int nKey) const
{
	...
	while (nRightIndex >= nLeftIndex)
	{
		...
		XTP_EDIT_LMDATA* pMediumPrevData = nMediumIndex > 0 ? m_array.GetAt(nMediumIndex - 1)
															: NULL;

		if (nKey <= pMediumData->m_nRow && (!pMediumPrevData || nKey > pMediumPrevData->m_nRow))
		{
			return nMediumIndex;
		}
		else if (nKey > pMediumData->m_nRow)
...
void CXTPSyntaxEditLineMarksManager::CLineMarksList::RefreshLineMarks(int nRowFrom, int nRowTo,
																	  int nRefreshType)
{
	...
	for (int nIndex = 0; nIndex < nCount; nIndex++)
	{
		...
		if (pData->m_nRow < nRowL || pData->m_nRow == nRowL && !bDel_only1 && !bInsertAt0)
		{
		...
		else if (bDel && pData->m_nRow < nRowU || bDel_only1 && pData->m_nRow == nRowL
...
XTP_EDIT_LMDATA* CXTPSyntaxEditLineMarksManager::CLineMarksList::GetNextLineMark(POSITION& pos) const
{
	...
	XTP_EDIT_LMDATA* pData = (INT_PTR)pos > (INT_PTR)m_array.GetSize()
								 ? NULL
								 : m_array.GetAt((INT_PTR)pos - 1);
...
XTP_EDIT_LMDATA*
	CXTPSyntaxEditLineMarksManager::CLineMarksList::GetLineMarkAt(const POSITION pos) const
{
	// Rule: pos == element index + 1  (because 0=NULL and mean no more elements)
...
void CXTPSyntaxEditLineMarksManager::CLineMarksListsMap::RefreshLineMarks(int nRowFrom, int nRowTo,
																		  int nRefreshType)
{
...
POSITION CXTPSyntaxEditLineMarksManager::FindPrevLineMark(int& nRow,
														  const XTP_EDIT_LINEMARKTYPE lmType) const
{
	int nPrevRow	 = -1;
...
POSITION CXTPSyntaxEditLineMarksManager::FindNextLineMark(int& nRow,
														  const XTP_EDIT_LINEMARKTYPE lmType) const
{
...
XTP_EDIT_LMDATA* CXTPSyntaxEditLineMarksManager::GetNextLineMark(
	POSITION& pos, const XTP_EDIT_LINEMARKTYPE lmType) const
{
	CLineMarksListPtr ptrList = m_mapLists.GetList(lmType);
...
XTP_EDIT_LMDATA* CXTPSyntaxEditLineMarksManager::GetLineMarkAt(
	const POSITION pos, const XTP_EDIT_LINEMARKTYPE lmType) const
{
	// get corresponding list
	CLineMarksListPtr ptrList = m_mapLists.GetList(lmType);
...
BEGIN_INTERFACE_MAP(CXTPSyntaxEditLineMarksManager, CXTPCmdTarget)
	INTERFACE_PART(CXTPSyntaxEditLineMarksManager, XTPDIID_SyntaxEditLineMarksManager, Dispatch)
END_INTERFACE_MAP()

IMPLEMENT_OLETYPELIB_EX(CXTPSyntaxEditLineMarksManager, XTPDIID_SyntaxEditLineMarksManager)

BEGIN_DISPATCH_MAP(CXTPSyntaxEditLineMarksManager, CXTPCmdTarget)

	DISP_FUNCTION_ID(CXTPSyntaxEditLineMarksManager, "AddRemoveLineMark", 1, OleAddRemoveLineMark,
					 VT_EMPTY, VTS_I4 VTS_BSTR)
	DISP_FUNCTION_ID(CXTPSyntaxEditLineMarksManager, "SetLineMark", 2, OleSetLineMark, VT_EMPTY,
					 VTS_I4 VTS_BSTR)
	DISP_FUNCTION_ID(CXTPSyntaxEditLineMarksManager, "DeleteLineMark", 3, OleDeleteLineMark,
					 VT_EMPTY, VTS_I4 VTS_BSTR)
	DISP_FUNCTION_ID(CXTPSyntaxEditLineMarksManager, "HasRowMark", 4, OleHasRowMark, VT_BOOL,
					 VTS_I4 VTS_BSTR)
	DISP_FUNCTION_ID(CXTPSyntaxEditLineMarksManager, "FindPrevLineMark", 5, OleFindPrevLineMark,
					 VT_I4, VTS_I4 VTS_BSTR)
	DISP_FUNCTION_ID(CXTPSyntaxEditLineMarksManager, "FindNextLineMark", 6, OleFindNextLineMark,
					 VT_I4, VTS_I4 VTS_BSTR)
	DISP_FUNCTION_ID(CXTPSyntaxEditLineMarksManager, "GetLastLineMark", 7, OleGetLastLineMark,
					 VT_I4, VTS_BSTR)
	DISP_FUNCTION_ID(CXTPSyntaxEditLineMarksManager, "GetFirstLineMark", 8, OleGetFirstLineMark,
					 VT_I4, VTS_BSTR)
	DISP_FUNCTION_ID(CXTPSyntaxEditLineMarksManager, "GetLineMarkAt", 9, OleGetLineMarkAt, VT_I4,
					 VTS_I4 VTS_BSTR)
	DISP_FUNCTION_ID(CXTPSyntaxEditLineMarksManager, "RemoveAll", 10, OleRemoveAll, VT_EMPTY,
					 VTS_BSTR)
	DISP_FUNCTION_ID(CXTPSyntaxEditLineMarksManager, "GetCount", 11, OleGetCount, VT_I4, VTS_BSTR)

END_DISPATCH_MAP()

void CXTPSyntaxEditLineMarksManager::OleAddRemoveLineMark(int nRow,
														  const XTP_EDIT_LINEMARKTYPE lmType)
{
	AddRemoveLineMark(nRow, lmType, NULL);
}

void CXTPSyntaxEditLineMarksManager::OleSetLineMark(int nRow, const XTP_EDIT_LINEMARKTYPE lmType)
{
	SetLineMark(nRow, lmType);
}

void CXTPSyntaxEditLineMarksManager::OleDeleteLineMark(int nRow, const XTP_EDIT_LINEMARKTYPE lmType)
{
	DeleteLineMark(nRow, lmType);
}

BOOL CXTPSyntaxEditLineMarksManager::OleHasRowMark(int nRow, const XTP_EDIT_LINEMARKTYPE lmType)
{
	return HasRowMark(nRow, lmType);
}

POSITION CXTPSyntaxEditLineMarksManager::OleFindPrevLineMark(int nRow,
															 const XTP_EDIT_LINEMARKTYPE lmType)
{
	return FindPrevLineMark(nRow, lmType);
}

POSITION CXTPSyntaxEditLineMarksManager::OleFindNextLineMark(int nRow,
															 const XTP_EDIT_LINEMARKTYPE lmType)
{
	return FindNextLineMark(nRow, lmType);
}

POSITION CXTPSyntaxEditLineMarksManager::OleGetLastLineMark(const XTP_EDIT_LINEMARKTYPE lmType)
{
	return GetLastLineMark(lmType);
}

POSITION CXTPSyntaxEditLineMarksManager::OleGetFirstLineMark(const XTP_EDIT_LINEMARKTYPE lmType)
{
	return GetFirstLineMark(lmType);
}

long CXTPSyntaxEditLineMarksManager::OleGetLineMarkAt(const POSITION pos,
													  const XTP_EDIT_LINEMARKTYPE lmType)
{
	XTP_EDIT_LMDATA* pData = GetLineMarkAt(pos, lmType);
	if (pData)
		return pData->m_nRow;
...
void CXTPSyntaxEditLineMarksManager::OleRemoveAll(XTP_EDIT_LINEMARKTYPE lmType)
{
	RemoveAll(lmType);
}

long CXTPSyntaxEditLineMarksManager::OleGetCount(XTP_EDIT_LINEMARKTYPE lmType)
{
	return GetCount(lmType);
}
