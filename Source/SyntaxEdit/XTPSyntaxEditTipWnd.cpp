...
COLORREF CXTPSyntaxEditTipWnd::GetTextColor() const
{
	...
	return ::GetSysColor((m_pListBox->GetSel(m_iIndex) > 0) ? COLOR_HIGHLIGHTTEXT : COLOR_INFOTEXT);
}
...
COLORREF CXTPSyntaxEditTipWnd::GetBackColor() const
{
	...
	return ::GetSysColor((m_pListBox->GetSel(m_iIndex) > 0) ? COLOR_HIGHLIGHT : COLOR_INFOBK);
}
...
BOOL CXTPSyntaxEditTipWnd::IsOwnerDrawn()
{
	...
	{
		return (
			(::GetWindowLong(hWndLBox, GWL_STYLE) & (LBS_OWNERDRAWFIXED | LBS_OWNERDRAWVARIABLE))
			!= 0);
	}
	return FALSE;
}
...
BOOL CXTPSyntaxEditTipWnd::ShowTip(int iIndex)
{
	...
	if (CalcItemRect(iIndex, rItem))
	{
		...
		m_rWindow.InflateRect(XTP_DPI_X(0), XTP_DPI_Y(2));
