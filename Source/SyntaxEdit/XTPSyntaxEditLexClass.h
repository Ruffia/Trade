...
public:
	...
	CXTPSyntaxEditLexVariantPtrArray* m_ptrLVArrayPtr; // Value storage for type <b>
													   // xtpEditLVT_LVArrayPtr </b>

	int m_nValue;			  // Value storage for type <b> xtpEditLVT_valInt </b>
	CStringArray m_arStrVals; // Value storage for types <b> xtpEditLVT_className </b>, <b>
							  // xtpEditLVT_valStr </b>
...
static const TCHAR XTPLEX_ATTR_TXTPREFIX[] = _T("txt:"); // Text properties (like color, bold, ...)
														 // attributes prefix.
static const TCHAR XTPLEX_ATTR_COLORPREFIX[] = _T("txt:color"); // Prefix of properties which value
																// is color.
...
static const TCHAR XTPLEX_ATTR_TXT_COLORSELBK[] = _T("txt:colorSelBK"); // Selected text back ground
																		// color
...
static const TCHAR XTPLEX_ATTRG_PARSERTHREADIDLELIFETIME_SEC[] =
	_T("global:ParserThreadIdleLifeTime_sec"); // default=60 sec; -1 and 0 means infinite; time out
											   // for existing of parser thread when parser idle (no
											   // parse requests).
...
protected:
	XTP_EDIT_LEX_CLASS_OBJ_MEMBER(0,
								  m_previous); //   <b>'previous'</b> Lex Class property collection.
	...
	// internal objects
	XTP_EDIT_LEX_CLASS_OBJ_MEMBER(5,
								  m_ActiveTags); // Active Tags collection. Just only these tags can
												 // start or end current and children Lex Class(es).

	//}}AFX_CODEJOCK_PRIVATE

	int m_nActiv_EndTags_Offset; // First index of end active tags set in the Active Tags collection
								 // (m_ActiveTags member).
...
protected:
	...
	CMap<CString, LPCTSTR, CXTPSyntaxEditLexVariantPtr, CXTPSyntaxEditLexVariantPtr&>
		m_mapAttributes;
	//}}AFX_CODEJOCK_PRIVATE
