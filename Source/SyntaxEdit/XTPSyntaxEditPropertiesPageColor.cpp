...
AFX_STATIC int CALLBACK EnumFontFamExProc(ENUMLOGFONTEX* lpelfe,   // logical-font data
...
CXTPSyntaxEditPropertiesPageColor::CXTPSyntaxEditPropertiesPageColor(
	CXTPSyntaxEditView* pEditView /*=NULL*/)
	: m_bModified(FALSE)
...
BOOL CXTPSyntaxEditPropertiesPageColor::OnInitDialog()
{
	...
	for (POSITION pos = pinfoList->GetHeadPosition(); pos;)
	{
		...
		int iIndex = m_lboxName.AddString((info.csDesc.GetLength() > 0) ? info.csDesc
																		: info.csName);
		if (iIndex != LB_ERR)
		{
...
BOOL CXTPSyntaxEditPropertiesPageColor::WriteRegistryValues()
{
	...
	{
		CString csKey;
		CXTPSyntaxEditLexClassInfoArray* pLexClassArray = NULL;
...
{
	CPropertySheet* pParentDlg = DYNAMIC_DOWNCAST(CPropertySheet, GetParent());

	if (!pParentDlg)
		return;

	CXTPSyntaxEditPropertiesPageFont* pPage2 = DYNAMIC_DOWNCAST(CXTPSyntaxEditPropertiesPageFont,
																pParentDlg->GetPage(1));
...
void CXTPSyntaxEditPropertiesPageColor::OnSelChangeSchemaNames()
{
	...
	if (iIndex != LB_ERR)
	{
		XTP_EDIT_SCHEMAFILEINFO* pSchemaInfo = (XTP_EDIT_SCHEMAFILEINFO*)m_lboxName.GetItemData(
			iIndex);
		if (pSchemaInfo)
		{
...
int CXTPSyntaxEditPropertiesPageColor::PropExists(CXTPSyntaxEditLexPropInfoArray& arrProp,
...
BOOL CXTPSyntaxEditPropertiesPageColor::UpdateColorValue(CXTPSyntaxEditColorComboBox& combo,
														 COLORREF& color, LPCTSTR lpszPropName)
{
...
void CXTPSyntaxEditPropertiesPageColor::OnDblClickSchema()
{
	...
	CString csBuffer;
	XTPResourceManager()->LoadString(&csBuffer, XTP_IDS_EDIT_OPENSCHEMAMSG);
...
void CXTPSyntaxEditPropertiesPageColor::OnContextMenu(CWnd* pWnd, CPoint point)
{
	...
	if (iItem != LB_ERR)
	{
		...
		CString csBuffer;
		XTPResourceManager()->LoadString(&csBuffer, XTP_IDS_EDIT_OPENSCHEMACMD);
