	...
	, m_iHScrollPos(0)
{
	...
	{
		m_bAutoReload = (0
						 != (pWinApp->GetProfileInt(XTP_EDIT_REG_SETTINGS, XTP_EDIT_REG_AUTORELOAD,
													m_bAutoReload)));
	}
...
BOOL CXTPSyntaxEditDoc::CheckFileModified(LPCTSTR lpszPathName)
{
	...
	if (hFound != INVALID_HANDLE_VALUE)
	{
		...
		if (!m_bReloading
			&& ((m_curFileData.ftLastWriteTime.dwLowDateTime
				 != newFileData.ftLastWriteTime.dwLowDateTime)
				|| (m_curFileData.ftLastWriteTime.dwHighDateTime
					!= newFileData.ftLastWriteTime.dwHighDateTime)
...
BOOL CXTPSyntaxEditDoc::SetAutoReload(BOOL bAutoReload, BOOL bUpdateReg /*=FALSE*/)
{
	...
	if (bUpdateReg)
	{
		...
		if (pWinApp != NULL)
		{
			if (pWinApp->WriteProfileInt(XTP_EDIT_REG_SETTINGS, XTP_EDIT_REG_AUTORELOAD,
										 (int)bAutoReload))
				return TRUE;
		}
		return FALSE;
