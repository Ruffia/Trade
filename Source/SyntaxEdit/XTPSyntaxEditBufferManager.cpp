...
BOOL _IsUnicodeFile_heuristic(CFile* pFile)
{
	...
	int nSmallNumbersPc = XTPToIntChecked(nSmallNumbers1 * 100 / max(1, uDataXcount));
...
//===========================================================================
template<class _TFileChar>
BOOL _T_ReadStringFromFile(CFile* pFile, CString& rstrString, int& rnCRLFStyle, UINT uCodePage,
						   _TFileChar c = 0)
{
	UNREFERENCED_PARAMETER(c);
	ASSERT(pFile);
	...
	while ((UINT)pFile->GetPosition() < uFileSize)
	{
		...
		int nBufDataLen = XTPToIntChecked(uBufDataCountB / sizeof(_TFileChar));
...
#pragma warning(pop)
		...
		*pEOL = 0;
		...
		INT_PTR nEolPos = INT_PTR(pEOL - pBuff);
		LONG nSeekBack  = XTPToLong((nEolPos + nCRLFLen) * sizeof(_TFileChar)
									- XTPToIntChecked(uBufDataCountB));
...
int _T_WriteStringToFile(_TFile* pFile, LPCTSTR pszString, int nCRLFStyle, UINT uCodePage,
						 CByteArray* parBuffer = NULL, _TFileChar c = 0)
{
	UNREFERENCED_PARAMETER(c);
...
#pragma warning(pop)
	{
		...
		nFileBufferLenB = XTPToIntChecked(nStrLen * sizeof(_TFileChar));
	}
	else
	{
		...
		ZeroMemory(pFileBuffer, XTPToSizeTChecked(nNeedBufSizeB));
...
#pragma warning(pop)
		{
			int nMBCStextLen = WideCharToMultiByte(uCodePage, 0, (LPCWSTR)pszString, -1,
...
#pragma warning(pop)
		{
			int nWtextLen = MultiByteToWideChar(uCodePage, 0, (LPCSTR)pszString, -1,
	...
	{
		pFile->Write(pFileBuffer, XTPToUIntChecked(nFileBufferLenB));
	}

	if (nCRLFStyle >= 0)
	{
		_TFileChar* tszCRLF = _T_GetCRLF<_TFileChar>(nCRLFStyle);
		int nCRLFlenB		= XTPToIntChecked(_T_StrLen(tszCRLF) * sizeof(_TFileChar));

		pFile->Write(tszCRLF, XTPToUIntChecked(nCRLFlenB));
...
CXTPSyntaxEditBufferManager::CXTPSyntaxEditBufferManager()
{
	...
	if (pWinApp)
	{
		m_nTabSize = XTPToIntChecked(
			pWinApp->GetProfileInt(XTP_EDIT_REG_SETTINGS, XTP_EDIT_REG_TABSIZE, m_nTabSize));
	}

	m_nFileEncoding = XTP_SYNTAXEDIT_FILE_UTF_8;
...
CXTPSyntaxEditBufferManager::~CXTPSyntaxEditBufferManager()
{
	...
	if (!!m_ptrDefaultLexConfigurationManager)
	{
...
void CXTPSyntaxEditBufferManager::SerializeEx(CArchive& ar, BOOL bUnicode,
											  BOOL bWriteUnicodeFilePrefix, UINT nCodePage,
											  LPCTSTR pcszFileExt, int nDataSizeLimit)
{
	...
	if (ar.IsLoading())
	{
		...
		if (bUnicode < 0)
		{
			SetFileEncoding(XTP_SYNTAXEDIT_FILE_UTF_8);

			BYTE buff[3];

			pFile->SeekToBegin();
			UINT uRead = pFile->Read(buff, 3);

			if (uRead == 3 && buff[0] == 0xEF && buff[1] == 0xBB && buff[2] == 0xBF)
			{
				SetFileEncoding(XTP_SYNTAXEDIT_FILE_UTF_8_BOM);
			}
			else
			{
				pFile->SeekToBegin();
				uRead = pFile->Read(buff, 2);

				if (uRead == 2 && buff[0] == 0xFE && buff[1] == 0xFF)
				{
					m_bUnicodeFileFormat = TRUE;
					SetFileEncoding(XTP_SYNTAXEDIT_FILE_UTF_16_BE);
				}

				if (uRead == 2 && buff[0] == 0xFF && buff[1] == 0xFE)
				{
					m_bUnicodeFileFormat = TRUE;
					SetFileEncoding(XTP_SYNTAXEDIT_FILE_UTF_16_LE);
				}
			}

			if (GetFileEncoding() == XTP_SYNTAXEDIT_FILE_UTF_8)
				pFile->SeekToBegin();
		}
		else
		...
		while (
			ReadStringFromFileEx(pFile, strString, nCRLFStyle, m_nCodePage, m_bUnicodeFileFormat))
		{
			if (GetFileEncoding() == XTP_SYNTAXEDIT_FILE_UTF_16_BE)
			{
				BYTE* arr = (BYTE*)strString.GetBuffer(strString.GetLength());

				for (int n = 0; n < strString.GetLength() * 2; n += 2)
				{
					BYTE b	 = arr[n];
					arr[n]	 = arr[n + 1];
					arr[n + 1] = b;
		...
		CString strExt = (NULL != pcszFileExt ? CString(pcszFileExt) : GetFileExtention(pFile));
		if (!strExt.IsEmpty())
			SetFileExt(strExt);
	}
	else if (ar.IsStoring())
	{
		...
		if (bWriteUnicodeFilePrefix)
		{
			if (bUnicode)
			{
				...
				if (GetFileEncoding() == XTP_SYNTAXEDIT_FILE_UTF_16_LE)
				{
					ar << (BYTE)0xFF;
					ar << (BYTE)0xFE;
				}
			...
			else
			{
				...
				if (GetFileEncoding() == XTP_SYNTAXEDIT_FILE_UTF_8_BOM)
				{
					ar << (BYTE)0xEF;
					ar << (BYTE)0xBB;
					ar << (BYTE)0xBF;
		...
		for (int i = 1; i < nRowsCount; i++)
		{
			...
			if (GetFileEncoding() == XTP_SYNTAXEDIT_FILE_UTF_16_BE)
			{
				BYTE* arr = (BYTE*)strLine.GetBuffer(strLine.GetLength());

				for (int n = 0; n < strLine.GetLength() * 2; n += 2)
				{
					BYTE b	 = arr[n];
					arr[n]	 = arr[n + 1];
					arr[n + 1] = b;
			...
			int nWrittenBytes = _T_WriteStringToFileEx(&ar, strLine, nCRLFStyle, m_nCodePage,
													   &arBuffer, bUnicode);
...
void CXTPSyntaxEditBufferManager::SetFileEncoding(int val)
{
	m_nFileEncoding = (XTP_SYNTAXEDIT_FILE_ENCODING)val;
}

int CXTPSyntaxEditBufferManager::GetFileEncoding()
{
	return m_nFileEncoding;
}
	...
	CXTPSyntaxEditConfigurationManager* pMan)
{
	...
	if (m_ptrDefaultLexConfigurationManager.operator!=(pMan))
	{
		m_ptrDefaultLexConfigurationManager->Close();
		m_ptrDefaultLexConfigurationManager = NULL;
	}
	...
	if (pMan)
	{
		...
		if (ptrConn)
		{
			m_pLexConfigManSinkMT->Advise(
				ptrConn, xtpEditClassSchWasChanged,
				CreateNotfySinkClassDelegate(
					this, &CXTPSyntaxEditBufferManager::OnLexConfigManEventHandler));
			m_pLexConfigManSinkMT->Advise(
				ptrConn, xtpEditThemeWasChanged,
				CreateNotfySinkClassDelegate(
					this, &CXTPSyntaxEditBufferManager::OnLexConfigManEventHandler));
			m_pLexConfigManSinkMT->Advise(
				ptrConn, xtpEditAllConfigWasChanged,
				CreateNotfySinkClassDelegate(
					this, &CXTPSyntaxEditBufferManager::OnLexConfigManEventHandler));
		}
	}
...
void CXTPSyntaxEditBufferManager::OnLexConfigManEventHandler(XTP_NOTIFY_CODE Event, WPARAM wParam,
															 LPARAM lParam)
{
	if (!m_bIsParserEnabled)
	{
	...
	if (Event == xtpEditClassSchWasChanged || Event == xtpEditAllConfigWasChanged)
	{
		...
		if (Event == xtpEditClassSchWasChanged)
		{
			...
			if (pTxtSch)
			{
				...
				else
				{
					...
					CString strName_current = ptrTxtSch_current ? ptrTxtSch_current->GetSchName()
																: CString();
		...
		if (bSetTxtSch)
		{
			...
			if (ptrNewTxtSch)
			{
				...
				BOOL bParseInThread		= m_pLexParser->GetSchemaOptions(GetFileExt())
										  ->m_bConfigChangedReparceInSeparateThread;
				if (bParseInThread)
				{
...
BOOL CXTPSyntaxEditBufferManager::InsertText(LPCTSTR szText, int nRow, int nCol, BOOL bCanUndo,
											 XTP_EDIT_LINECOL* pFinalLC)
{
	int nStrLenB = XTPToIntChecked(_tcslen(szText) * sizeof(TCHAR));
	CMemFile memFile((BYTE*)szText, XTPToUIntChecked(nStrLenB));
	CString strString;
	CString strReplased;
	...
	if (_T_ReadStringFromFile<TCHAR>(&memFile, strString, nLastLineEOLstyle, m_nCodePage))
	{
		...
		if (nCol - 1 < nLenC)
		{
			int nPosX = XTPToIntChecked(_tcsnbcnt(strRow1, XTPToSizeTChecked(nCol - 1)));
	...
	{
		if (!m_bOverwrite
			|| strReplased == "") // By Leva: added 2nd condition to eliminate undo bug
		{
			m_pUndoRedoManager->AddCommand(new CXTPSyntaxEditInsertStringCommand(
				this, szText, XTP_EDIT_LINECOL::MakeLineCol(nRow, nCol),
				XTP_EDIT_LINECOL::MakeLineCol(nRowTo, nColTo)));
		}
		else
		{
			m_pUndoRedoManager->SetGroupInsertMode(FALSE);
			m_pUndoRedoManager->AddCommand(new CXTPSyntaxEditReplaceStringCommand(
				this, szText, strReplased, XTP_EDIT_LINECOL::MakeLineCol(nRow, nCol),
				XTP_EDIT_LINECOL::MakeLineCol(nRowTo, nColTo)));
		}
	}
...
BOOL CXTPSyntaxEditBufferManager::InsertTextBlock(LPCTSTR szText, int nRow, int nCol, BOOL bCanUndo,
												  XTP_EDIT_LINECOL* pFinalLC)
{
	int nStrLenB = XTPToIntChecked(_tcslen(szText) * sizeof(TCHAR));
	CMemFile memFile((BYTE*)szText, XTPToUIntChecked(nStrLenB));
	...
	while (_T_ReadStringFromFile<TCHAR>(&memFile, strString, nCRLFStyleX, m_nCodePage))
	{
		...
		if (bCanUndo)
		{
			...
			m_pUndoRedoManager->AddCommand(
				new CXTPSyntaxEditInsertStringCommand(this, strString, posFrom, posTo));
		}
...
void CXTPSyntaxEditBufferManager::GetLineText(int nLine, CString& strText, BOOL bAddCRLF,
											  int nCRLFStyle)
{
...
int CXTPSyntaxEditBufferManager::CalcMaxLineTextLength(int nLineFrom, int nLineTo,
													   BOOL bExpandTabs) const
{
...
BOOL CXTPSyntaxEditBufferManager::DeleteText(int nRowFrom, int nColFrom, int nRowTo, int nColTo,
											 BOOL bCanUndo, BOOL bDispCol)
{
	...
	if (bCanUndo)
	{
		int nPos1 = bDispCol
						? nColFrom
						: StrPosToCol(nRowFrom, nColFrom - 1); // By Leva: moved -1 into brackets
		int nPos2 = bDispCol ? nColTo
							 : StrPosToCol(nRowTo, nColTo - 1); // By Leva: moved -1 into brackets
		...
		if (GetBuffer(nRowFrom, nPos1, nRowTo, nPos2, file))
		{
			...
			m_pUndoRedoManager->AddCommand(new CXTPSyntaxEditDeleteStringCommand(
				this, (LPCTSTR)pBytes,
				XTP_EDIT_LINECOL::MakeLineCol(nRowFrom, bDispCol
															? ColToStrPos(nRowFrom, nColFrom) + 1
															: nColFrom),
				XTP_EDIT_LINECOL::MakeLineCol(nRowTo, bDispCol ? ColToStrPos(nRowTo, nColTo) + 1
															   : nColTo)));
...
BOOL CXTPSyntaxEditBufferManager::RemoveLine(int nRow, BOOL bCanUndo, int nRowsCount)
{
	...
	if (bCanUndo)
	{
		...
		if (GetBuffer(nRow, 1, nRowEnd, 1, file))
		{
			...
			m_pUndoRedoManager->AddCommand(
				new CXTPSyntaxEditDeleteStringCommand(this, (LPCTSTR)pBytes,
...
BOOL CXTPSyntaxEditBufferManager::GetBuffer(int row1, int col1, int row2, int col2, CMemFile& file,
											BOOL bColumnSelection, BOOL bForceDOSStyleCRLF)
{
	...
	for (int i = row1; i <= row2; i++)
	{
		...
		if (bStart || bColumnSelection)
			nStartPos = XTPToIntChecked(_tcsnbcnt(strBuffer, XTPToSizeTChecked(nStartPos)));

		if (bEnd || bColumnSelection)
			nEndPos = XTPToIntChecked(_tcsnbcnt(strBuffer, XTPToSizeTChecked(nEndPos)));

		nSizeCopy = XTPToIntChecked(abs(nEndPos - nStartPos) * sizeof(TCHAR));

		if (nSizeCopy > 0)
			file.Write((LPVOID)((LPCTSTR)strBuffer + nStartPos), XTPToUIntChecked(nSizeCopy));

		if (bColumnSelection)
		{
			// add spaces for the free size of the column block
			int nAddSpaces = abs(col2 - col1)
							 - (int)_tcsnccnt((LPCTSTR)strBuffer + nStartPos,
											  XTPToSizeTChecked(abs(nEndPos - nStartPos)));
			if (nAddSpaces > 0)
			{
...
BOOL CXTPSyntaxEditBufferManager::IsTextCRLF(LPCTSTR szCompText, BOOL bMatchReverse) const
{
	...
	BOOL bIsCRLF = (_tcsncmp(szTextToCompare, szCurCRLF, XTPToSizeTChecked(nCRLFSize)) == 0);

	return bIsCRLF;
}

#ifdef _UNICODE
BOOL CXTPSyntaxEditBufferManager::IsTextCRLF(LPCSTR szCompText, BOOL bMatchReverse) const
{
	...
	BOOL bIsCRLF = (strncmp(szTextToCompare, szCurCRLF, XTPToSizeTChecked(nCRLFSize)) == 0);
...
BOOL CXTPSyntaxEditBufferManager::SetConfigFile(LPCTSTR szPath)
{
	...
	if (m_ptrLexConfigurationManager->m_bConfigFileMode && CString(szPath).IsEmpty())
		return FALSE;
...
CXTPSyntaxEditTextSchema*
	CXTPSyntaxEditBufferManager::GetMasterTextSchema(const CString& strExt) const
{
	if (!m_ptrLexConfigurationManager)
		return NULL;
...
void CXTPSyntaxEditBufferManager::EnableParser(BOOL bEnable)
{
	...
	if (m_bIsParserEnabled)
	{
		...
		XTP_EDIT_LINECOL pos1_0 = { 1, 0 };
		BOOL bParseInThread =
			m_pLexParser->GetSchemaOptions(GetFileExt())->m_bConfigChangedReparceInSeparateThread;
		if (bParseInThread)
		{
			m_pLexParser->StartParseInThread(this, &pos1_0, NULL, 0, TRUE);
		}
...
void CXTPSyntaxEditBufferManager::ChangeCase(int nRow, int nDispFrom, int nDispTo, BOOL bUpper,
											 BOOL bCanUndo)
{
	CString strText(m_Strings.GetStr(nRow));

	int nFrom = XTPToIntChecked(
		_tcsnbcnt(strText, XTPToSizeTChecked(ColToStrPos(nRow, nDispFrom))));

	int nTo = strText.GetLength();
	if (nDispTo <= strText.GetLength() * GetTabSize())
		nTo = XTPToIntChecked(_tcsnbcnt(strText, XTPToSizeTChecked(ColToStrPos(nRow, nDispTo))));
	...
	{
		m_pUndoRedoManager->AddCommand(
			new CXTPSyntaxEditReplaceStringCommand(this, strChanged, strOriginal,
												   XTP_EDIT_LINECOL::MakeLineCol(nRow, nFrom + 1),
												   XTP_EDIT_LINECOL::MakeLineCol(nRow, nTo + 1)));
	}
}

void CXTPSyntaxEditBufferManager::ChangeTabification(int nRow, int nDispFrom, int nDispTo,
													 BOOL bTabify, BOOL bCanUndo)
{
	CString strText(m_Strings.GetStr(nRow));
	...
	{
		m_pUndoRedoManager->AddCommand(
			new CXTPSyntaxEditDeleteStringCommand(this, strOriginal,
												  XTP_EDIT_LINECOL::MakeLineCol(nRow, nFrom + 1),
												  XTP_EDIT_LINECOL::MakeLineCol(nRow, nTo + 1)));
		m_pUndoRedoManager->AddCommand(new CXTPSyntaxEditInsertStringCommand(
			this, strChanged, XTP_EDIT_LINECOL::MakeLineCol(nRow, nFrom + 1),
...
CString* CXTPSyntaxEditBufferManager::CXTPSyntaxEditStringsManager::GetStrData(
	int nRow, BOOL bGrowArrayIfNeed)
{
	CString* pCStr = NULL;
																		   ...
																		   TCHAR chLeftSpaceFiller)
{
	...
	int nInsPos	= XTPToIntChecked(_tcsnbcnt(*pCStr, XTPToSizeTChecked(nPos)));
	...
	if (nFillCount)
	{
		if (chLeftSpaceFiller != _T('\0'))
		{
			...
			*pCStr += strFill;
			nInsPos = XTPToIntChecked(_tcsnbcnt(*pCStr, XTPToSizeTChecked(nPos)));
		}
		else
		{
...
BOOL CXTPSyntaxEditBufferManager::CXTPSyntaxEditStringsManager::DeleteText(int nRow, int nPos,
																		   int nCount)
{
	...
	int nPosX = XTPToIntChecked(_tcsnbcnt(*pCStr, XTPToSizeTChecked(nPos)));
	if (nPosX >= nStrLen0)
		return FALSE;
	...
	int nCountX = XTPToIntChecked(_tcsnbcnt(((LPCTSTR)*pCStr) + nPosX, XTPToSizeTChecked(nCount)));
...
CXTPSyntaxEditBufferManager::CXTPSyntaxEditStringsManager::CStringPtrArray::CStringPtrArray()
{
}
...
void CXTPSyntaxEditBufferManager::CXTPSyntaxEditStringsManager::CStringPtrArray::RemoveAll()
{
...
BEGIN_DISPATCH_MAP(CXTPSyntaxEditBufferManager, CXTPCmdTarget)
	DISP_PROPERTY_EX_ID(CXTPSyntaxEditBufferManager, "FileExt", 1, OleGetFileExt, OleSetFileExt,
						VT_BSTR)
END_DISPATCH_MAP()
