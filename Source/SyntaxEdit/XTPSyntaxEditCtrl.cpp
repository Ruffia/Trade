...
CXTPSyntaxEditCtrl::CImmWrapper::CImmWrapper()
{
	...
	if (m_hImmDll)
	{
		m_pfnImmIsIME	  = XTPToFunctionPtr<PFN_ImmIsIME>(GetProcAddress(m_hImmDll, "ImmIsIME"));
		m_pfnImmGetContext = XTPToFunctionPtr<PFN_ImmGetContext>(
			GetProcAddress(m_hImmDll, "ImmGetContext"));
		m_pfnImmReleaseContext = XTPToFunctionPtr<PFN_ImmReleaseContext>(
			GetProcAddress(m_hImmDll, "ImmReleaseContext"));
		m_pfnImmSetCompositionFont = XTPToFunctionPtr<PFN_ImmSetCompositionFont>(
			GetProcAddress(m_hImmDll, XTP_PROC_NAME_AW("ImmSetCompositionFont")));
		m_pfnImmSetCompositionWindow = XTPToFunctionPtr<PFN_ImmSetCompositionWindow>(
			GetProcAddress(m_hImmDll, "ImmSetCompositionWindow"));
	}
	else
	{
		m_pfnImmIsIME				 = NULL;
...
BOOL CXTPSyntaxEditCtrl::CImmWrapper::ImmSetCompositionWindow(XTP_HIMC hIMC,
...
//===========================================================================
class CXTPSyntaxEditCtrl::CScreenSearchCache
	: public CArray<CXTPSyntaxEditCtrl::CScreenSearchBlock, CXTPSyntaxEditCtrl::CScreenSearchBlock&>
{
	typedef CArray<CXTPSyntaxEditCtrl::CScreenSearchBlock, CXTPSyntaxEditCtrl::CScreenSearchBlock&>
		Base;
...
#define MARGIN_LENGTH XTP_DPI_X(20)
#define NODES_WIDTH XTP_DPI_X(10)
#define TEXT_LEFT_OFFSET XTP_DPI_X(4)
...
CXTPSyntaxEditCtrl::CXTPSyntaxEditCtrl()
	...
	, m_ptPrevMouse(CPoint(XTP_DPI_X(0), XTP_DPI_Y(0)))
	, m_pBuffer(NULL)
	, m_pParentWnd(NULL)
{
	...
	m_nHScrollMaxWidth	 = XTP_DPI_X(0);
	...
	if (pWinApp != NULL)
	{
		m_bVertScrollBar = (0
							!= (pWinApp->GetProfileInt(XTP_EDIT_REG_SETTINGS,
													   XTP_EDIT_REG_VSCROLLBAR, m_bVertScrollBar)));
		m_bHorzScrollBar = (0
							!= (pWinApp->GetProfileInt(XTP_EDIT_REG_SETTINGS,
													   XTP_EDIT_REG_HSCROLLBAR, m_bHorzScrollBar)));

		m_pOptions->m_bSyntaxColor  = (0
									   != (pWinApp->GetProfileInt(XTP_EDIT_REG_SETTINGS,
																  XTP_EDIT_REG_SYNTAXCOLOR,
																  m_pOptions->m_bSyntaxColor)));
		m_pOptions->m_bAutoIndent   = (0
									   != (pWinApp->GetProfileInt(XTP_EDIT_REG_SETTINGS,
																  XTP_EDIT_REG_AUTOINDENT,
																  m_pOptions->m_bAutoIndent)));
		m_pOptions->m_bSelMargin	= (0
									   != (pWinApp->GetProfileInt(XTP_EDIT_REG_SETTINGS,
																  XTP_EDIT_REG_SELMARGIN,
																  m_pOptions->m_bSelMargin)));
		m_pOptions->m_bLineNumbers  = (0
									   != (pWinApp->GetProfileInt(XTP_EDIT_REG_SETTINGS,
																  XTP_EDIT_REG_LINENUMBERS,
																  m_pOptions->m_bLineNumbers)));
		m_pOptions->m_bWideCaret	= (0
									   != (pWinApp->GetProfileInt(XTP_EDIT_REG_SETTINGS,
																  XTP_EDIT_REG_WIDECARET,
																  m_pOptions->m_bWideCaret)));
		m_pOptions->m_bTabWithSpace = (0
									   != (pWinApp->GetProfileInt(XTP_EDIT_REG_SETTINGS,
																  XTP_EDIT_REG_TABWITHSPACE,
																  m_pOptions->m_bTabWithSpace)));
		m_pOptions->m_bVirtualSpace = (0
									   != (pWinApp->GetProfileInt(XTP_EDIT_REG_SETTINGS,
																  XTP_EDIT_REG_VIRTUALSPACE,
																  m_pOptions->m_bVirtualSpace)));
		m_pOptions->m_bDrawNodes	= (0
									   != (pWinApp->GetProfileInt(XTP_EDIT_REG_SETTINGS,
																  XTP_EDIT_REG_COLLAPSIBLENODES,
																  m_pOptions->m_bDrawNodes)));
...
BOOL CXTPSyntaxEditCtrl::SetValueInt(LPCTSTR lpszValue, int nNewValue, int& nRefValue,
									 BOOL bUpdateReg)
{
	nRefValue = nNewValue;
...
BOOL CXTPSyntaxEditCtrl::SetValueBool(LPCTSTR lpszValue, BOOL bNewValue, BOOL& bRefValue,
									  BOOL bUpdateReg)
{
	bRefValue = bNewValue;
...
void CXTPSyntaxEditCtrl::OnPaint()
{
	...
	else
	{
		...
		if (!IsWindowEnabled())
		{
			XTPImageManager()->DisableBitmap(memDC, rcClient, XTP_EDIT_DISABLED_COLOR_LIGHT,
											 XTP_EDIT_DISABLED_COLOR_DARK);
		}
		...
		CPoint ptCaret  = m_pDrawTextProcessor->SetCaretPos(this, szCaret,
															max(0, GetCurrentVisibleRow() - 1), nCol,
	...
	m_aveRedrawScreenTime.AddValue(XTPToUIntChecked(::labs(XTPToLong(dwEndTime - dwStartTime))));
...
void CXTPSyntaxEditCtrl::Draw(CDC* pDC, const CRect& rcRect)
{
	...
	for (int nLine = 0; nLine < nLinesCount;)
	{
		...
		if (GetCollapsibleNodes())
		{
			...
			GetPaintManager()->DrawLineNode(pDC, rcNode, rcNodeFull, dwCollapcedType, nTextRow,
											this);
		}
		else
		{
...
int CXTPSyntaxEditCtrl::PrintPage(CDC* pDC, const CRect& rcRect,
								  int nFlags) // returns printed rows count
{
	...
	for (int nLine = 0; nLine < nLinesCount && nNextLineY + nRowHeight < rcText.Height();
		 nPrintedRowsCount++)
	{
		int nTextRow = m_nTopRow + nLine + nSkipLines;
		...
		int nLineHeight = GetPaintManager()->PrintLineTextEx(pDC, rcTextLine, nTextRow, nLine, this,
															 nFlags);
		if (nLineHeight < 0)
			break;
...
void CXTPSyntaxEditCtrl::UpdateRowInfoInternally(int nTextRow)
{
	...
	int nCollapsedRowsCount = 0;
	ProcessCollapsedRowsBeroreDraw(nTextRow,
								   nCollapsedRowsCount); // to update collapsed block info.
	...
	for (int i = 0; i < nCount; i++)
	{
		...
		if (coBlk.lcStart.nLine == nTextRow)
		{
			if (bIsRowCollapsed && !bCollapsedProcessed)
			{
				...
				GetLineMarksManager()->SetLineMark(nTextRow, xtpEditLMT_Collapsed, &LMCoParam);

				m_arCollapsedTextRows.SetAtGrow(m_nCollapsedTextRowsCount,
												XTPToUIntChecked(nTextRow));
		...
		if ((coBlk.lcStart.nLine < nTextRow) && (coBlk.lcEnd.nLine >= nTextRow))
		{
...
CRect CXTPSyntaxEditCtrl::CalcEditRects(CRect* prcBookMarks, CRect* prcLineNum, CRect* prcNodes,
										CRect* prcText, const CRect* prcClient) const
{
	CRect rcClient(0, 0, XTP_DPI_X(100), XTP_DPI_Y(100));
	if (prcClient)
		rcClient = *prcClient;
	else if (m_hWnd)
...
{
	return XTPDrawHelpers()->RegisterWndClass(hInstance, XTP_EDIT_CLASSNAME_EDITCTRL,
...
BOOL CXTPSyntaxEditCtrl::Create(CWnd* pParentWnd, BOOL bHorzScroll, BOOL bVertScroll,
								CXTPSyntaxEditBufferManager* pBuffer, CCreateContext* lpCS,
								UINT nID)
{
	if (pBuffer)
	{
	...
	BOOL bCreate = CWnd::CreateEx(WS_EX_ACCEPTFILES, XTP_EDIT_CLASSNAME_EDITCTRL, NULL, dwStyle,
								  CRect(0, 0, XTP_DPI_X(100), XTP_DPI_Y(100)), m_pParentWnd, nID,
								  (LPVOID)lpCS);
...
BOOL CXTPSyntaxEditCtrl::_InitEditControl()
{
	...
	CRect rcText(0, 0, XTP_DPI_X(3000), XTP_DPI_Y(2000));
	m_pDrawTextProcessor->SetTextRect(rcText);
	m_pDrawTextProcessor->SetTabSize(GetTabSize());
	...
	if (pMan)
	{
		...
		if (!m_bConfigFileMode && !m_strSyntaxScheme.IsEmpty() && !m_strColorScheme.IsEmpty())
		{
	...
	if (ptrConnParser)
	{
		m_pSink->Advise(ptrConnParser, xtpEditOnParserStarted,
						CreateNotfySinkClassDelegate(this, &CXTPSyntaxEditCtrl::OnParseEvent));
		m_pSink->Advise(ptrConnParser, xtpEditOnTextBlockParsed,
						CreateNotfySinkClassDelegate(this, &CXTPSyntaxEditCtrl::OnParseEvent));
		m_pSink->Advise(ptrConnParser, xtpEditOnParserEnded,
						CreateNotfySinkClassDelegate(this, &CXTPSyntaxEditCtrl::OnParseEvent));
	}
	...
	if (ptrConnBufMan)
	{
		m_pSink->Advise(ptrConnBufMan, xtpEditClassSchWasChanged,
						CreateNotfySinkClassDelegate(this,
													 &CXTPSyntaxEditCtrl::OnLexCfgWasChanged));
		m_pSink->Advise(ptrConnBufMan, xtpEditThemeWasChanged,
						CreateNotfySinkClassDelegate(this,
													 &CXTPSyntaxEditCtrl::OnLexCfgWasChanged));
		m_pSink->Advise(ptrConnBufMan, xtpEditAllConfigWasChanged,
						CreateNotfySinkClassDelegate(this,
													 &CXTPSyntaxEditCtrl::OnLexCfgWasChanged));
	}
...
BOOL CXTPSyntaxEditCtrl::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message)
{
	...
	if (GetCollapsibleNodes())
	{
		...
		DWORD dwType = XTP_EDIT_ROWNODE_NOTHING;
		if (GetRowNodes(nRow, dwType)
			&& (dwType & (XTP_EDIT_ROWNODE_COLLAPSED | XTP_EDIT_ROWNODE_EXPANDED)))
		{
...
LRESULT CXTPSyntaxEditCtrl::OnGetText(WPARAM wBufferSize, LPARAM lpBuffer)
{
	...
	int nBufferSize = XTPToIntChecked(wBufferSize);
	CMemFile memFile;
	if (!GetText(memFile, nBufferSize))
		return 0;

	void* pTextData = NULL;
	void* pTextEnd  = NULL;

	UINT uBufferB = memFile.GetBufferPtr(CFile::bufferRead, XTPToUIntChecked(wBufferSize),
										 &pTextData, &pTextEnd);
	...
	int nTextSize = XTPToIntChecked(uBufferB / sizeof(TCHAR) - sizeof(TCHAR));
	nTextSize	 = max(0, nTextSize);
	nTextSize	 = min(nTextSize, nBufferSize);
...
BOOL CXTPSyntaxEditCtrl::GetText(CMemFile& memFile, int nMaxLen) const
{
	...
	if (nMaxLen > 0)
		nMaxLen = XTPToIntChecked(nMaxLen * sizeof(TCHAR));
...
void CXTPSyntaxEditCtrl::SetText(LPCTSTR pcszText)
{
	...
	int nStrLenB = XTPToIntChecked(_tcslen(pcszText) * sizeof(TCHAR));
	CMemFile memFile((BYTE*)pcszText, XTPToUIntChecked(nStrLenB));
	memFile.SeekToBegin();
...
void CXTPSyntaxEditCtrl::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	...
	switch (nSBCode)
	{
		case SB_LINELEFT: nCurrPos -= nStep; break;
		case SB_LINERIGHT: nCurrPos += nStep; break;
		case SB_PAGELEFT: nCurrPos -= info.nPage; break;
		case SB_PAGERIGHT: nCurrPos += info.nPage; break;
		case SB_LEFT: nCurrPos = info.nMin; break;
		case SB_RIGHT: nCurrPos = info.nMax;
		case SB_ENDSCROLL: return;
		case SB_THUMBPOSITION:
		case SB_THUMBTRACK:
		{
			nCurrPos   = XTPToIntChecked(nPos);
...
void CXTPSyntaxEditCtrl::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	...
	switch (nSBCode)
	{
		...
		case SB_LINEUP:
			...
			break;
		case SB_PAGEUP:
			bChanged = ShiftCurrentVisibleRowUp(XTPToIntChecked(info.nPage));
			if (bChanged)
				SetScrollPos(SB_VERT, max(1, XTPToIntChecked(iPos - info.nPage)));
			break;
		case SB_PAGEDOWN:
			bChanged = ShiftCurrentVisibleRowDown(XTPToIntChecked(info.nPage));
			if (bChanged)
				SetScrollPos(SB_VERT, XTPToIntChecked(iPos + info.nPage));
			break;
		case SB_TOP:
...
void CXTPSyntaxEditCtrl::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	if (m_bDragging || m_bRightButtonDrag
		|| m_pSelection->bSelectingRunning /* || m_bSelectionStarted*/)
	{
		OnSetCursor(0, 0, 0);
		return;
	}
	...
	switch (nChar)
	{
		...
		case VK_DELETE:
		{
			...
			else if (!bCtrlKey)
			{
				...
				{
					CString strInsertText(CString(_T('\t'), m_nInsertTabCount)
					...
					OnEditChanged(GetCurrentDocumentRow(), 1, GetCurrentDocumentRow(), iNewCol,
								  xtpEditActInsert);
					//**----------------------
		...
		case VK_SPACE:
			if (bCtrlKey)
			{
				...
				// NON-COVERED CASE <<
				while (strText.GetLength() > 0 && nTextPos >= 0
					   && m_pAutoComplete->m_strDelims.Find(strText.GetAt(nTextPos)) < 0)
				{
...
void CXTPSyntaxEditCtrl::OnChar(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	else if (nChar != VK_BACK && nChar != VK_ESCAPE && !(bCtrlKey && !bAltKey) && !bProcessed)
	{
		...
		if (nChar == _T('\r'))
		{
			...
			{
				GetLineMarksManager()->DeleteLineMark(GetCurrentDocumentRow(),
													  xtpEditLMT_Collapsed);
			}
		}
		...
		BOOL bCanProcess = CreateInsertText(szText, strTextToIns, iNewRow, iNewCol, iNewDispCol,
											iEditRowFrom, iEditRowTo, nChainActionCount);
		...
		if (!bPrevOverwriteMode)
		{
			...
			if (_tcschr(szSeps, XTP_CHARTOCRT(nChar)))
				bGroupInsMode = FALSE;
			else
				bGroupInsMode = TRUE;
		...
		if (bCanProcess)
		{
			...
			if (nChar == VK_RETURN && m_nAutoIndentCol)
				nInsCol = min(m_nCurrentCol,
							  m_pBuffer->GetLineTextLengthC(GetCurrentDocumentRow()) + 1);

			BOOL bInsRes = m_pBuffer->InsertText(strTextToIns, GetCurrentDocumentRow(), nInsCol,
												 TRUE);
			bModified |= bInsRes;
		...
		//**----------------------
		BOOL bInsAt0 = (m_nCurrentCol == 1)
					   || GetLineText(GetCurrentDocumentRow()).GetLength() == 0;
...
void CXTPSyntaxEditCtrl::SetCurPos(int nTextRow, int nDispCol, BOOL bRemainSelected,
								   BOOL bForceVisible)
{
	if (nTextRow > GetRowCount())
...
void CXTPSyntaxEditCtrl::SetCurCaretPos(int nTextRow, int nDispCol, BOOL bRowColNotify,
										BOOL bEnsureVisible)
{
	if (!::IsWindow(m_hWnd) || !m_hWnd)
		return;
...
void CXTPSyntaxEditCtrl::_EnsureVisible(int nTextRow, int nDispCol /*, BOOL bRedraw*/)
{
	...
	//  BOOL bVirtSpace = _IsVirtualSpaceActive() || m_nAutoIndentCol;
	//  m_nCurrentCol = m_pDrawTextProcessor->DispPosToStrPos(nDispRow - 1, m_nDispCol-1,
	//  bVirtSpace) + 1;
...
BOOL CXTPSyntaxEditCtrl::EnsureVisibleCol(int nDispRow, int nDispCol)
{
	...
	int nColWidth = 0;
	int nColPos   = m_pDrawTextProcessor->GetColPosX(nDispRow - 1, nDispCol, &nColWidth,
													 IsEnabledVirtualSpace());
	...
	{
		nNewScrollOffset = nOffsetX + (nColPos + nColWidth) - rcText.right
						   + min(rcText.Width() / 3, nColWidth * 15);
	}
...
int CXTPSyntaxEditCtrl::CalcMaximumWidth(LPCTSTR szText)
{
	...
	for (LPCTSTR pcszChar = szText; pcszChar && *pcszChar; pcszChar = _tcsinc(pcszChar))
	{
		...
		{
			// Now calculate tab size
			iMaxWidth += ((GetTabSize() + 1)
						  - (iMaxWidth - ((iMaxWidth / GetTabSize()) * GetTabSize())));
		}
#ifdef XTP_FIXED
		else if (isleadbyte(*pcszChar))
			// multi byte character : DisplayLength 2
...
CString CXTPSyntaxEditCtrl::GetCollapsedText(XTP_EDIT_COLLAPSEDBLOCK* pCoDrawBlk,
											 int nMaxLinesCount) const
{
...
void CXTPSyntaxEditCtrl::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	rcClient.bottom = rcClient.top
					  + m_pDrawTextProcessor->GetRowHeight()
							* m_pDrawTextProcessor->GetRowsCount(FALSE);
	...
	//  TRACE(_T("*** OnLButtonDown. RowColFromPoint: nNewRow=%d, nNewCol=%d, nNewDispRow=%d,
	//  nNewDispCol=%d \n"),
	...
	if (!IsViewOnly())
	{
		...
		//  \n"),
		//          m_pSelection->GetStart_str().nLine, m_pSelection->GetStart_str().nCol,
		//          m_pSelection->GetStart_disp().nCol, m_pSelection->GetEnd_str().nLine,
		//          m_pSelection->GetEnd_str().nCol, m_pSelection->GetEnd_disp().nCol);
		...
		if (m_pSelection->bWordSelectionMode)
		{
			...
			UINT nFindDir = UINT(m_pSelection->IsSelNormal() ? XTP_EDIT_FINDWORD_NEXT
															 : XTP_EDIT_FINDWORD_PREV);
			BOOL bFind	= FindWordEx_str(nFindDir, m_pSelection->GetEnd_str(), lcWord1, lcWord2,
										   bOverSpace);
...
void CXTPSyntaxEditCtrl::OnLButtonUp(UINT nFlags, CPoint point)
{
	...
	{
		m_pSelection->Reset_disp(m_pSelection->GetStart_disp().nLine,
								 m_pSelection->GetStart_disp().nCol);
	}
	else
	{
		SetCurCaretPos(GetCurrentDocumentRow(), m_nDispCol, FALSE, FALSE);
...
XTP_EDIT_COLLAPSEDBLOCK* CXTPSyntaxEditCtrl::GetBlockFromPt(const CPoint& ptMouse) const
{
	...
	{
		int nRow = XTPToIntChecked(m_arCollapsedTextRows[i]);
...
AFX_STATIC CString AFX_CDECL TrimLeftIndent(const CString& csText)
{
	...
	CString csTrimText(
		csTrimmedText.Right(csTrimmedText.GetLength() - nLastLine).SpanIncluding(_T("\n \t")));
...
void CXTPSyntaxEditCtrl::ShowCollapsedToolTip(const CPoint& ptMouse)
{
	...
	if (pCoDrawBlk)
	{
		...
		rcTip.right += XTP_DPI_X(200);
		rcTip.bottom += XTP_DPI_Y(200);
		rcTip += XTP_DPI_SIZE(CSize(18, 18));
...
void CXTPSyntaxEditCtrl::OnRButtonDown(UINT nFlags, CPoint point)
{
	...
	if (!m_pSelection->IsSelExist() || !m_pSelection->IsInSel_str(iRow, iCol - 1))
...
BOOL CXTPSyntaxEditCtrl::RowColFromPoint(CPoint pt, int* pRow, int* pCol, int* pDispRow,
										 int* pDispCol, BOOL bVirtualSpace)
{
	if (bVirtualSpace < 0)
	...
	{
		nDispRow0 = pt.y < m_pDrawTextProcessor->GetTextRect().top
						? 0
						: m_pDrawTextProcessor->GetRowsCount(FALSE) - 1;
	}
	...
	if (!bRet)
	{
		...
		{
			int nX1 = max(0, m_pDrawTextProcessor->GetTextRect().right - XTP_DPI_X(1));
...
int CXTPSyntaxEditCtrl::ScrollWindowEx(int dx, int dy, LPCRECT lpRectScroll, LPCRECT lpRectClip,
									   CRgn* prgnUpdate, LPRECT lpRectUpdate, UINT flags)
{
	...
	if (pParentWnd != NULL && IsCreateScrollbarOnParent())
	{
		return pParentWnd->ScrollWindowEx(dx, dy, lpRectScroll, lpRectClip, prgnUpdate,
										  lpRectUpdate, flags);
	}
	...
	if (m_pParentWnd && IsCreateScrollbarOnParent())
		return m_pParentWnd->ScrollWindowEx(dx, dy, lpRectScroll, lpRectClip, prgnUpdate,
											lpRectUpdate, flags);
...
void CXTPSyntaxEditCtrl::EnableScrollBarCtrl(int nBar, BOOL bEnable)
{
	...
	if (pParentWnd != NULL && IsCreateScrollbarOnParent())
	{
		...
		DWORD dwScrollBar = DWORD((nBar == SB_HORZ) ? WS_HSCROLL : 0);
...
void CXTPSyntaxEditCtrl::RepositionBars(UINT nIDFirst, UINT nIDLast, UINT nIDLeftOver, UINT nFlag,
										LPRECT lpRectParam, LPCRECT lpRectClient, BOOL bStretch)
{
	...
	if (pParentWnd != NULL && IsCreateScrollbarOnParent())
	{
		pParentWnd->RepositionBars(nIDFirst, nIDLast, nIDLeftOver, nFlag, lpRectParam, lpRectClient,
								   bStretch);
	}
	else
	{
		...
		if (m_pParentWnd && IsCreateScrollbarOnParent())
		{
			m_pParentWnd->RepositionBars(nIDFirst, nIDLast, nIDLeftOver, nFlag, lpRectParam,
										 lpRectClient, bStretch);
		...
		CWnd::RepositionBars(nIDFirst, nIDLast, nIDLeftOver, nFlag, lpRectParam, lpRectClient,
							 bStretch);
	}
}
...
BOOL CXTPSyntaxEditCtrl::DeleteSelection()
{
	...
	if (!DeleteBuffer(m_pSelection->GetNormalStart_disp().nLine,
					  m_pSelection->GetNormalStart_disp().nCol,
					  m_pSelection->GetNormalEnd_disp().nLine,
					  m_pSelection->GetNormalEnd_disp().nCol, nFlags))
	{
		return FALSE;
	}
									  ...
									  int nFlags)
{
	...
	if ((nFlags & xtpEditTextAsBlock) && (iColTo < iColFrom))
	{
		// swap them
		int nTemp = iColTo;
		iColTo	= iColFrom;
...
BOOL CXTPSyntaxEditCtrl::DeleteChar(int iRow, int iCol, XTPSyntaxEditDeletePos pos)
{
	...
	if (pos == xtpEditDelPosAfter) // If deleting using DEL key
	{
		...
		if (iCol > nLineLen)
		{
			...
			CString strTabs = CString(_T('\t'), nAdditionalTabs)
							  + CString(_T(' '), nAdditionalSpaces);
		...
		if (iCol == 1)
		{
			if (iRow > 1)
			{
				...
				for (int i = 0; i < nCount; i++)
				{
					...
					{
						if (HasRowMark(coBlk.lcStart.nLine, xtpEditLMT_Collapsed))
							GetLineMarksManager()->DeleteLineMark(coBlk.lcStart.nLine,
																  xtpEditLMT_Collapsed);
					}
				}
...
BOOL CXTPSyntaxEditCtrl::DoUndoRedo(int nActionsCount, BOOL bUndoRedo)
{
	...
	for (int nAction = 0; nAction < nActionsCount; nAction++)
	{
		...
		// perform undo
		int nEditAction = bUndoRedo ? pUndoMgr->DoUndo(lcFrom, lcTo, this)
		...
		NotifyEditChanged(lcFrom.nLine, lcTo.nLine, XTPToUInt(nEditAction));
	}
	...
	nCurDocRow = GetCurrentDocumentRow();
	if ((nCurDocRow > 0 && m_nCurrentCol > 0) || !(nRowsAffected == 0 && nActionsCount == 1))
	{
		SetCurCaretPos(nCurDocRow, m_nDispCol);
	}
...
void CXTPSyntaxEditCtrl::Copy()
{
	...
	HGLOBAL hMBCSMem = ::GlobalAlloc(GMEM_MOVEABLE | GMEM_DDESHARE,
									 XTPToSizeTChecked(nLen
													   + 2)); // Maximum number of bytes is dwBytes
	if (!hMBCSMem)
	{
		CloseClipboard();
		return;
	...
	int nBytes = ::WideCharToMultiByte(uCodePage, 0, (LPWSTR)lpSource, -1, lpMBCSSource, nLen, NULL,
									   NULL);
...
#else
	SIZE_T nUBytes		= (dwBytes + 2) * sizeof(WCHAR);
	...
	int nLen = ::MultiByteToWideChar(uCodePage, 0, (LPCSTR)lpSource, -1, lpWSource,
									 XTPToIntChecked(nUBytes));

	ASSERT(nLen <= XTPToIntChecked(nUBytes / sizeof(WCHAR)));
	...
	if (m_pSelection->bBlockSelectionMode)
	{
		...
		CLIPFORMAT uCF_MSDEVColumnSelect = (CLIPFORMAT)::RegisterClipboardFormat(
			XTP_EDIT_CFMSDEVCOLSEL);
...
void CXTPSyntaxEditCtrl::SetDocModified(BOOL bModified)
{
	...
	dm.nmhdr.idFrom   = XTPToUIntPtr(GetDlgCtrlID());
	...
	{
		m_pParentWnd->SendMessage(WM_NOTIFY, (WPARAM)dm.nmhdr.idFrom, (LPARAM)&dm);
	}
}
...
void CXTPSyntaxEditCtrl::InsertString(LPCTSTR szText, int iRow, int iCol, BOOL bDeleteSelection)
{
	...
	XTP_EDIT_LINECOL finalLC;
	BOOL bInsRes = m_pBuffer->InsertText(szText, GetCurrentDocumentRow(), m_nCurrentCol, TRUE,
										 &finalLC);
...
void CXTPSyntaxEditCtrl::InsertTextBlock(LPCTSTR szText, int iRow, int iCol, BOOL bDeleteSelection)
{
	...
	XTP_EDIT_LINECOL finalLC;
	BOOL bInsRes = m_pBuffer->InsertTextBlock(szText, GetCurrentDocumentRow(), iCol, TRUE,
											  &finalLC);
...
void CXTPSyntaxEditCtrl::UpdateScrollPos(DWORD dwUpdate /* = XTP_EDIT_UPDATE_ALL*/)
{
	...
	ssp.nmhdr.idFrom   = XTPToUIntPtr(GetDlgCtrlID());
	...
	if (::IsWindow(m_pParentWnd->GetSafeHwnd()))
	{
		m_pParentWnd->SendMessage(WM_NOTIFY, (WPARAM)ssp.nmhdr.idFrom, (LPARAM)&ssp);
	}
}
...
void CXTPSyntaxEditCtrl::_EnableScrollBarNotify(DWORD dwScrollBar, DWORD dwState)
{
	...
	nmEScroll.nmhdr.idFrom   = XTPToUIntPtr(GetDlgCtrlID());
	...
	if (::IsWindow(m_pParentWnd->GetSafeHwnd()))
	{
		m_pParentWnd->SendMessage(WM_NOTIFY, (WPARAM)nmEScroll.nmhdr.idFrom, (LPARAM)&nmEScroll);
	}
}
...
void CXTPSyntaxEditCtrl::CopyOrMoveText(BOOL bCopy)
{
	...
	{
		bInsRes = m_pBuffer->InsertTextBlock(strDropText, lcDropPos_disp.nLine, nAbsDropCol, TRUE,
											 &finalLC);
	}
	else
	{
		bInsRes = m_pBuffer->InsertText(strDropText, lcDropPos_disp.nLine, nAbsDropCol, TRUE,
										&finalLC);
	}
	...
	//**----------------------
	OnEditChanged(nRowFrom, m_pSelection->GetNormalStart_str().nCol + 1, nRowTo,
				  m_pSelection->GetNormalEnd_str().nCol + 1, xtpEditActInsert);
	//**----------------------
...
void CXTPSyntaxEditCtrl::OnContextMenu(CWnd* pWnd, CPoint point)
{
	...
	else
	{
		...
		{
			CWnd::HideCaret();
			m_pParentWnd->SendMessage(WM_CONTEXTMENU, (WPARAM)pWnd->GetSafeHwnd(),
...
DWORD CXTPSyntaxEditCtrl::GetAutoscrollDirection(int* pnCols, int* pnRows)
{
	...
	int nAutoZoneX2 = min(m_pDrawTextProcessor->GetTextMetrics().tmAveCharWidth,
						  rcText.Width() / 4);
...
void CXTPSyntaxEditCtrl::OnTimer(UINT_PTR nIDEvent)
{
	...
	else if (nIDEvent == TIMER_SELECTION_ID)
	{
		...
		{
			//          TRACE(_T("OnTimer.sel. RowColFromPoint: nNewRow=%d, nNewCol=%d,
			//          nNewDispRow=%d, nNewDispCol=%d \n"),
			//                  nNewRow, nNewCol, nNewDispRow, nNewDispCol);
...
BOOL CXTPSyntaxEditCtrl::ReplaceSel(LPCTSTR szNewText)
{
	if (m_pSelection->IsSelExist())
	{
		...
		m_nCurrentCol = m_pSelection->GetNormalStart_str().nCol
						+ 1; // CalcAbsCol(GetLineText(m_pSelection->GetNormalStart_disp().nLine),
							 // m_nDispCol);
	}
...
int CXTPSyntaxEditCtrl::ReplaceAll(LPCTSTR szFindText, LPCTSTR szReplaceText, BOOL bMatchWholeWord,
								   BOOL bMatchCase)
{
	int nMatchFound = 0;
	...
	{
		BOOL bFound = Find(szFindText, bMatchWholeWord, bMatchCase, TRUE, FALSE);
...
BOOL CXTPSyntaxEditCtrl::Find(LPCTSTR szText, BOOL bMatchWholeWord, BOOL bMatchCase,
							  BOOL bSearchDown, BOOL bRedraw, int nStartRow, int nStartCol)
{
	...
	BOOL bResult = MatchText(iStartRow, strLineText, szText, iStartCol, bMatchWholeWord, bMatchCase,
							 bSearchDown, bRedraw);
	...
	if (bSearchDown)
	{
		for (int i = (iStartRow + 1); i <= GetRowCount(); i++)
		{
			...
			bResult = MatchText(i, strLineText, szText, 0, bMatchWholeWord, bMatchCase, TRUE,
								bRedraw);
	...
	else
	{
		for (int i = (iStartRow - 1); i >= 1; i--)
		{
			...
			bResult = MatchText(i, strLineText, szText, (int)_tcsclen(strLineText) + 1,
								bMatchWholeWord, bMatchCase, FALSE, bRedraw);
...
BOOL CXTPSyntaxEditCtrl::MatchText(int nRow, LPCTSTR szLineText, LPCTSTR szMatchText, int nStartPos,
								   ...
								   BOOL bRedraw)
{
	...
	if (bSearchForward)
	{
		for (int i = nStartPos; i <= (nLen - nSearchLen + 1); i++)
		{
			int nFound = -1;
			if (bMatchCase)
				nFound = _tcsncmp((szLineText + i - 1), szMatchText, XTPToSizeTChecked(nSearchLen));
			else
				nFound = _tcsnicmp((szLineText + i - 1), szMatchText,
								   XTPToSizeTChecked(nSearchLen));
			...
			if (bMatchWholeWord && nFound == 0)
			{
				...
				else
				{
					...
					bSepAfter			  = (_tcschr(szSeps, XTP_CHARTOCRT(*pcszTextAfter)) != 0);
				}

				if (i == 1)
					bSepBefore = TRUE;
				else
					bSepBefore = (_tcschr(szSeps, XTP_CHARTOCRT(*(szLineText + i - 2))) != 0);
...
void CXTPSyntaxEditCtrl::FindWord(UINT nFindWhat)
{
	...
	BOOL bFind = FindWordEx(nFindWhat, GetCurrentDocumentRow(), m_nCurrentCol, lcWord1, lcWord2,
							bOverSpace);
...
BOOL CXTPSyntaxEditCtrl::FindWordEx_str(UINT nFindWhat, XTP_EDIT_LINECOL lcPos_str,
										XTP_EDIT_LINECOL& rlcWordStart,
										XTP_EDIT_LINECOL& rlcWordEnd, BOOL& rbOverSpace)
{
	return FindWordEx(nFindWhat, lcPos_str.nLine, lcPos_str.nCol + 1, rlcWordStart, rlcWordEnd,
					  rbOverSpace);
}
									...
									BOOL& rbOverSpace)
{
	...
	// Is over space/tab
	BOOL bOverSpace = !!_tcschr(szSpaceSeps, XTP_CHARTOCRT(*pCh));
	BOOL bOverNL	= !!_tcschr(szNLSeps, XTP_CHARTOCRT(*pCh));
	rbOverSpace		= (bOverSpace || bOverNL);

	BOOL bStartCharSep = !!_tcschr(szSeps, XTP_CHARTOCRT(*pCh));

	if (rbOverSpace && nDir < 0)
	{
		while (pCh
			   && (_tcschr(szSpaceSeps, XTP_CHARTOCRT(*pCh))
				   || !bOverSpace && _tcschr(szNLSeps, XTP_CHARTOCRT(*pCh))))
		{
		...
		bStartCharSep = !!_tcschr(szSeps, XTP_CHARTOCRT(*pCh));
	}
	...
	if (rbOverSpace && nDir > 0) // one word border find - find the other one
	{
		...
		while (pCh
			   && (_tcschr(szSpaceSeps, XTP_CHARTOCRT(*pCh))
				   || _tcschr(szNLSeps, XTP_CHARTOCRT(*pCh))))
		{
...
BOOL CXTPSyntaxEditCtrl::IncreaseIndent()
{
	...
	if (!m_pBuffer->GetOverwriteFlag())
	{
		const CString strText(GetTabWithSpace() ? CString((TCHAR)(' '), GetTabSize())
												: CString(_T("\x09")));
...
BOOL CXTPSyntaxEditCtrl::DecreaseIndent()
{
	if (!m_pSelection->IsSelExist())
	{
		...
		m_nDispCol = 1
					 + ((((m_nDispCol - 1) / GetTabSize()) - iAddFactor)
						* GetTabSize()); // Align by tab size
...
void CXTPSyntaxEditCtrl::_RecalcScrollBars()
{
	...
	if (GetRowCount() <= 0)
	{
		if (GetVertScrollBar())
		{
			...
			info.nPage  = XTPToUIntChecked(nPageY);
			info.nMax   = nPageY;
		...
		if (GetHorzScrollBar())
		{
			...
			info.nPage = XTPToUIntChecked(nPageX);
...
void CXTPSyntaxEditCtrl::RecalcVertScrollPos()
{
	...
	if (bVertScrl)
	{
		...
		si.nPage	  = XTPToUIntChecked(nPageRows);
		si.nPos		  = nVisRow;
...
void CXTPSyntaxEditCtrl::RecalcHorzScrollPos()
{
	...
	si.nPage		  = XTPToUIntChecked(m_pDrawTextProcessor->GetTextRect().Width());
	si.nPos			  = nPos;
...
void CXTPSyntaxEditCtrl::NotifyCurRowCol(int iRow, int iCol) const
{
	...
	src.nmhdr.idFrom   = XTPToUIntPtr(GetDlgCtrlID());
	...
	if (::IsWindow(m_pParentWnd->GetSafeHwnd()))
	{
		m_pParentWnd->SendMessage(WM_NOTIFY, (WPARAM)src.nmhdr.idFrom, (LPARAM)&src);
	}
}
...
HGLOBAL CXTPSyntaxEditCtrl::GetSelectionBuffer(UINT nFormat)
{
	...
	HGLOBAL hMem = ::GlobalAlloc(GMEM_MOVEABLE, XTPToSizeTChecked(nLen));
...
#ifdef _UNICODE
	if (nFormat == CF_UNICODETEXT)
		MEMCPY_S(pText, pBytes, XTPToSizeTChecked(nLen));
	else
	{
...
#else
	if (nFormat == CF_TEXT)
		MEMCPY_S(pText, pBytes, XTPToSizeT(nLen));
	else
	{
		::MultiByteToWideChar(uCodePage, 0, (LPSTR)pBytes, -1, (LPWSTR)pText,
							  XTPToInt(nLen / sizeof(wchar_t)));
	}
#endif
...
void CXTPSyntaxEditCtrl::HandleDrop(BOOL bCopy)
{
	...
	else
	{
		...
		int nID = menu.TrackPopupMenu(TPM_LEFTALIGN | TPM_RIGHTBUTTON | TPM_NONOTIFY
										  | TPM_RETURNCMD,
									  pt.x, pt.y, this);
...
{
	if (!SetValueBool(XTP_EDIT_REG_TABWITHSPACE, bTabWithSpace, m_pOptions->m_bTabWithSpace,
					  bUpdateReg))
		return FALSE;
...
{
	if (!SetValueBool(XTP_EDIT_REG_SYNTAXCOLOR, bSyntaxColor, m_pOptions->m_bSyntaxColor,
					  bUpdateReg))
		return FALSE;
...
LRESULT CXTPSyntaxEditCtrl::NotifyParent(UINT uCode) const
{
	...
	if (::IsWindow(m_pParentWnd->GetSafeHwnd()))
	{
		...
		hdr.hwndFrom = m_hWnd;
		hdr.code	 = uCode;
		hdr.idFrom   = XTPToUIntPtr(GetDlgCtrlID());

		return m_pParentWnd->SendMessage(WM_NOTIFY, (WPARAM)hdr.idFrom, (LPARAM)&hdr);
	}
...
LRESULT CXTPSyntaxEditCtrl::NotifyMouseEvent(UINT uCode, UINT nFlags, const CPoint& point) const
{
	...
	if (::IsWindow(GetParent()->GetSafeHwnd()))
	{
		...
		hdrMouse.hdr.idFrom   = XTPToUIntPtr(GetDlgCtrlID());
		hdrMouse.pt			  = point;

		return GetParent()->SendMessage(WM_NOTIFY, (WPARAM)hdrMouse.hdr.idFrom, (LPARAM)&hdrMouse);
	}
...
BOOL CXTPSyntaxEditCtrl::SetScrollBars(BOOL bHorzSBar, BOOL bVertSBar, BOOL bUpdateReg /*=FALSE*/)
{
	...
	DWORD dwStyle = XTPToDWORD(::GetWindowLong(m_hWnd, GWL_STYLE) & ~(WS_HSCROLL | WS_VSCROLL));
...
void CXTPSyntaxEditCtrl::NotifyEditChanged(int iRowFrom, int iRowTo, UINT nActions)
{
	...
	sec.nmhdr.idFrom   = XTPToUIntPtr(GetDlgCtrlID());
	sec.nRowFrom	   = iRowFrom;
	sec.nRowTo		   = iRowTo;
	sec.nAction		   = XTPToInt(nActions);
	...
	if (::IsWindow(m_pParentWnd->GetSafeHwnd()))
	{
		m_pParentWnd->SendMessage(WM_NOTIFY, (WPARAM)sec.nmhdr.idFrom, (LPARAM)&sec);
	}
}

BOOL CXTPSyntaxEditCtrl::CreateInsertText(LPTSTR szText, CString& strTextToIns, int& iNewRow,
										  int& iNewCol, int& iNewDispCol, int& iEditRowFrom,
										  int& iEditRowTo, int& iChainActionCount)
{
	...
	if (!bTextCRLF)
	{
		if (szText[0] == _T('\t') && !GetTabWithSpace() && m_nCurrentCol > 1 && nLineLenC > 0)
		{
			...
			if (nCharsToCheck)
			{
				LPCTSTR pCheck = _tcsninc((LPCTSTR)strLineText,
										  XTPToSizeTChecked(max(0, nCurCol - 1 - 1)));
				int nBlanks;
				...
				if (nBlanks > 0)
				{
					...
					OnEditChanged(nCurDocRow, nCurCol - nBlanks, nCurDocRow, nCurCol,
								  xtpEditActDelete);
					//**----------------------
		...
		else
		{
			...
			// iNewCol = m_nCurrentCol + (szText[0] == 0x09 ? (GetTabWithSpace() ? GetTabSize() : 1)
			// : 1);
...
void CXTPSyntaxEditCtrl::DoAutoIndentIfNeed(int nBaseDocRow, int nDispCol_prev)
{
	...
	if (bFound)
	{
		...
		if (m_nInsertTabCount > 0 || m_nInsertSpaceCount > 0)
			m_nAutoIndentCol++;
	}
	else
	{
...
void CXTPSyntaxEditCtrl::FillTabs(CString& rstrTextToIns, int nDispCol)
{
	int nTabsCount  = GetTabWithSpace() ? 0 : (nDispCol - 1) / GetTabSize();
	int nSpaceCount = nDispCol - nTabsCount * GetTabSize() - 1;
...
BOOL CXTPSyntaxEditCtrl::DoChangeSelectionTabify(BOOL bTabify)
{
	...
	m_pBuffer->GetUndoRedoManager()->SetGroupInsertMode(FALSE);
	m_pBuffer->GetUndoRedoManager()->SetLastCommandText(
		UINT(bTabify ? XTP_IDS_EDIT_TABIFY : XTP_IDS_EDIT_UNTABIFY));
...
BOOL CXTPSyntaxEditCtrl::SetLineNumbers(BOOL bLineNumbers, BOOL bUpdateReg /*=FALSE*/)
{
	...
	if (!SetValueBool(XTP_EDIT_REG_LINENUMBERS, bLineNumbers, m_pOptions->m_bLineNumbers,
					  bUpdateReg))
		return FALSE;
	...
	else
	{
		...
		for (int i = 0; i < nCount; i++)
		{
			...
			if (coBlk.lcStart.nLine == nRow)
			{
				...
				m_arCollapsedTextRows.SetAtGrow(m_nCollapsedTextRowsCount, XTPToUIntChecked(nRow));
...
void CXTPSyntaxEditCtrl::RefreshLineMarks(XTP_EDIT_NMHDR_EDITCHANGED* pEditChanged)
{
	...
	GetLineMarksManager()->RefreshLineMarks(pEditChanged->nRowFrom, pEditChanged->nRowTo,
											eRefreshType);
...
void CXTPSyntaxEditCtrl::RefreshColors()
{
	...
	if (GetSyntaxColor())
	{
		...
		BOOL bParseInThread		= m_pBuffer->GetLexParser()
								  ->GetSchemaOptions(m_pBuffer->GetFileExt())
								  ->m_bFirstParseInSeparateThread;
		if (bParseInThread)
		{
...
void CXTPSyntaxEditCtrl::OnEditChanged(const XTP_EDIT_LINECOL& LCFrom, const XTP_EDIT_LINECOL& LCTo,
									   int eEditAction)
{
	OnEditChanged(LCFrom.nLine, LCFrom.nCol, LCTo.nLine, LCTo.nCol, eEditAction);
}

void CXTPSyntaxEditCtrl::OnEditChanged(int nRowFrom, int nColFrom, int nRowTo, int nColTo,
									   int eEditAction)
{
...
BOOL CXTPSyntaxEditCtrl::NotifyParseEvent(XTP_NOTIFY_CODE code, WPARAM wParam, LPARAM lParam)
{
	...
	pe.nmhdr.idFrom   = XTPToUIntPtr(GetDlgCtrlID());
	...
	if (::IsWindow(m_pParentWnd->GetSafeHwnd()))
	{
		return (BOOL)m_pParentWnd->SendMessage(WM_NOTIFY, (WPARAM)pe.nmhdr.idFrom, (LPARAM)&pe);
	}
...
void CXTPSyntaxEditCtrl::OnParseEvent(XTP_NOTIFY_CODE code, WPARAM wParam, LPARAM lParam)
{
	...
	//-----------------------------------------------------------------------
	int nRSFactor	  = 3;
	int nAveRedrawTime = XTPToIntChecked(m_aveRedrawScreenTime.GetAverageValue(333));
	...
	else if (code == xtpEditOnTextBlockParsed)
	{
		...
		DWORD dwTime   = ::GetTickCount();
		bRedrawTimeOut = labs(XTPToLong(dwTime - m_dwLastRedrawTime)) >= nUpdateTimeOut;
		if (bRedrawTimeOut)
		{
			m_dwLastRedrawTime = dwTime;
		}
	...
	if (bRedraw)
	{
		...
		{
			SetTimer(TIMER_REDRAW_WHEN_PARSE, XTPToUIntChecked(nUpdateTimeOut * 5), NULL);
		}
	}
}
...
int CXTPSyntaxEditCtrl::CalculateDocumentRow(int nStartDocumentRow, int nRowDelta)
{
	...
	while (pos != NULL)
	{
		...
		if (pData && pData->m_nRow >= nNextCollapsedRow) // mark should be not within previous
														 // collapsed block
		{
...
int CXTPSyntaxEditCtrl::CalculateVisibleRow_(int nStartDocumentRow, int nDocumentRow)
{
	...
	while (pos != NULL)
	{
		...
		{
			if (pData->m_nRow
				>= nDocumentRow) // finish if mark is greater then row to calculate for
				break;
...
int CXTPSyntaxEditCtrl::GetVisibleRowsCount(int nMaxDocRow)
{
	...
	if (pMgr)
	{
		...
		while (pos != NULL)
		{
			...
			{
				if (pData->m_nRow
					>= nMaxDocRow) // finish if mark is greater then row to calculate for
					break;
...
void CXTPSyntaxEditCtrl::GetLineText(int nRow, CString& strBuffer, BOOL bAddCRLF,
									 int iCRLFStyle) const
{
...
BOOL CXTPSyntaxEditCtrl::PreTranslateMessage(MSG* pMsg)
{
	if (m_pAutoComplete->IsActive())
	{
		switch (pMsg->message)
		{
			...
			case WM_DEADCHAR:
			{
				m_pAutoComplete->SendMessage(pMsg->message, pMsg->wParam, pMsg->lParam);
			}
			break;
		}
			 ...
			 && (pMsg->message == WM_KEYDOWN || pMsg->message == WM_SYSKEYDOWN))
	{
		...
		else if (bCtrl && pMsg->wParam == (BYTE)'C' || bCtrl && pMsg->wParam == VK_INSERT)
		{
			if (IsSelectionExist())
				Copy();
		}
		else if (bCtrl && pMsg->wParam == (BYTE)'V' || bShift && pMsg->wParam == VK_INSERT)
		{
			if (CanEditDoc())
				Paste();
		}
		else if (bAlt && pMsg->wParam == VK_BACK || bCtrl && pMsg->wParam == (BYTE)'Z')
		{
			if (CanUndo())
				Undo();
		}
		else if (bCtrl && pMsg->wParam == (BYTE)'Y' || bCtrlShift && pMsg->wParam == (BYTE)'Z')
		{
			if (CanRedo())
				Redo();
		}
...
BOOL CXTPSyntaxEditCtrl::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt)
{
	if (m_pAutoComplete->IsActive())
	{
		return (BOOL)m_pAutoComplete->SendMessage(WM_MOUSEWHEEL, MAKEWPARAM(nFlags, zDelta),
									  ...
									  CXTPSyntaxEditTextBlockList* rBlocks)
{
	...
	if (ptrTxtSch)
	{
		...
		if (pScreenSchFirstTB)
		{
			...
			ptrTxtSch->GetRowColors(&txtIter, nTextRow, nColFrom, nColTo, clrDefault, rBlocks, NULL,
									pScreenSchFirstTB);
		}
	}
}
												  ...
												  CXTPSyntaxEditRowsBlockArray& rarCoBlocks) const
{
	...
	if (GetSyntaxColor())
	{
		CXTPSyntaxEditLexTextSchema* ptrTxtSch = m_pBuffer->GetLexParser()->GetTextSchema();
		if (ptrTxtSch)
			ptrTxtSch->GetCollapsableBlocksInfo(nTextRow, rarCoBlocks,
												&m_fcCollapsable->ptrTBStart);
	}
}

CXTPSyntaxEditLexTextBlock* CXTPSyntaxEditCtrl::GetOnScreenSch(int nForRow)
{
	int nLifeTime = XTPToIntChecked(m_pBuffer->GetLexParser()
										->GetSchemaOptions(m_pBuffer->GetFileExt())
										->m_dwOnScreenSchCacheLifeTime_sec);
	...
	if (ptrTxtSch)
	{
		...
		{
			int nCoStartRow = XTPToIntChecked(m_arCollapsedTextRows[i]);
...
void CXTPSyntaxEditCtrl::OnLexCfgWasChanged(XTP_NOTIFY_CODE code, WPARAM wParam, LPARAM lParam)
{
	...
	if (code == xtpEditAllConfigWasChanged || code == xtpEditClassSchWasChanged
...
BOOL CXTPSyntaxEditCtrl::NotifyMarginLBtnClick(int nRow, int nDispRow) const
{
	...
	somc.nmhdr.idFrom   = XTPToUIntPtr(GetDlgCtrlID());
	...
	if (::IsWindow(m_pParentWnd->GetSafeHwnd()))
	{
		return (BOOL)m_pParentWnd->SendMessage(WM_NOTIFY, (WPARAM)somc.nmhdr.idFrom, (LPARAM)&somc);
	}
...
{
	UINT uSize = XTPToUIntChecked(m_nAverageLineLen * abs(nRowStart - nRowEnd));
...
DROPEFFECT CXTPSyntaxEditCtrl::OnDragOver(COleDataObject* pDataObject, DWORD dwKeyState,
										  CPoint point)
{
...
void CXTPSyntaxEditCtrl::CScreenSearchCache::RemoveOld(int nTimeOut_sec)
{
	...
	if (nTimeOut_sec == 0 || nTimeOut_sec == -1
		|| labs(XTPToLong(dwTime - m_dwLastRemoveOldTime) < cnCheckTimeOut_ms))
	{
		return;
	}
	...
	for (int i = (int)GetSize() - 1; i >= 0; i--)
	{
		...
		if (labs(XTPToLong(dwTime - rSchBlk.dwLastAccessTime) >= nTimeOut_ms))
		{
			CXTPSyntaxEditLexTextSchema::Close(rSchBlk.ptrTBFirst);
			Base::RemoveAt(i);
		}
...
CXTPSyntaxEditStringDlg::CXTPSyntaxEditStringDlg(CWnd* pParent)
	: CDialog()
{
...
void CXTPSyntaxEditStringDlg::OnSize(UINT nType, int cx, int cy)
{
	...
	if (GetDlgItem(IDC_SYNTAX_EDIT))
	{
		...
		pWnd->MoveWindow(0, 0, cx, cy - H - XTP_DPI_Y(5));
...
{
	BYTE DlgTempl[] =
		{ 0X01, 0X00, 0XFF, 0XFF, 0X00, 0X00, 0X00, 0X00, 0X00, 0X00, 0X00, 0X00, 0X48, 0X00, 0XCC,
		  0X80, 0X07, 0X00, 0X00, 0X00, 0X00, 0X00, 0X78, 0X01, 0X2B, 0X01, 0X00, 0X00, 0X00, 0X00,
		  0X43, 0X00, 0X6F, 0X00, 0X6E, 0X00, 0X74, 0X00, 0X65, 0X00, 0X6E, 0X00, 0X74, 0X00, 0X20,
		  0X00, 0X65, 0X00, 0X64, 0X00, 0X69, 0X00, 0X74, 0X00, 0X6F, 0X00, 0X72, 0X00, 0X00, 0X00,
		  0X08, 0X00, 0X90, 0X01, 0X00, 0X01, 0X4D, 0X00, 0X53, 0X00, 0X20, 0X00, 0X53, 0X00, 0X68,
		  0X00, 0X65, 0X00, 0X6C, 0X00, 0X6C, 0X00, 0X20, 0X00, 0X44, 0X00, 0X6C, 0X00, 0X67, 0X00,
		  0X00, 0X00, 0X00, 0X00, 0X00, 0X00, 0X00, 0X00, 0X00, 0X00, 0X01, 0X00, 0X01, 0X50, 0X01,
		  0X01, 0X18, 0X01, 0X32, 0X00, 0X0E, 0X00, 0X01, 0X00, 0X00, 0X00, 0XFF, 0XFF, 0X80, 0X00,
		  0X4F, 0X00, 0X4B, 0X00, 0X00, 0X00, 0X00, 0X00, 0X00, 0X00, 0X00, 0X00, 0X00, 0X00, 0X00,
		  0X00, 0X00, 0X00, 0X01, 0X50, 0X40, 0X01, 0X18, 0X01, 0X32, 0X00, 0X0E, 0X00, 0X02, 0X00,
		  0X00, 0X00, 0XFF, 0XFF, 0X80, 0X00, 0X43, 0X00, 0X61, 0X00, 0X6E, 0X00, 0X63, 0X00, 0X65,
		  0X00, 0X6C, 0X00, 0X00, 0X00, 0X00, 0X00, 0X00, 0X00, 0X00, 0X00, 0X00, 0X00, 0X00, 0X00,
		  0X84, 0X10, 0XB1, 0X50, 0X00, 0X00, 0X01, 0X00, 0X77, 0X01, 0X11, 0X01, 0XF1, 0X03, 0X00,
		  0X00, 0XFF, 0XFF, 0X81, 0X00, 0X00, 0X00, 0X00, 0X00, 0X00, 0X00, 0X00, 0X00, 0X00, 0X00,
		  0X00, 0X00, 0X00, 0X00, 0X01, 0X50, 0X05, 0X00, 0X18, 0X01, 0X32, 0X00, 0X0E, 0X00, 0X23,
		  0XE1, 0X00, 0X00, 0XFF, 0XFF, 0X80, 0X00, 0X43, 0X00, 0X75, 0X00, 0X74, 0X00, 0X00, 0X00,
		  0X00, 0X00, 0X00, 0X00, 0X00, 0X00, 0X00, 0X00, 0X00, 0X00, 0X00, 0X00, 0X00, 0X00, 0X01,
		  0X50, 0X44, 0X00, 0X18, 0X01, 0X32, 0X00, 0X0E, 0X00, 0X22, 0XE1, 0X00, 0X00, 0XFF, 0XFF,
		  0X80, 0X00, 0X43, 0X00, 0X6F, 0X00, 0X70, 0X00, 0X79, 0X00, 0X00, 0X00, 0X00, 0X00, 0X00,
		  0X00, 0X00, 0X00, 0X00, 0X00, 0X00, 0X00, 0X00, 0X00, 0X01, 0X50, 0X83, 0X00, 0X18, 0X01,
		  0X32, 0X00, 0X0E, 0X00, 0X25, 0XE1, 0X00, 0X00, 0XFF, 0XFF, 0X80, 0X00, 0X50, 0X00, 0X61,
		  0X00, 0X73, 0X00, 0X74, 0X00, 0X65, 0X00, 0X00, 0X00, 0X00, 0X00, 0X00, 0X00, 0X00, 0X00,
		  0X00, 0X00, 0X00, 0X00, 0X00, 0X00, 0X00, 0X00, 0X01, 0X50, 0XC2, 0X00, 0X18, 0X01, 0X32,
		  0X00, 0X0E, 0X00, 0X2B, 0XE1, 0X00, 0X00, 0XFF, 0XFF, 0X80, 0X00, 0X55, 0X00, 0X6E, 0X00,
		  0X64, 0X00, 0X6F, 0X00, 0X00, 0X00, 0X00, 0X00 };
