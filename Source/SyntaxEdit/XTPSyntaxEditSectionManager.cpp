	...
	, uValue((UINT)-1)
{
	...
	{
		CXTPSyntaxEditSectionManager().SplitString(csValue, csDesc, srcValue, _T(';'));
	}
}

const XTP_EDIT_SCHEMAFILEINFO& XTP_EDIT_SCHEMAFILEINFO::
	operator=(const XTP_EDIT_SCHEMAFILEINFO& info)
{
	csName  = info.csName;
	csValue = info.csValue;
	csDesc  = info.csDesc;
...
POSITION CXTPSyntaxEditSchemaFileInfoList::LookupName(const CString& csName,
													  XTP_EDIT_SCHEMAFILEINFO& info) const
{
...
POSITION CXTPSyntaxEditSchemaFileInfoList::LookupValue(const CString& csValue,
													   XTP_EDIT_SCHEMAFILEINFO& info) const
{
	for (POSITION pos = GetHeadPosition(); pos;)
	{
...
POSITION CXTPSyntaxEditSchemaFileInfoList::LookupDesc(const CString& csDesc,
													  XTP_EDIT_SCHEMAFILEINFO& info) const
{
	for (POSITION pos = GetHeadPosition(); pos;)
	{
...
POSITION CXTPSyntaxEditSchemaFileInfoList::LookupValue(const UINT& uValue,
													   XTP_EDIT_SCHEMAFILEINFO& info) const
{
	for (POSITION pos = GetHeadPosition(); pos;)
	{
...
BOOL CXTPSyntaxEditSectionManager::SplitString(CString& csLeft, CString& csRight,
											   const CString& csBuffer, TCHAR chSep) const
{
...
BOOL CXTPSyntaxEditSectionManager::ParseSectionKey(XTP_EDIT_SCHEMAFILEINFO& info,
												   const CString& csBuffer) const
{
...
int CXTPSyntaxEditSectionManager::GetSectionNames(CStringArray& arSections,
												  LPCTSTR lpszFilePath) const
{
	TCHAR* pszBuffer = NULL;
	int nMaxSize	 = 1024;
	for (;;)
	{
		pszBuffer  = new TCHAR[XTPToUIntChecked(nMaxSize)];
		long lSize = XTPToLongChecked(
			::GetPrivateProfileSectionNames(pszBuffer, XTPToDWORDChecked(nMaxSize), lpszFilePath));
...
int CXTPSyntaxEditSectionManager::GetSectionKeyList(CXTPSyntaxEditSchemaFileInfoList& infoList,
													...
													LPCTSTR lpszSectionName) const
{
	...
	{
		pszBuffer  = new TCHAR[XTPToUIntChecked(nMaxSize)];
		long lSize = XTPToLongChecked(::GetPrivateProfileSection(lpszSectionName, pszBuffer,
																 XTPToDWORDChecked(nMaxSize),
																 lpszFilePath));
		if ((lSize > 0) && (lSize == (nMaxSize - 2)))
		{
			nMaxSize *= 2;
...
int CXTPSyntaxEditSectionManager::GetSectionNames(CStringArray& arSections,
												  CStringArray& arPassedString) const
{
