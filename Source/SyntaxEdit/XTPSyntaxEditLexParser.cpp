...
#	define DBG_TRACE_TIME_END(nID, strComment)                                                    \
		{                                                                                          \
			DWORD dwTime1 = ::GetTickCount();                                                      \
			TRACE(_T("%s (%.3f sec) \n"), strComment,                                              \
				  labs(dwTime1 - dbg_TT_##nID##_dwTime_0) / 1000.0);                               \
		}
...
public:
	...
	~CXTPDebugHelper()
	{
		...
		TRACE(_T("%s (%.3f sec) \n"), (LPCTSTR)m_strComment,
			  labs(XTPToLongChecked(dwTime1 - m_dwTime0)) / 1000.0);
	};
};
...
extern BOOL SortTagsInLexVarArray(CXTPSyntaxEditLexVariantPtrArray& rarTags, BOOL bAscending,
								  BOOL bNoCase);
...
CString DBG_TraceTB_StartEndCls(CXTPSyntaxEditLexTextBlock* pTB)
{
	...
	CString str;
	str.Format(_T("(%d, %d - %d, %d) - %s"), pTB->m_PosStartLC.nLine, pTB->m_PosStartLC.nCol,
			   pTB->m_PosEndLC.nLine, pTB->m_PosEndLC.nCol,
...
const CXTPSyntaxEditLexTokensDef& CXTPSyntaxEditLexTokensDef::
	operator=(const CXTPSyntaxEditLexTokensDef& rSrc)
{
...
//  ASSERT(this == (CXTPSyntaxEditLexTextSchema*)m_CloserManager.m_ptrParentObj ||
//  !m_CloserManager.m_ptrParentObj);
//
//  m_CloserManager.SetParentObject(this);
//
...
BOOL CXTPSyntaxEditLexTextSchema::IsBlockStartStillHere(CTextIter* pTxtIter,
...
void CXTPSyntaxEditLexTextSchema::TraceTxtBlocks(BOOL bFull)
{
	...
	{
		TRACE(_T("(%05d) startPos=(%d, %d) endPos=(%d, %d), [%s]\n"), i, ptrTB->m_PosStartLC.nLine,
			  ptrTB->m_PosStartLC.nCol, ptrTB->m_PosEndLC.nLine, ptrTB->m_PosEndLC.nCol,
												...
												BOOL bSendProgressEvents)
{
	...
	if (bSendProgressEvents)
	{
		m_pConnectMT->PostEvent(xtpEditOnParserEnded, XTPToWPARAM(nParseRes), 0,
								xtpNotifyGuarantyPost | xtpNotifyDirectCallForOneThread);
	}
												  ...
												  BOOL bSendProgressEvents)
{
	...
	if (!pLCStart || *pLCStart == XTP_EDIT_LINECOL::Pos1 || nParseRes == -1 || !ptrStartTB)
	{
		nParseRes = 0;

		if (m_ptrFirstBlock)
			m_ptrOldChainTBFirst = m_ptrFirstBlock->m_ptrNext;
		else
			m_ptrOldChainTBFirst = NULL;
													...
													BOOL bStarted)
{
	...
	if (!bStarted)
	{
		...
		DBG_TRACE_PARSE_RUN_BLOCKS(
			_T("\nREPARSE will start from pos =(%d, %d), Run class [%s] {%d}-noEndedStack \n"),
			posStart.nLine, posStart.nCol,
			ptrTB->m_ptrParent ? (ptrTB->m_ptrParent->m_ptrLexClass
									  ? ptrTB->m_ptrParent->m_ptrLexClass->m_strClassName
									  : CString(_T("?<NULL> (parent)")))
							   : (ptrTB->m_ptrLexClass ? ptrTB->m_ptrLexClass->m_strClassName
													   : CString(_T("?<NULL>"))),
	...
	while (bStarted && !bEnded && (bRunEOF || !pTxtIter->IsEOF()))
	{
		...
		if (bEnded)
		{
			...
			DBG_TRACE_PARSE_RUN_BLOCKS(
				_T("(%08x) ENDED startPos=(%d, %d) endPos=(%d, %d), [%s] {%d}-noEndedStack \n"),
				(CXTPSyntaxEditLexTextBlock*)ptrStartTB, ptrStartTB->m_PosStartLC.nLine,
				ptrStartTB->m_PosStartLC.nCol, ptrStartTB->m_PosEndLC.nLine,
				ptrStartTB->m_PosEndLC.nCol, ptrStartTB->m_ptrLexClass->m_strClassName,
				m_nNoEndedClassesCount);
...
int CXTPSyntaxEditLexTextSchema::Run_ClassesUpdate2(
	CTextIter* pTxtIter, CXTPSyntaxEditLexClassPtrArray* pArClasses,
	CXTPSyntaxEditLexTextBlockPtr ptrParentTB, CXTPSyntaxEditLexOnScreenParseCnt* pOnScreenRunCnt)
{
	...
	for (int i = 0; i < nCCount; i++)
	{
		...
		do
		{
			...
			if ((nPres & xtpEditLPR_StartFound) && !!ptrTB && !ptrTB->m_ptrPrev)
			{
				...
				if (!pOnScreenRunCnt)
				{
					...
					if (!!m_ptrNewChainTB2 && !!m_ptrNewChainTB2->m_ptrNext)
					{
							...
							&& m_curInvalidZone.m_posEnd.IsValidData())
						{
							...
							DBG_TRACE_PARSE_RUN_UPDATE(
								_T("    <VALIDATE> TB next OLD: %s [NoEndedStack=%d]\n"),
								DBG_TraceTB_StartEndCls(m_ptrNewChainTB2->m_ptrNext),
								m_nNoEndedClassesCount);

							if (m_ptrNewChainTB2->m_ptrNext->m_PosStartLC == ptrTB->m_PosStartLC
								&& ptrTB->IsEqualLexClasses(m_ptrNewChainTB2->m_ptrNext)
								&& m_curInvalidZone.m_posEnd.IsValidData()
								&& m_ptrNewChainTB2->m_ptrNext->m_PosStartLC
									   > m_curInvalidZone.m_posEnd)
							{
								// BREAK.
				...
				// DEBUG ////////////////////////////////////////////////////////////////
				DBG_TRACE_PARSE_RUN_BLOCKS(_T("(%08x) START startPos=(%d, %d), ______________ ")
										   _T("[%s] {%d}-noEndedStack \n"),
										   (CXTPSyntaxEditLexTextBlock*)ptrTB,
										   ptrTB->m_PosStartLC.nLine, ptrTB->m_PosStartLC.nCol,
										   ptrTB->m_ptrLexClass->m_strClassName,
										   m_nNoEndedClassesCount);
				// END DEBUG ////////////////////////////////////////////////////////////
			}
			...
			if (bEnded)
			{
				...
				DBG_TRACE_PARSE_RUN_BLOCKS(
					_T("(%08x) ENDED startPos=(%d, %d) endPos=(%d, %d), [%s] {%d}-noEndedStack \n"),
					(CXTPSyntaxEditLexTextBlock*)ptrTB, ptrTB->m_PosStartLC.nLine,
					ptrTB->m_PosStartLC.nCol, ptrTB->m_PosEndLC.nLine, ptrTB->m_PosEndLC.nCol,
...
UINT CXTPSyntaxEditLexTextSchema::SendEvent_OnTextBlockParsed(CXTPSyntaxEditLexTextBlock* pTB)
{
	...
	BOOL bIsSubscribers = m_pConnectMT->PostEvent(xtpEditOnTextBlockParsed, dwTBid, 0,
												  xtpNotifyDirectCallForOneThread);
	if (bIsSubscribers)
	{
...
void CXTPSyntaxEditLexTextSchema::FinishNewChain(BOOL bByBreak, BOOL bEOF)
{
	...
	if (m_ptrNewChainTB2)
	{
		...
		if (bByBreak && !bEOF)
		{
			...
			while (!m_ptrNewChainTB2->m_PosEndLC.IsValidData() && !!m_ptrNewChainTB2->m_ptrPrev
				   && m_ptrNewChainTB2 != m_ptrNewChainTB1)
			{
...
void CXTPSyntaxEditLexTextSchema::EndBlocksByParent(CXTPSyntaxEditLexTextBlock* pTBStart,
													CXTPSyntaxEditLexTextBlock* pTBEnd)
{
	...
	for (; pTBEnd->m_ptrNext.operator!=(pTB_chi); pTB_chi = pTB_chi->m_ptrNext)
	{
											 ...
											 CXTPSyntaxEditLexOnScreenParseCnt* pOnScreenRunCnt)
{
	...
	for (int i = 0; i < _countof(pArClasses); i++)
	{
		if (pArClasses[i] && pArClasses[i]->GetSize())
		{
			...
			if (nRes
				& (xtpEditLPR_Error | xtpEditLPR_RunBreaked | xtpEditLPR_RunFinished
				   | xtpEditLPR_RunRestart))
			{
				break;
			}
		}
...
void CXTPSyntaxEditLexTextSchema::SeekNextEx(CTextIter* pTxtIter, CXTPSyntaxEditLexClass* pRunClass,
											 CXTPSyntaxEditLexOnScreenParseCnt* pOnScreenRunCnt,
											 int nChars)
{
	...
	pTxtIter->SeekNext(XTPToDWORDChecked(nChars));
...
void CXTPSyntaxEditLexTextSchema::UpdateLastSchBlock(CXTPSyntaxEditLexTextBlock* pLastTB,
													 BOOL bPermanently)
{
	CSingleLock singleLock(GetDataLoker(), TRUE);
...
void CXTPSyntaxEditLexTextSchema::GetTextAttributes(XTP_EDIT_TEXTBLOCK& rTB,
													CXTPSyntaxEditLexTextBlock* pTextBlock)
{
...
void CXTPSyntaxEditLexTextSchema::TraceClrBlocks(CXTPSyntaxEditTextBlockArray& arBlocks)
{
	...
	for (int i = 0; i < arBlocks.GetSize(); i++)
	{
		...
		TRACE(_T("(%02d) start =%d, ?end=%d, color=%X \n"), i, BlkI.nPos, BlkI.nNextBlockPos,
			  BlkI.clrBlock.crText);
	}
	TRACE(_T("***\n"));
#else
											  ...
											  CXTPSyntaxEditTextBlockArray& arBlocks)
{
	...
	{
		XTP_EDIT_TEXTBLOCK& BlkI = arBlocks[i];
		if (rClrB.nNextBlockPos < BlkI.nPos || rClrB.nPos >= BlkI.nNextBlockPos)
		{
			continue;
		}
	...
	for (i = nCount - 1; i >= 0; i--)
	{
		...
		else if (rClrB.nPos <= BlkI.nPos && rClrB.nNextBlockPos >= BlkI.nNextBlockPos)
		{
			arBlocks.RemoveAt(i);
		}
		else if (rClrB.nPos > BlkI.nPos && rClrB.nPos < BlkI.nNextBlockPos)
		{
			BlkI.nNextBlockPos = rClrB.nPos;
			arBlocks[i]		   = BlkI;
			if (!bInserted)
...
void CXTPSyntaxEditLexTextSchema::GetRowColors(CTextIter* pTxtIter, int nRow, int nColFrom,
											   int nColTo, const XTP_EDIT_COLORVALUES& clrDefault,
...

{
	...
	if (bLocked)
	{
		...
		if (!ptrTBstart || ptrTBstart == ptrTBfirst || ptrTBstart->IsLookLikeClosed())
		{
			ptrTBstart = ptrTBfirst ? ptrTBfirst->m_ptrNext : CXTPSyntaxEditLexTextBlockPtr(NULL);
		}
	}
		...
		for (; nStackCount >= 0;)
		{
			...
			if (pTB->m_PosStartLC > LCStart)
			{
				bltTB.nNextBlockPos = TB_PosEndLC.nLine == nRow
										  ? min(TB_PosEndLC.nCol + 1, nLineLen)
										  : nLineLen;
...
void CXTPSyntaxEditLexTextSchema::GetCollapsableBlocksInfo(
	int nRow, CXTPSyntaxEditRowsBlockArray& rArBlocks,
	CXTPSyntaxEditLexTextBlockPtr* pptrTBStartCache)
{
	...
	for (CXTPSyntaxEditLexTextBlock* pTB = ptrTBstart; pTB; pTB = pTB->m_ptrNext)
	{
		...
		if (nRow < 0 || (nRow >= pTB->m_PosStartLC.nLine && nRow <= TB_PosEndLC.nLine))
		{
			...
			if (pTB->m_PosStartLC.nLine < TB_PosEndLC.nLine && !!pTB->m_ptrParent)
			{
				...
				if (pLexClass && nCollapsable
					&& (nCollapsable == 2
						|| !pTB->m_bEndByParent && TB_PosEndLC != XTP_EDIT_LINECOL::MAXPOS))
				{
													  ...
													  CXTPSyntaxEditLexClassPtrArray* ptrClasses)
{
	...
	for (int i = 0; i < nCount; i++)
	{
		...
		CXTPSyntaxEditColorInfo* pColors = pTheme ? pTheme->GetColorInfo(strCName,
																		 pTheme->GetFileName())
												  : NULL;
		if (!pColors)
		{
		...
		while (posParam)
		{
			...
			if (_tcsnicmp(strAttrName, XTPLEX_ATTR_COLORPREFIX, XTPToSizeTChecked(c_nClrPrefLen))
				== 0)
			{
...
CXTPSyntaxEditLexClassPtrArray*
	CXTPSyntaxEditLexClassSchema::GetChildrenFor(CXTPSyntaxEditLexClass* pClass, BOOL& rbSelfChild)
{
	rbSelfChild = FALSE;
...
CXTPSyntaxEditLexClassPtrArray* CXTPSyntaxEditLexClassSchema::GetDynChildrenFor(
	CXTPSyntaxEditLexClass* pClass, BOOL& rbSelfChild)
{
	rbSelfChild = FALSE;
...
CXTPSyntaxEditLexObj_ActiveTags*
	CXTPSyntaxEditLexClassSchema::GetActiveTagsFor(CXTPSyntaxEditLexClass* pTopClass)
{
	if (!pTopClass)
	{
		ASSERT(FALSE);
												   ...
												   CXTPSyntaxEditLexClass* pCSrc, int nLevel)
{
	...
	for (int nC = 0; nC < _countof(pArClasses); nC++)
	{
		...
		for (int i = 0; i < nCount; i++)
		{
			...
			if (!ptrCsrc || bShort && ptrCsrc->GetAttribute_BOOL(XTPLEX_ATTR_PARSEONSCREEN, FALSE))
			{
				continue;
			}
...
BOOL CXTPSyntaxEditLexClassSchema::Build_ChildrenFor(BOOL bDynamic, CXTPSyntaxEditLexClass* pCBase,
...
CXTPSyntaxEditLexTextBlock*
	CXTPSyntaxEditLexTextBlock::GetPrevChild(CXTPSyntaxEditLexTextBlock* pChild, BOOL bWithAddRef)
{
...
{
	if (!m_ptrLexClass && !m_ptrPrev && !m_ptrNext && !m_ptrLastChild && !m_ptrParent)
	{
		return TRUE;
	}
	return FALSE;
...
void CXTPSyntaxEditLexParser::CXTPSyntaxEditParseThreadParams::AddParseZone(
	const CXTPSyntaxEditTextRegion& rZone)
{
	...
	for (int i = 0; i < nCount; i++)
	{
		...
		if (rZone.m_posEnd >= zoneI.m_posStart && rZone.m_posStart <= zoneI.m_posStart
		...
		else if (rZone.m_posStart <= zoneI.m_posEnd && rZone.m_posEnd >= zoneI.m_posEnd
		...
		else if (rZone.m_posStart >= zoneI.m_posStart && rZone.m_posEnd <= zoneI.m_posEnd)
		{
			return; // nothing
		}
	}
...
CXTPSyntaxEditLexTextBlock*
	CXTPSyntaxEditLexTextSchema::FindNearestTextBlock(XTP_EDIT_LINECOL posText)
{
	CSingleLock singleLock(GetDataLoker(), TRUE);
...

{
	...
	{
		ASSERT(FALSE);
		TRACE(_T("ERROR! Cannot enter critical section. [Dead lock, hang up???] ")
			  _T("CXTPSyntaxEditLexTextSchema::UpdateTextBlocks() \n"));
	...
	int nURes = UpdateTBNearest(ptrNarestTB1, nLineDiff, nColDiff, posFrom, posTo, eEditAction);
	...
	for (; pTB; pTB = pTB->m_ptrNext)
	{
		...
		//-------------------------------------------------------------------
		BOOL bIsBlockOutside = eEditAction == xtpEditActInsert && pTB->m_PosStartLC >= posFrom
							   || eEditAction == xtpEditActDelete && pTB->m_PosStartLC >= posTo;
		//-----------------------------
		int nELine = eEditAction == xtpEditActInsert ? posTo.nLine : posFrom.nLine;

		if (bIsBlockOutside && nLineDiff == 0 && pTB->m_PosStartLC.nLine != nELine
...
#ifdef DBG_TRACE_LOAD_CLASS_SCH
		{
			BOOL bEmpty = ptrLexClass->IsEmpty()
						  && ptrLexClass->m_Parent.eOpt != xtpEditOptParent_file;
...
void CXTPSyntaxEditLexParser::SetTextSchema(CXTPSyntaxEditLexTextSchema* pTextSchema)
{
	...
	if (pTextSchema)
	{
		...
		CXTPNotifyConnection* pConnection = m_ptrTextSchema ? m_ptrTextSchema->GetConnection()
															: NULL;
		if (pConnection)
		{
			m_pSinkMT->Advise(pConnection, xtpEditOnParserStarted,
							  CreateNotfySinkClassDelegate(
								  this, &XTPSyntaxEditLexAnalyser::CXTPSyntaxEditLexParser::
											OnParseEvent_NotificationHandler));
			m_pSinkMT->Advise(pConnection, xtpEditOnTextBlockParsed,
							  CreateNotfySinkClassDelegate(
								  this, &XTPSyntaxEditLexAnalyser::CXTPSyntaxEditLexParser::
											OnParseEvent_NotificationHandler));
			m_pSinkMT->Advise(pConnection, xtpEditOnParserEnded,
							  CreateNotfySinkClassDelegate(
								  this, &XTPSyntaxEditLexAnalyser::CXTPSyntaxEditLexParser::
											OnParseEvent_NotificationHandler));
		}
...
{
	if (!pLVArray || pLVArray->m_nObjType != xtpEditLVT_LVArrayPtr || !pLVArray->m_ptrLVArrayPtr)
	{
		ASSERT(FALSE);
		return;
	}
...
////////////////////////////////////////////////////////////////////////////
CXTPSyntaxEditLexParser::CXTPSyntaxEditParseThreadParams*
	CXTPSyntaxEditLexParser::GetParseInThreadParams()
{
...
void CXTPSyntaxEditLexParser::StartParseInThread(CXTPSyntaxEditBufferManager* pBuffer,
												 const XTP_EDIT_LINECOL* pLCStart,
												 const XTP_EDIT_LINECOL* pLCEnd, int eEditAction,
												 BOOL bRunWithoutWait)
{
	UNREFERENCED_PARAMETER(eEditAction);
	...
	if (!SetParseThreadPriority(m_nParseThreadPriority))
	{
		...
		StartParseInThread(pBuffer, pLCStart, pLCEnd, eEditAction, bRunWithoutWait);
	}
...
void CXTPSyntaxEditLexParser::OnParseEvent_NotificationHandler(XTP_NOTIFY_CODE Event, WPARAM wParam,
															   LPARAM lParam)
{
...
UINT CXTPSyntaxEditLexParser::ThreadParseProc(LPVOID pParentParser)
{
	...
	//  try
	{
		...
		DWORD dwSelfCloseTimeout_ms = ptrParser->GetSchemaOptions(pPRMs->ptrBuffer->GetFileExt())
										  ->m_dwParserThreadIdleLifeTime_ms;

		DWORD dwWaitFilter_ms =
			ptrParser->GetSchemaOptions(pPRMs->ptrBuffer->GetFileExt())->m_dwEditReparceTimeout_ms;
		...
		do
		{
			...
			if (dwWaitRes == WAIT_OBJECT_0)
			{
				...
				// Update Options
				dwSelfCloseTimeout_ms = ptrParser->GetSchemaOptions(pPRMs->ptrBuffer->GetFileExt())
											->m_dwParserThreadIdleLifeTime_ms;

				dwWaitFilter_ms = ptrParser->GetSchemaOptions(pPRMs->ptrBuffer->GetFileExt())
									  ->m_dwEditReparceTimeout_ms;
				// END Update Options
				...
				do
				{
					...
					int nParseRes = ptrTextSch->RunParseUpdate(TRUE, &txtIter, pLCStart, pLCEnd,
															   TRUE);
					if (nParseRes & xtpEditLPR_Error)
					{
						...
						nParseRes = ptrTextSch->RunParseUpdate(TRUE, &txtIter, &posLC1, NULL);
					...
					// DEBUG
					{
						CString sDBGzone = DBG_TraceIZone(&zoneValid.m_posStart,
														  &zoneValid.m_posEnd);
						// TRACE(_T("- Parser set START Event. Add invalid Zone [ %s ] \n"),
						// sDBGpos);
						DBG_TRACE_PARSE_START_STOP(
							_T("* Parser Ended. (result=%x) %s%s%s (%.3f sec) VALID Zone[ %s ] \n"),
							nParseRes,
							(nParseRes & xtpEditLPR_RunBreaked) ? _T(" BREAKED ") : _T(""),
							(nParseRes & xtpEditLPR_Error) ? _T(" ERROR ") : _T(""),
							(nParseRes & xtpEditLPR_RunFinished) ? _T(" <F I N I S H E D> ")
																 : _T(""),
							labs(XTPToLongChecked(dwTime1 - dwTime0)) / 1000.0, (LPCTSTR)sDBGzone);
					}
...
#	if _MSC_VER >= 1300
	dc << "\t Parent=";
	dc.DumpAsHex((INT_PTR)(CXTPSyntaxEditLexTextBlock*)m_ptrParent);
	dc << ", Prev=";
	dc.DumpAsHex((INT_PTR)(CXTPSyntaxEditLexTextBlock*)m_ptrPrev);
	dc << ", Next=";
	dc.DumpAsHex((INT_PTR)(CXTPSyntaxEditLexTextBlock*)m_ptrNext);
	dc << ", LastChild=";
	dc.DumpAsHex((INT_PTR)(CXTPSyntaxEditLexTextBlock*)m_ptrLastChild);
#	endif
}
#endif

BOOL CXTPSyntaxEditLexTextSchema::RunParseOnScreen(
	CTextIter* pTxtIter, int nRowStart, int nRowEnd,
...
CXTPSyntaxEditLexTextBlockPtr
	CXTPSyntaxEditLexTextSchema::InitScreenSch_RunTopClass(CTextIter* pTxtIter)
{
												...
												CXTPSyntaxEditLexTextBlockPtr& rPtrTBParentToRun)
{
	...
	if (!!m_ptrNewChainTB2 && !!pLastSchBlock
		&& (pLastSchBlock->m_PosEndLC.IsValidData() && nRowStart > pLastSchBlock->m_PosEndLC.nLine
			|| !pLastSchBlock->m_PosEndLC.IsValidData()
				   && nRowStart > pLastSchBlock->m_PosStartLC.nLine))
	{
		return FALSE;
	}
	...
	{
		if (pTB->m_PosStartLC < lcStart && pTB->GetPosEndLC() >= lcStart
			|| ptrTBstart.operator==(pTB))
		{
			pTBLast = pTB;
		}
	...
	for (pTB = pTBLast; pTB; pTB = pTB->m_ptrParent)
	{
		...
		if (pTB == pTBLast)
		{
			...
			if (pTB->m_PosStartLC.nLine <= nRowStart && pTB->m_PosEndLC.nLine >= nRowStart
				|| ptrTBstart.operator==(pTB))
			{
				pTBLast = pTB;
			}
...
CXTPSyntaxEditLexTextBlock*
	CXTPSyntaxEditLexTextSchema::CopyShortTBtoFull(CXTPSyntaxEditLexTextBlock* pTB)
{
...
CXTPSyntaxEditLexClass* CXTPSyntaxEditLexTextSchema::FindLexClassByID(
	CXTPSyntaxEditLexClassPtrArray* pClassesAr, int nClassID)
{
	if (!pClassesAr)
	{
		return NULL;
													 ...
													 CXTPSyntaxEditLexOnScreenParseCnt* pRunCnt)
{
	...
	while (!bEnded && (bRunEOF || !pTxtIter->IsEOF()))
	{
		...
		if (bEnded)
		{
			...
			DBG_TRACE_PARSE_RUN_BLOCKS(
				_T("(%08x) ENDED startPos=(%d, %d) endPos=(%d, %d), [%s] {%d}-noEndedStack \n"),
				(CXTPSyntaxEditLexTextBlock*)ptrTBParentToRun, ptrTBParentToRun->m_PosStartLC.nLine,
				ptrTBParentToRun->m_PosStartLC.nCol, ptrTBParentToRun->m_PosEndLC.nLine,
				ptrTBParentToRun->m_PosEndLC.nCol, ptrTBParentToRun->m_ptrLexClass->m_strClassName,
				m_nNoEndedClassesCount);
		}
...
CXTPSyntaxEditLexParserSchemaOptions::CXTPSyntaxEditLexParserSchemaOptions(
	const CXTPSyntaxEditLexParserSchemaOptions& rSrc)
{
...
const CXTPSyntaxEditLexParserSchemaOptions& CXTPSyntaxEditLexParserSchemaOptions::
												...
												CXTPSyntaxEditLexParserSchemaOptions* pOpt)
{
	...
	pOpt->m_bFirstParseInSeparateThread =
		ptrTopCls->GetAttribute_BOOL(XTPLEX_ATTRG_FIRSTPARSEINSEPARATETHREAD, FALSE, TRUE);

	pOpt->m_bEditReparceInSeparateThread =
		ptrTopCls->GetAttribute_BOOL(XTPLEX_ATTRG_EDITREPARCEINSEPARATETHREAD, FALSE, TRUE);
	...
	pOpt->m_dwMaxBackParseOffset = (DWORD)ptrTopCls->GetAttribute_int(
		XTPLEX_ATTRG_MAXBACKPARSEOFFSET, FALSE, XTP_EDIT_LEXPARSER_MAXBACKOFFSETDEFAULT);

	pOpt->m_dwEditReparceTimeout_ms = (DWORD)ptrTopCls->GetAttribute_int(
		XTPLEX_ATTRG_EDITREPARCETIMEOUT_MS, FALSE, XTP_EDIT_LEXPARSER_REPARSETIMEOUTMS);

	pOpt->m_dwOnScreenSchCacheLifeTime_sec =
		(DWORD)ptrTopCls->GetAttribute_int(XTPLEX_ATTRG_ONSCREENSCHCACHELIFETIME_SEC, FALSE,
										   XTP_EDIT_LEXPARSER_ONSCREENSCHCACHELIFETIMESEC);

	pOpt->m_dwParserThreadIdleLifeTime_ms = (DWORD)ptrTopCls->GetAttribute_int(
		XTPLEX_ATTRG_PARSERTHREADIDLELIFETIME_SEC, FALSE, XTP_EDIT_LEXPARSER_THREADIDLELIFETIMESEC);
...
const CXTPSyntaxEditLexParserSchemaOptions*
	CXTPSyntaxEditLexParser::GetSchemaOptions(const CString& strExt)
{
	CSingleLock singLockMain(&m_csParserData);
