...
void CXTPSyntaxEditPropertiesPageFont::DoDataExchange(CDataExchange* pDX)
{
	...
	DDV_MaxChars(pDX, m_csSize, XTPToIntChecked(m_uFaceSize));
	//}}AFX_DATA_MAP
}
...
{
	// update paint manager font(paint manager font is set with
	// CXTPSyntaxEditCtrl::CreateFontIndirect).
	LOGFONT lf = { 0 };
	if (!GetSafeLogFont(lf))
		return FALSE;
...
void CXTPSyntaxEditPropertiesPageFont::InitFontCombo()
{
	...
	CWindowDC dc(NULL);
	::EnumFontFamiliesEx(dc.m_hDC, &lfEnum, (FONTENUMPROC)EnumFontFamExProc,
						 (LPARAM)&m_wndComboName, 0);
...
void CXTPSyntaxEditPropertiesPageFont::InitSizeCombo()
{
	...
	CWindowDC dc(NULL);
	::EnumFontFamiliesEx(dc.m_hDC, &lf, (FONTENUMPROC)EnumFontFamExProc, (LPARAM)&m_wndComboSize,
						 0);

	if (m_wndComboSize.GetCount() == 0)
	{
		const TCHAR* szSize[] = { _T("8"),  _T("9"),  _T("10"), _T("11"), _T("12"), _T("14"),
	...
	m_csSize.Format(_T("%i"), -::MulDiv(lf.lfHeight, 72, ::GetDeviceCaps(dc.m_hDC, LOGPIXELSY)));
...
void CXTPSyntaxEditPropertiesPageFont::InitScriptCombo()
{
	...
	CWindowDC dc(NULL);
	::EnumFontFamiliesEx(dc.m_hDC, &lf, (FONTENUMPROC)EnumFontFamExProc, (LPARAM)&m_wndComboScript,
						 0);
...
void CXTPSyntaxEditPropertiesPageFont::OnSelChangeComboSizes()
{
	...
	CWindowDC dc(NULL);
	lf.lfHeight = -MulDiv(_ttoi(m_csSize), ::GetDeviceCaps(dc.m_hDC, LOGPIXELSY), 72);
