...
{
#ifdef _UNICODE
	return XTPToIntChecked(_tcsnbcnt(pcszStr, XTPToSizeTChecked(nChars)) * sizeof(TCHAR));
#else
	return (int)_tcsnbcnt(pcszStr, XTPToUInt(nChars));
#endif
}
...
AFX_INLINE int xtpEdit_StrCharsToBytes(LPCTSTR pcszStr, int nChars)
{
	...
	return XTPToIntChecked(nChars * sizeof(TCHAR));
#else
	LPCTSTR p2;
	if (nChars == 1)
	{
	...
	{
		p2 = _tcsninc(pcszStr, XTPToUInt(nChars));
	}
...
CXTPSyntaxEditTextIterator::CXTPSyntaxEditTextIterator(CXTPSyntaxEditBufferManager* pData)
{
	...
	int nMaxBackOffset_sch = (int)m_pData->GetLexParser()
								 ->GetSchemaOptions(m_pData->GetFileExt())
								 ->m_dwMaxBackParseOffset;
...
void CXTPSyntaxEditTextIterator::SeekBegin()
{
	...
	m_mapLine2Len.InitHashTable(XTPToUIntChecked(nHSize), FALSE);
	...
	memset(m_arBuffer.GetData(), 0, XTPToSizeTChecked(m_arBuffer.GetSize()));
...
BOOL CXTPSyntaxEditTextIterator::SeekPos(const XTP_EDIT_LINECOL& posLC, HANDLE hBreakEvent)
{
	...
	while (m_LCpos < posLC && !m_bEOF)
	{
		...
		{
			nSeek = 0;
			if (!m_mapLine2Len.Lookup(XTPToDWORDChecked(m_LCpos.nLine), nSeek) || !nSeek)
			{
		...
		SeekNext(XTPToDWORDChecked(nSeek));
...
LPCTSTR CXTPSyntaxEditTextIterator::GetText(int nCharsBuf) // don't remove line end chars
{
	...
	if (!m_nBufSizeB || (m_nBufSizeB < CHR_TO_B(nCharsBuf)))
	{
		...
		while (m_nNextLine <= nEndRow && m_nBufSizeB < CHR_TO_B(nCharsBuf))
		{
			...
			//---------------------------------------------------------------------------
			int nNeedSizeB = m_nBufOffsetB + m_nBufSizeB + nLenB + max(256, nCharsBuf * 2);
			...
			*pBuf = _T('\0');
			...
			STRCPY_S(pBuf, XTPToSizeTChecked(strBuf.GetLength() + 1), strBuf);
			...
			m_mapLine2Len[XTPToDWORDChecked(m_nNextLine)] = nLenC;
...
LPCTSTR CXTPSyntaxEditTextIterator::SeekNext(DWORD dwChars, int nCharsBuf)
{
	...
	{
		GetText(XTPToIntChecked(nCharsBuf + dwChars));
	}
	...
	if (m_nBufSizeB >= CHR_TO_B(dwChars) && m_nBufSizeB)
	{
		...
		int nStepB = xtpEdit_StrCharsToBytes(pText, XTPToIntChecked(dwChars));
		ASSERT(nStepB > 0);
		...
		if (dwChars && !m_bEOF)
		{
			LCPosAdd(m_LCpos, XTPToIntChecked(dwChars));
		}
...
LPCTSTR CXTPSyntaxEditTextIterator::SeekPrev()
{
	...
	if (m_nBufOffsetB > 0 && m_nBufSizeB > 0 && m_LCpos > XTP_EDIT_LINECOL::Pos1)
	{
		...
		int nStepB = XTPToIntChecked(int(pText1 - pText) * sizeof(TCHAR));
...
void CXTPSyntaxEditTextIterator::LCPosAdd(XTP_EDIT_LINECOL& rLC, int nCharsAdd) const
{
	...
	while (nRestChars)
	{
		if (m_mapLine2Len.Lookup(XTPToDWORDChecked(rLC.nLine), nLineLen))
		{
			if (nLineLen == 0)
			{
