...
//===========================================================================
// Summary:
//     Defines light and dark colors which are used by
...
#	define XTP_EDIT_DISABLED_COLOR_DARK                                                           \
...
#	define XTP_EDIT_NM_EDITCHANGING                                                               \
		(XTP_EDIT_NM_FIRST + 0) // Edit changing will be called before changing content
#	define XTP_EDIT_NM_EDITCHANGED                                                                \
		(XTP_EDIT_NM_FIRST + 1) // This will be called if content has been changed
#	define XTP_EDIT_NM_SETDOCMODIFIED                                                             \
		(XTP_EDIT_NM_FIRST + 2) // Indicates if the document is modified
#	define XTP_EDIT_NM_DRAWBOOKMARK                                                               \
		(XTP_EDIT_NM_FIRST + 3) // This will be called while drawing the bookmarks
#	define XTP_EDIT_NM_ROWCOLCHANGED                                                              \
		(XTP_EDIT_NM_FIRST + 4) // This will be fired if row or col has been changed
#	define XTP_EDIT_NM_INSERTKEY (XTP_EDIT_NM_FIRST + 8) // Update all of the vertical panes
#	define XTP_EDIT_NM_SELINIT                                                                    \
		(XTP_EDIT_NM_FIRST + 9) // Updates all the selections while one pane initiates one
#	define XTP_EDIT_NM_STARTOLEDRAG (XTP_EDIT_NM_FIRST + 10) // Start OLE drag
#	define XTP_EDIT_NM_MARGINCLICKED                                                              \
		(XTP_EDIT_NM_FIRST + 11) // Left mouse button was clicked in the gutter area
#	define XTP_EDIT_NM_UPDATESCROLLPOS                                                            \
		(XTP_EDIT_NM_FIRST + 12) // Edit control scroll positions changed.
#	define XTP_EDIT_NM_PARSEEVENT                                                                 \
		(XTP_EDIT_NM_FIRST + 13) // This notification used to reflect parser events. See
								 // XTPSyntaxEditOnParseEvent enum.

#	define XTP_EDIT_NM_LBUTTONDOWN                                                                \
		(XTP_EDIT_NM_FIRST                                                                         \
		 + 14) // Notify mouse Left button down. NMMOUSE structure passed with this notification.
#	define XTP_EDIT_NM_LBUTTONDBLCLICK                                                            \
		(XTP_EDIT_NM_FIRST + 15) // Notify mouse Left button double click. NMMOUSE structure passed
								 // with this notification.
#	define XTP_EDIT_NM_LBUTTONUP                                                                  \
		(XTP_EDIT_NM_FIRST                                                                         \
		 + 16) // Notify mouse Left button up. NMMOUSE structure passed with this notification.
#	define XTP_EDIT_NM_RBUTTONDOWN                                                                \
		(XTP_EDIT_NM_FIRST                                                                         \
		 + 17) // Notify mouse Right button down. NMMOUSE structure passed with this notification.
#	define XTP_EDIT_NM_RBUTTONUP                                                                  \
		(XTP_EDIT_NM_FIRST                                                                         \
		 + 18) // Notify mouse Right button up. NMMOUSE structure passed with this notification.
#	define XTP_EDIT_NM_MOUSEMOVE                                                                  \
		(XTP_EDIT_NM_FIRST                                                                         \
		 + 19) // Notify mouse movement. NMMOUSE structure passed with this notification.

#	define XTP_EDIT_NM_ENABLESCROLLBAR                                                            \
		(XTP_EDIT_NM_FIRST + 20) // Edit control scroll bar(s) state changed.
...
#	define XTP_EDIT_AVELINELEN                                                                    \
		30 // Average length of line. Used as default value for
		   // CXTPSyntaxEditCtrl::m_nAverageLineLen.
...
const TCHAR XTP_EDIT_REG_BREAKPOINTTEXTCOLOR[] = _T("ColorBreakpointText");
const TCHAR XTP_EDIT_REG_BREAKPOINTBACKCOLOR[] = _T("ColorBreakpointBack");
//}}AFX_CODEJOCK_PRIVATE
...
#	define XTP_EDIT_RGB_GETRED(rgb)                                                               \
		(((rgb) >> 16) & 0xff) // Get red color value for raw RGB color like "0xAABBCC".
#	define XTP_EDIT_RGB_GETGREEN(rgb)                                                             \
		(((rgb) >> 8) & 0xff) // Get green color value for raw RGB color like "0xAABBCC".
#	define XTP_EDIT_RGB_GETBLUE(rgb)                                                              \
		((rgb)&0xff) // Get blue color value for raw RGB color like "0xAABBCC".
#	define XTP_EDIT_RGB_INT2CLR(rgb)                                                              \
		RGB(XTP_EDIT_RGB_GETRED(rgb), XTP_EDIT_RGB_GETGREEN(rgb),                                  \
			XTP_EDIT_RGB_GETBLUE(rgb)) // Convert raw RGB color value like "0xAABBCC" to COLORREF.
...
//===========================================================================
// Summary:
//     These events are sent from different threads. Its receiver must be thread-safe.
//===========================================================================
enum XTPSyntaxEditOnParseEvent
{
	xtpEditOnParserStarted   = 1, // dwParam1 - unused
	xtpEditOnTextBlockParsed = 2, // dwParam1 = Text Block ID
	xtpEditOnParserEnded	 = 3  // dwParam1 = status flags : xtpEditLPR_RunFinished,
								  // xtpEditLPR_RunBreaked, xtpEditLPR_Error;
};
...
{
	xtpEditClassSchWasChanged = 10, // dwParam1 = LPCTSTR - Schema name;  dwParam2 =
									// CXTPSyntaxEditLexTextSchema* New text schema pointer, or NULL
									// if it was removed.
	xtpEditThemeWasChanged = 11,	// dwParam1 = LPCTSTR - Theme name;   dwParam2 =
								 // CXTPSyntaxEditColorTheme* New theme pointer, or NULL if it was
								 // removed.
...
//---------------------------------------------------------------------------
// Summary:
//      View update actions. Defines the way in which the view should be updated.
//---------------------------------------------------------------------------
