...
CXTPSyntaxEditDrawTextProcessor::CXTPSyntaxEditDrawTextProcessor()
{
	...
	m_nSpaceWidth = XTP_DPI_X(6); // 6 px

	m_rcTextRect.SetRect(0, 0, 0, 0);
	m_nRowHeight	 = XTP_DPI_Y(16);
	m_nScrollXOffset = 0;
...
CXTPSyntaxEditDrawTextProcessor::CXTPRowInfo*
	CXTPSyntaxEditDrawTextProcessor::GetRowInfo(int nRow) const
{
	if (nRow < 0)
		return NULL;
...
int CXTPSyntaxEditDrawTextProcessor::DispPosToStrPos(int nRow, int nDispPos,
...
int CXTPSyntaxEditDrawTextProcessor::StrPosToDispPos(int nRow, int nStrPos,
													 BOOL bVirtualSpace) const
{
	CXTPRowInfo* pRowI = GetRowInfo(nRow);
	ASSERT(pRowI);
...
int CXTPSyntaxEditDrawTextProcessor::ExpandChars(LPCTSTR pszChars, CString& strBuffer, int nDispPos,
												 BOOL bEnableWhiteSpace)
{
	...
	for (LPCTSTR p = pszChars; p && *p != 0; p = _tcsinc(p))
	{
		...
		else
		{
			if (bEnableWhiteSpace && *p == _T(' '))
				m_arExpandCharsBuffer.AddData(
					(TCHAR)(unsigned char)0xB7); // strBuffer += (TCHAR)(unsigned char)0xB7;
			else
			{
...
int CXTPSyntaxEditDrawTextProcessor::DrawRowPart(CDC* pDC, int nRow, LPCTSTR pcszText, int nchCount)
{
	...
	else
	{
		...
		nTextLen = nchCount;
#else
		nTextLen = XTPToIntChecked(_tcsnbcnt(pcszText, XTPToSizeTChecked(nchCount)));
#endif
	}
	...
	pDC->ExtTextOut(nX, nY, ETO_CLIPPED, &rcText, pcszText, XTPToUIntChecked(nTextLen), NULL);

	//--------------------------------------------------------
	if (m_arBuf_aDx.GetSize() < nTextLen)
		m_arBuf_aDx.SetSize(nTextLen + 100);

	int nMaxExtent = m_rcTextRect.Width() + m_nScrollXOffset - m_nNextRowPosX
					 + XTP_DPI_X(30); // 30 is a gap to be sure
	nMaxExtent = max(nMaxExtent, XTP_DPI_X(30));
...
void CXTPSyntaxEditDrawTextProcessor::DrawTextOfSize(CDC* pDC, int nX, int nY, const SIZE& textSize,
													 const RECT& rcText, LPCTSTR pcszText,
													 int nchCount) const
{
	const int MaxTTGridSize = 16384;
	if (textSize.cx < MaxTTGridSize)
	{
		pDC->ExtTextOut(nX, nY, ETO_CLIPPED, &rcText, pcszText, XTPToUIntChecked(nchCount), NULL);
	}
	else
	{
		...
		DrawTextOfSize(pDC, nX + leftPartSize.cx, nY, rightPartSize, rcText,
					   pcszText + nLeftPartLength, nRightPartLength);
	}
}

int CXTPSyntaxEditDrawTextProcessor::PrintRowPart(CDC* pDC, int nRow, int nPosY, UINT nFlags,
												  LPCTSTR pcszText, int nchCount,
												  int* pnPrintedTextLen)
{
	if (pnPrintedTextLen)
		*pnPrintedTextLen = 0;
	...
	for (pTxt = pcszText; nTextLen_rest;)
	{
		...
		//--------------------------------------------------------
		int nMaxExtent = max(0, rcText.right - nX - XTP_DPI_X(20));
		int nFit	   = 0;
		...
		{
			rcText.left = max(rcText.left, nX); // ENSURE THE TEXT IS CLIPPED APPROPRIATELY
			pDC->ExtTextOut(nX, nY, ETO_CLIPPED, &rcText, pTextToDtaw, XTPToUIntChecked(nFit),
							NULL);
		}
		...
		{
			int nIconX = m_rcTextRect.left + m_ptNextPrintPos.x + XTP_DPI_X(3);
			int nIconY = m_rcTextRect.top + nPosY + m_ptNextPrintPos.y;

			int nDelta = GetRowHeight() - GetRowHeight() / 4;

			CPen pen1(PS_SOLID, XTP_DPI_X(1), RGB(0, 0, 0));
			CPen pen2(PS_SOLID, XTP_DPI_X(2), RGB(0, 0, 0));

			CPen* pPen_old = pDC->SelectObject(&pen1);

			pDC->MoveTo(nIconX, nIconY + nDelta / 2);
			pDC->LineTo(nIconX, nIconY + nDelta);
			pDC->LineTo(nIconX + nDelta / 2 - XTP_DPI_X(1), nIconY + nDelta);

			pDC->SelectObject(&pen2);
			pDC->MoveTo(nIconX + XTP_DPI_X(1), nIconY + nDelta - XTP_DPI_Y(1));
	...
	if (pnPrintedTextLen)
	{
		...
		*pnPrintedTextLen = XTPToIntChecked(_tcsnccnt(pcszText, XTPToSizeTChecked(nLen0)));
	}

	return m_ptNextPrintPos.y + GetRowHeight();
}

int CXTPSyntaxEditDrawTextProcessor::GetColPosX(int nRow, int nCol, int* pnCharWidth,
												BOOL bVirtualSpace) const
{
	CXTPRowInfo* pRI = GetRowInfo(nRow);
	if (!pRI)
	...
	else if (nCol < nColsCount)
	{
		...
		if (pnCharWidth)
			*pnCharWidth = pRI->arCharsEnds[nCol] - pRI->arCharsEnds[nCol - 1];
	}
	else
	{
		if (nColsCount)
...
BOOL CXTPSyntaxEditDrawTextProcessor::ColFromXPos(int nRow, int nX, int& rnCol,
												  BOOL bVirtualSpace) const
{
	rnCol = 0;
...
BOOL CXTPSyntaxEditDrawTextProcessor::HitTest(const CPoint& pt, int& rnRow, int& rnCol,
											  BOOL bVirtualSpace) const
{
...
CPoint CXTPSyntaxEditDrawTextProcessor::SetCaretByPoint(CWnd* pWnd, const CPoint& pt,
														const CSize& szSize, int& rnRow, int& rnCol,
														BOOL bVirtualSpace)
{
...
CPoint CXTPSyntaxEditDrawTextProcessor::SetCaretPos(CWnd* pWnd, const CSize& szSize, int nRow,
													int& rnCol, BOOL bHideCaret, BOOL bVirtualSpace)
{
