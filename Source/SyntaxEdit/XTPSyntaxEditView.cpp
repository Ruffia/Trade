...
CXTPSyntaxEditView::CXTPSyntaxEditView()
	...
	, m_szPage(XTP_DPI_SIZE(CSize(850, 1100)))
	, m_iTopRow(1)
	, m_pParentWnd(NULL)
{
	...
	else
	{
		...
		if (GetEditCtrl().GetRowCount() > 0)
		{
			...
			if (bShiftKey)
			{
				...
				nStartCol = GetEditCtrl().GetEditBuffer()->GetLineTextLengthC(
					GetEditCtrl().GetRowCount() - 1);
			}

			if (GetEditCtrl().Find(GetFindReplaceDlg()->m_csFindText,
								   GetFindReplaceDlg()->m_bMatchWholeWord,
								   GetFindReplaceDlg()->m_bMatchCase, !bShiftKey, TRUE, nStartRow,
								   nStartCol))
			{
				SetDirty();
			}
			...
			GetParentFrame()->SetMessageText(
				XTPResourceManager()->LoadString(XTP_IDS_EDIT_MSG_FSEARCH));
		}
	}
}
...
void CXTPSyntaxEditView::OnSize(UINT nType, int cx, int cy)
{
	...
	if (m_bResizeControlWithView && ::IsWindow(GetEditCtrl().m_hWnd))
	{
		...
		// for case when RecalcScrollBars() call change a view size (by disabling/enabling
		// scrollbars)
...
{
	if (GetEditCtrl().SendMessage(WM_KEYDOWN, nChar, XTPToLPARAM(nFlags)) != 0)
	{
...
{
	if (GetEditCtrl().SendMessage(WM_CHAR, (WPARAM)nChar, (LPARAM)nFlags) != 0)
	{
...
{
	if (GetEditCtrl().SendMessage(WM_VSCROLL, MAKEWPARAM(nSBCode, nPos),
...
{
	if (GetEditCtrl().SendMessage(WM_HSCROLL, MAKEWPARAM(nSBCode, nPos),
								  (LPARAM)pScrollBar->GetSafeHwnd())
		!= 0)
	{
...
BOOL CXTPSyntaxEditView::OnNotify(WPARAM wParam, LPARAM lParam, LRESULT* pResult)
{
	...
	if (pNMHDR && pNMHDR->hwndFrom == GetEditCtrl().m_hWnd)
	{
		switch (pNMHDR->code)
		{
			case XTP_EDIT_NM_EDITCHANGING: return OnEditChanging(pNMHDR, pResult);

			case XTP_EDIT_NM_EDITCHANGED: return OnEditChanged(pNMHDR, pResult);

			case XTP_EDIT_NM_SETDOCMODIFIED: return OnSetDocModified(pNMHDR, pResult);

			case XTP_EDIT_NM_DRAWBOOKMARK: return OnDrawBookmark(pNMHDR, pResult);

			case XTP_EDIT_NM_ROWCOLCHANGED: return OnRowColChanged(pNMHDR, pResult);

			case XTP_EDIT_NM_UPDATESCROLLPOS: return OnUpdateScrollPos(pNMHDR, pResult);

			case XTP_EDIT_NM_ENABLESCROLLBAR: return OnEnableScrollBar(pNMHDR, pResult);

			case XTP_EDIT_NM_INSERTKEY: return OnInsertKey(pNMHDR, pResult);

			case XTP_EDIT_NM_SELINIT: return OnSelInit(pNMHDR, pResult);

			case XTP_EDIT_NM_STARTOLEDRAG: return OnStartOleDrag(pNMHDR, pResult);

			case XTP_EDIT_NM_MARGINCLICKED: return OnMarginClicked(pNMHDR, pResult);

			case XTP_EDIT_NM_PARSEEVENT: return OnParseEvent(pNMHDR, pResult);
		}
	}
...
void CXTPSyntaxEditView::OnPrepareDC(CDC* pDC, CPrintInfo* pInfo)
{
	...
	if (pInfo->m_nCurPage == 1 && nVisibleRowsCount == 0)
	{
		pInfo->m_bContinuePrinting = TRUE;
			 ...
			 && m_aPageStart[XTPToIntPtrChecked(pInfo->m_nCurPage - 1)]
					>= XTPToUIntChecked(nVisibleRowsCount))
	{
		// can't paginate to that page, thus cannot print it.
		pInfo->m_bContinuePrinting = FALSE;
	}
	else if (pInfo->m_nCurPage > (UINT)m_aPageStart.GetSize() && !PaginateTo(pDC, pInfo))
	{
		// can't paginate to that page, thus cannot print it.
		pInfo->m_bContinuePrinting = FALSE;
	}
...
void CXTPSyntaxEditView::OnBeginPrinting(CDC* pDC, CPrintInfo* pInfo)
{
	if (GetEditCtrl().GetSyntaxColor())
	{
		...
		CXTPSyntaxEditLexTextSchema* ptrTextSch =
			GetEditCtrl().GetEditBuffer()->GetLexParser()->GetTextSchema();
		if (ptrTextSch)
		{
	...
	if (str1.Find(_T("&P")) >= 0 || str2.Find(_T("&P")) >= 0)
	{
		...
		{
			pInfo->SetMaxPage(XTPToUIntChecked(m_aPageStart.GetSize() - 1));
		}
...
CString CXTPSyntaxEditView::GetPageTitle()
{
	...
	// CString strFileName =
	// strPathName.Right(strPathName.GetLength()-(strPathName.ReverseFind(_T('\\')) + 1));
	if (strTitle.IsEmpty())
	{
...
void CXTPSyntaxEditView::OnPrint(CDC* pDC, CPrintInfo* pInfo)
{
	...
	if (!m_pPrintOptions->GetPageHeader()->IsEmpty())
	{
		...
		rcPageHeader.top	= rcPageHeader.bottom + XTP_DPI_Y(3);
		rcPageHeader.bottom = rcPageHeader.top + XTP_DPI_Y(2);
		pDC->FillSolidRect(rcPageHeader, RGB(0, 0, 0));

		rcPrint.top = rcPageHeader.bottom + XTP_DPI_Y(7);
	}
	if (!m_pPrintOptions->GetPageFooter()->IsEmpty())
	{
		m_pPrintOptions->GetPageFooter()->Draw(pDC, rcPageFooter, TRUE);
		rcPrint.bottom = rcPageFooter.top - XTP_DPI_Y(7);
	}
	...
	GetEditCtrl().SetTopRow(XTPToIntChecked(nIndex));
	...
	if (!m_pPrintOptions->GetPageFooter()->IsEmpty())
	{
		...
		rcPageFooter.top	= rcPageFooter.top - XTP_DPI_Y(3);
		rcPageFooter.bottom = rcPageFooter.top + XTP_DPI_Y(1);
...
void CXTPSyntaxEditView::UpdateScrollPos(CView* pSender, DWORD dwUpdate /* = XTP_EDIT_UPDATE_ALL*/)
{
	...
	CXTPSyntaxEditView* pActiveView =
		(CXTPSyntaxEditView*)(pSender ? pSender : GetParentFrame()->GetActiveView());
	...
	if (pSplitterWnd && pActiveView && GetEditCtrl().IsCreateScrollbarOnParent())
	{
		...
		int nSenderRow = min(nSplRowsCount - 1,
							 (pActiveView->GetDlgCtrlID() - AFX_IDW_PANE_FIRST) / 16);
		int nSenderCol = min(nSplColsCount - 1,
							 (pActiveView->GetDlgCtrlID() - AFX_IDW_PANE_FIRST) % 16);
		...
		// TRACE(_T(" splitter.senderViev=%x-(Row, Col)(%d, %d).thisView[%d, %d] "), pActiveView,
		// nSenderRow, nSenderCol, nRow, nCol);
...
void CXTPSyntaxEditView::UpdateSiblingScrollPos(CXTPSyntaxEditView* pSender, DWORD dwUpdate)
{
	...
	switch (dwUpdate & XTP_EDIT_UPDATE_ALL)
	{
		...
		case XTP_EDIT_UPDATE_DIAG: { GetEditCtrl().Invalidate(FALSE);
		}
		break;
	}
}
...
void CXTPSyntaxEditView::UpdateSiblings(XTP_EDIT_NMHDR_EDITCHANGED* pNMHDR_EC, BOOL bTextChanged)
{
	...
	while (pos)
	{
		CXTPSyntaxEditView* pView = DYNAMIC_DOWNCAST(CXTPSyntaxEditView,
...
AFX_STATIC BOOL AFX_CDECL EnsureViewIsActive(CView* pView)
{
	...
	CFrameWnd* pParentFrame = DYNAMIC_DOWNCAST(CMDIChildWnd, pView->GetParentFrame());

	if (pParentFrame)
	{
		// If the top level frame is MDI activate.
		CMDIFrameWnd* pMDIFrameWnd = DYNAMIC_DOWNCAST(CMDIFrameWnd, pView->GetTopLevelFrame());
		...
		CSplitterWnd* pSplitterWnd = DYNAMIC_DOWNCAST(CSplitterWnd, pView->GetParent());
...
DROPEFFECT CXTPSyntaxEditView::OnDragEnter(COleDataObject* pDataObject, DWORD dwKeyState,
										   CPoint point)
{
#ifdef _UNICODE
...
DROPEFFECT CXTPSyntaxEditView::OnDragOver(COleDataObject* pDataObject, DWORD dwKeyState,
										  CPoint point)
{
...
BOOL CXTPSyntaxEditView::SetScrollBars(BOOL bHorz, BOOL bVert, BOOL bUpdateReg /*=FALSE*/,
									   BOOL bRecalcLayout /*=TRUE*/)
{
	...
	CSplitterWnd* pSplitterWnd = GetEditCtrl().IsCreateScrollbarOnParent()
...
void CXTPSyntaxEditView::OnUpdate(CView* pSender, LPARAM lHint, CObject* pHint)
{
	...
	switch (lHint)
	{
		...
		case xtpEditHintInitView:
		{
			...
			// Update scrollbars.
			SetScrollBars(GetEditCtrl().GetHorzScrollBar(), GetEditCtrl().GetVertScrollBar(), FALSE,
						  TRUE);
		...
		case xtpEditHintRefreshView: { Refresh();
		}
		break;
	}
}
