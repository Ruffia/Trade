...
{
	return (IsWindowEnabled() ? m_crText.operator DWORD() : ::GetSysColor(COLOR_GRAYTEXT));
}
...
{
	return (IsWindowEnabled() ? m_crBack.operator DWORD() : ::GetSysColor(COLOR_3DFACE));
}
...
{
	return (IsWindowEnabled() ? m_crBorder.operator DWORD() : ::GetSysColor(COLOR_GRAYTEXT));
}
