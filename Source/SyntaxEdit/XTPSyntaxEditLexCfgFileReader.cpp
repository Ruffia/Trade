...
CXTPSyntaxEditLexPropInfoArray::CXTPSyntaxEditLexPropInfoArray(
	const CXTPSyntaxEditLexPropInfoArray& rSrc)
{
	RemoveAll();
...
const CXTPSyntaxEditLexPropInfoArray& CXTPSyntaxEditLexPropInfoArray::
	operator=(const CXTPSyntaxEditLexPropInfoArray& rSrc)
{
	RemoveAll();
...
CXTPSyntaxEditLexClassInfoArray::CXTPSyntaxEditLexClassInfoArray(
	const CXTPSyntaxEditLexClassInfoArray& rSrc)
{
	RemoveAll();
...
const CXTPSyntaxEditLexClassInfoArray& CXTPSyntaxEditLexClassInfoArray::
	operator=(const CXTPSyntaxEditLexClassInfoArray& rSrc)
{
	RemoveAll();
	m_bModified = rSrc.m_bModified;
...
AFX_STATIC BOOL AFX_CDECL AfxCompareTime(const FILETIME& ft1, const FILETIME& ft2,
										 DWORD dwTimeDiff = 500)
{
	...
	ULONGLONG ullDiff = uFT1.QuadPart > uFT2.QuadPart ? uFT1.QuadPart - uFT2.QuadPart
													  : uFT2.QuadPart - uFT1.QuadPart;
...
void CXTPSyntaxEditLexCfgFileReader::CopyInfoMap(CMapStringToPtr& mapInfo)
{
	...
	for (POSITION pos = m_mapLexClassInfo.GetStartPosition(); pos;)
	{
		...
		CXTPSyntaxEditLexClassInfoArray* pDest = new CXTPSyntaxEditLexClassInfoArray(*pSource);
...
void CXTPSyntaxEditLexCfgFileReader::ReadSource(const CString& csFileName, BOOL bSaveInfo)
{
	try
	{
		...
		if (AfxOpenFile(file, csFileName, CFile::modeRead))
		{
			...
			char* pszBuffer = new char[XTPToUIntChecked(iLen + 2)];
			int iBytes		= (int)file.Read(pszBuffer, XTPToUIntChecked(iLen));
			ASSERT(iLen == iBytes);
			UNUSED(iBytes);
			...
			if (bSaveInfo)
			{
				...
				if (!m_mapLexClassInfo.Lookup(csKey, (void*&)pInfo))
					m_mapLexClassInfo[csKey] = new CXTPSyntaxEditLexClassInfoArray(
						m_arLexClassInfo);
				else
...
void CXTPSyntaxEditLexCfgFileReader::ParseLexClass(XTP_EDIT_LEXCLASSINFO& infoClass)
{
	...
	while (m_strToken.CompareNoCase(XTP_EDIT_LEXCLASS_STARTTOKEN) != 0 && m_nEOFPos > m_nCurrPos)
	{
		...
		while (m_strToken != _T('=') && m_nEOFPos > m_nCurrPos
...
int CXTPSyntaxEditLexCfgFileReader::GetLexToken()
{
	...
	while ((nTokenType == xtpEditTokType_Unknown || nTokenType == xtpEditTokType_Delim
...
int CXTPSyntaxEditLexCfgFileReader::GetToken()
{
	...
	switch (m_nTokenType)
	{
		case xtpEditTokType_Unknown:
			...
			break;
		case xtpEditTokType_Delim: nFoundedToken = ProcessDelimToken(tchCurrSymbol); break;
...
int CXTPSyntaxEditLexCfgFileReader::ProcessUnknowToken(TCHAR tchCurrSymbol)
{
	...
	if (_istalpha(XTP_CHARTOCRT(tchCurrSymbol)))
...
BOOL CXTPSyntaxEditLexCfgFileReader::WriteCfgFile(const CString& csFileName,
												  CXTPSyntaxEditLexClassInfoArray& arLexClassDesc)
{
	...
	try
	{
		...
		CFile file;
		if (!AfxOpenFile(file, csTmpFileName,
		...
		for (i = 0; i < nCount; i++)
		{
			...
			XTP_EDIT_LEXCLASSINFO* pNewClassInfo = FindClassDesc(arLexClassDesc,
																 oldClassInfo.csClassName);
			// class was deleted;
			if (!pNewClassInfo)
			{
			...
			for (k = 0; k < nPropsCount; k++)
			{
				...
				XTP_EDIT_LEXPROPINFO* pNewInfoProp = FindPropDesc(pNewClassInfo, &oldInfoProp,
																  mapSavedProps);
				if (pNewInfoProp)
				{
...
XTP_EDIT_LEXCLASSINFO* CXTPSyntaxEditLexCfgFileReader::FindClassDesc(
...
XTP_EDIT_LEXPROPINFO* CXTPSyntaxEditLexCfgFileReader::FindPropDesc(
	XTP_EDIT_LEXCLASSINFO* pInfoClass, XTP_EDIT_LEXPROPINFO* pInfoProp, CMapPtrToBool& mapUsed)
{
...
void CXTPSyntaxEditLexCfgFileReader::WriteString(CFile& file, LPCTSTR pcszString)
{
	...
	int nNewLen = WideCharToMultiByte(CP_ACP, 0, pcszString, -1, pTextMBCS, nStrLen * 2 + 1, NULL,
									  NULL);
	...
	file.Write(pTextMBCS, XTPToUIntChecked(nStrLen));
	file.Write("\r\n", 2);
}

void CXTPSyntaxEditLexCfgFileReader::WriteStrings(CFile& file, CStringArray& arBuffer, int nFrom,
												  int nTo)
{
...
void CXTPSyntaxEditLexCfgFileReader::WriteProp(CFile& file, CString& csOffset,
											   const XTP_EDIT_LEXPROPINFO& newInfoProp)
{
...
void CXTPSyntaxEditLexCfgFileReader::WriteProp(CFile& file, CString& csOffset,
											   const XTP_EDIT_LEXPROPINFO& oldInfoProp,
											   const XTP_EDIT_LEXPROPINFO& newInfoProp,
...
#	endif
		...
		case CFileException::badSeek:
			...
			break;
		case CFileException::hardIO: strErrorMsg = _T("There was a hardware error."); break;
		...
		case CFileException::lockViolation:
			...
			break;
		case CFileException::diskFull: strErrorMsg = _T("The disk is full."); break;
		case CFileException::endOfFile: strErrorMsg = _T("The end of file was reached."); break;
		default: strErrorMsg = _T("Unknown error.");
	}
...
BOOL CXTPSyntaxEditFileChangesMonitor::AddMonitorFile(CString& strFilename, DWORD dwOwnerFlags)
{
	// try open file
...
void CXTPSyntaxEditFileChangesMonitor::StartMonitoring()
{
	...
	m_pThread = AfxBeginThread(ThreadMonitorProc, (LPVOID)this, THREAD_PRIORITY_LOWEST);
...
void CXTPSyntaxEditFileChangesMonitor::StopMonitoring()
{
	if (m_pThread)
	{
		...
		if (dwThreadRes == WAIT_TIMEOUT)
		{
			...
			// ASSERT(FALSE);
			TRACE(_T("ERROR! FileChangesMonitor thread was not ended by normal way. It was ")
				  _T("terminated. \n"));
		}
	...
	for (int i = 0; i < nCount; i++)
	{
		...
		if (GetFileInfo(rFMFileInfo.m_strFileName, &sysFileInfo))
		{
				...
				|| !rFMFileInfo.m_bExists)
			{
				...
				// reload this file
				m_pConfigMgr->ReloadFile(rFMFileInfo.m_strFileName, rFMFileInfo.m_dwOwnerFlags,
										 nCfgFlags);
			}
		}
		else
												   ...
												   BY_HANDLE_FILE_INFORMATION* pInfo)
{
	...
	HANDLE hFile = ::CreateFile(pcszFilePath, GENERIC_READ, dwShareMode, NULL, OPEN_EXISTING,
...
const CXTPSyntaxEditFileChangesMonitor::CFMFileInfo& CXTPSyntaxEditFileChangesMonitor::CFMFileInfo::
	operator=(const CXTPSyntaxEditFileChangesMonitor::CFMFileInfo& rSrc)
{
