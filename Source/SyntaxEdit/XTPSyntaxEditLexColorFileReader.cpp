...
const CString CXTPSyntaxEditColorInfo::GetParam(const CString& strName, BOOL bDynamic)
{
	CString strLowerName(strName);
	strLowerName.MakeLower();
	...
	if (!m_mapParams.Lookup(strLowerName, strValue) && bDynamic)
	{
		...
		{
			CXTPSyntaxEditColorInfo* pParentColorInfo =
				pParentTheme->GetColorInfo(m_strClassName, m_pTheme->GetFileName());
			if (pParentColorInfo)
											...
											CXTPSyntaxEditColorInfoArray* pColorInfoArray_new)
{
	...
	BOOL bRes = ::WritePrivateProfileString(XTP_EDIT_LEXPARSER_SECTION_MAIN,
											XTP_EDIT_LEXPARSER_PARENT_SCHEMA, pcszString,
											strFileName_new);

	CString strSavedKey = XTP_EDIT_LEXPARSER_SECTION_MAIN + cstrCNDelim
						  + XTP_EDIT_LEXPARSER_PARENT_SCHEMA;
	...
	for (int i = 0; i < nCount; i++)
	{
		...
		while (posParam)
		{
			...
			{
				bRes = ::WritePrivateProfileString(csSection, strPrmName, strPrmValue,
												   strFileName_new);
				if (!bRes)
				{
					return FALSE;
	...
	while (posClrInfo_old)
	{
		...
		while (posParam)
		{
			...
			if (!mapSaved_ClsProp.Lookup(strSavedKey, pVoid))
			{
				VERIFY(::WritePrivateProfileString(csSection, strPrmName, NULL, strFileName_new));
			}
		}
	}
	...
	{
		TRACE(_T("ERROR! CXTPSyntaxEditColorTheme::WriteCfgFile() - Cannot rename file '%s' to ")
			  _T("'%s' \n"),
...
CXTPSyntaxEditColorInfo* CXTPSyntaxEditColorTheme::ParseSectionFromString(
	const CString& csSection, CStringArray& arPassedString)
{
...
CXTPSyntaxEditColorInfo* CXTPSyntaxEditColorTheme::GetColorInfo(const CString& strLexClass,
...
void CXTPSyntaxEditColorThemesManager::LoadThemeFromString(const CString& strThemeName,
...
void CXTPSyntaxEditTextSchemesManager::LoadTextStringScheme(XTP_EDIT_SCHEMAFILEINFO& info,
															CString* p_sSyntaxScheme)
{
	...
	CXTPSyntaxEditLexClassInfoArray& arLexClassInfo =
		XTPSyntaxEditLexConfig()->GetLexClassInfoArray();
...
CXTPSyntaxEditTextSchemaPtr
	CXTPSyntaxEditTextSchemesManager::FindSchema(const CString& strFileExt) const
{
	...
	if (m_bUseMonitor)
	{
		...
		if (m_hReloadThread)
		{
			...
			if (dwThreadRes == WAIT_TIMEOUT)
			{
				...
				TRACE(_T("ERROR! Configuration Manager reload thread was not ended by normal way. ")
					  _T("It was terminated. \n"));
			}
		}
		m_hReloadThread = NULL;
...
void CXTPSyntaxEditConfigurationManager::ReloadConfig()
{
	...
	if (!b)
	{
		m_pConnectMT->SendEvent(xtpEditAllConfigWasChanged, 0,
								0); //<<Aug 22>> prev scheme clean call
	...
	if (!m_bConfigFileMode)
	{
		...
		CXTPSyntaxEditSectionManager().GetSectionKeyList(listThemes, arPassedString,
														 XTP_EDIT_LEXPARSER_SECTION_THEMES);
	}
	else
	{
		CXTPSyntaxEditSectionManager().GetSectionKeyList(listThemes, m_strMainIniFilename,
														 XTP_EDIT_LEXPARSER_SECTION_THEMES);
	}
	...
	if (!m_bConfigFileMode)
	{
		...
		CXTPSyntaxEditSectionManager().GetSectionKeyList(listSchemes, arStrings,
														 XTP_EDIT_LEXPARSER_SECTION_SCHEMES);
	}
	else
	{
		CXTPSyntaxEditSectionManager().GetSectionKeyList(listSchemes, m_strMainIniFilename,
														 XTP_EDIT_LEXPARSER_SECTION_SCHEMES);
	}
...
void CXTPSyntaxEditConfigurationManager::ReloadFile(CString csFileName, DWORD dwOwnerFlags,
													int nCfgFlags)
{
	CSingleLock singleLock(&m_DataLockerCS, TRUE);
	...
	if ((dwOwnerFlags & xtpEditCfgObjThemeMan) || dwOwnerFlags == 0)
	{
		...
		if (!strItemName.IsEmpty())
		{
			...
			m_pConnectMT->SendEvent(xtpEditThemeWasChanged, (WPARAM)(LPCTSTR)strItemName,
									(LPARAM)pTheme);
			return;
		}
	}
