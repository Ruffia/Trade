...
struct CXTPGraphicBitmapPng::CCallback
{
	...
	static BOOL PNGAPI PngLoadImage(CFile* pFile, png_byte** ppbImageData, png_uint_32* piWidth,
...
BOOL CXTPGraphicBitmapPng::CCallback::PngLoadImage(CFile* pFile, png_byte** ppbImageData,
												   png_uint_32* piWidth, png_uint_32* piHeight,
												   int* piChannels, png_color* pBkgColor)
{
	...
	png_ptr = png_create_read_struct(PNG_LIBPNG_VER_STRING, NULL, (png_error_ptr)png_cexcept_error,
									 (png_error_ptr)NULL);
	if (!png_ptr)
	{
		*ppbImageData = pbImageData = NULL;
	...
	TRY
	{
		...
		png_get_IHDR(png_ptr, info_ptr, piWidth, piHeight, &iBitDepth, &iColorType, NULL, NULL,
					 NULL);
		...
		*piChannels = XTPToIntChecked(ulChannels);
...
HBITMAP CXTPGraphicBitmapPng::ConvertToBitmap(BYTE* pbImage, CSize szImage, int cImgChannels) const
{
	...
	wImgRowBytes = XTPToUIntChecked(cImgChannels * szImage.cx);
	wDIRowBytes  = XTPToUIntChecked(cDIChannels * szImage.cx);
...
BOOL CXTPGraphicBitmapPng::LoadFromFile(CFile* pFile)
{
	...
	HBITMAP hBitmap = ConvertToBitmap(pbImage,
									  CSize(XTPToIntChecked(cxImgSize), XTPToIntChecked(cyImgSize)),
									  cImgChannels);
