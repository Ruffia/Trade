...
const UINT CXTPSkinObject::m_nMsgUpdateSkinState = RegisterWindowMessage(
	_T("WM_SKINFRAMEWORK_UPDATESTATE"));
const UINT CXTPSkinObject::m_nMsgQuerySkinState = RegisterWindowMessage(
	_T("WM_SKINFRAMEWORK_QUERYSTATE"));
const UINT CXTPSkinObject::m_nMsgSkinChanged = RegisterWindowMessage(
	_T("WM_SKINFRAMEWORK_SKINCHANGED"));
...
void CXTPSkinObject::UnattachHook(BOOL bAuto)
{
	...
	{
		if (bAuto
			|| !::SendMessage(m_hWnd, CXTPHookManager::m_nMsgSinkRemoved,
							  (WPARAM)(LONG_PTR)CXTPSkinManager::HookWndProc,
							  (LPARAM)(LONG_PTR)m_pOldWndProc))
		{
			if (m_bUnicode)
				SetWindowLongPtrW(m_hWnd, GWLP_WNDPROC, reinterpret_cast<LONG_PTR>(m_pOldWndProc));
			else
				SetWindowLongPtrA(m_hWnd, GWLP_WNDPROC, reinterpret_cast<LONG_PTR>(m_pOldWndProc));
		}
...
LRESULT CXTPSkinObject::DefWindowProc(UINT nMsg, WPARAM wParam, LPARAM lParam)
{
	...
	if (des.defProc == xtpSkinDefaultCallWindowProc)
	{
		...
		return (XTPToFunctionPtr<LPFNCALLWINDOWPROC>(des.defProcAddr))((WNDPROC)des.lpPrev, m_hWnd,
																	   nMsg, wParam, lParam);
	}

	if (des.defProc == xtpSkinDefaultDefWindowProc || des.defProc == xtpSkinDefaultDefMDIChildProc
		|| des.defProc == xtpSkinDefaultDefDlgProc)
	{
		ASSERT(NULL != des.defProcAddr);

		typedef LRESULT(WINAPI * LPFNDEFWINDOWPROC)(HWND hWnd, UINT Msg, WPARAM wParam,
													LPARAM lParam);
		return (XTPToFunctionPtr<LPFNDEFWINDOWPROC>(des.defProcAddr))(m_hWnd, nMsg, wParam, lParam);
	}
	...
	if (des.defProc == xtpSkinDefaultDefFrameProc)
	{
		...
		typedef LRESULT(WINAPI * LPFNDEFWINDOWPROC)(HWND hWnd, HWND hWndMDIClient, UINT Msg,
													WPARAM wParam, LPARAM lParam);
		return (XTPToFunctionPtr<LPFNDEFWINDOWPROC>(des.defProcAddr))(m_hWnd, (HWND)des.lpPrev,
																	  nMsg, wParam, lParam);
	}
...
void CXTPSkinObject::SetWindowProc()
{
	...
	if (m_bUnicode)
	{
		SetWindowLongPtrW(m_hWnd, GWLP_WNDPROC,
						  reinterpret_cast<LONG_PTR>(CXTPSkinManager::HookWndProc));
	}
	else
	{
...
void CXTPSkinObject::AttachHook(HWND hWnd, LPCREATESTRUCT lpcs, BOOL bAuto)
{
	...
	if (CXTPSkinManagerApiHookBase::IsSystemWindowModule(m_pOldWndProc))
	{
		m_bSystemWindowModule = TRUE;
	}

	if (m_pManager->IsWin9x() && ((DWORD_PTR)m_pOldWndProc > (DWORD_PTR)0x70000000)
		&& (m_pOldWndProc
			!= reinterpret_cast<WNDPROC>(CXTPSkinManagerApiHookBase::GetAfxWndProc())))
	{
		m_bSystemWindowModule = TRUE;
	}
...
COLORREF CXTPSkinObject::GetColor(int nIndex) const
{
	...
	return pMetrics ? pMetrics->GetColor(nIndex) : GetXtremeColor(XTPToUIntChecked(nIndex));
}
...
{
	m_bCustomDraw = CWnd::FromHandlePermanent(m_hWnd) ? SendMessage(m_nMsgQuerySkinState) != 0
													  : FALSE;
...
{
	if (nMessage == WM_SETCURSOR || nMessage == WM_NOTIFY || nMessage == WM_COMMAND
...
BOOL CXTPSkinObject::OnHookDefWindowProc(UINT nMessage, WPARAM& wParam, LPARAM& lParam,
										 LRESULT& lResult)
{
...
BOOL CXTPSkinObject::OnHookMessage(UINT nMessage, WPARAM& wParam, LPARAM& lParam, LRESULT& lResult)
{
	...
	if (IsDefWindowProcAvail(XTPToInt(nMessage)))
		return FALSE;
...
void CXTPSkinObject::OnBeginHook(UINT nMessage, XTPSkinDefaultProc defProc, PROC defProcAddr,
...
CXTPSkinObject* CXTPSkinObjectClassInfo::CreateObject(LPCTSTR /*lpszClassName*/,
													  LPCREATESTRUCT /*lpcs*/)
{
	if (!m_pClass)
		return NULL;
...
#define RUNTIME_CLASS_INFOACTIVEX(x)                                                               \
	new CXTPSkinObjectClassInfoActiveX(RUNTIME_CLASS(x), FALSE, FALSE)
#define RUNTIME_CLASS_INFOSYSACTIVEX(x)                                                            \
	new CXTPSkinObjectClassInfoActiveX(RUNTIME_CLASS(x), TRUE, FALSE)
#define RUNTIME_CLASS_INFOSYSACTIVEX2(x)                                                           \
	new CXTPSkinObjectClassInfoActiveX(RUNTIME_CLASS(x), FALSE, TRUE)
...
void CXTPSkinObjectClassMap::AddStandardClasses()
{
	...
	AddClass(_T("ThunderRT6MDIForm"),
			 RUNTIME_CLASS_INFOSYSACTIVEX2(CXTPSkinObjectApplicationFrame));
...
CXTPSkinObjectClassInfo* CXTPSkinObjectClassMap::AddClass(LPCTSTR lpszClassName,
