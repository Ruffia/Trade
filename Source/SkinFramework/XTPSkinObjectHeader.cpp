...
void CXTPSkinObjectHeader::OnDraw(CDC* pDC)
{
	...
	BOOL bFilterBar = (GetStyle() & HDS_FILTERBAR)
					  && XTPSystemVersion()->GetComCtlVersion() >= MAKELONG(80, 5);
...
void CXTPSkinObjectHeader::DrawItemEntry(CDC* pDC, int nIndex, CRect rcItem, int nState)
{
	...
	CImageList* pImageList   = CImageList::FromHandle(
		  (HIMAGELIST)::SendMessage(pHeaderCtrl->m_hWnd, HDM_GETIMAGELIST, 0, 0L));
	...
	if (!bResult)
	{
		...
		::SendMessage(pHeaderCtrl->m_hWnd, HDM_GETITEMA, XTPToWPARAM(nIndex), (LPARAM)&hdia);
	...
	if (hdi.fmt & HDF_OWNERDRAW)
	{
		...
		dis.CtlID	  = XTPToUInt(GetDlgCtrlID());
		dis.itemID	 = XTPToUInt(nIndex);
		dis.itemAction = ODA_DRAWENTIRE;
		dis.itemState  = UINT((nState == HIS_PRESSED) ? ODS_SELECTED : 0);
		dis.hwndItem   = m_hWnd;
		dis.hDC		   = pDC->GetSafeHdc();
		dis.rcItem	 = rcItem;
		dis.itemData   = static_cast<ULONG_PTR>(hdi.lParam);

		// Now send it off to my parent...
		if (GetParent()->SendMessage(WM_DRAWITEM, dis.CtlID, (LPARAM)(DRAWITEMSTRUCT*)&dis))
		{
			return;
		}
	}
	...
	if (pImageList && (hdi.fmt & HDF_IMAGE) && hdi.iImage >= 0
		&& hdi.iImage < pImageList->GetImageCount())
	{
	...
	if (((hdi.fmt & HDF_IMAGE) == 0) && ((hdi.fmt & HDF_SORTUP) || (hdi.fmt & HDF_SORTDOWN)))
	{
		...
		{
			XTPPrimitiveDrawer()->DrawSymbol(pDC, xtpPrimitiveSymbolPlayUp, rcGlyph,
