...
#	include "Common/Tmschema.h"
	...
	TRACE(_T("Warning: %s class not supported\n"), lpszClass);
...
int CXTPSkinManagerSchema::DrawThemeFrameButtons(CDC* pDC, CXTPSkinObjectFrame* pFrame)
{
	...
	BOOL bModalFrame	= (lStyle & DS_MODALFRAME) == DS_MODALFRAME;
	BOOL bDlgModalFrame = (dwExStyle & WS_EX_DLGMODALFRAME) == WS_EX_DLGMODALFRAME;
	if (lStyle & WS_SIZEBOX || (bModalFrame && bDlgModalFrame))
...
#define GETPARTSTATE(ht, pressed, hot, normal, disabled, active)                                   \
	(!bEnabled ? disabled                                                                          \
			   : nPressedHt == ht ? pressed : nHotHt == ht ? hot : bActive ? (active) : normal)
