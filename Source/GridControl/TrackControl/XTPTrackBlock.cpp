...
void CXTPTrackBlock::SetPosition(int nPosition)
{
	...
	{
		m_pItem->GetTrackControl()->GetUndoManager()->AddUndoCommand(
			new CXTPTrackUndoSetBlockPositionCommand(this, m_nMRUPosition, m_nLength));
	}

	m_nPosition = m_nMRUPosition = nPosition;

	if (m_pItem)
		m_pItem->RedrawControl();
}
...
int CXTPTrackBlock::Draw(CDC* pDC, CRect rc, BOOL bSelected)
{
	...
	{
		return m_pItem->GetTrackControl()->GetPaintManager()->DrawTrackBlock(pDC, rc, bSelected,
																			 this);
	}
...
int CXTPTrackKey::Draw(CDC* pDC, CRect rc, BOOL bSelected)
{
	...
	COLORREF clrDark = CXTPDrawHelpers::HSLtoRGB(
		RGB(GetRValue(dwHSLBackground), GetGValue(dwHSLBackground), dwDark));
	COLORREF clrDarkDark = RGB(GetRValue(clrDark) / 2, GetGValue(clrDark) / 2,
							   GetBValue(clrDark) / 2);

	CXTPPenDC pen(*pDC, IsLocked() ? RGB(128, 128, 128) : bSelected ? clrLight : clrDarkDark);
	CXTPBrushDC brush(*pDC, m_clrBlock);

	CPoint pts[] = { CPoint(X - XTP_DPI_X(4), Y), CPoint(X, Y - XTP_DPI_Y(4)),
					 CPoint(X + XTP_DPI_X(4), Y), CPoint(X + XTP_DPI_X(4), Y + XTP_DPI_Y(4)),
					 CPoint(X - XTP_DPI_X(4), Y + XTP_DPI_Y(4)) };
	pDC->Polygon(pts, 5);
...
void CXTPTrackMarkers::RemoveAt(int nIndex)
{
	...
	{
		m_pControl->GetUndoManager()->AddUndoCommand(
			new CXTPTrackUndoDeleteMarkerCommand(m_arrMarkers[nIndex]));
...
void CXTPTrackMarkers::RemoveAll()
{
	...
	{
		m_pControl->GetUndoManager()->AddUndoCommand(
			new CXTPTrackUndoDeleteMarkerCommand(m_arrMarkers[i]));
		m_arrMarkers[i]->InternalRelease();
	}
...
BEGIN_DISPATCH_MAP(CXTPTrackBlock, CXTPCmdTarget)
	...
	DISP_PROPERTY_EX_ID(CXTPTrackBlock, "DescriptionText", 1005, OleGetDescriptionText,
						OleSetDescriptionText, VT_BSTR)
	DISP_PROPERTY_EX_ID(CXTPTrackBlock, "HeightPercent", 1006, OleGetHeightPercent,
						OleSetHeightPercent, VT_R8)
	DISP_PROPERTY_EX_ID(CXTPTrackBlock, "VerticalAlignment", 1007, OleGetVerticalAlignment,
						OleSetVerticalAlignment, VT_I4)
	...
	DISP_PROPERTY_EX_ID(CXTPTrackBlock, "HeightFixed", 1015, OleGetHeightFixed, OleSetHeightFixed,
						VT_I4)
...
BEGIN_DISPATCH_MAP(CXTPTrackSelectedBlocks, CXTPCmdTarget)
	...
	DISP_FUNCTION_ID(CXTPTrackSelectedBlocks, "Block", DISPID_VALUE, OleGetItem, VT_DISPATCH,
					 VTS_I4)
	DISP_FUNCTION_ID(CXTPTrackSelectedBlocks, "_NewEnum", DISPID_NEWENUM, OleNewEnum, VT_UNKNOWN,
					 VTS_NONE)
