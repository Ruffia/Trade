	...
	else
	{
		...
		if (pImages->IsValid())
		{
			...
			m_clrGradientColumnShadow.SetStandardValue(
				pImages->GetImageColor(_T("GridControl"), _T("ColumnShadow")));
			m_clrGradientColumnSeparator.SetStandardValue(
				pImages->GetImageColor(_T("GridControl"), _T("ColumnSeparator")));
			m_clrGroupBoxBack.SetStandardValue(
				pImages->GetImageColor(_T("GridControl"), _T("GroupBoxBackground")));
			m_crlNoGroupByText.SetStandardValue(
				pImages->GetImageColor(_T("GridControl"), _T("GroupBoxTextColor")));
			m_clrGroupShadeBack.SetStandardValue(
				pImages->GetImageColor(_T("GridControl"), _T("GroupShadeBackground")));
			m_clrGroupShadeBorder.SetStandardValue(
				pImages->GetImageColor(_T("GridControl"), _T("GroupShadeBorder")));
			m_clrIndentControl.SetStandardValue(
				pImages->GetImageColor(_T("GridControl"), _T("GroupShadeIndent")));
...
CSize CXTPTrackThemeResource::DrawCollapsedBitmap(CDC* pDC, const CXTPGridRow* pRow,
												  CRect& rcBitmap)
{
...
void CXTPTrackThemeResource::FillFooter(CDC* pDC, CRect rcFooter)
{
	...
	DrawHorizontalLine(pDC, rcFooter.left, rcFooter.top + 1, rcFooter.Width(),
					   m_clrGradientColumnShadow);
	DrawHorizontalLine(pDC, rcFooter.left, rcFooter.top + 2, rcFooter.Width(),
					   MixColor(m_grcGradientColumn.clrDark, m_clrGradientColumnShadow, 0.25));
	DrawHorizontalLine(pDC, rcFooter.left, rcFooter.top + 3, rcFooter.Width(),
					   MixColor(m_grcGradientColumn.clrDark, m_clrGradientColumnShadow, 0.1));

	DrawHorizontalLine(pDC, rcFooter.left, rcFooter.bottom - 1, rcFooter.Width(),
					   m_clrGradientColumnShadow);
}

void CXTPTrackThemeResource::DrawTriangle(CDC* pDC, CRect rcTriangle, BOOL bToDown,
										  int nShadowWidth, BOOL /*bColumnPressed*/)
{
	...
	if (rcTriangle.Width() > 10)
	{
		...
		if (bToDown)
		{
			CXTPDrawHelpers::Triangle(pDC, CPoint(pt.x - 4, pt.y - 2), CPoint(pt.x, pt.y + 2),
									  CPoint(pt.x + 4, pt.y - 2), m_clrGradientColumnSeparator);
		}
		else
		{
			CXTPDrawHelpers::Triangle(pDC, CPoint(pt.x - 4, pt.y + 2), CPoint(pt.x, pt.y - 2),
...
void CXTPTrackThemeResource::DrawColumnBackground(CDC* pDC, CXTPGridColumn* pColumn, CRect rcColumn,
												  BOOL& bColumnPressed,
												  ...
												  CXTPGridHeader* /*pHeader*/)
{
	...
	DrawHorizontalLine(pDC, rcColumn.left, rcColumn.bottom - 1, rcColumn.Width(),
					   m_clrGradientColumnShadow);

	if (pColumn->GetDrawHeaderDivider())
		DrawVerticalLine(pDC, rcColumn.right - 1, rcColumn.top + 2, rcColumn.Height() - 2 - 4,
...
void CXTPTrackThemeResource::DrawColumnFooterDivider(CDC* pDC, CXTPGridColumn* pColumn,
													 CRect rcColumn)
{
	if (pColumn->GetDrawFooterDivider())
	{
		DrawVerticalLine(pDC, rcColumn.right, rcColumn.top + 6, rcColumn.Height() - 9,
						 m_clrControlLightLight);
		DrawVerticalLine(pDC, rcColumn.right - 1, rcColumn.top + 5, rcColumn.Height() - 9,
						 m_clrGradientColumnSeparator);
	}
}

void CXTPTrackThemeResource::DrawColumnText(CDC* pDC, CRect rcText, CString strCaption,
...
void CXTPTrackThemeResource::DrawSectionDivider(CDC* pDC, const CRect& rc,
