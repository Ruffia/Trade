...
void CXTPFlowGraphDrawContextGdiPlus::Register(BOOL bReserved)
{
	UNREFERENCED_PARAMETER(bReserved);
}

CXTPFlowGraphDrawContextGdiPlus::CXTPFlowGraphDrawContextGdiPlus(HDC hDC, HDC hAttribDC,
																 BOOL bPrinting)
...
#	pragma pop_macro("new")
	...
	m_rcClipRect = CRect(rcScreenBounds.GetLeft(), rcScreenBounds.GetTop(),
						 rcScreenBounds.GetRight(), rcScreenBounds.GetBottom());
...
void CXTPFlowGraphDrawContextGdiPlus::Draw3dRect(LPCRECT lpRect, COLORREF clrTopLeft,
...
void CXTPFlowGraphDrawContextGdiPlus::Draw3dRect(int x, int y, int cx, int cy, COLORREF clrTopLeft,
												 COLORREF clrBottomRight)
{
...
void CXTPFlowGraphDrawContextGdiPlus::DrawMarkupElement(CXTPMarkupUIElement* pMarkupUIElement,
														LPCRECT lpRect, UINT /*uFormat*/)
{
	//  pMarkupUIElement->GetMarkupContext()->SetDefaultFont(&m_lfActiveFont);
...
CSize CXTPFlowGraphDrawContextGdiPlus::MeasureString(const CString& str)
{
	...
	if (str.GetLength() != 0)
	{
		m_pGraphics->MeasureString(XTP_CT2CW(str), -1, m_pActiveFont, layoutRect,
								   StringFormat::GenericTypographic(), &boundingBox);
	}
	else
	{
		m_pGraphics->MeasureString(L" ", 1, m_pActiveFont, layoutRect,
								   StringFormat::GenericTypographic(), &boundingBox);
	}
...
void CXTPFlowGraphDrawContextGdiPlus::DrawText(LPCTSTR lpszText, LPCRECT lpRect, UINT uFormat)
{
	...
	stringFormat.SetLineAlignment(uFormat & DT_VCENTER ? StringAlignmentCenter
													   : StringAlignmentNear);
	stringFormat.SetAlignment(uFormat & DT_CENTER
								  ? StringAlignmentCenter
								  : uFormat & DT_RIGHT ? StringAlignmentFar : StringAlignmentNear);

	RectF rect(static_cast<REAL>(lpRect->left), static_cast<REAL>(lpRect->top),
			   static_cast<REAL>(lpRect->right) - static_cast<REAL>(lpRect->left),
			   static_cast<REAL>(lpRect->bottom) - static_cast<REAL>(lpRect->top));

	m_pGraphics->DrawString(XTP_CT2CW(lpszText), -1, m_pActiveFont, rect, &stringFormat,
							m_pActiveTextBrush);
}
...
void CXTPFlowGraphDrawContextGdiPlus::Ellipse(LPCRECT lpRect)
{
	if (m_pActiveBrush)
	{
		m_pGraphics->FillEllipse(m_pActiveBrush, static_cast<INT>(lpRect->left),
								 static_cast<INT>(lpRect->top),
								 static_cast<INT>(lpRect->right - lpRect->left),
								 static_cast<INT>(lpRect->bottom - lpRect->top));
	}

	if (m_pActivePen)
	{
		m_pGraphics->DrawEllipse(m_pActivePen, static_cast<INT>(lpRect->left),
								 static_cast<INT>(lpRect->top),
								 static_cast<INT>(lpRect->right - lpRect->left),
								 static_cast<INT>(lpRect->bottom - lpRect->top));
	}
...
void CXTPFlowGraphDrawContextGdiPlus::DrawGrid(CRect rc, CPoint ptScrollOffset, double dZoomLevel,
...
void CXTPFlowGraphDrawContextGdiPlus::GradientFill(CRect rc, COLORREF clrFrom, COLORREF clrTo,
												   BOOL bHoriz)
{
	LinearGradientBrush brush(Rect(rc.left, rc.top, rc.Width(), rc.Height()),
							  ColorRefToARGB(clrFrom), ColorRefToARGB(clrTo),
							  bHoriz ? LinearGradientModeHorizontal : LinearGradientModeVertical);
