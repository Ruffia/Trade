...
CXTPFlowGraphConnectionPoint* CXTPFlowGraphConnectionPoints::AddConnectionPoint(
	CXTPFlowGraphConnectionPoint* pConnectionPoint, int nIndex)
{
	...
	if ((type == xtpFlowGraphPointInput || type == xtpFlowGraphPointOutput))
	{
		CXTPFlowGraphConnectionPoint* pSameNamePoint =
			m_pNode->GetConnectionPoints()->FindConnectionPoint(pConnectionPoint->GetName(),
																(type == xtpFlowGraphPointInput)
	...
	if (m_pNode->IsCustomMarkup())
		bAddUndoTask = FALSE; // not undo/redo operations for connection points in custom markup
							  // nodes

	if (bAddUndoTask)
		m_pNode->GetControl()->GetUndoManager()->AddUndoCommand(
			new CXTPFlowGraphUndoAddConnectionPointCommand(pConnectionPoint));
...
void CXTPFlowGraphConnectionPoints::RemoveAll()
{
	...
	for (int i = (int)m_arrConnectionPoints.GetSize() - 1; i >= 0; i--)
	{
		...
		if ((i - 1) >= 0) // there's a point before
		{
			...
			{
				if ((pNextPoint->GetType() == xtpFlowGraphPointInput
					 && pConnectionPoint->GetType() == xtpFlowGraphPointOutput)
					|| (pNextPoint->GetType() == xtpFlowGraphPointOutput
		...
		{
			if (m_pNode->IsCustomMarkup())
				bSkipUndo = TRUE; // not undo/redo operations for connection points in custom markup
								  // nodes

			pUndoManager->AddUndoCommand(
				new CXTPFlowGraphUndoDeleteConnectionPointCommand(pConnectionPoint));
		}
										   ...
										   BOOL bRemovePair)
{
	...
	if (pConnectionPoint->m_nConnectionPointIndex != -1)
	{
		// ASSERT(m_arrConnectionPoints[pConnectionPoint->m_nConnectionPointIndex] ==
		// pConnectionPoint);
...
void CXTPFlowGraphConnectionPoints::RemoveAt(int nIndex, BOOL bRemovePair /*= TRUE*/,
											 BOOL bSkipUndoTask /*= FALSE*/)
{
	...
	if (m_pNode->IsCustomMarkup())
		bSkipUndoTask = TRUE; // not undo/redo operations for connection points in custom markup
							  // nodes
	...
	{
		pSameNamePoint = FindConnectionPoint(pConnectionPoint->GetName(),
											 (type == xtpFlowGraphPointInput)
												 ? xtpFlowGraphPointOutput
												 : xtpFlowGraphPointInput);
...
void CXTPFlowGraphConnectionPoints::DoPropExchange(CXTPPropExchange* pPX)
{
	...
	else
	{
		...
		while (pos)
		{
			...
			if (!sec->ExchangeObjectInstance((CObject*&)pConnectionPoint,
											 RUNTIME_CLASS(CXTPFlowGraphConnectionPoint)))
				AfxThrowArchiveException(CArchiveException::badClass);

			pConnectionPoint->m_pNode				  = m_pNode;
			pConnectionPoint->m_nConnectionPointIndex = (int)m_arrConnectionPoints.Add(
				pConnectionPoint);
...
CXTPFlowGraphConnectionPoint* CXTPFlowGraphConnectionPoints::FindConnectionPoint(
...
CXTPFlowGraphConnectionPoint* CXTPFlowGraphConnectionPoints::FindConnectionPointByCaption(
...
BEGIN_DISPATCH_MAP(CXTPFlowGraphConnectionPoints, CXTPCmdTarget)
	...
	DISP_FUNCTION_ID(CXTPFlowGraphConnectionPoints, "ConnectionPoint", DISPID_VALUE, OleGetItem,
					 VT_DISPATCH, VTS_I4)
	DISP_FUNCTION_ID(CXTPFlowGraphConnectionPoints, "_NewEnum", DISPID_NEWENUM, OleNewEnum,
					 VT_UNKNOWN, VTS_NONE)
	...
	DISP_FUNCTION_ID(CXTPFlowGraphConnectionPoints, "FindConnectionPoint", 5,
					 OleFindConnectionPoint, VT_DISPATCH, VTS_I4)
	DISP_FUNCTION_ID(CXTPFlowGraphConnectionPoints, "FindConnectionPointByCaption", 6,
					 OleFindConnectionPointByCaption, VT_DISPATCH, VTS_BSTR VTS_VARIANT)
	DISP_FUNCTION_ID(CXTPFlowGraphConnectionPoints, "FindConnectionPointByName", 7,
					 OleFindConnectionPointByName, VT_DISPATCH, VTS_BSTR VTS_VARIANT)
...
LPDISPATCH CXTPFlowGraphConnectionPoints::OleFindConnectionPointByCaption(LPCTSTR lpszCaption,
																		  const VARIANT& vtType)
{
...
LPDISPATCH CXTPFlowGraphConnectionPoints::OleFindConnectionPointByName(LPCTSTR lpszName,
																	   const VARIANT& vtType)
{
	XTPFlowGraphConnectionPointType nType = xtpFlowGraphPointNone;
