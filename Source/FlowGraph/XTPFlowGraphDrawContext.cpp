...
void CXTPFlowGraphDrawContext::SetWorldTransform(CPoint ptOffset, double dZoomLevel)
{
	...
	SetViewportOrgEx(m_hDC, -ptOffset.x * m_sizeVpExt.cx / m_sizeWinExt.cx,
					 -ptOffset.y * m_sizeVpExt.cy / m_sizeWinExt.cy, NULL);
...
void CXTPFlowGraphDrawContext::Draw3dRect(LPCRECT lpRect, COLORREF clrTopLeft,
...
void CXTPFlowGraphDrawContext::Draw3dRect(int x, int y, int cx, int cy, COLORREF clrTopLeft,
										  COLORREF clrBottomRight)
{
...
void CXTPFlowGraphDrawContext::DrawMarkupElement(CXTPMarkupUIElement* pMarkupUIElement,
...
void CXTPFlowGraphDrawContext::DrawCurve(Gdiplus::GraphicsPath* pPath)
{
	...
	Point* lppt = new Point[XTPToUIntChecked(nCount)];
	pPath->GetPathPoints(lppt, nCount);

	BYTE* pTypes = new BYTE[XTPToUIntChecked(nCount)];
	...
	for (nIndex = 0; nIndex < nCount; nIndex++)
	{
		switch (pTypes[nIndex])
		{
			...
			case PathPointTypeLine: pDC->LineTo(lppt[nIndex].X, lppt[nIndex].Y); break;
...
void CXTPFlowGraphDrawContext::DrawGrid(CRect rc, CPoint ptScrollOffset, double dZoomLevel,
