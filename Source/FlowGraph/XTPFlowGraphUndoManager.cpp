...
void CXTPFlowGraphUndoDeleteConnectionPointCommand::Undo()
{
	if (m_pNode->GetPage() != NULL)
	{
		...
		// new code:
		m_pNode->AddNamedConnectionPoints(m_pConnectionPoint->GetName(), type,
										  m_pConnectionPoint->GetCaption());

		// hack:
		// but before that, in order to correctly complete
		// CXTPFlowGraphUndoDeleteConnectionCommand::Undo we need to set m_pConnectionPoint->m_pNode
		...
		// this line is required because connection point pointer will change (after
		// AddNamedConnectionPoints) but for later undo/redo operations the index will stay the same
	...
	: CXTPFlowGraphUndoCommand(XTP_ID_FLOWGRAPHUNDO_DELETECONNECTION)
{
	...
	if (m_pInputPoint)
		m_pInputPoint->InternalAddRef();

	m_pOutputPoint = m_pConnection->GetOutputPoint();
	if (m_pOutputPoint)
		m_pOutputPoint->InternalAddRef();
}
...
CXTPFlowGraphUndoDeleteConnectionCommand::~CXTPFlowGraphUndoDeleteConnectionCommand()
{
	...
	if (m_pInputPoint)
		m_pInputPoint->InternalRelease();
	if (m_pOutputPoint)
		m_pOutputPoint->InternalRelease();
}

void CXTPFlowGraphUndoDeleteConnectionCommand::Undo()
{
	if (!(m_pInputPoint && m_pInputPoint->GetNode()
		  && m_pInputPoint->GetNode()->GetPage() == m_pPage))
		return;

	if (!(m_pOutputPoint && m_pOutputPoint->GetNode()
		  && m_pOutputPoint->GetNode()->GetPage() == m_pPage))
		return;
	...
	CXTPFlowGraphConnectionPoint* pInputNew =
		m_pInputPoint->GetNode()->GetConnectionPoints()->FindConnectionPoint(
			m_pInputPoint->GetName(), m_pInputPoint->GetType());

	CXTPFlowGraphConnectionPoint* pOutputNew =
		m_pOutputPoint->GetNode()->GetConnectionPoints()->FindConnectionPoint(
			m_pOutputPoint->GetName(), m_pOutputPoint->GetType());
...
CXTPFlowGraphUndoSetNodePositionCommand::CXTPFlowGraphUndoSetNodePositionCommand(
	CXTPFlowGraphNode* pNode, CPoint ptOldPosition)
...
CXTPFlowGraphUndoSetNodeSizeCommand::CXTPFlowGraphUndoSetNodeSizeCommand(CXTPFlowGraphNode* pNode,
																		 CSize szOldSize)
...
CXTPFlowGraphUndoSetNodeCaptionCommand::CXTPFlowGraphUndoSetNodeCaptionCommand(
	CXTPFlowGraphElement* pNode, CString strOldCaption)
...
CXTPFlowGraphUndoSetConnectionPositionCommand::CXTPFlowGraphUndoSetConnectionPositionCommand(
	CXTPFlowGraphConnection* pConnection, CPoint ptOldPosition)
...
BEGIN_DISPATCH_MAP(CXTPFlowGraphUndoManager, CXTPCmdTarget)
	...
	DISP_PROPERTY_EX_ID(CXTPFlowGraphUndoManager, "CanUndo", 1002, CanUndo, SetNotSupported,
						VT_BOOL)
	DISP_PROPERTY_EX_ID(CXTPFlowGraphUndoManager, "CanRedo", 1003, CanRedo, SetNotSupported,
						VT_BOOL)
