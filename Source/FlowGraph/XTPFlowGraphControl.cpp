...
CMap<XTPFlowGraphCustomProperty, XTPFlowGraphCustomProperty, CXTPMarkupDependencyProperty*,
	 CXTPMarkupDependencyProperty*&>
	CXTPFlowGraphControl::m_mapCustomProperties;
...
void CXTPFlowGraphControl::RegisterCustomMarkupProperties()
{
	if (!m_mapCustomProperties.GetCount())
	{
		m_mapCustomProperties[xtpFlowGraphPropertyConnectionPoint] =
			CXTPMarkupDependencyProperty::RegisterAttached(XAML_MARKUP_PROPERTY_CPNAME,
														   MARKUP_TYPE(CXTPMarkupString),
														   MARKUP_TYPE(CXTPMarkupRectangle));
		m_mapCustomProperties[xtpFlowGraphPropertyConnectionPointType] =
			CXTPMarkupDependencyProperty::RegisterAttached(XAML_MARKUP_PROPERTY_CPTYPE,
														   MARKUP_TYPE(CXTPMarkupInt),
														   MARKUP_TYPE(CXTPMarkupRectangle));

		// may be not only border actually?
		m_mapCustomProperties[xtpFlowGraphPropertyCanSetColor] =
			CXTPMarkupDependencyProperty::RegisterAttached(XAML_MARKUP_PROPERTY_CANSETCOLOR,
														   MARKUP_TYPE(CXTPMarkupBool),
														   MARKUP_TYPE(CXTPMarkupBorder));

		m_mapCustomProperties[xtpFlowGraphPropertyConnectionPointIn] =
			CXTPMarkupDependencyProperty::RegisterAttached(XAML_MARKUP_PROPERTY_CPOINTS_IN,
														   MARKUP_TYPE(CXTPMarkupString),
														   MARKUP_TYPE(CXTPMarkupCanvas));

		m_mapCustomProperties[xtpFlowGraphPropertyResizable] =
			CXTPMarkupDependencyProperty::RegisterAttached(XAML_MARKUP_PROPERTY_RESIZABLE,
														   MARKUP_TYPE(CXTPMarkupBool),
														   MARKUP_TYPE(CXTPMarkupCanvas));
	}
}
...
{
	m_mapDefaultXamlTemplates[xtpFlowGraphTemplateNode] = LoadXamlTemplate(
		XAML_MARKUP_RESOURCENAME_NODE);
	m_mapDefaultXamlTemplates[xtpFlowGraphTemplateNodePointNone] = LoadXamlTemplate(
		XAML_MARKUP_RESOURCENAME_NODE_ITEM);
	m_mapDefaultXamlTemplates[xtpFlowGraphTemplateNodePointIn] = LoadXamlTemplate(
		XAML_MARKUP_RESOURCENAME_NODE_ITEM_IN);
	m_mapDefaultXamlTemplates[xtpFlowGraphTemplateNodePointOut] = LoadXamlTemplate(
		XAML_MARKUP_RESOURCENAME_NODE_ITEM_OUT);
	m_mapDefaultXamlTemplates[xtpFlowGraphTemplateNodePointInOut] = LoadXamlTemplate(
		XAML_MARKUP_RESOURCENAME_NODE_ITEM_INOUT);
}
...
CXTPMarkupDependencyProperty*
	CXTPFlowGraphControl::GetCustomMarkupProperty(XTPFlowGraphCustomProperty property) const
{
	CXTPMarkupDependencyProperty* pProperty = NULL;
	m_mapCustomProperties.Lookup(property, pProperty);
...
CXTPFlowGraphDrawContext* CXTPFlowGraphControl::CreateDrawContext(CDC& dc)
{
	...
	if ((m_nSmoothingMode == xtpFlowGraphSmoothingModeGDI)
		|| (m_nSmoothingMode == xtpFlowGraphSmoothingModeAuto
			&& GetZoomLevel() < GetPaintManager()->m_dMinZoomLevelGDIQuality))
	{
	...
	else
	{
		...
		if ((m_nSmoothingMode == xtpFlowGraphSmoothingModeAuto && m_bInAction)
			|| (m_nSmoothingMode == xtpFlowGraphSmoothingModeHighSpeed))
		{
...
CPoint CXTPFlowGraphControl::TransformPoint(CPoint pt) const
{
	...
	if (hDC && m_pActivePage)
	{
		...
		::SetViewportExtEx(hDC, int(m_pActivePage->m_dZoomLevel * 96),
						   int(m_pActivePage->m_dZoomLevel * 96), 0);
		...
		::SetViewportOrgEx(hDC,
						   -m_pActivePage->m_ptScrollOffset.x * m_sizeVpExt.cx / m_sizeWinExt.cx,
						   -m_pActivePage->m_ptScrollOffset.y * m_sizeVpExt.cy / m_sizeWinExt.cy,
						   NULL);
...
{
	CXTPFlowGraphEditItem* pWnd = DYNAMIC_DOWNCAST(CXTPFlowGraphEditItem,
												   CWnd::FromHandlePermanent(::GetFocus()));
	if (pWnd)
	{
		::SetFocus(m_hWnd);
	}
...
void CXTPFlowGraphControl::AdjustScrollBars()
{
	...
	// Vertical
	{
		int nPageSize = static_cast<int>(m_pActivePage->m_rcPage.Height()
										 / m_pActivePage->m_dZoomLevel);
		if (nPageSize < 0)
			nPageSize = 0;
		...
		else
		{
			...
			si.nPage = XTPToUIntChecked(nPageSize);

			si.nMin = __min(nPos, nWorkTop);
			si.nMax = __max(nPos + nPageSize, nWorkBottom);
	...
	// Horizontal
	{
		int nPageSize = static_cast<int>(m_pActivePage->m_rcPage.Width()
										 / m_pActivePage->m_dZoomLevel);
		if (nPageSize < 0)
			nPageSize = 0;
...
void CXTPFlowGraphControl::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	...
	{
		case SB_TOP:
			nCurPos = min(m_pActivePage->m_rcWorkRect.left,
						  min(nCurPos, m_pActivePage->m_rcWorkRect.left));
			break;

		case SB_BOTTOM: nCurPos = GetScrollLimit(SB_HORZ); break;

		case SB_LINEUP: nCurPos = nCurPos - m_pActivePage->m_rcPage.Width() / 10; break;

		case SB_LINEDOWN: nCurPos = nCurPos + m_pActivePage->m_rcPage.Width() / 10; break;

		case SB_PAGEUP:
			nCurPos = max(nCurPos - m_pActivePage->m_rcPage.Width(),
						  m_pActivePage->m_rcWorkRect.left);
			break;
...
void CXTPFlowGraphControl::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	...
	{
		case SB_TOP:
			nCurPos = min(m_pActivePage->m_rcWorkRect.top,
						  min(nCurPos, m_pActivePage->m_rcWorkRect.top));
			break;

		case SB_BOTTOM: nCurPos = GetScrollLimit(SB_VERT); break;

		case SB_LINEUP: nCurPos = nCurPos - m_pActivePage->m_rcPage.Height() / 10; break;

		case SB_LINEDOWN: nCurPos = nCurPos + m_pActivePage->m_rcPage.Height() / 10; break;

		case SB_PAGEUP:
			nCurPos = max(nCurPos - m_pActivePage->m_rcPage.Height(),
						  m_pActivePage->m_rcWorkRect.top);
			break;
...
bool CXTPFlowGraphControl::ResetSelectedNodes(CXTPFlowGraphNode* pNodeToSkip /*= NULL*/)
{
	...
	for (CXTPFlowGraphNode* pNode = m_pActivePage->m_pFirstVisibleNode; pNode != NULL;
		 pNode					  = pNode->m_pNextVisibleNode)
	{
...
void CXTPFlowGraphControl::UpdateActiveNodeAtPoint(CPoint pt,
												   CXTPFlowGraphNode* pNodeToSkip /*= NULL*/)
{
...
BOOL CXTPFlowGraphControl::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message)
{
	if (nHitTest == HTCLIENT)
	{
		...
		if (pNode)
		{
			HCURSOR hCursor = AfxGetApp()->LoadStandardCursor(
				MAKEINTRESOURCE(CXTPFlowGraphSelectionRect::GetCursorBySelectionHitTest(hitTest)));
...
CXTPFlowGraphNode* CXTPFlowGraphControl::IsPointInSizeRectOfSelectedNode(CPoint point,
																		 UINT& uHitTest) const
{
	...
	if (m_pActivePage)
	{
		...
		if (pSelection->GetCount() == 1) // 1 node selected
		{
			...
			{
				double zl = GetActivePage()->GetZoomLevel();
				CXTPFlowGraphSelectionRect::SetZoomLevel(zl);

				SelectionRectHitTest hitTest =
					CXTPFlowGraphSelectionRect::HitTest(pNode->GetSelectionRect(), point);
...
void CXTPFlowGraphControl::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	if (pNode)
	{
		if (!pNode->IsSelected())
		{
			...
			if (pSelection->GetCount() > 1)
			{
				...
				{
					CXTPFlowGraphConnection* pConnection = m_pActivePage->GetConnections()->GetAt(
						j);
					if (pConnection->GetInputPoint()
						&& pConnection->GetInputPoint()->GetNode()->IsSelected()
						&& pConnection->GetOutputPoint()
						&& pConnection->GetOutputPoint()->GetNode()->IsSelected()
						&& (pConnection->GetInputPoint()->GetNode() == pNode
							|| pConnection->GetOutputPoint()->GetNode() == pNode))
					{
	...
	if (pConnection)
	{
		...
		XTPFlowGraphConnectorType type = pConnection->GetStyle() != -1
											 ? (XTPFlowGraphConnectorType)pConnection->GetStyle()
											 : GetPaintManager()->m_nConnectorType;
...
void CXTPFlowGraphControl::StartDragConnectionPoint(CXTPFlowGraphConnectionPoint* pPoint)
{
	...
	if (pOutputPoint->GetMaxConnections() >= 0
		&& pOutputPoint->GetConnectionsCount() >= pOutputPoint->GetMaxConnections())
	{
		m_pUndoManager->EndGroup();
		return;
	}
	...
	if (pConnectionPoint
		&& (pConnectionPoint->GetType() == xtpFlowGraphPointInput
			|| pConnectionPoint->GetType() == xtpFlowGraphPointBoth)
	...
	while (::GetCapture() == m_hWnd)
	{
		...
		else if (msg.message == WM_MOUSEMOVE && pt != msg.pt)
		{
			...
			if (pHitPoint
				&& (pHitPoint->GetType() == xtpFlowGraphPointInput
					|| pHitPoint->GetType() == xtpFlowGraphPointBoth)
				&& !pHitPoint->IsLocked()
...
void CXTPFlowGraphControl::StartResizeNode(CXTPFlowGraphNode* pNode, UINT uhitTest)
{
	...
	while (::GetCapture() == m_hWnd)
	{
		...
		{
			CPoint ptOffset(int(1 / m_pActivePage->m_dZoomLevel * (msg.pt.x - m_ptStartDrag.x)),
							int(1 / m_pActivePage->m_dZoomLevel * (msg.pt.y - m_ptStartDrag.y)));
...
void CXTPFlowGraphControl::StartDragNodes()
{
	...
	for (int i = 0; i < m_pActivePage->GetSelection()->GetCount(); i++)
	{
		CXTPFlowGraphNode* pNode = DYNAMIC_DOWNCAST(CXTPFlowGraphNode,
													m_pActivePage->GetSelection()->GetAt(i));
		if (pNode)
		{
			if (!pNode->IsLocked())
			{
		...
		else
		{
			CXTPFlowGraphConnection* pConnection =
				DYNAMIC_DOWNCAST(CXTPFlowGraphConnection, m_pActivePage->GetSelection()->GetAt(i));
	...
	while (::GetCapture() == m_hWnd)
	{
		...
		else if (msg.message == WM_MOUSEMOVE && pt != msg.pt)
		{
			...
			for (int i = 0; i < m_arrDragNodes.GetSize(); i++)
			{
				...
				if (dn.bNode)
				{
					((CXTPFlowGraphNode*)dn.pElement)
						->SetLocation(
							CPoint(dn.ptOrigin.x + ptOffset.x, dn.ptOrigin.y + ptOffset.y));
				}
				else
				{
					((CXTPFlowGraphConnection*)dn.pElement)
						->SetControlPoint(
							CPoint(dn.ptOrigin.x + ptOffset.x, dn.ptOrigin.y + ptOffset.y));
				}
			}
...
void CXTPFlowGraphControl::StartDragScreen()
{
	...
	while (::GetCapture() == m_hWnd)
	{
		...
		{
			CPoint ptOffset(int(1.0 / m_pActivePage->m_dZoomLevel * (msg.pt.x - m_ptStartDrag.x)),
							int(1.0 / m_pActivePage->m_dZoomLevel * (msg.pt.y - m_ptStartDrag.y)));

			m_pActivePage->m_ptScrollOffset = CPoint(ptOrigin.x - ptOffset.x,
													 ptOrigin.y - ptOffset.y);
			SendNotifyMessage(XTP_FGN_PAGESCROLLOFFSETCHANGED);
...
void CXTPFlowGraphControl::ZoomToSelection()
{
	...
	for (int i = 0; i < m_pActivePage->GetSelection()->GetCount(); i++)
	{
		...
		if (CXTPFlowGraphNode* pNode = DYNAMIC_DOWNCAST(CXTPFlowGraphNode,
														m_pActivePage->GetSelection()->GetAt(i)))
		{
			rc = GetPaintManager()->GetNodeBoundingRectangle(pNode);
		}
		else if (CXTPFlowGraphConnection* pConnection = DYNAMIC_DOWNCAST(
					 CXTPFlowGraphConnection, m_pActivePage->GetSelection()->GetAt(i)))
		{
...
void CXTPFlowGraphControl::AnimatePageTransition(CXTPFlowGraphNode* pNodeFrom,
												 CXTPFlowGraphNode* pNodeTo)
{
	...
	CBitmap bmpFrom;
	bmpFrom.Attach(
		CreateDIBSection(NULL, (BITMAPINFO*)&BMI, DIB_RGB_COLORS, (void**)&pSrcBits, 0, 0l));

	CBitmap bmpTo;
	BYTE* pDestBits = NULL;
	bmpTo.Attach(
		CreateDIBSection(NULL, (BITMAPINFO*)&BMI, DIB_RGB_COLORS, (void**)&pDestBits, 0, 0l));
	...
	for (;;)
	{
		...
		{
			m_pActivePage->m_dZoomLevel = dZoomLevelToSrc
										  + (dZoomLevelToDest - dZoomLevelToSrc) * (step + 1)
												/ stepCount;

			m_pActivePage->m_ptScrollOffset = CPoint(
				int((xFrom + (xTo - xFrom) * (step + 1) / stepCount) / m_pActivePage->m_dZoomLevel),
				int((yFrom + (yTo - yFrom) * (step + 1) / stepCount)
					/ m_pActivePage->m_dZoomLevel));
		}

		double dZoomLevelFrom = dZoomLevelFromSrc
								+ (dZoomLevelFromDest - dZoomLevelFromSrc) * (step + 1) / stepCount;
		CPoint ptScroolOffsetFrom = CPoint(int((ptScrollOffsetFromSrc.x * dZoomLevelFromSrc
												+ (ptScrollOffsetFromDest.x * dZoomLevelFromDest
												   - ptScrollOffsetFromSrc.x * dZoomLevelFromSrc)
													  * (step + 1) / stepCount)
											   / dZoomLevelFrom),
										   int((ptScrollOffsetFromSrc.y * dZoomLevelFromSrc
												+ (ptScrollOffsetFromDest.y * dZoomLevelFromDest
												   - ptScrollOffsetFromSrc.y * dZoomLevelFromSrc)
													  * (step + 1) / stepCount)
											   / dZoomLevelFrom));

		{
...

			{
				...
				pDC->SetWorldTransform(m_pActivePage->m_ptScrollOffset,
									   m_pActivePage->m_dZoomLevel);
...
void CXTPFlowGraphControl::AnimateTo(CPoint ptScrollOffset, double dZoomLevel)
{
	...
	UINT_PTR nIDTimer = SetTimer(1001, XTPToUIntChecked(m_nAnimationDelay), NULL);
	...
	for (;;)
	{
		if ((int)GetTickCount() - dwStart > m_nAnimationDelay)
		{
			...
			{
				m_pActivePage->m_dZoomLevel = dOldZoomLevel
											  + (dZoomLevel - dOldZoomLevel) * (step + 1)
													/ stepCount;

				m_pActivePage->m_ptScrollOffset =
					CPoint(int((xFrom + (xTo - xFrom) * (step + 1) / stepCount)
							   / m_pActivePage->m_dZoomLevel),
						   int((yFrom + (yTo - yFrom) * (step + 1) / stepCount)
							   / m_pActivePage->m_dZoomLevel));
			}
...
CXTPFlowGraphNodeCustom* CXTPFlowGraphControl::AddCustomNodeFromToolkitResource(
	UINT uResShapeID, CXTPFlowGraphPage* pPage /*= NULL*/)
{
	if (!pPage)
		pPage = GetActivePage();
...
INT_PTR CXTPFlowGraphControl::OnToolHitTest(CPoint point, TOOLINFO* pTI) const
{
	...
	if (pConnection)
	{
		...
		CXTPToolTipContext::FillInToolInfo(
			pTI, m_hWnd, CRect(point.x - 3, point.y - 3, point.x + 3, point.y + 3), nHit, strTip);
...
LRESULT CXTPFlowGraphControl::SendNotifyMessage(UINT nMessage, NMHDR* pNMHDR) const
{
	...
	pNMHDR->idFrom   = XTPToUIntPtr(GetDlgCtrlID());
...
void CXTPFlowGraphControl::SetScrollbarTheme(XTPScrollBarTheme theme)
{
	...
	if (theme != xtpScrollBarThemeWindowsDefault)
	{
		if (!m_pCustomScrollBars[SB_VERT])
		{
			...
			m_pCustomScrollBars[SB_VERT]->Create(WS_CHILD | WS_VISIBLE | SBS_VERT,
		...
		if (!m_pCustomScrollBars[SB_HORZ])
		{
			...
			m_pCustomScrollBars[SB_HORZ]->Create(WS_CHILD | WS_VISIBLE | SBS_HORZ,
...
void CXTPFlowGraphControl::RepositionCustomScrollBars()
{
	...
	if (pVertical)
	{
		...
		pVertical->SetWindowPos(NULL, vertScrollRect.left, vertScrollRect.top,
								vertScrollRect.Width(), vertScrollRect.Height(), SWP_NOZORDER);
	}

	if (pHorizontal)
	{
		CRect horzScrollRect = rcWindow;
		horzScrollRect.DeflateRect(0, rcWindow.Height() - ::GetSystemMetrics(SM_CYHSCROLL),
								   ::GetSystemMetrics(SM_CXVSCROLL), 0);

		pScrollParent->ScreenToClient(horzScrollRect);

		pHorizontal->SetWindowPos(NULL, horzScrollRect.left, horzScrollRect.top,
								  horzScrollRect.Width(), horzScrollRect.Height(), SWP_NOZORDER);
	}
}
...
void CXTPFlowGraphControl::EnableScrollBarCtrl(int nBar, BOOL bEnable)
{
	...
	{
		m_pCustomScrollBars[nBar]->EnableScrollBar(
			UINT(bEnable ? ESB_ENABLE_BOTH : ESB_DISABLE_BOTH));
		m_pCustomScrollBars[nBar]->EnableWindow(bEnable);
	}
	else
		::EnableScrollBar(m_hWnd, XTPToUIntChecked(nBar),
						  UINT(bEnable ? ESB_ENABLE_BOTH : ESB_DISABLE_BOTH));
}
