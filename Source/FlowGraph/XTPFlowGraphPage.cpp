...
void CXTPFlowGraphPage::Reposition(CXTPFlowGraphDrawContext* pDC, CRect rcPage)
{
	...
	for (i = 0; i < m_pConnections->GetCount(); i++)
	{
		...
		if (pConnection->m_bDirty
			|| (pConnection->GetInputPoint() && pConnection->GetInputPoint()->GetNode()->m_bDirty)
			|| (pConnection->GetOutputPoint()
				&& pConnection->GetOutputPoint()->GetNode()->m_bDirty))
		{
	...
	for (i = 0; i < nNodesCount; i++)
	{
		...
		if (rcBounds.right < rcNodeBounds.left || rcBounds.left > rcNodeBounds.right
			|| rcBounds.top > rcNodeBounds.bottom || rcBounds.bottom < rcNodeBounds.top)
			continue;
...
CRect CXTPFlowGraphPage::PageToScreen(LPCRECT lpRect) const
{
	...
	return CRect(int(rc.left * dZoomLevel), int(rc.top * dZoomLevel), int(rc.right * dZoomLevel),
				 int(rc.bottom * dZoomLevel));
}
...
CRect CXTPFlowGraphPage::ScreenToPage(LPCRECT lpRect) const
{
	...
	return CRect(int(lpRect->left / dZoomLevel + GetScrollOffset().x),
				 int(lpRect->top / dZoomLevel + GetScrollOffset().y),
				 (int)ceil(lpRect->right / dZoomLevel + GetScrollOffset().x),
				 (int)ceil(lpRect->bottom / dZoomLevel + GetScrollOffset().y));
}
...
void CXTPFlowGraphPage::Copy()
{
	try
	{
		...
		ar.WriteCount(XTPToULongPtr(nCount));
		...
		for (i = 0; i < GetSelection()->GetCount(); i++)
		{
			...
			{
				CXTPFlowGraphNodeCustom* pNode = DYNAMIC_DOWNCAST(CXTPFlowGraphNodeCustom,
																  GetSelection()->GetAt(i));
...
void CXTPFlowGraphPage::Cut()
{
	...
	for (int i = GetSelection()->GetCount() - 1; i >= 0; i--)
	{
		...
		CXTPFlowGraphConnection* pConnection = DYNAMIC_DOWNCAST(CXTPFlowGraphConnection,
																GetSelection()->GetAt(i));
...
void CXTPFlowGraphPage::SetVisibleNodesWindowsRegions()
{
	...
	if (bAtLeastOneWindowNode) // for optimization purposes
	{
		...
		// a tricky step: combine reagons to detect the visible region of each node on the final
		// picture
		...
		for (i = 0; i < nodesWindowVisibleRegions.GetSize(); i++)
		{
			for (k = 0; k < i; k++)
			{
				...
				int result = CombineRgn(hRegionIntersection, nodesWindowVisibleRegions[k],
										nodesWindowVisibleRegions[i], RGN_AND);

				ASSERT(result != ERROR);

				if (result != NULLREGION) // intersection not empty
				{
					result = CombineRgn(nodesWindowVisibleRegions[k], nodesWindowVisibleRegions[k],
										hRegionIntersection, RGN_DIFF);
		...
		for (i = 0; i < nodesWindowVisibleRegions.GetSize(); i++)
		{
			if (allNodes[i]->GetWindowHandle())
			{
				...
				int result = CombineRgn(nodesWindowVisibleRegions[i], nodesWindowVisibleRegions[i],
										hRegionWindow, RGN_AND);
				UNUSED(result);
...
void CXTPFlowGraphPage::SetArrangeLevel(int v, CArray<CDWordArray*, CDWordArray*>& graph)
{
	...
	{
		int t					 = XTPToIntChecked(graph[v]->GetAt(i));
...
int CXTPFlowGraphPage::SetArrangeLocation(int v, int x, int y, int nLevelWidth,
...
void CXTPFlowGraphPage::Arrange()
{
	...
	for (i = 0; i < GetConnections()->GetCount(); i++)
	{
		...
		graph[nFrom]->Add(XTPToDWORDChecked(nTo));
		tgraph[nTo]->Add(XTPToDWORDChecked(nFrom));
	}
	...
	for (i = 0; i < nNodesCount; i++)
	{
		if (m_pNodes->GetAt(i)->m_nArrangeComponent == -1)
		{
			...
			CDWordArray stack;
			stack.Add(XTPToDWORDChecked(i));

			while (stack.GetSize())
			{
				int v = XTPToIntChecked(stack[stack.GetSize() - 1]), j;
				stack.RemoveAt(stack.GetSize() - 1);

				for (j = 0; j < graph[v]->GetSize(); j++)
				{
					int t = XTPToIntChecked(graph[v]->GetAt(j));
					if (m_pNodes->GetAt(t)->m_nArrangeComponent == -1)
					{
						m_pNodes->GetAt(t)->m_nArrangeComponent = nCompCount;
						stack.Add(XTPToDWORDChecked(t));
					}
				}

				for (j = 0; j < tgraph[v]->GetSize(); j++)
				{
					int t = XTPToIntChecked(tgraph[v]->GetAt(j));
					if (m_pNodes->GetAt(t)->m_nArrangeComponent == -1)
					{
						m_pNodes->GetAt(t)->m_nArrangeComponent = nCompCount;
						stack.Add(XTPToDWORDChecked(t));
	...
	for (i = 0; i < nNodesCount; i++)
	{
		...
		for (int j = 0; j < nSize; j++)
		{
			...
			{
				CXTPFlowGraphNode* pj = m_pNodes->GetAt(XTPToIntChecked(pConnections->GetAt(j)));
				CXTPFlowGraphNode* pk = m_pNodes->GetAt(XTPToIntChecked(pConnections->GetAt(k)));

				if ((pj->m_nArrangeLevel > pk->m_nArrangeLevel)
					|| (pj->m_nArrangeLevel == pk->m_nArrangeLevel
						&& pj->m_nArrangeIndex > pk->m_nArrangeIndex))
				{
					int t			   = XTPToIntChecked((*pConnections)[j]);
					(*pConnections)[j] = (*pConnections)[k];
					(*pConnections)[k] = XTPToDWORDChecked(t);
				}
			}
		}
	}
	...
	for (i = 0; i < order.GetSize(); i++)
	{
		...
		{
			CDWordArray& pi = *graph[XTPToIntPtrChecked(order[i])];
			CDWordArray& pj = *graph[XTPToIntPtrChecked(order[j])];

			BOOL bOrder = TRUE;

			if (m_pNodes->GetAt(XTPToIntChecked(order[i]))->m_nArrangeComponent
				!= m_pNodes->GetAt(XTPToIntChecked(order[j]))->m_nArrangeComponent)
			{
				if (m_pNodes->GetAt(XTPToIntChecked(order[i]))->m_nArrangeComponent
					> m_pNodes->GetAt(XTPToIntChecked(order[j]))->m_nArrangeComponent)
				{
			...
			if (!bOrder)
			{
				int t	= XTPToIntChecked(order[i]);
				order[i] = order[j];
				order[j] = XTPToDWORDChecked(t);
			}
		}
	}
	...
	for (i = 0; i < order.GetSize(); i++)
	{
		y = SetArrangeLocation(XTPToIntChecked(order[i]), x, y, 50 + nMaxWidth, graph);
	}
...
BEGIN_DISPATCH_MAP(CXTPFlowGraphPage, CXTPCmdTarget)
	...
	DISP_PROPERTY_EX_ID(CXTPFlowGraphPage, "Connections", 2, OleGetConnections, SetNotSupported,
						VT_DISPATCH)

	DISP_PROPERTY_EX_ID(CXTPFlowGraphPage, "Caption", DISPID_CAPTION, OleGetCaption, OleSetCaption,
						VT_BSTR)
	DISP_PROPERTY_ID(CXTPFlowGraphPage, "Id", 5, m_nId, VT_I4)

	DISP_PROPERTY_EX_ID(CXTPFlowGraphPage, "Selection", 6, OleGetSelection, SetNotSupported,
						VT_DISPATCH)
	...
	DISP_FUNCTION_ID(CXTPFlowGraphPage, "ScreenToPage", 12, OleScreenToPage, VT_EMPTY,
					 VTS_PI4 VTS_PI4)
	DISP_FUNCTION_ID(CXTPFlowGraphPage, "PageToScreen", 13, OlePageToScreen, VT_EMPTY,
					 VTS_PI4 VTS_PI4)

	DISP_PROPERTY_EX_ID(CXTPFlowGraphPage, "ScrollOffsetX", 14, OleGetScrollOffsetX,
						OleSetScrollOffsetX, VT_I4)
	DISP_PROPERTY_EX_ID(CXTPFlowGraphPage, "ScrollOffsetY", 15, OleGetScrollOffsetY,
						OleSetScrollOffsetX, VT_I4)
