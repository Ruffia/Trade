...
{
protected:
	Gdiplus::PointF* GetBezierCurveNodes(float p1X, float p1Y, float p2X, float p2Y, float p1cX,
...
public:
	...
	void Draw(CXTPFlowGraphDrawContext* pDC, float p1X, float p1Y, float p2X, float p2Y, float p1cX,
			  float p1cY, float p2cX, float p2cY);

	// draw an arrow when a line is a bezier curve (with 2 control points)
	void Draw(CXTPFlowGraphDrawContext* pDC, Gdiplus::PointF p1, Gdiplus::PointF p2,
			  Gdiplus::PointF cp1, Gdiplus::PointF cp2);
