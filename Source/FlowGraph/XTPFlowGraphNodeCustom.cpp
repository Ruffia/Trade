...
IMPLEMENT_SERIAL(CXTPFlowGraphNodeCustom, CXTPFlowGraphNode,
...
void CXTPFlowGraphNodeCustom::DoPropExchange(CXTPPropExchange* pPX)
{
	...
	PX_Int(pPX, _T("CaptionPosition"), (int&)m_CaptionPosition,
		   (int)xtpFlowGraphCustomNodeCaptionCenter);
...
CXTPMarkupPathData*
	CXTPFlowGraphNodeCustom::CreateStretchedPathData(const CXTPMarkupPath* pNodePath) const
{
...
CRect CXTPFlowGraphNodeCustom::GetPointRect(CPoint point) const
{
	...
	return CRect(point.x - nHalfEllipseSize, point.y - nHalfEllipseSize, point.x + nHalfEllipseSize,
				 point.y + nHalfEllipseSize);
}

void CXTPFlowGraphNodeCustom::UpdateColorRecursive(CXTPMarkupObject* /*pRootElement*/,
												   COLORREF /*clrColor*/)
{
	SetColor(m_clrNode, m_clrSecondColor, m_FillType, m_bAlpha);
}
...
void CXTPFlowGraphNodeCustom::UpdateConnectionPoints(CXTPMarkupObject* pRootElement)
{
	...
	if (pRootElement)
	{
		...
		const CXTPMarkupPath* pNodePath =
			MARKUP_DYNAMICCAST(CXTPMarkupPath, pRootElement->FindName(XAML_MARKUP_TAG_NODEPATH));
		const CXTPMarkupCanvas* pNodeCanvas =
			MARKUP_DYNAMICCAST(CXTPMarkupCanvas, m_pMarkup->FindName(XAML_MARKUP_TAG_NODECANVAS));

		CXTPMarkupString* pConnectionPointsIn = NULL;

		if (pNodeCanvas)
		{
			pConnectionPointsIn = MARKUP_STATICCAST(
				CXTPMarkupString, pNodeCanvas->GetValue(GetControl()->GetCustomMarkupProperty(
									  xtpFlowGraphPropertyConnectionPointIn)));

			// probably not the best place to get this property (Resizable) according to the logic

			CXTPMarkupBool* pResizableProperty = MARKUP_STATICCAST(
				CXTPMarkupBool, pNodeCanvas->GetValue(GetControl()->GetCustomMarkupProperty(
									xtpFlowGraphPropertyResizable)));
		...
		{
			ASSERT(pNodePath->GetStretch()
				   != xtpMarkupStretchNone); // Path must be with Stretch property to support
											 // correct resizing of the node

			const CRect pathRect = GetMarkupVisualElementRect(pNodePath);

			const CXTPMarkupPathData* pGeometryStretch = CreateStretchedPathData(
				pNodePath); // need to delete afterwards
		...
		else
		{
			...
			ASSERT(pConnectionPointsIn); // this property should be defined for nodes without
										 // connection points defined by a path

			if (pConnectionPointsIn)
			{
				if (0
					== wcscmp(*pConnectionPointsIn,
							  XAML_MARKUP_PROPERTY_CONNECTIONPOINTS_AS_RHOMBUS))
				{
					CRect pointOfRhombus[4];
					pointOfRhombus[0] = GetPointRect(CPoint(0, pNodeCanvas->GetHeight() / 2));
					pointOfRhombus[1] = GetPointRect(
						CPoint(pNodeCanvas->GetWidth(), pNodeCanvas->GetHeight() / 2));
					pointOfRhombus[2] = GetPointRect(CPoint(pNodeCanvas->GetWidth() / 2, 0));
					pointOfRhombus[3] = GetPointRect(
						CPoint(pNodeCanvas->GetWidth() / 2, pNodeCanvas->GetHeight()));
					...
					for (int i = 0; i < 4; i++)
					{
						...
						if (pPoint)
						{
						 ...
						 == wcscmp(*pConnectionPointsIn,
								   XAML_MARKUP_PROPERTY_CONNECTIONPOINTS_IN_CENTER))
				{
					// 1 point

					CRect pointRect = GetPointRect(
						CPoint(pNodeCanvas->GetWidth() / 2, pNodeCanvas->GetHeight() / 2));
...
void CXTPFlowGraphNodeCustom::SetSize(CSize sz)
{
	...
	if (m_pMarkup)
	{
		CXTPMarkupCanvas* pNodeCanvas =
			MARKUP_DYNAMICCAST(CXTPMarkupCanvas, m_pMarkup->FindName(XAML_MARKUP_TAG_NODECANVAS));
		CXTPMarkupShape* pNodeShape =
			MARKUP_DYNAMICCAST(CXTPMarkupShape, m_pMarkup->FindName(XAML_MARKUP_TAG_NODEPATH));
		CXTPMarkupImage* pNodeImage =
			MARKUP_DYNAMICCAST(CXTPMarkupImage, m_pMarkup->FindName(XAML_MARKUP_TAG_NODEIMAGE));

		CXTPMarkupBorder* pNodeBorder =
			MARKUP_DYNAMICCAST(CXTPMarkupBorder, m_pMarkup->FindName(XAML_MARKUP_TAG_NODEBORDER));
...
CRect CXTPFlowGraphNodeCustom::GetBoundingRectangle(
	BOOL bIncludeConnectionPointsEllipses /*= TRUE*/) const
{
	// CXTPFlowGraphNodeCustom node can have connection points on top and bottom of the rectangle
	// unlike CXTPFlowGraphNode so we need to inflate rect once again
...
CXTPMarkupTextBlock* CXTPFlowGraphNodeCustom::UpdateMarkupCaption()
{
	...
	if (m_pMarkup)
	{
		...
		{
			// seems to be a bug(?) of markup, need to manually call Arrange for correct caption
			// positioning
			...
			if (actualSize.cx)
			{
				...
				pNodeCaption->SetValue(CXTPMarkupCanvas::m_pTopProperty,
									   new CXTPMarkupInt(iCanvasTop));
				pNodeCaption->SetValue(CXTPMarkupCanvas::m_pLeftProperty,
									   new CXTPMarkupInt(iCanvasLeft));
			}
		}
	}
...
public:
	...
	dColorStop(COLORREF clr, double offset)
		: m_clr(clr)
		, m_offset(offset)
	{
	}
...
CXTPMarkupLinearGradientBrush* CreateLinearGradient(dColorStop clr1, dColorStop clr2, dPoint p1,
													dPoint p2, dColorStop clr3 = dColorStop())
{
...
void CXTPFlowGraphNodeCustom::SetColor(COLORREF clrOne, COLORREF clrTwo,
									   XTPFlowGraphCustomNodeFillType fType, BYTE bAlpha /*= 255*/)
{
	m_clrNode		 = clrOne;
	...
	if (m_pMarkup)
	{
		...
		if (pNodeBorder)
		{
			...
			CXTPMarkupDependencyProperty* pPropertyCanSetColor =
				GetControl()->GetCustomMarkupProperty(xtpFlowGraphPropertyCanSetColor);

			if (pPropertyCanSetColor)
			{
				CXTPMarkupBool* pCanSetColor =
					MARKUP_STATICCAST(CXTPMarkupBool, pNodeBorder->GetValue(pPropertyCanSetColor));
		...
		if (pNodeShape || pNodeBorder)
		{
			...
			switch (m_FillType)
			{
				...
				{
					pBrush = CreateLinearGradient(dColorStop(m_clrNode, 0),
												  dColorStop(m_clrSecondColor, 1), dPoint(0.5, 0),
												  dPoint(0.5, 1));
				...
				{
					pBrush = CreateLinearGradient(dColorStop(m_clrNode, 0),
												  dColorStop(m_clrSecondColor, 1), dPoint(0, 0.5),
												  dPoint(1, 0.5));
				...
				{
					pBrush = CreateLinearGradient(dColorStop(m_clrNode, 0),
												  dColorStop(m_clrSecondColor, 1), dPoint(0, 0),
												  dPoint(1, 1));
				...
				{
					pBrush = CreateLinearGradient(dColorStop(m_clrNode, 0),
												  dColorStop(m_clrSecondColor, 1), dPoint(0, 1),
												  dPoint(1, 0));
				...
				{
					pBrush = CreateLinearGradient(dColorStop(m_clrNode, 0),
												  dColorStop(m_clrSecondColor, 0.5), dPoint(0, 0.5),
												  dPoint(1, 0.5), dColorStop(m_clrNode, 1));
				...
				{
					pBrush = CreateLinearGradient(dColorStop(m_clrNode, 0),
												  dColorStop(m_clrSecondColor, 0.5), dPoint(0.5, 0),
												  dPoint(0.5, 1), dColorStop(m_clrNode, 1));
...
void CXTPFlowGraphNodeCustom::CalcMinSize()
{
	...
	if (m_CaptionPosition == xtpFlowGraphCustomNodeCaptionCenter)
	{
		if (m_pMarkup)
		{
			CXTPMarkupTextBlock* pNodeCaption = MARKUP_DYNAMICCAST(
				CXTPMarkupTextBlock, m_pMarkup->FindName(XAML_MARKUP_TAG_NODECAPTION));
			;
...
BEGIN_DISPATCH_MAP(CXTPFlowGraphNodeCustom, CXTPFlowGraphNode)
	...
	DISP_PROPERTY_EX_ID(CXTPFlowGraphNodeCustom, "CaptionPosition", 30, OleGetCaptionPosition,
						OleSetCaptionPosition, VT_I4)
	DISP_PROPERTY_EX_ID(CXTPFlowGraphNodeCustom, "MarkupText", 31, OleGetMarkupText,
						OleSetMarkupText, VT_BSTR)

	DISP_PROPERTY_EX_ID(CXTPFlowGraphNodeCustom, "SecondColor", 32, OleGetSecondColor,
						OleSetSecondColor, VT_COLOR)
	DISP_PROPERTY_EX_ID(CXTPFlowGraphNodeCustom, "FillType", 33, OleGetFillType, OleSetFillType,
						VT_I4)
