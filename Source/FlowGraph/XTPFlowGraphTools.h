...
public:
	...
	~CXTPFlowGraphGDIHandles()
	{
		...
		{
			CXTPFlowGraphGDIHandles::DestroyHandle(m_arr[i].hTo);
		}
	}
	...
	ToType GetHandle(const FromType& hFrom)
	{
		...
		{
			if (Traits::CompareHandle(m_arr[i].hFrom, hFrom))
				return m_arr[i].hTo;
		}

		if (m_nCount == MAX_HANDLE)
		{
			Traits::DestroyHandle(m_arr[m_nCount - 1].hTo);
		}
		else if (m_nCount)
		{
		...
		m_arr[0].hFrom = hFrom;
		m_arr[0].hTo   = Traits::CreateHandle(hFrom);
