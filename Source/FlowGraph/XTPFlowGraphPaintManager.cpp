...
void CXTPFlowGraphPaintManager::DrawControlBackground(CXTPFlowGraphDrawContext* pDC,
													  CXTPFlowGraphControl* pControl,
													  BOOL bPrintToFile /*= FALSE*/,
													  const CRect& drawRect /*= CRect()*/)
{
	...
	if (m_bShowGrid && !pDC->IsPrinting() && pControl->GetZoomLevel() >= m_dMinZoomLevelBackground)
	{
		...
		pDC->DrawGrid(rc, !bPrintToFile ? pControl->GetScrollOffset() : CPoint(0, 0),
					  !bPrintToFile ? pControl->GetZoomLevel() : 1.0, m_clrGridColor);
	}
}

// p1 and p2 - two points of intersection
bool IntersectLineWithCircle(PointF linep1, PointF linep2, PointF circleCenter, double circleR,
							 PointF& p1, PointF& p2)
{
...
bool GetCirclesSmartConnectionPoints(PointF point1, PointF point2, CPoint& ptFrom, CPoint& ptTo,
									 double R)
{
	PointF p1, p2;
...
{
	PointF pt;
	pt.X = static_cast<REAL>(rc.TopLeft().x) + static_cast<REAL>(rc.Width()) / 2;
	pt.Y = static_cast<REAL>(rc.TopLeft().y) + static_cast<REAL>(rc.Height()) / 2;
...
{
	return (abs(p1.x - p2.x) < POINTS_PROXIMITY_THRESHOLD
			|| abs(p1.y - p2.y) < POINTS_PROXIMITY_THRESHOLD);
}

BOOL ArePointsApproxOnTheSameLine(PointF p1, PointF p2)
{
	return (fabs(p1.X - p2.X) < POINTS_PROXIMITY_THRESHOLD
			|| fabs(p1.Y - p2.Y) < POINTS_PROXIMITY_THRESHOLD);
}

BOOL CXTPFlowGraphPaintManager::CalculateConnectionPointsPositions(
	CXTPFlowGraphConnection* pConnection, CPoint& p1, CPoint& p2)
{
	...
	CRect rc2 = pTo ? pTo->GetRect()
					: CRect(0, 0, 0, 0); // this happens when a connection is dragged
	...
	else
	{
		...
		{
			point2 = PointF((REAL)pConnection->m_ptInputPoint.x,
							(REAL)pConnection->m_ptInputPoint.y);
		}
		else
		{
			return FALSE;
...
void CXTPFlowGraphPaintManager::RecalcConnectionLayout(CXTPFlowGraphDrawContext* /*pDC*/,
...
#	pragma pop_macro("new")
	...
	if (IsStraightConnection(type) || bForceUseStraight)
	{
		...
		{
			Point pts[] = { Point(ptFrom.x, ptFrom.y), Point(ptControlPoint.x, ptControlPoint.y),
							Point(ptTo.x, ptTo.y) };
		...
		{
			path->AddLine(static_cast<INT>(ptFrom.x), static_cast<INT>(ptFrom.y),
						  static_cast<INT>(ptTo.x), static_cast<INT>(ptTo.y));
		}
	}
	else if (IsCurvedConnection(type))
	{
		if (ptControlPoint != CPoint(-1, -1))
		{
			PointF pt2[4] = { PointF((REAL)ptFrom.x, (REAL)ptFrom.y),
							  PointF((REAL)ptControlPoint.x, (REAL)ptFrom.y),
							  PointF((REAL)ptControlPoint.x, (REAL)ptTo.y),
		...
		{
			PointF pt[4] = { PointF((REAL)ptFrom.x, (REAL)ptFrom.y),
							 PointF((REAL)(ptFrom.x + ptTo.x) / 2, (REAL)ptFrom.y),
							 PointF(REAL(ptFrom.x + ptTo.x) / 2, (REAL)ptTo.y),
							 PointF((REAL)ptTo.x, (REAL)ptTo.y) };
	...
	else // elbow
	{
		...
		path->AddLine(static_cast<INT>(p1.x), static_cast<INT>(p1.y),
					  static_cast<INT>(controlPoint1.x), static_cast<INT>(controlPoint1.y));
		path->AddLine(static_cast<INT>(controlPoint1.x), static_cast<INT>(controlPoint1.y),
					  static_cast<INT>(controlPoint2.x), static_cast<INT>(controlPoint2.y));
		path->AddLine(static_cast<INT>(controlPoint2.x), static_cast<INT>(controlPoint2.y),
...
COLORREF AFX_CDECL CXTPFlowGraphPaintManager::GetDarkColor(COLORREF clrBackground)
{
	...
	return CXTPDrawHelpers::HSLtoRGB(
		RGB(GetRValue(dwHSLBackground), GetGValue(dwHSLBackground), dwDark));
}

void CXTPFlowGraphPaintManager::DrawConnection(CXTPFlowGraphDrawContext* pDC,
											   CXTPFlowGraphConnection* pConnection)
{
	...
	COLORREF clrConnection = pConnection->GetColor() != (COLORREF)-1 ? pConnection->GetColor()
																	 : m_clrConnection;
	...
	if (HasEndingArrow(type))
	{
		if (pConnection->m_pPath)
		{
			...
			if (pointCount == 2 && IsElbowConnection(type)
				&& ArePointsApproxOnTheSameLine(points[0], points[1]))
				bForceUseStraight = true;
	...
	{
		CXTPTextOnPath textOnPath(pConnection->GetCaption(), pDC->GetGraphics(),
								  pConnection->m_pPath);

		if (pConnection->m_ptOutputPoint.x > pConnection->m_ptInputPoint.x
			&& !pConnection->IsPathReversed())
		{
...
BOOL CXTPFlowGraphPaintManager::HitTestConnectionArea(const CXTPFlowGraphConnectionPoint* pPoint,
													  CPoint point)
{
...
void CXTPFlowGraphPaintManager::DrawTransparentRectangle(CXTPFlowGraphDrawContext* pDC, CRect rect,
														 COLORREF clrColor)
{
...
void CXTPFlowGraphPaintManager::DrawNodeGroup(CXTPFlowGraphDrawContext* pDC,
											  CXTPFlowGraphNodeGroup* pNodeGroup)
{
...
BEGIN_DISPATCH_MAP(CXTPFlowGraphPaintManager, CXTPCmdTarget)
	...
	DISP_PROPERTY_EX_ID(CXTPFlowGraphPaintManager, "Font", DISPID_FONT, OleGetFont, OleSetFont,
						VT_DISPATCH)
	...
	DISP_PROPERTY_ID(CXTPFlowGraphPaintManager, "NodeBackgroundColor", 6, m_clrNodeBackground,
					 VT_COLOR)
	DISP_PROPERTY_EX_ID(CXTPFlowGraphPaintManager, "CaptionFont", 7, OleGetCaptionFont,
	...
	DISP_PROPERTY_ID(CXTPFlowGraphPaintManager, "ShowNodeGroupsAlways", 13, m_bShowNodeGroupsAlways,
					 VT_BOOL)
	DISP_PROPERTY_ID(CXTPFlowGraphPaintManager, "GridColor", 14, m_clrGridColor, VT_COLOR)
	DISP_PROPERTY_ID(CXTPFlowGraphPaintManager, "ShowConnectionPointsAlways", 15,
					 m_bAlwaysDrawConnectionPoints, VT_BOOL)
	DISP_PROPERTY_ID(CXTPFlowGraphPaintManager, "RecalculateConnectionsLayoutWhenNeeded", 16,
					 m_bRecalcConnectionLayoutWhenNeeded, VT_BOOL)
	DISP_PROPERTY_ID(CXTPFlowGraphPaintManager, "ChangeNodeColorWhenSelected", 17,
					 m_bChangeNodeColorWhenSelected, VT_BOOL)
END_DISPATCH_MAP()
...
{
	return AxCreateOleFont(m_lfCaption, this,
						   (LPFNFONTCHANGED)&CXTPFlowGraphPaintManager::OleSetCaptionFont);
}
