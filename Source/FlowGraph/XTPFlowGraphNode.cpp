	...
	if (pControl && pRootElement)
	{
		...
		for (int i = 0, c = pRootElement->GetLogicalChildrenCount(); i < c; i++)
		{
			...
			if (pElement->IsKindOf(MARKUP_TYPE(CXTPMarkupVisual)))
			{
				...
				CXTPMarkupString* pValueCPName = MARKUP_STATICCAST(
					CXTPMarkupString, pElementVisual->GetValue(pControl->GetCustomMarkupProperty(
										  xtpFlowGraphPropertyConnectionPoint)));
				CXTPMarkupInt* pValueCPType = MARKUP_STATICCAST(
					CXTPMarkupInt, pElementVisual->GetValue(pControl->GetCustomMarkupProperty(
									   xtpFlowGraphPropertyConnectionPointType)));
				...
				if (pValueCPName)
				{
					...
					// if (!rect.IsRectEmpty())
					{
						...
						XTPFlowGraphConnectionPointType type = (XTPFlowGraphConnectionPointType)(
							int)*pValueCPType;

						CXTPFlowGraphConnectionPoint* pPoint =
							GetConnectionPoints()->FindConnectionPoint(cpName, type);
...
void CXTPFlowGraphNode::SetWindowSize(CSize sz)
{
	...
	if (m_pMarkup)
	{
		...
		CXTPMarkupPanel* pNodeCustomControl =
			MARKUP_DYNAMICCAST(CXTPMarkupPanel, m_pMarkup->FindName(XAML_MARKUP_TAG_CUSTOMCONTROL));
...
void CXTPFlowGraphNode::UpdateMarkupImage()
{
	...
	{
		CXTPMarkupImage* pNodeImage =
			MARKUP_DYNAMICCAST(CXTPMarkupImage, m_pMarkup->FindName(XAML_MARKUP_TAG_NODEIMAGE));
...
int CXTPFlowGraphNode::RemoveMarkupTableRow(CXTPFlowGraphConnectionPoint* pPoint)
{
	...
	CXTPMarkupBorder* pNodeItem = MARKUP_DYNAMICCAST(CXTPMarkupBorder,
													 m_pMarkup->FindName(szNameToSearch));
	if (pNodeItem)
	{
		...
		{
			CXTPMarkupPanel* pNodeItemsCollection = MARKUP_DYNAMICCAST(
				CXTPMarkupPanel, m_pMarkup->FindName(XAML_MARKUP_TAG_ITEMSCOLLECTION));
...
CXTPMarkupTextBlock*
	CXTPFlowGraphNode::GetTableRowTextBlock(const CString& strConnectionPointName) const
{
	...
	if (pNodeItem)
	{
		pReturnTextBlock = MARKUP_DYNAMICCAST(CXTPMarkupTextBlock,
											  pNodeItem->FindName(
												  XAML_MARKUP_TAG_NODE_ITEM_CAPTION));
	}
...
void CXTPFlowGraphNode::UpdateMarkupPointColor(CXTPFlowGraphConnectionPoint* pPoint)
{
	...
	if (pNodeItemCaption)
	{
		CXTPMarkupSolidColorBrush* pBrush = new CXTPMarkupSolidColorBrush(
			CXTPFlowGraphPaintManager::GetDarkColor(pPoint->GetColor()));
...
void CXTPFlowGraphNode::UpdateMarkupPointImage(CXTPFlowGraphConnectionPoint* pPoint)
{
	...
	if (m_pMarkup && iImageIndex != -1)
	{
		...
		if (pNodeItem)
		{
			CXTPMarkupImage* pNodeImage = MARKUP_DYNAMICCAST(CXTPMarkupImage,
															 pNodeItem->FindName(
																 XAML_MARKUP_TAG_NODE_ITEM_IMAGE));
...
int CXTPFlowGraphNode::GetMarkupTableRowIndex(CXTPFlowGraphConnectionPoint* pPoint,
...
void CXTPFlowGraphNode::AddNamedConnectionPoints(
	...
	if (m_pMarkup && pControl)
	{
		...
		if (pNodeItemsCollection)
		{
			...
			if (pChildren)
			{
				...
				{
					pExistingPoint = GetConnectionPoints()->FindConnectionPoint(
						szName, (type == xtpFlowGraphPointInput) ? xtpFlowGraphPointOutput
																 : xtpFlowGraphPointInput);
				...
				{
					pExistingPoint =
						GetConnectionPoints()->FindConnectionPoint(szName, xtpFlowGraphPointInput);

					if (!pExistingPoint)
						pExistingPoint = GetConnectionPoints()->FindConnectionPoint(
							szName, xtpFlowGraphPointOutput);
				}
				...
				if (pExistingPoint)
				{
					...
					// the XAML markup will be removed when calling
					// CXTPFlowGraphConnectionPoint::OnRemoved() do not remove pairs !
				...
				switch (type)
				{
					case xtpFlowGraphPointNone:
						markupTemplate = GetControl()->GetDefaultMarkupTemplate(
							xtpFlowGraphTemplateNodePointNone);
						break;
					case xtpFlowGraphPointInput:
						markupTemplate = GetControl()->GetDefaultMarkupTemplate(
							xtpFlowGraphTemplateNodePointIn);
						break;
					case xtpFlowGraphPointOutput:
						markupTemplate = GetControl()->GetDefaultMarkupTemplate(
							xtpFlowGraphTemplateNodePointOut);
						break;
					case xtpFlowGraphPointInputAndOutput:
						markupTemplate = GetControl()->GetDefaultMarkupTemplate(
							xtpFlowGraphTemplateNodePointInOut);
						break;
				}
				...
				if (markupTemplate.GetLength())
				{
					...
					markup.Replace(XAML_MARKUP_CPCAPTION_PLACEHOLDER,
								   szCaption ? szCaption : szName);

					CXTPMarkupUIElement* pTableRow =
						XTPMarkupParseText(pControl->GetMarkupContext(), markup);
...
void CXTPFlowGraphNode::UpdateMarkupFont()
{
	if (m_pMarkup && GetControl() && GetControl()->GetPaintManager())
	{
		...
		XTPMarkupSetDefaultFont(XTPMarkupElementContext(m_pMarkup), font,
...
void CXTPFlowGraphNode::CalcMinSize()
{
	...
	if (m_pMarkup)
	{
		CXTPMarkupBorder* pNodeCaptionBorder = MARKUP_DYNAMICCAST(
			CXTPMarkupBorder, m_pMarkup->FindName(XAML_MARKUP_TAG_NODECAPTIONBORDER));
		...
		{
			if (iMarkupRowHeight)
				iMinHeight = (pNodeItemsCollection->GetChildren()->GetCount() + 1)
							 * iMarkupRowHeight;
		}
		...
		if (pNodeCaption)
		{
			...
			iMinWidth					= pNodeCaption->GetActualSize().cx
						+ nCaptionBorderPadding_X * 2; // left and right padding
		}
...
void CXTPFlowGraphNode::RecalLayout()
{
	...
	if (m_pMarkup)
	{
		...
		// connection points markup (rects) size and position is actually CRect(0,0,0,0) before it's
		// first arranged so we need to arrange the markup within a rect
		...
		if (GetControl()->GetPaintManager()
			&& GetControl()->GetPaintManager()->m_bChangeNodeColorWhenSelected)
		{
			if (IsSelected())
	...
	if (pControl && pRootElement)
	{
		for (int i = 0, c = pRootElement->GetLogicalChildrenCount(); i < c; i++)
		{
			...
			if (pElement->IsKindOf(MARKUP_TYPE(CXTPMarkupBorder)))
			{
				...
				CXTPMarkupSolidColorBrush* pBrush = new CXTPMarkupSolidColorBrush(
					CXTPFlowGraphPaintManager::GetDarkColor(clrColor));

				pBorder->SetBorderBrush(pBrush);

				CXTPMarkupDependencyProperty* pPropertyCanSetColor =
					pControl->GetCustomMarkupProperty(xtpFlowGraphPropertyCanSetColor);

				if (pPropertyCanSetColor)
				{
					CXTPMarkupBool* pCanSetColor =
						MARKUP_STATICCAST(CXTPMarkupBool, pElement->GetValue(pPropertyCanSetColor));

					if (pCanSetColor)
					{
						CXTPMarkupSolidColorBrush* pBrushBackground = new CXTPMarkupSolidColorBrush(
							clrColor);
...
void CXTPFlowGraphNode::DrawConnectionPoints(CXTPFlowGraphDrawContext* pDC)
{
	...
	if (GetControl())
	{
		if (GetControl()->GetPaintManager()->m_bAlwaysDrawConnectionPoints == FALSE
			&& IsSelected() == FALSE)
		{
	...
	if (bDrawConnectionPoints)
	{
		...
		for (int i = 0; i < nCount; i++)
		{
			...
			COLORREF clrBrush = pPoint->IsSelected()
									? clrSelection
									: pPoint->GetConnectionsCount() > 0 ? RGB(250, 252, 58)
																		: clrEllipse;
...
{
	CRect rc(GetLocation(), GetSize());
	rc.InflateRect(GetControl()->GetPaintManager()->m_nEllipseSize,
				   GetControl()->GetPaintManager()->m_nEllipseSize); // for connection points
																	 // ellipses
...
void CXTPFlowGraphNode::DrawSelectionRect(CXTPFlowGraphDrawContext* pDC)
{
	...
	double zl = GetControl()->GetActivePage()->GetZoomLevel();
	CXTPFlowGraphSelectionRect::SetZoomLevel(zl);

	CRect rcSel = GetSelectionRect();
	rcSel.SetRect(static_cast<int>(rcSel.left * zl), static_cast<int>(rcSel.top * zl),
				  static_cast<int>(rcSel.right * zl), static_cast<int>(rcSel.bottom * zl));

	CPoint offset = GetControl()->GetActivePage()->GetScrollOffset();
	offset.x	  = static_cast<int>(offset.x * zl);
	offset.y	  = static_cast<int>(offset.y * zl);

	pDC->SetWorldTransform(offset, 1);

	m_pSelectionRect->Draw(pDC, rcSel);

	pDC->SetWorldTransform(offset, GetControl()->GetActivePage()->GetZoomLevel());
}
...
void CXTPFlowGraphNode::SetLocation(CPoint ptLocation)
{
	...
	if (pControl)
	{
		pControl->GetUndoManager()->AddUndoCommand(
			new CXTPFlowGraphUndoSetNodePositionCommand(this, m_ptLocation));
	}
...
void CXTPFlowGraphNode::SetSizeCore(CSize sz)
{
	...
	{
		pControl->GetUndoManager()->AddUndoCommand(
			new CXTPFlowGraphUndoSetNodeSizeCommand(this, m_szUserSize));
	}
...
void CXTPFlowGraphNode::SetSize(CSize sz)
{
	...
	if (m_pMarkup)
	{
		CXTPMarkupStackPanel* pNodeContent = MARKUP_DYNAMICCAST(CXTPMarkupStackPanel,
																m_pMarkup->FindName(
																	XAML_MARKUP_TAG_NODECONTENT));
...
CXTPMarkupTextBlock* CXTPFlowGraphNode::GetCaptionTextBlock() const
{
	...
	if (m_pMarkup)
	{
		pNodeCaption = MARKUP_DYNAMICCAST(CXTPMarkupTextBlock,
										  m_pMarkup->FindName(XAML_MARKUP_TAG_NODECAPTION));
	}
...
void CXTPFlowGraphNode::UpdateCaptionColor()
{
	...
	if (pNodeCaption)
	{
		...
		{
			CXTPMarkupSolidColorBrush* pBrush = new CXTPMarkupSolidColorBrush(
				IsSelected() ? GetControl()->GetPaintManager()->m_clrTextColorSelected
							 : GetControl()->GetPaintManager()->m_clrTextColor);
...
CMap<HWND, HWND, CXTPFlowGraphNode*, CXTPFlowGraphNode*>
	CXTPFlowGraphNode::m_WindowToNodeHash; // used in the custom window 'ControlWindowProc'

LRESULT CALLBACK CXTPFlowGraphNode::ControlWindowProc(
	HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) // new control window procedure, static
{
...
void CXTPFlowGraphNode::SetCustomWndProc()
{
	if (m_hWnd)
	{
		...
		if (dwWindowProcessID == GetCurrentProcessId())
		{
			...
			m_pOriginalWindowProc = reinterpret_cast<WNDPROC>(
				SetWindowLongPtr(m_hWnd, GWLP_WNDPROC,
								 reinterpret_cast<LONG_PTR>(ControlWindowProc)));
...
BEGIN_DISPATCH_MAP(CXTPFlowGraphNode, CXTPFlowGraphElement)
	DISP_PROPERTY_EX_ID(CXTPFlowGraphNode, "Caption", DISPID_CAPTION, OleGetCaption, OleSetCaption,
						VT_BSTR)
	...
	DISP_PROPERTY_EX_ID(CXTPFlowGraphNode, "ConnectionPoints", 6, OleGetConnectionPoints,
						SetNotSupported, VT_DISPATCH)

	DISP_PROPERTY_EX_ID(CXTPFlowGraphNode, "WindowHandle", 18, OleGetWindowHandle,
						OleSetWindowHandle, VT_HANDLE)
	...
	DISP_PROPERTY_EX_ID(CXTPFlowGraphNode, "WindowWidth", 19, OleGetWindowWidth, OleSetWindowWidth,
						VT_I4)
	DISP_PROPERTY_EX_ID(CXTPFlowGraphNode, "WindowHeight", 20, OleGetWindowHeight,
						OleSetWindowHeight, VT_I4)
	...
	DISP_FUNCTION_ID(CXTPFlowGraphNode, "HitTestConnectionPoint", 17, OleHitTestConnectionPoint,
					 VT_DISPATCH, VTS_XPOS_PIXELS VTS_YPOS_PIXELS)

	DISP_FUNCTION_ID(CXTPFlowGraphNode, "AddNamedConnectionPoints", 21, OleAddNamedConnectionPoints,
					 VT_EMPTY, VTS_BSTR VTS_I4)
	DISP_FUNCTION_ID(CXTPFlowGraphNode, "AddNamedConnectionPointsWithCaption", 22,
					 OleAddNamedConnectionPointsWithCaption, VT_EMPTY, VTS_BSTR VTS_I4 VTS_BSTR)
...
void CXTPFlowGraphNode::OleAddNamedConnectionPointsWithCaption(LPCTSTR lpszName, long type,
															   LPCTSTR lpszCaption)
{
	// more info at
	// http://support.microsoft.com/kb/154039
