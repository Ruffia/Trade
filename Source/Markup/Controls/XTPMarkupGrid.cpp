...
BEGIN_DISPATCH_MAP(CXTPMarkupDefinitionCollection, CXTPMarkupObject)
	...
	DISP_FUNCTION_ID(CXTPMarkupDefinitionCollection, "Item", DISPID_VALUE, OleGetItem, VT_DISPATCH,
					 VTS_I4)
	DISP_FUNCTION_ID(CXTPMarkupDefinitionCollection, "_NewEnum", DISPID_NEWENUM, OleNewEnum,
					 VT_UNKNOWN, VTS_NONE)
	DISP_FUNCTION_ID(CXTPMarkupDefinitionCollection, "Add", 2, OleAdd, VT_EMPTY, VTS_DISPATCH)
	DISP_FUNCTION_ID(CXTPMarkupDefinitionCollection, "Insert", 3, OleInsert, VT_EMPTY,
					 VTS_I4 VTS_DISPATCH)
...
{
	m_pColumnDefinitionsProperty = CXTPMarkupDependencyProperty::Register(
		L"ColumnDefinitions", MARKUP_TYPE(CXTPMarkupColumnDefinitionCollection),
		MARKUP_TYPE(CXTPMarkupGrid));

	m_pRowDefinitionsProperty = CXTPMarkupDependencyProperty::Register(
		L"RowDefinitions", MARKUP_TYPE(CXTPMarkupRowDefinitionCollection),
		MARKUP_TYPE(CXTPMarkupGrid));

	m_pColumnProperty = CXTPMarkupDependencyProperty::RegisterAttached(
		L"Column", MARKUP_TYPE(CXTPMarkupInt), MARKUP_TYPE(CXTPMarkupGrid),
		new CXTPMarkupPropertyMetadata(NULL, &CXTPMarkupGrid::OnCellAttachedPropertyChanged));

	m_pRowProperty = CXTPMarkupDependencyProperty::RegisterAttached(
		L"Row", MARKUP_TYPE(CXTPMarkupInt), MARKUP_TYPE(CXTPMarkupGrid),
		new CXTPMarkupPropertyMetadata(NULL, &CXTPMarkupGrid::OnCellAttachedPropertyChanged));

	m_pColumnSpanProperty = CXTPMarkupDependencyProperty::RegisterAttached(
		L"ColumnSpan", MARKUP_TYPE(CXTPMarkupInt), MARKUP_TYPE(CXTPMarkupGrid),
		new CXTPMarkupPropertyMetadata(NULL, &CXTPMarkupGrid::OnCellAttachedPropertyChanged));

	m_pRowSpanProperty = CXTPMarkupDependencyProperty::RegisterAttached(
		L"RowSpan", MARKUP_TYPE(CXTPMarkupInt), MARKUP_TYPE(CXTPMarkupGrid),
		new CXTPMarkupPropertyMetadata(NULL, &CXTPMarkupGrid::OnCellAttachedPropertyChanged));
}
...
void CXTPMarkupGrid::OnCellAttachedPropertyChanged(CXTPMarkupObject* d,
...
{
	CXTPMarkupInt* pColumn = MARKUP_STATICCAST(CXTPMarkupInt,
											   pElement->GetValue(m_pColumnProperty));
...
{
	CXTPMarkupInt* pColumnSpan = MARKUP_STATICCAST(CXTPMarkupInt,
												   pElement->GetValue(m_pColumnSpanProperty));
	if (!pColumnSpan)
		return 1;
...
{
	CXTPMarkupInt* pRowSpan = MARKUP_STATICCAST(CXTPMarkupInt,
												pElement->GetValue(m_pRowSpanProperty));
	if (!pRowSpan)
		return 1;
	...
	if (pProperty == m_pColumnDefinitionsProperty)
	{
		if (!pValue || !pValue->IsKindOf(MARKUP_TYPE(CXTPMarkupColumnDefinitionCollection)))
		{
			pBuilder->ThrowBuilderException(
				_T("CXTPMarkupColumnDefinitionCollection class expected"));
		}
...
void CXTPMarkupGrid::ValidateDefinitionsLayout(CXTPMarkupDefinitionCollection* pDefinitions,
											   BOOL bTreatStarAsAuto)
{
	for (int i = 0; i < pDefinitions->GetCount(); i++)
	{
		...
		{
			case CXTPMarkupGridLength::unitTypeAuto: pDefinition->m_nSizeType = sizeTypeAuto; break;
...
int CXTPMarkupGrid::GetLengthTypeForRange(CXTPMarkupDefinitionCollection* pDefinitions, int start,
										  int count)
{
...
void CXTPMarkupGrid::ValidateCells()
{
	...
	m_pCellCachesCollection = new CELLCACHE[XTPToUIntChecked(GetChildren()->GetCount())];
...
int CXTPMarkupGrid::GetMeasureSizeForRange(CXTPMarkupDefinitionCollection* pDefinitions, int start,
										   int count)
{
	double num = 0;
	for (int nIndex = start; nIndex < start + count; nIndex++)
	{
		...
		num += pDefinition->m_nSizeType == sizeTypeAuto ? pDefinition->m_nMinSize
														: pDefinition->m_nMeasureSize;
	}
...
void CXTPMarkupGrid::MeasureCell(CXTPMarkupDrawingContext* pDC, int cell, BOOL forceInfinityV)
{
	...
	{
		nPositiveInfinity = GetMeasureSizeForRange(m_pDefinitionsU,
												   m_pCellCachesCollection[cell].nColumnIndex,
												   m_pCellCachesCollection[cell].nColumnSpan);
	}
	...
	{
		num2 = GetMeasureSizeForRange(m_pDefinitionsV, m_pCellCachesCollection[cell].nRowIndex,
									  m_pCellCachesCollection[cell].nRowSpan);
	}
...
void CXTPMarkupGrid::MeasureCellsGroup(CXTPMarkupDrawingContext* pDC, int cellsHead,
									   CSize /*referenceSize*/, BOOL ignoreDesiredSizeU,
									   BOOL forceInfinityV)
{
	...
	if (cellsHead < nCount)
	{
		...
		do
		{
			...
			if (!ignoreDesiredSizeU)
			{
				...
				{
					m_pDefinitionsU->GetItem(cellCache.nColumnIndex)
						->UpdateMinSize(GetChildren()->GetItem(cell)->GetDesiredSize().cx);
				}
				else
				{
					SPANKEY sk;
			...
			if (!forceInfinityV)
			{
				...
				{
					m_pDefinitionsV->GetItem(cellCache.nRowIndex)
						->UpdateMinSize(GetChildren()->GetItem(cell)->GetDesiredSize().cy);
				}
				else
				{
					SPANKEY sk;
	...
	for (int i = 0; i < (int)arrSpans.GetSize(); i++)
	{
		...
		EnsureMinSizeInDefinitionRange(key.bU ? m_pDefinitionsU : m_pDefinitionsV, key.nStart,
									   key.nCount, key.nValue);
	}
}

void CXTPMarkupGrid::EnsureMinSizeInDefinitionRange(CXTPMarkupDefinitionCollection* pDefinitions,
													int start, int count, int requestedSize)
{
	if (requestedSize == 0)
		return;

	CXTPMarkupDefinitionBase** pTempDefinitions =
		new CXTPMarkupDefinitionBase*[XTPToUIntChecked(pDefinitions->GetCount())];
	...
	if (requestedSize > dTotalMinSize)
	{
		...
		{
			qsort(pTempDefinitions, XTPToSizeTChecked(count), sizeof(CXTPMarkupDefinitionBase*),
		...
		else
		{
			...
			if (dAverage < dMaxSize)
			{
				...
				for (int j = 0; j < count; j++)
				{
					...
					pTempDefinitions[j]->UpdateMinSize(
						int(pTempDefinitions[j]->m_nSizeCache + num24));
				}
			}
			else
			{
...
void CXTPMarkupGrid::ResolveStar(CXTPMarkupDefinitionCollection* pDefinitions, double availableSize)
{
	...
	for (int i = 0; i < pDefinitions->GetCount(); i++)
	{
		...
		{
			case sizeTypePixel: num2 += pDefinition->m_nMeasureSize; continue;

			case sizeTypeAuto: num2 += pDefinition->m_nMinSize; continue;
	...
	if (length > 0)
	{
		qsort(pTempDefinitions, XTPToSizeTChecked(length), sizeof(CXTPMarkupDefinitionBase*),
...
int AFX_CDECL CXTPMarkupGrid::SpanPreferredDistributionOrderComparer(const void* arg1,
																	 const void* arg2)
{
	CXTPMarkupDefinitionBase* base2 = *((CXTPMarkupDefinitionBase**)arg1);
	CXTPMarkupDefinitionBase* base3 = *((CXTPMarkupDefinitionBase**)arg2);
...
void CXTPMarkupGrid::SetFinalSize(CXTPMarkupDefinitionCollection* pDefinitions, int finalSize)
{
	...
	{
		pDefinitions->GetItem(j)->m_nFinalOffset = pDefinitions->GetItem(j - 1)->m_nFinalOffset
												   + (int)pDefinitions->GetItem(j - 1)->m_nSizeCache;
	}

	delete[] pTempDefinitions;
}

int CXTPMarkupGrid::GetFinalSizeForRange(CXTPMarkupDefinitionCollection* pDefinitions, int start,
										 int count)
{
	int num = 0;
	for (int nIndex = start; nIndex < start + count; nIndex++)
...
BEGIN_DISPATCH_MAP(CXTPMarkupGrid, CXTPMarkupPanel)
	...
	DISP_FUNCTION_ID(CXTPMarkupGrid, "SetColumnSpan", 601, OleSetColumnSpan, VT_EMPTY,
					 VTS_DISPATCH VTS_I4)
	DISP_FUNCTION_ID(CXTPMarkupGrid, "SetRow", 602, OleSetRow, VT_EMPTY, VTS_DISPATCH VTS_I4)
	DISP_FUNCTION_ID(CXTPMarkupGrid, "SetRowSpan", 603, OleSetRowSpan, VT_EMPTY,
					 VTS_DISPATCH VTS_I4)
	...
	DISP_PROPERTY_EX_ID(CXTPMarkupGrid, "ColumnDefinitions", 608, OleGetColumnDefinitions,
						SetNotSupported, VT_DISPATCH)
	DISP_PROPERTY_EX_ID(CXTPMarkupGrid, "RowDefinitions", 609, OleGetRowDefinitions,
						SetNotSupported, VT_DISPATCH)
...
int CXTPMarkupGrid::OleGetColumnSpan(LPDISPATCH lpElementDisp)
{
	return GetColumnSpan(
		MARKUP_STATICCAST(CXTPMarkupUIElement, FromDispatch(lpElementDisp, FALSE)));
}

void CXTPMarkupGrid::OleSetColumnSpan(LPDISPATCH lpElementDisp, int nValue)
{
	SetColumnSpan(MARKUP_STATICCAST(CXTPMarkupUIElement, FromDispatch(lpElementDisp, FALSE)),
				  nValue);
}
