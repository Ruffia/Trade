...
CSize CXTPMarkupTextBlock::ArrangeOverride(CSize szFinalSize)
{
	...
	for (CLine* pLine = m_pLineList->GetHead(); pLine != NULL; pLine = pLine->m_pNextChain)
	{
		...
		int xOffset = textAlignment == xtpMarkupTextAlignmentRight
						  ? nJustifyWidth
...
void CXTPMarkupTextBlock::RenderTextDecorations(CXTPMarkupDrawingContext* pDC, CLinePart* part)
{
	...
	{
		int nLineTop   = CXTPMathUtils::Round(rc.GetBottom()) - part->nBaseline + nLineHeight;
		nTopDecoration = nLineTop + ((CXTPMathUtils::Round(rc.GetBottom()) - nLineTop) % 2);
	}
	else if (pTextDecorations->GetValue() == 2) // Strikethrough
	{
		nTopDecoration = CXTPMathUtils::Round(rc.GetBottom()) - part->nBaseline
	...
	CRect rcDecoration(CXTPMathUtils::Round(rcDecorationF.GetLeft()),
					   CXTPMathUtils::Round(rcDecorationF.GetTop()),
					   CXTPMathUtils::Round(rcDecorationF.GetRight()),
					   CXTPMathUtils::Round(rcDecorationF.GetBottom()));
...
void CXTPMarkupTextBlock::OnRender(CXTPMarkupDrawingContext* pDC)
{
	...
	for (CLine* pLine = m_pLineList->GetHead(); pLine != NULL; pLine = pLine->m_pNextChain)
	{
		for (CLinePart* part = pLine->m_arrParts.GetHead(); part != NULL; part = part->m_pNextChain)
		{
			...
			CRect rc(CXTPMathUtils::Round(rcF.GetLeft()), CXTPMathUtils::Round(rcF.GetTop()),
					 CXTPMathUtils::Round(rcF.GetRight()), CXTPMathUtils::Round(rcF.GetBottom()));
