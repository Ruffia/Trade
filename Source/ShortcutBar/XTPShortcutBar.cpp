...
#include "ShortcutBar/Themes/XTPShortcutBarVisualStudio2017Theme.h"
#include "ShortcutBar/Themes/XTPShortcutBarVisualStudio2019Theme.h"
#include "ShortcutBar/Themes/XTPShortcutBarVisualStudio2022Theme.h"
...
{
	int nIconId = GetIconId();
	if (nIconId < 0)
	{
		return NULL;
	}

	return m_pShortcutBar->GetImageManager()->GetImage(static_cast<UINT>(nIconId), nWidth);
}
...
{
	CXTPShortcutBarItem* pItem = new CXTPShortcutBarItem(this, IDR_NAVIGATION_BUTTON, 0);
	pItem->m_bNavigateButton   = TRUE;

	CXTPResourceManager::AssertValid(
		XTPResourceManager()->LoadString(&pItem->m_strCaption, XTP_IDS_SHORTCUT_NAVIGATIONPANE));
...
CXTPShortcutBarItem* CXTPShortcutBar::CreateConfigureButton()
{
	...
	CXTPResourceManager::AssertValid(
		XTPResourceManager()->LoadString(&pItem->m_strCaption, XTP_IDS_SHORTCUT_CONFIGURE));
...
void CXTPShortcutBar::SetTheme(XTPShortcutBarPaintTheme paintTheme)
{
	...
	switch (paintTheme)
	{
		...
		case xtpShortcutThemeOfficeXP: m_pPaintManager = new CXTPShortcutBarOfficeXPTheme(); break;

		case xtpShortcutThemeOffice2003:
			m_pPaintManager = new CXTPShortcutBarOffice2003Theme();
			break;

		case xtpShortcutThemeOffice2007 | xtpShortcutThemeOffice2010:
		...
		case xtpShortcutThemeVisualStudio2015:
			...
			break;
		case xtpShortcutThemeVisualStudio2017:
			m_pPaintManager = new CXTPShortcutBarVisualStudio2017Theme();
			break;
		case xtpShortcutThemeVisualStudio2019:
			m_pPaintManager = new CXTPShortcutBarVisualStudio2019Theme();
			break;
		case xtpShortcutThemeVisualStudio2022:
			m_pPaintManager = new CXTPShortcutBarVisualStudio2022Theme();
			break;

		case xtpShortcutThemeNativeWindows10:
			m_pPaintManager = new CXTPShortcutBarNativeWindows10Theme();
			break;
	}
...
{
	CXTPShortcutBarItem* pItem = new CXTPShortcutBarItem(this, XTPToInt(nID), pWnd);
...
void CXTPShortcutBar::Reposition()
{
	...
	else
	{
		...
		CXTPShortcutBarPane* pPaneClient = DYNAMIC_DOWNCAST(CXTPShortcutBarPane,
															CWnd::FromHandle(m_hwndClient));
		if (pPaneClient)
			nMinClientHeight = max(nMinClientHeight, pPaneClient->GetMinimumClientHeight());
		...
		else
		{
			...
			if (nHeight - nTopItemHeight - m_nExpandedLines * m_szItem.cy - nGripperHeight
				< nMinClientHeight)
			{
				m_nExpandedLines = (nHeight - nTopItemHeight - nGripperHeight - nMinClientHeight)
								   / m_szItem.cy;
			}
	...
	for (int nCollapsedItem = nExpandedItem; nCollapsedItem < m_arrItems.GetSize();
		 nCollapsedItem++)
	{
		...
		if (pItem->IsItemExpandButton())
			pItem->m_rcItem.SetRect(nWidth - m_szItem.cx, rcShortcutBar.Height() - m_szItem.cy,
									nWidth, rcShortcutBar.Height());
		else if ((nCollapsedCount > 0) || ((nCollapsedCount == 0) && !bExpandButtonVisible))
			pItem->m_rcItem.SetRect(nCollapsedPos, rcShortcutBar.Height() - m_szItem.cy,
									nCollapsedPos + m_szItem.cx, rcShortcutBar.Height());
		else
	...
	{
		::MoveWindow(m_hwndClient, m_rcClient.left, m_rcClient.top, m_rcClient.Width(),
					 m_rcClient.Height(), TRUE);
	}
	...
	if (m_bMinimized)
	{
		...
		if (GetPaintManager()->m_bNavigationButton)
			m_pNavigationPaneItem->m_rcItem.bottom = m_rcClient.Width();
...
INT_PTR CXTPShortcutBar::OnToolHitTest(CPoint point, TOOLINFO* pTI) const
{
	...
	if (bShowToolTip)
	{
		...
		CXTPToolTipContext::FillInToolInfo(pTI, m_hWnd, pItem->GetItemRect(), nHit, strTip,
...
CXTPShortcutBarItem* CXTPShortcutBar::HitTest(CPoint pt) const
{
	...
	for (int i = 0; i < m_arrItems.GetSize(); i++)
	{
		...
		if (pItem->m_bVisible && pItem->IsEnabled() && !pItem->m_bHidden
			&& pItem->m_rcItem.PtInRect(pt))
			return m_arrItems[i];
	}
	return NULL;
}
...
void CXTPShortcutBar::SelectItem(CXTPShortcutBarItem* pItem)
{
	...
	if (GetOwner()->SendMessage(XTPWM_SHORTCUTBAR_NOTIFY, XTP_SBN_SELECTION_CHANGING, (LPARAM)pItem)
		== -1)
		return;
...
void CXTPShortcutBar::OnExpandButtonDown(CXTPShortcutBarItem* pExpandButton)
{
	...
	CXTPControlPopup* pControlAddOrRemove = (CXTPControlPopup*)pPopup->GetControls()->Add(
	...
	for (int j = 0; j < m_arrItems.GetSize() - 1; j++)
	{
		...
		CXTPControl* pControl = pControlAddOrRemove->GetCommandBar()->GetControls()->Add(
			xtpControlButton, XTP_ID_SHORTCUT_CUSTOMIZE);
	...
	for (int i = 0; i < m_arrItems.GetSize(); i++)
	{
		...
		{
			CXTPControl* pControl = pPopup->GetControls()->Add(xtpControlButton,
															   XTP_ID_SHORTCUT_SELECTED);
			pControl->SetIconId(pItem->GetID());
			pControl->SetCaption(pItem->GetCaption());
			pControl->SetTag((DWORD_PTR)pItem);
...
{
	m_nDesiredExpandedLinesHeight = XTPToIntChecked(
		AfxGetApp()->GetProfileInt(lpszProfileName, _T("DesiredExpandedLinesHeight"),
								   m_nDesiredExpandedLinesHeight));
	...
	if (AfxGetApp()->GetProfileBinary(lpszProfileName, _T("Hidden"), &lpbData, &dwDataSize))
	{
		try
		{
			...
			{
				CXTPShortcutBarItem* pItem = FindItem(XTPToIntChecked(arrHiddenItems[i]));
				if (pItem && !pItem->IsItemExpandButton())
...
{
	AfxGetApp()->WriteProfileInt(lpszProfileName, _T("DesiredExpandedLinesHeight"),
								 m_nDesiredExpandedLinesHeight);
	...
	for (int i = 0; i < m_arrItems.GetSize(); i++)
	{
		if (!m_arrItems[i]->IsVisible() && !m_arrItems[i]->IsItemExpandButton())
			arrHiddenItems.Add(XTPToDWORD(m_arrItems[i]->GetID()));
	}
...
BEGIN_DISPATCH_MAP(CXTPShortcutBarItem, CXTPCmdTarget)
	DISP_PROPERTY_EX_ID(CXTPShortcutBarItem, "Caption", dispidCaption, OleGetCaption, SetCaption,
						VT_BSTR)
	DISP_PROPERTY_EX_ID(CXTPShortcutBarItem, "Tooltip", dispidTooltip, OleGetTooltip, SetTooltip,
						VT_BSTR)

	DISP_PROPERTY_EX_ID(CXTPShortcutBarItem, "Id", dispidId, GetID, SetID, VT_I4)
	DISP_PROPERTY_EX_ID(CXTPShortcutBarItem, "Visible", dispidVisible, IsVisible, SetVisible,
						VT_BOOL)
	DISP_PROPERTY_EX_ID(CXTPShortcutBarItem, "IconHandle", dispidIconHandle, OleGetIconHandle,
						OleSetIconHandle, VT_I4)
...
{
	m_pShortcutBar->GetImageManager()->SetIcon(reinterpret_cast<HICON>((INT_PTR)nIcon), m_nID);
	m_pShortcutBar->Reposition();
}
...
{
	return (OLE_HANDLE) reinterpret_cast<LONG_PTR>(m_hwndChild);
}
...
void CXTPShortcutBarItem::OleSetHandle(OLE_HANDLE hWnd)
{
	...
	m_hwndChild = reinterpret_cast<HWND>((LONG_PTR)hWnd);
