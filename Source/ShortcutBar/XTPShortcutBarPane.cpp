...
void CXTPShortcutBarPane::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	if (m_pHighlighted && m_pHighlighted->IsExpandable())
	{
		...
		while (::GetCapture() == m_hWnd)
		{
			...
			{
				point						   = CPoint(LOWORD(msg.lParam), HIWORD(msg.lParam));
				CXTPShortcutBarPaneItem* pItem = pHighlighted->GetCaptionRect().PtInRect(point)
													 ? pHighlighted
													 : NULL;
...
void CXTPShortcutBarPane::RecalcLayout()
{
	...
	for (int i = 0; i < m_arrItems.GetSize(); i++)
	{
		...
		if (pItem->IsExpanded())
		{
			CRect rcClient(0 + m_rcIndent.left, m_rcIndent.top + nClientTop,
						   rc.right - m_rcIndent.right,
						   m_rcIndent.top + nClientTop + pItem->m_nHeight);
			...
			{
				pItem->m_pWndClient->SetWindowPos(0, rcClient.left, rcClient.top, rcClient.Width(),
												  rcClient.Height(), SWP_NOZORDER | SWP_SHOWWINDOW);
			}
		}
		else
		{
			...
			if (pItem->m_pWndClient)
			{
				pItem->m_pWndClient->SetWindowPos(0, 0, 0, 0, 0,
	...
	if (m_pShortcutBar->IsMinimizeButtonVisible() && (nPaneCaptionHeight > 0 || m_bFlatStyle))
	{
		...
		m_rcMinimizeButton = CRect(CPoint(rc.right - sz.cx - XTP_DPI_X(4),
										  max(0, (nPaneCaptionHeight - sz.cy) / 2)),
								   sz);
	}
	m_bMinimizeButtonHighlighted = FALSE;
...
{
	return m_bFlatStyle ? GetShortcutBar()->GetPaintManager()->m_clrFlatBackground
						: GetXtremeColor(COLOR_WINDOW);
}

COLORREF CXTPShortcutBarPane::GetTextColor() const
{
	return m_bFlatStyle ? GetShortcutBar()->GetPaintManager()->m_clrFlatTextColor
