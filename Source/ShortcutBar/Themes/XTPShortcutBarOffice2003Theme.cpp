...
void CXTPShortcutBarOffice2003Theme::RefreshMetrics()
{
	...
	m_grcItemCaption.SetStandardValue(XTPColorManager()->grcDockBar.clrDark,
									  XTPColorManager()->grcDockBar.clrLight);
	...
	{
		m_grcShortcutItem.SetStandardValue(GetXtremeColor(COLOR_3DFACE));
		m_grcShortcutBarGripper.SetStandardValue(GetXtremeColor(COLOR_3DFACE),
...
int CXTPShortcutBarOffice2003Theme::DrawPaneCaption(CDC* pDC, CXTPShortcutBarPane* pPane,
													BOOL bDraw)
{
	if (bDraw)
	{
...
int CXTPShortcutBarOffice2003Theme::DrawPaneItemCaption(CDC* pDC, CXTPShortcutBarPaneItem* pItem,
														BOOL bDraw)
{
	if (bDraw)
	{
...
void CXTPShortcutBarOffice2003Theme::FillShortcutBar(CDC* pDC, CXTPShortcutBar* pShortcutBar)
{
	...
	else
	{
		XTPDrawHelpers()->GradientFill(
			pDC,
			CRect(0, (pShortcutBar->GetExpandedLinesCount() - 1) * nItemHeight - 1, rc.Width(),
				  rc.Height()),
			m_grcShortcutItem, FALSE);
	}
}
...
void CXTPShortcutBarOffice2003Theme::DrawImage(CDC* pDC, CPoint pt, CSize sz,
											   ...
											   BOOL bPressed, BOOL bChecked, BOOL bEnabled)
{
	...
	{
		pImage->Draw(pDC, pt,
					 bSelected || bPressed ? xtpImageHot
