...
void CXTPShortcutBarResourceTheme::RefreshMetrics()
{
	...
	m_clrSelectedText.SetStandardValue(
		pImages->GetImageColor(_T("ShortcutBar"), _T("SelectedText")));

	m_clrShortcutItemShadow.SetStandardValue(
		pImages->GetImageColor(_T("ShortcutBar"), _T("FrameBorder")));
	...
	m_grcShortcutBarGripper.SetStandardValue(pImages->GetImageColor(_T("ShortcutBar"),
																	_T("GripperLight")),
											 pImages->GetImageColor(_T("ShortcutBar"),
																	_T("GripperDark")));

	m_grcItemCaption.SetStandardValue(pImages->GetImageColor(_T("ShortcutBar"),
															 _T("ItemNormalCaptionLight")),
									  pImages->GetImageColor(_T("ShortcutBar"),
															 _T("ItemNormalCaptionDark")));

	m_grcItemHotCaption.SetStandardValue(pImages->GetImageColor(_T("ShortcutBar"),
																_T("ItemHotCaptionLight")),
										 pImages->GetImageColor(_T("ShortcutBar"),
																_T("ItemHotCaptionDark")));

	m_clrItemCaptionText.SetStandardValue(
		pImages->GetImageColor(_T("ShortcutBar"), _T("ItemCaptionText")));

	m_clrItemCaptionTop.SetStandardValue(
		pImages->GetImageColor(_T("ShortcutBar"), _T("ItemCaptionTop")));
	m_clrItemCaptionShadow.SetStandardValue(
		pImages->GetImageColor(_T("ShortcutBar"), _T("ItemCaptionShadow")));
	...
	m_clrPopupBackground.SetStandardValue(
		pImages->GetImageColor(_T("ShortcutBar"), _T("CaptionLight")));
	...
	m_clrAppSplitterFace = pImages->GetImageColor(_T("Window"), _T("SplitterBackground"),
												  m_clrAppSplitterFace);
...
void CXTPShortcutBarResourceTheme::DrawPaneCaptionMinimizeButton(CDC* pDC,
																 CXTPShortcutBarPane* pPane)
{
	...
	if (!rcMinimizeButton.IsRectEmpty())
	{
		...
		{
			int nState = pPane->IsMinimizeButtonPressed() && pPane->IsMinimizeButtonHighlighted()
							 ? 3
							 : 1;
		...
		CRect rcSrc(
			pGlyphs->GetSource(pPane->GetShortcutBar()->IsShortcutBarMinimized() ? 0 : 1, 5));

		CRect rcDest(
			CPoint((rcMinimizeButton.left + rcMinimizeButton.right - XTP_DPI_X(rcSrc.Width())) / 2,
				   (rcMinimizeButton.top + rcMinimizeButton.bottom - XTP_DPI_Y(rcSrc.Height()))
					   / 2),
			XTP_DPI(rcSrc.Size()));
...
int CXTPShortcutBarResourceTheme::DrawPaneItemCaption(CDC* pDC, CXTPShortcutBarPaneItem* pItem,
													  BOOL bDraw)
{
	if (bDraw)
	{
		...
		COLORREF clrLight = pItem->IsHighlighted() ? m_grcItemHotCaption.clrLight
												   : m_grcItemCaption.clrLight;
		COLORREF clrDark = pItem->IsHighlighted() ? m_grcItemHotCaption.clrDark
												  : m_grcItemCaption.clrDark;

		XTPDrawHelpers()->GradientFill(pDC, CRect(rc.left, rc.top, rc.CenterPoint().x, rc.bottom),
									   clrDark, clrLight, TRUE);
		XTPDrawHelpers()->GradientFill(pDC, CRect(rc.CenterPoint().x, rc.top, rc.right, rc.bottom),
									   clrLight, clrDark, TRUE);
		...
		if (pItem->IsExpandable())
		{
			...
			{
				CRect rcDest(CPoint(rc.left + 3, (rc.top + rc.bottom + 2 - rcSrc.Height()) / 2),
							 rcSrc.Size());
				pImage->DrawImage(pDC, rcDest, rcSrc);
...
void CXTPShortcutBarResourceTheme::FillShortcutBar(CDC* pDC, CXTPShortcutBar* pShortcutBar)
{
	...
	else
	{
		...
		if (pShortcutBar->IsClientPaneVisible())
		{
			...
			pImage->DrawImage(pDC, CRect(0, nTopExpandedLines, rc.Width(), rc.bottom),
							  pImage->GetSource(0, 4), CRect(2, 2, 2, 2));
		}
		else
		{
			pImage->DrawImage(pDC,
							  CRect(0,
									(pShortcutBar->GetExpandedLinesCount() - 1) * nItemHeight - 1,
									rc.Width(), rc.Height()),
							  pImage->GetSource(0, 4), CRect(2, 2, 2, 2));
		}
	}
	...
	else
	{
		if (pItem->IsSelected()
			&& (pShortcutBar->GetHotItem() != pItem
				|| (pShortcutBar->GetPressedItem() != NULL
	...
	if (bDraw)
	{
		...
		if (m_bFlatStyle)
		{
			XTPDrawHelpers()->GradientFill(pDC,
										   CRect(rc.left + 1, rc.top + 1, rc.right - 1,
												 rc.bottom - 1),
										   m_grcShortcutBarGripper, FALSE);
			pDC->Draw3dRect(rc.left + 1, rc.top, rc.Width() - 2, rc.Height(),
							m_clrShortcutItemShadow, m_clrShortcutItemShadow);
		}
		else
		{
			...
			CRect rcDest(CPoint((rc.left + rc.right - rcSrc.Width()) / 2,
								(rc.top + rc.bottom - rcSrc.Height()) / 2),
						 rcSrc.Size());
