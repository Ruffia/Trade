...
//     of a CXTPCalendarMAPIDataProvider object. Then use the Open() member function
//     to establish a connection to the data source and initialize the object.
//     This class provides a number of functions to manipulate events and events
//     related data. After finishing work with the data source, the Close()
//     member function must be called to close the connection and to de-initialize
//     the object and free any unused resources.
//
//     MAPI data provider automatically opens the default message store and
//     default user's calendar folder and reads all appointments there. MAPI data
//     provider uses Memory Data Provider internally as a private cache.
//     Also it uses a MAPI notifications mechanism in order to have MAPI
//     appointments always updated. When an event is updated in MAPI storage,
...
public:
	...
	//     This method firstly initializes MAPI, logs in with the specified
	//     user's credentials and then reads all appointments.
	// Returns:
	...
	//-----------------------------------------------------------------------
	// Summary:
	//     For MAPI Data Provider this method does the same sequence of
	//     actions as the Open method.
	// Returns:
	//     TRUE is successful. FALSE otherwise.
	// See also:
	//     Open
...
protected:
	//-----------------------------------------------------------------------
	// Summary:
	//     Retrieve day events from the memory cache.
	// Parameters:
	//     dtDay - A specified day.
	// Remarks:
	//     As long as we are getting the cache synchronized with the MAPI storage,
	//     this method works from the memory cache.
	// Returns:
	...
	// Remarks:
	//     This method works with the memory cache only and removes all data there.
	//-----------------------------------------------------------------------
	virtual void DoRemoveAllEvents();

	//-----------------------------------------------------------------------
	// Summary:
	//     Reads an event from the MAPI storage.
	// Parameters:
	...
	//-----------------------------------------------------------------------
	// Summary:
	//     Reads a recurrence pattern from the MAPI storage.
	// Parameters:
	...
	// Remarks:
	//     As long as we are getting the cache synchronized with the MAPI storage,
	//     this method actually works from the memory cache.
...
protected:
	//-----------------------------------------------------------------------
	// Summary:
	//      This member function is used to retrieve parameters for the MAPI session
	//      login. The default implementation tries to read them from the connection
	//      string or return values for logging in with default credentials.
	// Parameters:
	...
	//-----------------------------------------------------------------------
	// Summary:
	//      This method is called by the calendar control framework when the Time Zone is Changed.
	// Remarks:
	//      The data provider implementation may use this method to adjust its data for
	//      a new timezone.
	// Returns:
	//      TRUE if internal data has changed and the calendar must be re-populated,
	//      FALSE if this event is ignored (no data changed).
...
protected:
	...
	//=======================================================================
	// Summary:
	//     Internal extension of the CXTPCalendarMemoryDataProvider
