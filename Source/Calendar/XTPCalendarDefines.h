...
enum XTPCalendarTheme
{
	...
	xtpCalendarThemeNativeWindows10  = 6,						   // Windows 10 theme.
	xtpCalendarThemeVisualStudio2017 = 7,						   // VisualStudio 2017 theme.
	xtpCalendarThemeVisualStudio2019 = 8,						   // VisualStudio 2019 theme.
	xtpCalendarThemeVisualStudio2022 = 9,						   // VisualStudio 2022 theme.
...
enum XTPCalendarThemeVersion
{
	...
	xtpCalendarVersion10,
	xtpCalendarVersion2017,
	xtpCalendarVersion2019,
	xtpCalendarVersion2022,
};
...
#	define XTP_CALENDAR_DISABLED_COLOR_DARK                                                       \
		RGB(128, 128, 128) //<COMBINE XTP_CALENDAR_DISABLED_COLOR_LIGHT>
...
#	define XTP_CALENDAR_SHOW_BUTTON_TIMEOUT_MS                                                    \
		1400 //<COMBINE XTP_CALENDAR_SHOW_TOOLTIP_TIMEOUT_MS>
...
// </code>
// See Also:
//     CXTPCalendarEvent::SetBusyStatus
//===========================================================================
...
// </code>
// See Also:
//     CXTPCalendarEvent::SetImportance
//===========================================================================
...
enum XTPCalendarPatternEnd
{
	...
	xtpCalendarPatternEndAfterOccurrences, // End of the recurrence pattern specified by the
										   // occurrences amount.
...
enum XTPCalendarWeekDay
{
	...
	xtpCalendarDayAllWeek = xtpCalendarDayMonday | xtpCalendarDayTuesday | xtpCalendarDayWednesday
							| xtpCalendarDayThursday | xtpCalendarDayFriday | xtpCalendarDaySaturday
...
enum XTPCalendarDataProvider
{
	...
	xtpCalendarDataProviderDatabase = 2, // Database data provider -
										 // CXTPCalendarDatabaseDataProvider.
...
{
	xtpCalendarMouseNothing,		 // User is doing nothing - just watching the control.
	xtpCalendarMouseEventPreResizeV, // User is moving mouse under event view vertical "resize"
									 // area.
	xtpCalendarMouseEventPreResizeH, // User is moving mouse under event view horizontal "resize"
									 // area.
	xtpCalendarMouseEventPreDrag,	// User is moving mouse under event view "drag" area.
	xtpCalendarMouseEventResizingV,  // User is resizing event vertically (holding mouse left button
									 // down).
	xtpCalendarMouseEventResizingH,  // User is resizing event horizontally (holding mouse left
									 // button down).
	xtpCalendarMouseEventDragCopy, // User is dragging event view (holding left mouse button down).
	xtpCalendarMouseEventDragMove, // User is dragging event view (holding left mouse button down
...
{
	xtpCalendarViewChangedSend = 1, // Set send request for XTP_NC_CALENDARVIEWWASCHANGED
									// notification and lock sending.
	xtpCalendarViewChangedLock = 2  // Lock sending only, send request for
									// XTP_NC_CALENDARVIEWWASCHANGED notification is not set.
};
...
enum XTPCalendarHitTestCode
{
	...
	xtpCalendarHitTestGroupArea = xtpCalendarHitTestDayArea, // The position is inside a group area
															 // (formerly day).
	xtpCalendarHitTestGroupHeader = 0x000008, // The position is inside Group (resource) header.
	xtpCalendarHitTestDay_Mask	= 0x00000F, // The mask for any position inside days area with the
											  // header.
	...
	xtpCalendarHitTestEventResizeArea_Mask =
		xtpCalendarHitTestEventResizeVArea
		| xtpCalendarHitTestEventResizeHArea, // The mask for both event resize areas.
	...
	xtpCalendarHitTestDayViewScrollUp = 0x010000,   // DayView Scroll Up button (presents in office
													// 2007 theme only).
	xtpCalendarHitTestDayViewScrollDown = 0x020000, // DayView Scroll Down button (presents in
													// office 2007 theme only).
	...
	xtpCalendarHitTestPrevEventButton = 0x100000, // Go to Previous event button (presents in
												  // office 2007 theme only).
	xtpCalendarHitTestNextEventButton = 0x200000, // Go to Next event button (presents in office
												  // 2007 theme only).
	xtpCalendarHitTestTimeLineScale = 0x400000,   // Calendar HitTest TimeLineScale

	xtpCalendarHitTestAllDayExpandButton =
		xtpCalendarHitTestDayExpandButton
		| xtpCalendarHitTestDayViewAllDayEvent, // The position is over Expand button icon in allday
												// header.
};
...
{
	xtpCalendarCopyFlagROccurrence2RException = 0x0001, // Convert recurrence Occurrence to
														// Exception before paste.
	xtpCalendarCopyFlagKeepRException = 0x0002,	// Keep recurrence Exception event state before
												   // paste.
	...
	xtpCalendarCopyFlagCutMask = xtpCalendarCopyFlagROccurrence2RException
								 ...
								 | xtpCalendarCopyFlagClearCBAfterPaste // Default flags for Cut
																		// operation.
...
enum XTPCalendarCurrentTimeMarkFlags
{
	...
	xtpCalendarCurrentTimeMarkPrinted = 4 // If this flag set - Time Mark is visible for printed
										  // calendar also, otherwise it is hidden when printing.
};
...
public:
	...
	//-----------------------------------------------------------------------
	// Summary:
	//     Determines if the values of date time object members are valid.
	// Returns:
	...
	BOOL IsValid() const
	{
		return dtBegin.GetStatus() == COleDateTime::valid
			   && dtEnd.GetStatus() == COleDateTime::valid && nGroupIndex >= 0;
	}
...
struct XTP_CALENDAR_MONTHVIEW_DAYPOS
{
	...
	*/
	...
	// See Also:
	//     operator=(int nPOS), SetPOS(int nPOS)
	//-----------------------------------------------------------------------
	...
	// See Also:
	//     operator=(int nPOS), SetPOS(int nWidx, int nWDidx)
	//-----------------------------------------------------------------------
...
// See Also:
//     XTP_CALENDAR_HITTESTINFO, XTP_CALENDAR_HITTESTINFO,
...
//      Microsoft KB221542
//===========================================================================
#	define XTP_CALENDAR_TIMEZONESKEY_NT                                                           \
...
//      Microsoft KB221542
//===========================================================================
#	define XTP_CALENDAR_TIMEZONESKEY_9X                                                           \
...
static const LPCTSTR cszEventCustProp_NextReminderTime_Snoozed =
	_T("*xtp*NextReminderTime_Snoozed"); // String name of the corresponding property.
static const LPCTSTR cszEventCustProp_IconsIDs = _T("*xtp*CustomIcons"); // String name of the
																		 // corresponding property.
static const LPCTSTR cszEventCustProp_CategoryIDs =
	_T("*xtp*Categories"); // String name of the corresponding property.

// Public custom properties
static const LPCTSTR cszEventCustProp_MarkupTemplate =
	_T("xtpMarkupTemplate"); // String name of the MarkupTemplate property.

// Markup template params
static const LPCTSTR cszCalendarMarkupPrm_Subject = _T("/Calendar/Events/Event/Subject"); // Subject.
static const LPCTSTR cszCalendarMarkupPrm_Location =
	_T("/Calendar/Events/Event/Location");											// Location.
static const LPCTSTR cszCalendarMarkupPrm_Body = _T("/Calendar/Events/Event/Body"); // Body.
static const LPCTSTR cszCalendarMarkupPrm_StartTime =
	_T("/Calendar/Events/Event/StartTime"); // Start time.
static const LPCTSTR cszCalendarMarkupPrm_EndTime = _T("/Calendar/Events/Event/EndTime"); // End
																						  // time.
...
struct XTP_DAY_VIEW_LAYOUT
{
	...
	int m_nAllDayEventsCountMax; // Reserved amount of all day events area to avoid area 'blinking'
								 // when dragging.
...
struct XTP_WEEK_VIEW_LAYOUT
{
	...
	int m_nDayHeight; // Height of day rectangle.
};

//============================================================================
// Summary:
//     A predefined set of different scales based on time intervals.
//============================================================================
...
enum XTPCalendarEditOperation
{
	...
	xtpCalendarEO_Paste, // Paste event. wParam = XTP_EVENT_PASTE_OPERATION_PARAMS*. pEventViews
						 // member is NULL.
...
enum XTPCalendarSelectionChanged
{
	...
	xtpCalendarSelectionDays = 1,  // Days (or day hours for day view) selection. Use GetSelection()
								   // method to retrieve current selection.
	xtpCalendarSelectionEvents = 2 // Events selection. Use GetSelectedViewEvents() and
								   // GetSelectedEvents() methods to retrieve currently selected
								   // events.
};
...
//     user action.
// See Also:
//     XTP_NC_CALENDAR_USERACTION, XTP_CALENDAR_USERACTION_PARAMS
//-----------------------------------------------------------------------
...
{
	xtpCalendarScrollDayButton_DayViewUp = xtpCalendarHitTestDayViewScrollUp, // Defines Day view
																			  // Scroll Up button.
	xtpCalendarScrollDayButton_DayViewDown =
		xtpCalendarHitTestDayViewScrollDown, // Defines Day view Scroll Down button.
...
//-----------------------------------------------------------------------
// Summary:
//     This enum defines a set of flags which enable calendar items
//     to send XTP_NC_CALENDAR_GETITEMTEXT notifications.
// See Also:
...
{
	xtpCalendarItemText_EventSubject = 0x00000001,  // Send request to customize Event subject text
													// before draw it.
	xtpCalendarItemText_EventLocation = 0x00000002, // Send request to customize Event location text
													// before draw it.

	xtpCalendarItemText_DayViewDayHeader = 0x00000004,   // Send request to customize Day view day
														 // header text. Used for office 2003 theme
														 // only.
	xtpCalendarItemText_WeekViewDayHeader = 0x00000008,  // Send request to customize Week view day
														 // header text. Used for office 2003 theme
														 // only.
	xtpCalendarItemText_MonthViewDayHeader = 0x00000010, // Send request to customize Month view day
														 // header text. Used for office 2003 theme
														 // only.

	xtpCalendarItemText_MonthViewWeekDayHeader = 0x00000020, // Send request to customize month view
															 // week day header text.

	xtpCalendarItemText_EventToolTipText = 0x00000040, // Send request to customize event tooltip
													   // text.
	// xtpCalendarItemText_GroupToolTipText      = 0x00000080,

	xtpCalendarItemText_EventStartTimeText = 0x00000100, // Send request to customize event start
														 // time text.
	xtpCalendarItemText_EventEndTimeText = 0x00000200,   // Send request to customize event end time
														 // text.

	xtpCalendarItemText_CaptionBarDayViewDateLabel =
		0x00000300, // Send request to customize caption bar date label for day view
	xtpCalendarItemText_CaptionBarWeekViewDateLabel =
		0x00000400, // Send request to customize caption bar date label for week view
	xtpCalendarItemText_CaptionBarMonthViewDateLabel =
		0x00000500 // Send request to customize caption bar date label for month view
};
...
{
	xtpCalendarItemText_EventBody = 0x00010000, // Send request to customize Event body text before
												// draw it.
	xtpCalendarItemText_EventMarkupTemplate = 0x04000000, // Send request to set Event Markup
														  // Template text before draw it.

	xtpCalendarItemText_DayViewDayHeaderLeft = 0x00020000, // Send request to customize Day view day
														   // header text on the left.
	xtpCalendarItemText_DayViewDayHeaderCenter = 0x00040000, // Send request to customize Day view
															 // day header text on the center.
	xtpCalendarItemText_DayViewDayHeaderRight = 0x00080000,  // Send request to customize Day view
															 // day header text on the right.

	xtpCalendarItemText_WeekViewDayHeaderLeft = 0x00100000,   // Send request to customize Week view
															  // day header text on the left.
	xtpCalendarItemText_WeekViewDayHeaderCenter = 0x00200000, // Send request to customize Week view
															  // day header text on the center.
	xtpCalendarItemText_WeekViewDayHeaderRight = 0x00400000,  // Send request to customize Week view
															  // day header text on the right.

	xtpCalendarItemText_MonthViewDayHeaderLeft = 0x00800000, // Send request to customize Month view
															 // day header text on the left.
	xtpCalendarItemText_MonthViewDayHeaderCenter =
		0x01000000, // Send request to customize Month view day header text on the center.
	xtpCalendarItemText_MonthViewDayHeaderRight = 0x02000000 // Send request to customize Month view
															 // day header text on the right.
