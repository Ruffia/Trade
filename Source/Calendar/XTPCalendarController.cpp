...
////////////////////////////////////////////////////////////////////////////
CXTPCalendarController::CXTPDayInfoCache::CXTPMapTimers
	CXTPCalendarController::CXTPDayInfoCache::m_mapTimers;
...
void CXTPCalendarController::CXTPDayInfoCache::ClearDays(COleDateTime dtDayFrom,
														 COleDateTime dtDayTo)
{
...
void CXTPCalendarController::CXTPDayInfoCache::RequestToRefreshDays(COleDateTime dtDayFrom,
																	COleDateTime dtDayTo)
{
	m_uActivePriority++;
...
void CXTPCalendarController::CXTPDayInfoCache::_RequestToRefreshDays(COleDateTime dtDayFrom,
																	 COleDateTime dtDayTo,
																	 UINT uPriority)
{
	ASSERT(m_pOwner);
...
void CXTPCalendarController::CXTPDayInfoCache::OnRefreshDays(int nDaysCountToRefresh)
{
	...
	for (int i = 0; i < nDaysCountToRefresh; i++)
	{
		...
		while (pos)
		{
			...
			if (uPriority == uPriority_max && nDay < nDay_min || uPriority > uPriority_max)
			{
...
VOID CALLBACK CXTPCalendarController::CXTPDayInfoCache::OnTimerCallback(HWND hwnd, UINT uMsg,
																		UINT_PTR idEvent,
																		DWORD dwTime)
{
...
void CXTPCalendarController::AdviseToNotifications()
{
	...
	if (m_pCalendarCtrl)
	{
		CXTPNotifyConnection* pConnection = m_pCalendarCtrl ? m_pCalendarCtrl->GetConnection()
															: NULL;
		ASSERT(pConnection);
		...
		m_pSink->Advise(
			...
			CreateNotfySinkClassDelegate(this, &CXTPCalendarController::OnEvent_CalendarView));
		m_pSink->Advise(pConnection, XTP_NC_CALENDAROPTIONSWASCHANGED,
						CreateNotfySinkClassDelegate(
							this, &CXTPCalendarController::OnEvent_CalendarOptionsChanged));
		m_pSink->Advise(pConnection, XTP_NC_CALENDAR_RESOURCES_WHERE_CHANGED,
						CreateNotfySinkClassDelegate(
							this, &CXTPCalendarController::OnEvent_CalendarResourcesChanged));
		m_pSink->Advise(pConnection, XTP_NC_CALENDAR_THEME_CHANGED,
						CreateNotfySinkClassDelegate(
							this, &CXTPCalendarController::OnEvent_CalendarThemeChanged));
	}

	// Advise to Date Picker notifications
	if (m_pDatePickerCtrl)
	{
		CXTPNotifyConnection* pConnection = m_pDatePickerCtrl ? m_pDatePickerCtrl->GetConnection()
															  : NULL;
		ASSERT(pConnection);
		...
		m_pSink->Advise(pConnection, XTP_NC_DATEPICKERBUTTONCLICKED,
						CreateNotfySinkClassDelegate(this,
													 &CXTPCalendarController::OnEvent_DatePicker));
		m_pSink->Advise(pConnection, XTP_NC_DATEPICKERSELECTIONCHANGED,
						CreateNotfySinkClassDelegate(this,
													 &CXTPCalendarController::OnEvent_DatePicker));
		m_pSink->Advise(pConnection, XTP_NC_DATEPICKERBEFOREGOMODAL,
						CreateNotfySinkClassDelegate(this,
													 &CXTPCalendarController::OnEvent_DatePicker));
		m_pSink->Advise(pConnection, XTP_NC_DATEPICKERMONTHCHANGED,
						CreateNotfySinkClassDelegate(this,
													 &CXTPCalendarController::OnEvent_DatePicker));
		m_pSink->Advise(pConnection, XTP_NC_DATEPICKERGETDAYMETRICS,
						CreateNotfySinkClassDelegate(
							this, &CXTPCalendarController::OnEvent_DtPickGetItemMetrics));
	}
}
...
void CXTPCalendarController::_AdviseToDataChanged()
{
	...
	m_cnidEventAdded = m_pSink->Advise(
		pConnection, XTP_NC_CALENDAREVENTWASADDED,
		CreateNotfySinkClassDelegate(this, &CXTPCalendarController::OnEvent_CalendarDataChanged));
	m_cnidEventChanged = m_pSink->Advise(
		pConnection, XTP_NC_CALENDAREVENTWASCHANGED,
		CreateNotfySinkClassDelegate(this, &CXTPCalendarController::OnEvent_CalendarDataChanged));
	m_cnidEventDeleted = m_pSink->Advise(
		pConnection, XTP_NC_CALENDAREVENTWASDELETED,
		CreateNotfySinkClassDelegate(this, &CXTPCalendarController::OnEvent_CalendarDataChanged));
}
...
void CXTPCalendarController::OnEvent_CalendarView(XTP_NOTIFY_CODE Event, WPARAM /*wParam*/,
												  LPARAM lParam)
{
...
void CXTPCalendarController::OnEvent_DatePicker(XTP_NOTIFY_CODE Event, WPARAM wParam,
												LPARAM /*lParam*/)
{
	wParam;
	...
	switch (Event)
	{
		...
		case XTP_NC_DATEPICKERBUTTONCLICKED:
			// do nothing
			break;
		case XTP_NC_DATEPICKERMONTHCHANGED: m_DayInfoCache.UpActivePriority(); break;
		case XTP_NC_DATEPICKERSELECTIONCHANGED:
			if (m_pDatePickerCtrl && m_pCalendarCtrl && m_pCalendarCtrl->GetActiveView())
			{
				...
				BOOL bChanged  = FALSE;
				CXTPCalendarControl::CViewChangedContext viewChanged(m_pCalendarCtrl,
																	 xtpCalendarViewChangedLock);
				...
				if (m_pDatePickerCtrl->GetSelRange(dtFrom, dtTo))
				{
					...
					// Day view
					int eViewMode = XTP_SAFE_GET2(m_pCalendarCtrl, GetActiveView(), GetViewType(),
												  xtpCalendarDayView);

					if (eViewMode == xtpCalendarTimeLineView)
					{
						CXTPCalendarTimeLineView* pView = (CXTPCalendarTimeLineView*)
															  m_pCalendarCtrl->GetActiveView();
						if (pView)
						{
					...
					if (nSelRangeDays <= nXTPMaxDayViewDays || !bDaysCont
						|| !bWeekStart && nSelRangeDays <= 8)
					{
						...
						{
							CXTPCalendarMonthView* pMonthView = DYNAMIC_DOWNCAST(
								CXTPCalendarMonthView, m_pCalendarCtrl->GetActiveView());
							if (pMonthView)
							{
								pMonthView->SelectDay(dtFrom);
								COleDateTime dtBegin = dtFrom;
								// Calculate weeks count
								COleDateTime dtWeekFromBegin =
									pMonthView->GetGrid()->ShiftDateToCell_00(dtFrom);
								COleDateTime dtWeekToEnd =
									pMonthView->GetGrid()->ShiftDateToCell_00(dtTo);
								dtWeekToEnd += COleDateTimeSpan(7, 0, 0, 0);
								int nSelectedWeeksRange = CXTPCalendarUtils::GetTotalDays(
									dtWeekToEnd - dtWeekFromBegin);
						...
						{
							CXTPCalendarDayView* pDayView = DYNAMIC_DOWNCAST(
							...
							if (pDayView)
							{
								...
								pDayView->SetSelection(dtWWStart,
													   dtWWStart + pDayView->GetCellDuration());
							}
							else
							{
								CXTPCalendarWeekView* pWeekView = DYNAMIC_DOWNCAST(
									CXTPCalendarWeekView, m_pCalendarCtrl->GetActiveView());
								if (pWeekView)
								{
						...
						CXTPCalendarDayView* pDayView =
							DYNAMIC_DOWNCAST(CXTPCalendarDayView, m_pCalendarCtrl->GetActiveView());
					...
					else
					{
						...
						if (pMonthView)
						{
							...
							COleDateTime dtWeekFromBegin =
								pMonthView->GetGrid()->ShiftDateToCell_00(dtFrom);
							COleDateTime dtWeekToEnd = pMonthView->GetGrid()->ShiftDateToCell_00(
								dtTo);
							dtWeekToEnd += COleDateTimeSpan(7, 0, 0, 0);
							int nSelectedWeeksRange = CXTPCalendarUtils::GetTotalDays(
								dtWeekToEnd - dtWeekFromBegin);
					...
					if (bChanged)
					{
						OnEvent_CalendarView(XTP_NC_CALENDARVIEWWASCHANGED, 0,
											 XTP_CC_DONT_SCROLL_DP);
...
void CXTPCalendarController::OnEvent_CalendarOptionsChanged(XTP_NOTIFY_CODE Event, WPARAM wParam,
															LPARAM /*lParam*/)
{
...
void CXTPCalendarController::OnEvent_CalendarResourcesChanged(XTP_NOTIFY_CODE Event,
...
void CXTPCalendarController::OnEvent_CalendarThemeChanged(XTP_NOTIFY_CODE Event, WPARAM /*wParam*/,
														  LPARAM /*lParam*/)
{
...
void CXTPCalendarController::OnEvent_CalendarDataChanged(XTP_NOTIFY_CODE Event, WPARAM /*wParam*/,
														 LPARAM lParam)
{
	if (!m_pDatePickerCtrl)
	{
	...
	if (Event != XTP_NC_CALENDAREVENTWASADDED && Event != XTP_NC_CALENDAREVENTWASCHANGED
...
void CXTPCalendarController::OnEvent_DtPickGetItemMetrics(XTP_NOTIFY_CODE Event, WPARAM wParam,
														  LPARAM lParam)
{
...
BOOL CXTPCalendarController::_HasEvents(COleDateTime dtDay)
{
	...
	for (int i = 0; i < nRCCount; i++)
	{
		...
		if (pRC)
			ptrEvents = pRC->RetrieveDayEvents(dtDay);
