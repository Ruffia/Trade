...
BOOL CXTPCalendarMemoryDataProvider::IsProviderXML(BOOL bCheckXMLsupport, BOOL* pbXMLsupportError)
{
	...
	if (bCheckXMLsupport)
	{
		CXTPDOMDocumentPtr xmlDocPtr = XTPCreateXmlDocumentInstance();
		BOOL bCreated				 = (NULL != xmlDocPtr);
...
BOOL CXTPCalendarMemoryDataProvider::Open()
{
	...
	try
	{
		...
		if (!IsProviderXML(TRUE, &bXMLsupportError))
		{
			if (bXMLsupportError)
			{
				TRACE(_T("Warning: CXTPCalendarMemoryDataProvider::Open() cannot create XML ")
					  _T("parcer instance.\n"));
				TRACE(
					_T("\tperhaps AfxOleInit() has not been called, or msxml3.dll not found. \n"));
...
BOOL CXTPCalendarMemoryDataProvider::Save()
{
	...
	try
	{
		...
		if (!IsProviderXML(TRUE, &bXMLsupportError))
		{
			if (bXMLsupportError)
			{
				TRACE(_T("Warning: CXTPCalendarMemoryDataProvider::Save() cannot create XML ")
					  _T("parcer instance.\n"));
				TRACE(
					_T("\tperhaps AfxOleInit() has not been called, or msxml3.dll not found. \n"));
				TRACE(_T("\tThe binary data format is used. \n."));
...
CXTPCalendarEventsPtr CXTPCalendarMemoryDataProvider::RetrieveEvents(COleDateTime dtStartDay,
...
CXTPCalendarEventsPtr CXTPCalendarMemoryDataProvider::DoGetUpcomingEvents(COleDateTime dtFrom,
...
void CXTPCalendarMemoryDataProvider::DST::MarkRange(CXTPCalendarEvent* pEvent,
													const DSTScanType eScanType)
{
	DWORD dwStartDay = (DWORD)pEvent->GetStartTime();
	DWORD dwEndDay   = (DWORD)pEvent->GetEndTime();

	ScanRange(pHead, xtpBitsScan - 1, 0, (DWORD)(1 << xtpBitsScan), dwStartDay, dwEndDay, pEvent,
			  NULL, eScanType);
}

void CXTPCalendarMemoryDataProvider::DST::SearchForEvents(DWORD dwStart, DWORD dwEnd,
														  CXTPCalendarEvents* pEvents)
{
	...
	ScanRange(pHead, xtpBitsScan - 1, 0, (DWORD)(1 << xtpBitsScan), dwStart, dwEnd, NULL,
			  &tmp_mapEvents, dstScanFind);
...
void CXTPCalendarMemoryDataProvider::DST::ScanRange(DSTNode* const pNode, const int nDepth,
													const DWORD dwMinValue, const DWORD dwMaxValue,
													const DWORD dwStart, const DWORD dwEnd,
													CXTPCalendarEvent* const pEvent,
													CMapIDtoEvent* pMapEvents,
													const DSTScanType eScanType)
{
	...
	if (dwStart <= dwMediumValue)
	{
		...
		if (pNode->pLeft)
		{
			ScanRange(pNode->pLeft, nDepth - 1, dwMinValue, dwMediumValue, dwStart, dwEnd, pEvent,
					  pMapEvents, eScanType);
		}
	}
	...
	if (dwEnd >= dwMediumValue)
	{
		...
		if (pNode->pRight)
		{
			ScanRange(pNode->pRight, nDepth - 1, dwMediumValue, dwMaxValue, dwStart, dwEnd, pEvent,
					  pMapEvents, eScanType);
		}
	}
}
...
BOOL CXTPCalendarMemoryDataProvider::_Load(CXTPPropExchange* pPX)
{
	...
	while (pos)
	{
		...
		ASSERT(nRecState == xtpCalendarRecurrenceMaster
			   || nRecState == xtpCalendarRecurrenceNotRecurring);
		...
		if (nRecState == xtpCalendarRecurrenceMaster)
		{
			...
			CXTPPropExchangeEnumeratorPtr pEnumeratorExc(
				secPattern->GetEnumerator(_T("Exception")));
...
BOOL CXTPCalendarMemoryDataProvider::DoUpdate_Event(CXTPCalendarEvent* pEvent)
{
	...
	if (nRState == xtpCalendarRecurrenceException || nRState == xtpCalendarRecurrenceOccurrence)
	{
...
BOOL CXTPCalendarMemoryDataProvider::DoCreate_RPattern(CXTPCalendarRecurrencePattern* pPattern,
