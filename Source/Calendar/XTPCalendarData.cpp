...
BOOL CXTPCalendarData::CXTPDPCache::IsEventInCache(CXTPCalendarEvent* pEvent)
{
	...
	else
	{
		...
		CXTPCalendarRecurrencePatternPtr ptrRPattern = m_pCacheDP->GetRecurrencePattern(
			dwRPatternID);
		return !!ptrRPattern;
	}
}
...
void CXTPCalendarData::CXTPDPCache::_AddToCacheIfNeed(CXTPCalendarRecurrencePattern* pPattern)
{
	...
	CXTPCalendarEventPtr ptrMasterEvent = pPattern->GetMasterEvent(); // this call also add master
																	  // event to cache
	ASSERT(!!ptrMasterEvent);
...
void CXTPCalendarData::SetLabelList(CXTPCalendarEventLabels* pLabelList)
{
	...
	m_pLabelList->SetDefaultValues(FALSE); // custom values, don't update them on theme change
										   // (RefreshMetrics of Paint Manager)
...
BOOL CXTPCalendarData::AddEventLabel(int nID, CString strName, COLORREF crLabel)
{
	if (m_pLabelList)
	{
		m_pLabelList->Add(new CXTPCalendarEventLabel(nID, crLabel, strName));
...
BOOL CXTPCalendarData::AddEventCategory(UINT uID, CString strName, COLORREF crBorder,
										COLORREF crBkBase)
{
	if (m_pEventCategories)
	{
		m_pEventCategories->Add(new CXTPCalendarEventCategory(uID, strName, crBorder, crBkBase));
...
CXTPCalendarEventPtr CXTPCalendarData::CreateNewEvent(DWORD dwEventID)
{
	...
	if (ptrEvent)
	{
		DWORD dwTempID = dwEventID != XTP_CALENDAR_UNKNOWN_EVENT_ID ? dwEventID
																	: GetNextFreeTempID();
...
CXTPCalendarRecurrencePatternPtr CXTPCalendarData::CreateNewRecurrencePattern(DWORD dwPatternID)
{
	...
	if (ptrPattern)
	{
		DWORD dwTempID = dwPatternID != XTP_CALENDAR_UNKNOWN_RECURRENCE_PATTERN_ID
							 ? dwPatternID
							 : GetNextFreeTempID();
...
CXTPCalendarEventsPtr CXTPCalendarData::RetrieveDayEvents(COleDateTime dtDay)
{
	...
	if ((m_cache.m_eCacheMode & xtpCalendarDPCacheModeOnMask) && m_cache.IsDayInCache(dtDay))
	{
...
BOOL CXTPCalendarData::AddEvent(CXTPCalendarEvent* pEvent)
{
	...
	if (nRState == xtpCalendarRecurrenceMaster)
	{
		...
		VERIFY(
			pPatternRef->GetOccReminders()->Save(pPatternRef->GetCustomProperties(), pPatternRef));
...
BOOL CXTPCalendarData::DeleteEvent(CXTPCalendarEvent* pEvent)
{
	...
	if (bRes)
	{
		SendNotification(XTP_NC_CALENDAREVENTWASDELETED, (WPARAM)pEvent->GetEventID(),
						 (LPARAM)pEvent);
	}
...
BOOL CXTPCalendarData::_PostProcessRecurrenceIfNeed(CXTPCalendarEvent* pEvent)
{
	...
	BOOL bRState_prop = pEvent->GetCustomProperties()->GetProperty(cszProcess_RecurrenceState,
																   varRState);
	BOOL bRPatternID_prop =
		pEvent->GetCustomProperties()->GetProperty(cszProcess_RecurrencePatternID, varRPatternID);
...
BOOL CXTPCalendarData::_ChangeRPatternWithExceptions(CXTPCalendarRecurrencePattern* pPattern)
{
	...
	// CXTPCalendarRecurrencePatternPtr ptrPatternPrev =
	// _GetRecurrencePattern_UseCache(dwPatternID);
													  ...
													  BOOL bSend_EventDeleted)
{
	...
	if (bRes)
	{
		XTP_NOTIFY_CODE nfCode = bSend_EventDeleted ? XTP_NC_CALENDAREVENTWASDELETED
...
void CXTPCalendarData::_PostProcessOccurrencesFromMaster(COleDateTime dtDay,
														 CXTPCalendarEvents* pEvents)
{
	_PostProcessOccurrencesFromMaster2(dtDay, dtDay, pEvents);
}

void CXTPCalendarData::_PostProcessOccurrencesFromMaster2(COleDateTime dtDayFrom,
...
CXTPCalendarEventsPtr CXTPCalendarData::GetUpcomingEvents(COleDateTime dtFrom,
...
CXTPCalendarEventsPtr CXTPCalendarData::DoGetUpcomingEvents(COleDateTime dtFrom,
															COleDateTimeSpan spPeriod)
{
...
BOOL CXTPCalendarData::DoCreate_RPattern(CXTPCalendarRecurrencePattern* pPattern,
...
CString CXTPCalendarData::DataSourceFromConStr(LPCTSTR lpszConnectionString)
{
	...
	if (nIndex != -1)
	{
		...
		if ((strDataSource[0] == _T('\'') || strDataSource[0] == _T('"'))
			&& strDataSource.GetLength() > 2)
		{
...
CXTPCalendarData* CXTPCalendarData::CreateDataProvider(LPCTSTR lpszConnectionString)
{
	...
	XTPCalendarDataProvider eDPType = DataProviderTypeFromConStr(lpszConnectionString,
...
CXTPCalendarData* CXTPCalendarData::CreateDataProvider(XTPCalendarDataProvider eDataProvider)
{
	...
	switch (eDataProvider)
	{
		...
		case xtpCalendarDataProviderDatabase:
			...
			break;
		case xtpCalendarDataProviderMAPI: pDataProvider = new CXTPCalendarMAPIDataProvider(); break;
...
BEGIN_DISPATCH_MAP(CXTPCalendarData, CXTPCmdTarget)
	...
	DISP_FUNCTION_ID(CXTPCalendarData, "CreateRecurrencePattern", 52, OleCreateRecurrencePattern,
					 VT_DISPATCH, VTS_I4)
	...
	DISP_FUNCTION_ID(CXTPCalendarData, "RetrieveDayEvents", 10, OleRetrieveDayEvents, VT_DISPATCH,
					 VTS_DATE)

	DISP_FUNCTION_ID(CXTPCalendarData, "GetEvent", 11, OleGetEvent, VT_DISPATCH, VTS_I4)
	DISP_FUNCTION_ID(CXTPCalendarData, "GetRecurrencePattern", 12, OleGetRecurrencePattern,
					 VT_DISPATCH, VTS_I4)
	...
	DISP_FUNCTION_ID(CXTPCalendarData, "GetAllEventsRaw", 16, OleGetAllEventsRaw, VT_DISPATCH,
					 VTS_NONE)
	...
	DISP_FUNCTION_ID(CXTPCalendarData, "EventCategories", 22, OleGetEventCategories, VT_DISPATCH,
					 VTS_NONE)
	DISP_PROPERTY_EX_ID(CXTPCalendarData, "OwnershipMode", 23, OleGetOwnershipMode,
						OleSetOwnershipMode, VT_BOOL)
END_DISPATCH_MAP()
