...
#define XTP_CALENDAR_OFFICE2007_BASE_GREEN                                                         \
	RGB(114, 164, 90) // default base color for a "green" theme
...
#define XTP_CALCLR(clrName, nScale)                                                                \
	GetColorsSetPart()->SetColor(xtpCLR_##clrName, _T(#clrName), nScale);
...
protected:
	class CViewEventsMap
		: public CMap<CXTPCalendarDayViewEvent*, CXTPCalendarDayViewEvent*, int, int>
	{
	public:
		CViewEventsMap();
	...
	virtual void _Clear();
	virtual void _UpdateDataFor(CXTPCalendarDayViewEvent* pViewEvent,
...
protected:
	...
	virtual void _XAddOverlapped(CXTPCalendarDayViewEvent* pVEv_base,
...
////////////////////////////////////////////////////////////////////////
CSize CXTPCalendarThemeOffice2007::DrawText_Auto2SL3ML(
	CDC* pDC, LPCTSTR pcszText1, LPCTSTR pcszText2, LPCTSTR pcszText3,
	CThemeFontColorSet* pFontColor1, CThemeFontColorSet* pFontColor2,
	CThemeFontColorSet* pFontColor3, CRect& rcRect, LPCTSTR pcszText1Separator, UINT uWordBreakMode)

{
	...
	if ((szSubject.cy + szLocation.cy + XTP_DPI_Y(1)) > rcRect.Height())
	{
		...
		return DrawLine2_CenterLR(pDC, strText1, pcszText2, pFontColor1, pFontColor2, rcRect,
								  DT_VCENTER | DT_LEFT, DT_VCENTER | DT_LEFT);
	}
...
COLORREF AFX_CDECL CXTPCalendarThemeOffice2007::PowerColor(COLORREF clrBase, double dFactor,
														   double dPower)
{
	BYTE bR = (BYTE)max(0,
						min(255, (int)GetRValue(clrBase)
									 + (int)(pow((double)GetRValue(clrBase), dPower) * dFactor)));
	BYTE bG = (BYTE)max(0,
						min(255, (int)GetGValue(clrBase)
									 + (int)(pow((double)GetGValue(clrBase), dPower) * dFactor)));
	BYTE bB = (BYTE)max(0,
						min(255, (int)GetBValue(clrBase)
									 + (int)(pow((double)GetBValue(clrBase), dPower) * dFactor)));
...
int AFX_CDECL CXTPCalendarThemeOffice2007::DrawClock(CDC* pDC, COleDateTime dtClockTime,
													 CRect rcView, COLORREF clrBackground,
													 XTPCalendarClockAlignFlags cafAlign)
{
	...
	CSize m_szClkRc(XTP_DPI_X(15), XTP_DPI_Y(15));
	// Stores dimensions for a rectangle used to display the time as a clock glyph.

	const CPoint arrHours[] =
		{ CPoint(XTP_DPI_X(0), XTP_DPI_Y(-6)),  CPoint(XTP_DPI_X(3), XTP_DPI_Y(-5)),
		  CPoint(XTP_DPI_X(5), XTP_DPI_Y(-3)),  CPoint(XTP_DPI_X(6), XTP_DPI_Y(0)),
		  CPoint(XTP_DPI_X(5), XTP_DPI_Y(3)),   CPoint(XTP_DPI_X(3), XTP_DPI_Y(5)),
		  CPoint(XTP_DPI_X(0), XTP_DPI_Y(6)),   CPoint(XTP_DPI_X(-3), XTP_DPI_Y(5)),
		  CPoint(XTP_DPI_X(-5), XTP_DPI_Y(3)),  CPoint(XTP_DPI_X(-6), XTP_DPI_Y(0)),
		  CPoint(XTP_DPI_X(-5), XTP_DPI_Y(-3)), CPoint(XTP_DPI_X(-3), XTP_DPI_Y(-5)),
...
CXTPCalendarViewEventSubjectEditor*
	CXTPCalendarThemeOffice2007::StartEditSubject(CXTPCalendarViewEvent* pViewEvent)
{
	...
	CXTPCalendarViewEventSubjectEditorColored* pSE = new CXTPCalendarViewEventSubjectEditorColored(
		pViewEvent->GetCalendarControl(), pViewEvent, grclrBk);
	...
	pSE->SetWindowText(XTP_SAFE_GET2(pViewEvent, GetEvent(), GetSubject(), CString()));
	pSE->SetSel(0, 0);

	return pSE;
}

CXTPCalendarThemeOffice2007::CTOEvent*
	CXTPCalendarThemeOffice2007::GetThemePartForEvent(CXTPCalendarViewEvent* pViewEvent)
{
	if (!pViewEvent)
	{
		ASSERT(FALSE);
	...
	if (pCalendar && XTPResourceImages() && XTPResourceImages()->GetConnection())
	{
		m_pSink->Advise(XTPResourceImages()->GetConnection(), XTP_NC_COMMON_RESOURCEIMAGES_CHANGED,
						CreateNotfySinkClassDelegate(
							this, &CXTPCalendarThemeOffice2007::OnEvent_ResourceImagesChanged));
	}
}

void CXTPCalendarThemeOffice2007::OnEvent_ResourceImagesChanged(XTP_NOTIFY_CODE Event,
...
void CXTPCalendarThemeOffice2007::RefreshMetrics(BOOL bRefreshChildren)
{
	...
	XTP_CALCLR(DayViewSingleDayEventBkGRfrom,
			   255); // 255, 255, 255 // single-day event gradient fill Light
	XTP_CALCLR(DayViewSingleDayEventBkGRto,
			   155); // 191, 210, 234 // single-day event gradient fill Dark
...
int CXTPCalendarThemeOffice2007::CTODay::HitTestExpandDayButton(const CXTPCalendarViewDay* pViewDay,
																const CPoint* pPoint)
{
...
{
	CXTPCalendarThemeOffice2007* pThemeX = DYNAMIC_DOWNCAST(CXTPCalendarThemeOffice2007,
															GetTheme());
	ASSERT(pThemeX);
	if (!pThemeX)
	{
...
int CXTPCalendarThemeOffice2007::CTODayViewTimeScale::CalcMinRowHeight(
	CDC* pDC, CXTPCalendarDayViewTimeScale* pTimeScale)
{
	if (!pDC || !pTimeScale)
	{
		ASSERT(FALSE);
...
void CXTPCalendarThemeOffice2007::CTODayViewTimeScale::RefreshMetrics(BOOL)
{
	...
	COLORREF clrText = GetTheme()->GetColor2(_T("TimeScaleTextColor"),
											 m_clrLine.GetStandardColor());
...
void CXTPCalendarThemeOffice2007::CTOHeader::RefreshMetrics(BOOL bRefreshChildren)
{
	...
	m_clrTodayBaseColor.SetStandardValue(
		pThemeX->GetColor2(_T("TodayBorder"), XTP_CALENDAR_OFFICE2007_TODAYBORDER));
...
void CXTPCalendarThemeOffice2007::CTODayViewDayGroupCell::RefreshMetrics(BOOL bRefreshChildren)
{
	...
	m_clrsetWorkCell.clrBorderBottomInHour.SetStandardValue(
		GetTheme()->GetColor(xtpCLR_DayViewCellWorkBorderBottomInHour));
	m_clrsetWorkCell.clrBorderBottomHour.SetStandardValue(
		GetTheme()->GetColor(xtpCLR_DayViewCellWorkBorderBottomHour));
	m_clrsetWorkCell.clrSelected.SetStandardValue(GetTheme()->GetColor(xtpCLR_SelectedBk));

	m_clrsetNonWorkCell.clrBackground.SetStandardValue(
		GetTheme()->GetColor(xtpCLR_DayViewCellNonWorkBk));
	m_clrsetNonWorkCell.clrBorderBottomInHour.SetStandardValue(
		GetTheme()->GetColor(xtpCLR_DayViewCellNonWorkBorderBottomInHour));
	m_clrsetNonWorkCell.clrBorderBottomHour.SetStandardValue(
		GetTheme()->GetColor(xtpCLR_DayViewCellNonWorkBorderBottomHour));
	m_clrsetNonWorkCell.clrSelected.SetStandardValue(GetTheme()->GetColor(xtpCLR_SelectedBk));

	m_clrDynHatchBrush = COLORREF_NULL;
}

void CXTPCalendarThemeOffice2007::CTODayViewDayGroupAllDayEvents::RefreshMetrics(
...
void CXTPCalendarThemeOffice2007::CTOEvent::RefreshFromParent(CTOEvent* pParentSrc)
{
	...
	if (pParentSrc->GetHeightFormulaPart())
		XTP_SAFE_CALL1(GetHeightFormulaPart(),
					   RefreshFromParent(pParentSrc->GetHeightFormulaPart()));

	if (pParentSrc->GetEventIconsToDrawPart())
		XTP_SAFE_CALL1(GetEventIconsToDrawPart(),
					   RefreshFromParent(pParentSrc->GetEventIconsToDrawPart()));
}
...
void CXTPCalendarThemeOffice2007::CTOEvent::CEventFontsColorsSet::doPX(CXTPPropExchange* pPX,
																	   LPCTSTR pcszPropName,
...
void CXTPCalendarThemeOffice2007::CTOEvent::CEventFontsColorsSet::Serialize(CArchive& ar)
{
...
void CXTPCalendarThemeOffice2007::CTODayViewEvent_MultiDay::RefreshMetrics(BOOL bRefreshChildren)
{
	...
	m_fcsetSelected.clrBorder.SetStandardValue(
		pThemeX->GetColor(xtpCLR_MultiDayEventSelectedBorder));
	m_fcsetSelected.grclrBackground.SetStandardValue(m_fcsetNormal.grclrBackground);
	...
	m_fcsetNormal.fcsetStartEnd.clrColor.SetStandardValue(
		pThemeX->GetColor(xtpCLR_MultiDayEventFromToDates));

	m_fcsetSelected.fcsetStartEnd.fntFont.SetStandardValue(pThemeX->m_fntBaseFont);
	m_fcsetSelected.fcsetStartEnd.clrColor.SetStandardValue(
		pThemeX->GetColor(xtpCLR_MultiDayEventFromToDates));
}
...
void CXTPCalendarThemeOffice2007::CTODayViewEvent_SingleDay::RefreshMetrics(BOOL bRefreshChildren)
{
	...
	m_fcsetNormal.grclrBackground.SetStandardValue(
		pThemeX->GetColor(xtpCLR_DayViewSingleDayEventBkGRfrom),
		pThemeX->GetColor(xtpCLR_DayViewSingleDayEventBkGRto));

	m_fcsetSelected.clrBorder.SetStandardValue(
		pThemeX->GetColor(xtpCLR_DayViewSingleDayEventSelectedBorder));
	m_fcsetSelected.grclrBackground.SetStandardValue(m_fcsetNormal.grclrBackground);
														...
														CRect& rrcRect, LONG lArrowOffset)
{
	...
	pArrowLR->DrawImage(pDC, rcArrow, pArrowLR->GetSource(0, 2), CRect(0, 0, 0, 0),
						RGB(255, 0, 255));

	return TRUE;
}

BOOL CXTPCalendarThemeOffice2007::CTOEvent::Draw_ArrowR(CXTPCalendarViewEvent* pViewEvent, CDC* pDC,
														CRect& rrcRect)
{
	CXTPCalendarThemeOffice2007* pThemeX = DYNAMIC_DOWNCAST(CXTPCalendarThemeOffice2007,
															GetTheme());
	...
	pArrowLR->DrawImage(pDC, rcArrow, pArrowLR->GetSource(1, 2), CRect(0, 0, 0, 0),
						RGB(255, 0, 255));

	return TRUE;
}

CSize CXTPCalendarThemeOffice2007::CTOEvent::Draw_Icons(CXTPCalendarViewEvent* pViewEvent, CDC* pDC,
														const CRect& rcIconsMax, BOOL bCalculate)
{
	CXTPCalendarThemeOffice2007* pThemeX = DYNAMIC_DOWNCAST(CXTPCalendarThemeOffice2007,
															GetTheme());
	...
	for (int i = 0; i < nCount; i++)
	{
		...
		if (pDC->IsPrinting())
		{
			...
			if (ii.m_pImage)
				ii.m_pImage->DrawImage(&dcMem, rcIcon00, ii.m_rcSrc, CRect(0, 0, 0, 0),
									   RGB(255, 0, 255));
			else if (ii.m_pIcon)
			...
			pDC->BitBlt(rcIcon.left, rcIcon.top, rcIcon.Width(), rcIcon.Height(), &dcMem, 0, 0,
						SRCCOPY);
		}
		else
		{
			if (ii.m_pImage)
				ii.m_pImage->DrawImage(pDC, ii.m_rcDestination, ii.m_rcSrc, CRect(0, 0, 0, 0),
									   RGB(255, 0, 255));
			else if (ii.m_pIcon)
...
BOOL CXTPCalendarThemeOffice2007::CTODayViewEvent_MultiDay::Draw_ArrowLtext(
	CXTPCalendarViewEvent* pViewEvent, CDC* pDC, CThemeFontColorSetValue* pfcsetText,
	CRect& rrcRect, int nLeft_x)
{
...
BOOL CXTPCalendarThemeOffice2007::CTODayViewEvent_MultiDay::Draw_ArrowRtext(
	CXTPCalendarViewEvent* pViewEvent, CDC* pDC, CThemeFontColorSetValue* pfcsetText,
	CRect& rrcRect, int nRight_x)
{
	if (!pDC || !pViewEvent || !pViewEvent->GetEvent() || !pfcsetText)
	{
		ASSERT(FALSE);
...
CString CXTPCalendarThemeOffice2007::CTODayViewEvent_MultiDay::Format_FromToDate(
	CXTPCalendarViewEvent* pViewEvent, int nStart1End2)
{
	...
	COleDateTime dtDate = nStart1End2 == 1 ? pViewEvent->GetEvent()->GetStartTime()
	...
	CXTPCalendarDayViewDay* pDayViewDay, const CRect& rcDay)
{
	BOOL bIsOffice2010Theme = (pDayViewDay && pDayViewDay->GetCalendarControl())
								  ? (pDayViewDay->GetCalendarControl()->GetThemeVersion()
...
void CXTPCalendarThemeOffice2007::CTODayViewDay::Draw_DayBorder(CXTPCalendarDayViewDay* pDayViewDay,
																CDC* pDC)
{
...
void CXTPCalendarThemeOffice2007::CTODayViewDayHeader::AdjustLayout(CCmdTarget* pObject, CDC* pDC,
																	const CRect& rcRect)
{
	CXTPCalendarDayViewDay* pDayViewDay = DYNAMIC_DOWNCAST(CXTPCalendarDayViewDay, pObject);
	...
	else
	{
		...
		for (int i = 0; i < 7; i++)
		{
			CString strWDLong  = CXTPCalendarUtils::GetLocaleString(XTPToUInt(LOCALE_SDAYNAME1 + i),
																	100);
			CString strWDShort = CXTPCalendarUtils::GetLocaleString(
				XTPToUInt(CXTPCalendarUtils::GetDayOfWeekLocaleStringId(i + 1)), 100);
...
void CXTPCalendarThemeOffice2007::CTODayViewDayHeader::Draw(CCmdTarget* pObject, CDC* pDC)
{
	...
	if (bScroll && nMinColumnWidth > 0 && iResCnt > 1)
	{
		...
		if (nPage <= iResCnt * nMinColumnWidth && pThemeX->GetCalendarControl()->m_bFullDateMode)
		{
			bResMax = FALSE;
	...
	int nFlags = bIsToday ? CTOHeader::xtpCalendarItemState_Today
						  : CTOHeader::xtpCalendarItemState_Normal;
	...
	else if (!pThemeX->GetCalendarControl()->m_bFullDateMode)
	{
		...
		{
			LCTYPE lctWDName1 = XTPToUInt(m_nWeekDayFormat > 1
											  ? LOCALE_SDAYNAME1
											  : CXTPCalendarUtils::GetDayOfWeekLocaleStringId(1));

			int nWD		  = dtDay.GetDayOfWeek();
			strTextCenter = CXTPCalendarUtils::GetLocaleString(lctWDName1 + ((nWD - 2 + 7) % 7),
															   100);
		}

		pThemeX->GetItemTextIfNeed(xtpCalendarItemText_DayViewDayHeaderLeft, &strTextLeft,
								   pDayViewDay);
		pThemeX->GetItemTextIfNeed(xtpCalendarItemText_DayViewDayHeaderCenter, &strTextCenter,
								   pDayViewDay);
		pThemeX->GetItemTextIfNeed(xtpCalendarItemText_DayViewDayHeaderRight, &strTextRight,
								   pDayViewDay);
	}
		...
		if (!sFmt.IsEmpty())
		{
			...
			if (sFmt.Find(_T("%a, %b %d")) > -1)
			{
				CString strDayName = CXTPCalendarUtils::GetLocaleString(
					XTPToUInt(LOCALE_SABBREVDAYNAME1 + ((dtDay.GetDayOfWeek() - 2 + 7) % 7)), 100);

				CString strMonthName = CXTPCalendarUtils::GetLocaleString(
					XTPToUInt(LOCALE_SABBREVMONTHNAME1 + (dtDay.GetMonth() - 1)), 100);
...
void CXTPCalendarThemeOffice2007::CTODayViewDayGroupHeader::Draw(CCmdTarget* pObject, CDC* pDC)
{
	...
	XTP_DAY_VIEW_GROUP_LAYOUT& groupLayoutX = CXTPCalendarTheme::GetDayViewGroupLayout(
		pDayViewGroup);
	...
	BOOL bLastDay  = pDayViewGroup->GetGroupIndex()
					== pDayViewGroup->GetViewDay()->GetViewGroupsCount() - 1;
...
void CXTPCalendarThemeOffice2007::CTODayViewDay::OnMouseMove(CCmdTarget* pObject, UINT nFlags,
															 CPoint point)
{
	...
	{
		CXTPCalendarControl::CUpdateContext updateContext(pThemeX->GetCalendarControl(),
	...
	const XTP_DAY_VIEW_GROUP_LAYOUT& groupLayout, int nButton)
{
	...
	switch (nButton)
	{
		...
		case XTP_DAY_VIEW_GROUP_LAYOUT::xtpHotScrollDown:
			...
			rc.bottom -= XTP_DPI_Y(5);
			break;
		default: ASSERT(FALSE); rc.SetRect(0, 0, 0, 0);
	}

	return rc;
}

int CXTPCalendarThemeOffice2007::CTOCellDynBackground::GetCellDynBackground(
	CXTPCalendarViewGroup* pViewGroup, COleDateTime dtCellTime, int nCellIndex,
	CXTPCalendarTheme::XTP_BUSY_STATUS_CONTEXT& rContext)
{
	...
	if (!m_pBkViewEvent || !m_pBkViewEvent->GetEvent()
		|| !m_pBkViewEvent->GetEvent()->GetCategories())
		return xtpCalendarBusyStatusUnknown;
	...
	if (nBusyStatus == xtpCalendarBusyStatusBusy || nBusyStatus == xtpCalendarBusyStatusTentative)
	{
		if (!pCat)
			return nBusyStatus;
...
CXTPCalendarViewEvent* CXTPCalendarThemeOffice2007::CTOCellDynBackground::GetCellBackgroundEvent(
	CXTPCalendarViewGroup* pViewGroup, COleDateTime dtCellTime, int nCellIndex)
{
	ASSERT(pViewGroup);
	if (!pViewGroup)
	...
	BOOL bFillAllDayEventsBkgnd		 = XTP_SAFE_GET3(pViewGroup, GetCalendarControl(),
												 GetCalendarOptions(), bHatchAllDayViewEventsBkgnd,
												 TRUE);
	...
	for (int i = 0; i < nCount; i++)
	{
		...
		if (XTP_SAFE_GET2(pViewGroup, GetViewEvent_(i), IsMultidayEvent(), FALSE) && pEvent)
		{
			...
			COleDateTime dtEventStartTime = pEvent->IsAllDayEvent() ? CXTPCalendarUtils::ResetTime(
																		  pEvent->GetStartTime())
																	: pEvent->GetStartTime();
			COleDateTime dtEventEndTime = pEvent->IsAllDayEvent()
											  ? CXTPCalendarUtils::SetTime_235959(
													pEvent->GetEndTime())
											  : pEvent->GetEndTime();

			if (nCellIndex < 0 || (dtCellTime >= dtEventStartTime && dtCellTime <= dtEventEndTime))
			{
...
void CXTPCalendarThemeOffice2007::CTOCellDynBackground::CheckIsBackgroundHatchNeeded()
{
	...
	if (CXTPCalendarUtils::ResetTime(pBkEvent->GetStartTime())
		<= CXTPCalendarUtils::ResetTime(m_pBkTentativeEvent->GetStartTime()))
		return;
...
int CXTPCalendarThemeOffice2007::CTOCellDynBackground::_GetEventBusyStatusRank(
	CXTPCalendarEvent* pEvent)
{
	...
	ASSERT(xtpCalendarBusyStatusFree < cnRankSize && xtpCalendarBusyStatusElsewhere < cnRankSize
		   && xtpCalendarBusyStatusTentative < cnRankSize && xtpCalendarBusyStatusBusy < cnRankSize
...
int CXTPCalendarThemeOffice2007::CTOCellDynBackground::CmpEventBusyStatus(
	CXTPCalendarEvent* pEvent1, CXTPCalendarEvent* pEvent2)
{
...
void CXTPCalendarThemeOffice2007::CTODayViewDayGroupCell::Draw(
	CDC* pDC, const CRect& rcRect, const XTP_CALENDAR_THEME_DAYVIEWCELL_PARAMS& cellParams,
	CXTPPropsStateContext* pStateCnt)
{
	CXTPCalendarThemeOffice2007* pThemeX = DYNAMIC_DOWNCAST(CXTPCalendarThemeOffice2007,
															GetTheme());
	...
	if (!pDC || !pThemeX || !cellParams.pViewGroup || !pCellDynBkPart
		|| !pThemeX->GetColorsSetPart())
	{
		ASSERT(FALSE);
		return;
	}
	...
	int nCellBusyStatus = pCellDynBkPart->GetCellDynBackground(cellParams.pViewGroup,
															   cellParams.dtBeginTime,
															   cellParams.nIndex, bsColors);
	...
	{
		if (bsColors.clrBkAllDayEvents != 0
			&& bsColors.clrCellBorder != 0) // is Category or Tentative
		{
			...
			if (cellParams.bWorkCell)
			{
				...
				m_clrsetWorkCell.clrBorderBottomInHour =
					XTPDrawHelpers()->LightenColor(nScale, bsColors.clrCellBorder);
			}
			else
			{
				if (nCellBusyStatus != xtpCalendarBusyStatusTentative)
				...
				m_clrsetNonWorkCell.clrBorderBottomInHour =
					XTPDrawHelpers()->LightenColor(nScale, bsColors.clrCellBorder);
			}
		}
		else
		{
				...
				|| nCellBusyStatus == xtpCalendarBusyStatusTentative)
			{
				if (nCellBusyStatus == xtpCalendarBusyStatusBusy)
				{
					m_clrsetWorkCell.clrBackground =
						XTPDrawHelpers()->LightenColor(155, GetTheme()->m_clrBaseColor);
					m_clrsetNonWorkCell.clrBackground = (COLORREF)m_clrsetWorkCell.clrBackground;
				}

				m_clrsetWorkCell.clrBorderBottomHour = GetTheme()->GetColorsSetPart()->GetColor(
					xtpCLR_HeaderBorder); // 141, 174, 217
				m_clrsetNonWorkCell.clrBorderBottomHour = (COLORREF)
															  m_clrsetWorkCell.clrBorderBottomHour;

				m_clrsetWorkCell.clrBorderBottomInHour = GetTheme()->GetColorsSetPart()->GetColor(
					xtpCLR_DayViewCellWorkBorderBottomHour);
				m_clrsetNonWorkCell.clrBorderBottomInHour =
					(COLORREF)m_clrsetWorkCell.clrBorderBottomInHour;
			}
	...
	if (pStateCnt)
		pStateCnt->SendBeforeDrawThemeObject((LPARAM)&cellParams,
	...
	if (m_clrDynHatchBrush != COLORREF_NULL)
	{
		...
		if (cellParams.bSelected)
			clrBk = cellParams.bWorkCell ? m_clrsetWorkCell.clrSelected
										 : m_clrsetNonWorkCell.clrSelected;
		else
			clrBk = cellParams.bWorkCell ? m_clrsetWorkCell.clrBackground
										 : m_clrsetNonWorkCell.clrBackground;
...
void CXTPCalendarThemeOffice2007::CTODayViewDayGroupAllDayEvents::SetDynColors(
	CXTPCalendarDayViewGroup* pViewGroup)
{
	CXTPCalendarThemeOffice2007* pThemeX = DYNAMIC_DOWNCAST(CXTPCalendarThemeOffice2007,
															GetTheme());
	...
	{
		if (bsColors.clrBkAllDayEvents != 0
			&& bsColors.clrCellBorder != 0) // is Category or OutOfOffice
		{
			...
			if (m_nDynBusyStatus != xtpCalendarBusyStatusTentative)
			{
				...
				m_clrBackgroundSelected =
					CXTPCalendarThemeOffice2007::PowerColor(bsColors.clrBkAllDayEvents, -7.5, 0.25);
			}
			else
			{
		...
		else
		{
			if (m_nDynBusyStatus == xtpCalendarBusyStatusBusy)
			{
				m_clrBackground = GetTheme()->GetColorsSetPart()->GetColor(
					xtpCLR_HeaderBorder); // 141, 174, 217
			}
			else
			{
			...
			m_clrDynHatch = GetTheme()->GetColorsSetPart()->GetColor(
				xtpCLR_HeaderBorder); // 141, 174, 217;
		}
	}
	else
...
void CXTPCalendarThemeOffice2007::CTODayViewDayGroupAllDayEvents::Draw(CCmdTarget* pObject,
																	   CDC* pDC)
{
	CXTPCalendarThemeOffice2007* pThemeX	= DYNAMIC_DOWNCAST(CXTPCalendarThemeOffice2007,
															   GetTheme());
	...
	BOOL bIsSelected = GetTheme()->GetDayViewPart()->GetDayPart()->GetGroupPart()->IsSelected(
		pDayViewGroup);
	BOOL bIsToday = GetTheme()->GetDayViewPart()->GetHeaderPart()->IsToday(
		pDayViewGroup->GetViewDay());

	COLORREF clrBk	 = bIsSelected ? m_clrBackgroundSelected : m_clrBackground;
	COLORREF clrBorder = bIsToday
							 ? (COLORREF)pThemeX->GetDayViewPartX()->GetDayPartX()->m_clrTodayBorder
							 : m_clrBottomBorder.operator COLORREF();
...
void CXTPCalendarThemeOffice2007::CTODayViewDayGroup::Draw(CCmdTarget* pObject, CDC* pDC)
{
	...
	groupLayout.m_nHotState &= ~(XTP_DAY_VIEW_GROUP_LAYOUT::xtpHotScrollUp
								 | XTP_DAY_VIEW_GROUP_LAYOUT::xtpHotScrollDown);
...
void CXTPCalendarThemeOffice2007::CTODayViewDayGroup::OnMouseMove(CCmdTarget* pObject, UINT nFlags,
																  CPoint point)
{
	UNREFERENCED_PARAMETER(nFlags);
...
BOOL CXTPCalendarThemeOffice2007::CTODayViewDayGroup::OnLButtonDown(CCmdTarget* pObject,
																	UINT nFlags, CPoint point)
{
	...
	{
		pThemeX->GetCalendarControl()->PostMessage(
			xtp_wm_UserAction,
			(WPARAM)(int(xtpCalendarUserAction_OnScrollDay)
					 | int(xtpCalendarScrollDayButton_DayViewUp)),
	...
	if (nHit & xtpCalendarHitTestDayViewScrollDown)
	{
		pThemeX->GetCalendarControl()->PostMessage(
			xtp_wm_UserAction,
			(WPARAM)(int(xtpCalendarUserAction_OnScrollDay)
					 | int(xtpCalendarScrollDayButton_DayViewDown)),
			MAKELPARAM((WORD)pDVGroup->GetGroupIndex(), pDVGroup->GetViewDay()->GetDayIndex()));
		return TRUE;
	}
...
int CXTPCalendarThemeOffice2007::CTODayViewDayGroup::HitTestAllDayEventsScrollIcons(
	const CXTPCalendarDayViewGroup* pDVGroup, const CPoint& point)
{
	...
	{
		CRect rcUpScrollIcon = pDVGroup->GetAllDayEventsScrollButtonRect(
			XTP_DAY_VIEW_GROUP_LAYOUT::xtpHotAllDayEventsScrollUp);
	...
	{
		CRect rcDownScrollIcon = pDVGroup->GetAllDayEventsScrollButtonRect(
			XTP_DAY_VIEW_GROUP_LAYOUT::xtpHotAllDayEventsScrollDown);
...
void CXTPCalendarTheme_DayViewSingleDayEventPlaceBuilder::BuildEventPlaces(
	CXTPCalendarDayViewGroup* pViewGroup)
{
	ASSERT(pViewGroup);
	if (!pViewGroup)
	...
	m_spCellDuration = XTP_SAFE_GET3(pViewGroup, GetViewDay(), GetView(),
									 GetCellDuration().operator DATE(), 0);
	if (double(m_spCellDuration) <= XTP_HALF_SECOND)
	{
		m_spCellDuration.SetDateTimeSpan(0, 0, 30, 0);
	}

	DATE dtStart = XTP_SAFE_GET2(pViewGroup, GetViewDay(), GetDayDate().operator DATE(), (DATE)0);

	m_dtMinStartTime = dtStart + COleDateTimeSpan(1, 0, 0, 1) - m_spCellDuration;

	DATE dtScaleMaxTime = XTP_SAFE_GET2(pViewGroup->GetCalendarControl(), GetCalendarOptions(),
										dtScaleMaxTime.operator DATE(), (DATE)0);
...
void CXTPCalendarTheme_DayViewSingleDayEventPlaceBuilder::_UpdateDataFor(
	CXTPCalendarDayViewEvent* pViewEvent, CXTPCalendarDayViewGroup* pViewGroup)
{
	...
	for (int i = 0; i < nCount; i++)
	{
		...
		// is Overlapped
		if (!(dtEnd_i < dtStart || dtStart_i > dtEnd || CXTPCalendarUtils::IsEqual(dtEnd_i, dtStart)
...
void CXTPCalendarTheme_DayViewSingleDayEventPlaceBuilder::_UpdateEventsPaces(
	CXTPCalendarDayViewGroup* pViewGroup)
{
...
CXTPCalendarTheme_DayViewSingleDayEventPlaceBuilder::CViewEventsMap*
	CXTPCalendarTheme_DayViewSingleDayEventPlaceBuilder::_XGetDataFor(
...
void CXTPCalendarTheme_DayViewSingleDayEventPlaceBuilder::_XAddOverlapped(
	CXTPCalendarDayViewEvent* pVEv_base, CXTPCalendarDayViewEvent* pVEv_overlapped)
{
...
BOOL CXTPCalendarThemeOffice2007::CTODayViewDayGroup::IsSelected(CXTPCalendarViewGroup* pViewGroup)
{
	...
	if (pViewGroup->GetViewDay_()->GetView_()->GetSelection(&dtBegin, &dtEnd))
	{
		...
		CXTPCalendarDayView* pDayView =
			DYNAMIC_DOWNCAST(CXTPCalendarDayView,
							 XTP_SAFE_GET2(GetTheme(), GetCalendarControl(), GetDayView(), NULL));
		if (pDayView != NULL && pDayView->GetTotalGroupsCount() > 1)
		{
			...
			if (pViewGroup->GetViewDay_()->GetView_()->GetSelection(&dtB, &dtE, &bAllDay, &iGroup,
																	&dtSpan))
			{
...
void CXTPCalendarThemeOffice2007::CTODayViewEvent_MultiDay::CalcEventYs(CRect& rrcRect,
																		int nEventPlaceNumber)
{
	CXTPCalendarDayView* pView = DYNAMIC_DOWNCAST(CXTPCalendarDayView,
												  XTP_SAFE_GET2(GetTheme(), GetCalendarControl(),
																GetDayView(), NULL));
	if (!pView)
	{
		ASSERT(FALSE);
...
void CXTPCalendarThemeOffice2007::CTODayViewEvent_MultiDay::AdjustLayout(CCmdTarget* pObject,
																		 CDC* pDC,
																		 const CRect& rcRect,
																		 int nEventPlaceNumber)
{
	...
	CXTPCalendarDayView* pView = XTP_SAFE_GET3(pDayViewEvent, GetViewGroup(), GetViewDay(),
											   GetView(), NULL);
...
void CXTPCalendarThemeOffice2007::CTODayViewEvent_MultiDay::Draw(CCmdTarget* pObject, CDC* pDC)
{
	...
	{
		pThemeX->_DrawEventLTHighlight(
			pDC, CPoint(rcEvent.left + XTP_DPI_X(nSelOffset), rcEvent.top + XTP_DPI_Y(nSelOffset)),
	...
	if ((nMDEFlags & xtpCalendarMultiDayFirst) == 0)
	{
		...
		CXTPCalendarTheme::DrawRectPartly(pDC, rcSQ, clrBorder, bSelected, grclrBk,
										  CRect(XTP_DPI_X(1), XTP_DPI_Y(1), 0, XTP_DPI_Y(1)));

		pThemeX->_DrawEventLTHighlight(pDC,
									   CPoint(rcEvent.left + XTP_DPI_X(nSelOffset),
											  rcEvent.top + XTP_DPI_Y(nSelOffset)),
									   rcEvent.right - XTP_DPI_X(nSelOffset), grclrBk, 0,
									   rcEvent.bottom - XTP_DPI_Y(nSelOffset));
		...
		{
			CRect rcArrowText;
			CThemeFontColorSetValue* pFCsetFromTo = bSelected ? &m_fcsetSelected.fcsetStartEnd
															  : &m_fcsetNormal.fcsetStartEnd;

			BOOL bArrowText = Draw_ArrowLtext(pViewEvent, pDC, pFCsetFromTo, rcArrowText,
											  rcArrow.right);
	...
	if ((nMDEFlags & xtpCalendarMultiDayLast) == 0)
	{
		...
		CXTPCalendarTheme::DrawRectPartly(pDC, rcSQ, clrBorder, bSelected, grclrBk,
										  CRect(0, XTP_DPI_Y(1), XTP_DPI_X(1), XTP_DPI_Y(1)));

		pThemeX->_DrawEventLTHighlight(pDC, CPoint(rcSQ.left, rcEvent.top + XTP_DPI_Y(nSelOffset)),
									   rcEventMax.right - XTP_DPI_X(nSelOffset), grclrBk, 0,
									   INT_MAX, FALSE);
		if (!bHideArrow)
		{
			CRect rcArrow;
			...
			if (bArrow && bShowArrowText && !bHideArrowText)
			{
				...
				BOOL bArrowText = Draw_ArrowRtext(pViewEvent, pDC, pFCsetFromTo, rcArrowText,
												  rcArrow.left);
				if (bArrowText)
				{
	...
	CThemeFontColorSet* pFntClr1 = bSelected ? &m_fcsetSelected.fcsetSubject
											 : &m_fcsetNormal.fcsetSubject;
	CThemeFontColorSet* pFntClr2 = bSelected ? &m_fcsetSelected.fcsetLocation
											 : &m_fcsetNormal.fcsetLocation;
	...
	if (pThemeX->GetCalendarControl()->IsMarkupEnabled())
	{
		...
		if (pUIElement && pMarkupContext)
		{
			...
			XTPMarkupSetDefaultFont(pMarkupContext, (HFONT)pFntClr1->Font()->GetSafeHandle(),
									pFntClr1->Color());
	...
	if (!bMarkupDrawn)
	{
		...
		CSize szText = pThemeX->DrawLine2_CenterLR(pDC, strSubj, strLoc, pFntClr1, pFntClr2, rcText,
												   uTextShortAlign | DT_CALCRECT);
		...
		//- Draw
		pThemeX->DrawLine2_CenterLR(pDC, strSubj, strLoc, pFntClr1, pFntClr2, rcText,
									uTextShortAlign);
	}
...
#define XTP_THEMEOFFICE2007_CATEGORYICONWIDTH XTP_DPI_X(6)
#define XTP_THEMEOFFICE2007_CATEGORYICONOFFSET XTP_DPI_X(1)

void CXTPCalendarThemeOffice2007::CTODayViewEvent_MultiDay::DrawCategoryIcons(
	CDC* pDC, CXTPCalendarViewEvent* pViewEvent, CRect rcCategoryIconsBase, const CRect& rcEvent)
{
	CXTPCalendarEvent* pEvent = pViewEvent->GetEvent();
	...
	CRect rcCategoryIcons	  = AdjustCategoryIconsRectWidth(rcCategoryIconsBase, rcEvent,
														  nCategoriesCount);

	CTODayViewEvent::DrawCategoryIcons(pDC, pEvent, rcCategoryIcons);
}

CRect CXTPCalendarThemeOffice2007::CTODayViewEvent_MultiDay::AdjustCategoryIconsRectWidth(
	const CRect& rcCategoryIconsBase, const CRect& rcEvent, int nCategoriesCount)
{
	...
	if (rcCategoryIcons.Width() < nCategoryAreaWidth)
		rcCategoryIcons.left = max(rcEvent.left,
								   rcCategoryIconsBase.left
									   - (nCategoryAreaWidth - rcCategoryIconsBase.Width()));

	return rcCategoryIcons;
}

void CXTPCalendarThemeOffice2007::CTODayViewEvent::DrawCategoryIcons(CDC* pDC,
																	 CXTPCalendarEvent* pEvent,
																	 const CRect& rcCategoryIcons)
{
	...
	rcCurrentIcon.right = rcCurrentIcon.left + XTP_THEMEOFFICE2007_CATEGORYICONWIDTH;
	...
	for (int nCategoryIdx = 1; nCategoryIdx < nCategoriesCount; nCategoryIdx++)
	{
		...
		if (!GetCategoryIconColors(pCategories, pCategoryIDs, nCategoryIdx, clrCatColor,
								   clrIconBorder))
			continue;

		CBrush iconBrush(clrCatColor);
		CXTPCalendarTheme::DrawRect(pDC, rcCurrentIcon, clrIconBorder, FALSE, &iconBrush);
		rcCurrentIcon.OffsetRect(XTP_THEMEOFFICE2007_CATEGORYICONOFFSET
									 + XTP_THEMEOFFICE2007_CATEGORYICONWIDTH,
								 0);
...
BOOL CXTPCalendarThemeOffice2007::CTODayViewEvent::NeedDrawCategoryIcons(CXTPCalendarEvent* pEvent)
{
	...
	BOOL bShowCategoryIcons = XTP_SAFE_GET3(GetTheme(), GetCalendarControl(), GetCalendarOptions(),
...
{
	nCategoriesCount--; // one is the main category, we use it for background and we dont need to
						// draw it's icon
	if (nCategoriesCount <= 0)
		return 0;

	return (nCategoriesCount * XTP_THEMEOFFICE2007_CATEGORYICONWIDTH
			+ (nCategoriesCount - 1) * XTP_THEMEOFFICE2007_CATEGORYICONOFFSET);
}

bool CXTPCalendarThemeOffice2007::CTODayViewEvent::GetCategoryIconColors(
	CXTPCalendarEventCategories* pCategories, CXTPCalendarEventCategoryIDs* pCategoryIDs,
...
void CXTPCalendarThemeOffice2007::CTOEvent::GetEventColors(
	CDC* pDC, CXTPCalendarViewEvent* pViewEvent, COLORREF& rClrBorder, COLORREF& rClrBorderNotSel,
	CXTPPaintManagerColorGradient& rGrclrBk, BOOL bSelected, int* pnColorsSrc_1Cat2Lbl)
{
...
void CXTPCalendarThemeOffice2007::CTODayViewEvent_SingleDay::Draw(CCmdTarget* pObject, CDC* pDC)
{
	...
	if (!bShortEvent)
	{
		...
		if (DrawShowAsStatus(pDC, pViewEvent, pThemeX, rcEvent, lShowAsBlockWidth))
		{
			...
			pThemeX->_DrawEventLTHighlight(pDC,
										   CPoint(rcText.left, rcEvent.top + XTP_DPI_Y(nSelOffset)),
	...
	else
	{
		...
		CXTPCalendarTheme::DrawRectPartly(pDC, rcBS, clrBorder, nBorderWidth,
										  CRect(XTP_DPI_X(1), XTP_DPI_Y(1), 0, 0));

		rcBS.right = rcEventX.left + XTP_DPI_X(1);
		rcBS.top += nBorderWidth;
		if (pBSBrush)
		{
			CXTPCalendarTheme::DrawRectPartly(pDC, rcBS, clrBorderNotSel, XTP_DPI_X(1),
											  CRect(0, 0, XTP_DPI_X(1), 0));
		}

		rcBS.right += bSelected ? XTP_DPI_X(1) : 0;
		CXTPCalendarTheme::DrawRectPartly(pDC, rcBS, clrBorder, nBorderWidth,
										  CRect(0, 0, 0, XTP_DPI_Y(1)));
		...
		{
			pThemeX->_DrawEventLTHighlight(pDC,
										   CPoint(rcEventX.left + XTP_DPI_X(1),
												  rcEvent.top + XTP_DPI_Y(nSelOffset)),
										   rcEvent.right - XTP_DPI_X(nSelOffset), grclrBk);
		}
		else
		{
			pThemeX->_DrawEventLTHighlight(pDC,
										   CPoint(rcEvent.left + XTP_DPI_X(nSelOffset),
												  rcEvent.top + XTP_DPI_Y(nSelOffset)),
										   rcEvent.right - XTP_DPI_X(nSelOffset), grclrBk, 0,
										   rcTimeframe.bottom - XTP_DPI_Y(nSelOffset));
		}
	}
	...
	if (bSelected)
	{
		...
		// draw top Gripper
		rcGripper.top	= rcEvent.top - XTP_DPI_Y(1);
		rcGripper.bottom = rcGripper.top + XTP_DPI_Y(cnGripperSize);
		...
		// draw bottom Gripper
		rcGripper.bottom = rcEvent.bottom + XTP_DPI_Y(1);
	...
	else
	{
		...
		rcText.right	= max(rcText.right - szIcons.cx - XTP_DPI_X(2),
							  rcText.left + rcTopRow.Height() - XTP_DPI_Y(2));
	}
	...
	if (!bMarkupDrawn)
	{
		...
		CThemeFontColorSet* pFntClr2 = bSelected ? &m_fcsetSelected.fcsetLocation
												 : &m_fcsetNormal.fcsetLocation;
		CThemeFontColorSet* pFntClr3 = bSelected ? &m_fcsetSelected.fcsetBody
												 : &m_fcsetNormal.fcsetBody;

		UINT uWordBreakMode = DT_WORDBREAK;
		DWORD dwOpt			= XTP_SAFE_GET3(GetTheme(), GetCalendarControl(), GetCalendarOptions(),
		...
		pThemeX->DrawText_Auto2SL3ML(pDC, strSubj, strLoc, strBody, pFntClr1, pFntClr2, pFntClr3,
									 rcText, _T(";"), uWordBreakMode);
	}
...
void CXTPCalendarThemeOffice2007::CTODayViewEvent_SingleDay::DrawCategoryIcons(
	CDC* pDC, CXTPCalendarViewEvent* pViewEvent, const CRect& rcText, const CRect& rcTopRow)
{
	CXTPCalendarEvent* pEvent = pViewEvent->GetEvent();
...
CRect CXTPCalendarThemeOffice2007::CTODayViewEvent_SingleDay::CalcCategoryIconsRect(
	int nCategoriesCount, const CRect& rcText, const CRect& rcTopRow)
{
	CRect rcCategoryIcons = rcText;
	rcCategoryIcons.top   = rcCategoryIcons.bottom
						  - min(XTP_DPI_Y(20), (rcTopRow.Height() - XTP_DPI_Y(6)));
	rcCategoryIcons.left = max(rcCategoryIcons.left,
							   rcCategoryIcons.right - CalcCategoryAreaWidth(nCategoriesCount));

	return rcCategoryIcons;
}

void CXTPCalendarThemeOffice2007::CTODayViewEvent_SingleDay::DrawShadow(
	CDC* pDC, CXTPCalendarDayViewEvent* pViewEvent)
{
	...
	CRect rcCShadow(rcVShadow.left - XTP_DPI_X(3), rcHShadow.top - XTP_DPI_Y(3), rcVShadow.left,
					rcHShadow.top);
	oShadow.DrawShadowRectPart(rcCShadow, oShadow.xtpShadow2007Center);
}

void CXTPCalendarThemeOffice2007::_DrawEventLTHighlight(CDC* pDC, CPoint ptLT, int nRTx,
														const CXTPPaintManagerColorGradient& grclrBk,
														int nLTCorner, int nLBy, BOOL bRCorner)
{
	...
	if (bRCorner)
	{
		...
		pDC->LineTo(rcTop.right + cnCornerSize / 2 + XTP_DPI_X(1),
					rcTop.top + cnCornerSize / 2 + XTP_DPI_Y(1));
	}
}
...
void CXTPCalendarThemeOffice2007::CTOEvent::InitBusyStatusDefaultColors()
{
	...
	CXTPResourceManager::AssertValid(
		XTPResourceManager()->LoadBitmap(&m_bmpElsewherePattern,
										 XTP_IDB_CALENDAR_ELSEWHERE_PATTERN_2007));
	...
	CXTPResourceManager::AssertValid(
		XTPResourceManager()->LoadBitmap(&m_bmpTentativePattern,
										 XTP_IDB_CALENDAR_TENTATIVE_PATTERN_2007));
	VERIFY(m_xtpBrushTentative.CreatePatternBrush(&m_bmpTentativePattern));

	CXTPCalendarViewPartBrushValue* pBVal_Elsewhere =
		_GetBusyStatusBrushValue(xtpCalendarBusyStatusElsewhere, TRUE);
	CXTPCalendarViewPartBrushValue* pBVal_Tentative =
		_GetBusyStatusBrushValue(xtpCalendarBusyStatusTentative, TRUE);
																		  ...
																		  int nEventPlaceNumber)
{
	...
	int nEventY1 = (int)floor(dblPixelPerTime * dblEventOffsetTime1
							  + 0.5); // this is a correct rounding
	nEventY1	 = max(rcRow0.top, rcTopRow.top + nEventY1);

	int nEventY2 = (int)floor(dblPixelPerTime * dblEventOffsetTime2
							  + 0.5); // this is a correct rounding
	...
	pViewEvent->m_rcEventMax.bottom = min(pViewEvent->m_rcEvent.bottom + XTP_DPI_Y(5),
										  rcRect.bottom);
...
void CXTPCalendarThemeOffice2007::CTOMonthViewEvent_SingleDay::RefreshMetrics(BOOL bRefreshChildren)
{
	...
	m_fcsetNormal.clrBorder.SetStandardValue(
		pThemeX->GetColor(xtpCLR_MonthViewSingleDayEventBorder));
	m_fcsetNormal.grclrBackground.SetStandardValue(
		pThemeX->GetColor(xtpCLR_MonthViewSingleDayEventBkGRfrom),
		pThemeX->GetColor(xtpCLR_MonthViewSingleDayEventBkGRto));

	m_fcsetSelected.clrBorder.SetStandardValue(
		pThemeX->GetColor(xtpCLR_MonthViewSingleDayEventSelectedBorder));
	m_fcsetSelected.grclrBackground.SetStandardValue(
		pThemeX->GetColor(xtpCLR_MonthViewSingleDayEventBkGRfrom),
		pThemeX->GetColor(xtpCLR_MonthViewSingleDayEventBkGRto));
	...
	m_fcsetNormal.fcsetStartEnd.clrColor.SetStandardValue(
		pThemeX->GetColor(xtpCLR_MonthViewEventTime));
...
void CXTPCalendarThemeOffice2007::CTOMonthView::AdjustLayout(CDC* pDC, const CRect& rcRect,
															 BOOL bCallPostAdjustLayout)
{
	CXTPCalendarMonthView* pMonthView =
		DYNAMIC_DOWNCAST(CXTPCalendarMonthView,
...
void CXTPCalendarThemeOffice2007::CTOMonthViewEvent::AdjustLayout(CCmdTarget* pObject, CDC* pDC,
																  const CRect& rcRect,
																  int nEventPlaceNumber)
{
	...
	int nEventHeight = XTP_SAFE_GET4(pViewEvent, GetViewGroup_(), GetViewDay_(), GetView_(),
									 GetRowHeight(), 0);
	...
	pViewEvent->m_rcText.CopyRect(pViewEvent->m_rcEvent);
	pViewEvent->m_rcText.DeflateRect(XTP_DPI_X(nDyx) + XTP_DPI_X(3), XTP_DPI_Y(3),
									 XTP_DPI_X(nDyx) + XTP_DPI_X(2), XTP_DPI_Y(3));
...
BOOL CXTPCalendarThemeOffice2007::CTOMonthView::OnLButtonDown(CCmdTarget* pObject, UINT nFlags,
															  CPoint point)
{
...
void CXTPCalendarThemeOffice2007::CTOMonthViewWeekDayHeader::RefreshMetrics(BOOL bRefreshChildren)
{
	...
	m_clrBaseColor.SetStandardValue(
		pThemeX->GetColor2(_T("MonthViewWeekDayHeaderBk"), RGB(227, 239, 255)));
	...
	m_TextCenter.fcsetNormal.clrColor.SetStandardValue(
		pThemeX->GetColor2(_T("MonthViewWeekDayHeaderText"), RGB(101, 147, 201)));
	...
	for (int i = 0; i < 7; i++)
	{
		...
		CString sWDshort = CXTPCalendarUtils::GetLocaleString(
			XTPToUInt(CXTPCalendarUtils::GetDayOfWeekLocaleStringId(i + 1)), 100);
...
void CXTPCalendarThemeOffice2007::CTOMonthViewWeekHeader::RefreshMetrics(BOOL bRefreshChildren)
{
	...
	m_clrFreeSpaceBk.SetStandardValue(
		pThemeX->GetColor2(_T("MonthViewWeekHeaderFreeSpaceBk"), RGB(227, 239, 255)));
...
void CXTPCalendarThemeOffice2007::CTOMonthViewWeekDayHeader::AdjustLayout(CCmdTarget* pObject,
																		  CDC* pDC,
																		  const CRect& rcRect)
{
	CXTPCalendarThemeOffice2007* pThemeX = DYNAMIC_DOWNCAST(CXTPCalendarThemeOffice2007,
	...
	if (!nColsCount || m_arWeekDayNamesLong.GetSize() < 8 || m_arWeekDayNamesShort.GetSize() < 8)
	{
		ASSERT(FALSE);
		return;
	}
...
void CXTPCalendarThemeOffice2007::CTOMonthViewWeekDayHeader::Draw(CCmdTarget* pObject, CDC* pDC)
{
	...
	for (int i = 0; i < 7; i++)
	{
		...
		if (!sVal.IsEmpty())
		{
			...
			arColHeaderText[nCol] = m_bWeekDayNameSaSuLong ? m_arWeekDayNamesLong[7]
														   : m_arWeekDayNamesShort[7];
			arWeekDays[nCol] = 7;
		}
		else
		{
...
void CXTPCalendarThemeOffice2007::CTOMonthViewWeekHeader::AdjustLayout(CCmdTarget* pObject,
																	   CDC* pDC,
																	   const CRect& rcRect)
{
	CXTPCalendarThemeOffice2007* pThemeX = DYNAMIC_DOWNCAST(CXTPCalendarThemeOffice2007,
...
BOOL CXTPCalendarThemeOffice2007::CTOMonthViewWeekHeader::OnLButtonDown(CCmdTarget* pObject,
																		UINT nFlags, CPoint point)
{
	UNREFERENCED_PARAMETER(nFlags);
...
void CXTPCalendarThemeOffice2007::CTOMonthViewWeekHeader::GetWeekHeaderRect(
	CXTPCalendarThemeOffice2007* pThemeX, CXTPCalendarMonthViewDay* pViewDay, CRect& rcWHeader)
{
...
void CXTPCalendarThemeOffice2007::CTOMonthViewWeekHeader::SwitchCalendarActiveViewToWeekView(
	CXTPCalendarControl* pCalendar)
{
...
void CXTPCalendarThemeOffice2007::CTOMonthViewWeekHeader::Draw_TextCenter(
	CDC* pDC, const CRect& rcRect, int nState, LPCTSTR pcszCenterText, int* pnWidth)
{
	...
	{
		rcWHeaderTextVert.top = max(rcWHeaderTextVert.top - (nRectSize - szText.cx) / 2,
									rcWHeaderTextVert.bottom);
	}

	pDC->DrawText(CString(pcszCenterText), &rcWHeaderTextVert,
																			...
																			int nShort1Long2)
{
	...
	if (nShort1Long2 == 0 || nShort1Long2 == 1)
	{
		...
		rstrShort = _FormatWCaption(szWDay1, szMonth1, szWDay6, szMonth6, strDSeparator,
									nDateOrdering);
	}

	if (nShort1Long2 == 0 || nShort1Long2 == 2)
	{
		CString strMonth1 = CXTPCalendarUtils::GetLocaleString(XTPToUInt(LOCALE_SABBREVMONTHNAME1
																		 + dtWDay1.GetMonth() - 1),
															   100);
		CString strMonth6 = CXTPCalendarUtils::GetLocaleString(XTPToUInt(LOCALE_SABBREVMONTHNAME1
																		 + dtWDay6.GetMonth() - 1),
															   100);
...
CString CXTPCalendarThemeOffice2007::CTOMonthViewWeekHeader::_FormatWCaption(
	LPCTSTR pcszDay1, LPCTSTR pcszMonth1, LPCTSTR pcszDay6, LPCTSTR pcszMonth6,
	LPCTSTR pcszDayMonthSeparator, int nDateOrdering)
{
	...
	if (_tcsicmp(pcszMonth1, pcszMonth6) == 0)
	{
		if (nDateOrdering == 1)
		{
			strCaption.Format(_T("%s - %s%s%s"), pcszDay1, pcszDay6, pcszDayMonthSeparator,
							  pcszMonth6);
		}
		else
		{
			strCaption.Format(_T("%s%s%s - %s"), pcszMonth1, pcszDayMonthSeparator, pcszDay1,
							  pcszDay6);
		}
	}
	else
	{
		if (nDateOrdering == 1)
		{
			strCaption.Format(_T("%s%s%s - %s%s%s"), pcszDay1, pcszDayMonthSeparator, pcszMonth1,
							  pcszDay6, pcszDayMonthSeparator, pcszMonth6);
		}
		else
		{
			strCaption.Format(_T("%s%s%s - %s%s%s"), pcszMonth1, pcszDayMonthSeparator, pcszDay1,
							  pcszMonth6, pcszDayMonthSeparator, pcszDay6);
		}
	}
	return strCaption;
}
...
void CXTPCalendarThemeOffice2007::CTOMonthViewDay::SetDynColors(CXTPCalendarMonthViewDay* pMViewDay)
{
	...
	CXTPCalendarViewGroup* pViewGroup = pMViewDay->GetViewGroupsCount()
											? pMViewDay->GetViewGroup_(0)
											: NULL;

	CXTPCalendarTheme::XTP_BUSY_STATUS_CONTEXT bsColors;
	m_nDynBusyStatus = pCellDynBkPart->GetCellDynBackground(pViewGroup, pMViewDay->GetDayDate(), -1,
															bsColors);
		...
		|| m_nDynBusyStatus == xtpCalendarBusyStatusTentative)
	{
		...
		else
		{
			if (m_nDynBusyStatus == xtpCalendarBusyStatusBusy)
			{
				m_clrBackgroundLight = GetTheme()->GetColorsSetPart()->GetColor(
					xtpCLR_HeaderBorder); // 141, 174, 217
	...
	if (m_nDynBusyStatus != xtpCalendarBusyStatusTentative && m_fHatchBackground)
	{
		...
		if (pTentativeEvent->GetCategories()->GetSize())
		{
			...
			pCat		  = XTP_SAFE_GET3(pTentativeEvent, GetDataProvider(), GetEventCategories(),
								  Find(nCatID), NULL);
...
void CXTPCalendarThemeOffice2007::CTOMonthViewDay::Draw(CCmdTarget* pObject, CDC* pDC)
{
	...
	COleDateTime dtMViewBase = XTP_SAFE_GET2(pMViewDay, GetView(), GetMDayViewLightColorBaseDate(),
											 COleDateTime());
	int nCurrentMonth = dtMViewBase.GetStatus() == COleDateTime::valid ? dtMViewBase.GetMonth()
																	   : dtNow.GetMonth();
	int nLiteBkColorIdx = nCurrentMonth % 2;

	int nMonth			   = dtDay.GetMonth();
	COLORREF clrBackground = nMonth % 2 == nLiteBkColorIdx ? m_clrBackgroundLight
														   : m_clrBackgroundDark;
	if (bIsSelected)
	{
	...
	if (bIsToday)
	{
		CXTPCalendarTheme::DrawRectPartly(pDC, rcBorders, m_clrTodayBorder, XTP_DPI_X(2),
										  CRect(XTP_DPI_X(1), 0, XTP_DPI_X(1), XTP_DPI_Y(1)));
		CXTPCalendarTheme::DrawRectPartly(pDC, rcBorders, m_clrBorder, XTP_DPI_X(1),
										  CRect(0, 0, XTP_DPI_X(1), 0));
	}
	else
	{
		...
		CXTPCalendarTheme::DrawRectPartly(pDC, rcBorders, m_clrBorder, XTP_DPI_X(1),
										  CRect(nIsLBorder, 0, XTP_DPI_X(1), 0));
		CXTPCalendarTheme::DrawRectPartly(pDC, rcBorders, m_clrBorder, XTP_DPI_X(2),
										  CRect(0, 0, 0, XTP_DPI_Y(1)));
	}
	...
	for (int i = 0; i < nCount; i++)
	{
		...
		if (pViewEvent && pViewEvent->IsVisible())
		{
			...
			{
				if (!bCnt_EvMul)
					autoStateCnt_EvMul.SetData(GetMultiDayEventPartX(),
											   xtpCalendarBeforeDraw_MonthViewEvent, TRUE);
				bCnt_EvMul = TRUE;
				...
				else
				{
					...
					DWORD dwOpt			= XTP_SAFE_GET3(GetTheme(), GetCalendarControl(),
					...
					BOOL bDrawEndTime   = bShowEndTime
										&& 0 != (dwOpt & xtpCalendarOptMonthViewShowEndTimeAlways);
			...
			{
				if (!bCnt_EvS)
					autoStateCnt_EvS.SetData(GetSingleDayEventPartX(),
											 xtpCalendarBeforeDraw_MonthViewEvent, TRUE);
				bCnt_EvS = TRUE;
	...
	if (!pDC->IsPrinting() && pMViewDay->ContainsInvisibleEventsUpdate())
	{
		CRect rc = dayLayout.m_rcDay;
		rc.right -= XTP_DPI_X(5);
		rc.bottom -= XTP_DPI_Y(5);
...
void CXTPCalendarThemeOffice2007::CTOMonthViewDay::OnMouseMove(CCmdTarget* pObject, UINT nFlags,
															   CPoint point)
{
	UNREFERENCED_PARAMETER(nFlags);
	...
	if (pMViewDay->ContainsInvisibleEventsUpdate())
	{
...
BOOL CXTPCalendarThemeOffice2007::CTOMonthViewDay::OnLButtonDown(CCmdTarget* pObject, UINT nFlags,
																 CPoint point)
{
	UNREFERENCED_PARAMETER(nFlags);
	...
	if (pMViewDay->ContainsInvisibleEventsUpdate())
	{
		if (pThemeX->ExpandDayButton_HitTest(point, dayLayout.m_rcExpandSign, FALSE))
		{
			pThemeX->GetCalendarControl()->PostMessage(
				xtp_wm_UserAction,
				(WPARAM)(int(xtpCalendarUserAction_OnExpandDay)
						 | int(xtpCalendarExpandDayButton_MonthView)),
...
void CXTPCalendarThemeOffice2007::CTOMonthViewDayHeader::RefreshMetrics(BOOL bRefreshChildren)
{
	...
	m_bkToday.rcBitmapBorder.SetStandardValue(
		CRect(XTP_DPI_X(2), XTP_DPI_Y(2), XTP_DPI_X(2), XTP_DPI_Y(2)));
	m_bkTodaySelected.rcBitmapBorder.SetStandardValue(
		CRect(XTP_DPI_X(2), XTP_DPI_Y(2), XTP_DPI_X(2), XTP_DPI_Y(2)));
}
...
void CXTPCalendarThemeOffice2007::CTOMonthViewDayHeader::Draw(CCmdTarget* pObject, CDC* pDC)
{
	CXTPCalendarThemeOffice2007* pThemeX = DYNAMIC_DOWNCAST(CXTPCalendarThemeOffice2007,
															GetTheme());
	CXTPCalendarMonthViewDay* pMViewDay  = DYNAMIC_DOWNCAST(CXTPCalendarMonthViewDay, pObject);
	if (!pDC || !pThemeX || !pMViewDay || !pThemeX->GetCalendarControl() || !pMViewDay->GetView()
		|| !pMViewDay->GetView()->GetGrid())
	{
		ASSERT(FALSE);
		return;
	}
	XTP_VIEW_DAY_LAYOUT& dayLayout = CXTPCalendarTheme::GetViewDayLayout(pMViewDay);
	COleDateTime dtDay			   = pMViewDay->GetDayDate();
	BOOL bCompressWeD			   = XTP_SAFE_GET3(pMViewDay, GetView(), GetCalendarControl(),
									   MonthView_IsCompressWeekendDays(), FALSE);
	...
	{
		CString strMonth = CXTPCalendarUtils::GetLocaleString(XTPToUInt(LOCALE_SABBREVMONTHNAME1
																		+ nMonth - 1),
															  100);
	...
	pThemeX->GetItemTextIfNeed(xtpCalendarItemText_MonthViewDayHeaderCenter, &strTextCenter,
							   pMViewDay);
	pThemeX->GetItemTextIfNeed(xtpCalendarItemText_MonthViewDayHeaderRight, &strTextRight,
							   pMViewDay);

	Draw_Header(pDC, rcRect, nFlags, strTextLeft, strTextCenter, strTextRight);
}

void CXTPCalendarThemeOffice2007::CTOEvent::GetMultidayTimeFontAndColor(CFont*& pFont,
																		COLORREF& clrColor,
																		BOOL bSelected)
{
	pFont = bSelected ? m_fcsetSelected.fcsetStartEnd.Font() : m_fcsetNormal.fcsetStartEnd.Font();
	clrColor = bSelected ? m_fcsetSelected.fcsetStartEnd.Color()
						 : m_fcsetNormal.fcsetStartEnd.Color();
}

void CXTPCalendarThemeOffice2007::CTOEvent::MultidayDrawTimeBlocks(
	CDC* pDC, const CRect& rcEventRect, CXTPCalendarViewEvent* pViewEvent, BOOL MultiDayFirstPart,
	BOOL MultiDayLastPart, BOOL fStartTimeAsClock, BOOL fEndTimeAsClock)
{
	...
	DWORD dwOpt			= XTP_SAFE_GET3(GetTheme(), GetCalendarControl(), GetCalendarOptions(),
								dwAdditionalOptions, 0);
	BOOL bShowEndTime   = XTP_SAFE_GET2(pViewEvent, GetCalendarControl(), MonthView_IsShowEndDate(),
										TRUE);
...
void CXTPCalendarThemeOffice2007::CTOEvent::MultidayDrawTime(int nMDEFlags,
															 CXTPCalendarViewEvent* pViewEvent,
															 CDC* pDC, const CRect& rcEvent,
															 BOOL fStartTimeAsClock,
															 BOOL fEndTimeAsClock, CRect& rcText)
{
	CXTPCalendarThemeOffice2007* pThemeX = DYNAMIC_DOWNCAST(CXTPCalendarThemeOffice2007,
															GetTheme());
	BOOL bHideTimes = XTP_SAFE_GET3(GetTheme(), GetCalendarControl(), GetCalendarOptions(),
									bMonthView_HideTimes, FALSE);
	if (pThemeX->GetCalendarControl()->GetActiveView()->GetViewType() == xtpCalendarMonthView
		&& bHideTimes)
		return;
	...
	if (nMDEFlags & xtpCalendarMultiDayFirst || nMDEFlags & xtpCalendarMultiDayLast)
	{
		...
		if (!pViewEvent->GetEvent()->IsAllDayEvent() && !fAllTimesZero)
		{
			...
			MultidayDrawTimeBlocks(pDC, rcEvent, pViewEvent, fMultidayFirstPart, fMultidayLastPart,
								   fStartTimeAsClock, fEndTimeAsClock);
...
void CXTPCalendarThemeOffice2007::CTOMonthViewEvent_MultiDay::Draw(CCmdTarget* pObject, CDC* pDC)
{
	...
	if (!pDC || !pThemeX || !pViewEvent || !pViewEvent->GetEvent() || !pMView || !pMVDay)
	{
		ASSERT(FALSE);
		return;
	}
	...
	BOOL bCompressWD = XTP_SAFE_GET2(pViewEvent, GetCalendarControl(),
									 MonthView_IsCompressWeekendDays(), FALSE);
	...
	if (pThemeX->m_bFlatStyle)
	{
		fShowAsStatusDrawn = DrawShowAsStatus(pDC, pViewEvent, pThemeX, rcEventMax,
											  lShowAsBlockWidth);
	}
	...
	MultidayDrawTime(nMDEFlags, pViewEvent, pDC, rcEvent, pViewEvent->IsTimeAsClock(),
					 pViewEvent->IsTimeAsClock(), rcText);
...
BOOL CXTPCalendarThemeOffice2007::CTOEvent::DrawShowAsStatus(CDC* pDC,
															 CXTPCalendarViewEvent* pViewEvent,
															 CXTPCalendarThemeOffice2007* pThemeX,
															 const CRect& rcEvent,
															 LONG& lShowAsBlockWidth)
{
	lShowAsBlockWidth = 0;
	...
	CXTPCalendarTheme::DrawRectPartly(pDC, rcBS, clrBorder, bSelected, grclrBk,
									  CRect(0, XTP_DPI_Y(1), 0, XTP_DPI_Y(1)));
...
void CXTPCalendarThemeOffice2007::CTOEvent::GetShowAsStatusColors(
	CDC* pDC, CXTPCalendarViewEvent* pViewEvent, COLORREF& clrBorder, COLORREF& clrBorderNotSel,
	CXTPPaintManagerColorGradient& grclrBk)
{
	GetEventColors(pDC, pViewEvent, clrBorder, clrBorderNotSel, grclrBk);
}

void CXTPCalendarThemeOffice2007::CTOMonthViewEvent_SingleDay::GetShowAsStatusColors(
	CDC* pDC, CXTPCalendarViewEvent* pViewEvent, COLORREF& clrBorder, COLORREF& clrBorderNotSel,
	CXTPPaintManagerColorGradient& grclrBk)
{
	BOOL bSelected = pViewEvent->IsSelected();
	...
	GetEventColors(pDC, pViewEvent, clrBorder, clrBorderNotSel, grclrBk, bSelected,
				   &nColorsSrc_1Cat2Lbl);
...
void CXTPCalendarThemeOffice2007::CTOMonthViewEvent_SingleDay::Draw_Background(
	...
	CDC* pDC, const CRect& rcEventRect, CXTPCalendarMonthViewEvent* pViewEvent)
{
	...
	CFont* pFont = bSelected ? m_fcsetSelected.fcsetStartEnd.Font()
							 : m_fcsetNormal.fcsetStartEnd.Font();
	COLORREF clrColor = bSelected ? m_fcsetSelected.fcsetStartEnd.Color()
								  : m_fcsetNormal.fcsetStartEnd.Color();
	...
	BOOL bDrawEndTime = bShowEndTime
						&& szStart.cx + szEnd.cx + XTP_DPI_X(12) < rcEventRect.Width() / 2 && //||
	...
	if (bDrawStartTime)
	{
		...
		{
			iOffset = DrawClock(pDC, dtStart, rcStart, RGB(224, 224, 224),
	...
	CDC* pDC, const CRect& rcTextRect, CXTPCalendarMonthViewEvent* pViewEvent)
{
	...
	pThemeX->DrawLine2_CenterLR(pDC, strSubj, strLoc, pFntClr1, pFntClr2, rcText, nAlign, nAlign);
...
void CXTPCalendarThemeOffice2007::CTOWeekView::AdjustLayout(CDC* pDC, const CRect& rcRect,
															BOOL bCallPostAdjustLayout)
{
	CXTPCalendarThemeOffice2007* pThemeX = DYNAMIC_DOWNCAST(CXTPCalendarThemeOffice2007,
															GetTheme());
...
void CXTPCalendarThemeOffice2007::CTOWeekViewEvent_SingleDay::RefreshMetrics(BOOL bRefreshChildren)
{
	...
	m_fcsetNormal.clrBorder.SetStandardValue(
		pThemeX->GetColor(xtpCLR_WeekViewSingleDayEventBorder));
	m_fcsetNormal.grclrBackground.SetStandardValue(
		pThemeX->GetColor(xtpCLR_WeekViewSingleDayEventBkGRfrom),
		pThemeX->GetColor(xtpCLR_WeekViewSingleDayEventBkGRto));

	m_fcsetSelected.clrBorder.SetStandardValue(
		pThemeX->GetColor(xtpCLR_WeekViewSingleDayEventSelectedBorder));
	m_fcsetSelected.grclrBackground.SetStandardValue(
		pThemeX->GetColor(xtpCLR_WeekViewSingleDayEventBkGRfrom),
		pThemeX->GetColor(xtpCLR_WeekViewSingleDayEventBkGRto));
	...
	m_fcsetNormal.fcsetStartEnd.clrColor.SetStandardValue(
		pThemeX->GetColor(xtpCLR_WeekViewEventTime));
	m_fcsetNormal.fcsetStartEnd.fntFont.SetStandardValue(pFontDef);
...
void CXTPCalendarThemeOffice2007::CTOWeekViewDayHeader::AdjustLayout(CCmdTarget* pObject, CDC* pDC,
																	 const CRect& rcRect)
{
...
void CXTPCalendarThemeOffice2007::CTOWeekViewEvent::AdjustLayout(CCmdTarget* pObject, CDC* pDC,
																 const CRect& rcRect,
																 int nEventPlaceNumber)
{
	...
	int nEventHeight = XTP_SAFE_GET4(pViewEvent, GetViewGroup(), GetViewDay(), GetView(),
									 GetRowHeight(), 0);
	...
	pViewEvent->m_rcText.CopyRect(pViewEvent->m_rcEvent);
	pViewEvent->m_rcText.DeflateRect(nDyx + XTP_DPI_X(3), XTP_DPI_Y(3), nDyx + XTP_DPI_X(2),
									 XTP_DPI_Y(3));
...
void CXTPCalendarThemeOffice2007::CTOWeekViewDay::SetDynColors(CXTPCalendarWeekViewDay* pWViewDay)
{
	...
	CXTPCalendarViewGroup* pViewGroup = pWViewDay->GetViewGroupsCount()
											? pWViewDay->GetViewGroup_(0)
											: NULL;

	CXTPCalendarTheme::XTP_BUSY_STATUS_CONTEXT bsColors;
	m_nDynBusyStatus = pCellDynBkPart->GetCellDynBackground(pViewGroup, pWViewDay->GetDayDate(), -1,
															bsColors);
...
void CXTPCalendarThemeOffice2007::CTOWeekViewDay::Draw(CCmdTarget* pObject, CDC* pDC)
{
	...
	CXTPCalendarWeekViewDay* pWViewDay   = DYNAMIC_DOWNCAST(CXTPCalendarWeekViewDay, pObject);
	if (!pDC || !pThemeX || !pWViewDay || !pWViewDay->GetView()
		|| pWViewDay->GetViewGroupsCount() == 0 || !pWViewDay->GetViewGroup(0))
	{
		ASSERT(FALSE);
		return;
	}
	...
	if (pWViewDay->GetCalendarControl() && pWViewDay->GetCalendarControl()->GetCalendarOptions())
	{
	...
	{
		CXTPCalendarTheme::DrawRectPartly(pDC, rcBorders, m_clrTodayBorder, XTP_DPI_X(2),
										  CRect(XTP_DPI_X(1), 0, 0, XTP_DPI_Y(1)));
		CXTPCalendarTheme::DrawRectPartly(pDC, rcBorders, m_clrTodayBorder, XTP_DPI_X(1),
										  CRect(0, 0, XTP_DPI_X(1), 0));
	}
	else
	{
		...
		CXTPCalendarTheme::DrawRectPartly(pDC, rcBorders, m_clrBorder, XTP_DPI_X(1),
										  CRect(nIsLBorder, 0, nIsRBorder, 0));
		CXTPCalendarTheme::DrawRectPartly(pDC, rcBorders, m_clrBorder, XTP_DPI_X(2),
										  CRect(0, 0, 0, XTP_DPI_Y(1)));
	}
	...
	for (int i = 0; i < nCount; i++)
	{
		...
		if (pViewEvent && pViewEvent->IsVisible())
		{
			if (pViewEvent->IsMultidayEvent())
			{
				if (!bCnt_EvMul)
					autoStateCnt_EvMul.SetData(GetMultiDayEventPartX(),
											   xtpCalendarBeforeDraw_WeekViewEvent, TRUE);
				bCnt_EvMul = TRUE;
	...
	if (!pDC->IsPrinting() && pWViewDay->ContainsInvisibleEventsUpdate())
	{
		CRect rc = dayLayout.m_rcDay;
		rc.right -= XTP_DPI_X(5);
		rc.bottom -= XTP_DPI_Y(5);
...
void CXTPCalendarThemeOffice2007::CTOWeekViewDay::OnMouseMove(CCmdTarget* pObject, UINT nFlags,
															  CPoint point)
{
	UNREFERENCED_PARAMETER(nFlags);
	...
	if (pWViewDay->ContainsInvisibleEventsUpdate())
	{
		BOOL bHot	 = pThemeX->ExpandDayButton_HitTest(point, dayLayout.m_rcExpandSign, FALSE);
		BOOL bHotPrev = !!(dayLayout.m_nHotState & XTP_VIEW_DAY_LAYOUT::xtpHotExpandButton);
...
BOOL CXTPCalendarThemeOffice2007::CTOWeekViewDay::OnLButtonDown(CCmdTarget* pObject, UINT nFlags,
																CPoint point)
{
	UNREFERENCED_PARAMETER(nFlags);
	...
	if (pViewDay->ContainsInvisibleEventsUpdate())
	{
		if (pThemeX->ExpandDayButton_HitTest(point, dayLayout.m_rcExpandSign, FALSE))
		{
			pThemeX->GetCalendarControl()->PostMessage(
				xtp_wm_UserAction,
				(WPARAM)(int(xtpCalendarUserAction_OnExpandDay)
						 | int(xtpCalendarExpandDayButton_WeekView)),
...
void CXTPCalendarThemeOffice2007::CTOWeekViewDayHeader::Draw(CCmdTarget* pObject, CDC* pDC)
{
	...
	else
	{
		...
		pThemeX->GetItemTextIfNeed(xtpCalendarItemText_WeekViewDayHeaderLeft, &strTextLeft,
								   pWViewDay);
		pThemeX->GetItemTextIfNeed(xtpCalendarItemText_WeekViewDayHeaderCenter, &strTextCenter,
								   pWViewDay);
		pThemeX->GetItemTextIfNeed(xtpCalendarItemText_WeekViewDayHeaderRight, &strTextRight,
								   pWViewDay);
	}
...
void CXTPCalendarThemeOffice2007::CTOWeekViewEvent_MultiDay::GetMultidayTimeFontAndColor(
	CFont*& pFont, COLORREF& clrColor, BOOL bSelected)
{
	pFont	= bSelected ? m_fcsetSelected.fcsetSubject.Font() : m_fcsetNormal.fcsetSubject.Font();
	clrColor = bSelected ? m_fcsetSelected.fcsetSubject.Color()
						 : m_fcsetNormal.fcsetSubject.Color();
}
...
void CXTPCalendarThemeOffice2007::CTOWeekViewEvent_SingleDay::Draw(CCmdTarget* pObject, CDC* pDC)
{
	...
	if (pViewEvent->GetCalendarControl() && pViewEvent->GetCalendarControl()->GetCalendarOptions())
		dwAdditionalOpt =
			pViewEvent->GetCalendarControl()->GetCalendarOptions()->dwAdditionalOptions;
...
void CXTPCalendarThemeOffice2007::CTOWeekViewEvent_SingleDay::Draw_Background(
	...
	CDC* pDC, const CRect& rcEventRect, CXTPCalendarWeekViewEvent* pViewEvent)
{
	...
	DWORD dwOpt			= XTP_SAFE_GET3(GetTheme(), GetCalendarControl(), GetCalendarOptions(),
								dwAdditionalOptions, 0);
	BOOL bShowEndTime   = XTP_SAFE_GET2(pViewEvent, GetCalendarControl(), WeekView_IsShowEndDate(),
										TRUE);
...
BEGIN_DISPATCH_MAP(CXTPCalendarThemeOffice2007, CXTPCalendarTheme)
	...
	DISP_FUNCTION_ID(CXTPCalendarThemeOffice2007, "Header", 50, OleGet_Header, VT_DISPATCH,
					 VTS_NONE)
	DISP_FUNCTION_ID(CXTPCalendarThemeOffice2007, "Event", 51, OleGet_Event, VT_DISPATCH, VTS_NONE)

	DISP_FUNCTION_ID(CXTPCalendarThemeOffice2007, "DayView", 52, OleGet_DayView, VT_DISPATCH,
					 VTS_NONE)
	DISP_FUNCTION_ID(CXTPCalendarThemeOffice2007, "MonthView", 53, OleGet_MonthView, VT_DISPATCH,
					 VTS_NONE)
	DISP_FUNCTION_ID(CXTPCalendarThemeOffice2007, "WeekView", 54, OleGet_WeekView, VT_DISPATCH,
					 VTS_NONE)

	DISP_FUNCTION_ID(CXTPCalendarThemeOffice2007, "PrevNextEventButtons", 55,
					 OleGet_PrevNextEventButtons, VT_DISPATCH, VTS_NONE)
