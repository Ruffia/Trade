...
//{{AFX_CODEJOCK_PRIVATE
//===========================================================================
// In VS 7.1 the call TBase::memberFunc will sometimes crash.
// The Service Pack 1 for VS 7.1 fix this compiler bug.
// The XTP_SAFE_CALL_BASE macro helps to avoid this problem in any VS version.
//===========================================================================
#	if (_MSC_VER == 1310)
...
public:
	...
	public:
		...
		{
			virtual ~XTP_GROUP_CELL_COLOR_SET()
			{
			}
...
public:
	...
	// text is aligned to the left (or the right, see nFormat) of the rect.
...
public:
	...
	//-----------------------------------------------------------------------
	// Summary:
	//     This member operator is used to copy standard and custom values.
	// Remarks:
	//     The standard value is always copied from the specified source.
	//     The custom value is copied from the source only if this object has
	//     no custom value and the source object has a custom value set.
	//-----------------------------------------------------------------------
