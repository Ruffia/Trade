// XTPCalendarDatabaseDataProvider.cpp: implementation of the CXTPCalendarDatabaseDataProvider
// class.
//
...
_TData _GetField(XTPADODB::_Recordset* pRS, LPCWSTR pcszFName, _TData defVal)
{
	...
	try
	{
		...
		if (ptrField)
		{
			...
			if (vtValue.vt == VT_NULL || vtValue.vt == VT_EMPTY || vtValue.vt == VT_ERROR)
			{
				return defVal;
			}
	...
	catch (_com_error& e)
	{
		e;
#ifdef _DEBUG
		if (pRS->GetActiveConnection().vt == VT_DISPATCH && pRS->GetActiveConnection().pdispVal)
		{
			XTPADODB::_Connection* pConnDB = NULL;
			HRESULT hr					   = pRS->GetActiveConnection().pdispVal->QueryInterface(
				__uuidof(XTPADODB::_Connection), (void**)&pConnDB);
			if (SUCCEEDED(hr))
			{
...
public:
	...
	void SetParameter(XTPADODB::DataTypeEnum adDataType, long nSize, const _variant_t& vtValue);
...
BOOL CXTPCalendarDatabaseDataProvider::Open()
{
	...
	if (IsOpen())
	{
		TRACE(_T("Provider is alredy opened. ")
			  _T("CXTPCalendarDatabaseDataProvider::Open(ConnectionString='%s') \n"),
...
BOOL CXTPCalendarDatabaseDataProvider::IsTableExist(XTPADOX::_CatalogPtr ptrCatalog,
...
void CXTPCalendarDatabaseDataProvider::CreateEventTable(XTPADOX::_CatalogPtr ptrCatalog)
{
	...
	ptrRS->Properties->GetItem(L"Description")
		->PutValue(_variant_t(L"normal - 0, master 1, exception 3"));
	...
	ptrIsADE->Properties->GetItem(L"Description")
		->PutValue(_variant_t(L"all day event 1, 0 - otherwise"));
	...
	ptrTable->Columns->Append(_variant_t((IDispatch*)ptrLocation, true), XTPADOX::adLongVarWChar,
							  0);
	...
	ptrTable->Columns->Append(_variant_t((IDispatch*)ptrImportanceLevel, true), XTPADOX::adInteger,
							  0);
	...
	ptrTable->Columns->Append(_variant_t((IDispatch*)ptrRecurrencePatternID, true),
							  XTPADOX::adInteger, 0);
...
void CXTPCalendarDatabaseDataProvider::CreateRPatternTable(XTPADOX::_CatalogPtr ptrCatalog)
{
	...
	ptrTable->Columns->Append(_variant_t((IDispatch*)ptrPatternStartDateTime, true),
							  XTPADOX::adDate, 0);
	...
	ptrTable->Columns->Append(_variant_t((IDispatch*)ptrEndAfterOccurrence, true),
							  XTPADOX::adInteger, 0);
	...
	ptrRecurrenceTypeID->Properties->GetItem(L"Description")
		->PutValue(_variant_t(L"XTPCalendarEventRecurrenceType"));

	ptrTable->Columns->Append(_variant_t((IDispatch*)ptrRecurrenceTypeID, true), XTPADOX::adInteger,
							  0);
	...
	ptrOccurrenceInterval->Properties->GetItem(L"Description")
		->PutValue(_variant_t(L"occurs evere Nth (day, week, month)"));

	ptrTable->Columns->Append(_variant_t((IDispatch*)ptrOccurrenceInterval, true),
							  XTPADOX::adInteger, 0);
	...
	ptrDayOfWeekMask->Properties->GetItem(L"Description")
		->PutValue(_variant_t(L"stores weeks day (for  weeks, month) in packed form"));

	ptrTable->Columns->Append(_variant_t((IDispatch*)ptrDayOfWeekMask, true), XTPADOX::adInteger,
							  0);
	...
	ptrDayOfMonth->Properties->GetItem(L"Description")
		->PutValue(_variant_t(L"day of month number (for monthly, yearly)"));
	...
	ptrMonthOfYear->Properties->GetItem(L"Description")
		->PutValue(_variant_t(L"for yearly: (month number mapped to month name)"));
	...
	ptrIsEveryWeekDay->Properties->GetItem(L"Description")
		->PutValue(_variant_t(L"For daily: is occurred each weekday"));

	ptrTable->Columns->Append(_variant_t((IDispatch*)ptrIsEveryWeekDay, true), XTPADOX::adInteger,
							  0);
	...
	ptrDayOccurrenceTypeID->Properties->GetItem(L"Description")
		->PutValue(_variant_t(L"Day occurrence:  first, second, third, fourth or last occurrence "
							  L"of day  ( WhichDay)"));

	ptrTable->Columns->Append(_variant_t((IDispatch*)ptrDayOccurrenceTypeID, true),
							  XTPADOX::adInteger, 0);
	...
	ptrMasterEventID->Properties->GetItem(L"Description")
		->PutValue(_variant_t(L"EventID of master event"));

	ptrTable->Columns->Append(_variant_t((IDispatch*)ptrMasterEventID, true), XTPADOX::adInteger,
							  0);
...
CXTPCalendarSchedulesPtr CXTPCalendarDatabaseDataProvider::LoadSchedules()
{
	...
	CADOCommand cmdGetSchedules(m_pconnDb.GetInterfacePtr(), L"SELECT * FROM Schedules");
	try
	{
...
void CXTPCalendarDatabaseDataProvider::_AddSchedules(CXTPCalendarSchedules* pAddSet)
{
	...
	for (int i = 0; i < nCount; i++)
	{
		...
		try
		{
			...
			m_pcmdAddSchedule->SetParameterValue(L"prm_CustomPropertiesXMLData",
												 STRING2VAR(strData));

			m_pcmdAddSchedule->Execute();

			// get ID
			CADOCommand cmdGetLastID(m_pconnDb.GetInterfacePtr(),
									 L"SELECT MAX(ScheduleID) AS NewScheduleID FROM Schedules");
			...
			if (ptrRS_NewID)
			{
				...
				{
					UINT uNewSchID =
						(DWORD)(long)ptrRS_NewID->Fields->GetItem(L"NewScheduleID")->Value;
					pSch->SetID(uNewSchID);

#ifdef _BUSINESS_MODEL_
					UINT uNewSchSec =
						(DWORD)(long)ptrRS_NewID->Fields->GetItem(L"NewScheduleSecurity")->Value;
...
void CXTPCalendarDatabaseDataProvider::_UpdateSchedules(CXTPCalendarSchedules* pUpdateSet,
														CXTPCalendarSchedules* pOrigSet)
{
	...
	for (int i = 0; i < nCount; i++)
	{
		...
		if (pOrigSet)
		{
			int nIdx = pOrigSet->FindIndex(XTPToUInt(nID));
			ASSERT(nIdx >= 0);
			...
			if (pSch_orig)
			{
				...
				if (strName == pSch_orig->GetName() && strData_orig == strData)
				{
		...
		try
		{
			...
			m_pcmdUpdSchedule->SetParameterValue(L"prm_CustomPropertiesXMLData",
												 STRING2VAR(strData));
...
void CXTPCalendarDatabaseDataProvider::Close()
{
	...
	try
	{
		...
		if (m_bCloseDbConnection)
		{
			...
			if (FAILED(hr))
			{
				TRACE(_T("ERROR: Cannot close XTPADODB::Connection. (HRESULT = %08lx). ")
					  _T("CXTPCalendarDatabaseDataProvider::Close() \n"),
					  hr);
			}
		}
		return;
																		 ...
																		 BOOL bEcxception)
{
	...
	try
	{
		...
		ptrEvent->SetScheduleID(XTPToUInt(_GetField<long>(pRS, L"ScheduleID", 0)));
		...
		ptrEvent->SetReminderMinutesBeforeStart(
			_GetField<long>(pRS, L"ReminderMinutesBeforeStart", 0));
		...
		if (ptrEvent->GetCustomProperties())
		{
			...
			{
				VERIFY(
					ptrEvent->GetCustomProperties()->RemoveProperty(cszEventCustProp_CategoryIDs));
		...
		if (!bEcxception)
		{
			long nRState	= _GetField<long>(pRS, L"RecurrenceState",
											  xtpCalendarRecurrenceNotRecurring);
			long nPatternID = _GetField<long>(pRS, L"RecurrencePatternID",
											  XTP_CALENDAR_UNKNOWN_RECURRENCE_PATTERN_ID);

			ptrEvent->GetCustomProperties()->SetProperty(cszProcess_RecurrenceState,
														 COleVariant((long)nRState));
			ptrEvent->GetCustomProperties()->SetProperty(cszProcess_RecurrencePatternID,
														 COleVariant((long)nPatternID));
		}
	...
	COleDateTime dtFrom, COleDateTimeSpan spPeriod)
{
	...
	TCHAR szBuff[64];
	strSqlText.Replace(_T("$normal$"), ITOT_S((int)xtpCalendarRecurrenceNotRecurring, szBuff,
											  _countof(szBuff), 10));
	strSqlText.Replace(_T("$master$"),
					   ITOT_S((int)xtpCalendarRecurrenceMaster, szBuff, _countof(szBuff), 10));
	strSqlText.Replace(_T("$exception$"),
					   ITOT_S((int)xtpCalendarRecurrenceException, szBuff, _countof(szBuff), 10));
...
BOOL CXTPCalendarDatabaseDataProvider::DoCreate_Event(CXTPCalendarEvent* pEvent,
													  DWORD& rdwNewEventID)
{
	if (!IsOpen())
	{
	...
	try
	{
		...
		ASSERT(nRState == xtpCalendarRecurrenceMaster
			   || nRState == xtpCalendarRecurrenceNotRecurring
		...
		m_pcmdAddEvent->SetParameterValue(L"prm_Modified",
										  DATE2VAR(pEvent->GetLastModificationTime()));
		m_pcmdAddEvent->SetParameterValue(L"prm_BusyStatus",
										  _variant_t((long)pEvent->GetBusyStatus()));
		m_pcmdAddEvent->SetParameterValue(L"prm_ImportanceLevel",
										  _variant_t((long)pEvent->GetImportance()));
		m_pcmdAddEvent->SetParameterValue(L"prm_LabelID", _variant_t((long)pEvent->GetLabelID()));
		m_pcmdAddEvent->SetParameterValue(L"prm_RecurrencePatternID",
										  _variant_t((long)pEvent->GetRecurrencePatternID()));
		m_pcmdAddEvent->SetParameterValue(L"prm_ScheduleID",
										  _variant_t((long)pEvent->GetScheduleID()));

		_variant_t vtIsRecurrenceExceptionDeleted = (long)(pEvent->IsRExceptionDeleted() ? 1 : 0);

		m_pcmdAddEvent->SetParameterValue(L"prm_IsRecurrenceExceptionDeleted",
										  vtIsRecurrenceExceptionDeleted);
		m_pcmdAddEvent->SetParameterValue(L"prm_RExceptionStartTimeOrig",
										  DATE2VAR(pEvent->GetRException_StartTimeOrig()));
		m_pcmdAddEvent->SetParameterValue(L"prm_RExceptionEndTimeOrig",
										  DATE2VAR(pEvent->GetRException_EndTimeOrig()));
		...
		m_pcmdAddEvent->SetParameterValue(L"prm_ReminderMinutesBeforeStart",
										  _variant_t(
											  (long)pEvent->GetReminderMinutesBeforeStart()));
		...
		if (pEvent->GetCustomProperties())
		{
			...
			if (pEvent->GetCustomIcons() && pEvent->GetCustomIcons()->GetSize())
			{
				...
				VERIFY(pEvent->GetCustomProperties()->SetProperty(cszEventCustProp_IconsIDs,
																  STRING2VAR(strArray)));
			}
			...
			if (pEvent->GetCategories() && pEvent->GetCategories()->GetSize())
			{
				...
				VERIFY(pEvent->GetCustomProperties()->SetProperty(cszEventCustProp_CategoryIDs,
																  STRING2VAR(strArray)));
			}
...
BOOL CXTPCalendarDatabaseDataProvider::DoUpdate_Event(CXTPCalendarEvent* pEvent)
{
	...
	try
	{
		...
		m_pcmdUpdEvent->SetParameterValue(L"prm_RecurrenceState",
										  _variant_t((long)pEvent->GetRecurrenceState()));
		...
		m_pcmdUpdEvent->SetParameterValue(L"prm_Modified",
										  DATE2VAR(pEvent->GetLastModificationTime()));
		m_pcmdUpdEvent->SetParameterValue(L"prm_BusyStatus",
										  _variant_t((long)pEvent->GetBusyStatus()));
		m_pcmdUpdEvent->SetParameterValue(L"prm_ImportanceLevel",
										  _variant_t((long)pEvent->GetImportance()));
		m_pcmdUpdEvent->SetParameterValue(L"prm_LabelID", _variant_t((long)pEvent->GetLabelID()));
		m_pcmdUpdEvent->SetParameterValue(L"prm_RecurrencePatternID",
										  _variant_t((long)pEvent->GetRecurrencePatternID()));
		m_pcmdUpdEvent->SetParameterValue(L"prm_ScheduleID",
										  _variant_t((long)pEvent->GetScheduleID()));

		_variant_t vtIsRecurrenceExceptionDeleted = (long)(pEvent->IsRExceptionDeleted() ? 1 : 0);

		m_pcmdUpdEvent->SetParameterValue(L"prm_IsRecurrenceExceptionDeleted",
										  vtIsRecurrenceExceptionDeleted);
		m_pcmdUpdEvent->SetParameterValue(L"prm_RExceptionStartTimeOrig",
										  DATE2VAR(pEvent->GetRException_StartTimeOrig()));
		m_pcmdUpdEvent->SetParameterValue(L"prm_RExceptionEndTimeOrig",
										  DATE2VAR(pEvent->GetRException_EndTimeOrig()));
		...
		m_pcmdUpdEvent->SetParameterValue(L"prm_ReminderMinutesBeforeStart",
										  _variant_t(
											  (long)pEvent->GetReminderMinutesBeforeStart()));
	...
	TRACE(_T("ERROR: Exception in CXTPCalendarDatabaseDataProvider::UpdateEvent(ID = %d)"),
		  dwEventID);
	return FALSE;
}
...
CXTPCalendarRecurrencePatternPtr CXTPCalendarDatabaseDataProvider::DoRead_RPattern(DWORD dwPatternID)
{
	...
	try
	{
		...
		ptrPattern->SetMasterEventID(
			(DWORD)(long)ptrDataRS->Fields->GetItem(L"MasterEventID")->Value);
		...
		ptrPattern->SetPatternStartDate(
			(DATE)ptrDataRS->Fields->GetItem(L"PatternStartDateTime")->Value);
		...
		{
			ptrPattern->SetEndAfterOccurrences(
				_GetField<long>(ptrDataRS, L"EndAfterOccurrence", 1));
		}
	...
	TRACE(_T("ERROR: Exception in CXTPCalendarDatabaseDataProvider::ReadRPattern(ID = %d)"),
		  dwPatternID);
	return FALSE;
}
...
BOOL CXTPCalendarDatabaseDataProvider::_ReadRExceptions(CXTPCalendarRecurrencePattern* pPattern)
{
	...
	try
	{
		...
		while (!ptrDataRS->bEOF) //
		{
			...
			int nRState = _GetField<long>(ptrDataRS, L"RecurrenceState",
			...
			if (nRState == xtpCalendarRecurrenceException)
			{
				...
				ptrEvent->SetRExceptionDeleted(
					0 != _GetField<long>(ptrDataRS, L"IsRecurrenceExceptionDeleted", 1));
	...
	TRACE(_T("ERROR: Exception in CXTPCalendarDatabaseDataProvider::ReadRExceptions(PatternID = ")
		  _T("%d)"),
		  dwPatternID);
	return FALSE;
}

BOOL CXTPCalendarDatabaseDataProvider::DoCreate_RPattern(CXTPCalendarRecurrencePattern* pPattern,
														 DWORD& rdwNewPatternID)
{
	if (!IsOpen())
	{
		return FALSE;
	...
	try
	{
		m_pcmdAddRPattern->SetParameterValue(L"prm_MasterEventID",
											 _variant_t((long)pPattern->GetMasterEventID()));

		m_pcmdAddRPattern->SetParameterValue(L"prm_PatternStartDateTime",
											 DATE2VAR(pPattern->GetPatternStartDate()));

		m_pcmdAddRPattern->SetParameterValue(L"prm_StartDateTime",
											 DATE2VAR(pPattern->GetStartTime()));
		m_pcmdAddRPattern->SetParameterValue(L"prm_Duration",
											 _variant_t((long)pPattern->GetDurationMinutes()));

		int nUseEnd = pPattern->GetUseEndMethod();

		m_pcmdAddRPattern->SetParameterValue(L"prm_UseEnd", _variant_t((long)nUseEnd));
		m_pcmdAddRPattern->SetParameterValue(L"prm_PatternEndDateTime",
											 DATE2VAR((nUseEnd == xtpCalendarPatternEndDate)
														  ? (DATE)pPattern->GetPatternEndDate()
														  : (DATE)0));
		m_pcmdAddRPattern->SetParameterValue(L"prm_EndAfterOccurrences",
											 (long)pPattern->GetEndAfterOccurrences());
		...
		// get event ID
		m_pcmdGetLastID->SetSQLString(
			L"SELECT MAX(RecurrencePatternID) AS NewRecurrencePatternID FROM RecurrencePattern");
		XTPADODB::_RecordsetPtr ptrDataRS = m_pcmdGetLastID->Execute();
		...
		if (ptrDataRS)
		{
			...
			{
				rdwNewPatternID =
					(DWORD)(long)ptrDataRS->Fields->GetItem(L"NewRecurrencePatternID")->Value;
			}
			ptrDataRS->Close();
		}
...
BOOL CXTPCalendarDatabaseDataProvider::DoUpdate_RPattern(CXTPCalendarRecurrencePattern* pPattern)
{
	...
	//-------------------------------------
	m_pcmdUpdRPattern->SetParameterValue(L"prm_MasterEventID",
										 _variant_t((long)pPattern->GetMasterEventID()));
	...
	m_pcmdUpdRPattern->SetParameterValue(L"prm_PatternStartDateTime",
										 DATE2VAR(pPattern->GetPatternStartDate()));

	int nUseEnd = pPattern->GetUseEndMethod();

	m_pcmdUpdRPattern->SetParameterValue(L"prm_UseEnd", _variant_t((long)nUseEnd));
	m_pcmdUpdRPattern->SetParameterValue(L"prm_PatternEndDateTime",
										 DATE2VAR((nUseEnd == xtpCalendarPatternEndDate)
													  ? (DATE)pPattern->GetPatternEndDate()
													  : (DATE)0));
	m_pcmdUpdRPattern->SetParameterValue(L"prm_EndAfterOccurrences",
										 (long)pPattern->GetEndAfterOccurrences());
	...
	m_pcmdUpdRPattern->SetParameterValue(L"prm_RecurrencePatternID",
										 _variant_t((long)dwPatternID)); //
...
BOOL CXTPCalendarDatabaseDataProvider::_GetRPatternOptions(
	XTPADODB::_Recordset* pRS, XTP_CALENDAR_RECURRENCE_OPTIONS* rROptions)
{
	try
	{
		rROptions->m_nRecurrenceType =
			(XTPCalendarEventRecurrenceType)(long)pRS->Fields->GetItem(L"RecurrenceTypeID")->Value;

		switch (rROptions->m_nRecurrenceType)
		{
			case xtpCalendarRecurrenceDaily: // = 1
				rROptions->m_Daily.bEveryWeekDayOnly =
					(long)pRS->Fields->GetItem(L"IsEveryWeekDay")->Value > 0;
				...
				{
					rROptions->m_Daily.nIntervalDays =
						(long)pRS->Fields->GetItem(L"OccurrenceInterval")->Value;
				}
				break;
			case xtpCalendarRecurrenceWeekly: //   = 2,
				rROptions->m_Weekly.nIntervalWeeks =
					(long)pRS->Fields->GetItem(L"OccurrenceInterval")->Value;
				rROptions->m_Weekly.nDayOfWeekMask =
					(long)pRS->Fields->GetItem(L"DayOfWeekMask")->Value;
				break;
			case xtpCalendarRecurrenceMonthly: //      = 3,
				rROptions->m_Monthly.nIntervalMonths =
					(long)pRS->Fields->GetItem(L"OccurrenceInterval")->Value;
				rROptions->m_Monthly.nDayOfMonth = (long)pRS->Fields->GetItem(L"DayOfMonth")->Value;
				break;
			case xtpCalendarRecurrenceMonthNth: // = 4,
				rROptions->m_MonthNth.nIntervalMonths =
					(long)pRS->Fields->GetItem(L"OccurrenceInterval")->Value;
				rROptions->m_MonthNth.nWhichDay =
					(long)pRS->Fields->GetItem(L"DayOccurrenceTypeID")->Value;
			...
			case xtpCalendarRecurrenceYearNth: //      = 6,
				rROptions->m_YearNth.nWhichDay =
					(long)pRS->Fields->GetItem(L"DayOccurrenceTypeID")->Value;
				rROptions->m_YearNth.nWhichDayMask = (long)pRS->Fields->GetItem(L"DayMask")->Value;
				rROptions->m_YearNth.nMonthOfYear =
					(long)pRS->Fields->GetItem(L"MonthOfYear")->Value;
				break;
...
BOOL CXTPCalendarDatabaseDataProvider::_SetRPatternOptions(
	CXTPCalendarDatabaseDataProvider::CADOCommand* pCmd, CXTPCalendarRecurrencePattern* pPattern)
{
	try
	{
		...
		switch (nRecurrenceTypeID)
		{
			case xtpCalendarRecurrenceDaily: // = 1
				...
				else
				{
					...
					pCmd->SetParameterValue(L"prm_OccurrenceInterval",
											_variant_t((long)pROptions.m_Daily.nIntervalDays));
				}
				break;
			case xtpCalendarRecurrenceWeekly: //   = 2,
				pCmd->SetParameterValue(L"prm_OccurrenceInterval",
										_variant_t((long)pROptions.m_Weekly.nIntervalWeeks));
				pCmd->SetParameterValue(L"prm_DayOfWeekMask",
										_variant_t((long)pROptions.m_Weekly.nDayOfWeekMask));
				break;
			case xtpCalendarRecurrenceMonthly: //      = 3,
				pCmd->SetParameterValue(L"prm_OccurrenceInterval",
										_variant_t((long)pROptions.m_Monthly.nIntervalMonths));
				pCmd->SetParameterValue(L"prm_DayOfMonth",
										_variant_t((long)pROptions.m_Monthly.nDayOfMonth));
				break;
			case xtpCalendarRecurrenceMonthNth: // = 4,
				pCmd->SetParameterValue(L"prm_OccurrenceInterval",
										_variant_t((long)pROptions.m_MonthNth.nIntervalMonths));
				pCmd->SetParameterValue(L"prm_DayOccurrenceType",
										_variant_t((long)pROptions.m_MonthNth.nWhichDay));
				pCmd->SetParameterValue(L"prm_DayMask",
										_variant_t((long)pROptions.m_MonthNth.nWhichDayMask));
				break;
			case xtpCalendarRecurrenceYearly: //   = 5,
				pCmd->SetParameterValue(L"prm_MonthOfYear",
										_variant_t((long)pROptions.m_Yearly.nMonthOfYear));
				pCmd->SetParameterValue(L"prm_DayOfMonth",
										_variant_t((long)pROptions.m_Yearly.nDayOfMonth));
				break;
			case xtpCalendarRecurrenceYearNth: //      = 6,
				pCmd->SetParameterValue(L"prm_DayOccurrenceType",
										_variant_t((long)pROptions.m_YearNth.nWhichDay));
				pCmd->SetParameterValue(L"prm_DayMask",
										_variant_t((long)pROptions.m_YearNth.nWhichDayMask));
				pCmd->SetParameterValue(L"prm_MonthOfYear",
										_variant_t((long)pROptions.m_YearNth.nMonthOfYear));
				break;
			default: ASSERT(FALSE); return FALSE;
		}
		return TRUE;
...
";
	...
	// m_pcmdAddEvent->CreateParameter(L"StartDateTime", XTPADODB::adDate, XTPADODB::adParamInput,
	// -1, vtEmpty); m_pcmdAddEvent->CreateParameter(L"EndDateTime", XTPADODB::adDate,
	// XTPADODB::adParamInput, -1, vtEmpty);

	m_pcmdAddEvent->CreateParameter(L"prm_StartDateTime", XTPADODB::adDate, XTPADODB::adParamInput,
									-1, vtEmpty);
	m_pcmdAddEvent->CreateParameter(L"prm_EndDateTime", XTPADODB::adDate, XTPADODB::adParamInput,
									-1, vtEmpty);

	m_pcmdAddEvent->CreateParameter(L"prm_RecurrenceState", XTPADODB::adInteger,
									XTPADODB::adParamInput, -1, vtEmpty);
	m_pcmdAddEvent->CreateParameter(L"prm_IsAllDayEvent", XTPADODB::adInteger,
									XTPADODB::adParamInput, -1, vtEmpty);
	m_pcmdAddEvent->CreateParameter(L"prm_Subject", XTPADODB::adBSTR, XTPADODB::adParamInput, 0,
									vtEmptyStr);
	m_pcmdAddEvent->CreateParameter(L"prm_Location", XTPADODB::adBSTR, XTPADODB::adParamInput, 0,
									vtEmptyStr);
	m_pcmdAddEvent->CreateParameter(L"prm_RemainderSoundFile", XTPADODB::adBSTR,
									XTPADODB::adParamInput, 255, vtEmptyStr);
	m_pcmdAddEvent->CreateParameter(L"prm_Body", XTPADODB::adBSTR, XTPADODB::adParamInput, 0,
									vtEmptyStr);
	m_pcmdAddEvent->CreateParameter(L"prm_Created", XTPADODB::adDate, XTPADODB::adParamInput, -1,
									vtEmpty);
	m_pcmdAddEvent->CreateParameter(L"prm_Modified", XTPADODB::adDate, XTPADODB::adParamInput, -1,
									vtEmpty);
	m_pcmdAddEvent->CreateParameter(L"prm_BusyStatus", XTPADODB::adInteger, XTPADODB::adParamInput,
									-1, vtEmpty);
	m_pcmdAddEvent->CreateParameter(L"prm_ImportanceLevel", XTPADODB::adInteger,
									XTPADODB::adParamInput, -1, vtEmpty);
	m_pcmdAddEvent->CreateParameter(L"prm_LabelID", XTPADODB::adInteger, XTPADODB::adParamInput, -1,
									vtEmpty);
	m_pcmdAddEvent->CreateParameter(L"prm_RecurrencePatternID", XTPADODB::adInteger,
									XTPADODB::adParamInput, -1, vtEmpty);
	m_pcmdAddEvent->CreateParameter(L"prm_ScheduleID", XTPADODB::adInteger, XTPADODB::adParamInput,
									-1, vtEmpty);
	m_pcmdAddEvent->CreateParameter(L"prm_IsRecurrenceExceptionDeleted", XTPADODB::adInteger,
									XTPADODB::adParamInput, -1, vtEmpty);
	m_pcmdAddEvent->CreateParameter(L"prm_RExceptionStartTimeOrig", XTPADODB::adDate,
									XTPADODB::adParamInput, -1, vtEmpty);
	m_pcmdAddEvent->CreateParameter(L"prm_RExceptionEndTimeOrig", XTPADODB::adDate,
									XTPADODB::adParamInput, -1, vtEmpty);
	m_pcmdAddEvent->CreateParameter(L"prm_IsMeeting", XTPADODB::adInteger, XTPADODB::adParamInput,
									-1, vtEmpty);
	m_pcmdAddEvent->CreateParameter(L"prm_IsPrivate", XTPADODB::adInteger, XTPADODB::adParamInput,
									-1, vtEmpty);

	m_pcmdAddEvent->CreateParameter(L"prm_IsReminder", XTPADODB::adInteger, XTPADODB::adParamInput,
									-1, vtEmpty);
	m_pcmdAddEvent->CreateParameter(L"prm_ReminderMinutesBeforeStart", XTPADODB::adInteger,
									XTPADODB::adParamInput, -1, vtEmpty);

	m_pcmdAddEvent->CreateParameter(L"prm_CustomPropertiesXMLData", XTPADODB::adBSTR,
									XTPADODB::adParamInput, 0, vtEmptyStr);
}
...
void CXTPCalendarDatabaseDataProvider::CreateDelEventCommand()
{
	...
	m_pcmdDelEvent->CreateParameter(L"EventID", XTPADODB::adInteger, XTPADODB::adParamInput, -1,
									vtEmpty);
}
...
";
	...
	m_pcmdUpdEvent->CreateParameter(L"prm_StartDateTime", XTPADODB::adDate, XTPADODB::adParamInput,
									-1, vtEmpty);
	m_pcmdUpdEvent->CreateParameter(L"prm_EndDateTime", XTPADODB::adDate, XTPADODB::adParamInput,
									-1, vtEmpty);
	m_pcmdUpdEvent->CreateParameter(L"prm_RecurrenceState", XTPADODB::adInteger,
									XTPADODB::adParamInput, -1, vtEmpty);
	m_pcmdUpdEvent->CreateParameter(L"prm_IsAllDayEvent", XTPADODB::adInteger,
									XTPADODB::adParamInput, -1, vtEmpty);
	m_pcmdUpdEvent->CreateParameter(L"prm_Subject", XTPADODB::adBSTR, XTPADODB::adParamInput, 0,
									vtEmptyStr);
	m_pcmdUpdEvent->CreateParameter(L"prm_Location", XTPADODB::adBSTR, XTPADODB::adParamInput, 0,
									vtEmptyStr);
	m_pcmdUpdEvent->CreateParameter(L"prm_RemainderSoundFile", XTPADODB::adBSTR,
									XTPADODB::adParamInput, 255, vtEmptyStr);
	m_pcmdUpdEvent->CreateParameter(L"prm_Body", XTPADODB::adBSTR, XTPADODB::adParamInput, 0,
									vtEmptyStr);
	m_pcmdUpdEvent->CreateParameter(L"prm_Created", XTPADODB::adDate, XTPADODB::adParamInput, -1,
									vtEmpty);
	m_pcmdUpdEvent->CreateParameter(L"prm_Modified", XTPADODB::adDate, XTPADODB::adParamInput, -1,
									vtEmpty);
	m_pcmdUpdEvent->CreateParameter(L"prm_BusyStatus", XTPADODB::adInteger, XTPADODB::adParamInput,
									-1, vtEmpty);
	m_pcmdUpdEvent->CreateParameter(L"prm_ImportanceLevel", XTPADODB::adInteger,
									XTPADODB::adParamInput, -1, vtEmpty);
	m_pcmdUpdEvent->CreateParameter(L"prm_LabelID", XTPADODB::adInteger, XTPADODB::adParamInput, -1,
									vtEmpty);
	m_pcmdUpdEvent->CreateParameter(L"prm_RecurrencePatternID", XTPADODB::adInteger,
									XTPADODB::adParamInput, -1, vtEmpty);
	m_pcmdUpdEvent->CreateParameter(L"prm_ScheduleID", XTPADODB::adInteger, XTPADODB::adParamInput,
									-1, vtEmpty);
	m_pcmdUpdEvent->CreateParameter(L"prm_IsRecurrenceExceptionDeleted", XTPADODB::adInteger,
									XTPADODB::adParamInput, -1, vtEmpty);
	m_pcmdUpdEvent->CreateParameter(L"prm_RExceptionStartTimeOrig", XTPADODB::adDate,
									XTPADODB::adParamInput, -1, vtEmpty);
	m_pcmdUpdEvent->CreateParameter(L"prm_RExceptionEndTimeOrig", XTPADODB::adDate,
									XTPADODB::adParamInput, -1, vtEmpty);
	m_pcmdUpdEvent->CreateParameter(L"prm_IsMeeting", XTPADODB::adInteger, XTPADODB::adParamInput,
									-1, vtEmpty);
	m_pcmdUpdEvent->CreateParameter(L"prm_IsPrivate", XTPADODB::adInteger, XTPADODB::adParamInput,
									-1, vtEmpty);
	m_pcmdUpdEvent->CreateParameter(L"prm_IsReminder", XTPADODB::adInteger, XTPADODB::adParamInput,
									-1, vtEmpty);
	m_pcmdUpdEvent->CreateParameter(L"prm_ReminderMinutesBeforeStart", XTPADODB::adInteger,
									XTPADODB::adParamInput, -1, vtEmpty);
	m_pcmdUpdEvent->CreateParameter(L"prm_CustomPropertiesXMLData", XTPADODB::adBSTR,
									XTPADODB::adParamInput, 0, vtEmptyStr);

	m_pcmdUpdEvent->CreateParameter(L"prm_EventID", XTPADODB::adInteger, XTPADODB::adParamInput, -1,
									vtEmpty);
}
...
void CXTPCalendarDatabaseDataProvider::CreateUpdEventPatternIDCommand()
{
	...
	m_pcmdUpdEventPatternID->CreateParameter(L"RecurrencePatternID", XTPADODB::adInteger,
											 XTPADODB::adParamInput, -1, vtEmpty);
	m_pcmdUpdEventPatternID->CreateParameter(L"EventID", XTPADODB::adInteger,
											 XTPADODB::adParamInput, -1, vtEmpty);
}
...
void CXTPCalendarDatabaseDataProvider::CreateGetDayEventCommand()
{
	...
	m_pcmdGetDayEvents->CreateParameter(L"prm_DayDate", XTPADODB::adDate, XTPADODB::adParamInput,
										-1, vtEmpty);
}
...
void CXTPCalendarDatabaseDataProvider::CreateGetEventCommand()
{
	...
	m_pcmdGetEvent->CreateParameter(L"EventID", XTPADODB::adInteger, XTPADODB::adParamInput, -1,
									vtEmpty);
}
...
void CXTPCalendarDatabaseDataProvider::CreateGetRExceptionsCommand()
{
	...
	m_pcmdGetRExceptions->CreateParameter(L"PatternID", XTPADODB::adInteger, XTPADODB::adParamInput,
										  -1, vtEmpty);
}
...
void CXTPCalendarDatabaseDataProvider::CreateAddRPatternCommand()
{
	...
	m_pcmdAddRPattern->CreateParameter(L"prm_PatternStartDateTime", XTPADODB::adDate,
									   XTPADODB::adParamInput, -1, vtEmpty);
	m_pcmdAddRPattern->CreateParameter(L"prm_PatternEndDateTime", XTPADODB::adDate,
									   XTPADODB::adParamInput, -1, vtEmpty);
	m_pcmdAddRPattern->CreateParameter(L"prm_StartDateTime", XTPADODB::adDate,
									   XTPADODB::adParamInput, -1, vtEmpty);
	m_pcmdAddRPattern->CreateParameter(L"prm_Duration", XTPADODB::adInteger, XTPADODB::adParamInput,
									   -1, vtEmpty);
	m_pcmdAddRPattern->CreateParameter(L"prm_EndAfterOccurrences", XTPADODB::adInteger,
									   XTPADODB::adParamInput, -1, vtEmpty);
	m_pcmdAddRPattern->CreateParameter(L"prm_UseEnd", XTPADODB::adInteger, XTPADODB::adParamInput,
									   -1, vtEmpty);
	m_pcmdAddRPattern->CreateParameter(L"prm_RecurrenceTypeID", XTPADODB::adInteger,
									   XTPADODB::adParamInput, -1, vtEmpty);
	m_pcmdAddRPattern->CreateParameter(L"prm_OccurrenceInterval", XTPADODB::adInteger,
									   XTPADODB::adParamInput, -1, vtEmpty);
	m_pcmdAddRPattern->CreateParameter(L"prm_DayOfWeekMask", XTPADODB::adInteger,
									   XTPADODB::adParamInput, -1, vtEmpty);
	m_pcmdAddRPattern->CreateParameter(L"prm_DayOfMonth", XTPADODB::adInteger,
									   XTPADODB::adParamInput, -1, vtEmpty);
	m_pcmdAddRPattern->CreateParameter(L"prm_MonthOfYear", XTPADODB::adInteger,
									   XTPADODB::adParamInput, -1, vtEmpty);
	m_pcmdAddRPattern->CreateParameter(L"prm_IsEveryWeekDay", XTPADODB::adInteger,
									   XTPADODB::adParamInput, -1, vtEmpty);
	m_pcmdAddRPattern->CreateParameter(L"prm_DayOccurrenceType", XTPADODB::adInteger,
									   XTPADODB::adParamInput, -1, vtEmpty);
	m_pcmdAddRPattern->CreateParameter(L"prm_DayMask", XTPADODB::adInteger, XTPADODB::adParamInput,
									   -1, vtEmpty);
	m_pcmdAddRPattern->CreateParameter(L"prm_MasterEventID", XTPADODB::adInteger,
									   XTPADODB::adParamInput, -1, vtEmpty);
	m_pcmdAddRPattern->CreateParameter(L"prm_CustomPropertiesXMLData", XTPADODB::adBSTR,
									   XTPADODB::adParamInput, 0, vtEmptyStr);
}
...
void CXTPCalendarDatabaseDataProvider::CreateDelRPatternCommand()
{
	...
	m_pcmdDelRPattern->CreateParameter(L"RecurrencePatternID", XTPADODB::adInteger,
									   XTPADODB::adParamInput, -1, vtEmpty);
}
...
void CXTPCalendarDatabaseDataProvider::CreateUpdRPatternCommand()
{
	...
	m_pcmdUpdRPattern->CreateParameter(L"prm_PatternStartDateTime", XTPADODB::adDate,
									   XTPADODB::adParamInput, -1, vtEmpty);
	m_pcmdUpdRPattern->CreateParameter(L"prm_PatternEndDateTime", XTPADODB::adDate,
									   XTPADODB::adParamInput, -1, vtEmpty);
	m_pcmdUpdRPattern->CreateParameter(L"prm_StartDateTime", XTPADODB::adDate,
									   XTPADODB::adParamInput, -1, vtEmpty);
	m_pcmdUpdRPattern->CreateParameter(L"prm_Duration", XTPADODB::adInteger, XTPADODB::adParamInput,
									   -1, vtEmpty);
	m_pcmdUpdRPattern->CreateParameter(L"prm_EndAfterOccurrences", XTPADODB::adInteger,
									   XTPADODB::adParamInput, -1, vtEmpty);
	m_pcmdUpdRPattern->CreateParameter(L"prm_UseEnd", XTPADODB::adInteger, XTPADODB::adParamInput,
									   -1, vtEmpty);
	m_pcmdUpdRPattern->CreateParameter(L"prm_RecurrenceTypeID", XTPADODB::adInteger,
									   XTPADODB::adParamInput, -1, vtEmpty);
	m_pcmdUpdRPattern->CreateParameter(L"prm_OccurrenceInterval", XTPADODB::adInteger,
									   XTPADODB::adParamInput, -1, vtEmpty);
	m_pcmdUpdRPattern->CreateParameter(L"prm_DayOfWeekMask", XTPADODB::adInteger,
									   XTPADODB::adParamInput, -1, vtEmpty);
	m_pcmdUpdRPattern->CreateParameter(L"prm_DayOfMonth", XTPADODB::adInteger,
									   XTPADODB::adParamInput, -1, vtEmpty);
	m_pcmdUpdRPattern->CreateParameter(L"prm_MonthOfYear", XTPADODB::adInteger,
									   XTPADODB::adParamInput, -1, vtEmpty);
	m_pcmdUpdRPattern->CreateParameter(L"prm_IsEveryWeekDay", XTPADODB::adInteger,
									   XTPADODB::adParamInput, -1, vtEmpty);
	m_pcmdUpdRPattern->CreateParameter(L"prm_DayOccurrenceType", XTPADODB::adInteger,
									   XTPADODB::adParamInput, -1, vtEmpty);
	m_pcmdUpdRPattern->CreateParameter(L"prm_DayMask", XTPADODB::adInteger, XTPADODB::adParamInput,
									   -1, vtEmpty);
	m_pcmdUpdRPattern->CreateParameter(L"prm_MasterEventID", XTPADODB::adInteger,
									   XTPADODB::adParamInput, -1, vtEmpty);
	m_pcmdUpdRPattern->CreateParameter(L"prm_CustomPropertiesXMLData", XTPADODB::adBSTR,
									   XTPADODB::adParamInput, 0, vtEmptyStr);

	m_pcmdUpdRPattern->CreateParameter(L"prm_RecurrencePatternID", XTPADODB::adInteger,
									   XTPADODB::adParamInput, -1, vtEmpty);
}
...
void CXTPCalendarDatabaseDataProvider::CreateGetRPatternCommand()
{
	...
	m_pcmdGetRPattern->CreateParameter(L"RecurrencePatternID", XTPADODB::adInteger,
									   XTPADODB::adParamInput, -1, vtEmpty);
}
...
void CXTPCalendarDatabaseDataProvider::CreateAddOptionsCommand()
{
	...
	_variant_t vtEmpty((long)0);
	_variant_t vtEmptyStr(EMPTY_DB_STR);
	m_pcmdAddOptions->CreateParameter(L"prm_SaveDateTime", XTPADODB::adDate, XTPADODB::adParamInput,
									  -1, vtEmpty);
	m_pcmdAddOptions->CreateParameter(L"prm_XMLData", XTPADODB::adBSTR, XTPADODB::adParamInput, 0,
									  vtEmptyStr);
}
...
void CXTPCalendarDatabaseDataProvider::CreateAddScheduleCommand()
{
	...
	m_pcmdAddSchedule->CreateParameter(L"prm_Name", XTPADODB::adBSTR, XTPADODB::adParamInput, 0,
									   vtEmptyStr);
	m_pcmdAddSchedule->CreateParameter(L"prm_CustomPropertiesXMLData", XTPADODB::adBSTR,
									   XTPADODB::adParamInput, 0, vtEmptyStr);
	m_pcmdAddSchedule->CreateParameter(L"prm_Modified", XTPADODB::adDate, XTPADODB::adParamInput,
									   -1, vtEmpty);
}
...
void CXTPCalendarDatabaseDataProvider::CreateUpdScheduleCommand()
{
	//---------------- sql statement body -------------------------
#ifdef _BUSINESS_MODEL_
	LPCWSTR strSqlText = L"PARAMETERS \
				prm_ScheduleID              Integer, \
	...
	m_pcmdUpdSchedule->CreateParameter(L"prm_ScheduleID", XTPADODB::adInteger,
									   XTPADODB::adParamInput, -1, vtEmpty);

#ifdef _BUSINESS_MODEL_
	m_pcmdUpdSchedule->CreateParameter(L"prm_ScheduleSecurity", XTPADODB::adInteger,
									   XTPADODB::adParamInput, -1, vtEmpty);
#endif

	m_pcmdUpdSchedule->CreateParameter(L"prm_Name", XTPADODB::adBSTR, XTPADODB::adParamInput, 0,
									   vtEmptyStr);
	m_pcmdUpdSchedule->CreateParameter(L"prm_CustomPropertiesXMLData", XTPADODB::adBSTR,
									   XTPADODB::adParamInput, 0, vtEmptyStr);
	m_pcmdUpdSchedule->CreateParameter(L"prm_Modified", XTPADODB::adDate, XTPADODB::adParamInput,
									   -1, vtEmpty);
}
...
CXTPCalendarDatabaseDataProvider::CADOCommand::CADOCommand(XTPADODB::_ConnectionPtr pConnection,
														   LPCWSTR pcszSQLText)
{
	m_pconnDb = NULL;
	try
...
void CXTPCalendarDatabaseDataProvider::CADOCommand::SetParameter(XTPADODB::DataTypeEnum adDataType,
																 long nSize,
																 const _variant_t& vtValue)
{
	CreateParameter(L"", adDataType, XTPADODB::adParamInput, nSize, vtValue);
}

void CXTPCalendarDatabaseDataProvider::CADOCommand::CreateParameter(
	LPCWSTR pcszParamName, XTPADODB::DataTypeEnum adDataType,
	XTPADODB::ParameterDirectionEnum adDirection, long nSize, const _variant_t& vtValue)
{
...
void CXTPCalendarDatabaseDataProvider::CADOCommand::SetParameterValue(LPCWSTR pcszParamName,
																	  const _variant_t& vtValue)
{
	try
	{
