...
public:
	...
	virtual void AdjustLayout2(
		CDC* pDC, const CRect& rcGroup); //<COMBINE
										 // CXTPCalendarDayViewGroup::AdjustLayout@CDC*@const
										 // CRect&>
	...
	//-----------------------------------------------------------------------
	// Summary:
	//     This member function is used to check if the specified date time
	//     is work or non-work.
	// Parameters:
	...
	//-----------------------------------------------------------------------
	// Summary:
	//     This member function is used to determine if the group header
	//     is visible or not.
	// Returns:
	...
	//-----------------------------------------------------------------------
	// Summary:
	//     Call this member function to set the ExpandUp sign visible.
	// Remarks:
	...
	//-----------------------------------------------------------------------
	// Summary:
	//     Call this member function to set the ExpandDown sign visible.
	// Remarks:
	//     Can be useful to implement visualization themes.
	//-----------------------------------------------------------------------
	void SetExpandDown();
	...
	//-----------------------------------------------------------------------
	// Summary:
	//     Call this member function to determine if the ExpandUp sign is visible.
	// Remarks:
	//     Can be useful to implement visualization themes.
	// Returns:
	//     TRUE if the ExpandUp sign is visible, FALSE otherwise.
	//-----------------------------------------------------------------------
	BOOL IsExpandUp() const;

	//-----------------------------------------------------------------------
	// Summary:
	//     Call this member function to determine if the ExpandDown sign is visible.
	// Remarks:
	//     Can be useful to implement visualization themes.
	// Returns:
	//     TRUE if the ExpandDown sign is visible, FALSE otherwise.
	//-----------------------------------------------------------------------
	BOOL IsExpandDown() const;

	//-----------------------------------------------------------------------
	// Summary:
	//     Calculates the rectangle to fill with a scroll button icon for day view
	...
	//-----------------------------------------------------------------------
	// Summary:
	//     Call this member function to determine if all day events scroll icons
	//     will be shown in day view.
	// Returns:
	//     TRUE if all day events scroll icons will be shown in day view.
	//-----------------------------------------------------------------------
	BOOL NeedScrollAllDayEvents() const;

	//-----------------------------------------------------------------------
	// Summary:
	//     Call this member function to determine if the up scroll button for the
	//     alldayevents area will be shown.
	// Returns:
	//     TRUE if the up scroll button for the alldayevents area will be shown.
	//-----------------------------------------------------------------------
	BOOL IsShowAllDayEventsUpScrollButton() const;

	//-----------------------------------------------------------------------
	// Summary:
	//     Call this member function to determine if the down scroll button for the
	//     alldayevents area will be shown.
	// Returns:
	//     TRUE if the down scroll button for the alldayevents area will be shown.
	//-----------------------------------------------------------------------
	BOOL IsShowAllDayEventsDownScrollButton() const;

	//-----------------------------------------------------------------------
	// Summary:
	//     Call this member function to set the visibility for the up scroll button in
	//     the all day events area.
	//-----------------------------------------------------------------------
	void SetShowAllDayEventsUpScrollButton(BOOL bShow);

	//-----------------------------------------------------------------------
	// Summary:
	//     Call this member function to set the visibility for the down scroll button in
	//     the all day events area.
	//-----------------------------------------------------------------------
	...
	//-----------------------------------------------------------------------
	// Summary:
	//     Call this member function to determine if the up scroll icon in the
	//     all day events area is visible\highlighted.
	// Returns:
	...
	//-----------------------------------------------------------------------
	// Summary:
	//     Call this member function to determine if the down scroll icon in the
	//     all day events area is visible\highlighted.
	// Returns:
	//     TRUE if visible, FALSE otherwise.
	//-----------------------------------------------------------------------
	...
	//-----------------------------------------------------------------------
	// Summary:
	//     Call this member function to find the top and bottom events in a group.
	// Parameters:
	...
	//-----------------------------------------------------------------------
	// Summary:
	//      This member function is used to process the user action
	//      xtpCalendarUserAction_OnScrollDay.
	// Parameters:
	//      eButton - [in] a button from which this action comes.
	// Remarks:
	//      Implementation sends an XTP_NC_CALENDAR_USERACTION notification.
	//
	// Returns:
	//      TRUE if the action was handled by the user and default processing must be
	//      skipped, FALSE otherwise.
	// See Also:
	//      XTP_NC_CALENDAR_USERACTION
	//-----------------------------------------------------------------------
	BOOL UserAction_OnScrollDay(XTPCalendarScrollDayButton eButton);

	//-----------------------------------------------------------------------
	// Summary:
	//      This member function is used to process the user actions
	...
	//-----------------------------------------------------------------------
	// Summary:
	//      This member function processes click on up scroll icon in the all day events area.
	// Returns:
	//      TRUE if the action was handled by default,
	//      FALSE if the user canceled default handler.
	// See Also:
	//      XTP_NC_CALENDAR_USERACTION
	//-----------------------------------------------------------------------
	...
	//-----------------------------------------------------------------------
	// Summary:
	//      This member function processes click on down scroll icon in the all day events area.
	// Returns:
	//      TRUE if the action was handled by default,
	//      FALSE if the user canceled default handler.
	// See Also:
	...
	// -----------------------------------------------------------------------
	// Summary:
	//       This member function returns events count in the all day area of the group.
	// -----------------------------------------------------------------------
...
protected:
	...
	//     icons presses in the all days events area.
	// Parameters:
	//     nFlags  - A UINT that is used to indicate whether various virtual
	//               keys are down.
	//     point   - A CPoint that specifies the x- and y- coordinates of the cursor.
	...
	// Returns:
	//     A pointer to a CXTPCalendarViewPart object.
...
//
//          CXTPCalendarDayViewDay is based on the CXTPCalendarViewDayT template
//          class. It inherits basic behavior from the CXTPCalendarViewDayT template
//          class. Furthermore, CXTPCalendarDayViewDay overlaps and adds some
...
public:
	...
	// Remarks:
	//     Constructs a CXTPCalendarDayViewDay object.
	// See Also:
	//     ~CXTPCalendarDayViewDay()
	//-----------------------------------------------------------------------
	...
	//-----------------------------------------------------------------------
	virtual void Draw(CDC* pDC);
	virtual void DrawEvents(CDC* pDC);		 // <combine CXTPCalendarDayViewDay::Draw@CDC*>
	virtual void DrawTopmostLayer(CDC* pDC); // <combine CXTPCalendarDayViewDay::Draw@CDC*>
	...
	virtual void AdjustLayout2(
		CDC* pDC, const CRect& rcDay); //<COMBINE
									   // CXTPCalendarDayViewDay::AdjustLayout@CDC*@const
									   // CRect&>
