...
union XTP_EDIT_OPERATION_PARAMS {
	XTP_EVENT_EDIT_OPERATION_PARAMS header;
	XTP_EVENT_PASTE_OPERATION_PARAMS pasteParams;
	XTP_EVENT_IN_PLACE_CREATE_EVENT_OPERATION_PARAMS inPlaceCreateEventParams;
	XTP_EVENT_DRAG_OPERATION_PARAMS dragOperationParams;
};
...
public:
	...
	{
		CXTPCalendarEventPtr ptrPrevCopy;
		if (pPrev)
			ptrPrevCopy = pPrev->CloneEvent();

		CXTPCalendarEventPtr ptrNewCopy;
		if (pNew)
			ptrNewCopy = pNew->CloneEvent();
...
#ifdef _XTP_ACTIVEX
	XTP_SAFE_CALL1(m_pSelOleWrapper,
				   Detach()); // clear internal reference to CXTPCalendarView owner object.
...
void CXTPCalendarView::OnActivateView(BOOL bActivate, CXTPCalendarView* pActivateView,
...
BOOL CXTPCalendarView::OnLButtonUp(UINT nFlags, CPoint point)
{
	...
	if (GetViewType() == xtpCalendarTimeLineView)
	{
		CXTPCalendarTimeLineView* pView =
			(CXTPCalendarTimeLineView*)GetCalendarControl()->GetActiveView();
		if (pView)
	...
	if (!OnEndDragging(point, &hitInfo) && !m_pControl->IsReadonlyMode())
	{
		if (!bHandled && bHitTest)
		{
			...
			if ((hitInfo.uHitCode & uESmask) && !IsEditingSubject() && hitInfo.pViewEvent
				&& hitInfo.pViewEvent->IsSelected())
			{
				BOOL bEnabled = XTP_SAFE_GET1(GetCalendarOptions(),
											  bEnableInPlaceEditEventSubject_ByMouseClick, FALSE);
				bEnabled	  = bEnabled && IsSingleEventSelected();
				if (bEnabled)
					bEnabled = !OnBeforeEditOperationNotify(xtpCalendarEO_EditSubject_ByMouseClick,
															hitInfo.pViewEvent);
	...
	if (bHitTest && hitInfo != m_LastHitInfo)
	{
		...
		else if (nFlags & MK_LBUTTON && m_bStartedClickInside)
		{
				...
				|| abs(point.y - m_ptStartDragging.y) >= XTP_DPI_Y(cEpsilon))
			{
					...
					&& m_eDraggingMode == xtpCalendaDragModeUnknown)
				{
					...
					if (HitTest(m_ptStartDragging, &hitInfoSD))
					{
						if (hitInfoSD.uHitCode & xtpCalendarHitTestEvent_Mask)
						{
							...
							if (!IsEditOperationDisabledNotify(_DragMod2Operation(eDragMode),
															   hitInfoSD.pViewEvent))
				...
				BOOL bDragging = !!m_ptrDraggingEventNew
				...
				if (bDragging)
				{
					...
					if (bDragged)
					{
						XTP_EDIT_OPERATION_PARAMS* pOpParamsDrag = new XTP_EDIT_OPERATION_PARAMS;
						memset(pOpParamsDrag, 0, sizeof(*pOpParamsDrag));

						pOpParamsDrag->header.eOperation = _DragMod2Operation(m_eDraggingMode);
						++pOpParamsDrag->header.nRefCounter;

						pOpParamsDrag->dragOperationParams.pDraggingEventOriginal =
							m_ptrDraggingEventOrig;
						pOpParamsDrag->dragOperationParams.pDraggingEvent	= ptrDragEventPrev;
						pOpParamsDrag->dragOperationParams.pDraggingEventNew = m_ptrDraggingEventNew;

						BOOL bHandled = FALSE;
						if (m_pControl)
						{
							m_pControl->SendNotification(XTP_NC_CALENDAR_BEFORE_EVENT_EDIT_OPERATION,
														 (WPARAM)pOpParamsDrag, (LPARAM)&bHandled);
						}

						if (0 == --pOpParamsDrag->header.nRefCounter)
						{
							delete pOpParamsDrag;
						}
						...
						CXTPCalendarControl::CUpdateContext updateContext1(
							m_pControl, xtpCalendarUpdateLayout | xtpCalendarUpdateRedraw);

						if (!!m_ptrDraggingEventNew && !!m_ptrDraggingEventOrig
							&& (!m_ptrDraggingEventNew->IsEqualStartEnd(m_ptrDraggingEventOrig)
								|| m_ptrDraggingEventNew->GetScheduleID()
			...
			{
				if ((hitInfo.uHitCode & xtpCalendarHitTestEvent_Mask) && hitInfo.pViewEvent
					&& hitInfo.pViewEvent->IsSelected())
				{
				}
				else
	...
	if (GetCalendarControl())
	{
			...
			&& GetCalendarControl()->m_mouseMode != xtpCalendarMouseEventDraggingOut)
		{
			GetCalendarControl()->m_mouseMode = m_eDraggingMode == xtpCalendaDragModeCopy
													? xtpCalendarMouseEventDragCopy
													: xtpCalendarMouseEventDragMove;
...
void CXTPCalendarView::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	if (nChar == VK_RETURN || nChar == VK_ESCAPE)
	{
		if (IsEditingSubject())
		{
			...
			EndEditSubject((nChar == VK_RETURN) ? xtpCalendarEditSubjectCommit
												: xtpCalendarEditSubjectCancel);
		...
		if (nChar == VK_ESCAPE && !!m_ptrDraggingEventNew)
		{
			...
			{
				CXTPCalendarControl::CUpdateContext updateContext1(GetCalendarControl(),
	...
	else if (nChar == VK_F2)
	{
		...
		if (GetViewType() == xtpCalendarDayView || GetViewType() == xtpCalendarWorkWeekView
			|| GetViewType() == xtpCalendarFullWeekView)
		{
			...
			if (HitTest(point, &hitInfo))
			{
				if (pViewEvent)
				{
					...
					if (pEvent)
						pViewEvent = FindViewEventByGroup(pEvent, pEvent->GetStartTime(),
														  hitInfo.nGroup);
				}
			}
		}

		BOOL bEnabled = XTP_SAFE_GET1(GetCalendarOptions(), bEnableInPlaceEditEventSubject_ByF2,
									  FALSE);
	...
	else if (nChar == VK_DELETE)
	{
		if (m_eDraggingMode == xtpCalendaDragModeUnknown && !m_ptrEditingViewEvent)
		{
			...
			if (m_selectedBlock.dtEnd.GetStatus() == COleDateTime::valid && GetCalendarControl()
				&& pViewEvent && pViewEvent->IsSelected())
			{
				CXTPCalendarControl::CUpdateContext updateContext1(GetCalendarControl(),
																   xtpCalendarUpdateRedraw
																	   | xtpCalendarUpdateLayout);
	...
	else if (nChar == VK_TAB)
	{
			...
			|| m_eDraggingMode == xtpCalendaDragModeEditSubject)
		{
			...
			if (pViewEvent && pViewEvent->IsVisible() && pViewEvent->GetEvent())
			{
				...
				BOOL bEnabled = XTP_SAFE_GET1(GetCalendarOptions(),
											  bEnableInPlaceEditEventSubject_ByTab, FALSE);
				if (bEnabled)
				{
					bEnabled = !OnBeforeEditOperationNotify(xtpCalendarEO_EditSubject_ByTab,
															pViewEvent);
				}
...
BOOL CXTPCalendarView::CommitDraggingEvent()
{
	...
	CXTPAutoResetValue<BOOL> autoSet_UpdateCtrlNf(
		GetCalendarControl()->m_bUpdateWhenEventChangedNotify);
	autoSet_UpdateCtrlNf = FALSE;

	if (!!m_ptrDraggingEventNew && !!m_ptrDraggingEventOrig)
	{
		BOOL bDPdiff = m_ptrDraggingEventNew->GetDataProvider()
					   != m_ptrDraggingEventOrig->GetDataProvider();

		BOOL bChanged = !m_ptrDraggingEventNew->IsEqualStartEnd(m_ptrDraggingEventOrig);
		bChanged	  = bChanged
				   || m_ptrDraggingEventNew->GetSubject() != m_ptrDraggingEventOrig->GetSubject();
		bChanged = bChanged
				   || m_ptrDraggingEventNew->IsAllDayEvent()
						  != m_ptrDraggingEventOrig->IsAllDayEvent();
		bChanged = bChanged
				   || m_ptrDraggingEventNew->GetScheduleID()
						  != m_ptrDraggingEventOrig->GetScheduleID();
		...
		if (bChanged || m_eDraggingMode == xtpCalendaDragModeCopy)
		{
			...
			CXTPCalendarData* pDataNew =
				m_ptrDraggingEventNew
					->GetDataProvider(); // _GetDataProviderForDraggingEvent(m_ptrDraggingEventNew);
			CXTPCalendarData* pDataOrig = m_ptrDraggingEventOrig->GetDataProvider();

			if (m_eDraggingMode == xtpCalendaDragModeCopy)
			{
				if (m_ptrDraggingEventNew->GetRecurrenceState()
														  ...
														  COleDateTime dtDay)
{
	...
	CXTPCalendarEventsPtr ptrEvents = pRC ? pRC->RetrieveDayEvents(dtDay).Detach()
	...
	COleDateTime dtDay1 = GetDateTimeDay(m_ptrDraggingEventNew->GetStartTime(),
										 m_ptrDraggingEventNew->IsAllDayEvent());
	COleDateTime dtDay2 = GetDateTimeDay(m_ptrDraggingEventNew->GetEndTime(),
										 m_ptrDraggingEventNew->IsAllDayEvent());

	UINT uSchID					  = m_ptrDraggingEventNew->GetScheduleID();
	BOOL bNewEventScheduleVisible = m_ptrDraggingEventNew->GetDataProvider()
										== pRC->GetDataProvider()
...
void CXTPCalendarView::OnStartDragging(CPoint /*point*/, XTP_CALENDAR_HITTESTINFO* pHitTest)
{
	...
	if (_IsDragModeCopyMove(m_eDraggingMode))
	{
		...
		{
			m_spDraggingStartOffset = pHitTest->dt
									  - pHitTest->pViewEvent->GetEvent()->GetStartTime();
		}
		else
		{
			m_spDraggingStartOffset = CXTPCalendarUtils::ResetTime(pHitTest->dt)
									  - CXTPCalendarUtils::ResetTime(
											pHitTest->pViewEvent->GetEvent()->GetStartTime());
		}
...
BOOL CXTPCalendarView::OnEndDragging(CPoint point, XTP_CALENDAR_HITTESTINFO* phitInfo)
{
	...
	if (!!ptrEventToSelect
		&& (nDragMode == xtpCalendaDragModeResizeBegin || nDragMode == xtpCalendaDragModeResizeEnd))
	{
		CXTPCalendarControl::CUpdateContext updateContext(m_pControl, xtpCalendarUpdateRedraw);

		COleDateTime dtFVDate = GetDateTimeDay(ptrEventToSelect->GetStartTime(),
											   ptrEventToSelect->IsAllDayEvent());
		...
		if (pViewEvent && pViewEvent->GetEvent())
		{
			...
			BOOL bEnabled = XTP_SAFE_GET1(GetCalendarOptions(),
										  bEnableInPlaceEditEventSubject_AfterEventResize, FALSE);
			if (bEnabled)
				bEnabled = !OnBeforeEditOperationNotify(xtpCalendarEO_EditSubject_AfterEventResize,
														pViewEvent);
...
CXTPCalendarViewEvent* CXTPCalendarView::FindViewEvent(CXTPCalendarEvent* pEvent,
...
BOOL CXTPCalendarView::GetSelection(COleDateTime* pBegin, COleDateTime* pEnd, BOOL* pbAllDayEvent,
									int* pnGroupIndex,
...
void CXTPCalendarView::UpdateNextTimeEditByTAB(COleDateTime dtNext, BOOL /*bReverse*/,
											   BOOL /*bReset*/)
{
															  ...
															  CXTPCalendarEvent* pAfterEvent)
{
	...
	if (pAfterEvent)
	{
		COleDateTime dtAEStartDay = GetDateTimeDay(pAfterEvent->GetStartTime(),
												   pAfterEvent->IsAllDayEvent());
		 ...
		 bReverse ? nDay-- : nDay++)
	{
		...
		for (int nGroup = bReverse ? nGroupsCount - 1 : 0;
			 bReverse ? nGroup >= 0 : nGroup < nGroupsCount; bReverse ? nGroup-- : nGroup++)
		{
			...
			for (int nEvent = bReverse ? nEvents - 1 : 0; bReverse ? nEvent >= 0 : nEvent < nEvents;
...
void CXTPCalendarView::UnselectAllEvents()
{
	...
	if (nCount)
	{
		CXTPCalendarControl::CUpdateContext updateContext(GetCalendarControl(),
														  xtpCalendarUpdateRedraw);
...
BOOL CXTPCalendarView::OnDragging(CPoint /*point*/, XTP_CALENDAR_HITTESTINFO* pHitTest)
{
	...
	if (_IsDragModeCopyMove(m_eDraggingMode))
	{
		if (!pHitTest->bTimePartValid)
		{
			dtNewTime = CXTPCalendarUtils::UpdateDate(m_ptrDraggingEventNew->GetStartTime(),
													  pHitTest->dt);
		}
		else
		{
	...
	else if (m_eDraggingMode == xtpCalendaDragModeResizeBegin)
	{
		...
		if (!CXTPCalendarUtils::IsEqual(dtNewTime, m_ptrDraggingEventNew->GetStartTime()))
		{
			...
			BOOL bIsZeroStartEnd =
				CXTPCalendarUtils::IsZeroTime(m_ptrDraggingEventNew->GetStartTime())
	...
	else if (m_eDraggingMode == xtpCalendaDragModeResizeEnd)
	{
		...
		if (!CXTPCalendarUtils::IsEqual(dtNewTime, m_ptrDraggingEventNew->GetEndTime()))
		{
			...
			{
				dtNewTime.SetDateTime(dtEventStart.GetYear(), dtEventStart.GetMonth(),
									  dtEventStart.GetDay(), dtNewTime.GetHour(),
									  dtNewTime.GetMinute(), dtNewTime.GetSecond());
			}
...
BOOL CXTPCalendarView::OnTimer(UINT_PTR uTimerID)
{
	...
	{
		BOOL bIsCtrl						  = (GetKeyState(VK_CONTROL) < 0);
		XTPCalendarDraggingMode eDraggingMode = bIsCtrl ? xtpCalendaDragModeCopy
														: xtpCalendaDragModeMove;
		...
		else if (m_eDraggingMode != eDraggingMode)
		{
			...
			if (m_ptrDraggingEventNew)
			{
				CXTPCalendarViewEvent* pEventView =
					FindViewEvent(m_ptrDraggingEventNew, m_ptrDraggingEventNew->GetStartTime());
...
void CXTPCalendarView::StartEditNewEventInplace(LPCTSTR pcszInitialSubject)
{
	...
	m_ptrDraggingEventNew = pData ? pData->CreateNewEvent() : CXTPCalendarEventPtr(NULL);
	if (!m_ptrDraggingEventNew)
		return;
	...
	m_ptrDraggingEventNew->SetBusyStatus(bAllDayEvent ? xtpCalendarBusyStatusFree
...
CRect CXTPCalendarView::GetTooltipRect(const CPoint& ptHit,
	...
	if (hitInfo.pViewGroup)
	{
		...
		if (pThemeX)
		{
			CRect rcExclude = XTP_SAFE_GET2(pThemeX->GetPrevNextEventButtonsPartX(),
											GetNextEventButtonPartX(), GetRect(),
											CRect(0, 0, 0, 0));
...
void CXTPCalendarView::ShowToolTip(BOOL bShow)
{
	...
	if (GetViewType() != xtpCalendarTimeLineView)
	{
		...
		BOOL bTipEnabled  = XTP_SAFE_GET2(pControl, GetCalendarOptions(), bEnableAddNewTooltip,
										  TRUE);
		BOOL bAllowCreate = XTP_SAFE_GET2(pControl, GetCalendarOptions(), bEnableInPlaceCreateEvent,
										  TRUE);
		...
		if (!pControl->m_bReadOnlyMode)
			strTip = XTP_SAFE_GET2(pControl, GetCalendarOptions(), strTooltipAddNewText,
								   CString(_T("Click to add appointment")));
		else
			strTip.Empty();
	}
	else if (hitInfo.pViewEvent) // TimeLine mode
	{
		if (hitInfo.pViewEvent->GetEvent() && hitInfo.pViewEvent->GetEvent()->IsEventVisible())
			strTip = hitInfo.pViewEvent->GetEvent()->GetSubject();
	}
	// show tooltip window
	if (pControl)
	{
		CFont* pFont = &XTP_SAFE_GET2(GetPaintManager(), GetDayViewEventPart(), GetTextFont(),
									  *((CFont*)(NULL)));
...
BOOL CXTPCalendarView::_Undo(CXTPCalendarEvent* pEvent)
{
	...
	else
	{
		...
		if (ptrUndoEvent)
		{
			...
			else if (m_eUndoMode == xtpCalendarUndoModeRedo)
			{
...
BOOL CXTPCalendarView::_CanSubjectEditor(UINT idEditCmd) const
{
	...
	switch (idEditCmd)
	{
		case ID_EDIT_UNDO: return pSEditor->CanUndo();
		case ID_EDIT_CUT: return nSel1 < nSel2;
		case ID_EDIT_COPY: return nSel1 < nSel2;
		case ID_EDIT_PASTE: return ::IsClipboardFormatAvailable(CF_TEXT);
	}
...
int CXTPCalendarView::_GetMaxWidth(CDC* pDC, CString strFormat) const
{
	...
	const TCHAR* chFormats[] = { _T("yyyy"), _T("yyy"),  _T("yy"),  _T("y"),  _T("MMMM"),
								 _T("MMM"),  _T("dddd"), _T("ddd"), _T("dd"), _T("d") };
...
CString CXTPCalendarView::_GetDayHeaderFormat(int nLevel) const
{
	...
	{
		case 0:
			return m_strDayHeaderFormatLong.IsEmpty() ? m_strDayHeaderFormatDefaultLong
													  : m_strDayHeaderFormatLong;
		case 1:
			return m_strDayHeaderFormatMiddle.IsEmpty() ? m_strDayHeaderFormatDefaultMiddle
														: m_strDayHeaderFormatMiddle;
		case 2:
			return m_strDayHeaderFormatShort.IsEmpty() ? m_strDayHeaderFormatDefaultShort
													   : m_strDayHeaderFormatShort;
		case 3:
			return m_strDayHeaderFormatShortest.IsEmpty() ? m_strDayHeaderFormatDefaultShortest
														  : m_strDayHeaderFormatShortest;
	}
...
CXTPCalendarEventsPtr CXTPCalendarView::_ReadEventsFromClipboard(COleDataObject* pData,
																 DWORD* pdwCopyFlags)
{
	//- Register or get existing Clipboard Format ID -------------------------
	...
	try
	{
		...
		while (pos)
		{
			...
			if (pDataProvider)
				ptrEvent = pDataProvider->CreateNewEvent();
	...
	{
		TRACE(
			_T("EXCEPTION: CXTPCalendarView::_ReadEventsFromClipboard() - Unhandled Exception!\n"));
	}
...
COleDataSource* CXTPCalendarView::_CopySelectedEvents(DWORD dwCopyFlags)
{
	...
	try
	{
		...
		for (int i = 0; i < nCount; i++)
		{
			...
			CString strDataProvider = XTP_SAFE_GET2(pEvent, GetDataProvider(),
...
BOOL CXTPCalendarView::IsEditOperationDisabledNotify(XTPCalendarEditOperation eOperation,
													 CXTPCalendarViewEvent* pEventView) const
{
	return _EditOperationNotify(XTP_NC_CALENDAR_IS_EVENT_EDIT_OPERATION_DISABLED, eOperation,
								pEventView);
}

BOOL CXTPCalendarView::OnBeforeEditOperationNotify(XTPCalendarEditOperation eOperation,
												   CXTPCalendarViewEvent* pEventView)
{
	if (IsEditOperationDisabledNotify(eOperation, pEventView))
		return TRUE;

	return _EditOperationNotify(XTP_NC_CALENDAR_BEFORE_EVENT_EDIT_OPERATION, eOperation,
								pEventView);
}
...
{
	if (_EditOperationNotify(XTP_NC_CALENDAR_IS_EVENT_EDIT_OPERATION_DISABLED, eOperation, NULL,
							 pEventViews))
		return TRUE;

	return _EditOperationNotify(XTP_NC_CALENDAR_BEFORE_EVENT_EDIT_OPERATION, eOperation, NULL,
								pEventViews);
}

BOOL CXTPCalendarView::OnBeforePasteNotify(CXTPCalendarEvent* pEvent,
										   BOOL bPasteWillChangeExistingEvent)
{
	XTP_EDIT_OPERATION_PARAMS* pOpPasteParams = new XTP_EDIT_OPERATION_PARAMS;
	memset(pOpPasteParams, 0, sizeof(*pOpPasteParams));

	pOpPasteParams->header.eOperation = xtpCalendarEO_Paste;
	++pOpPasteParams->header.nRefCounter;

	pOpPasteParams->pasteParams.pEvent						  = pEvent;
	pOpPasteParams->pasteParams.bPasteWillChangeExistingEvent = bPasteWillChangeExistingEvent;
	...
	if (m_pControl)
	{
		m_pControl->SendNotification(XTP_NC_CALENDAR_IS_EVENT_EDIT_OPERATION_DISABLED,
									 (WPARAM)pOpPasteParams, (LPARAM)&bDisabledHandled);
	}
	...
	if (0 == --pOpPasteParams->header.nRefCounter)
	{
		delete pOpPasteParams;
	}
...
{
	XTP_EDIT_OPERATION_PARAMS* pOpParams = new XTP_EDIT_OPERATION_PARAMS;
	memset(pOpParams, 0, sizeof(*pOpParams));

	pOpParams->header.eOperation = xtpCalendarEO_InPlaceCreateEvent;
	++pOpParams->header.nRefCounter;
	pOpParams->inPlaceCreateEventParams.pcszInitialSubject = pcszInitialSubject;
	...
	{
		m_pControl->SendNotification(XTP_NC_CALENDAR_IS_EVENT_EDIT_OPERATION_DISABLED,
									 (WPARAM)pOpParams, (LPARAM)&bDisabledHandled);
	}
	...
	if (m_pControl)
	{
		m_pControl->SendNotification(XTP_NC_CALENDAR_BEFORE_EVENT_EDIT_OPERATION, (WPARAM)pOpParams,
									 (LPARAM)&bDisabledHandled);
	}

	if (0 == --pOpParams->header.nRefCounter)
	{
		delete pOpParams;
	}
											...
											CXTPCalendarViewEvents* pEventViews) const
{
	...
	BOOL bReleaseEvents = FALSE;

	pOpParams->header.eOperation = eOperation;
	++pOpParams->header.nRefCounter;

	if (NULL != pEventViews)
	{
		pOpParams->header.pEventViews = pEventViews;
	}
	else
	{
		CXTPCalendarViewEvents* pEvents = new CXTPCalendarViewEvents();
		pEvents->Add(pEventView, TRUE);
		pOpParams->header.pEventViews = pEvents;
		bReleaseEvents				  = TRUE;
	}
	...
	if (m_pControl)
	{
		m_pControl->SendNotification(ncEvent, (WPARAM)pOpParams, (LPARAM)&bDisabledHandled);
	}

	if (bReleaseEvents)
	{
		pOpParams->header.pEventViews->InternalRelease();
	}
	...
	XTP_NOTIFY_CODE nfCode, CXTPCalendarEvent* pEvent)
{
	...
	if (nfCode != XTP_NC_CALENDAREVENTWASADDED && nfCode != XTP_NC_CALENDAREVENTWASDELETED
		&& nfCode != XTP_NC_CALENDAREVENTWASCHANGED)
		return xtpCalendar_Skip;

	int nDaysCount			= GetViewDayCount();
	COleDateTime dtFirstDay = nDaysCount ? XTP_SAFE_GET1(GetViewDay_(0),
														 GetDayDate().operator DATE(), (DATE)0)
										 : (DATE)0;
	COleDateTime dtLastDay = nDaysCount ? XTP_SAFE_GET1(GetViewDay_(nDaysCount - 1),
														GetDayDate().operator DATE(), (DATE)0)
										: (DATE)0;
...
CXTPCalendarData* CXTPCalendarView::_GetDataProviderByConnStr(LPCTSTR pcszConnStr,
															  BOOL bCompareNoCase)
{
	...
	for (int g = 0; g < nGCount; g++)
	{
		...
		if (pViewGroup)
		{
			...
			if (pData)
			{
				...
				int nCmp		  = bCompareNoCase ? strConn_i.CompareNoCase(pcszConnStr)
										  : strConn_i.Compare(pcszConnStr);
				if (nCmp == 0)
					return pData;
			}
		}
...
BOOL CXTPCalendarView::_IsScheduleVisible(CXTPCalendarData* pDataProvider, UINT uScheduleID)
{
	...
	for (int g = 0; g < nGCount; g++)
	{
		...
		if (pData && pData == pDataProvider)
		{
			...
			{
				if (XTP_SAFE_GET3(pViewGroup, GetResources(), GetAt(i),
								  ExistsScheduleID(uScheduleID), FALSE))
					return TRUE;
			}
		}
	}
...
//
CXTPCalendarView::CSelectionChangedContext::CSelectionChangedContext(CXTPCalendarView* pView,
																	 int eSelType)
{
	if (!pView)
	{
...
CXTPCalendarView::CSelectionChangedContext::~CSelectionChangedContext()
{
	...
	if (m_pView->m_cntSelChanged.m_nLockCount == 0 && m_pView->m_cntSelChanged.m_bRequest_Day)
	{
		...
		m_pView->SendNotification(XTP_NC_CALENDAR_SELECTION_CHANGED,
								  (WPARAM)xtpCalendarSelectionDays);
	}
	if (m_pView->m_cntSelChanged.m_nLockCount == 0 && m_pView->m_cntSelChanged.m_bRequest_Event)
	{
		...
		m_pView->SendNotification(XTP_NC_CALENDAR_SELECTION_CHANGED,
								  (WPARAM)xtpCalendarSelectionEvents);
	}
}
...
BEGIN_DISPATCH_MAP(CXTPCalendarViewSelection, CXTPCmdTarget)
	...
	DISP_PROPERTY_EX_ID(CXTPCalendarViewSelection, "GroupIndex", 4, OleGetGroupIndex,
						OleSetGroupIndex, VT_I4)
	DISP_FUNCTION_ID(CXTPCalendarViewSelection, "IsValid", 10, OleIsValid, VT_BOOL, VTS_NONE)

	DISP_PROPERTY_EX_ID(CXTPCalendarViewSelection, "BeginV", 36, OleGetBeginV, SetNotSupported,
						VT_DISPATCH)
	DISP_PROPERTY_EX_ID(CXTPCalendarViewSelection, "EndV", 37, OleGetEndV, SetNotSupported,
						VT_DISPATCH)
...
//{{AFX_CODEJOCK_PRIVATE
class CXTPCalendarHitTestInfo
	: public CXTPCmdTarget
	, public XTP_CALENDAR_HITTESTINFO
{
public:
...
BEGIN_DISPATCH_MAP(CXTPCalendarView, CXTPCalendarWMHandler)
	...
	DISP_PROPERTY_ID(CXTPCalendarView, "DayHeaderFormatMiddle", 55, m_strDayHeaderFormatMiddle,
					 VT_BSTR)
	DISP_PROPERTY_ID(CXTPCalendarView, "DayHeaderFormatShort", 60, m_strDayHeaderFormatShort,
					 VT_BSTR)
	DISP_PROPERTY_ID(CXTPCalendarView, "DayHeaderFormatShortest", 65, m_strDayHeaderFormatShortest,
					 VT_BSTR)

	DISP_FUNCTION_ID(CXTPCalendarView, "ResetSelection", 98, OleResetSelection, VT_EMPTY, VTS_NONE)
	DISP_FUNCTION_ID(CXTPCalendarView, "SetSelection", 99, OleSetSelection, VT_EMPTY,
					 VTS_DATE VTS_DATE VTS_BOOL)
	DISP_FUNCTION_ID(CXTPCalendarView, "GetSelection", 100, OleGetSelection, VT_BOOL,
					 VTS_PDATE VTS_PDATE VTS_PBOOL)
	DISP_FUNCTION_ID(CXTPCalendarView, "GetSelectionV", 150, OleGetSelectionV, VT_BOOL,
	...
	DISP_FUNCTION_ID(CXTPCalendarView, "GetSelectedEvents", 110, OleGetSelectedEvents, VT_DISPATCH,
					 VTS_NONE)
	DISP_FUNCTION_ID(CXTPCalendarView, "SelectViewEvent", 111, OleSelectViewEvent, VT_EMPTY,
					 VTS_DISPATCH VTS_BOOL)
	DISP_FUNCTION_ID(CXTPCalendarView, "UnselectAllEvents", 112, OleUnselectAllEvents, VT_EMPTY,
					 VTS_NONE)
	...
	DISP_FUNCTION_ID(CXTPCalendarView, "MultipleResources", 130, OleGetMultipleResources,
					 VT_DISPATCH, VTS_NONE)
	DISP_FUNCTION_ID(CXTPCalendarView, "SetMultipleResources", 131, OleSetMultipleResources,
...
BOOL CXTPCalendarView::OleGetSelection(DATE* pBegin, DATE* pEnd, VARIANT_BOOL* pvbAllDay)
{
...
BOOL CXTPCalendarView::OleGetSelectionV(VARIANT* pBegin, VARIANT* pEnd, VARIANT* pvbAllDay)
{
...
void CXTPCalendarView::OleSelectViewEvent(LPDISPATCH pViewEventDisp, BOOL bSelect)
{
	CXTPCalendarViewEvent* pViewEvent = XTP_DYNAMIC_DOWNCAST_REMOTE_(
		CXTPCalendarViewEvent, CXTPCalendarViewEvent::FromIDispatchSafe(pViewEventDisp));
	if (!pViewEvent)
	{
...
BEGIN_DISPATCH_MAP(CXTPCalendarHitTestInfo, CXTPCmdTarget)
	DISP_PROPERTY_EX_ID(CXTPCalendarHitTestInfo, "ViewEvent", 1, OleGetViewEvent, SetNotSupported,
						VT_DISPATCH)
	DISP_PROPERTY_EX_ID(CXTPCalendarHitTestInfo, "ViewDay", 2, OleGetViewDay, SetNotSupported,
						VT_DISPATCH)
	DISP_PROPERTY_EX_ID(CXTPCalendarHitTestInfo, "HitCode", 3, OleGetHitCode, SetNotSupported,
						VT_I4)
	DISP_FUNCTION_ID(CXTPCalendarHitTestInfo, "HitDateTime", 4, OleGetHitDateTime, VT_DATE,
					 VTS_NONE)
	DISP_FUNCTION_ID(CXTPCalendarHitTestInfo, "TimePartValid", 5, OleGetTimePartValid, VT_BOOL,
					 VTS_NONE)
	DISP_PROPERTY_EX_ID(CXTPCalendarHitTestInfo, "ViewGroup", 6, OleGetViewGroup, SetNotSupported,
						VT_DISPATCH)
