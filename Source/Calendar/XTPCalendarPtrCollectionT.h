...
public:
	...
	//     the largest index.
	// Example:
	//     See example for CXTPCalendarEvents::Add method.
	// Returns:
	...
	// </code>
	// See Also:
	//     CXTPCalendarPtrCollectionT overview, GetAt, RemoveAll, GetCount
	//-----------------------------------------------------------------------
	...
	//-----------------------------------------------------------------------
	// Summary:
	//      This member function finds an element index in the collection using
	//      its pointer.
	// Parameters:
	...
	// Example:
	//     See the example for the Add() method.
	// Returns:
	...
	//              and less than the value returned by GetCount().
	// Remarks:
	//     Removes the pointer from this array and releases the instance
...
//          array is constant and is independent of the array size.
//
//          Overridden method RemoveAll() calls the delete operator for all non-NULL
//          elements before calling the RemoveAll() method from the base class.
//
//          The RemoveAll() method is called from the array destructor too.
//
// See Also:
//     CTypedPtrArray, CPtrArray, CObArray, CArray
//===========================================================================
...
public:
	...
	virtual void RemoveAll()
	{
		for (int i = 0; i < TBase::GetSize(); i++)
		{
			PTR_TYPE pObj = TBase::GetAt(i);
			if (pObj)
			{
				delete pObj;
				TBase::SetAt(i, NULL);
...
//
//          Overridden method RemoveAll() calls the delete operator for all non-NULL
//          spored elements before calling the RemoveAll() method from the base class.
//
//          The RemoveAll() method is called from the map destructor too.
//
// See Also:
//     CMap
//===========================================================================
...
public:
	...
	// Parameters:
	//     bDeletePtr - If TRUE, delete operator will be called for non-null
	//                  elements. Default value is TRUE.
	// Remarks:
	//     Removes all data from the map and deletes all stored objects if
	//     this is specified by the bDeletePtr parameter.
	//-----------------------------------------------------------------------
	virtual void RemoveAll(BOOL bDeletePtr = TRUE)
	{
		if (bDeletePtr)
		{
			POSITION pos = TBase::GetStartPosition();
			while (pos)
			{
				KEY tmpKey;
				TPtr pValue = NULL;
				TBase::GetNextAssoc(pos, tmpKey, pValue);
				if (pValue)
					delete pValue;
			}
		}
...
private:
	...
	{
		TPtr pValPrev = NULL;
		if (TBase::Lookup(key, pValPrev) && pValPrev)
		{
			delete pValPrev;
		}
	}
