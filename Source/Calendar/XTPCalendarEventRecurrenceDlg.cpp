...
BEGIN_MESSAGE_MAP(CXTPCalendarEventRecurrenceDlg, CDialog)
	...
	ON_CONTROL_RANGE(BN_CLICKED, XTP_IDC_RADIO_DAILY, XTP_IDC_RADIO_YEARLY,
					 OnBnClickedRadioDailyYearly)
	ON_WM_TIMER()
END_MESSAGE_MAP()
...
void CXTPCalendarEventRecurrenceDlg::InitStartEndCB()
{
	...
	if (!bInListStart)
	{
		strTime = CXTPCalendarUtils::GetTimeFormat(m_dtStart.GetHour(), m_dtStart.GetMinute(),
												   strTimeFormat_HrMin);
	...
	if (!bInListEnd)
	{
		strTime = CXTPCalendarUtils::GetTimeFormat(m_dtEnd.GetHour(), m_dtEnd.GetMinute(),
												   strTimeFormat_HrMin);
		m_wndCbEnd.SetWindowText(strTime);
	}
}

int CXTPCalendarEventRecurrenceDlg::_AddString(CComboBox& wndCB, UINT nStrResID,
...
int CXTPCalendarEventRecurrenceDlg::_AddLocaleString(CComboBox& wndCB, LCTYPE lcidStr,
													 DWORD_PTR dwItemData)
{
...
BOOL CXTPCalendarEventRecurrenceDlg::OnInitDialog()
{
	...
	switch (UseEnd)
	{
		...
		case xtpCalendarPatternEndAfterOccurrences:
			...
			break;
		case xtpCalendarPatternEndNoDate: nRadio = XTP_IDC_RADIO_NOEND; break;
	}
	...
	switch (nRecType)
	{
		...
		case xtpCalendarRecurrenceYearNth:
			...
			break;
		default: ASSERT(nRecType == xtpCalendarRecurrenceUnknown);
	}

	CheckRadioButton(XTP_IDC_RADIO_DAILY, XTP_IDC_RADIO_YEARLY, nRadio);
	OnBnClickedRadioDailyYearly(XTPToUInt(nRadio));
...
void CXTPCalendarEventRecurrenceDlg::_OnStartChanged()
{
	...
	CString strTimeFormat_HrMin = CXTPCalendarUtils::GetTimeFormatString_HrMin();
	CString strTmp = CXTPCalendarUtils::GetTimeFormat(m_dtEnd.GetHour(), m_dtEnd.GetMinute(),
													  strTimeFormat_HrMin);
...
{
	CWnd* pDlgItem = GetDlgItem(XTPToInt(nCtrlID));
	if (NULL != pDlgItem)
	{
		pDlgItem->SetFocus();
	}
...
void CXTPCalendarEventRecurrenceDlg::OnOK()
{
	...
	{
		// Any exceptions associated with this recurring appointment will be lost. If any of the
		// exceptions are meetings, the attendees will not be notified. Is this OK?
		int nRes = XTPResourceManager()->ShowMessageBox(XTP_IDS_CALENDAR_REMOVE_EXCEPTIONS,
														MB_OKCANCEL);
		if (nRes != IDOK)
		{
			return;
	...
	switch (nRadio)
	{
		...
		case XTP_IDC_RADIO_MONTHLY:
			...
			else
			{
				...
				opt.m_MonthNth.nWhichDayMask   = (int)m_wndCbMonthDay.GetItemData(
					  m_wndCbMonthDay.GetCurSel());

				opt.m_MonthNth.nWhichDay = (int)m_wndCbMonthWhich.GetItemData(
					m_wndCbMonthWhich.GetCurSel());
			}
		...
		case XTP_IDC_RADIO_YEARLY:
				...
				== XTP_IDC_RADIO_YEAR_RECUR)
			{
				...
				opt.m_Yearly.nMonthOfYear = (int)m_wndYearMonth.GetItemData(
					m_wndYearMonth.GetCurSel());
			}
			else
			{
				opt.m_nRecurrenceType = xtpCalendarRecurrenceYearNth;

				opt.m_YearNth.nMonthOfYear = (int)m_wndComboYearMonth2.GetItemData(
					m_wndComboYearMonth2.GetCurSel());

				opt.m_YearNth.nWhichDay = (int)m_wndCbYearWhich.GetItemData(
					m_wndCbYearWhich.GetCurSel());

				opt.m_YearNth.nWhichDayMask = (int)m_wndCbYearDay.GetItemData(
					m_wndCbYearDay.GetCurSel());
			}
	...
	switch (nRadio)
	{
		...
		case XTP_IDC_RADIO_ENDAFTER:
			...
			break;
		case XTP_IDC_RADIO_NOEND: m_ptrPattern->SetNoEndDate(); break;
	}
