...
////////////////////////////////////////////////////////////////////////
void CXTPCalendarTheme::CopySettings(CXTPPaintManagerColor& refDest,
...
void CXTPCalendarTheme::CopySettings(CXTPPaintManagerColorGradient& refDest,
									 const CXTPPaintManagerColorGradient& refSrc)
{
...
XTP_DAY_VIEW_GROUP_LAYOUT&
	CXTPCalendarTheme::GetDayViewGroupLayout(CXTPCalendarDayViewGroup* pDayViewGroup)
{
...
////////////////////////////////////////////////////////////////////////
CSize AFX_CDECL CXTPCalendarTheme::DrawLine_CenterLR(CDC* pDC, LPCTSTR pcszText, CRect& rcRect,
													 UINT nFormat)
{
...
CSize AFX_CDECL CXTPCalendarTheme::DrawLine2_CenterLR(
	CDC* pDC, LPCTSTR pcszText1, LPCTSTR pcszText2, CThemeFontColorSet* pFontColor1,
	CThemeFontColorSet* pFontColor2, CRect& rcRect, UINT nShortAlign, UINT nFullAlign)
{
...
void AFX_CDECL CXTPCalendarTheme::_DrawRoundRect(CDC* pDC, const CRect& rcRect, COLORREF clrBorder,
												 BOOL bBoldBorder,
												 CXTPPaintManagerColorGradient* pgrclrBk,
												 CBrush* pBrushBk)
{
	ASSERT_VALID(pDC);
	...
	if (pDC->IsPrinting() || pBrushBk)
	{
		if (!pBrushBk)
		{
			if (pgrclrBk)
			{
				COLORREF clrColorBk = XTPDrawHelpers()->BlendColors(pgrclrBk->clrLight,
																	pgrclrBk->clrDark, 0.5f);
	...
	else
	{
		...
		CRgn rgnEvent;
		BOOL b = rgnEvent.CreateRoundRectRgn(rcClip.left, rcClip.top, rcClip.right, rcClip.bottom,
												 ...
												 CRect isBorders)
{
	...
	if (pDC->IsPrinting())
	{
		COLORREF clrColorBk = XTPDrawHelpers()->BlendColors(grclrBk.clrLight, grclrBk.clrDark,
															0.5f);
...
void CXTPCalendarThemePart::SendNotificationAlways(XTP_NOTIFY_CODE EventCode, WPARAM wParam,
												   LPARAM lParam)
{
...
//=================================================================
CXTPCalendarThemePart::CXTPPropsStateContext::CXTPPropsStateContext(CXTPCalendarThemePart* pPart,
...
void CXTPCalendarThemePart::CXTPPropsStateContext::SendBeforeDrawThemeObject(LPARAM lParam,
																			 int eBDFlag)
{
...
void CXTPCalendarTheme::CTOEventIconsToDraw::SetStandardValue(BOOL bRmd, BOOL bOcc, BOOL bExc,
															  BOOL bMee, BOOL bPrv)
{
	...
	CXTPCalendarTheme::CTOHeader::CHeaderText::GetFontColor(int nItemState)
{
	...
	switch (nItemState)
	{
		case CTOHeader::xtpCalendarItemState_Normal: return &fcsetNormal;
		case CTOHeader::xtpCalendarItemState_Selected: return &fcsetSelected;
		case CTOHeader::xtpCalendarItemState_Today: return &fcsetToday;
...
void CXTPCalendarTheme::CThemeFontColorSetValue::doPX(CXTPPropExchange* pPX, LPCTSTR pcszPropName,
...
void CXTPCalendarTheme::CTOHeader::CHeaderText::doPX(CXTPPropExchange* pPX, LPCTSTR pcszPropName,
													 CXTPCalendarTheme* pTheme)
{
	if (!pPX || !pcszPropName || !pTheme)
	{
...
BOOL CXTPCalendarTheme::CTOHeader::IsSelected(CXTPCalendarViewGroup* pViewGroup)
{
	...
	BOOL bSel = pViewGroup->GetViewDay_()->GetView_()->GetSelection(NULL, NULL, NULL,
																	&nSelGroupIndex);
	if (!bSel)
	{
		return FALSE;
...
void CXTPCalendarTheme::CTOHeader::Draw_Background(CDC* pDC, const CRect& rcRect, int nState)
{
	...
	int nIndex = (nState & CTOHeader::xtpCalendarItemFirst)
					 ? 0
...
void CXTPCalendarTheme::CTOHeader::Draw_Header(CDC* pDC, const CRect& rcRect, int nFlags,
											   LPCTSTR pcszLeftText, LPCTSTR pcszCenterText,
											   LPCTSTR pcszRightText, BOOL bMultiCase)
{
	if (!bMultiCase)
	...
	if (!sLeft.IsEmpty())
		Draw_TextLR(pDC, rcTextL, nFlags, sLeft, DT_LEFT | DT_NOPREFIX | DT_SINGLELINE | DT_VCENTER,
					&nWidth_L);
...
void CXTPCalendarTheme::CTOHeader::RefreshMetrics(BOOL bRefreshChildren)
{
	...
	m_bkNormal.rcBitmapBorder.SetStandardValue(
		CRect(XTP_DPI_X(2), XTP_DPI_Y(2), XTP_DPI_X(2), XTP_DPI_Y(2)));
	m_bkSelected.rcBitmapBorder.SetStandardValue(
		CRect(XTP_DPI_X(2), XTP_DPI_Y(2), XTP_DPI_X(2), XTP_DPI_Y(2)));
...
int CXTPCalendarTheme::CTODayView::CalcMinCellHeight(CDC* pDC, CXTPCalendarDayView* pDayView)
{
	...
	if (pDayView->IsAdditionalScaleLeftVisible() && pDayView->GetTimeScale(2))
	{
		nTimeScale2RowHeight = GetTimeScalePart()->CalcMinRowHeight(pDC, pDayView->GetTimeScale(2));
	}

	int nMinEventHeight = GetDayPart()->GetGroupPart()->GetSingleDayEventPart()->CalcMinEventHeight(
		pDC);
...
{
	CXTPCalendarDayView* pDayView = DYNAMIC_DOWNCAST(CXTPCalendarDayView,
	...
	int nCellWidth			 = max(0, rcView.Width() / max(nDaysCount, 1)
								  - (pDayView->IsAdditionalScaleLeftVisible() ? XTP_DPI_X(84)
																			  : XTP_DPI_X(42)));
	BOOL bGroupHeaderVisible = pDayView->IsGroupHeaderVisible();

	int nMinCellHeight = CalcMinCellHeight(pDC, pDayView);
	int nAllDayEventHeight =
		GetDayPart()->GetGroupPart()->GetMultiDayEventPart()->CalcMinEventHeight(pDC);
	...
	{
		nGroupHeaderHeight = GetDayPart()->GetGroupPart()->GetHeaderPart()->CalcHeight(
			pDC, nCellWidth / nGroupsCount);
	}
	...
	{
		nAllDayEventsHeight = max(1, nAllDayEventCount) * nAllDayEventHeight
							  + nAllDayEventHeight / 3;
	...
	if (!pDC->IsPrinting()
		&& rLayoutX.m_nTopRow + rLayoutX.m_nVisibleRowCount >= rLayoutX.m_nRowCount)
	{
		rLayoutX.m_nTopRow = rLayoutX.m_nRowCount - rLayoutX.m_nVisibleRowCount;
	}

	nAllDayEventsHeight = max(0,
							  nEventsHeight - (rLayoutX.m_nVisibleRowCount * rLayout.m_nRowHeight));
	if (bIsHideAllDayEventsArea)
	{
	...
	//--------------------------------------------------------------
	int nTimeScaleLeftWidth  = 0;
	int nTimeScaleRightWidth = 0;
	int nTimeScale2Width	 = 0;

	// regular time scale
	if (pDayView->GetTimeScale() && pDayView->GetTimeScale()->GetVisible())
	{
		nTimeScaleLeftWidth = GetTimeScalePart()->CalcWidth(pDC, pDayView->GetTimeScale(),
															rLayout.m_nRowHeight);
	}
	if (pDayView->GetTimeScale(3) && pDayView->GetTimeScale(3)->GetVisible())
	{
		nTimeScaleRightWidth = GetTimeScalePart()->CalcWidth(pDC, pDayView->GetTimeScale(3),
															 rLayout.m_nRowHeight);
	}
	...
	if (pDayView->IsAdditionalScaleLeftVisible() && pDayView->GetTimeScale(2))
	{
		nTimeScale2Width = GetTimeScalePart()->CalcWidth(pDC, pDayView->GetTimeScale(2),
														 rLayout.m_nRowHeight);
	}
	rcView.left += nTimeScaleLeftWidth + nTimeScale2Width;
	rcView.right -= nTimeScaleRightWidth;

	//-----------------------------------------------------------------
	rLayoutX.m_rcDayHeader.SetRect(rcView.left, rcView.top, rcView.right,
								   rcView.top + nDayHeaderHeight);
	...
	if (pDayView->GetTimeScale() && pDayView->GetTimeScale()->GetVisible())
	{
		...
		rcTimeScale.right = rcTimeScale.left + nTimeScaleLeftWidth;
	...
	{
		rcTimeScale		 = rcRect;
		rcTimeScale.left = rcTimeScale.right - nTimeScaleRightWidth;

		GetTimeScalePart()->AdjustLayout(pDayView->GetTimeScale(3), pDC, rcTimeScale);
	}
...
void CXTPCalendarTheme::CTODayViewTimeScale::RefreshMetrics(BOOL)
{
	...
	CXTPCalendarDayViewTimeScale* pTimeScale1 = XTP_SAFE_GET3(GetTheme(), GetCalendarControl(),
															  GetDayView(), GetTimeScale(1), NULL);
	if (pTimeScale1)
		pTimeScale1->RefreshTimeFormat();

	CXTPCalendarDayViewTimeScale* pTimeScale2 = XTP_SAFE_GET3(GetTheme(), GetCalendarControl(),
															  GetDayView(), GetTimeScale(2), NULL);
	if (pTimeScale2)
	...
	m_ShowMinutes.SetStandardValue(XTP_SAFE_GET3(GetTheme(), GetCalendarControl(),
												 GetCalendarOptions(),
...
CString CXTPCalendarTheme::CTODayViewTimeScale::FormatTime(const XTP_LC_TIMEFORMAT& lcTimeFormat,
														   COleDateTime dtTime, BOOL bAmPmEnshure,
														   CString& rstrHour, CString& rstrMin,
														   CString& rstrAmPm)
{
	rstrHour = lcTimeFormat.bAMPM_timeFormat ? dtTime.Format(_T("%I")) : dtTime.Format(_T("%H"));

	if (!lcTimeFormat.bLeadingZeroTime && rstrHour.GetLength() == 2 && rstrHour[0] == _T('0'))
	{
		rstrHour.Delete(0, 1);
	}
...
BOOL CXTPCalendarTheme::CTODayViewTimeScale::IsDrawNowLine(CDC* pDC,
														   CXTPCalendarDayViewTimeScale* pTS)
{
	if (!pDC || !pTS || !pTS->GetDayView() || !GetTheme())
	{
		ASSERT(FALSE);
	...
	int nNowLineFlags = XTP_SAFE_GET4(pTS, GetDayView(), GetCalendarControl(), GetCalendarOptions(),
									  nDayView_CurrentTimeMarkVisible, 0);
	...
	if (bDrawNowLine)
	{
		...
		{
			int nTAMask = xtpCalendarCurrentTimeMarkVisibleForToday
						  | xtpCalendarCurrentTimeMarkVisibleAlways;
			bDrawNowLine = bDrawNowLine && (nNowLineFlags & nTAMask);
		}
		else
		{
			bDrawNowLine = bDrawNowLine
...
int CXTPCalendarTheme::CTODayViewTimeScale::CalcMinRowHeight(
...
int CXTPCalendarTheme::CTODayViewTimeScale::CalcWidth_AmPmMinutesPart(
	CDC* pDC, CXTPCalendarDayViewTimeScale* pTimeScale, int* pnCharWidth)
{
	if (!pDC || !pTimeScale)
	{
		ASSERT(FALSE);
...
{
	CXTPCalendarDayViewTimeScale* pTimeScale = DYNAMIC_DOWNCAST(CXTPCalendarDayViewTimeScale,
																pObject);
...
void CXTPCalendarTheme::CTODayViewTimeScale::DrawTimescaleLTR(CCmdTarget* pObject, CDC* pDC)
{
	...
	if (CXTPCalendarUtils::GetTotalHours(spScaleInterval) >= 1)
	{
		...
		{
			CRect rcCell(layoutTS.m_rcTimeZone.left, layoutTS.m_rcTimeZone.top + nRowHeight * i,
			...
			FormatTime(timeFormat, dtCell, bAmPmEnshure, strHour, strMin, strAmPm);
			...
			Draw_SmallHourCell(pDC, pTimeScale, rcCell, strHour, strPart2, &m_fcsetSmall,
							   bAmPm ? &m_fcsetAMPM : &m_fcsetSmall, autoStateCnt2, &tsCell);
	...
	else
	{
		...
		// maximum number of nDrawCellCount (number of covered hours on the screen) should be set to
		// 2 the second check (nDrawCellCount < 2) is introduced just in case
		...
		for (int i = 0; i < nDrawCellCount; i++)
		{
			...
			Draw_BigHourCell(pDC, pTimeScale, rcCell, nRowPerHour, strHour,
							 ...
							 bAmPm ? &m_fcsetAMPM : &m_fcsetBigBase, autoStateCnt2, &tsCell);
	...
	autoStateCnt1.SendBeforeDrawThemeObject((LPARAM)dwTSnumber,
											xtpCalendarBeforeDraw_DayViewTimeScaleCaption);
...
CRect CXTPCalendarTheme::CTODayViewTimeScale::Calc_NowLineRect(
	CDC* pDC, CXTPCalendarDayViewTimeScale* pTimeScale)
{
	...
	double dPixelPerSecond = layoutTS.m_rcTimeZone.Height()
							 / (double)CXTPCalendarUtils::GetTotalSeconds(spView);
...
int CXTPCalendarTheme::CTODayViewTimeScale::GetRightOffsetX(
	CDC* pDC, CXTPCalendarDayViewTimeScale* pTimeScale)
{
	if (!pDC || !pTimeScale || !pTimeScale->GetDayView())
	{
...
void CXTPCalendarTheme::CTODayViewTimeScale::Draw_SmallHourCell(
	CDC* pDC, CXTPCalendarDayViewTimeScale* pTimeScale, const CRect& rcCell,
	const CString& strPart1, const CString& strPart2, CThemeFontColorSetValue* pfcsetPart1,
	CThemeFontColorSetValue* pfcsetPart2, CXTPPropsStateContext& autoStateCnt,
	XTP_CALENDAR_THEME_DAYVIEWTIMESCALECELL_PARAMS* tsCell)
{
...
void CXTPCalendarTheme::CTODayViewTimeScale::Draw_BigHourCell(
	CDC* pDC, CXTPCalendarDayViewTimeScale* pTimeScale, const CRect& rcCell, int nRowPerHour,
	const CString& strHour, const CString& strMinAmPm, CThemeFontColorSetValue* pfcsetHour,
	CThemeFontColorSetValue* pfcsetMinAmPm, CXTPPropsStateContext& autoStateCnt,
	XTP_CALENDAR_THEME_DAYVIEWTIMESCALECELL_PARAMS* tsCell)
{
	...
	rcMinutes.left = max(rcCell.left, rcMinutes.right - m_nWidthAmPmMinutesPart_
										  + m_nCharWidthAmPmMinutesPart_ / 2);
	...
	if (nRowPerHour >= 2)
	{
		...
		int nLeftX = max(rcHour.left,
						 rcCell.right - m_nWidthAmPmMinutesPart_ - max(nROffsetX, XTP_DPI_X(1)));
		for (int i = 1; i < nRowPerHour; i++)
		{
			int y = rcCell.top + i * nRowHeight;
			pDC->FillSolidRect(nLeftX, y - XTP_DPI_Y(1), m_nWidthAmPmMinutesPart_, XTP_DPI_Y(1),
							   m_clrLine);
	...
	rcBottom.DeflateRect(XTP_DPI_X(5), 0, nROffsetX ? nROffsetX : m_nCharWidthAmPmMinutesPart_ / 2,
						 0);
	pDC->FillSolidRect(rcBottom.left, rcBottom.bottom - XTP_DPI_Y(1), rcBottom.Width(),
					   XTP_DPI_Y(1), m_clrLine);
}
...
void CXTPCalendarTheme::CTODayViewDayGroupCell::XTP_GROUP_CELL_COLOR_SET::doPX(
...
void CXTPCalendarTheme::CTODayViewDayGroupCell::XTP_GROUP_CELL_COLOR_SET::Serialize(CArchive& ar)
{
														 ...
														 const CRect& rcRect)
{
	...
	for (nEvent = 0; nEvent < nEventsCount; nEvent++)
	{
		...
		if (pViewEvent && pViewEvent->IsMultidayEvent())
		{
			...
			if (nSlostIndex == -1)
			{
				...
				mapEventIdEventSlotId[pEvent->GetEventID()] = XTPToUIntChecked(
					pViewEvent->GetSlotNumber());
			}
			else
			{
				int nCount  = (int)arrEvents.GetSize();
	...
	{
		arrEvents[nEvent]->AdjustLayout2(pDC, rcAllDayEvents,
										 nOffset + arrEvents[nEvent]->GetSlotNumber());
	}
...
void CXTPCalendarTheme::CTODayViewDayGroup::AdjustDayEvents(CXTPCalendarDayViewGroup* pDayViewGroup,
															CDC* pDC)
{
...
CXTPCalendarViewPartBrushValue*
	CXTPCalendarTheme::CTOEvent::_GetBusyStatusBrushValue(int nBusyStatus, BOOL bCreateIfNotExist)
{
...
void CXTPCalendarTheme::CTOEvent::SetBusyStatusBrush(int nBusyStatus, CBrush* pBrush,
...
void CXTPCalendarTheme::CTOEvent::InitBusyStatusDefaultColors()
{
	...
	CXTPCalendarViewPartBrushValue* pBVal_Free = _GetBusyStatusBrushValue(xtpCalendarBusyStatusFree,
																		  TRUE);
	...
	CXTPCalendarViewPartBrushValue* pBVal_Busy = _GetBusyStatusBrushValue(xtpCalendarBusyStatusBusy,
																		  TRUE);
	CXTPCalendarViewPartBrushValue* pBVal_OutOO =
		_GetBusyStatusBrushValue(xtpCalendarBusyStatusOutOfOffice, TRUE);
...
void CXTPCalendarTheme::CTODayView::Draw(CDC* pDC)
{
	...
	if (pDayView->GetTimeScale(3) && pDayView->GetTimeScale(3)->GetVisible())
		GetTimeScalePart()->Draw(pDayView->GetTimeScale(3), pDC);
}
...
void CXTPCalendarTheme::CTODayViewDayGroup::Draw(CCmdTarget* pObject, CDC* pDC)
{
	...
	for (int i = 0; i < nVisibleRowCount; i++)
	{
		...
		COleDateTime dtCell = pDayViewGroup->GetCellDateTime(nTopRow + i)
		...
		tsCell.bSelected = !bHasSelectedEvents
						   && pDayView->SelectionContains(dtCell, nGroupIndex, &bAllDaySelection);
		tsCell.bSelected &= !bAllDaySelection;
		tsCell.bWorkCell = pDayViewGroup->IsWorkDateTime(dtCell);

		// autoStateCnt2Cell.SendBeforeDrawThemeObject((LPARAM) &tsCell,
	...
	CXTPPropsStateContext autoStateCnt3SE(GetSingleDayEventPart(),
										  xtpCalendarBeforeDraw_DayViewEvent, TRUE);
	...
	for (nEvent = 0; nEvent < nEventsCount; nEvent++)
	{
		...
		if (pViewEvent
			...
			&& pViewEvent->GetEvent()->IsEventVisible()
			//<<EventVisible>>
			&& !pViewEvent->IsMultidayEvent() && !pViewEvent->IsSelected())
		{
			autoStateCnt3SE.SendBeforeDrawThemeObject((LPARAM)pViewEvent);
	...
	CXTPPropsStateContext autoStateCnt5AD(GetAllDayEventsPart(),
										  xtpCalendarBeforeDraw_AllDayViewEvent, TRUE);
	// draw all day events
	for (nEvent = 0; nEvent < nEventsCount; nEvent++)
	{
		CXTPCalendarDayViewEvent* pViewEvent = pDayViewGroup->GetViewEvent(nEvent);
...
void CXTPCalendarTheme::CTODayViewDayGroup::DrawAllDayAreaScrollIcons(CCmdTarget* pObject, CDC* pDC)
{
	...
	{
		CRect rcIconsMax2 = pDayViewGroup->GetAllDayEventsScrollButtonRect(
			XTP_DAY_VIEW_GROUP_LAYOUT::xtpHotAllDayEventsScrollUp);
		pThemeX->DrawExpandDayButton(pDC, rcIconsMax2, TRUE,
									 iFisrtVisibleAllDayEvent > 0 ? TRUE : FALSE);
	}

	if (pDayViewGroup->IsDownAllDayEventsScrollButtonVisible())
	{
		CRect rcIconsMax1 = pDayViewGroup->GetAllDayEventsScrollButtonRect(
			XTP_DAY_VIEW_GROUP_LAYOUT::xtpHotAllDayEventsScrollDown);
		pThemeX->DrawExpandDayButton(
			pDC, rcIconsMax1, FALSE,
			iLastVisibleAllDayEvent < pDayViewGroup->GetEventsCountInAllDayArea() ? TRUE : FALSE);
	}
}

void CXTPCalendarTheme::CTODayViewDayGroupCell::Draw(
	...
	CXTPPropsStateContext* pStateCnt)
{
	...
	COleDateTime dtCellNext =
		cellParams
			.dtBeginTime; // + cellParams.pViewGroup->GetViewDay()->GetView()->GetCellDuration();
	BOOL bInHour = dtCellNext.GetMinute() != 0;

	int nLine = (cellParams.nIndex == cellParams.pViewGroup->GetViewDay()->GetView()->GetTopRow())
					? 0
					: 1;
	...
	COLORREF clr1 = cellParams.bWorkCell ? m_clrsetWorkCell.clrBackground
										 : m_clrsetNonWorkCell.clrBackground;
	if (cellParams.bSelected)
		clr1 = cellParams.bWorkCell ? m_clrsetWorkCell.clrSelected
									: m_clrsetNonWorkCell.clrSelected;

	COLORREF clr2 = cellParams.bWorkCell
						? (bInHour ? m_clrsetWorkCell.clrBorderBottomInHour.operator COLORREF()
								   : m_clrsetWorkCell.clrBorderBottomHour.operator COLORREF())
						: 0;
	if (!cellParams.bWorkCell)
		clr2 = bInHour ? m_clrsetNonWorkCell.clrBorderBottomInHour.operator COLORREF()
					   : m_clrsetNonWorkCell.clrBorderBottomHour.operator COLORREF();
...
void CXTPCalendarTheme::CTOMonthView::AdjustLayout(CDC* pDC, const CRect& rcRect,
...
void CXTPCalendarTheme::CTOMonthViewDay::AdjustLayout(CCmdTarget* pObject, CDC* pDC,
													  const CRect& rcRect)
{
	...
	rDayLayout.m_rcDayHeader.bottom = min(rDayLayout.m_rcDayHeader.top + nDayHeaderHeight,
										  rDayLayout.m_rcDayHeader.bottom);
...
void CXTPCalendarTheme::CTOWeekView::AdjustLayout(CDC* pDC, const CRect& rcRect,
												  BOOL bCallPostAdjustLayout)
{
	UNREFERENCED_PARAMETER(bCallPostAdjustLayout);

	CXTPCalendarWeekView* pWeekView = DYNAMIC_DOWNCAST(
		CXTPCalendarWeekView, XTP_SAFE_GET2(GetTheme(), GetCalendarControl(), GetWeekView(), NULL));
	...
	rLayoutX.m_nDayHeight = rcView2.Height() / nRows;

	rLayoutX.m_nDayHeaderHeight = GetDayPart()->GetHeaderPart()->CalcHeight(pDC,
																			rLayoutX.m_nDayWidth);
	...
	for (int nDayCol = 0; nDayCol < rLayoutX.m_nGridColumns; nDayCol++)
	{
		...
		for (int nDayRow = 0; nDayRow < nRows; nDayRow++)
		{
			...
			rc.SetRect(rcView2.left + rLayoutX.m_nDayWidth * nDayCol, nRowTopY,
			...
			// assignment - e.g. Wed and Thu as free days! if (nWday ==
			// pWeekView->GetFirstDayOfWeek() + 5)
...
void CXTPCalendarTheme::CTOWeekViewDay::AdjustLayout(CCmdTarget* pObject, CDC* pDC,
													 const CRect& rcRect)
{
...
int CXTPCalendarTheme::CTODay::_HitTestExpandDayButton(const CXTPCalendarViewDay* pViewDay,
													   const CRect& rc, const CPoint* pPoint)
{
	...
	if (pViewDay->ContainsInvisibleEvents())
	{
		CPoint pt(0, 0);
		if (pPoint)
		{
...
int CXTPCalendarTheme::CTODay::HitTestExpandDayButton(const CXTPCalendarViewDay* pViewDay,
													  const CPoint* pPoint)
{
	if (!pViewDay || !GetTheme() || !GetTheme()->GetCalendarControl())
	{
...
int CXTPCalendarTheme::CTODayViewDayGroup::HitTestScrollButton(
	const CXTPCalendarDayViewGroup* pViewGroup, const CPoint* pPoint)
{
...
void CXTPCalendarTheme::CTOWeekView::Draw(CDC* pDC)
{
	...
	for (int i = 0; i < nCount; i++)
	{
		...
		if (pViewDay)
		{
			...
			autoStateCnt1.SendBeforeDrawThemeObject((LPARAM)pViewDay,
													xtpCalendarBeforeDraw_WeekViewDay);
...
CRect CXTPCalendarTheme::DrawExpandDayButton(CDC* pDC, const CRect& rc, BOOL bUp, BOOL bHot)
{
	...
	//
		...
		if (pBmpHeader)
		{
			...
			XTPImageManager()->AlphaBlend_calc(pDC->GetSafeHdc(), rcButton, dcSrc.GetSafeHdc(),
	rcBmp, clrBase);
		}*/
...
void CXTPCalendarTheme::SetTipWindowRgn(CWnd* pTip)
{
	...
	CRgn rgnEvent;
	rgnEvent.CreateRoundRectRgn(rc.left, rc.top, rc.right + XTP_DPI_X(1), rc.bottom + XTP_DPI_Y(1),
								XTP_DPI_X(cnCornerSize), XTP_DPI_Y(cnCornerSize));
...
BEGIN_DISPATCH_MAP(CXTPCalendarTheme, CXTPCalendarThemePart)
	DISP_PROPERTY_EX_ID(CXTPCalendarTheme, "BaseColor", 1, OleGetBaseColor, OleSetBaseColor,
						VT_COLOR)

	DISP_PROPERTY_EX_ID(CXTPCalendarTheme, "BaseFont", 2, OleGetBaseFont, OleSetBaseFont, VT_FONT)
	DISP_PROPERTY_EX_ID(CXTPCalendarTheme, "BaseFontBold", 3, OleGetBaseFontBold,
						OleSetBaseFontBold, VT_FONT)

	DISP_PROPERTY_EX_ID(CXTPCalendarTheme, "CustomIcons", 150, OleGetCustomIcons, OleSetCustomIcons,
						VT_DISPATCH)
	DISP_PROPERTY_EX_ID(CXTPCalendarTheme, "ImageList", 151, OleGetImageList, SetNotSupported,
						VT_DISPATCH)

	DISP_FUNCTION_ID(CXTPCalendarTheme, "RefreshMetrics", 100, OleRefreshMetrics, VT_EMPTY,
					 VTS_NONE)

	DISP_FUNCTION_ID(CXTPCalendarTheme, "SetCustomIcons", 102, OleSetCustomIconsImageList, VT_EMPTY,
					 VTS_DISPATCH)
	DISP_FUNCTION_ID(CXTPCalendarTheme, "DoPropExchange", 103, OleDoPropExchange, VT_EMPTY,
					 VTS_DISPATCH)
END_DISPATCH_MAP()
