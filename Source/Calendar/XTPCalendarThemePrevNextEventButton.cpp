...
void CTOPrevNextEventButton::RefreshMetrics(BOOL /*bRefreshChildren*/)
{
	...
	m_rcBitmapBorder.SetStandardValue(
		CRect(XTP_DPI_X(1), XTP_DPI_Y(8), XTP_DPI_X(1), XTP_DPI_Y(8)));
...
CSize CTOPrevNextEventButton::GetSize(const CRect* prcRect)
{
	...
	if (prcRect)
	{
		if (prcRect->Height() < XTP_DPI_Y(35) || prcRect->Width() < szSize.cx * 2 + XTP_DPI_X(1))
		{
...
void CTOPrevNextEventButton::AdjustLayout(CXTPCalendarView* /*pObject*/, CDC* /*pDC*/,
										  const CRect& rcRect)
{
...
void CTOPrevNextEventButton::Draw(CXTPCalendarView* /*pObject*/, CDC* pDC)
{
	...
	if (rcText.Height() > XTP_DPI_X(100))
	{
		...
		//---
		COLORREF clrText = m_bDisabled ? (COLORREF)m_clrDisabledText
									   : (COLORREF)m_fcsetText.clrColor;
		...
		pDC->DrawText((const CString&)m_strText, &rcText,
...
BOOL CTOPrevNextEventButton::OnLButtonDown(CCmdTarget* /*pObject*/, UINT /*nFlags*/, CPoint point,
										   CTOPrevNextEventButtons* pButtons)
{
	...
	if (bHitTest)
	{
		...
		CXTPCalendarView* pView = XTP_SAFE_GET2(GetTheme(), GetCalendarControl(), GetActiveView(),
												NULL);
		ASSERT(pView);
		if (pView && bDayValid)
		{
