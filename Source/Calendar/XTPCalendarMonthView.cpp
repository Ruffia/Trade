...
void CXTPCalendarMonthView::_ReadDefaultDateFormats()
{
	...
	for (int i = 0; i < nCount; i++)
	{
		...
		if (!bMonthAdded
			&& (strToken == _T("M") || strToken == _T("MM") || strToken == _T("MMM")
				|| strToken == _T("MMMM")))
		{
	...
	else if (!bDayAdded || !bMonthAdded)
	{
		...
		CString strCHlast = nLen ? strLongDateFormat.Right(1) : CString();
...
COleDateTime CXTPCalendarMonthView::GetViewDayDate(int nIndex) const
{
	...
	return XTP_SAFE_GET1(m_pGrid, GetDayDate(nWeekIndex, nWeekDayIndex).operator DATE(), (DATE)0);
}
...
void CXTPCalendarMonthView::_CalculateEventCaptionFormat(CDC* pDC)
{
	...
	//--------------------
	m_nEventCaptionFormat = bShowEnd ? xtpCalendarCaptionFormatStartEndSubject
									 : xtpCalendarCaptionFormatStartSubject;
...
COleDateTime CXTPCalendarMonthView::CMonthViewGrid::GetDayDate(int nWeekIndex,
															   int nWeekDayIndex) const
{
...
CRect CXTPCalendarMonthView::CMonthViewGrid::GetDayRect(int nWeekIndex, int nWeekDayIndex,
														BOOL bIncludeBorders) const
{
	BOOL bCompressWeD = XTP_SAFE_GET2(m_pView, GetCalendarControl(),
...
CRect CXTPCalendarMonthView::CMonthViewGrid::GetCelRect(int nWeekIndex, int nColIndex,
														BOOL bIncludeBorders) const
{
...
CXTPCalendarMonthViewDay* CXTPCalendarMonthView::CMonthViewGrid::GetViewDay(int nWeekIndex,
																			int nWeekDayIndex) const
{
...
void CXTPCalendarMonthView::GetWeekDayTextIfNeed(CString* pstrText, int nWeekDay)
{
	...
	if (dwFlags & xtpCalendarItemText_MonthViewWeekDayHeader)
	{
		...
		XTP_SAFE_CALL1(GetCalendarControl(),
...
{
	CXTPCalendarViewPart* pPart = XTP_SAFE_GET2(m_pView, GetPaintManager(),
												GetMonthViewHeaderPart(), NULL);
	...
	{
		LC_WeekDAYNAME1 = XTPToUInt(CXTPCalendarUtils::GetDayOfWeekLocaleStringId(1));
	}
	//---------------------------------------------------------------------------
	...
	for (i = 0; i < 7; i++)
	{
		...
		if (!sVal.IsEmpty())
		{
			...
			CString sSat = CXTPCalendarUtils::GetLocaleString(
				XTPToUInt(CXTPCalendarUtils::GetDayOfWeekLocaleStringId(6)), 100);
			CString sSun = CXTPCalendarUtils::GetLocaleString(
				XTPToUInt(CXTPCalendarUtils::GetDayOfWeekLocaleStringId(7)), 100);
		...
		for (nDayIdx = 0; nDayIdx < 7; nDayIdx++)
		{
			...
			for (int nEvent = 0; nEvent < nEventsCount; nEvent++)
			{
				CXTPCalendarMonthViewEvent* pViewEvent =
					XTP_SAFE_GET2(pViewDay, GetViewGroup(0), GetViewEvent(nEvent),
								  NULL); // pViewDay->GetViewEvent(nEvent);
				ASSERT(pViewEvent);
				if (!pViewEvent)
				{
					continue;
...
BEGIN_DISPATCH_MAP(CXTPCalendarMonthView, CXTPCalendarView)
	DISP_PROPERTY_EX_ID(CXTPCalendarMonthView, "WeeksCount", 1, OleGetWeeksCount, OleSetWeeksCount,
						VT_I4)
	DISP_PROPERTY_EX_ID(CXTPCalendarMonthView, "DraftMode", 2, OleGetDraftMode, OleSetDraftMode,
						VT_BOOL)
END_DISPATCH_MAP()
