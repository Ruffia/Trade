...
void CXTPCalendarRemindersDialog::SetShowInTaskBar(BOOL bShow)
{
	...
	if (m_hWnd)
	{
		...
		ModifyStyleEx(XTPToDWORD(m_bShowInTaskBar ? 0 : WS_EX_APPWINDOW),
...
BOOL CXTPCalendarRemindersDialog::OnInitDialog()
{
	...
	ListView_SetExtendedListViewStyleEx(m_ctrlRemindersList.m_hWnd, LVS_EX_FULLROWSELECT,
	...
	else
	{
		...
		if (ptrEvent)
		{
			...
			strDescription.Format(_T("%s \n\n%s: %s %s"), (LPCTSTR)strSubject,
								  (LPCTSTR)CXTPCalendarUtils::LoadString(
...
void CXTPCalendarRemindersDialog::SetRemindersManager(CXTPCalendarControl* pCalendar)
{
	...
	if (pConnection)
	{
		m_pSink->Advise(pConnection, XTP_NC_CALENDAR_ON_REMINDERS,
						CreateNotfySinkClassDelegate(this,
													 &CXTPCalendarRemindersDialog::OnReminders));
	}
...
void CXTPCalendarRemindersDialog::UpdateTitle()
{
	...
	CString strTitle;
	strTitle.Format((LPCTSTR)CXTPCalendarUtils::LoadString(XTP_IDS_CALENDAR_REMINDERS),
					nItemsCount);
...
void CXTPCalendarRemindersDialog::OnReminders(XTP_NOTIFY_CODE Event, WPARAM wParam, LPARAM lParam)
{
	...
	if (eAction == xtpCalendarRemindersFire || eAction == xtpCalendarReminderDismissedAll)
	{
		UpdateFromManager();
	...
	else if (eAction == xtpCalendarReminderSnoozed || eAction == xtpCalendarReminderDismissed)
	{
...
void CXTPCalendarRemindersDialog::OnTimer(UINT_PTR nIDEvent)
{
	...
	if (nIDEvent == XTP_CALENDAR_RMD_REFRESH_TIMERID)
	{
		if (m_nNextRefreshIndex < 0)
		{
			...
			SetTimer(XTP_CALENDAR_RMD_REFRESH_ITEM_TIMERID, XTP_CALENDAR_RMD_REFRESH_ITEM_TIMEOUT,
					 NULL);
		}
		return;
	}
