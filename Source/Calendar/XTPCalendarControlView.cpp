...
void CXTPCalendarControlView::CBeforePrintState::Save(CXTPCalendarControlView* pView)
{
	...
	if (nViewType == xtpCalendarDayView || (b && nViewType == xtpCalendarWeekView)
		|| nViewType == xtpCalendarWorkWeekView || nViewType == xtpCalendarFullWeekView)
	{
		CXTPCalendarDayView* pDayView = DYNAMIC_DOWNCAST(CXTPCalendarDayView,
														 pView->GetCalendarCtrl().GetDayView());
		ASSERT(pDayView);
		if (!pDayView)
			return;
...
CXTPCalendarPageSetupDialog::CXTPCalendarPageSetupDialog(
	CXTPCalendarControlViewPrintOptions* pOptions, DWORD dwFlags, CWnd* pParentWnd)
	: CPageSetupDialog(dwFlags, pParentWnd)
{
	...
	if (m_pOptions)
	{
		...
		DWORD dwMeasure = XTPToDWORD(bIsInches ? PSD_INTHOUSANDTHSOFINCHES
...
BOOL CXTPCalendarPageSetupDialog::OnInitDialog()
{
	...
	{
		if (i < 24 * 60)
			strTime = CXTPCalendarUtils::GetTimeFormat((int)(i / 60), (int)(i % 60),
													   strTimeFormat_HrMin);
		else
			strTime = _T("23:59:59");
	...
	UINT uStrID		   = XTPToUInt(bIsInches ? XTP_IDS_CALENDAR_MARGINS_INCH
...
CXTPCalendarControlViewPrintOptions::CXTPCalendarControlViewPrintOptions()
{
	...
	m_lfDateHeaderWeekDayFont.lfHeight = -max(abs(lfIcon.lfHeight),
											  abs(m_lfDateHeaderFont.lfHeight * 2 / 3));
											  ...
											  BOOL bReleaseWhenDestroy)
{
	...
	m_pSink->UnadviseAll();
	m_pSink->Advise(
		GetCalendarCtrl().GetConnection(), XTP_NC_CALENDAR_THEME_CHANGED,
		CreateNotfySinkClassDelegate(this, &CXTPCalendarControlView::OnEvent_CalendarThemeChanged));
...
BOOL CXTPCalendarControlView::OnEraseBkgnd(CDC* pDC)
{
	...
	if (GetCalendarCtrl().m_bTimelineMode && ::IsWindow(m_pVScrollBar->GetSafeHwnd()))
	{
		...
		COLORREF crBack = pPaintManager ? pPaintManager->m_clrTLVBack.operator DWORD()
...
void CXTPCalendarControlView::_ResizeControls()
{
	...
	if (::IsWindow(GetCalendarCtrl().m_hWnd))
	{
		...
		if (::IsWindow(m_pHScrollBar->GetSafeHwnd()))
		{
			...
			if (GetCalendarCtrl().IsLayoutRTL())
			{
				m_pHScrollBar->MoveWindow(cxVScroll, cy - cyHScroll, cx - cxVScroll, cyHScroll);
			}
			else
			{
				m_pHScrollBar->MoveWindow(0, cy - cyHScroll, cx - cxVScroll, cyHScroll);
			}
		}
		...
		if (::IsWindow(m_pVScrollBar->GetSafeHwnd()))
		{
			if (GetCalendarCtrl().IsLayoutRTL())
			{
				m_pVScrollBar->MoveWindow(0, cyCaptionBar, cxVScroll,
										  max(0, cy - cyCaptionBar) - cyHScroll);

				rcCalendar.left = cxVScroll;
			}
			else
			{
				m_pVScrollBar->MoveWindow(cx - cxVScroll, cyCaptionBar, cxVScroll,
										  max(0, cy - cyCaptionBar) - cyHScroll);
...
void CXTPCalendarControlView::OnEvent_CalendarThemeChanged(XTP_NOTIFY_CODE Event, WPARAM wParam,
														   LPARAM lParam)
{
...
int CXTPCalendarControlView::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	if (!m_wndCalendarCaptionBar.Create(WS_CHILD | WS_TABSTOP, CRect(0, 0, 0, 0), this,
										XTP_ID_CALENDARCAPTIONBAR_CONTROL))
	{
		TRACE(_T("Failed to create Calendar Caption Bar control window\n"));
		return -1;
	}

	if (!GetCalendarCtrl().Create(WS_CHILD | WS_TABSTOP | WS_VISIBLE, CRect(0, 0, 0, 0), this,
								  XTP_ID_CALENDAR_CONTROL))
	{
...
void CXTPCalendarControlView::OnDestroy()
{
	SAFE_DELETE(m_pUpdateContext);
	CView::OnDestroy();
}
...
void CXTPCalendarControlView::OnBeginPrinting(CDC* /*pDC*/, CPrintInfo* pInfo)
{
	...
	ASSERT(m_pUpdateContext == NULL);
	SAFE_DELETE(m_pUpdateContext);
	m_pUpdateContext = new CXTPCalendarControl::CUpdateContext(&GetCalendarCtrl(),
			...
			if (m_bDF_mode)
			{
				...
				if (pDayView->m_nDayViewPrintInterval == 60)
					...
					pDayView->SetScaleInterval(
						COleDateTimeSpan(0, 0, pDayView->m_nDayViewPrintInterval, 0));
	...
	else if (m_bMonthViewMode)
	{
		...
		if (m_dtMonthViewPrintBeginDate.GetStatus() != COleDateTime::valid
			|| m_dtMonthViewPrintEndDate.GetStatus() != COleDateTime::valid)
		{
			m_dtMonthViewPrintBeginDate =
				GetCalendarCtrl().GetMonthView()->GetGrid()->GetBeginDate();
...
void CXTPCalendarControlView::SetupCurrentDayViewPrintPage(CDC* pDC, CPrintInfo* pInfo)
{
	...
	if (pInfo->m_bPreview) // to allow preview go back
	{
		if ((int)pInfo->m_nCurPage - 1 < m_PageStart.GetSize())
			m_nCurCell = XTPToIntChecked(
				m_PageStart.GetAt(XTPToIntPtrChecked(pInfo->m_nCurPage - 1)));
	}
...
void CXTPCalendarControlView::SelectNextDayViewPrintPage(CPrintInfo* pInfo)
{
	...
	else // preview mode - user can switch form page to page in any direction
	{
		...
		{
			m_nCurCell = XTPToIntChecked(m_PageStart.GetAt(XTPToIntPtr(pInfo->m_nCurPage - 1))
										 + m_nCellSteps);
			m_PageStart.Add(XTPToUIntChecked(m_nCurCell));
		}
...
void CXTPCalendarControlView::DoPrintPage(CDC* pDC, CPrintInfo* pInfo)
{
	...
	else
	{
		...
		pDC->BitBlt(rcPrint.left, rcPrint.top, rcPrint.Width(), rcPrint.Height(), &memDC, 0, 0,
					SRCCOPY); // ORIG
	}
}
...
void CXTPCalendarControlView::_OnPrint2(CDC* pDC, CPrintInfo* pInfo, CRect rcPrint)
{
	...
	if (m_bPrintFineMode && m_pPrintOptions->m_bPrintDateHeader
...
void CXTPCalendarControlView::DrawDateHeader(BOOL bCalculate, CDC* pDC, CRect& rrcHeader)
{
	...
	int nMonths = _GetDateHeaderMonths(dtYearMonth1, dtYearMonth2, strHeader1, strHeader2,
									   strHeaderWeekDay);
...
void CXTPCalendarControlView::_DrawDateHeaderCalendar(BOOL bCalculate, CDC* pDC,
													  ...
													  CRect& rrcHeaderCalendar)
{
	...
	rrcHeaderCalendar.bottom = rrcHeaderCalendar.top + nMonthRowY + nWSaysRowY + sizeCell.cy * 6;
	if (bCalculate)
		return;
	...
	for (i = 0; i < 7; i++)
	{
		CString sWD = CXTPCalendarUtils::GetLocaleString(
			XTPToUInt(CXTPCalendarUtils::GetDayOfWeekLocaleStringId(((i - 1 + 7) % 7) + 1)), 100);
	...
	CString strMonthName =
		CXTPCalendarUtils::GetLocaleString(XTPToUInt(LOCALE_SMONTHNAME1 + nMonth - 1), 100);
	...
	if (dtYearMonth.GetYear() == dtToday.GetYear() && dtYearMonth.GetMonth() == dtToday.GetMonth())
...
int CXTPCalendarControlView::_GetDateHeaderMonths(COleDateTime& dtYearMonth1,
												  COleDateTime& dtYearMonth2, CString& strHeader1,
												  CString& strHeader2, CString& strHeaderWeekDay)
{
	...
	if (nViewType == xtpCalendarDayView || (b && nViewType == xtpCalendarWeekView)
		|| nViewType == xtpCalendarWorkWeekView || (b && nViewType == xtpCalendarFullWeekView))
	{
		CXTPCalendarDayView* pDayView = DYNAMIC_DOWNCAST(CXTPCalendarDayView,
														 GetCalendarCtrl().GetDayView());
		if (!pDayView)
		...
		if (nCount == 1)
		{
			...
			strHeaderWeekDay =
				CXTPCalendarUtils::GetLocaleString(XTPToUInt(LOCALE_SDAYNAME1 + nWdShift), 100);
		}

		return 1;
	}
	else if (nViewType == xtpCalendarWeekView || (!b && nViewType == xtpCalendarFullWeekView))
	{
	...
	else if (nViewType == xtpCalendarMonthView)
	{
		...
		if (nWeeksCount)
		{
			CXTPCalendarViewDay* pMVday1 =
				(CXTPCalendarViewDay*)pMonthView->GetGrid()->GetViewDay(0, 6);
			CXTPCalendarViewDay* pMVday2 =
				(CXTPCalendarViewDay*)pMonthView->GetGrid()->GetViewDay(nWeeksCount - 1, 0);
			if (pMVday1 && pMVday2)
			{
...
BEGIN_DISPATCH_MAP(CXTPCalendarControlViewPrintOptions, CXTPPrintOptions)
	...
	DISP_PROPERTY_ID(CXTPCalendarControlViewPrintOptions, "PrintFromToExactly", 13,
					 m_bPrintFromToExactly, VT_BOOL)
	DISP_PROPERTY_ID(CXTPCalendarControlViewPrintOptions, "ThreeSmallCal", 14, m_b3SmallCal,
					 VT_BOOL)
	DISP_PROPERTY_ID(CXTPCalendarControlViewPrintOptions, "MonthViewPrintBeginDate", 24,
					 m_dtMonthViewPrintBeginDate, VT_DATE)
	DISP_PROPERTY_ID(CXTPCalendarControlViewPrintOptions, "MonthViewPrintEndDate", 25,
					 m_dtMonthViewPrintEndDate, VT_DATE)

	DISP_PROPERTY_ID(CXTPCalendarControlViewPrintOptions, "PrintDateHeader", 21, m_bPrintDateHeader,
					 VT_BOOL)
...
LPFONTDISP CXTPCalendarControlViewPrintOptions::OleGetDateHeaderFont()
{
	return AxCreateOleFont(
		m_lfDateHeaderFont, this,
		(LPFNFONTCHANGED)&CXTPCalendarControlViewPrintOptions::OleSetDateHeaderFont);
}
...
{
	return AxCreateOleFont(
		m_lfDateHeaderWeekDayFont, this,
		(LPFNFONTCHANGED)&CXTPCalendarControlViewPrintOptions::OleSetDateHeaderWeekDayFont);
}
...
{
	return AxCreateOleFont(
		m_lfDateHeaderCalendarFont, this,
		(LPFNFONTCHANGED)&CXTPCalendarControlViewPrintOptions::OleSetDateHeaderCalendarFont);
}
