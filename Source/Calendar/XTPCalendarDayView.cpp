...
{
	m_pTimeScaleHeaderLeft			  = new CXTPCalendarDayViewTimeScale(this);
	m_pTimeScaleHeaderRight			  = new CXTPCalendarDayViewTimeScale(this);
	m_pTimeScaleHeaderAdditionalLeft  = new CXTPCalendarDayViewTimeScale(this, FALSE, FALSE);
	m_pTimeScaleHeaderAdditionalRight = new CXTPCalendarDayViewTimeScale(this, FALSE, FALSE);
	...
	m_nScrollOffsetX  = 0;
	m_nMinColumnWidth = -1;
	// 0 - means disabled; -1 use auto for multiresources; -X use for multiresources only; +X used
	// always (in single and multi resources mode).
...
{
	CMDTARGET_RELEASE(m_pTimeScaleHeaderLeft);
	CMDTARGET_RELEASE(m_pTimeScaleHeaderRight);
	CMDTARGET_RELEASE(m_pTimeScaleHeaderAdditionalLeft);
	CMDTARGET_RELEASE(m_pTimeScaleHeaderAdditionalRight);
...
{
	CXTPCalendarControl::CUpdateContext updateContext2(GetCalendarControl(),
...
{
	return m_pTimeScaleHeaderLeft ? m_pTimeScaleHeaderLeft->GetScaleInterval().operator double()
								  : 0.0;
}
	...
	for (int nDayIdx = 0; nDayIdx < nDaysCount; nDayIdx++)
	{
		...
		for (int g = 0; g < nGroupsCount; g++)
		{
			...
			for (int nEvent = 0; nEvent < nEventsCount; nEvent++)
			{
				...
				if ((nMDEFlags & xtpCalendarMultiDayMSmask)
					|| !(nMDEFlags & xtpCalendarMultiDayFMLmask))
				...
				if ((nMDEFlags & xtpCalendarMultiDayLast) == 0)
				{
					...
					for (int i = nDayIdx + 1; i < nDaysCount; i++)
					{
						...
						CXTPCalendarViewEvent* pViewEvent2 =
							XTP_SAFE_GET2(pViewDay2, GetViewGroup_(0),
...
CXTPCalendarData* CXTPCalendarDayView::_GetDataProviderByConnStr(LPCTSTR pcszConnStr,
																 BOOL bCompareNoCase)
{
...
void CXTPCalendarDayView::AdjustLayout(CDC* pDC, const CRect& rcView, BOOL bCallPostAdjustLayout)
{
	...
	XTP_SAFE_CALL1(m_pTimeScaleHeaderLeft, SetCaption(GetScaleText()));
	XTP_SAFE_CALL1(m_pTimeScaleHeaderLeft,
				   SetVisible(!m_bShowAllDayEventsOnly && IsScaleLeftVisible()));

	XTP_SAFE_CALL1(m_pTimeScaleHeaderRight, SetCaption(GetScaleText()));
	XTP_SAFE_CALL1(m_pTimeScaleHeaderRight,
				   SetVisible(!m_bShowAllDayEventsOnly && IsScaleRightVisible()));

	XTP_SAFE_CALL1(m_pTimeScaleHeaderAdditionalLeft, SetCaption(GetAdditionalScaleText()));
	XTP_SAFE_CALL1(m_pTimeScaleHeaderAdditionalLeft, SetVisible(IsAdditionalScaleLeftVisible()));

	XTP_SAFE_CALL1(m_pTimeScaleHeaderAdditionalRight, SetCaption(GetAdditionalScaleText()));
	XTP_SAFE_CALL1(m_pTimeScaleHeaderAdditionalRight, SetVisible(IsAdditionalScaleRightVisible()));
	...
	double dRows = spDaySpan
				   / max((double)m_pTimeScaleHeaderLeft->GetScaleInterval(), XTP_HALF_SECOND * 2);
	...
	if (IsAdditionalScaleLeftVisible())
	{
		int nTimeScale2Width = XTP_SAFE_GET1(m_pTimeScaleHeaderAdditionalLeft, CalcWidth(pDC), 0);

		m_pTimeScaleHeaderAdditionalLeft->m_Layout.m_rcHeader.SetRect(
			rcView2.left, rcView2.top, rcView2.left + nTimeScale2Width, rcView2.bottom);
		rcView2.left += nTimeScale2Width;

		AdjustAdditionalScaleTimeZone();
	}

	if (IsAdditionalScaleRightVisible())
	{
		int nTimeScale2Width = XTP_SAFE_GET1(m_pTimeScaleHeaderAdditionalRight, CalcWidth(pDC), 0);

		m_pTimeScaleHeaderAdditionalRight->m_Layout.m_rcHeader.SetRect(
			rcView2.right - nTimeScale2Width, rcView2.top, rcView2.right, rcView2.bottom);

		rcView2.right -= nTimeScale2Width;

		AdjustAdditionalScaleTimeZone();
	}

	// regular time scale
	if (IsScaleLeftVisible())
	{
		int nTimeScaleWidth = XTP_SAFE_GET1(m_pTimeScaleHeaderLeft, CalcWidth(pDC), 0);
		m_pTimeScaleHeaderLeft->m_Layout.m_rcHeader.SetRect(rcView2.left, rcView2.top,
															rcView2.left + nTimeScaleWidth,
															rcView2.bottom);
		rcView2.left += nTimeScaleWidth;
	}

	if (IsScaleRightVisible())
	{
		int nTimeScaleWidth = XTP_SAFE_GET1(m_pTimeScaleHeaderRight, CalcWidth(pDC), 0);
		m_pTimeScaleHeaderRight->m_Layout.m_rcHeader.SetRect(rcView2.right - nTimeScaleWidth,
															 rcView2.top, rcView2.right,
															 rcView2.bottom);
		rcView2.right -= nTimeScaleWidth;
	}
	...
	const BOOL bIsHideAllDayEventsArea = GetIsHideAllDayEventsArea();
	...
	else
	{
		...
		{
			m_LayoutX.m_nAllDayEventsCountMax = max(m_LayoutX.m_nAllDayEventsCountMax,
													m_nAllDayEventsCountMin_WhenDrag);
	...
	double dVisibleRowCountMax = (double)(rcView2.Height())
								 / (double)(max(m_Layout.m_nRowHeight, 1));
	...
	if (!pDC->IsPrinting()
		&& m_LayoutX.m_nTopRow + m_LayoutX.m_nVisibleRowCount >= m_LayoutX.m_nRowCount)
		m_LayoutX.m_nTopRow = m_LayoutX.m_nRowCount - m_LayoutX.m_nVisibleRowCount;

	int nAllDayEventsHeight = max(0, rcView2.Height()
										 - (m_LayoutX.m_nVisibleRowCount * m_Layout.m_nRowHeight
											+ nDGHeadersHeight));

	m_LayoutX.m_rcDayHeader.SetRect(rcView2.left, rcView2.top, rcView2.right,
									rcView2.top + nDayHeaderHeight
										+ (bIsHideAllDayEventsArea ? nAllDayEventsHeight : 0));

	m_LayoutX.m_rcAllDayEvents.SetRect(rcView2.left, rcView2.top + nDGHeadersHeight, rcView2.right,
									   rcView2.top
										   + (bIsHideAllDayEventsArea ? 0 : nAllDayEventsHeight)
										   + nDGHeadersHeight);
	...
	for (int i = 0; i < nDaysCount; i++)
	{
		...
		// int nWidth = (i == nDaysCount - 1) ? rcView2.Width() : rcView2.Width() / (nColumnsCount -
		// nCols);
	...
	//----------------------------------------------------------
	XTP_SAFE_CALL1(m_pTimeScaleHeaderLeft, AdjustLayout(pDC));
	XTP_SAFE_CALL1(m_pTimeScaleHeaderRight, AdjustLayout(pDC));
	XTP_SAFE_CALL1(m_pTimeScaleHeaderAdditionalLeft, AdjustLayout(pDC));
	XTP_SAFE_CALL1(m_pTimeScaleHeaderAdditionalRight, AdjustLayout(pDC));
	...
	if (nDVCount)
	{
			...
			|| m_selectedBlock.dtBegin.GetStatus() != COleDateTime::valid)
		{
			...
			m_selectedBlock.dtBegin		 = CXTPCalendarUtils::UpdateTime(dtFirstDay,
																	 GetCellTime(nSelRow));
		...
		COleDateTime dtSelEndDay   = GetDateTimeDay(m_selectedBlock.dtEnd
													- COleDateTimeSpan(0, 0, 0, 1));

		// compare with the first day
		if (dtSelBeginDay < dtFirstDay)
		{
			m_selectedBlock.dtBegin = CXTPCalendarUtils::UpdateDate(m_selectedBlock.dtBegin,
																	dtFirstDay);
			bSelChanged				= TRUE;
		}
		if (dtSelEndDay < dtFirstDay)
		{
			m_selectedBlock.dtEnd = CXTPCalendarUtils::UpdateDate(m_selectedBlock.dtEnd,
																  dtFirstDay);
			bSelChanged			  = TRUE;
		}
...
void CXTPCalendarDayView::AdjustLayout2(CDC* pDC, const CRect& rcView, BOOL bCallPostAdjustLayout)
{
	...
	if (IsAdditionalScaleLeftVisible() || IsAdditionalScaleRightVisible())
	{
		AdjustAdditionalScaleTimeZone();
	}
	...
	if (m_eDraggingMode == xtpCalendaDragModeResizeBegin
		|| m_eDraggingMode == xtpCalendaDragModeResizeEnd || _IsDragModeCopyMove(m_eDraggingMode))
	{
		m_LayoutX.m_nAllDayEventsCountMax = max(m_LayoutX.m_nAllDayEventsCountMax,
												m_nAllDayEventsCountMin_WhenDrag);
		m_nAllDayEventsCountMin_WhenDrag  = m_LayoutX.m_nAllDayEventsCountMax;
...
void CXTPCalendarDayView::OnDraw(CDC* pDC)
{
	...
	if (m_pTimeScaleHeaderLeft && IsScaleLeftVisible())
		m_pTimeScaleHeaderLeft->MoveTo(m_nScrollOffsetX);

	if (m_pTimeScaleHeaderRight && IsScaleRightVisible())
		m_pTimeScaleHeaderRight->MoveTo(m_nScrollOffsetX);

	if (m_pTimeScaleHeaderAdditionalLeft && IsAdditionalScaleLeftVisible())
		m_pTimeScaleHeaderAdditionalLeft->MoveTo(m_nScrollOffsetX);

	if (m_pTimeScaleHeaderAdditionalRight && IsAdditionalScaleRightVisible())
		m_pTimeScaleHeaderAdditionalRight->MoveTo(m_nScrollOffsetX);
...
void CXTPCalendarDayView::Draw(CDC* pDC)
{
	...
	// additional time scale
	if (IsAdditionalScaleLeftVisible() && m_pTimeScaleHeaderAdditionalLeft)
		m_pTimeScaleHeaderAdditionalLeft->Draw(pDC);

	if (IsDrawNowLineOverAppoinment())
		for (i = nCount - 1; i >= 0; i--)
			XTP_SAFE_CALL1(GetViewDay(i), DrawEvents(pDC));

	if (m_pTimeScaleHeaderLeft && IsScaleLeftVisible())
		m_pTimeScaleHeaderLeft->Draw(pDC);

	if (m_pTimeScaleHeaderRight && IsScaleRightVisible())
		m_pTimeScaleHeaderRight->Draw(pDC);

	if (IsAdditionalScaleRightVisible() && m_pTimeScaleHeaderAdditionalRight)
		m_pTimeScaleHeaderAdditionalRight->Draw(pDC);
...
BOOL CXTPCalendarDayView::HitTestEx(CPoint pt, XTP_CALENDAR_HITTESTINFO* pHitTest) const
{
	...
	if (m_pTimeScaleHeaderAdditionalLeft
		&& m_pTimeScaleHeaderAdditionalLeft->HitTestDateTime(pHitTest))
		return TRUE;

	if (m_pTimeScaleHeaderAdditionalRight
		&& m_pTimeScaleHeaderAdditionalRight->HitTestDateTime(pHitTest))
		return TRUE;

	if (m_pTimeScaleHeaderLeft && m_pTimeScaleHeaderLeft->HitTestDateTime(pHitTest))
		return TRUE;

	if (m_pTimeScaleHeaderRight && m_pTimeScaleHeaderRight->HitTestDateTime(pHitTest))
		return TRUE;
		...
		if (bNeedScroll && m_LayoutX.m_rcAllDayEvents.PtInRect(pt))
		{
			...
			for (int nDayIdx = 0; nDayIdx < nDaysCount; nDayIdx++)
			{
				...
				for (int g = 0; g < nGroupsCount; g++)
				{
					...
					if (pThemeX->ExpandDayButton_HitTest(
							pt,
							pViewGroup->GetAllDayEventsScrollButtonRect(
					...
					if (iAllDayCount > nVisAllDayEvents)
					{
						...
						if (pThemeX->ExpandDayButton_HitTest(pt, rcIconsMax1, FALSE))
						{
							...
							*pHitTest = emptyData;
							...
							// xtpCalendarHitTestDayViewAllDayEvent;
						...
						for (int nEvent = 0; nEvent < pViewGroup->GetViewEventsCount(); nEvent++)
						{
							...
							if (pViewEvent)
							{
								if (pViewEvent->IsMultidayEvent())
								{
									...
									{
										if ((pViewEvent->GetMultiDayEventFlags()
											 & xtpCalendarMultiDaySlave)
											> 0)
											return FALSE;
...
void CXTPCalendarDayView::OnStartDragging(CPoint point, XTP_CALENDAR_HITTESTINFO* pHitTest)
{
	...
	if (pHitTest->pViewEvent != NULL && pHitTest->pViewEvent->GetEvent()->IsAllDayEvent())
	{
...
BOOL CXTPCalendarDayView::OnDragging(CPoint point, XTP_CALENDAR_HITTESTINFO* pHitTest)
{
	...
	if (_IsDragModeCopyMove(m_eDraggingMode)
		&& (m_pTimeScaleHeaderAdditionalLeft
				&& m_pTimeScaleHeaderAdditionalLeft->HitTestDateTime(&hitTestInfo)
			|| (m_pTimeScaleHeaderAdditionalRight
				&& m_pTimeScaleHeaderAdditionalRight->HitTestDateTime(&hitTestInfo))
			|| (m_pTimeScaleHeaderLeft && m_pTimeScaleHeaderLeft->HitTestDateTime(&hitTestInfo))
			|| (m_pTimeScaleHeaderRight && m_pTimeScaleHeaderRight->HitTestDateTime(&hitTestInfo))))
	{
	...
	BOOL bAllMultiDayEvent	= bAllDayEventNew_prev
	...
	if (bChanged)
	{
		...
		{
			m_ptrDraggingEventNew->SetStartTime(
				GetDateTimeDay(m_ptrDraggingEventOrig->GetStartTime()));
			m_ptrDraggingEventNew->SetEndTime(m_ptrDraggingEventNew->GetStartTime()
											  + COleDateTimeSpan(1, 0, 0, 0));
		}
		else
		{
				...
				if (GetCalendarControl()->DayView_IsAutoResetBusyFlag())
				{
					if (!bAllDayEventNew_prev
						...
						&& !(bAllMultiDayEventOrig
							 && xtpCalendarBusyStatusBusy
									== m_ptrDraggingEventOrig->GetBusyStatus()))
					{
	...
	if (_IsDragModeCopyMove(m_eDraggingMode))
	{
		...
		if (uScheduleID != m_ptrDraggingEventNew->GetScheduleID())
		{
			...
			else if (m_ptrDraggingEventNew->GetScheduleID()
					 != m_ptrDraggingEventOrig->GetScheduleID())
			{
...
{
	int nVScrollZone = XTP_DPI_Y(20);
	int nHScrollZone = XTP_DPI_X(80);
	...
	if (b2Up && pnt.y >= nTop && pnt.y <= (nTop + nVScrollZone) && nCurPos > 0)
		return xtpCalendarDayViewScrollUp;
	if (b2Down && (pnt.y + XTP_DPI_Y(3)) < nHeight
		&& (pnt.y + XTP_DPI_Y(3)) >= (nHeight - nVScrollZone) && nCurPos < nLimit)
	...
	if (b2Left && pnt.x > rcVisible.left && pnt.x <= (rcVisible.left + nHScrollZone)
		&& nCurPosH > 0)
		return xtpCalendarDayViewScrollLeft;
	if (b2Right && (pnt.x + XTP_DPI_X(3)) < rcVisible.right
		&& (pnt.x + XTP_DPI_X(3)) >= (rcVisible.right - nHScrollZone) && nCurPosH < nLimitH)
...
BOOL CXTPCalendarDayView::IsOutOfClientRect(CPoint pnt, XTP_CALENDAR_HITTESTINFO* pHitInfo)
{
	...
	if (m_pTimeScaleHeaderAdditionalLeft
		&& m_pTimeScaleHeaderAdditionalLeft->HitTestDateTime(pHitInfo))
		return FALSE;

	if (m_pTimeScaleHeaderAdditionalRight
		&& m_pTimeScaleHeaderAdditionalRight->HitTestDateTime(pHitInfo))
		return FALSE;

	if (m_pTimeScaleHeaderLeft && m_pTimeScaleHeaderLeft->HitTestDateTime(pHitInfo))
		return FALSE;

	if (m_pTimeScaleHeaderRight && m_pTimeScaleHeaderRight->HitTestDateTime(pHitInfo))
		return FALSE;
...
void CXTPCalendarDayView::SetMouseOutOfDragArea(BOOL bOutOfArea)
{
	...
	if (bChanged && !m_bMouseOutOfDragArea)
	{
		...
		switch (m_eDraggingMode)
		{
			...
			case xtpCalendaDragModeEditSubject: mouseMode = xtpCalendarMouseNothing; break;
			case xtpCalendaDragModeCopy: mouseMode = xtpCalendarMouseEventDragCopy; break;
			case xtpCalendaDragModeMove: mouseMode = xtpCalendarMouseEventDragMove; break;
			case xtpCalendaDragModeResizeBegin:
			case xtpCalendaDragModeResizeEnd: mouseMode = xtpCalendarMouseEventResizingV; break;
		}
...
BOOL CXTPCalendarDayView::OnEndDragging(CPoint point, XTP_CALENDAR_HITTESTINFO* pHitInfo)
{
	...
	if (_IsDragModeCopyMove(m_eDraggingMode) && !!m_ptrDraggingEventNew && !!m_ptrDraggingEventOrig)
	{
		if (!m_ptrDraggingEventNew->IsEqualStartEnd(m_ptrDraggingEventOrig))
		{
			...
			m_selectedBlock.dtBegin =
				CXTPCalendarUtils::UpdateTime(dtS, GetCellTime(GetCellNumber(dtS0, FALSE)));
			m_selectedBlock.dtEnd = GetCellTime(GetCellNumber(dtE0, dtS0 == dtE0 ? FALSE : TRUE))
									+ GetCellDuration();
...
BOOL CXTPCalendarDayView::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	if (HitTestEx(point, &hitTestInfo))
	{
		...
		else
		{
			...
			{
				dtNewEnd		   = CXTPCalendarUtils::UpdateTime(dtNewEnd,
...
void CXTPCalendarDayView::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	if ((m_eDraggingMode != xtpCalendaDragModeUnknown) || ((nFlags & MK_LBUTTON) == 0)
	...
	if (HitTestEx(point, &hitTestInfo))
	{
		...
		if (hitTestInfo.uHitCode & xtpCalendarHitTestDayViewTimeScale)
		{
			...
			m_selectedBlock.dtEnd = CXTPCalendarUtils::UpdateTime(m_selectedBlock.dtEnd,
																  hitTestInfo.dt);
...
void CXTPCalendarDayView::SetScaleInterval(const COleDateTimeSpan spScaleInterval)
{
	...
	if (m_pTimeScaleHeaderLeft && bValidSVal)
	{
...
void CXTPCalendarDayView::SetScaleMaxTime(const COleDateTime dtNewScaleMaxTime)
{
	if (dtNewScaleMaxTime.GetStatus() == COleDateTime::valid)
	{
		...
		if ((double)dtNewScaleMaxTime > 1
			&& (double)dtNewScaleMaxTime < 2) // Special Case for 6:00 - 5:59 of second day.
...
{
	XTP_SAFE_CALL1(m_pTimeScaleHeaderLeft, SetCaption(strText));
	XTP_SAFE_CALL1(m_pTimeScaleHeaderRight, SetCaption(strText));
...
{
	return XTP_SAFE_GET2(m_pControl, GetCalendarOptions(), strDayView_ScaleLabel, CString());
}

void CXTPCalendarDayView::SetAdditionalScaleText(LPCTSTR strText)
{
	XTP_SAFE_CALL1(m_pTimeScaleHeaderAdditionalLeft, SetCaption(strText));
	XTP_SAFE_CALL1(m_pTimeScaleHeaderAdditionalRight, SetCaption(strText));

	XTP_SAFE_SET1(GetCalendarOptions(), strDayView_AdditionalScaleLabel, strText);

	SendNotification(XTP_NC_CALENDAROPTIONSWASCHANGED, xtpCalendarDayView, 0);
	SendNotification(XTP_NC_CALENDARVIEWTIMESCALEWASCHANGED, 0, 0);
}

CString CXTPCalendarDayView::GetAdditionalScaleText() const
{
	return XTP_SAFE_GET2(m_pControl, GetCalendarOptions(), strDayView_AdditionalScaleLabel,
						 CString());
}

void CXTPCalendarDayView::SetAdditionalScaleTimeZone(const TIME_ZONE_INFORMATION* pTzInfo)
{
	...
	m_pControl->GetCalendarOptions()->tziDayView_AdditionalScaleTimeZone = *pTzInfo;
...
void CXTPCalendarDayView::AdjustAdditionalScaleTimeZone()
{
	...
	TIME_ZONE_INFORMATION tziScale2 = GetAdditionalScaleTimeZone();
	...
	if (!CXTPCalendarUtils::SystemTimeToTzSpecificLocalTime(&tziCurrent, &systm00, &systmScale1))
	{
		ASSERT(FALSE);
		return;
	}
	if (!CXTPCalendarUtils::SystemTimeToTzSpecificLocalTime(&tziScale2, &systm00, &systmScale2))
	{
		ASSERT(FALSE);
		return;
	}
	...
	XTP_SAFE_CALL1(m_pTimeScaleHeaderAdditionalLeft, SetTimeshift(nTimeShift2_min));
	XTP_SAFE_CALL1(m_pTimeScaleHeaderAdditionalRight, SetTimeshift(nTimeShift2_min));
}

const TIME_ZONE_INFORMATION& CXTPCalendarDayView::GetAdditionalScaleTimeZone() const
{
	static TIME_ZONE_INFORMATION s_tzEmty = { 0 };

	return XTP_SAFE_GET2(m_pControl, GetCalendarOptions(), tziDayView_AdditionalScaleTimeZone,
						 s_tzEmty);
}

void CXTPCalendarDayView::ShowAdditionalScale(BOOL bShow)
{
	if (!m_pTimeScaleHeaderAdditionalLeft || !m_pControl)
	{
		ASSERT(FALSE);
		return;
	}

	if (!m_pTimeScaleHeaderAdditionalRight || !m_pControl)
	{
		ASSERT(FALSE);
		return;
	}

	m_pTimeScaleHeaderAdditionalLeft->SetVisible(bShow);
	m_pTimeScaleHeaderAdditionalRight->SetVisible(bShow);

	m_pControl->GetCalendarOptions()->bDayView_AdditionalScaleLeftVisible  = bShow;
	m_pControl->GetCalendarOptions()->bDayView_AdditionalScaleRightVisible = bShow;
...
BOOL CXTPCalendarDayView::IsScaleLeftVisible() const
{
	return XTP_SAFE_GET2(m_pControl, GetCalendarOptions(), bDayView_ScaleLeftVisible, FALSE);
}

BOOL CXTPCalendarDayView::IsScaleRightVisible() const
{
	return XTP_SAFE_GET2(m_pControl, GetCalendarOptions(), bDayView_ScaleRightVisible, FALSE);
}

BOOL CXTPCalendarDayView::IsAdditionalScaleLeftVisible() const
{
	return XTP_SAFE_GET2(m_pControl, GetCalendarOptions(), bDayView_AdditionalScaleLeftVisible,
						 FALSE);
}

BOOL CXTPCalendarDayView::IsAdditionalScaleRightVisible() const
{
	return XTP_SAFE_GET2(m_pControl, GetCalendarOptions(), bDayView_AdditionalScaleRightVisible,
						 FALSE);
}

BOOL CXTPCalendarDayView::IsDrawNowLineOverAppoinment() const
{
	return XTP_SAFE_GET2(m_pControl, GetCalendarOptions(), bDayView_DrawNowLineOverAppoinment,
						 FALSE);
}
...
COleDateTime CXTPCalendarDayView::GetViewDayDate(int nIndex) const
{
	...
	ASSERT(pViewDay);
	return pViewDay ? pViewDay->m_dtDate : GetDateTimeDay(CXTPCalendarUtils::GetCurrentTime());
}
...
void CXTPCalendarDayView::_ScrollDays(int nScrollDaysCount, BOOL bPrev)
{
	...
	for (int i = 0; i < nScrollDaysCount; i++)
	{
		...
		if (pDay)
		{
			...
			// moreover without this check there was a crash when Calendar.Unicode.ocx is built on
			// VC6 and vb calendar sample tried scrolling days (with keyboard) and tooltip was
			// active possible reason of the crash was calling HideToolTip from the smart pointer
			// destructor (invoked by RemoveAt) when a pointer on the call stack became invalid
...
void CXTPCalendarDayView::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	if (nChar == VK_LEFT || nChar == VK_RIGHT || nChar == VK_UP || nChar == VK_DOWN
		|| nChar == VK_PRIOR || nChar == VK_NEXT || nChar == VK_HOME || nChar == VK_END)
	{
		CXTPCalendarControl::CUpdateContext updateContext(m_pControl, xtpCalendarUpdateRedraw);
		...
		int nSelCell   = GetCellNumber(
			  GetDateTimeTime(m_selectedBlock.dtEnd - COleDateTimeSpan(0, 0, 0, bEndIsEnd ? 1 : 0)),
			  bEndIsEnd);
		...
		if (GetViewType() == xtpCalendarDayView || GetViewType() == xtpCalendarWorkWeekView)
		{
			COleDateTime dtSelDay = CXTPCalendarUtils::ResetTime(dtNewSelEnd);
			...
			if (nChar == VK_RIGHT)
			{
				for (int i = 0; i < GetViewDayCount(); i++)
				{
					...
					if (pDay)
					{
						COleDateTime dtDay = pDay->GetDayDate();
						if (dtDay > dtSelDay)
						{
							spDay = COleDateTimeSpan(dtDay - dtSelDay);
							break;
						}
					}
				}
			...
			if (nChar == VK_LEFT)
			{
				for (int i = GetViewDayCount() - 1; i >= 0; i--)
				{
					CXTPCalendarDayViewDay* pDay = GetViewDay(i);
					if (pDay)
					{
						...
						if (dtDay < dtSelDay)
						{
							spDay = COleDateTimeSpan(dtSelDay - dtDay);
							break;
						}
					}
				}
		...
		if (GetScrollBarInfoH(&si, &nScrollStep))
		{
			...
			if (bVKControl && iStP > 0
				&& iResCnt > 1) // control horizontal scrolling for multi-schedules case
			{
				if (nChar == VK_LEFT)
				{
					if (bVKShift)
			...
			else if (nChar == VK_RIGHT)
			{
				...
				if (!m_bScrollH_Disabled || dtNSel < dtLastDay || bCanHScrollGroupsRight)
				{
					...
					// dtNewSelEnd.Format(_T("%d %m %Y")), nCurPos);
					if (m_selectedBlock.bAllDayEvent)
					{
						nCurPos = min(nCurPos + iStP, nLimit);
						ScrollH(nCurPos, nCurPos);
			...
			{
				BOOL bRange = !m_bScrollV_Disabled
							  || nSelCell < m_LayoutX.m_nTopRow + m_LayoutX.m_nVisibleRowCount - 1;
				if (nSelCell + 1 < GetRowCount() && bRange)
				{
					dtNewSelEnd = CXTPCalendarUtils::UpdateTime(dtNewSelEnd,
																GetCellTime(nSelCell + 1))
								  + spCell;
			...
			else if (nChar == VK_NEXT)
			{
				if (!m_bScrollV_Disabled)
				{
					nSelCell = min(max(0, GetRowCount() - 1),
								   nSelCell + m_LayoutX.m_nVisibleRowCount);
					m_LayoutX.m_nTopRow += m_LayoutX.m_nVisibleRowCount;
					dtNewSelEnd = CXTPCalendarUtils::UpdateTime(dtNewSelEnd, GetCellTime(nSelCell))
								  + spCell;
		...
		else
		{
			if (nChar == VK_LEFT)
			{
				...
				if (!m_bScrollH_Disabled || GetDateTimeDay(dtNewSelEnd) > dtFirstDay
					|| bCanHScrollGroupsLeft)
				{
				...
				{
					CXTPCalendarControl::CUpdateContext updateContextAll(GetCalendarControl(),
																		 xtpCalendarUpdateAll);
			...
			else if (nChar == VK_RIGHT)
			{
				...
				if (!m_bScrollH_Disabled || GetDateTimeDay(dtNewSelEnd) < dtLastDay
					|| bCanHScrollGroupsRight)
				{
...
void CXTPCalendarDayView::ProcessCellSelection(COleDateTime dtNewSelEnd, BOOL bFixSelBegin,
											   BOOL bAllDayEventSel, int nGroupIndex)
{
	...
	int nSelEndCell = GetCellNumber(GetDateTimeTime(m_selectedBlock.dtEnd
													- COleDateTimeSpan(0, 0, 0, bEndIsEnd ? 1 : 0)),
									bEndIsEnd);
...
BOOL CXTPCalendarDayView::_EnsureVisibleSelectionH()
{
	...
	CXTPCalendarViewDay* pVDay = _GetViewDay(m_selectedBlock.bAllDayEvent
												 ? m_selectedBlock.dtEnd
												 : GetDateTimeDay(m_selectedBlock.dtEnd));
	ASSERT(pVDay);
...
BOOL CXTPCalendarDayView::_EnsureVisibleV(CXTPCalendarViewEvent* pViewEvent)
{
	...
	if (pViewEvent->IsMultidayEvent())
	{
		...
		{
			int nEventIndex = (pViewEvent->m_rcEvent.top - m_LayoutX.m_rcAllDayEvents.top)
								  / m_LayoutX.m_nAllDayEventHeight
							  + m_LayoutX.m_nTopAllDayRow;
	...
	//---------------------------------------------------------------------------
	COleDateTime dtDay = XTP_SAFE_GET3(pViewEvent, GetViewGroup_(), GetViewDay_(),
...
CXTPCalendarViewEvent* CXTPCalendarDayView::FindEventToEditByTAB(COleDateTime dtMinStart,
																 BOOL bReverse,
																 CXTPCalendarEvent* pAfterEvent)
{
	CXTPCalendarViewEvent* pViewEvent = TBase::FindEventToEditByTAB(dtMinStart, bReverse,
																	pAfterEvent);
	if (pViewEvent)
	{
...
CXTPCalendarTimeZonePtr CXTPCalendarDayView::GetAdditionalScaleTimeZoneInfo() const
{
	return CXTPCalendarTimeZone::GetTimeZoneInfo(&GetAdditionalScaleTimeZone());
}
...
void CXTPCalendarDayView::ScrollToWorkDayBegin()
{
	...
	COleDateTime dtSelBegin  = CXTPCalendarUtils::UpdateTime(
		 dtStartDate, m_pControl->GetCalendarOptions()->dtWorkDayStartTime);
...
BEGIN_DISPATCH_MAP(CXTPCalendarDayView, CXTPCalendarView)
	DISP_PROPERTY_EX_ID(CXTPCalendarDayView, "TimeScale", 1, OleGetScaleInterval,
						OleSetScaleInterval, VT_I4)
	DISP_PROPERTY_EX_ID(CXTPCalendarDayView, "ScaleText", 2, OleGetScaleText, SetScaleText, VT_BSTR)

	DISP_PROPERTY_EX_ID(CXTPCalendarDayView, "Scale2Visible", 4, OleIsScale2Visible, OleShowScale2,
						VT_BOOL)
	DISP_PROPERTY_EX_ID(CXTPCalendarDayView, "Scale2Text", 5, OleGetScale2Text,
						SetAdditionalScaleText, VT_BSTR)
	...
	DISP_FUNCTION_ID(CXTPCalendarDayView, "ScrollToWorkDayBegin", 12, OleScrollToWorkDayBegin,
					 VT_EMPTY, VTS_NONE)
	DISP_FUNCTION_ID(CXTPCalendarDayView, "EnsureVisible", 13, OleEnsureVisible, VT_BOOL,
					 VTS_DISPATCH)
	DISP_FUNCTION_ID(CXTPCalendarDayView, "EnsureVisibleGroup", 15, OleEnsureVisibleGroup, VT_BOOL,
					 VTS_DISPATCH)
	DISP_FUNCTION_ID(CXTPCalendarDayView, "ScrollToColumn", 16, OleScrollToColumn, VT_EMPTY, VTS_I4)

	DISP_PROPERTY_EX_ID(CXTPCalendarDayView, "MinColumnWidth", 14, GetMinColumnWidth,
						SetMinColumnWidth, VT_I4)

	DISP_PROPERTY_EX_ID(CXTPCalendarDayView, "TimeScaleMinTime", 21, OleGetTimeScaleMinTime,
						OleSetTimeScaleMinTime, VT_DATE)
	DISP_PROPERTY_EX_ID(CXTPCalendarDayView, "TimeScaleMaxTime", 22, OleGetTimeScaleMaxTime,
						OleSetTimeScaleMaxTime, VT_DATE)

	// DISP_PROPERTY_ID(CXTPCalendarDayView, "EcoMode", 23, m_bEcoMode, VT_BOOL)
	DISP_PROPERTY_EX_ID(CXTPCalendarDayView, "DayHeaderPeriod", 24, OleGetDayHeaderPeriod,
						OleSetDayHeaderPeriod, VT_I4)
	DISP_PROPERTY_ID(CXTPCalendarDayView, "DraftMode", 25, m_bDraftMode, VT_BOOL)
	DISP_PROPERTY_ID(CXTPCalendarDayView, "RepeatHeaderMode", 26, m_bPrintRepeatHeaderFooterMode,
					 VT_BOOL)
	...
	DISP_PROPERTY_EX_ID(CXTPCalendarDayView, "PreventAutoDropAllDayMode", 29,
						OleGetPreventAutoDropAllDayMode, OleSetPreventAutoDropAllDayMode, VT_BOOL)
	DISP_PROPERTY_EX_ID(CXTPCalendarDayView, "DayViewPrintInterval", 30, OleGetDayViewPrintInterval,
						OleSetDayViewPrintInterval, VT_I4)
	DISP_PROPERTY_ID(CXTPCalendarDayView, "PrintFullScale", 31, m_bPrintFullScale, VT_BOOL)
	DISP_PROPERTY_ID(CXTPCalendarDayView, "PreventDragAllDayEventInDayView", 32,
					 m_bPreventDragAllDayEventInDayView, VT_BOOL)
	...
	DISP_PROPERTY_ID(CXTPCalendarDayView, "ShowAllDayEventsOnly", 35, m_bShowAllDayEventsOnly,
					 VT_BOOL)

//  DISP_FUNCTION_ID(CXTPCalendarDayView, "GetScale2TimeZone", 104, OleGetScale2TimeZone,
//  VT_DISPATCH, VTS_NONE) DISP_FUNCTION_ID(CXTPCalendarDayView, "SetScale2TimeZone", 105,
//  OleSetScale2TimeZone, VT_EMPTY, VTS_DISPATCH) DISP_FUNCTION_ID(CXTPCalendarDayView,
//  "EnumAllTimeZones", 106, OleEnumAllTimeZones, VT_DISPATCH, VTS_NONE)
//  DISP_FUNCTION_ID(CXTPCalendarDayView, "GetCurrentTimeZone", 107, OleGetCurrentTimeZone,
...
{
	return GetAdditionalScaleText().AllocSysString();
}

BOOL CXTPCalendarDayView::OleIsScale2Visible()
{
	return IsAdditionalScaleLeftVisible() ? TRUE : FALSE;
}

void CXTPCalendarDayView::OleShowScale2(BOOL bShow)
{
	ShowAdditionalScale(bShow != FALSE);
}
...
{
	CXTPCalendarViewEvent* pViewEvent = XTP_DYNAMIC_DOWNCAST_REMOTE_(
		CXTPCalendarViewEvent, CXTPCalendarViewEvent::FromIDispatchSafe(pdispViewEvent));
	if (!pViewEvent)
	{
		AfxThrowOleException(E_INVALIDARG);
	}
...
{
	CXTPCalendarViewGroup* pViewGroup = XTP_DYNAMIC_DOWNCAST_REMOTE_(
		CXTPCalendarViewGroup, CXTPCalendarViewGroup::FromIDispatchSafe(pdispViewGroup));
	if (!pViewGroup)
	{
		AfxThrowOleException(E_INVALIDARG);
	}
...
{
	if (nDayViewPrintInterval == 60 || nDayViewPrintInterval == 45 || nDayViewPrintInterval == 30
