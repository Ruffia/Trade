...
#include "Calendar/Themes/XTPCalendarVisualStudio2017Theme.h"
#include "Calendar/Themes/XTPCalendarVisualStudio2019Theme.h"
#include "Calendar/Themes/XTPCalendarVisualStudio2022Theme.h"
...
CXTPCalendarControl::CXTPCalendarControl()
{
	...
	//  pTheme2007->GetDayViewPartX()->GetDayPartX()->GetGroupPartX()->GetSingleDayEventPartX()->m_fcsetSelected.clrBorder
	//  = RGB(255, 100, 230);
	/*
...
{
	return XTPDrawHelpers()->RegisterWndClass(hInstance, XTPCALENDARCTRL_CLASSNAME,
...
void CXTPCalendarControl::OnPaint()
{
	...
	else
	{
		...
		if (!IsWindowEnabled())
		{
			XTPImageManager()->DisableBitmap(memDC, rc, XTP_CALENDAR_DISABLED_COLOR_LIGHT,
											 XTP_CALENDAR_DISABLED_COLOR_DARK);
		}
...
void CXTPCalendarControl::AdjustScrollBarEx(int nSBType)
{
	...
	::EnableScrollBar(m_hWnd, XTPToUInt(nSBType),
					  XTPToUInt((bEnabled && IsWindowEnabled()) ? ESB_ENABLE_BOTH
																: ESB_DISABLE_BOTH));
}
...
void CXTPCalendarControl::RedrawControl(BOOL bForce)
{
	...
	if (GetSafeHwnd() && (bForce || (m_nLockUpdateCount == 0)))
	{
		Invalidate(FALSE);
		if (bForce)
			UpdateWindow();
...
void CXTPCalendarControl::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	...
	switch (nSBCode)
	{
		case SB_TOP: nCurPos = nCurPos_raw = 0; break;
		case SB_BOTTOM: nCurPos = nCurPos_raw = nLimit; break;
		case SB_LINEUP:
		...
		case SB_PAGEDOWN:
			...
			break;
		case SB_THUMBTRACK:
		case SB_THUMBPOSITION: nCurPos = nCurPos_raw = XTPToIntChecked(nPos); break;
		case SB_ENDSCROLL:
...
void CXTPCalendarControl::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	...
	if (m_pActiveView->GetScrollBarInfoH(&si, &nScrollStep))
	{
		...
		switch (nSBCode)
		{
			case SB_LEFT: nCurPos = nCurPos_raw = 0; break;
			case SB_RIGHT: nCurPos = nCurPos_raw = nLimit; break;
			case SB_LINELEFT:
				if (m_bTimelineMode)
				{
			...
			case SB_THUMBPOSITION: nCurPos = nCurPos_raw = XTPToIntChecked(nPos); break;
			case SB_ENDSCROLL: m_pActiveView->OnScrollEnd(SB_HORZ); return;
		}
...
COleDateTime CXTPCalendarControl::GetTodayDate() const
{
	...
	return (GetTodayDateOverride().GetStatus() == COleDateTime::valid
				? GetTodayDateOverride()
...
BOOL CXTPCalendarControl::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt)
{
	...
	if (m_pActiveView)
	{
		...
		BOOL bScrollEnabled = m_bTimelineMode ? m_pActiveView->GetScrollBarInfoH(&si)
											  : m_pActiveView->GetScrollBarInfoV(&si);
		...
		if (bScrollEnabled)
		{
			...
			if (nViewType == xtpCalendarDayView || nViewType == xtpCalendarWorkWeekView)
			{
				...
				GetScrollRange(m_bTimelineMode ? SB_HORZ : SB_VERT, &nPosMin, &nPosMax);

				nPosMax = GetScrollLimit(m_bTimelineMode ? SB_HORZ : SB_VERT);

				int nPos = GetScrollPos(m_bTimelineMode ? SB_HORZ : SB_VERT);
...
void CXTPCalendarControl::SwitchActiveView(XTPCalendarViewType eViewType)
{
	...
	if (pPrevView)
	{
		...
		if (bSelDay)
		{
			spSelDayDuration = CXTPCalendarUtils::ResetTime(dtSelDay2)
							   - CXTPCalendarUtils::ResetTime(dtSelDay);
		...
		{
			COleDateTime dtEvent = XTP_SAFE_GET3(ptrSelEvents, GetAt(i), GetEvent(),
												 GetStartTime().operator DATE(), (DATE)0);
			dtSelEvent			 = min(dtSelEvent, dtEvent);
		}

		dtSelStartDate = nCount ? dtSelEvent
								: (bSelDay ? dtSelDay : COleDateTime::GetCurrentTime());
		//<<----------------------------------------------------------------------------------------->>
	...
	COleDateTime dtWDStartTime = XTP_SAFE_GET1(GetCalendarOptions(),
											   dtWorkDayStartTime.operator DATE(), (DATE)0);
	...
	else if (eViewType == xtpCalendarWorkWeekView)
	{
		...
		COleDateTime dtRes, dtCurDay = COleDateTime::GetCurrentTime();
		int nSelDay = dtSelDay.GetDayOfWeek();
		int nCurDay = dtCurDay.GetDayOfWeek();
		if ((GetWorkWeekMask() & CXTPCalendarUtils::GetDayOfWeekMask(nSelDay)) ? 1 : 0)
		{
			dtRes = dtSelDay;
		}
		else
		{
			if ((GetWorkWeekMask() & CXTPCalendarUtils::GetDayOfWeekMask(nCurDay)) ? 1 : 0)
				dtRes = dtCurDay;
			else
				dtRes = pDayView->GetViewDayDate(0);
		}

		COleDateTime dtSelBegin = CXTPCalendarUtils::UpdateTime(dtRes, dtWDStartTime);
		if (bTL)
	...
	else if (eViewType == xtpCalendarWeekView)
	{
		if (m_bMultiColumnWeekMode)
		{
			...
			COleDateTime dtSelBegin = CXTPCalendarUtils::UpdateTime(pDayView->GetViewDayDate(0),
																	dtWDStartTime);
			if (bTL)
				dtSelBegin = dtSelStartDateTL;
			COleDateTime dtSelEnd = dtSelBegin + pDayView->GetCellDuration();
	...
	else if (eViewType == xtpCalendarMonthView)
	{
		...
		COleDateTime dtFirstOfMonth(dtSelStartDate.GetYear(), dtSelStartDate.GetMonth(), 1, 0, 0,
									0);
	...
	else if (eViewType == xtpCalendarTimeLineView)
	{
		...
		CXTPCalendarTimeLineView* pTLView = DYNAMIC_DOWNCAST(CXTPCalendarTimeLineView,
															 GetTimeLineView());
		if (!pTLView)
			return;
...
void CXTPCalendarControl::OnOptionsChanged(int nOptionRelation)
{
	...
	CWnd::SetTimer(XTP_CALENDAR_REFRESHMETRICS_REQUEST_TIMERID,
				   XTP_CALENDAR_REFRESHMETRICS_REQUEST_TIMEOUT, NULL);
}
...
void CXTPCalendarControl::SetDataProvider(XTPCalendarDataProvider eDataProvider,
										  LPCTSTR lpszConnectionString)
{
	...
	// this call is required because CXTPCalendarEventLabels::InitDefaultValues() is using default
	// values
...
void CXTPCalendarControl::AdviseToDataProvider()
{
	...
	if (m_pResourcesNf)
	{
		...
		{
			m_pSink->Advise(
				pDPConn, XTP_NC_CALENDAROPTIONSWASCHANGED,
				CreateNotfySinkClassDelegate(this, &CXTPCalendarControl::OnEvent_FromDataProvider));

			m_pSink->Advise(
				pDPConn, XTP_NC_CALENDAREVENTWASADDED,
				CreateNotfySinkClassDelegate(this, &CXTPCalendarControl::OnEvent_FromDataProvider));
			m_pSink->Advise(
				pDPConn, XTP_NC_CALENDAREVENTWASDELETED,
				CreateNotfySinkClassDelegate(this, &CXTPCalendarControl::OnEvent_FromDataProvider));
			m_pSink->Advise(
				pDPConn, XTP_NC_CALENDAREVENTWASCHANGED,
				CreateNotfySinkClassDelegate(this, &CXTPCalendarControl::OnEvent_FromDataProvider));
			m_pSink->Advise(
				pDPConn, XTP_NC_CALENDARPATTERNWASADDED,
				CreateNotfySinkClassDelegate(this, &CXTPCalendarControl::OnEvent_FromDataProvider));
			m_pSink->Advise(
				pDPConn, XTP_NC_CALENDARPATTERNWASDELETED,
				CreateNotfySinkClassDelegate(this, &CXTPCalendarControl::OnEvent_FromDataProvider));
			m_pSink->Advise(
				pDPConn, XTP_NC_CALENDARPATTERNWASCHANGED,
				CreateNotfySinkClassDelegate(this, &CXTPCalendarControl::OnEvent_FromDataProvider));

			m_pSink->Advise(
				pDPConn, XTP_NC_CALENDAR_DoRetrieveDayEvents,
				CreateNotfySinkClassDelegate(this, &CXTPCalendarControl::OnEvent_FromDataProvider));
			m_pSink->Advise(
				pDPConn, XTP_NC_CALENDAR_DoRemoveAllEvents,
				CreateNotfySinkClassDelegate(this, &CXTPCalendarControl::OnEvent_FromDataProvider));
			m_pSink->Advise(
				pDPConn, XTP_NC_CALENDAR_DoRead_Event,
				CreateNotfySinkClassDelegate(this, &CXTPCalendarControl::OnEvent_FromDataProvider));
			m_pSink->Advise(
				pDPConn, XTP_NC_CALENDAR_DoRead_RPattern,
				CreateNotfySinkClassDelegate(this, &CXTPCalendarControl::OnEvent_FromDataProvider));
			m_pSink->Advise(
				pDPConn, XTP_NC_CALENDAR_DoCreate_Event,
				CreateNotfySinkClassDelegate(this, &CXTPCalendarControl::OnEvent_FromDataProvider));
			m_pSink->Advise(
				pDPConn, XTP_NC_CALENDAR_DoUpdate_Event,
				CreateNotfySinkClassDelegate(this, &CXTPCalendarControl::OnEvent_FromDataProvider));
			m_pSink->Advise(
				pDPConn, XTP_NC_CALENDAR_DoDelete_Event,
				CreateNotfySinkClassDelegate(this, &CXTPCalendarControl::OnEvent_FromDataProvider));
			m_pSink->Advise(
				pDPConn, XTP_NC_CALENDAR_DoCreate_RPattern,
				CreateNotfySinkClassDelegate(this, &CXTPCalendarControl::OnEvent_FromDataProvider));
			m_pSink->Advise(
				pDPConn, XTP_NC_CALENDAR_DoUpdate_RPattern,
				CreateNotfySinkClassDelegate(this, &CXTPCalendarControl::OnEvent_FromDataProvider));
			m_pSink->Advise(
				pDPConn, XTP_NC_CALENDAR_DoDelete_RPattern,
				CreateNotfySinkClassDelegate(this, &CXTPCalendarControl::OnEvent_FromDataProvider));
			m_pSink->Advise(
				pDPConn, XTP_NC_CALENDAR_DoGetAllEvents_raw,
				CreateNotfySinkClassDelegate(this, &CXTPCalendarControl::OnEvent_FromDataProvider));
			m_pSink->Advise(
				pDPConn, XTP_NC_CALENDAR_DoGetUpcomingEvents,
				CreateNotfySinkClassDelegate(this, &CXTPCalendarControl::OnEvent_FromDataProvider));
		}
...
void CXTPCalendarControl::_AdviseToReminders_StartMonitoring()
{
	if (m_pRemindersManager && m_pResourcesNf)
	{
		...
		if (pRMConn)
		{
			m_cnidOnReminders = m_pSink->Advise(
				pRMConn, XTP_NC_CALENDAR_ON_REMINDERS,
				CreateNotfySinkClassDelegate(this, &CXTPCalendarControl::OnEvent_Reminders));
			ASSERT(m_cnidOnReminders);

			VERIFY(m_pRemindersManager->StartMonitoring(m_pResourcesNf, m_spRemindersUpdatePeriod));
		}
	}
}
									  ...
									  BOOL bAdvanced)
{
	...
	if (!bAdvanced)
	{
		...
		CSize rcNeeded = m_pWndTip->CalcToolTipRect(TRUE); // change from default FALSE (Issue
														   // 20746)
...
void CXTPCalendarControl::OnCalendarTimeline()
{
	...
	if (m_bTimelineMode)
	{
		XTPCalendarViewType viewtype = GetActiveView() ? GetActiveView()->GetViewType()
													   : xtpCalendarDayView;
		...
		if (m_bHideCaptionBar)
		{
			...
			if (pParWnd != NULL && pParWnd->IsKindOf(RUNTIME_CLASS(CXTPCalendarControlView)))
			{
				CXTPCalendarControlView* pCtlView = DYNAMIC_DOWNCAST(CXTPCalendarControlView,
																	 pParWnd);
				if (pCtlView)
...
void CXTPCalendarControl::OnContextMenu(CWnd* /*pWnd*/, CPoint pos)
{
	...
	if (hitInfo.nDay == -1 && hitInfo.nGroup == -1 && hitInfo.nEvent == -1
...
BOOL CXTPCalendarControl::DoDeleteSelectedEvents(CXTPCalendarViewEvent* pViewEvent)
{
	...
	{
		bHandled = m_pActiveView->OnBeforeEditOperationNotify(xtpCalendarEO_DeleteSelectedEvents,
															  ptrSelEventViews);
	}
	if (bHandled)
	{
...
BOOL CXTPCalendarControl::DoDeleteEvent(CXTPCalendarViewEvent* pViewEvent)
{
	CXTPCalendarView* pView = GetActiveView();
	if (!pView->IsKindOf(RUNTIME_CLASS(CXTPCalendarTimeLineView)))
		pView = XTP_SAFE_GET3(pViewEvent, GetViewGroup_(), GetViewDay_(), GetView_(),
							  m_pActiveView);
	...
	BOOL bHandled = FALSE;
	if (m_pActiveView)
		bHandled = m_pActiveView->OnBeforeEditOperationNotify(xtpCalendarEO_DeleteEvent,
															  pViewEvent);
	...
	{
		int iShId = XTPToInt(pViewEvent->GetEvent()->GetScheduleID());
			...
			if (pSch)
			{
				...
				if (!m_OwnerUser.IsEmpty())
				{
					if (m_OwnerUser == sName)
						iShID = XTPToInt(pSch->GetID());
					else if (sName == _T("Public"))
						iShIDPublic = XTPToInt(pSch->GetID());
...
void CXTPCalendarControl::AddCustomTimeIntervalForReminder(int nMin,
														   LPCTSTR szCustomMeaning /*= NULL*/,
...
void CXTPCalendarControl::OnEvent_FromDataProvider(XTP_NOTIFY_CODE Event, WPARAM wParam,
												   LPARAM lParam)
{
	ASSERT(
		Event == XTP_NC_CALENDAROPTIONSWASCHANGED || Event == XTP_NC_CALENDAREVENTWASADDED
		|| Event == XTP_NC_CALENDAREVENTWASDELETED || Event == XTP_NC_CALENDAREVENTWASCHANGED
		|| Event == XTP_NC_CALENDARPATTERNWASADDED || Event == XTP_NC_CALENDARPATTERNWASDELETED
		|| Event == XTP_NC_CALENDARPATTERNWASCHANGED ||

		Event == XTP_NC_CALENDAR_DoRetrieveDayEvents || Event == XTP_NC_CALENDAR_DoRemoveAllEvents
		|| Event == XTP_NC_CALENDAR_DoRead_Event || Event == XTP_NC_CALENDAR_DoRead_RPattern
		|| Event == XTP_NC_CALENDAR_DoCreate_Event || Event == XTP_NC_CALENDAR_DoUpdate_Event
		|| Event == XTP_NC_CALENDAR_DoDelete_Event || Event == XTP_NC_CALENDAR_DoCreate_RPattern
		|| Event == XTP_NC_CALENDAR_DoUpdate_RPattern || Event == XTP_NC_CALENDAR_DoDelete_RPattern
...
void CXTPCalendarControl::SendNotificationAlways(XTP_NOTIFY_CODE EventCode, WPARAM wParam,
												 LPARAM lParam)
{
...
XTPCalendarThemeVersion CXTPCalendarControl::GetThemeVersion() const
{
	...
	switch (theme)
	{
		case xtpCalendarThemeOffice2000: themeVersion = xtpCalendarVersion2000; break;
		case xtpCalendarThemeOfficeXP: themeVersion = xtpCalendarVersionXP; break;
		case xtpCalendarThemeOffice2003: themeVersion = xtpCalendarVersion2003; break;
		case xtpCalendarThemeOffice2013: themeVersion = xtpCalendarVersion2013; break;
		case xtpCalendarThemeVisualStudio2015: themeVersion = xtpCalendarVersion2015; break;
		case xtpCalendarThemeVisualStudio2017: themeVersion = xtpCalendarVersion2017; break;
		case xtpCalendarThemeVisualStudio2019: themeVersion = xtpCalendarVersion2019; break;
		case xtpCalendarThemeVisualStudio2022: themeVersion = xtpCalendarVersion2022; break;
		case xtpCalendarThemeNativeWindows10: themeVersion = xtpCalendarVersion10; break;
		case xtpCalendarThemeResource:
		{
			if (m_pTheme)
			{
				CXTPCalendarThemeOffice2007* ptheme2007 =
					DYNAMIC_DOWNCAST(CXTPCalendarThemeOffice2007, m_pTheme);
				if (ptheme2007)
				{
		...
		default: themeVersion = xtpCalendarVersionUnknown; break;
	}

	return themeVersion;
}

void CXTPCalendarControl::SetPaintTheme(CXTPCalendarPaintManager* pPaintManager,
...
void CXTPCalendarControl::SetPaintTheme(XTPCalendarTheme ePaintTheme)
{
	switch (ePaintTheme)
	{
		case xtpCalendarThemeOfficeXP: SetPaintTheme(new CXTPCalendarOfficeXPTheme, NULL); break;
		...
		case xtpCalendarThemeVisualStudio2017:
			SetPaintTheme(new CXTPCalendarVisualStudio2017Theme, NULL);
			break;

		case xtpCalendarThemeVisualStudio2019:
			SetPaintTheme(new CXTPCalendarVisualStudio2019Theme, NULL);
			break;

		case xtpCalendarThemeVisualStudio2022:
			SetPaintTheme(new CXTPCalendarVisualStudio2022Theme, NULL);
			break;

		case xtpCalendarThemeNativeWindows10:
			SetPaintTheme(new CXTPCalendarNativeWindows10Theme, NULL);
			break;
		...
		default: SetPaintTheme(new CXTPCalendarOffice2000Theme, NULL); break;
	}
...
BOOL CXTPCalendarControl::SelectEventById(DWORD dEventID, BOOL bSelect)
{
	...
	if (pEvPtr)
	{
		...
		//          CXTPCalendarMonthView* pView = DYNAMIC_DOWNCAST(CXTPCalendarMonthView,
		//          GetActiveView()); pView->ShowDay(dtS); pView->SelectDay(dtS);
		//      }
		...
		//          CXTPCalendarWeekView* pView = DYNAMIC_DOWNCAST(CXTPCalendarWeekView,
		//          GetActiveView()); pView->ShowDay(dtS); pView->SelectDay(dtS);
		//      }
		//      else if (viewType == xtpCalendarTimeLineView)
		//      {
		//          SwitchActiveView(xtpCalendarTimeLineView);
		//          CXTPCalendarTimeLineView* pView = DYNAMIC_DOWNCAST(CXTPCalendarTimeLineView,
		//          GetActiveView()); pView->ShowDay(dtS); pView->SelectDay(dtS);
		//      }
		//      return TRUE;
		//  }
...
BOOL CXTPCalendarControl::SelectViewEventByIdAndGroup(int nScheduleID, DWORD dEventID, BOOL bSelect)
{
	...
	if (pEvPtr)
	{
		...
		{
			pDayView->ShowDay(dtS);
			CXTPCalendarViewEvent* pCVE = GetActiveView()->FindViewEventByGroup(pEvPtr, dtS,
																				nScheduleID);
			if (pCVE)
			{
				pDayView->EnsureVisible(pCVE);
...
CXTPCalendarControl::CViewChangedContext::CViewChangedContext(CXTPCalendarView* pView, int eType)
{
	if (!pView)
	{
		ASSERT(FALSE);
...
CXTPCalendarControl::CViewChangedContext::CViewChangedContext(CXTPCalendarControl* pControl,
															  int eType)
{
	Init(pControl, eType);
}
...
CXTPCalendarControl::CViewChangedContext::~CViewChangedContext()
{
	...
	if (m_pControl->m_cntViewChanged.m_nLockCount == 0 && m_pControl->m_cntViewChanged.m_bRequest)
	{
