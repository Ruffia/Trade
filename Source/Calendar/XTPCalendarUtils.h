...
//---------------------------------------------------------------------------
// Summary:
//     Defines a lower bound of the time frame for the Calendar control.
// See Also:
//     xtpCalendarDateTime_max
//---------------------------------------------------------------------------
static const COleDateTime xtpCalendarDateTime_min(100, 1, 1, 0, 0, 0);

//---------------------------------------------------------------------------
// Summary:
//     Defines an upper bound of the time frame for the Calendar control.
// See Also:
//     xtpCalendarDateTime_min
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
// Summary:
//     Interface for overriding the GetCurrentTime() function in calendar control.
//     Allows a custom date\time to be set for calendar
//---------------------------------------------------------------------------
class ITimeProvider
{
public:
...
public:
	...
	//     lpFormat - A pointer to a format picture string that is used to form the
	//                time string. The format picture string must be zero
	//                terminated. If lpFormat is NULL, the function uses the time
	//                format of the specified locale.
	// Remarks:
	...
	//                   for a resulting date. If nMonthDay is greater then the
	...
	//                   positive and negative numbers.
	//     nMonthDay   - An int that contains the day of the month to set
	//                   for the resulting date. If nMonthDay is greater
	...
	//                   If nMonthDay is greater then the maximum month day in
	...
	static COleDateTime AFX_CDECL RetriveEventEndDate(
		const COleDateTime& dtStart,
		const COleDateTime& dtEnd); // <combine
									// CXTPCalendarUtils::RetriveEventEndDate@CXTPCalendarEvent*>
	...
	//     XTPCalendarWeekDay.
	// See Also:
	//     GetDayOfWeekIndex, enum XTPCalendarWeekDay
	//-----------------------------------------------------------------------
	...
	//     1-Sunday, 2-Monday, ...
	// See Also:
	//     GetDayOfWeekMask, enum XTPCalendarWeekDay
	//-----------------------------------------------------------------------
	...
	//                      XTPCalendarWeekDay.
	// Returns:
	//     An int that contains the integer value of the day of the week
	...
	//-----------------------------------------------------------------------
	// Summary:
	//      This function is used to determine if the program is running on
	...
	//-----------------------------------------------------------------------
	// Summary:
	//      Converts a number of minutes into its corresponding COleDateTimeSpan
	//      object.
	// Parameters:
...
public:
	...
	//-----------------------------------------------------------------------
	// Summary:
	//      Overrides the local time provider with the custom provider. The date\time returned
	//      by the custom provider will be used as the current time for calendar control.
	...
	//      time provider is reset by a ResetTimeProvider() call. The caller should free provider
	//      instance memory after a ResetTimeProvider() call.
	//-----------------------------------------------------------------------
	static void AFX_CDECL OverrideTimeProvider(ITimeProvider* pTimeProvider);

	//-----------------------------------------------------------------------
	// Summary:
	//      Resets time provider to default which will use the current machine date time.
	//-----------------------------------------------------------------------
	...
	// Parameters:
	//      nIDResource - ID of the string in the resource.
	// Returns:
	...
	//-----------------------------------------------------------------------
	// Summary:
	//      Used to set which locale is used as the active locale: current user locale
	//      or resource file locale.
	// Parameters:
	...
	//-----------------------------------------------------------------------
	// Summary:
	//      This function is used to determine a font with maximum symbols height
	//      in an array of fonts.
	// Parameters:
	//      arFonts     - [in] A reference to an array of fonts.
	//      pDC         - [in] A pointer to a valid device context or NULL.
	//      pnMaxHeight - [out] A pointer to an int variable to store maximum font
	//                          height or NULL.
	// Remarks:
...
public:
	//-----------------------------------------------------------------------
	// Summary:
	//      Call this function to check if a character is a format character
	//      (Mm Dd Yy).
	// Parameters:
	//      t  - character.
	// Returns:
	//      TRUE if the input character is a format character, FALSE otherwise.
	//-----------------------------------------------------------------------
	...
	//-----------------------------------------------------------------------
	// Summary:
	//      This function removes a fixed number of characters from the format
...
public:
	//-----------------------------------------------------------------------
	// Summary:
	//      This function is used to round date time values by adding (or subtracting)
	//      a half-second double value.
	// Parameters:
	...
	//-----------------------------------------------------------------------
	// Summary:
	//      This function is used to reset the time part.
	// Parameters:
	//      dblDT - [in] A date time value in COLEDateTime double representation.
	// Returns:
	...
	//-----------------------------------------------------------------------
	// Summary:
	//      This function used is to reset the date part.
	// Parameters:
	//      dblDT - [in] A date time value in COLEDateTime double representation.
	// Returns:
...
public:
	...
	//      dwHoverTime - Specifies the hover time-out (if TME_HOVER was specified in dwFlags),
	//                    in milliseconds.
...
public:
	...
	//      Object constructor.
	// Parameters:
	//      rData - A reference to the managing variable.
	// Remarks:
	//      The original value of rData is stored in a separate member and it
	...
	// Parameters:
	//      rData    - A reference to the managing variable.
	//      valReset - A value to set when destroyed.
	// Remarks:
	//      The original value of rData is ignored.
	//      The valReset value will be used on destroy.
	//-----------------------------------------------------------------------
	...
	//      Object destructor.
	// Remarks:
	//      The managing variable value will be restored to its original value or to
	//      a value specified as the second constructor parameter.
	//-----------------------------------------------------------------------
	...
	//-----------------------------------------------------------------------
	// Summary:
	//     This member operator is used to access the managing variable.
	// Returns:
