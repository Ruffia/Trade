...
void CXTPCalendarDayViewDay::Populate(COleDateTime dtDayDate)
{
	...
	for (int i = 0; i < nCount; i++)
	{
		...
		{
			int iSchID = XTPToInt(pRC->GetSchedules()->GetAt(I));
			arScheduleIDs.Add(XTPToUInt(iSchID));
			arGroupIDs.Add(XTPToUInt(i));
		}
			...
			if (pGroup)
			{
				...
				for (int j = 0; j < nGroupViewEventsCount; j++)
				{
					...
					if (pVe)
					{
						...
						if (pE)
						{
							int iOwnId   = XTPToInt(pE->GetScheduleID());
									...
									if (iOwnId != jSch)
									{
										for (int I = 0; I < (int)arScheduleIDs.GetSize(); I++)
										{
											if (jSch == (int)arScheduleIDs.GetAt(I))
											{
												...
												if (jGr != i)
												{
													CXTPCalendarDayViewGroup* pGroup1 =
														GetViewGroup(jGr);
													if (pGroup1)
													{
...
CString CXTPCalendarDayViewDay::GetCaption() const
{
	...
	CString strHeaderFormat = GetView() ? GetView()->GetDayHeaderFormat() : CString();
...
void CXTPCalendarDayViewGroup::AdjustLayout2(CDC* pDC, const CRect& rcGroup)
{
	...
	GetViewDay()->GetView()->GetTheme()->GetDayViewPart()->GetDayPart()->GetGroupPart()->AdjustLayout(
		this, pDC, rcGroup);
}

CRect CXTPCalendarDayViewGroup::GetAllDayEventsScrollButtonRect(
	XTP_DAY_VIEW_GROUP_LAYOUT::XTPEnumHotItem nButton) const
{
...
void CXTPCalendarDayViewGroup::Draw(CDC* pDC)
{
	...
	if (nRowHeight <= 0 || !XTP_SAFE_GET2(GetViewDay(), GetView(), GetPaintManager(), NULL))
	{
		return;
	}
	...
	{
		CRect rcCell(m_Layout.m_rcGroup.left, m_LayoutX.m_rcDayDetails.top + nRowHeight * i,
					 m_Layout.m_rcGroup.right, m_LayoutX.m_rcDayDetails.top + nRowHeight * (i + 1));
		...
		CXTPCalendarPaintManager::CDayViewCellPart* pViewPart =
			(CXTPCalendarPaintManager::CDayViewCellPart*)GetCellViewPart(dtDateTime);
		ASSERT(pViewPart);
		if (pViewPart)
		{
			...
			XTP_SAFE_CALL2(GetViewDay_(), GetView_(),
						   SendNotification(XTP_NC_CALENDAR_BEFORE_DRAW_DAYVIEWCELL, wParam,
											lParam));
			//---------------------------------
			...
			ASSERT(dtDateTime == COleDateTime(cellParams.dtBeginTime));
...
void CXTPCalendarDayViewGroup::DrawEvents(CDC* pDC)
{
	...
	if (!m_LayoutX.m_rcAllDayEvents.IsRectEmpty())
	{
		if (GetViewDay() && GetViewDay()->GetView())
		{
			BOOL bSelected =
				!bHasSelectedEvents
				&& GetViewDay()->GetView()->SelectionContains(GetViewDay()->GetDayDate())
				&& GetViewDay()->GetView()->SelectionContains(GetViewDay()->GetDayDate()
																  + COleDateTimeSpan(0, 23, 59, 59),
															  nGroupIndex);
...
void CXTPCalendarDayViewGroup::DrawAllDayAreaScrollIcons(CDC* pDC)
{
	...
	CRect rcIconsUp = GetAllDayEventsScrollButtonRect(
		XTP_DAY_VIEW_GROUP_LAYOUT::xtpHotAllDayEventsScrollUp);
	CRect rcIconsDown = GetAllDayEventsScrollButtonRect(
	...
	XTP_SAFE_CALL4(GetViewDay(), GetView(), GetPaintManager(),
				   GetDayViewAllDayEventScrollIconsPart(),
				   OnDraw(pDC, this, IsShowAllDayEventsUpScrollButton(),
						  IsShowAllDayEventsDownScrollButton(), rcIconsUp, rcIconsDown,
						  bHighlightUp, bHighlightDown));
}

BOOL CXTPCalendarDayViewGroup::HitTestAllDayEventsScrollIcons(
	CPoint pt, XTP_CALENDAR_HITTESTINFO* pHitTest) const
{
	if (IsShowAllDayEventsUpScrollButton())
	{
		CRect rcIconsUp = GetAllDayEventsScrollButtonRect(
...
BOOL CXTPCalendarDayViewGroup::ScrollIcon_ScrollAllDayEventsUp()
{
	...
	if (UserAction_OnScrollAllDayEvents(XTP_DAY_VIEW_GROUP_LAYOUT::xtpHotAllDayEventsScrollUp)
		== FALSE)
	{
...
{
	BOOL bHandledByDefault = FALSE;
	if (UserAction_OnScrollAllDayEvents(XTP_DAY_VIEW_GROUP_LAYOUT::xtpHotAllDayEventsScrollDown)
		== FALSE)
	{
		XTP_DAY_VIEW_LAYOUT& rDayLayout = GetViewDay()->GetView()->GetLayout();
...
void CXTPCalendarDayViewGroup::GetCellEvents(int nCell, TViewEventsCollection* pViewEvents,
											 COleDateTime& rdtMinEventTime,
											 COleDateTime& rdtMaxEventTime)
{
	...
	for (int i = 0; i < nEventsCount; i++)
	{
		...
		if (!(dtE < dtCellBegin || dtS > dtCellEnd || CXTPCalendarUtils::IsEqual(dtE, dtCellBegin)
...
COleDateTime CXTPCalendarDayViewGroup::GetCellDateTime(int nCell) const
{
	...
	return CXTPCalendarUtils::UpdateTime(GetViewDay()->GetDayDate(),
										 GetViewDay()->GetView()->GetCellTime(nCell));
}
...
CRect CXTPCalendarDayViewGroup::GetCellRect(int nCell) const
{
	nCell -= GetTopRow();
	int nRowHeight = GetRowHeight();
	return CRect(m_Layout.m_rcGroup.left, m_LayoutX.m_rcDayDetails.top + nRowHeight * nCell,
				 m_Layout.m_rcGroup.right, m_LayoutX.m_rcDayDetails.top + nRowHeight * (nCell + 1));
}
...
CXTPCalendarViewPart* CXTPCalendarDayViewGroup::GetCellViewPart(const COleDateTime& dtDateTime) const
{
	...
	{
		return (CXTPCalendarViewPart*)XTP_SAFE_GET3(GetViewDay(), GetView(), GetPaintManager(),
													GetDayViewWorkCellPart(), NULL);
	}
	return (CXTPCalendarViewPart*)XTP_SAFE_GET3(GetViewDay(), GetView(), GetPaintManager(),
												GetDayViewNonworkCellPart(), NULL);
}
...
void CXTPCalendarDayViewGroup::FindMinMaxGroupDayEvents(CXTPCalendarDayViewEvent*& rpMin,
...
BOOL CXTPCalendarDayViewGroup::UserAction_OnScrollDay(XTPCalendarScrollDayButton eButton)
{
	...
	XTP_SAFE_CALL2(GetViewDay(), GetCalendarControl(),
				   SendNotification(XTP_NC_CALENDAR_USERACTION, (WPARAM)&uaParams,
									(LPARAM)&bCancel));

	return bCancel;
}

BOOL CXTPCalendarDayViewGroup::UserAction_OnScrollAllDayEvents(
	XTP_DAY_VIEW_GROUP_LAYOUT::XTPEnumHotItem eHotBtn)
{
	XTP_CALENDAR_USERACTION_PARAMS uaParams;
	::ZeroMemory(&uaParams, sizeof(uaParams));
...
CRect CXTPCalendarDayViewGroup::GetTooltipRect(const CPoint& ptHit,
