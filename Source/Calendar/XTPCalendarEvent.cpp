...
void CXTPCalendarEvent::SetStartTime(COleDateTime dtStart)
{
	...
	if (m_dtStartTime.GetStatus() == COleDateTime::valid
		...
		&& (m_dtStartTime > m_dtEndTime || CXTPCalendarUtils::IsEqual(m_dtEndTime, m_dtStartTime)))
	{
...
void CXTPCalendarEvent::SetEndTime(COleDateTime dtEnd)
{
	...
	if (m_dtStartTime.GetStatus() == COleDateTime::valid
		&& m_dtEndTime.GetStatus() == COleDateTime::valid
		&& (m_dtEndTime < m_dtStartTime || CXTPCalendarUtils::IsEqual(m_dtEndTime, m_dtStartTime)))
	{
...
CXTPCalendarRecurrencePatternPtr CXTPCalendarEvent::GetRecurrencePattern() const
{
	...
	CXTPCalendarRecurrencePatternPtr ptrPattern = m_pDataProvider->GetRecurrencePattern(
		m_dwRecurrencePatternID);
	return ptrPattern;
}
...
BOOL CXTPCalendarEvent::SetRecurrenceState_ExceptionOccurrence(
	XTPCalendarEventRecurrenceState nState, DWORD dwNewPatternID)
{
...
CXTPCalendarEventPtr CXTPCalendarEvent::CloneForOccurrence(COleDateTime dtStart, COleDateTime dtEnd,
														   DWORD dwNewEventID)
{
	...
	ptrNewEvent->SetRecurrenceState_ExceptionOccurrence(xtpCalendarRecurrenceOccurrence,
														m_dwRecurrencePatternID);
...
{
	if (m_nRecurrenceState != xtpCalendarRecurrenceMaster || !m_pDataProvider || !m_ptrRPattern
		|| !pRPattern)
	{
		ASSERT(FALSE);
		return FALSE;
...
void CXTPCalendarEvent::DoPropExchange(CXTPPropExchange* pPX)
{
	...
	if (pPX->IsStoring())
	{
		PX_Enum(pPX, cszEventProp_RecurrenceState, m_nRecurrenceState,
				xtpCalendarRecurrenceNotRecurring);
		PX_DWord(pPX, cszEventProp_RecurrencePatternID, m_dwRecurrencePatternID,
	...
	{
		//===========================================================================
		XTPCalendarEventRecurrenceState nRecurrenceState = m_nRecurrenceState =
			xtpCalendarRecurrenceNotRecurring;
		PX_Enum(pPX, cszEventProp_RecurrenceState, nRecurrenceState,
				xtpCalendarRecurrenceNotRecurring);

		DWORD dwRecurrencePatternID = XTP_CALENDAR_UNKNOWN_RECURRENCE_PATTERN_ID;
		PX_DWord(pPX, cszEventProp_RecurrencePatternID, dwRecurrencePatternID,
				 XTP_CALENDAR_UNKNOWN_RECURRENCE_PATTERN_ID);
				 ...
				 || nRecurrenceState == xtpCalendarRecurrenceOccurrence)
		{
			...
			{
				PX_Bool(pPX, cszEventProp_RExceptionDeleted, m_bRExceptionDeleted, FALSE);
				PX_DateTime(pPX, cszEventProp_RException_StartTimeOrig,
							m_dtRException_StartTimeOrig);
				PX_DateTime(pPX, cszEventProp_RException_EndTimeOrig, m_dtRException_EndTimeOrig);
			}
		}
	}
...
BEGIN_DISPATCH_MAP(CXTPCalendarEvent, CXTPCmdTarget)
	...
	DISP_PROPERTY_EX_ID(CXTPCalendarEvent, "StartTime", 8, OleGetStartTime, OleSetStartTime,
						VT_DATE)
	DISP_PROPERTY_EX_ID(CXTPCalendarEvent, "EndTime", 9, OleGetEndTime, OleSetEndTime, VT_DATE)

	DISP_PROPERTY_EX_ID(CXTPCalendarEvent, "StartTimeV", 36, OleGetStartTimeV, SetNotSupported,
						VT_DISPATCH)
	DISP_PROPERTY_EX_ID(CXTPCalendarEvent, "EndTimeV", 37, OleGetEndTimeV, SetNotSupported,
						VT_DISPATCH)

	DISP_PROPERTY_ID(CXTPCalendarEvent, "AllDayEvent", 14, m_bAllDayEvent, VT_BOOL)

	DISP_PROPERTY_EX_ID(CXTPCalendarEvent, "RecurrenceState", 10, OleGetRecurrence, SetNotSupported,
						VT_I4)

	DISP_PROPERTY_EX_ID(CXTPCalendarEvent, "Id", 11, OleGetEventID, SetNotSupported, VT_I4)
	DISP_PROPERTY_EX_ID(CXTPCalendarEvent, "CreationTime", 12, OleGetCreationTime,
						OleSetCreationTime, VT_DATE)
	DISP_PROPERTY_EX_ID(CXTPCalendarEvent, "LastModificationTime", 13, OleGetLastModificationTime,
						OleSetLastModificationTime, VT_DATE)

	DISP_PROPERTY_EX_ID(CXTPCalendarEvent, "RecurrencePattern", 15, OleGetRecurrencePattern,
						SetNotSupported, VT_DISPATCH)
	DISP_FUNCTION_ID(CXTPCalendarEvent, "CreateRecurrence", 16, OleCreateRecurrence, VT_DISPATCH,
					 VTS_NONE)
	DISP_FUNCTION_ID(CXTPCalendarEvent, "CreateRecurrenceEx", 116, OleCreateRecurrenceEx,
					 VT_DISPATCH, VTS_I4)
	DISP_FUNCTION_ID(CXTPCalendarEvent, "UpdateRecurrence", 17, OleUpdateRecurrence, VT_EMPTY,
					 VTS_DISPATCH)
	DISP_FUNCTION_ID(CXTPCalendarEvent, "RemoveRecurrence", 18, OleRemoveRecurrence, VT_EMPTY,
					 VTS_NONE)

	DISP_FUNCTION_ID(CXTPCalendarEvent, "MakeAsRException", 19, OleMakeAsRException, VT_EMPTY,
					 VTS_NONE)
	DISP_FUNCTION_ID(CXTPCalendarEvent, "MakeAsRExceptionEx", 119, OleMakeAsRExceptionEx, VT_EMPTY,
					 VTS_I4)
	DISP_FUNCTION_ID(CXTPCalendarEvent, "SetRExceptionDeleted", 20, OleSetRExceptionDeleted,
					 VT_EMPTY, VTS_BOOL)
	...
	DISP_PROPERTY_EX_ID(CXTPCalendarEvent, "CustomProperties", 23, OleGetCustomProperties,
						SetNotSupported, VT_DISPATCH)
	...
	DISP_PROPERTY_ID(CXTPCalendarEvent, "ReminderMinutesBeforeStart", 25,
					 m_nReminderMinutesBeforeStart, VT_I4)
	DISP_PROPERTY_ID(CXTPCalendarEvent, "ReminderSoundFile", 26, m_strReminderSoundFile, VT_BSTR)

	DISP_PROPERTY_ID(CXTPCalendarEvent, "RExceptionStartTimeOrig", 27, m_dtRException_StartTimeOrig,
					 VT_DATE)
	DISP_PROPERTY_ID(CXTPCalendarEvent, "RExceptionEndTimeOrig", 28, m_dtRException_EndTimeOrig,
					 VT_DATE)
	...
	DISP_FUNCTION_ID(CXTPCalendarEvent, "DataProvider", 32, OleGetDataProvider, VT_DISPATCH,
					 VTS_NONE)
...
{
	CXTPCalendarRecurrencePattern* pPattern = XTP_DYNAMIC_DOWNCAST_REMOTE_(
		CXTPCalendarRecurrencePattern, CXTPCalendarRecurrencePattern::FromIDispatchSafe(Pattern));
	if (!pPattern)
	{
...
{
	CXTPCalendarEvent* pSourceEvent = XTP_DYNAMIC_DOWNCAST_REMOTE_(
		CXTPCalendarEvent, CXTPCalendarEvent::FromIDispatchSafe(SourceEvent));
	if (!pSourceEvent)
	{
		AfxThrowOleException(E_INVALIDARG);
	}
