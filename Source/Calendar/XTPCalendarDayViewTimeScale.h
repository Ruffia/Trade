...
struct XTP_LC_TIMEFORMAT
{
	...
	void SetCustomFormat(BOOL bAmPm, BOOL bLeadingZero, const CString& separator, const CString& am,
						 const CString& pm);
	void ResetToDefault();
...
//     is visible by default. The second time scale is the alternative
//     and could be enabled programmatically. Time scale interval could be
//     changed using the SetScaleInterval member function.
//
// See Also:
...
public:
	...
	// ---------------------------------------------------------------------
	CXTPCalendarDayViewTimeScale(CXTPCalendarDayView* pDayView, BOOL bVisible = TRUE,
								 BOOL bDrawNowLine = TRUE);
	...
	//                AM/PM 12-hour time format. If this parameter is FALSE, the
	//                minutes value 0 is replaced only if the hour value is 12.
	...
	//-----------------------------------------------------------------------
	virtual CString Format(COleDateTime dtTime, BOOL bTopCell, CString* pHour = NULL,
						   CString* pMin = NULL);

	//-----------------------------------------------------------------------
	// Summary:
	//     This member function is used to determine if the Current Time Mark
	...
	//     hours for the time scale.
	// Remarks:
	//     Use this member function to retrieve the current time shift for
	//     the time scale.
	// Returns:
	...
	//-----------------------------------------------------------------------
	// Summary:
	//     This member function is used to set the time scale visibility
	...
	// Returns:
	//     TRUE if the test is succesful, FALSE otherwise.
	// Remarks:
	...
	// Remarks:
	//     Use this member function to get the day view object which owns the time
	//     scale.
	// Returns:
	...
	//-----------------------------------------------------------------------
	// Summary:
	//     This member function shifts the time scale object.
	// Parameters:
	//     nXPos - Offset from base position.
	// Remarks:
	//     Used by horizontal scrolling to place the timescale always visible
	//     on the left border.
	//-----------------------------------------------------------------------
	...
	//-----------------------------------------------------------------------
	// Summary:
	//    Sets the custom time format for the day view timescale.
	// Parameters:
	...
	// Remarks:
	//    This function overrides the locale time format for calendar control. To reset the time
	//    format back to locale, call ResetCustomTimeFormat()
	//
	//-----------------------------------------------------------------------
	void SetCustomTimeFormat(BOOL bAmPm, BOOL bLeadingZero = TRUE,
							 const CString& separator = _T(":"), const CString& am = _T("am"),
							 const CString& pm = _T("pm"));

	//-----------------------------------------------------------------------
	// Summary:
	//     Resets the timescale time format to the current locale format.
	//-----------------------------------------------------------------------
...
//
// This dialog allows for the customization of the Calendar Time Scale part as
// well as the setting of a proper time zone and its description.
//
// This dialog is a simplified version of a similar Outlook dialog.
//
/////////////////////////////////////////////////////////////////////////////
...
public:
	...
	enum
	{
		IDD = XTP_IDD_CALENDAR_TIMESCALE_PROPERTIES
	};
	//}}AFX_CODEJOCK_PRIVATE
...
public:
	...
	BOOL m_bShowAdditionalScale; // [in, out] Store is additional timescale used or not.
...
protected:
	CEdit m_edtCaption1;			  // An edit control for the main time zone caption.
	CComboBox m_cmbZones1;			  // A combobox to display the main time zone.
	CButton m_btnAutoAdjustDayLight1; // A check box control to show the 'Auto Adjust Day Light'
									  // option for the main timezone.

	CEdit m_edtCaption2;			  // An edit control for an additional time zone caption.
	CComboBox m_cmbZones2;			  // A combobox to edit an additional time zone.
	CButton m_btnAutoAdjustDayLight2; // A check box control to edit the 'Auto Adjust Day Light'
									  // option for an additional timezone.

	BOOL m_bAutoAdjustDayLight2; // A BOOL member which is used to edit the 'Auto Adjust Day Light'
								 // option for an additional timezone.
