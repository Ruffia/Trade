...
typedef void(CALLBACK* PFNITEMMETRICS)(CXTPDatePickerControl* pDatePicker,
									   ...
									   XTP_DAYITEM_METRICS* pDayItemMetrics, void* pUserData);
...
public:
	...
	//-----------------------------------------------------------------------
	// Summary:
	//     Destroys a CXTPDatePickerButton object, handles cleanup and deallocation.
	//-----------------------------------------------------------------------
...
//     CArray
// ----------------------------------------------------------------------
class _XTP_EXT_CLASS CXTPDatePickerButtons
	: public CArray<CXTPDatePickerButton*, CXTPDatePickerButton*>
{
public:
	// ----------------------------------------------------------------------
	// Summary:
...
public:
	...
	enum DatePickerMouseMode
	{
		...
		mouseSelecting, // User pressed left mouse down on unselected day item and is now selecting
						// a range of day items.
		mouseDeselecting,		// User pressed left mouse down on selected day item and is now
								// de-selecting a range of day items.
...
public:
	...
	//     around the control.
	// Parameters:
	//     borderStyle - New border style value determined by XTPDatePickerBorderStyle
	//                   enumeration.
	// See Also:
	...
	//
	//     HitTest(PMCHITTESTINFO pMCHitTest) returns a DWORD value. Equal to the uHit member of the
	...
	virtual DWORD HitTest(
		PMCHITTESTINFO pMCHitTest) const; // <combine CXTPDatePickerControl::HitTest@CPoint@const>
	...
	// </code>
	// Returns:
	//     An LRESULT that is used to indicate the success of the function. Nonzero
	...
	// Parameters:
	//     dtDate - Reference to the COleDateTime item whose visibility
	//              must be ensured.
	// Remarks:
	...
	virtual BOOL GetMinReqRect(RECT* pRect, int nRows, int nCols)
		const; // <combine CXTPDatePickerControl::GetMinReqRect@RECT*@const>
	...
	// Returns:
	//     A BOOL that indicates the success of the function.
	...
	//     Nonzero if successful. Otherwise 0.
	// See Also:
	//     GetCurSel, CMonthCalCtrl::GetCurSel
	//-----------------------------------------------------------------------
	...
	// See Also:
	//     GetHighlightToday, SetHighlightToday, SetToday, GetToday,
	...
	//     Nonzero if successful. Otherwise 0.
	// See Also:
	//     GetRange, CMonthCalCtrl::SetRange
	//-----------------------------------------------------------------------
	...
	//                   following:
	//
	//                      * <b>GMR_DAYSTATE</b>: Include preceding and trailing
	//                                             months of visible range that
	//                                             are only partially displayed.
	//                      * <b>GMR_VISIBLE</b>: Include only those months that
...
protected:
	...
	//     listeners.
	// See Also:
	//     XTP_NOTIFICATIONCODE, GetConnection
	//-----------------------------------------------------------------------
...
protected:
	...
	//     This member function redraws the control windows.
	// Parameters:
	//     bUpdateNow - If TRUE then CWnd::UpdateWindow() will be called,
	...
	// Returns:
	//     A BOOL value that specifies if the function is successful.
...
protected:
	...
	CXTPNotifyConnection* m_pConnection; // This member variable is a connection object that is used
	...
	CArray<CXTPDatePickerItemMonth*, CXTPDatePickerItemMonth*>
		m_arrMonths; // This member variable is an internal storage for the month items.

	CXTPDatePickerDaysCollection* m_pSelectedDays; // This member variable is a collection of
												   // selected days.
	...
	BOOL m_bShowNonMonthDays; // This member variable is a flag that determines if the "non-month
							  // days" are displayed.
	XTPDatePickerBorderStyle m_borderStyle; // This member variable is a flag that determines if the
											// control displays a 3D border.
	...
	COleDateTime m_dtFirstClicked; // This member variable is used to specify the day that was
								   // clicked before starting the mouse selection.
	COleDateTime m_dtLastClicked;  // This member variable is used to specify the day that was
								   // clicked last in the mouse selection.
	BOOL m_bSelectWeek;			   // TRUE when user is started selecting the whole week.

	COleDateTime m_dtFocused;  // This member variable is used to specify the focused day. Used only
							   // for keyboard navigation and selecting.
	COleDateTime m_dtFSelBase; // This member variable is used to specify start day of the
							   // continuous selection. Used only for keyboard navigation and
							   // selecting.

	CXTPDatePickerList* m_pListControl; // This member variable is used to specify the "header month
										// list" pop-up control.

	CString* m_arMonthNames;	 // This member variable contains the localized month names values.
	CString* m_arDayOfWeekNames; // This member variable contains the day abbreviation for drawing.
								 // in the month header.
...
{
	return nMonth >= 1 && nMonth <= 12 ? m_arMonthNames[nMonth - 1] : CString();
}
...
{
	return nDayOfWeek >= 1 && nDayOfWeek <= 7 ? m_arDayOfWeekNames[nDayOfWeek - 1] : CString();
}
...
AFX_INLINE BOOL CXTPDatePickerControl::Create(LPCTSTR lpszClassName, LPCTSTR lpszWindowName,
