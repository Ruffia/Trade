...
#include "Calendar/DatePicker/Themes/XTPDatePickerVisualStudio2017Theme.h"
#include "Calendar/DatePicker/Themes/XTPDatePickerVisualStudio2019Theme.h"
#include "Calendar/DatePicker/Themes/XTPDatePickerVisualStudio2022Theme.h"
#include "Calendar/DatePicker/Themes/XTPDatePickerResourceTheme.h"
#include "Calendar/DatePicker/Themes/XTPDatePickerNativeWindows10Theme.h"
...
#pragma warning(disable : 4571) // warning C4571: catch(...) blocks compiled with /EHs do not catch
								// or re-throw Structured Exceptions
...
BEGIN_DISPATCH_MAP(XTP_DAYITEM_METRICS, CXTPCmdTarget)
	...
	DISP_PROPERTY_EX_ID(XTP_DAYITEM_METRICS, "Picture", DISPID_PICTURE, OleGetPicture,
						OleSetPicture, VT_PICTURE)
END_DISPATCH_MAP()
...
CString CXTPDatePickerButton::GetCaption() const
{
	if (m_strCaption.IsEmpty())
		return CXTPCalendarUtils::LoadString(XTPToUInt(m_nID));
	...
	if (XTPResourceImages() && XTPResourceImages()->GetConnection())
	{
		m_pSink->Advise(XTPResourceImages()->GetConnection(), XTP_NC_COMMON_RESOURCEIMAGES_CHANGED,
						CreateNotfySinkClassDelegate(
							this, &CXTPDatePickerControl::OnEventResourceImagesChanged));
	}
}
...
CXTPDatePickerControl::~CXTPDatePickerControl()
{
	...
	if (m_nTimerID != 0 && m_hWnd)
		KillTimer(XTPToUIntPtr(m_nTimerID));
...
void CXTPDatePickerControl::OnEventResourceImagesChanged(XTP_NOTIFY_CODE Event, WPARAM /*wParam*/,
														 LPARAM /*lParam*/)
{
...
BOOL CXTPDatePickerControl::GoModal(const RECT& rect, CWnd* pParentWnd)
{
	...
	if (!::IsWindow(m_hWnd)
		&& !CreateEx(WS_EX_TOOLWINDOW, XTP_DATEPICKERCTRL_CLASSNAME, NULL,
					 pParentWnd ? WS_CHILD : WS_POPUP, rcScreen, pParentWnd, 0))
		return FALSE;
	...
	SetWindowPos(&CWnd::wndTopMost, rcScreen.left, rcScreen.top, rcScreen.Width(),
				 rcScreen.Height(), SWP_SHOWWINDOW | SWP_NOACTIVATE | SWP_NOOWNERZORDER);
	...
	if (m_dtMaxRange.GetStatus() == COleDateTime::valid)
	{
		...
		if (m_dtMinRange.GetStatus() == COleDateTime::valid)
		{
			int nTotalMaxRangeMonths = (m_dtMaxRange.GetYear() * 12 + m_dtMaxRange.GetMonth())
									   - (m_dtMinRange.GetYear() * 12 + m_dtMinRange.GetMonth())
									   + 1;
		...
		if (m_dtMinRange.GetStatus() == COleDateTime::valid && m_dtFirstMonth < m_dtMinRange)
		{
...
void CXTPDatePickerControl::AdjustLayout(CRect rcClient)
{
	...
	for (int nRow = 0; nRow < m_nRows; nRow++)
	{
		...
		{
			rcMonth = CRect(CPoint(m_rcGrid.left + nCol * szMonth.cx,
								   m_rcGrid.top + nRow * szMonth.cy),
							szMonth);
...
BOOL CXTPDatePickerControl::Create(DWORD dwStyle, const RECT& rect, CWnd* pParentWnd, UINT nID,
...
void CXTPDatePickerControl::CreateMonthArray()
{
	...
	for (int nRow = 0; nRow < m_nRows; nRow++)
	{
		for (int nCol = 0; nCol < m_nColumns; nCol++)
		{
			...
			CXTPDatePickerItemMonth* pMonth = new CXTPDatePickerItemMonth(this, dtNextMonth, nRow,
																		  nCol);
	...
	if (nMonthCount > 0)
	{
		...
		if (m_dtMinRange.GetStatus() == COleDateTime::valid)
		{
			if (pFirstMonth->GetMonth() <= m_dtMinRange)
			{
				bScrollLeft = FALSE;
			}

			if (m_dtMinRange < pFirstMonth->m_arrDays.GetAt(0)->GetDate())
			{
				bScrollLeft = TRUE;
			}
		}
		...
		if (m_dtMaxRange.GetStatus() == COleDateTime::valid)
		{
			if ((DATE)pLastMonth->GetMonth() + 30.0 >= (DATE)m_dtMaxRange)
			{
				bScrollRight = FALSE;
			}

			if (m_dtMaxRange
				> pFirstMonth->m_arrDays.GetAt(pFirstMonth->GetDayCount() - 1)->GetDate())
			{
				bScrollRight = TRUE;
			}
		}
...
void CXTPDatePickerControl::DrawButtons(CDC* pDC)
{
	for (int i = 0; i < GetButtonCount(); i++)
	{
		CXTPDatePickerButton* pButton = GetButton(i);
		if (pButton->m_bVisible)
			m_pPaintManager->DrawButton(pDC, pButton->m_rcButton, pButton->GetCaption(),
...
void CXTPDatePickerControl::OnTimer(UINT_PTR nIDEvent)
{
	if (nIDEvent == XTP_DATEPICKER_TIMERID)
	{
		...
		if ((m_mouseMode == mouseTrackingHeaderList) && (m_pListControl != NULL))
		{
...
void CXTPDatePickerControl::ShowListHeader(CRect rcHeader, COleDateTime dtMonth)
{
	...
	m_nTimerID = XTPToInt(
		SetTimer(XTP_DATEPICKER_TIMERID, 2 * XTP_DATEPICKER_TIMER_INTERVAL / 3, NULL));
...
BOOL CXTPDatePickerControl::IsSelected(const COleDateTime& dtDay) const
{
	...
	if (m_mouseMode == mouseSelecting || m_mouseMode == mouseDeselecting)
	{
		BOOL bBetween = FALSE;
		if (m_dtFirstClicked <= m_dtLastClicked && m_dtFirstClicked <= dtDay
			&& m_dtLastClicked >= dtDay)
		{
			bBetween = TRUE;
		}
		if (m_dtFirstClicked >= m_dtLastClicked && m_dtFirstClicked >= dtDay
...
void CXTPDatePickerControl::ProcessButtons(CPoint point)
{
	for (int i = 0; i < GetButtonCount(); i++)
	{
		...
		else if (pButton->m_bPressed)
		{
			...
			if (bHighlight)
				OnButtonClick(XTPToUInt(pButton->m_nID));
			_RedrawControl(FALSE);
		}
...
void CXTPDatePickerControl::InitNames()
{
	...
	{
		CString strMonth =
			CXTPCalendarUtils::GetLocaleString(XTPToUInt(LOCALE_SMONTHNAME1 + nMonth), 255);
	...
	for (nDay = 0; nDay < 7; nDay++)
	{
		CString strDayName = CXTPCalendarUtils::GetLocaleString(
			XTPToUInt(CXTPCalendarUtils::GetDayOfWeekLocaleStringId(nDay + 1)), 255);
		...
		{
			nEqualLeftSymbols = min(nEqualLeftSymbols,
									GetEqualLeftSymbols(strDayName, m_arDayOfWeekNames[1]));
		}
	}
...
void CXTPDatePickerControl::SetShowNonMonthDays(BOOL bShow)
{
	if (bShow != m_bShowNonMonthDays)
	{
		...
		if (nMonthCount > 0)
		{
			CXTPDatePickerItemMonth* pFirstMonth = m_arrMonths.GetAt(0);
			CXTPDatePickerItemMonth* pLastMonth  = m_arrMonths.GetAt(nMonthCount - 1);
			// by default set show previous days for first month and show following days for last
			// month
...
BOOL CXTPDatePickerControl::SetSelRange(const COleDateTime& dtMinRange,
										const COleDateTime& dtMaxRange)
{
	ClearFocus();
...
void CXTPDatePickerControl::SetDesiredVisibleAndFullRange(COleDateTime FirstVisibleDay,
														  COleDateTime LastVisibleDay,
														  COleDateTime FirstDay,
...
int CXTPDatePickerControl::GetMonthRange(COleDateTime& refMinRange, COleDateTime& refMaxRange,
										 DWORD dwFlags) const
{
	...
	if (GMR_VISIBLE == dwFlags)
	{
		...
		if (nMonthCount > 0)
		{
			...
			if (pLastMonth)
			{
				CXTPDatePickerItemDay* pLastDay = pLastMonth->m_arrDays.GetAt(
					XTP_MAX_WEEKS * XTP_WEEK_DAYS - 1);
	...
	if (pHitMonth)
	{
		...
		else
		{
			...
			if (pHitMonth->m_rcDaysOfWeek.PtInRect(ptHit))
			{
				...
				// The SYSTEMTIME structure at lpMCHitTest>st is set to the corresponding date in
				// the top row.
				...
				if (pHitMonth->m_rcHeader.PtInRect(ptHit))
			{
				...
				// MCHT_TITLEBTNNEXT
				if (pHitMonth->GetShowRightScroll() && pHitMonth->m_rcRightScroll.PtInRect(ptHit))
			{
				pMCHitTest->uHit = MCHT_TITLEBTNNEXT;
			}
			else
				// MCHT_TITLEBTNPREV
				if (pHitMonth->GetShowLeftScroll() && pHitMonth->m_rcLeftScroll.PtInRect(ptHit))
			{
...
void CXTPDatePickerControl::SetPaintTheme(XTPCalendarTheme ePaintTheme)
{
	switch (ePaintTheme)
	{
		case xtpCalendarThemeOfficeXP: SetTheme(new CXTPDatePickerThemeOfficeXP()); break;

		case xtpCalendarThemeOffice2003: SetTheme(new CXTPDatePickerThemeOffice2003()); break;

		case xtpCalendarThemeOffice2013: SetTheme(new CXTPDatePickerThemeOffice2013()); break;
		...
		case xtpCalendarThemeVisualStudio2017:
			SetTheme(new CXTPDatePickerThemeVisualStudio2017());
			break;

		case xtpCalendarThemeVisualStudio2019:
			SetTheme(new CXTPDatePickerThemeVisualStudio2019());
			break;

		case xtpCalendarThemeVisualStudio2022:
			SetTheme(new CXTPDatePickerThemeVisualStudio2022());
			break;

		case xtpCalendarThemeNativeWindows10:
			SetTheme(new CXTPDatePickerThemeNativeWindows10());
			break;

		case xtpCalendarThemeResource: SetTheme(new CXTPDatePickerThemeResource()); break;
		...
		default: SetTheme(new CXTPDatePickerThemeOffice2000()); break;
	}
}
...
void CXTPDatePickerControl::SendNotification(XTP_NOTIFY_CODE EventCode, WPARAM wParam,
											 LPARAM lParam)
{
