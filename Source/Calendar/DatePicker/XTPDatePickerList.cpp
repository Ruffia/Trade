...
void CXTPDatePickerList::OnPaint()
{
	...
	for (int nItem = 0; nItem < m_nItemsAbove + 1 + m_nItemsBelow; nItem++)
	{
		...
		{
			strDisplayText = CXTPCalendarUtils::GetDateFormat(COleDateTime(nYear, nMonth, 1, 0, 0,
																		   0),
															  m_pControl->m_strYearMonthFormat);
		}
		else
		{
...
BOOL CXTPDatePickerList::IsSelected(int nX, CRect rcItem) const
{
	...
	if (ptPos.x < rcClient.left || ptPos.x > rcClient.right || ptPos.y < rcClient.top
		|| ptPos.y > rcClient.bottom)
		return FALSE;
...
CSize CXTPDatePickerList::GetListSize() const
{
	...
	// calculate rectangle
	CSize szCalendar(szMaxItem.cx + XTP_DPI_X(20),
					 szMaxItem.cy * (m_nItemsAbove + 1 + m_nItemsBelow) + XTP_DPI_Y(2));
...
BOOL CXTPDatePickerList::Create(CRect rcList)
{
	...
	if (!CreateEx(0, AfxRegisterWndClass(0, AfxGetApp()->LoadStandardCursor(IDC_ARROW)), NULL,
				  dwStyle, rcList, m_pControl, 0, 0))
		return FALSE;
	...
	SetWindowPos(NULL, m_rcListControl.left, m_rcListControl.top, m_rcListControl.Width(),
				 m_rcListControl.Height(), SWP_NOZORDER | SWP_NOACTIVATE | SWP_SHOWWINDOW);
	ScrollDown(0);
	RedrawWindow();
