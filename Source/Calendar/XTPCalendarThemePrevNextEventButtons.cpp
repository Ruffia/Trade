...
void CTOPrevNextEventButtons::RefreshMetrics(BOOL bRefreshChildren)
{
	...
	CXTPNotifyConnection* pConnection = XTP_SAFE_GET2(GetTheme(), GetCalendarControl(),
													  GetConnection(), NULL);
	if (pConnection)
	{
		m_pSink->Advise(pConnection, XTP_NC_CALENDAREVENTWASADDED,
						CreateNotfySinkClassDelegate(this,
													 &CTOPrevNextEventButtons::OnEvent_Calendar));
...
void CTOPrevNextEventButtons::AdjustLayout(CXTPCalendarView* /*pObject*/, CDC* pDC,
										   const CRect& rcRect)
{
...
void CTOPrevNextEventButtons::OnStatusChanged()
{
	...
	CXTPCalendarControl::CUpdateContext updateContext(GetTheme()->GetCalendarControl(),
...
BOOL CTOPrevNextEventButtons::IsPrevNextButtonsVisible()
{
	...
	BOOL bEnabled = XTP_SAFE_GET3(GetTheme(), GetCalendarControl(), GetCalendarOptions(),
...
{
	if ((BOOL)m_bUseActiveViewResources)
		return XTP_SAFE_GET3(GetTheme(), GetCalendarControl(), GetActiveView(), GetResources(),
							 NULL);
	else
...
void CTOPrevNextEventButtons::CDaysMap::_GrowArrayIfNeed(long nDay)
{
	...
	if ((nDay_idx <= 10 || nDay_idx >= nSize - 10) && nNewSize < cnMaxArraySize)
	{
