...
#define MAPI_FREEBUFFER(pProvider, pBuffer)                                                        \
	if (pProvider && pBuffer)                                                                      \
	{                                                                                              \
		pProvider->MAPIFreeBuffer(pBuffer);                                                        \
		pBuffer = NULL;                                                                            \
	}
#define MAPI_RELEASE(pProvider, x)                                                                 \
	if (pProvider && x)                                                                            \
	{                                                                                              \
		pProvider->UlRelease(x);                                                                   \
		x = 0;                                                                                     \
	}
...
public:
	...
	protected:
		typedef int(AFX_CDECL* T_CompareFunc)(const CXTPRcException* pExc1,
											  const CXTPRcException* pExc2);
		void _Sort(T_CompareFunc);

		static int AFX_CDECL CompareAsc_ByOrigStart(const CXTPRcException* pExc1,
													const CXTPRcException* pExc2);
		static int AFX_CDECL CompareAsc_ByNewStart(const CXTPRcException* pExc1,
												   const CXTPRcException* pExc2);
	};
	...
	//-----------------------------------------------------------------------
	static BOOL FillRCData(CXTPRcData& rRCData, CXTPCalendarRecurrencePattern* pPattern,
...
//
CXTPCalendarMAPI_Recurrence::XTP_RCDATA_LAYOUT CXTPCalendarMAPI_Recurrence::s_arRCDataLayout[] = {
	//  daily
	{
		4,					   // type
		6,					   // type2
...
void CXTPCalendarMAPI_Recurrence::CXTPRcException::Set(CXTPCalendarEvent* pExc,
													   CXTPCalendarEvent* pMaster)
{
	...
	*this = rexZero;
	...
	dwOrigStart = CXTPCalendarMAPI_Recurrence::ConvertRecurrenceDateToMinutes(
...
const CXTPCalendarMAPI_Recurrence::CXTPRcException& CXTPCalendarMAPI_Recurrence::CXTPRcException::
	operator=(const CXTPCalendarMAPI_Recurrence::CXTPRcException& rSrc)
{
...
int AFX_CDECL CXTPCalendarMAPI_Recurrence::CXTPRcExceptions::CompareAsc_ByOrigStart(
	const CXTPRcException* pExc1, const CXTPRcException* pExc2)
{
...
int AFX_CDECL CXTPCalendarMAPI_Recurrence::CXTPRcExceptions::CompareAsc_ByNewStart(
	const CXTPRcException* pExc1, const CXTPRcException* pExc2)
{
...
BOOL CXTPCalendarMAPI_Recurrence::ReadRCData(CXTPCalendarMAPI_Recurrence::CXTPRcData& rRCData,
...
void CXTPCalendarMAPI_Recurrence::_ReadRCDataExceptions(
	CXTPCalendarMAPI_Recurrence::CXTPRcData& rRCData, const BYTE* pExcData, const BYTE* pMaxPtr,
...
void CXTPCalendarMAPI_Recurrence::SetRecurrenceOptions(
	CXTPCalendarRecurrencePattern* pPattern, const CXTPCalendarMAPI_Recurrence::CXTPRcData& RCData)
{
	ASSERT(pPattern);
	if (!pPattern)
	{
	...
	switch (RCData.dwEndByType)
	{
		...
		case xtpMAPIRcEnd_Number:
			...
			break;
		case xtpMAPIRcEnd_Never: rpEnd.m_nUseEnd = xtpCalendarPatternEndNoDate; break;
...
void CXTPCalendarMAPI_Recurrence::SetRecurrenceExceptions(CXTPCalendarEvent* pMasterEvent,
														  CXTPCalendarRecurrencePattern* pPattern,
														  const CXTPRcData& RCData)
{
											 ...
											 CXTPCalendarEvent* pMaster)
{
	...
	switch (opt.m_nRecurrenceType)
	{
		...
		default: ASSERT(!"Unexpected recurrence type"); break;
	}
	...
	switch (rpEnd.m_nUseEnd)
	{
		case xtpCalendarPatternEndDate:
			...
			pPattern->GetOccurrencesDates(arOccDates, pPattern->GetPatternStartDate(),
										  pPattern->GetPatternEndDate());
		...
		case xtpCalendarPatternEndAfterOccurrences:
			...
			break;
		case xtpCalendarPatternEndNoDate: rRCData.dwEndByType = xtpMAPIRcEnd_Never; break;
		default: // Use No end date by default
			rRCData.dwEndByType = xtpMAPIRcEnd_Never;
	}

	COleDateTime dtOccStartTime = pPattern->GetStartTime();
	rRCData.dwOccStartTime		= XTPToUIntChecked(dtOccStartTime.GetHour() * 60
											   + dtOccStartTime.GetMinute());
...
COleDateTime CXTPCalendarMAPI_Recurrence::ConvertRecurrenceMinutesToDate(DWORD dwMinutes)
{
	...
	FILETIME ft		= *((FILETIME*)&ullDT); // { (DWORD) ( fileTimeUnitsBeforeStart & 0xFFFFFFFF),
											// (DWORD) ( fileTimeUnitsBeforeStart >> 32)};
...
DWORD CXTPCalendarMAPI_Recurrence::ConvertRecurrenceDateToMinutes(COleDateTime dt)
{
	...
	ULONGLONG ullMinutes = *(
		(ULONGLONG*)&ft); //( ( (ULONGLONG) ft.dwHighDateTime) << 32) + ft.dwLowDateTime;
...
STDMETHODIMP CXTPCalendarMAPIDataProvider::OpenInbox(LPMDB lpMDB, LPMAPIFOLDER* lpInboxFolder,
													 ...
													 CXTPCalendarMAPIDataProvider* pProvider)
{
	...
	*lpCalendarFolder = NULL;
	...
	hRes = lpMDB->GetReceiveFolder(NULL,	  // Get default receive folder
								   NULL,	  // Flags
								   &cbInbox,  // Size and ...
		...
		if (SUCCEEDED(hRes))
		{
			...
			*lpInboxFolder = lpTempFolder;
			...
			if (hRes == S_OK || hRes == MAPI_W_ERRORS_RETURNED)
			{
				...
				{
					hRes = lpMDB->OpenEntry(props[0].Value.bin.cb,			  // Size and...
											(ENTRYID*)props[0].Value.bin.lpb, // Value of the
																			  // Calendar's EntryID
											NULL, // We want the default interface (IMAPIFolder)
											MAPI_BEST_ACCESS,			// Flags
											&ulObjType,					// Object returned type
	...
	LPMAPISESSION lpMAPISession, LPMDB* lpMDB, CXTPCalendarMAPIDataProvider* pProvider)
{
	...
	*lpMDB = NULL;
	...
	if (SUCCEEDED(hRes))
	{
		...
		// Only one message store should have PR_DEFAULT_STORE set to true, so only one will be
		// returned
		hRes = pProvider->HrQueryAllRows(pStoresTbl,				// Table to query
...
/////////////////////////////////////////////////////////////////////////////
CXTPCalendarMAPIDataProvider::CXTPCalendarMemDPInternal::CXTPCalendarMemDPInternal(
	CXTPCalendarData* pDPExternal)
{
	ASSERT(pDPExternal);
	m_pDPExternal = pDPExternal;
}

void CXTPCalendarMAPIDataProvider::CXTPCalendarMemDPInternal::SendNotification(
	XTP_NOTIFY_CODE EventCode, WPARAM wParam, LPARAM lParam)
{
...
{
	if (m_pMemDP)
		return m_pMemDP->RetrieveDayEvents(dtDay);
...
{
	if (m_pMemDP)
		return m_pMemDP->GetEvent(dwEventID);
...
{
	if (m_pMemDP)
		return m_pMemDP->GetRecurrencePattern(dwPatternID);
...
BOOL CXTPCalendarMAPIDataProvider::DoCreate_Event(CXTPCalendarEvent* pEvent, DWORD& rdwNewEventID)
{
	...
	if (nRState != xtpCalendarRecurrenceNotRecurring && nRState != xtpCalendarRecurrenceMaster)
	{
	...
	rdwNewEventID = GetEventID(XTPToUInt(arSearchKey.GetSize()), (LPENTRYID)arSearchKey.GetData(),
							   TRUE);
	//-----------------------------------------------------------------------
...
BOOL CXTPCalendarMAPIDataProvider::DoUpdate_Event(CXTPCalendarEvent* pEvent)
{
	...
	HRESULT hr = m_lpCalendarFolder->OpenEntry(eid.GetBinarySize(), (LPENTRYID)eid.GetBinaryData(),
											   NULL, // default interface
											   MAPI_BEST_ACCESS, &ulObjType, (LPUNKNOWN*)&pMessage);
...
BOOL CXTPCalendarMAPIDataProvider::DoCreate_RPattern(CXTPCalendarRecurrencePattern* pPattern,
...
CXTPCalendarEventsPtr CXTPCalendarMAPIDataProvider::DoGetUpcomingEvents(COleDateTime dtFrom,
																		COleDateTimeSpan spPeriod)
{
	UNREFERENCED_PARAMETER(dtFrom);
	UNREFERENCED_PARAMETER(spPeriod);
...
{
	if (m_pMemDP)
		return m_pMemDP->GetAllEvents_raw();

	return NULL;
}

BOOL AFX_CDECL CXTPCalendarMAPIDataProvider::GetParameterValueFromConStr(
	LPCTSTR lpszConnectionString, LPCTSTR pcszParameterName, CString& rstrValue)
{
	rstrValue.Empty();

	ASSERT(lpszConnectionString && pcszParameterName);

	if (!lpszConnectionString || !pcszParameterName || 0 == _tcslen(lpszConnectionString)
		|| 0 == _tcslen(pcszParameterName))
	{
		return FALSE;
	}
...
BOOL CXTPCalendarMAPIDataProvider::Open()
{
	...
	if (SUCCEEDED(hRes))
	{
		...
		if (SUCCEEDED(hRes) && m_lpAdviseSink_ThrSafe)
		{
			...
			hRes = m_lpMDB->Advise(0, NULL, uEventsMask, m_lpAdviseSink_ThrSafe, &m_ulMAPIConID0);
			ASSERT(SUCCEEDED(hRes));

			// to receive fnevObjectDeleted notification
			hRes = m_lpMDB->Advise(uEIDsize, pEIDdata, uEventsMask, m_lpAdviseSink_ThrSafe,
								   &m_ulMAPIConID1);
			ASSERT(SUCCEEDED(hRes));
...
LPSRestriction AFX_CDECL CXTPCalendarMAPIDataProvider::BuildBinaryRestriction(ULONG cbSize,
																			  LPBYTE lpData,
																			  ULONG ulPropTag)
{
	static SRestriction sres;
...
CXTPCalendarEventPtr CXTPCalendarMAPIDataProvider::ImportEvent(ULONG cbEntryID, LPENTRYID lpEntryID)
{
...
CXTPCalendarEventPtr CXTPCalendarMAPIDataProvider::ImportEvent(LPMESSAGE pMessage)
{
	...
	DWORD dwEventID = GetEventID(XTPToUInt(arSearchKey.GetSize()), (LPENTRYID)arSearchKey.GetData(),
								 TRUE);
...
CString CXTPCalendarMAPIDataProvider::GetEventGlobalPropVal_str(LPCTSTR pcszPropName,
																LPCTSTR pcszDefault)
{
	CXTPCalendarCustomProperties* pProps = GetCustomProperties();
...
BOOL CXTPCalendarMAPIDataProvider::UpdateMAPIEvent(LPMESSAGE pMessage, CXTPCalendarEvent* pEvent)
{
	...
	COleDateTime dtTmp = (nIsRecurrence == 0)
							 ? pEvent->GetStartTime()
							 : COleDateTime(pEvent->GetStartTime()
											+ pEvent->GetRecurrencePattern()->GetStartTime());
	...
	dtTmp = (nIsRecurrence == 0)
				...
				: COleDateTime(
					  ...
					  + COleDateTimeSpan(0, 0, pEvent->GetRecurrencePattern()->GetDurationMinutes(),
										 0));
	...
	{
		VERIFY(_setPropTagVal_UtcTime(pMessage, nPTagRmdDate,
									  stUTCstart)); // always equal to start time
	}
	else
	{
	...
	VERIFY(_setPropTagVal_int(pMessage, nPropTag, nTmp));
	VERIFY(_setPropTagVal_int(pMessage, PR_SENSITIVITY,
							  XTPToInt(nTmp ? SENSITIVITY_PRIVATE : SENSITIVITY_NONE)));
	...
	else
	{
		...
		arPropsTags.Add(
			(ULONG)m_MapiHelper.GetPropTagByID(pMessage, xtpMAPIpropEvent_RecurrenceState));
		arPropsTags.Add(
			(ULONG)m_MapiHelper.GetPropTagByID(pMessage, xtpMAPIpropEvent_RecurrenceStart));
		arPropsTags.Add(
			(ULONG)m_MapiHelper.GetPropTagByID(pMessage, xtpMAPIpropEvent_RecurrenceEnd));
...
UINT AFX_CDECL CXTPCalendarMAPIDataProvider::_getStreamSize(IStream* pStream)
{
	...
	ZeroMemory(&uliSize, sizeof(uliSize));
...
int AFX_CDECL CXTPCalendarMAPIDataProvider::_getSimpleMAPITypeSize(int nType)
{
	switch (nType)
	{
		...
		case PT_CURRENCY:
			return 8; /* Signed 64-bit int (decimal w/    4 digits right of decimal pt) */
		...
		case PT_SYSTIME:
			return sizeof(
				FILETIME); /* FILETIME 64-bit int w/ number of 100ns periods since Jan 1,1601 */
		...
		case PT_BINARY: /* Uninterpreted (counted byte array) */ return 0;
		default: ASSERT(FALSE);
	}
	return 0;
}

BOOL CXTPCalendarMAPIDataProvider::_setPropTagVal_int(LPMESSAGE pMessage, ULONG ulPropTag,
													  int nValue)
{
	...
	switch (nType)
	{
		case PT_I2: PropVal.Value.i = (short int)nValue; break;
		case PT_LONG: /* Signed 32-bit value */ PropVal.Value.l = (long)nValue; break;
		case PT_ERROR: /* 32-bit error value */ PropVal.Value.err = (SCODE)nValue; break;
		case PT_BOOLEAN: /* 16-bit boolean (non-zero true) */
			...
			//  case PT_R4:     return 4;       /* 4-byte floating point */
			//  case PT_DOUBLE:     return 8;   /* Floating point double */
			//  case PT_CURRENCY:   return 8;   /* Signed 64-bit int (decimal w/    4 digits right
			//  of decimal pt) */ case PT_APPTIME:    return sizeof(double);  /* Application time */

			//  case PT_I8:         return 8;   /* 8-byte signed integer */
			//  case PT_SYSTIME:    return sizeof(FILETIME);    /* FILETIME 64-bit int w/ number of
			//  100ns periods since Jan 1,1601 */ case PT_CLSID:      return sizeof(GUID);        /*
			//  OLE GUID */
...
BOOL CXTPCalendarMAPIDataProvider::_setPropTagVal_str(LPMESSAGE pMessage, ULONG ulPropTag,
													  LPCTSTR pcszValue)
{
	int nType = XTPToInt(PROP_TYPE(ulPropTag));
...
BOOL CXTPCalendarMAPIDataProvider::_setPropTagVal_UtcTime(LPMESSAGE pMessage, ULONG ulPropTag,
...
BOOL CXTPCalendarMAPIDataProvider::_setPropTagVal_bin(LPMESSAGE pMessage, ULONG ulPropTag,
													  const CByteArray& arData)
{
	int nType = XTPToInt(PROP_TYPE(ulPropTag));
...
HRESULT CXTPCalendarMAPIDataProvider::_getPropVal(LPMESSAGE pMessage, ULONG ulPropTag,
												  CByteArray& rData)
{
	rData.RemoveAll();
	...
	{
		hRes = pMessage->OpenProperty(ulPropTag, &IID_IStream, STGM_READ, NULL,
									  (LPUNKNOWN*)&pStream);
...
int CXTPCalendarMAPIDataProvider::_getPropVal_int(LPMESSAGE pMessage, ULONG ulPropTag)
{
	...
	int nType = XTPToInt(PROP_TYPE(ulPropTag));
	ASSERT(nType == PT_I2 || nType == PT_LONG || nType == PT_ERROR || nType == PT_BOOLEAN
		   || nType == PT_I8);
	UNUSED(nType);
	...
	if (SUCCEEDED(hRes))
	{
		...
		if (SUCCEEDED(hRes))
		{
			...
			while (pos != NULL)
			{
				...
				// ASSERT(ptrEventToDel); // could be NULL if processing a few notifications in a
				// row
				if (ptrEventToDel)
				{
...
LONG STDAPICALLTYPE CXTPCalendarMAPIDataProvider::MAPICallBack_OnNotify(LPVOID lpvContext,
																		ULONG cNotif,
																		LPNOTIFICATION lpNotif)
{
	...
	for (int i = 0; i < (int)cNotif; i++)
	{
		...
		if (!pThis->Equal(eidObj, pThis->m_eidCalendarFolder))
		{
			...
			{
				eidObj.Set(lpNotif[i].info.obj.cbOldParentID,
						   (LPBYTE)lpNotif[i].info.obj.lpOldParentID);
				if (!pThis->Equal(eidObj, pThis->m_eidCalendarFolder))
				{
					continue;
				}
		...
		//-------------------------------------------------------------------
		CXTPMAPIBinary keyID		  = pThis->GetSearchKey(lpNotif[i].info.obj.cbEntryID,
													lpNotif[i].info.obj.lpEntryID);
		DWORD dwEventID				  = pThis->GetEventID(keyID.GetBinarySize(),
											  (LPENTRYID)keyID.GetBinaryData());
		...
		{
			// Delete the event
			// Note that this message will never be received for deleted item for Exchange 5.5 and
			// 2000. More info at MS KB261172
			if (ptrEvent)
			{
				VERIFY(pMemDP->DeleteEvent(ptrEvent));

				//#pragma NOTE("Advice to DP notification and resend")
				// pThis->m_pConnect->SendEvent(XTP_NC_CALENDAREVENTWASDELETED, (WPARAM)dwEventID,
				// (LPARAM)(CXTPCalendarEvent*)ptrEvent);
			}
			continue;
		}
		...
		if ((ulNfType & fnevObjectCreated) && ulNfSender == MAPI_MESSAGE)
		{
			...
			// Add a new event
			CXTPCalendarEventPtr ptrEvent2 = pThis->ImportEvent(lpNotif[i].info.obj.cbEntryID,
																lpNotif[i].info.obj.lpEntryID);
		...
		if ((ulNfType & fnevObjectMoved) && ulNfSender == MAPI_MESSAGE)
		{
			CXTPCalendarEventPtr ptrEvent2 = pThis->ImportEvent_FromCalendarFolderOnly(
				lpNotif[i].info.obj.cbEntryID, lpNotif[i].info.obj.lpEntryID);
...
CXTPMAPIBinary CXTPCalendarMAPIDataProvider::GetEntryID(CXTPMAPIBinary& eSearchKey)
{
	...
	LPSRestriction pRestriction = BuildBinaryRestriction(eSearchKey.GetBinarySize(),
														 eSearchKey.GetBinaryData(), PR_SEARCH_KEY);
...
DWORD CXTPCalendarMAPIDataProvider::GetEventID(ULONG cbSearchKey, LPENTRYID lpSearchKey,
											   BOOL bAddNew)
{
...
BOOL CXTPCalendarMAPIDataProvider::Equal(const CXTPMAPIBinary& eid1, const CXTPMAPIBinary& eid2)
{
	...
	ULONG ulRes;
	if (SUCCEEDED(m_lpMAPISession->CompareEntryIDs(eid1.m_cb, (LPENTRYID)eid1.GetBinaryData(),
												   eid2.m_cb, (LPENTRYID)eid2.GetBinaryData(), 0,
												   &ulRes)))
	{
		bRes = (BOOL)ulRes;
	}
...
BOOL AFX_CDECL CXTPMAPIBinary::IsBinaryEqual(const CXTPMAPIBinary& eid1, const CXTPMAPIBinary& eid2)
{
	if ((eid1.m_cb != eid2.m_cb) || (eid1.m_arBytes.GetSize() != eid2.m_arBytes.GetSize()))
		return FALSE;

	return 0
		   == memcmp(eid1.m_arBytes.GetData(), eid2.m_arBytes.GetData(),
...
BOOL CXTP_ID_Collection::Lookup(const DWORD dwEventID, CXTPMAPIBinary& obEntryID)
{
	return m_mapEventID.Lookup(dwEventID, obEntryID);
}

BOOL CXTP_ID_Collection::Lookup(CXTPMAPIBinary& obEntryID, DWORD& dwEventID)
{
	return m_mapEntryID.Lookup(obEntryID, dwEventID);
}

BOOL CXTP_ID_Collection::Add(const DWORD dwEventID, CXTPMAPIBinary& obEntryID)
{
...
void CXTPCalendarMAPIDataProvider::CMAPIPropIDMap::_UpdateMaps(LPMESSAGE pMessage)
{
	...
	{
		hRes = pMessage->GetNamesFromIDs(&pPropTags, NULL, MAPI_NO_STRINGS, &ulPropNames,
										 &ppArPropNames);
	...
	LPMESSAGE pMessage, const XTP_MAPI_PROP_NAME& propNameEx)
{
	...
	if (SUCCEEDED(hRes) && pPropName)
	{
		...
		pPropName->Kind.lID = XTPToLong(propNameEx.m_ulID);
