...
void CXTPCalendarCustomProperties::GetNextProperty(POSITION& rPos, CString& rStrName,
												   COleVariant& rVarValue) const
{
...
void CXTPCalendarCustomProperties::_Load(CXTPPropExchange* pPX)
{
	...
	while (posStorage)
	{
		...
		if (lVt != VT_EMPTY && varValue.vt != lVt)
		{
			...
			HRESULT hr = VariantChangeTypeEx(&varValue2, &varValue, lcidID, 0, (VARTYPE)lVt);
			if (FAILED(hr))
			{
...
BEGIN_DISPATCH_MAP(CXTPCalendarCustomProperties, CXTPCmdTarget)
	DISP_PROPERTY_PARAM_ID(CXTPCalendarCustomProperties, "Property", DISPID_VALUE, OleGetProperty,
						   OleSetProperty, VT_VARIANT, VTS_BSTR)
	DISP_PROPERTY_EX_ID(CXTPCalendarCustomProperties, "Count", 1, OleGetCount, SetNotSupported,
						VT_I4)
	...
	DISP_FUNCTION_ID(CXTPCalendarCustomProperties, "NewEnum", DISPID_NEWENUM, OleNewEnum,
					 VT_DISPATCH, VTS_NONE)

	DISP_FUNCTION_ID(CXTPCalendarCustomProperties, "LoadFromString", 4, OleLoadFromString, VT_EMPTY,
					 VTS_BSTR)
	DISP_FUNCTION_ID(CXTPCalendarCustomProperties, "SaveToString", 5, OleSaveToString, VT_BSTR,
					 VTS_NONE)
END_DISPATCH_MAP()
...
BOOL CXTPCalendarCustomProperties::OleGetNextItem(/* [in, out] */ POSITION& rPos,
												  /* [out] */ VARIANT* pVar)
{
	ASSERT(pVar);
	if (!rPos || !pVar)
