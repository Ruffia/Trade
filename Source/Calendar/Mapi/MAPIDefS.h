...
#		define MAPI_NT_SERVICE                                                                    \
			((ULONG)0x00010000) /* Provider is being loaded in an NT service   \ \ \ \ \                                                                                                 \
								 */
...
#		define PT_UNSPECIFIED                                                                     \
			((ULONG)0)			   /* (Reserved for interface use) type doesn't matter to caller */
...
#		define PT_CURRENCY                                                                        \
			((ULONG)6) /* Signed 64-bit int (decimal w/    4 digits right of decimal pt) */
...
#		define PT_SYSTIME                                                                         \
			((ULONG)64) /* FILETIME 64-bit int w/ number of 100ns periods since Jan 1,1601 */
...
#		define CbSPropTagArray(_lparray)                                                          \
			(offsetof(SPropTagArray, aulPropTag) + (UINT)((_lparray)->cValues) * sizeof(ULONG))
/*  SPropTagArray */
...
#		define CbSRowSet(_lprowset)                                                               \
			(offsetof(SRowSet, aRow) + (UINT)((_lprowset)->cRows * sizeof(SRow)))
#		define SizedSRowSet(_crow, _name)                                                         \
	...
	typedef SCODE(STDMETHODCALLTYPE ALLOCATEMORE)(ULONG cbSize, LPVOID lpObject,
...
#		define INTERFACE IMAPIAdviseSink
	DECLARE_MAPI_INTERFACE_(IMAPIAdviseSink, IUnknown){ BEGIN_INTERFACE MAPI_IUNKNOWN_METHODS(PURE)
															MAPI_IMAPIADVISESINK_METHODS(PURE) };

	/* Callback function type for MAPIAllocAdviseSink */

	typedef long(STDAPICALLTYPE NOTIFCALLBACK)(LPVOID lpvContext, ULONG cNotification,
...
#		define INTERFACE IMAPIProgress
	DECLARE_MAPI_INTERFACE_(IMAPIProgress, IUnknown){ BEGIN_INTERFACE MAPI_IUNKNOWN_METHODS(PURE)
...
#		define MAPI_IMAPIPROP_METHODS(IPURE)                                                      \
			...
			MAPIMETHOD(GetProps)                                                                   \
			(THIS_ LPSPropTagArray lpPropTagArray, ULONG ulFlags, ULONG FAR * lpcValues,           \
			...
			(THIS_ ULONG ulPropTag, LPCIID lpiid, ULONG ulInterfaceOptions, ULONG ulFlags,         \
			 LPUNKNOWN FAR * lppUnk) IPURE;                                                        \
			MAPIMETHOD(SetProps)                                                                   \
			(THIS_ ULONG cValues, LPSPropValue lpPropArray, LPSPropProblemArray FAR * lppProblems) \
				IPURE;                                                                             \
			MAPIMETHOD(DeleteProps)                                                                \
			(THIS_ LPSPropTagArray lpPropTagArray, LPSPropProblemArray FAR * lppProblems) IPURE;   \
			MAPIMETHOD(CopyTo)                                                                     \
			(THIS_ ULONG ciidExclude, LPCIID rgiidExclude, LPSPropTagArray lpExcludeProps,         \
			 ULONG ulUIParam, LPMAPIPROGRESS lpProgress, LPCIID lpInterface, LPVOID lpDestObj,     \
			 ULONG ulFlags, LPSPropProblemArray FAR * lppProblems) IPURE;                          \
			MAPIMETHOD(CopyProps)                                                                  \
			(THIS_ LPSPropTagArray lpIncludeProps, ULONG ulUIParam, LPMAPIPROGRESS lpProgress,     \
			 LPCIID lpInterface, LPVOID lpDestObj, ULONG ulFlags,                                  \
			 LPSPropProblemArray FAR * lppProblems) IPURE;                                         \
			MAPIMETHOD(GetNamesFromIDs)                                                            \
			(THIS_ LPSPropTagArray FAR * lppPropTags, LPGUID lpPropSetGuid, ULONG ulFlags,         \
			 ULONG FAR * lpcPropNames, LPMAPINAMEID FAR * FAR * lpppPropNames) IPURE;              \
			MAPIMETHOD(GetIDsFromNames)                                                            \
			(THIS_ ULONG cPropNames, LPMAPINAMEID FAR * lppPropNames, ULONG ulFlags,               \
			 LPSPropTagArray FAR * lppPropTags) IPURE;

#		undef INTERFACE
#		define INTERFACE IMAPIProp
	DECLARE_MAPI_INTERFACE_(IMAPIProp, IUnknown){ BEGIN_INTERFACE MAPI_IUNKNOWN_METHODS(PURE)
...
#		define CbSSortOrderSet(_lpset)                                                            \
			(offsetof(SSortOrderSet, aSort) + (UINT)((_lpset)->cSorts * sizeof(SSortOrder)))
...
#		define MAPI_IMAPITABLE_METHODS(IPURE)                                                     \
			...
			(THIS_ HRESULT hResult, ULONG ulFlags, LPMAPIERROR FAR * lppMAPIError) IPURE;          \
			MAPIMETHOD(Advise)                                                                     \
			(THIS_ ULONG ulEventMask, LPMAPIADVISESINK lpAdviseSink, ULONG FAR * lpulConnection)   \
				IPURE;                                                                             \
			MAPIMETHOD(Unadvise)                                                                   \
			...
			(THIS_ ULONG FAR * lpulRow, ULONG FAR * lpulNumerator, ULONG FAR * lpulDenominator)    \
				IPURE;                                                                             \
			MAPIMETHOD(FindRow)                                                                    \
			...
			(THIS_ ULONG cbInstanceKey, LPBYTE pbInstanceKey, ULONG ulRowCount, ULONG ulFlags,     \
			 LPSRowSet FAR * lppRows, ULONG FAR * lpulMoreRows) IPURE;                             \
			MAPIMETHOD(CollapseRow)                                                                \
			(THIS_ ULONG cbInstanceKey, LPBYTE pbInstanceKey, ULONG ulFlags,                       \
			...
			(THIS_ ULONG ulFlags, ULONG cbInstanceKey, LPBYTE lpbInstanceKey,                      \
			 ULONG FAR * lpcbCollapseState, LPBYTE FAR * lppbCollapseState) IPURE;                 \
			MAPIMETHOD(SetCollapseState)                                                           \
			(THIS_ ULONG ulFlags, ULONG cbCollapseState, LPBYTE pbCollapseState,                   \
			 BOOKMARK FAR * lpbkLocation) IPURE;

#		undef INTERFACE
#		define INTERFACE IMAPITable
	DECLARE_MAPI_INTERFACE_(IMAPITable, IUnknown){ BEGIN_INTERFACE MAPI_IUNKNOWN_METHODS(PURE)
...
#		define PS_PROFILE_PROPERTIES_INIT                                                         \
			{                                                                                      \
				0x98, 0x15, 0xAC, 0x08, 0xAA, 0xB0, 0x10, 0x1A, 0x8C, 0x93, 0x08, 0x00, 0x2B,      \
					0x2A, 0x56, 0xC2                                                               \
			}
...
#		define MAPI_IMAPISTATUS_METHODS(IPURE)                                                    \
			...
			(THIS_ ULONG ulUIParam, ULONG cbTargetTransport, LPENTRYID lpTargetTransport,          \
			 ULONG ulFlags) IPURE;
...
#		define MAPI_IMAPICONTAINER_METHODS(IPURE)                                                 \
			...
			(THIS_ ULONG cbEntryID, LPENTRYID lpEntryID, LPCIID lpInterface, ULONG ulFlags,        \
			 ULONG FAR * lpulObjType, LPUNKNOWN FAR * lppUnk) IPURE;                               \
			MAPIMETHOD(SetSearchCriteria)                                                          \
			(THIS_ LPSRestriction lpRestriction, LPENTRYLIST lpContainerList, ULONG ulSearchFlags) \
				IPURE;                                                                             \
			MAPIMETHOD(GetSearchCriteria)                                                          \
			(THIS_ ULONG ulFlags, LPSRestriction FAR * lppRestriction,                             \
			 LPENTRYLIST FAR * lppContainerList, ULONG FAR * lpulSearchState) IPURE;
...
#		define MAPI_IABCONTAINER_METHODS(IPURE)                                                   \
			...
			(THIS_ ULONG cbEntryID, LPENTRYID lpEntryID, ULONG ulCreateFlags,                      \
			 LPMAPIPROP FAR * lppMAPIPropEntry) IPURE;                                             \
			MAPIMETHOD(CopyEntries)                                                                \
			(THIS_ LPENTRYLIST lpEntries, ULONG ulUIParam, LPMAPIPROGRESS lpProgress,              \
			 ULONG ulFlags) IPURE;                                                                 \
			MAPIMETHOD(DeleteEntries)                                                              \
			(THIS_ LPENTRYLIST lpEntries, ULONG ulFlags) IPURE;                                    \
			MAPIMETHOD(ResolveNames)                                                               \
			(THIS_ LPSPropTagArray lpPropTagArray, ULONG ulFlags, LPADRLIST lpAdrList,             \
			 LPFlagList lpFlagList) IPURE;

#		undef INTERFACE
#		define INTERFACE IABContainer
	DECLARE_MAPI_INTERFACE_(IABContainer, IMAPIContainer){
		BEGIN_INTERFACE MAPI_IUNKNOWN_METHODS(PURE) MAPI_IMAPIPROP_METHODS(PURE)
...
#		define INTERFACE IDistList
	DECLARE_MAPI_INTERFACE_(IDistList, IMAPIContainer){ BEGIN_INTERFACE MAPI_IUNKNOWN_METHODS(PURE)
															MAPI_IMAPIPROP_METHODS(PURE)
																MAPI_IMAPICONTAINER_METHODS(PURE)
...
#		define MAPI_IMAPIFOLDER_METHODS(IPURE)                                                    \
			...
			(THIS_ LPENTRYLIST lpMsgList, LPCIID lpInterface, LPVOID lpDestFolder,                 \
			 ULONG ulUIParam, LPMAPIPROGRESS lpProgress, ULONG ulFlags) IPURE;                     \
			MAPIMETHOD(DeleteMessages)                                                             \
			(THIS_ LPENTRYLIST lpMsgList, ULONG ulUIParam, LPMAPIPROGRESS lpProgress,              \
			 ULONG ulFlags) IPURE;                                                                 \
			MAPIMETHOD(CreateFolder)                                                               \
			(THIS_ ULONG ulFolderType, LPTSTR lpszFolderName, LPTSTR lpszFolderComment,            \
			 LPCIID lpInterface, ULONG ulFlags, LPMAPIFOLDER FAR * lppFolder) IPURE;               \
			MAPIMETHOD(CopyFolder)                                                                 \
			(THIS_ ULONG cbEntryID, LPENTRYID lpEntryID, LPCIID lpInterface, LPVOID lpDestFolder,  \
			 LPTSTR lpszNewFolderName, ULONG ulUIParam, LPMAPIPROGRESS lpProgress, ULONG ulFlags)  \
				IPURE;                                                                             \
			MAPIMETHOD(DeleteFolder)                                                               \
			(THIS_ ULONG cbEntryID, LPENTRYID lpEntryID, ULONG ulUIParam,                          \
			 LPMAPIPROGRESS lpProgress, ULONG ulFlags) IPURE;                                      \
			MAPIMETHOD(SetReadFlags)                                                               \
			(THIS_ LPENTRYLIST lpMsgList, ULONG ulUIParam, LPMAPIPROGRESS lpProgress,              \
			 ULONG ulFlags) IPURE;                                                                 \
			MAPIMETHOD(GetMessageStatus)                                                           \
			(THIS_ ULONG cbEntryID, LPENTRYID lpEntryID, ULONG ulFlags,                            \
			 ULONG FAR * lpulMessageStatus) IPURE;                                                 \
			MAPIMETHOD(SetMessageStatus)                                                           \
			(THIS_ ULONG cbEntryID, LPENTRYID lpEntryID, ULONG ulNewStatus, ULONG ulNewStatusMask, \
...
#		define MAPI_IMSGSTORE_METHODS(IPURE)                                                      \
			MAPIMETHOD(Advise)                                                                     \
			(THIS_ ULONG cbEntryID, LPENTRYID lpEntryID, ULONG ulEventMask,                        \
			 LPMAPIADVISESINK lpAdviseSink, ULONG FAR * lpulConnection) IPURE;                     \
			MAPIMETHOD(Unadvise)                                                                   \
			(THIS_ ULONG ulConnection) IPURE;                                                      \
			MAPIMETHOD(CompareEntryIDs)                                                            \
			(THIS_ ULONG cbEntryID1, LPENTRYID lpEntryID1, ULONG cbEntryID2, LPENTRYID lpEntryID2, \
			 ULONG ulFlags, ULONG FAR * lpulResult) IPURE;                                         \
			MAPIMETHOD(OpenEntry)                                                                  \
			(THIS_ ULONG cbEntryID, LPENTRYID lpEntryID, LPCIID lpInterface, ULONG ulFlags,        \
			 ULONG FAR * lpulObjType, LPUNKNOWN FAR * lppUnk) IPURE;                               \
			MAPIMETHOD(SetReceiveFolder)                                                           \
			(THIS_ LPTSTR lpszMessageClass, ULONG ulFlags, ULONG cbEntryID, LPENTRYID lpEntryID)   \
				IPURE;                                                                             \
			MAPIMETHOD(GetReceiveFolder)                                                           \
			(THIS_ LPTSTR lpszMessageClass, ULONG ulFlags, ULONG FAR * lpcbEntryID,                \
			 LPENTRYID FAR * lppEntryID, LPTSTR FAR * lppszExplicitClass) IPURE;                   \
...
#		define MAPI_IMESSAGE_METHODS(IPURE)                                                       \
			...
			(THIS_ ULONG ulAttachmentNum, LPCIID lpInterface, ULONG ulFlags,                       \
			 LPATTACH FAR * lppAttach) IPURE;                                                      \
			MAPIMETHOD(CreateAttach)                                                               \
			(THIS_ LPCIID lpInterface, ULONG ulFlags, ULONG FAR * lpulAttachmentNum,               \
			 LPATTACH FAR * lppAttach) IPURE;                                                      \
			MAPIMETHOD(DeleteAttach)                                                               \
			(THIS_ ULONG ulAttachmentNum, ULONG ulUIParam, LPMAPIPROGRESS lpProgress,              \
			 ULONG ulFlags) IPURE;                                                                 \
...
#		define SET_ADRPARM_VERSION(ulFlags, ulVersion)                                            \
			(((ULONG)ulVersion) | (((ULONG)ulFlags) & 0x0FFFFFFF))
...
#		define DT_FOLDER_SPECIAL ((ULONG)0x04000000)
	...
	typedef SCODE(STDMETHODCALLTYPE FAR* LPFNBUTTON)(ULONG ulUIParam, LPVOID lpvContext,
													 ULONG cbEntryID, LPENTRYID lpSelection,
													 ULONG ulFlags);
...
#		define INTERFACE IMAPIControl
	DECLARE_MAPI_INTERFACE_(IMAPIControl, IUnknown){ BEGIN_INTERFACE MAPI_IUNKNOWN_METHODS(PURE)
...
#		define SERVICE_UI_ALWAYS                                                                  \
			0x00000002 /* Duplicate UI_SERVICE for consistency and compatibility */
...
#		define MAPI_IPROVIDERADMIN_METHODS(IPURE)                                                 \
			...
			(THIS_ LPTSTR lpszProvider, ULONG cValues, LPSPropValue lpProps, ULONG ulUIParam,      \
			 ULONG ulFlags, MAPIUID FAR * lpUID) IPURE;                                            \
			MAPIMETHOD(DeleteProvider)                                                             \
			(THIS_ LPMAPIUID lpUID) IPURE;                                                         \
			MAPIMETHOD(OpenProfileSection)                                                         \
			(THIS_ LPMAPIUID lpUID, LPCIID lpInterface, ULONG ulFlags,                             \
			 LPPROFSECT FAR * lppProfSect) IPURE;

#		undef INTERFACE
#		define INTERFACE IProviderAdmin
	DECLARE_MAPI_INTERFACE_(IProviderAdmin, IUnknown){ BEGIN_INTERFACE MAPI_IUNKNOWN_METHODS(PURE)
