...
CString CXTPCalendarUtils::GetLocaleString(LCTYPE LCType, int nMaxLength)
{
	...
	int nResult = ::GetLocaleInfo(GetActiveLCID(), LCType, strResult.GetBufferSetLength(nMaxLength),
								  nMaxLength);
...
CString CXTPCalendarUtils::GetDateFormat(CONST SYSTEMTIME* lpDate, LPCTSTR lpFormat)
{
	...
	int nResult = ::GetDateFormat(GetActiveLCID(), 0, lpDate, lpFormat,
...
CString CXTPCalendarUtils::GetTimeFormat(CONST SYSTEMTIME* lpTime, LPCTSTR lpFormat)
{
	...
	int nResult = ::GetTimeFormat(GetActiveLCID(), 0, lpTime, lpFormat,
								  strResult.GetBufferSetLength(255), 255);
	strResult.ReleaseBuffer();
	ASSERT(strResult.GetLength() == nResult - 1);
	UNUSED(nResult);
	...
	if (strResult.IsEmpty())
	{
		...
		nResult = ::GetTimeFormat(GetActiveLCID(), 0, &MySystemTime, lpFormat,
								  strResult.GetBufferSetLength(255), 255);
		strResult.ReleaseBuffer();
		ASSERT(strResult.GetLength() == nResult - 1);
		UNUSED(nResult);
...
BOOL CXTPCalendarUtils::CalcDayOfMonth(COleDateTime& rdtDate, int nWhichDay, int nWhichDayMask)
{
	...
	int nFCount = nWhichDay == xtpCalendarWeekLast ? xtpCalendarWeekLast - 1
												   : xtpCalendarWeekFirst - 1;
...
BOOL CXTPCalendarUtils::SystemTimeToTzSpecificLocalTime(const TIME_ZONE_INFORMATION* pTZI,
														...
														SYSTEMTIME* pTzTime)
{
	...
	{
		return ::SystemTimeToTzSpecificLocalTime((TIME_ZONE_INFORMATION*)pTZI, (SYSTEMTIME*)pUTime,
												 pTzTime);
	}
...
BOOL CXTPCalendarUtils::IsInDst(const TIME_ZONE_INFORMATION* pTZI, COleDateTime dtTime)
{
	if (pTZI->StandardDate.wMonth == 0 || pTZI->DaylightDate.wMonth == 0 || pTZI->DaylightBias == 0)
	{
		return FALSE;
	}
...
CString CXTPCalendarUtils::FormatTimeDuration(int nMinutes, BOOL bRoundly)
{
	...
	else
	{
		if (s_RCData.s_mapMinutes.Lookup(nMinutes, nStrID) && nStrID)
			...
			strDuration.Format(_T("%d %s"), nMinutes,
							   (LPCTSTR)LoadString(XTP_IDS_CALENDAR_MINUTES));
	}
...
void AFX_CDECL CXTPCalendarUtils::StripFormat(CString& strFormat, int nIndex, int nRemove)
{
	...
	while (nIndex >= 0 && !IsFormatChar(strFormat[nIndex]))
	{
		...
		{
			nIndex--;
			while (nIndex > 0 && strFormat[nIndex] != '\'')
				nIndex--;
		}
		nIndex--;
	}
	...
	while (nIndex < strFormat.GetLength() && !IsFormatChar(strFormat[nIndex]))
	{
		...
		{
			nIndex++;
			while (nIndex < strFormat.GetLength() && strFormat[nIndex] != '\'')
				nIndex++;
		}
		nIndex++;
	}
...
BOOL CXTPCalendarUtils::IsEqual(const COleDateTime& dt1, const COleDateTime& dt2,
								BOOL bOneSecTimeDiff)
{
	if (dt1.GetStatus() == COleDateTime::null || dt2.GetStatus() == COleDateTime::null)
		return FALSE;
...
int AFX_CDECL CXTPCalendarUtils::GetDayOfWeekLocaleStringId(int nWeekDay)
{
	...
	int nFirstID = XTPSystemVersion()->IsWinVistaOrGreater() ? LOCALE_SSHORTESTDAYNAME1
	...
	return (dt.GetStatus() == hitInfo2.dt.GetStatus())
		   && ((dt.GetStatus() == COleDateTime::valid)
			   && (CXTPCalendarUtils::IsEqual(dt, hitInfo2.dt)
				   || (dt.GetStatus() != COleDateTime::valid)))
		   && (bTimePartValid == hitInfo2.bTimePartValid) && (uHitCode == hitInfo2.uHitCode)
		   && (pViewDay == hitInfo2.pViewDay) && (pViewGroup == hitInfo2.pViewGroup)
		   && (pViewEvent == hitInfo2.pViewEvent) && (nDay == hitInfo2.nDay)
		   && (nGroup == hitInfo2.nGroup) && (nEvent == hitInfo2.nEvent);
};
