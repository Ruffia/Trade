...
void XTP_LC_TIMEFORMAT::SetCustomFormat(BOOL bAmPm, BOOL bLeadingZero, const CString& separator,
										const CString& am, const CString& pm)
{
...
CXTPCalendarDayViewTimeScale::CXTPCalendarDayViewTimeScale(CXTPCalendarDayView* pDayView,
														   BOOL bVisible, BOOL bDrawNowLine)
...
int CXTPCalendarDayViewTimeScale::CalcWidth(CDC* pDC)
{
	...
	int nHourHeight					 = nRowHeight
					  * (CXTPCalendarUtils::GetTotalMinutes(spScaleInterval) < 60 ? 2 : 1);
...
void CXTPCalendarDayViewTimeScale::DrawLTR(CDC* pDC)
{
	...
	if (m_bDrawNowLineDyn)
	{
		...
		double dPixelPerSecond = m_Layout.m_rcTimeZone.Height()
		...
		XTP_SAFE_CALL3(m_pDayView, GetPaintManager(), GetDayViewTimeScaleHeaderPart(),
					   DrawNowLine(pDC, m_pDayView, this, m_Layout.m_rcTimeZone, nNowLineY, TRUE));
	}
	...
	if (CXTPCalendarUtils::GetTotalHours(spScaleInterval) >= 1)
	{
		...
		{
			CRect rcCell(m_Layout.m_rcTimeZone.left, m_Layout.m_rcTimeZone.top + nRowHeight * i,
	...
	else
	{
		...
		for (int i = 0; i < nDrawCellCount; i++)
		{
			...
			// draw text
			XTP_SAFE_CALL3(m_pDayView, GetPaintManager(), GetDayViewTimeScaleCellPart(),
						   DrawBigHourCell(pDC, rcCell, strHour, strMin, nRowPerHour, TRUE,
										   bCurrentTime));
	...
	{
		XTP_SAFE_CALL3(m_pDayView, GetPaintManager(), GetDayViewTimeScaleHeaderPart(),
					   DrawNowLine(pDC, m_pDayView, this, m_Layout.m_rcTimeZone, nNowLineY, FALSE));
	}
	...
	//*** Draw header
	CRect rcHeader(m_Layout.m_rcHeader.left, m_Layout.m_rcHeader.top, m_Layout.m_rcHeader.right,
				   m_Layout.m_rcTimeZone.top);

	XTP_SAFE_CALL3(m_pDayView, GetPaintManager(), GetDayViewTimeScaleHeaderPart(),
				   OnDraw(pDC, m_pDayView, rcHeader, m_strCaption));
}

CString CXTPCalendarDayViewTimeScale::Format(COleDateTime dtTime, BOOL bTopCell, CString* pHour,
											 CString* pMin)
{
	RefreshTimeFormat();
...
void CXTPCalendarDayViewTimeScale::AdjustLayout(CDC* pDC)
{
	...
	//----------------------------------------------------
	int nNowLineFlags = XTP_SAFE_GET2(m_pDayView, GetCalendarOptions(),
	...
	if (m_bDrawNowLineDyn)
	{
		...
		{
			m_bDrawNowLineDyn = m_bDrawNowLineDyn
...
BOOL CXTPCalendarDayViewTimeScale::HitTestDateTime(XTP_CALENDAR_HITTESTINFO* pHitTest) const
{
	...
	{
		int nCell = (pHitTest->pt.y - m_Layout.m_rcTimeZone.top)
					/ max(1, m_pDayView->GetRowHeight());
...
void CXTPCalendarDayViewTimeScale::SetCustomTimeFormat(BOOL bAmPm, BOOL bLeadingZero,
													   const CString& separator, const CString& am,
													   const CString& pm)
{
...
void CXTPCalendarTimeScalePropertiesDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Check(pDX, XTP_IDC_CHECK_ADDITIONAL_TIMEZONE, m_bShowAdditionalScale);
...
void CXTPCalendarTimeScalePropertiesDlg::OnBnClickedScale2()
{
	...
	m_edtCaption2.EnableWindow(m_bShowAdditionalScale);
	m_cmbZones2.EnableWindow(m_bShowAdditionalScale);
	m_btnAutoAdjustDayLight2.EnableWindow(m_bShowAdditionalScale);

	if (m_bShowAdditionalScale)
	{
		OnSelchangeZones2();
	}
}
