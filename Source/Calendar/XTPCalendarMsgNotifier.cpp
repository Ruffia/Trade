...
BOOL CXTPCalendarMsgNotifier::CreateWnd()
{
	...
	BOOL bCreated = CreateEx(0, pcszSimpleWndClass, XTP_TOP_LEVEL_WND_NOTIFIER_NAME, WS_POPUP,
							 rcEmpty, NULL, 0);
...
void CXTPCalendarMsgNotifier::OnTimer(UINT_PTR uTimerID)
{
	if (uTimerID == 1)
	{
		...
		// Is date changed ?
		if (timeCurrent.wDay != m_timeLastDate.wDay || timeCurrent.wMonth != m_timeLastDate.wMonth
...
BOOL CXTPCalendarMsgNotifier::IsTimeZoneEqual(const TIME_ZONE_INFORMATION& rTzi1,
