...
void CXTPCalendarResourcesManager::_SaveCloseDPifNeed(CXTPCalendarData* pData)
{
	...
	BOOL bPropSave  = pData->GetCustomProperties()->GetProperty(cszCalendarDPF_SaveOnDestroy,
																ovarSave);
	BOOL bPropClose = pData->GetCustomProperties()->GetProperty(cszCalendarDPF_CloseOnDestroy,
																ovarClose);
...
BOOL CXTPCalendarResourcesManager::SetDataProvider(int nIndex, LPCTSTR pcszConnectionString,
												   int eDPFlags)
{
...
CXTPCalendarData* CXTPCalendarResourcesManager::GetDataProvider(LPCTSTR pcszConnectionString,
																int* pnIndex) const
{
...
void CXTPCalendarResourcesManager::_Load(CXTPPropExchange* pPX)
{
	...
	while (posStorage)
	{
		...
		{
			const int eDPFAll = xtpCalendarDPF_CreateIfNotExists | xtpCalendarDPF_SaveOnDestroy
...
BEGIN_DISPATCH_MAP(CXTPCalendarResourceDescription, CXTPCmdTarget)
	...
	DISP_PROPERTY_EX_ID(CXTPCalendarResourceDescription, "Resource", 3, OleGetResource,
						OleSetResource, VT_DISPATCH)
END_DISPATCH_MAP()
...
void CXTPCalendarResourceDescription::OleSetResource(LPDISPATCH pDispResource)
{
	...
	if (pDispResource)
	{
		pRes = XTP_DYNAMIC_DOWNCAST_REMOTE_(CXTPCalendarResource,
											CXTPCalendarResource::FromIDispatchSafe(pDispResource));
		if (!pRes)
		{
...
BEGIN_DISPATCH_MAP(CXTPCalendarResourcesManager, CXTPCmdTarget)
	DISP_FUNCTION_ID(CXTPCalendarResourcesManager, "AddDataProvider", 10, AddDataProvider, VT_BOOL,
					 VTS_BSTR VTS_I4)
	DISP_FUNCTION_ID(CXTPCalendarResourcesManager, "SetDataProvider", 11, SetDataProvider, VT_BOOL,
					 VTS_I4 VTS_BSTR VTS_I4)

	DISP_FUNCTION_ID(CXTPCalendarResourcesManager, "DataProvidersCount", 12, GetDataProvidersCount,
					 VT_I4, VTS_NONE)
	DISP_FUNCTION_ID(CXTPCalendarResourcesManager, "DataProvider", 13, OleGetDataProvider,
					 VT_DISPATCH, VTS_I4)

	DISP_FUNCTION_ID(CXTPCalendarResourcesManager, "RemoveDataProvider", 14, RemoveDataProvider,
					 VT_EMPTY, VTS_I4)
	DISP_FUNCTION_ID(CXTPCalendarResourcesManager, "GetDataProvider", 15, OleGetDataProvider2,
					 VT_DISPATCH, VTS_BSTR)
	DISP_FUNCTION_ID(CXTPCalendarResourcesManager, "GetDataProviderIndex", 16,
					 OleGetDataProviderIndex, VT_I4, VTS_BSTR)
	DISP_FUNCTION_ID(CXTPCalendarResourcesManager, "GetDataProviderConnStr", 17,
					 OleGetDataProviderConnStr, VT_BSTR, VTS_I4)

	//***
	DISP_FUNCTION_ID(CXTPCalendarResourcesManager, "AddResource", 20, AddResource, VT_BOOL,
					 VTS_BSTR VTS_BOOL)

	DISP_FUNCTION_ID(CXTPCalendarResourcesManager, "ResourcesCount", 21, GetResourcesCount, VT_I4,
					 VTS_NONE)
	DISP_FUNCTION_ID(CXTPCalendarResourcesManager, "Resource", 22, OleGetResource, VT_DISPATCH,
					 VTS_I4)

	DISP_FUNCTION_ID(CXTPCalendarResourcesManager, "RemoveResource", 23, RemoveResource, VT_EMPTY,
					 VTS_I4)
	DISP_FUNCTION_ID(CXTPCalendarResourcesManager, "MoveResource", 24, MoveResource, VT_EMPTY,
					 VTS_I4 VTS_I4)

	//***
	DISP_FUNCTION_ID(CXTPCalendarResourcesManager, "DoExchangeCfg", 30, OleDoExchangeCfg, VT_EMPTY,
					 VTS_DISPATCH)
	...
	DISP_FUNCTION_ID(CXTPCalendarResourcesManager, "ApplyToCalendar", 33, OleApplyToCalendar,
