	...
	if (IsEditingSubject() && m_pSubjectEditor)
	{
		...
		if (!IsMultidayEvent())
		{
			...
			else
			{
				...
				{
					GetViewGroup()->GetViewDay()->GetView()->OnUpdateEditingSubject(
						GetEditingSubjectFromEditor());
				}
...
void CXTPCalendarDayViewEvent::AdjustLayout(CDC* pDC, const CRect& rcEventMax,
											int nEventPlaceNumber)
{
	...
	if (IsMultidayEvent())
	{
		...
		CSize szText = XTP_SAFE_GET3(GetView(), GetPaintManager(), GetDayViewEventPart(),
	...
	if (nBottomCell < nTopCell)
	{
		...
		else if (pEvent->GetEndTime().GetDay() != GetViewGroup()->GetCellDateTime(0).GetDay())
		{
	...
	m_rcEventMax.top	= max(m_rcEvent.top - XTP_DPI_Y(5),
							  GetViewGroup()->m_LayoutX.m_rcDayDetails.top);
	m_rcEventMax.bottom = min(m_rcEvent.bottom + XTP_DPI_Y(5),
							  GetViewGroup()->m_LayoutX.m_rcDayDetails.bottom);
	...
	double dSecondsPerPixel =
		GetViewGroup()->GetViewDay()->GetView()->GetCellDuration().GetTotalSeconds()
...
void CXTPCalendarDayViewEvent::AdjustLayout2(CDC* pDC, const CRect& rcEventMax,
											 int nEventPlaceNumber)
{
	TBase::AdjustLayout(pDC, rcEventMax, nEventPlaceNumber);
	...
	if (IsMultidayEvent())
	{
		...
		pTheme->GetDayViewPart()->GetDayPart()->GetGroupPart()->GetMultiDayEventPart()->AdjustLayout(
			this, pDC, rcEventMax, nEventPlaceNumber);
		return;
	}

	pTheme->GetDayViewPart()->GetDayPart()->GetGroupPart()->GetSingleDayEventPart()->AdjustLayout(
		this, pDC, rcEventMax, nEventPlaceNumber);
}
...
{
	CXTPCalendarTheme* pTheme = XTP_SAFE_GET3(GetViewGroup_(), GetViewDay_(), GetView_(),
											  GetTheme(), NULL);

	if (pTheme)
	{
		CRect rcEventNew = m_rcEvent;
		pTheme->GetDayViewPart()->GetDayPart()->GetGroupPart()->GetMultiDayEventPart()->CalcEventYs(
			rcEventNew, nNewPlace);
...
void CXTPCalendarDayViewEvent::Draw(CDC* pDC)
{
	...
	// ASSERT((m_nMultiDayEventFlags & xtpCalendarMultiDayMaster) || m_nMultiDayEventFlags ==
	// xtpCalendarMultiDayNoMultiDay);
...
BOOL CXTPCalendarDayViewEvent::HitTestEx(CPoint point, XTP_CALENDAR_HITTESTINFO* pInfo) const
{
	...
	if (IsMultidayEvent())
	{
		...
		{
			tmpInfo.uHitCode = xtpCalendarHitTestEventResizeHArea
							   | xtpCalendarHitTestEventResizeBegin;
			*pInfo = tmpInfo;
			return TRUE;
		}
		...
		{
			tmpInfo.uHitCode = xtpCalendarHitTestEventResizeHArea
							   | xtpCalendarHitTestEventResizeEnd;
			*pInfo = tmpInfo;
			return TRUE;
		}
...
{
	CFont* pFont = &XTP_SAFE_GET3(GetView(), GetPaintManager(), GetDayViewEventPart(),
