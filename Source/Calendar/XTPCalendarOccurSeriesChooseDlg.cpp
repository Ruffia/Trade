...
CXTPCalendarOccurSeriesChooseDlg::CXTPCalendarOccurSeriesChooseDlg(CWnd* pParent,
																   UINT nIDResourceLabels,
																   LPCTSTR lpszResourceIcon,
																   UINT nIDTemplate)
{
...
void CXTPCalendarOccurSeriesChooseDlg::SetEvent(CXTPCalendarEvent* pEvent)
{
	...
	CXTPResourceManager::AssertValid(
		XTPResourceManager()->LoadString(&strDialog, m_nIDResourceLabels));
...
#define OFFSETWINDOW(ID, OffsetTop, OffsetHeight)                                                  \
	...
	GetDlgItem(ID)->MoveWindow(rcCtrl.left, rcCtrl.top + OffsetTop, rcCtrl.Width(),                \
							   rcCtrl.Height() + OffsetHeight);
