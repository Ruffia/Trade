...
BOOL CXTPRibbonBuilder::LoadFromResource(UINT uiXMLResID, LPCTSTR lpszResType /*= RT_RIBBON*/,
...
void CXTPRibbonBuilder::AddImageIndex(int nIndex, int nId, int nImageType)
{
	...
	arrImages[nIndex] = XTPToUInt(nId);
}
...
void CXTPRibbonBuilder::LoadIcons(int nId, CUIntArray& arrIcons)
{
	...
	pImageManager->SetIcons(XTPToUInt(nId), arrIcons.GetData(), (int)arrIcons.GetSize(),
							CSize(0, 0), xtpImageNormal);
}
...
BOOL CXTPRibbonBuilder::BuildMainButton(CXTPPropExchange* pPX, CXTPRibbonBar* pRibbonBar)
{
	...
	if (nId != -1)
	{
		...
		pRibbonBar->GetImageManager()->SetIcons(XTPToUInt(nId), nIcons, 1, CSize(0, 0),
												xtpImageNormal);
...
void CXTPRibbonBuilder::BuildControlPopupBar(CXTPPropExchange* pPX, CXTPControl*& pParent,
											 CRuntimeClass* pPopupBarClass)
{
	CXTPPropExchangeEnumeratorPtr pEnumerator(pPX->GetEnumerator(_T("ELEMENT")));
	...
	while (pos)
	{
		...
		if (bEmpty)
		{
			...
			{
				pPopupBar->SetIconSize(XTPSystemMetrics()->GetIconSize());
...
BOOL CXTPRibbonBuilder::BuildMainButtonPopupBar(CXTPPropExchange* pPX, CXTPRibbonBar* pRibbonBar)
{
	...
	if (strEnabled == _T("TRUE"))
	{
		CXTPControl* pControl = pPopupBar->GetControls()->Add(
			new CXTPRibbonControlSystemRecentFileList());
