...
public:
	...
	void Draw(CDC* pDC)
	{
		GetParent()->GetPaintManager()->GetRibbonPaintManager()->DrawRibbonScrollButton(
			pDC, this, m_bScrollLeft);
	}
	...
	virtual void OnClick(BOOL /*bKeyboard  = FALSE */, CPoint /*pt  = CPoint */)
	{
		...
		int cmsTimer = XTPToIntChecked(GetDoubleClickTime() * 4 / 5);
		m_pParent->SetTimer(IDSYS_SCROLL, XTPToUIntChecked(cmsTimer), NULL);
		BOOL bFirst = TRUE;
		...
		while (::GetCapture() == m_pParent->GetSafeHwnd() && IsVisible())
		{
			...
			if (msg.message == WM_TIMER && msg.wParam == IDSYS_SCROLL)
			{
				...
				if (bFirst)
				{
					cmsTimer = XTPToIntChecked(GetDoubleClickTime() / 10);
					m_pParent->SetTimer(IDSYS_SCROLL, XTPToUIntChecked(cmsTimer), NULL);
				}
				bFirst = FALSE;
			}
...
void CXTPRibbonScrollableBar::EnableGroupsScroll(BOOL bScrollLeft, BOOL bScrollRight)
{
	...
	if (bScrollLeft)
	{
		...
		m_pControlScrollGroupsLeft->SetRect(CRect(rcGroups.left - XTP_DPI_X(1), rcGroups.top,
												  rcGroups.left + XTP_DPI_X(12), rcGroups.bottom));
	}
	else
	{
	...
	if (bScrollRight)
	{
		...
		m_pControlScrollGroupsRight->SetRect(CRect(rcGroups.right - XTP_DPI_X(12), rcGroups.top,
												   rcGroups.right + XTP_DPI_X(1), rcGroups.bottom));
	}
	else
	{
...
CSize CXTPRibbonTabPopupToolBar::CalcDynamicLayout(int, DWORD /*nMode*/)
{
	...
	m_pRibbonTab->GetGroups()->CalcDynamicSize(&dc, m_rcGroups.Width(), LM_COMMIT,
											   CRect(m_rcGroups.left + XTP_DPI_X(4),
													 m_rcGroups.top + XTP_DPI_Y(3), XTP_DPI_X(6),
													 XTP_DPI_Y(3)));
...
void CXTPRibbonTabPopupToolBar::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	CXTPRibbonGroup* pHighlightedGroup =
		bMouseLocked || !m_pRibbonBar->GetRibbonPaintManager()->m_bHotTrackingGroups
			? NULL
			: HitTestGroup(point);
