...
BOOL CXTPRibbonControlTab::DrawIcon(CDC* pDC, CPoint pt, CXTPTabManagerItem* pItem, BOOL bDraw,
									CSize& szIcon) const
{
	if (!pItem)
...
BOOL CXTPRibbonControlTab::OnHookKeyDown(UINT nChar, LPARAM /*lParam*/)
{
	...
	if (nCharLayout == VK_UP || nCharLayout == VK_DOWN || nCharLayout == VK_LEFT
		|| nCharLayout == VK_RIGHT)
	{
	...
	{
		m_pParent->SetSelected(m_pControls->GetNext(CXTPControl::GetIndex(), +1),
							   TRUE_KEYBOARD_NEXT);
		return TRUE;
	}
...
HRESULT CXTPRibbonControlTab::GetAccessibleRole(VARIANT varChild, VARIANT* pvarRole)
{
	...
	pvarRole->vt   = VT_I4;
	pvarRole->lVal = GetChildIndex(&varChild) == CHILDID_SELF ? ROLE_SYSTEM_PAGETABLIST
															  : ROLE_SYSTEM_PAGETAB;
	return S_OK;
}
