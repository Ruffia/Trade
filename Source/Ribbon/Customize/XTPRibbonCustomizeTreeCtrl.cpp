...
BOOL CXTPRibbonCustomizeTreeCtrl::OnDrop(COleDataObject* pDataObject, DROPEFFECT dropEffect,
										 CPoint /*point*/)
{
	if (!m_pDragOverItem)
		return FALSE;
	...
	if (pControlToClone)
	{
		if (m_nDropPosition == xtpCoreTreeDropAfter)
		{
			...
			if (pControlDragOver)
			{
				CXTPControl* pControlNew = pControlDragOver->GetRibbonGroup()->AddClone(
					pControlToClone,
					pControlDragOver->GetRibbonGroup()->IndexOf(pControlDragOver) + 1);
		...
		else if (m_nDropPosition == xtpCoreTreeDropBefore)
		{
			...
			{
				CXTPControl* pControlNew = pControlDragOver->GetRibbonGroup()->AddClone(
					pControlToClone, pControlDragOver->GetRibbonGroup()->IndexOf(pControlDragOver));
				pItemDrop->SetItemData((DWORD_PTR)pControlNew);
			}
		}
		else if (m_nDropPosition == xtpCoreTreeDropChild)
		{
			...
			CXTPRibbonGroup* pGroupDragOver =
				DYNAMIC_DOWNCAST(CXTPRibbonGroup, (CCmdTarget*)pDragOverItem->GetItemData());
	...
	{
		if (m_pDraggingItem
			&& DYNAMIC_DOWNCAST(CXTPRibbonTab, (CCmdTarget*)m_pDraggingItem->GetItemData()))
		{
			pItemDrop->Release();

			CXTPRibbonTab* pTabDragging =
				DYNAMIC_DOWNCAST(CXTPRibbonTab, (CCmdTarget*)m_pDraggingItem->GetItemData());
			...
			if (m_nDropPosition == xtpCoreTreeDropAfter)
			{
				...
				CXTPRibbonTab* pTabDragOver =
					DYNAMIC_DOWNCAST(CXTPRibbonTab, (CCmdTarget*)pDragOverItem->GetItemData());
				ASSERT(pTabDragOver);

				if (pTabDragOver)
				{
					pTabDragging->Move(pTabDragging->GetIndex() <= pTabDragOver->GetIndex()
										   ? pTabDragOver->GetIndex()
										   : pTabDragOver->GetIndex() + 1);
				}
			}
			else if (m_nDropPosition == xtpCoreTreeDropBefore)
			{
...
DROPEFFECT CXTPRibbonCustomizeTreeCtrl::OnDragOver(COleDataObject* /*pDataObject*/,
												   DWORD dwKeyState, CPoint point)
{
	...
	if (bDragControl)
	{
		...
		if (nLevel == 2)
		{
			...
			XTPCoreTreeDropPosition nDropPosition = rcItem.CenterPoint().y > point.y
	...
	else if (m_pDraggingItem && GetItemLevel(m_pDraggingItem) == 0)
	{
		...
		return static_cast<DROPEFFECT>(m_pDragOverItem != NULL ? DROPEFFECT_MOVE : DROPEFFECT_NONE);
	}
	else
	{
		SetDragOverItem(NULL);
