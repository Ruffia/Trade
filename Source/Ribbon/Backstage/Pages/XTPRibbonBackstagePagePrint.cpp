...
CXTPRibbonBackstagePagePrint::CXTPRibbonBackstagePagePrint()
	: CXTPRibbonBackstagePage(CXTPRibbonBackstagePagePrint::IDD)
{
	...
	m_imgMgr.SetVectorIcon(type, XTP_XAML_ICON_PRINTER, XTP_IDC_RIBBONBACKSTAGE_BUTTON_PRINT,
						   XTPToUInt(nWidth));
}
...
{
	CXTPRibbonBackstageButton* pButton = (CXTPRibbonBackstageButton*)GetDlgItem(XTPToInt(nID));
	if (!pButton)
		return;

	CXTPImageManagerIcon* pIcon = m_imgMgr.GetImage(XTP_IDC_RIBBONBACKSTAGE_BUTTON_PRINT);
	if (!pIcon)
		return;

	pButton->SetTextImageRelation(xtpButtonImageAboveText);
	pButton->SetIcon(CSize(GetSystemMetrics(SM_CXICON), GetSystemMetrics(SM_CYICON)), pIcon);
...
BOOL CXTPRibbonBackstagePagePrint::OnInitDialog()
{
	...
	m_pFrameWnd->SetWindowPos(0, rc.left, rc.top, rc.Width(), rc.Height(),
...
BOOL CXTPRibbonBackstagePagePrint::CreatePrintPreview()
{
	...
	if (!pView->Create(NULL, NULL, AFX_WS_DEFAULT_VIEW, CRect(0, 0, 0, 0), pParent,
