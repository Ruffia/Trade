...
BOOL CXTPDockingPaneTabbedContainer::DrawIcon(CDC* pDC, CPoint pt, CXTPTabManagerItem* pItem,
...
void CXTPDockingPaneTabbedContainer::OnSizeParent(CWnd* pParent, CRect rect, LPVOID lParam)
{
	...
	if (lpLayout == 0 || lpLayout->hDWP != NULL)
	{
		...
		{
			SetWindowPos(&CWnd::wndBottom, rect.left, rect.top, rect.Width(), rect.Height(),
		...
		GetPinButton()->SetState(DWORD(
			IsHidden()
				? xtpPanePinVisible | xtpPanePinPushed
				: DYNAMIC_DOWNCAST(CXTPDockingPaneMiniWnd, pParent) == 0 ? xtpPanePinVisible : 0));
...
void CXTPDockingPaneTabbedContainer::CreateContainer()
{
	...
	if (!m_hWnd)
	{
		Create(_T("XTPDockingPaneTabbedContainer"), _T(""),
			   WS_CLIPCHILDREN | WS_CLIPSIBLINGS | WS_CHILD | WS_VISIBLE, CRect(0, 0, 0, 0),
			   GetDockingSite(), 0);
...
{
	return m_pSelectedPane ? m_pSelectedPane->GetTitle() : CString();
}
...
void CXTPDockingPaneTabbedContainer::SelectPane(CXTPDockingPane* pPane, BOOL bSetFocus,
												BOOL bDelayRedraw)
{
	BOOL bIsWindows = ::IsWindow(m_hWnd);
	...
	if (m_pSelectedPane != pPane)
	{
		...
		if (bIsWindows && NULL != m_pSelectedPane)
		{
			SetWindowText(GetTitle());
		}
	}

	if (bSetFocus && bIsWindows)
	{
		if (m_bActive)
			GetDockingPaneManager()->OnActivatePane(TRUE, m_pSelectedPane);
...
void CXTPDockingPaneTabbedContainer::NormalizeDockingSize()
{
	...
	if (bHoriz)
		m_szDocking.cx = m_rcWindow.Width();
	else
		m_szDocking.cy = m_rcWindow.Height();
	...
	while (pos)
	{
		...
		{
			if (bHoriz)
				pPane->m_szDocking.cx = m_szDocking.cx;
			else
				pPane->m_szDocking.cy = m_szDocking.cy;
		}
	}
}
		...
		case XTP_IDS_DOCKINGPANE_AUTOHIDE:
			...
			else
			{
				GetDockingPaneManager()->ToggleDocking(pManager->m_bHideGroupOnButtonClick
														   ? (CXTPDockingPaneBase*)this
														   : (CXTPDockingPaneBase*)pSelectedPane);
			}

			break;

		case XTP_IDS_DOCKINGPANE_MAXIMIZE: Maximize(); break;

		case XTP_IDS_DOCKINGPANE_RESTORE: Restore(); break;
	}
}
	...
	switch (pButton->GetID())
	{
		case XTP_IDS_DOCKINGPANE_MENU:
			if (m_pSelectedPane)
			{
				...
				menu.pt = GetExStyle() & WS_EX_LAYOUTRTL
							  ? CPoint(menu.rcExclude.right, menu.rcExclude.bottom)
							  : CPoint(menu.rcExclude.left, menu.rcExclude.bottom);
				menu.pPane			= m_pSelectedPane;
...
void CXTPDockingPaneTabbedContainer::OnLButtonDblClk(UINT /*nFlags*/, CPoint point)
{
	...
	if (nHit == DOCKINGPANE_HITCAPTION)
	{
		...
		if (!IsHidden() && pSelected
			&& ((pSelected->GetOptions() & xtpPaneNoFloatableByCaptionDoubleClick) != 0))
			return;
...
void CXTPDockingPaneTabbedContainer::Copy(CXTPDockingPaneBase* pCloneBase, CXTPPaneToPaneMap* pMap,
	...
	if (this != pClone)
	{
		...
		else
		{
			...
			while (pos)
			{
				...
				{
					if (pClone->GetSelected() == pPane && pSelected == NULL)
						pSelected = pPane;
...
BOOL CXTPDockingPaneTabbedContainer::IsAllowMaximize() const
{
	...
	((CXTPDockingPaneSplitterContainer*)m_pParentContainer)
		->FindChildPane(xtpPaneTypeTabbedContainer, &lst);
...
void CXTPDockingPaneTabbedContainer::Maximize()
{
	...
	while (pos)
	{
		CXTPDockingPaneTabbedContainer* pTabbedContainer = (CXTPDockingPaneTabbedContainer*)
															   lst.GetNext(pos);
...
BOOL CXTPDockingPaneTabbedContainer::IsCaptionButtonVisible(CXTPDockingPaneCaptionButton* pButton)
{
	...
	{
		case XTP_IDS_DOCKINGPANE_CLOSE:
			bVisible = m_pSelectedPane
					   && ((m_pSelectedPane->GetOptions() & xtpPaneNoCloseable) == 0);
			break;

		case XTP_IDS_DOCKINGPANE_AUTOHIDE:
			bVisible = m_pSelectedPane && ((m_pSelectedPane->GetOptions() & xtpPaneNoHideable) == 0)
					   && (pButton->GetState() & xtpPanePinVisible);
			break;

		case XTP_IDS_DOCKINGPANE_MAXIMIZE: bVisible = IsPaneRestored(); break;

		case XTP_IDS_DOCKINGPANE_RESTORE: bVisible = IsPaneMaximized(); break;
...
LRESULT CXTPDockingPaneTabbedContainer::OnHelpHitTest(WPARAM, LPARAM lParam)
{
	...
	return XTPToLRESULT(HID_BASE_RESOURCE + nIDHelp);
}

CXTPDockingPaneCaptionButton*
	CXTPDockingPaneTabbedContainer::HitTestCaptionButton(CPoint point) const
{
	if (IsTitleVisible())
	{
...
void CXTPDockingPaneTabbedContainer::AdjustMinMaxInfoClientRect(LPMINMAXINFO pMinMaxInfo,
																BOOL bCaptionOnly) const
{
	...
	{
		CXTPDockingPaneBase::GetPaintManager()->AdjustClientRect(
			(CXTPDockingPaneTabbedContainer*)this, rc, FALSE);
	}
...
void CXTPDockingPaneTabbedContainer::GetMinMaxInfo(LPMINMAXINFO pMinMaxInfo) const
{
	...
	if (IsPaneMinimized())
	{
		if (m_pParentContainer
			&& ((CXTPDockingPaneSplitterContainer*)m_pParentContainer)->IsHoriz())
	...
	else
	{
		...
		while (pos)
		{
			...
			pMinMaxInfo->ptMinTrackSize.x = max(pMinMaxInfo->ptMinTrackSize.x,
												info.ptMinTrackSize.x);
			pMinMaxInfo->ptMinTrackSize.y = max(pMinMaxInfo->ptMinTrackSize.y,
												info.ptMinTrackSize.y);

			pMinMaxInfo->ptMaxTrackSize.x = min(pMinMaxInfo->ptMaxTrackSize.x,
												info.ptMaxTrackSize.x);
			pMinMaxInfo->ptMaxTrackSize.y = min(pMinMaxInfo->ptMaxTrackSize.y,
...
BOOL CXTPDockingPaneTabbedContainer::OnWndMsg(UINT message, WPARAM wParam, LPARAM lParam,
											  LRESULT* pResult)
{
	CXTPDockingPaneManager* pManager = GetDockingPaneManager();
...
BOOL CXTPDockingPaneTabbedContainer::IsCaptionVertical() const
{
	...
	{
		case xtpPaneCaptionHorizontal: return FALSE;

		case xtpPaneCaptionAutoBySize: return m_rcWindow.Width() > m_rcWindow.Height();
...
HRESULT CXTPDockingPaneTabbedContainer::GetAccessibleChild(VARIANT varChild,
														   IDispatch* FAR* ppdispChild)
{
	...
	*ppdispChild = NULL;
	...
	if (nChild == 0 && GetSelected())
		return AccessibleObjectFromWindow(GetSelected()->m_hwndChild, OBJID_WINDOW, IID_IDispatch,
										  (void**)ppdispChild);
...
HRESULT CXTPDockingPaneTabbedContainer::GetAccessibleName(VARIANT varChild, BSTR* pszName)
{
	...
	if (nChild >= 0 && nChild < m_pCaptionButtons->GetSize())
	{
		...
		CString strTip;
		XTPResourceManager()->LoadString(&strTip, XTPToUInt(nId));
...
HRESULT CXTPDockingPaneTabbedContainer::AccessibleLocation(long* pxLeft, long* pyTop,
...
HRESULT CXTPDockingPaneTabbedContainer::AccessibleHitTest(long xLeft, long yTop, VARIANT* pvarID)
{
	...
	if (nItem == -1)
	{
		...
		if (GetSelected())
			return AccessibleObjectFromWindow(GetSelected()->m_hwndChild, OBJID_WINDOW,
											  IID_IDispatch, (void**)&pvarID->pdispVal);

		return S_FALSE;
	}

	pvarID->vt		 = VT_DISPATCH;
	pvarID->pdispVal = GetItemPane(nItem)->CreateAccProxy();

	return S_OK;
}

HRESULT CXTPDockingPaneTabbedContainer::GetAccessibleDefaultAction(VARIANT varChild,
...
BEGIN_DISPATCH_MAP(CXTPDockingPaneTabbedContainer, CWnd)
	...
	DISP_FUNCTION_ID(CXTPDockingPaneTabbedContainer, "Pane", DISPID_VALUE, OleGetPane, VT_DISPATCH,
					 VTS_I4)
	DISP_FUNCTION_ID(CXTPDockingPaneTabbedContainer, "_NewEnum", DISPID_NEWENUM, OleNewEnum,
					 VT_UNKNOWN, VTS_NONE)
	DISP_PROPERTY_EX_ID(CXTPDockingPaneTabbedContainer, "ParentContainer", 20, OleGetContainer,
						SetNotSupported, VT_DISPATCH)
	DISP_PROPERTY_EX_ID(CXTPDockingPaneTabbedContainer, "Type", 21, OleGetType, SetNotSupported,
						VT_I4)
	DISP_PROPERTY_EX_ID(CXTPDockingPaneTabbedContainer, "hWnd", DISPID_HWND, OleGetHwnd,
						SetNotSupported, VT_HANDLE)
	DISP_PROPERTY_EX_ID(CXTPDockingPaneTabbedContainer, "IsEmpty", 22, OleIsEmpty, SetNotSupported,
						VT_BOOL)
	DISP_PROPERTY_EX_ID(CXTPDockingPaneTabbedContainer, "Position", 23, OleGetPosition,
