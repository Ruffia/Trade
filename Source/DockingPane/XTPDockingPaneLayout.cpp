	...
	: m_pManager(pManager)
{
	...
	m_pTopContainer = (CXTPDockingPaneSplitterContainer*)
						  m_pManager->OnCreatePane(xtpPaneTypeSplitterContainer, this);
...
// The main difference between the .Net versions and the normal versions of _Save is that in the
// .Net versions all the save is done is done in the exact same order of the load.
...
{
	PX_DPI_X_Long(pPX, _T("DockingCX"), m_szDocking.cx, 0);
	PX_DPI_Y_Long(pPX, _T("DockingCY"), m_szDocking.cy, 0);
...
BOOL CXTPDockingPane::DoPropExchange(CXTPPropExchange* pPX)
{
	...
	else
	{
		...
		PX_Int(pPX, _T("DockingHolder"), nIndex, 0);
		if (nIndex)
			info.pDockingHolder = (CXTPDockingPaneBase*)(*pMap)[nIndex];
		PX_Int(pPX, _T("FloatingHolder"), nIndex, 0);
		if (nIndex)
			info.pFloatingHolder = (CXTPDockingPaneBase*)(*pMap)[nIndex];
		PX_Int(pPX, _T("LastHolder"), nIndex, 0);
		if (nIndex)
			info.pLastHolder = (CXTPDockingPaneBase*)(*pMap)[nIndex];
...
BOOL CXTPDockingPaneTabbedContainer::DoPropExchange(CXTPPropExchange* pPX)
{
	...
	else
	{
		...
		for (int i = 1; i <= nCount; i++)
		{
			...
			if (!pPane || pPane->GetType() != xtpPaneTypeDockingPane)
				return FALSE;

			_InsertPane((CXTPDockingPane*)pPane);

			if (nIndex == nSelected)
				pSelected = (CXTPDockingPane*)pPane;
		}
...
BOOL CXTPDockingPaneMiniWnd::DoPropExchange(CXTPPropExchange* pPX)
{
	...
	PX_DPI_Y_Long(pPX, _T("WindowRectTopPos"), m_rcWindow.top);
	PX_DPI_Y_Long(pPX, _T("WindowRectBottomPos"), m_rcWindow.bottom);
	PX_DPI_X_Long(pPX, _T("WindowRectLeftPos"), m_rcWindow.left);
	PX_DPI_X_Long(pPX, _T("WindowRectRightPos"), m_rcWindow.right);
	PX_Bool(pPX, _T("Collapsed"), m_bCollapsed, FALSE);
...
void CXTPDockingPaneLayout::Copy(const CXTPDockingPaneLayout* pLayout)
{
	if (this != pLayout)
	{
		...
		for (int i = 0; i < 4; i++)
			if (pLayout->m_wndPanels[i])
				m_wndPanels[i] = (CXTPDockingPaneAutoHidePanel*)map[pLayout->m_wndPanels[i]];
		...
		while (pos)
		{
			...
			ASSERT(map.Lookup((CXTPDockingPaneBase*)info.pPane,
							  (CXTPDockingPaneBase*&)infoCopy.pPane));

			infoCopy.pPane = (CXTPDockingPane*)map[(CXTPDockingPaneBase*)info.pPane];

			if (info.pDockingHolder)
				infoCopy.pDockingHolder = (CXTPDockingPaneBase*)map[info.pDockingHolder];
			if (info.pLastHolder)
				infoCopy.pLastHolder = (CXTPDockingPaneBase*)map[info.pLastHolder];
			if (info.pFloatingHolder)
				infoCopy.pFloatingHolder = (CXTPDockingPaneBase*)map[info.pFloatingHolder];
				...
				case xtpPaneTypeSplitterContainer:
					if (((CXTPDockingPaneSplitterContainer*)pPane)->m_lstPanes.GetCount() == 0)
					{
						...
						bFound = TRUE;
							 ...
							 && (pParentContainer->GetType() == xtpPaneTypeSplitterContainer)
							 && (((CXTPDockingPaneSplitterContainer*)pPane)->m_lstPanes.GetCount()
								 == 1)
							 && (((CXTPDockingPaneSplitterContainer*)pParentContainer)
									 ->m_lstPanes.GetCount()
								 == 1))
					{
						CXTPDockingPaneBase* pChildPane =
							((CXTPDockingPaneSplitterContainer*)pPane)->m_lstPanes.RemoveHead();
						((CXTPDockingPaneSplitterContainer*)pParentContainer)
							->m_lstPanes.RemoveHead();
						((CXTPDockingPaneSplitterContainer*)pParentContainer)
							->m_lstPanes.AddHead(pChildPane);
...
XTPDockingPaneDirection
	CXTPDockingPaneLayout::_GetPaneDirection(const CXTPDockingPaneBase* pPane) const
{
...
void CXTPDockingPaneLayout::_AddPanesTo(CXTPDockingPaneTabbedContainer* pContainer,
										CXTPDockingPaneBaseList& lst, DWORD dwIgnoredOptions)
{
...
BOOL CXTPDockingPaneLayout::_FindTabbedPaneToHide(CXTPDockingPaneAutoHidePanel* pPanel,
...
void CXTPDockingPaneLayout::HidePane(CXTPDockingPaneBase* pPane)
{
	...
	{
		m_wndPanels[direction] = (CXTPDockingPaneAutoHidePanel*)
									 m_pManager->OnCreatePane(xtpPaneTypeAutoHidePanel, this);
	...
	if (pPane->GetType() == xtpPaneTypeDockingPane)
	{
		...
		CXTPDockingPaneTabbedContainer* pContainer =
			(CXTPDockingPaneTabbedContainer*)m_pManager->OnCreatePane(xtpPaneTypeTabbedContainer,
																	  this);
...
void CXTPDockingPaneLayout::RepositionMargins(CRect& rect, const CRect& rcClientMargins)
{
	...
	m_wndMargins[xtpPaneDockLeft]->MoveWindow(
		CRect(rect.left, rect.top, rcClient.left, rect.bottom));
	m_wndMargins[xtpPaneDockRight]->MoveWindow(
		CRect(rcClient.right, rect.top, rect.right, rect.bottom));
	m_wndMargins[xtpPaneDockTop]->MoveWindow(
		CRect(rcClient.left, rect.top, rcClient.right, rcClient.top));
	m_wndMargins[xtpPaneDockBottom]->MoveWindow(
		CRect(rcClient.left, rcClient.bottom, rcClient.right, rect.bottom));
...
BEGIN_DISPATCH_MAP(CXTPDockingPaneLayout, CXTPCmdTarget)
	...
	DISP_FUNCTION_ID(CXTPDockingPaneLayout, "DoPropExchange", 1, OleDoPropExchange, VT_EMPTY,
					 VTS_DISPATCH)
	DISP_FUNCTION_ID(CXTPDockingPaneLayout, "PanesCount", 2, OleGetPaneCount, VT_I4, VTS_NONE)
	DISP_FUNCTION_ID(CXTPDockingPaneLayout, "_NewEnum", DISPID_NEWENUM, OleNewEnum, VT_UNKNOWN,
					 VTS_NONE)
	...
	DISP_PROPERTY_EX_ID(CXTPDockingPaneLayout, "TopContainer", 5, OleGetTopContainer,
						SetNotSupported, VT_DISPATCH)
	DISP_PROPERTY_EX_ID(CXTPDockingPaneLayout, "ClientContainer", 6, OleGetClientContainer,
						SetNotSupported, VT_DISPATCH)
	//}}AFX_DISPATCH_MAP
END_DISPATCH_MAP()
...
CXTPDockingPaneLayout* AFX_CDECL CXTPDockingPaneLayout::FromDispatch(LPDISPATCH pDisp)
{
	...
	if (NULL != pDisp)
	{
		pInst = XTP_DYNAMIC_DOWNCAST_REMOTE_(CXTPDockingPaneLayout,
											 CXTPDockingPaneLayout::FromIDispatchSafe(pDisp));
		if (NULL == pInst)
		{
