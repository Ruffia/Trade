...
//=============================================================================
// Summary:
//     CXTPDockingPaneVisualStudio2012Theme is used to define drawing routines
...
public:
	...
	//-------------------------------------------------------------------------
	// Summary:
	//     Destroys a CXTPDockingPaneVisualStudio2012Theme object, handles
	...
	//-------------------------------------------------------------------------
	// Summary:
	//     Override this member to change the client rectangle of the
	//     child docking pane.
	// Parameters:
...
#	endif
	...
	//-------------------------------------------------------------------------
	// Summary:
	//     Override this member to change the caption rectangle of the
	//     child docking pane.
	// Parameters:
...
#	if _MSC_VER > 1200
	using CXTPDockingPanePaintManager::AdjustCaptionRect;
#	endif
...
protected:
	...
	//-------------------------------------------------------------------------
	// Summary:
	//     This member function is called to draw a gripper for the
	//     docking pane.
	// Parameters:
	//     pDC       - Pointer to a valid device context.
	//     rcGripper - Size of the gripper to draw.
	//     clr       - Specifies the color of the gripper.
	//     bVertical - TRUE it the gripper should be drawn vertically.
	//-------------------------------------------------------------------------
	void DrawGripper(CDC* pDC, CRect rcGripper, COLORREF clr, BOOL bVertical);

	//-------------------------------------------------------------------------
	// Summary:
	//     This member function is called to draw a dotted line.
	// Parameters:
	//     pDC       - Pointer to a valid device context.
	...
	//     bVertical - TRUE if the line should be drawn vertically.
	//-------------------------------------------------------------------------
	void DrawDottedLine(CDC* pDC, int x, int y, int nSize, COLORREF clr, BOOL bVertical);

	//-------------------------------------------------------------------------
	// Summary:
	//     Call this member function to retrieve the active or inactive color
	//     set for the Docking Pane using the state specified by bActive.
	// Parameters:
	//     bActive - TRUE to retrieve the active color set,
	//               FALSE to retrieve the inactive color set.
	// Returns:
	//     A reference to a CXTPDockingPaneColorSet object representing
	...
	//-------------------------------------------------------------------------
	// Summary:
	//     Called to initialize the active and inactive colors sets for
	//     the docking pane theme.
	// Parameters:
	//     pcsActive   - Pointer to a CXTPDockingPaneColorSet representing
	...
	//-------------------------------------------------------------------------
	// Summary:
	//     Called to free the memory allocated for the docking pane
