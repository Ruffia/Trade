...
CXTPDockingPaneOffice2007Theme::CXTPDockingPaneOffice2007Theme()
{
	...
	m_pPanelPaintManager->m_rcButtonMargin.SetRect(XTP_DPI_X(2), XTP_DPI_Y(1), XTP_DPI_X(2),
												   XTP_DPI_Y(1));
	m_pTabPaintManager->m_rcButtonMargin.SetRect(XTP_DPI_X(2), XTP_DPI_Y(1), XTP_DPI_X(2),
												 XTP_DPI_Y(1));

	m_nCaptionFontGap = XTP_DPI_X(5);
}

void CXTPDockingPaneOffice2007Theme::DrawCaption(CDC* pDC, CXTPDockingPaneTabbedContainer* pPane,
												 CRect rc)
{
...
void CXTPDockingPaneOffice2007Theme::RefreshMetrics()
{
	...
	if (pImages->IsValid())
	{
		...
		m_arrColor[COLOR_3DHIGHLIGHT] = pImages->GetImageColor(_T("Window"),
															   _T("EdgeHighLightColor"));
		...
		m_arrColor[XPCOLOR_TOOLBAR_GRIPPER] = pImages->GetImageColor(_T("Toolbar"),
																	 _T("ToolbarGripper"));
		m_arrColor[XPCOLOR_MENUBAR_BORDER]  = pImages->GetImageColor(_T("Toolbar"),
																	 _T("MenuPopupBorder"));

		m_bLunaTheme = TRUE;

		m_clrSplitterGradient.SetStandardValue(pImages->GetImageColor(_T("DockingPane"),
																	  _T("SplitterLight")),
											   pImages->GetImageColor(_T("DockingPane"),
																	  _T("SplitterDark")));

		m_clrNormalCaption.SetStandardValue(
			pImages->GetImageColor(_T("DockingPane"), _T("NormalCaptionLight")),
			pImages->GetImageColor(_T("DockingPane"), _T("NormalCaptionDark")), 0.25f);
		m_clrNormalCaptionText.SetStandardValue(
			pImages->GetImageColor(_T("DockingPane"), _T("NormalCaptionText")));

		m_clrActiveCaption.SetStandardValue(
			pImages->GetImageColor(_T("DockingPane"), _T("ActiveCaptionLight")),
			pImages->GetImageColor(_T("DockingPane"), _T("ActiveCaptionDark")), 0.25f);
		m_clrActiveCaptionText.SetStandardValue(
			pImages->GetImageColor(_T("DockingPane"), _T("ActiveCaptionText")));
	}
	else
	{
