// XTPDockingPaneSplitterContainer.cpp : implementation of the CXTPDockingPaneSplitterContainer
// class.
//
...
void CXTPDockingPaneSplitterWnd::Create(CXTPDockingPaneManager* pManager, BOOL bHoriz)
{
	m_bHoriz   = bHoriz;
	m_pManager = pManager;
	CWnd::Create(_T("XTPDockingPaneSplitter"), _T("Splitter"),
				 WS_CHILD | WS_VISIBLE | WS_CLIPCHILDREN | WS_CLIPSIBLINGS, CRect(0, 0, 0, 0),
				 pManager->GetSite(), 0);
}

void CXTPDockingPaneSplitterWnd::Create(CXTPDockingPaneSplitterContainer* pContainer,
										CXTPDockingPaneBase* pFirst, CXTPDockingPaneBase* pSecond)
{
	m_pFirst	 = pFirst;
	m_pSecond	= pSecond;
	...
	m_hCursor = XTPResourceManager()->LoadCursor(
		UINT(pContainer->m_bHoriz ? XTP_IDC_HSPLITBAR : XTP_IDC_VSPLITBAR));
	XTPResourceManager()->AssertValid(m_hCursor != 0);

	CWnd::Create(_T("XTPDockingPaneSplitter"), _T("Splitter"),
				 WS_CHILD | WS_VISIBLE | WS_CLIPCHILDREN | WS_CLIPSIBLINGS, CRect(0, 0, 0, 0),
				 pContainer->GetDockingSite(), 0);
}
...
{
	CXTPDockingPanePaintManager* pPaintManager =
		m_pContainer->GetDockingPaneManager()->GetPaintManager();
	...
	if (m_pContainer->m_bHoriz)
	{
		...
		m_pFirst->m_szDocking.cx = GetExStyle() & WS_EX_LAYOUTRTL
									   ? nSum - int(d * nSum) - nSplitterSize
									   : int(d * nSum);
...
void CXTPDockingPaneSplitterContainer::NormalizeDockingSize()
{
	...
	while (pos)
	{
		...
		{
			if (m_bHoriz)
				pPane->m_szDocking.cx = pPane->m_rcWindow.Width();
			else
				pPane->m_szDocking.cy = pPane->m_rcWindow.Height();
		}
	}
}
...
BOOL CXTPDockingPaneSplitterWnd::GetAvailableRect(CRect& rcAvail, CRect& rcUnion)
{
	...
	if (bHoriz)
	{
		...
		if (rcUnion.Width() > mmiSecond.ptMaxTrackSize.x)
			rcAvail.left = max(rcAvail.left,
							   rcUnion.right - mmiSecond.ptMaxTrackSize.x - nSplitterSize);
	...
	{
		rcAvail.DeflateRect(0, max(mmiFirst.ptMinTrackSize.y, nGap), 0,
							max(mmiSecond.ptMinTrackSize.y + nSplitterSize, nGap));
		...
		if (rcUnion.Height() > mmiSecond.ptMaxTrackSize.y)
			rcAvail.top = max(rcAvail.top,
							  rcUnion.bottom - mmiSecond.ptMaxTrackSize.y - nSplitterSize);
...
void CXTPDockingPaneSplitterWnd::OnLButtonDown(UINT /*nFlags*/, CPoint point)
{
	...
	{
		CPoint ptOffset = bHoriz ? CPoint(rc.left - point.x, 0) : CPoint(0, rc.top - point.y);
...
void CXTPDockingPaneSplitterContainer::_Replace(CXTPDockingPaneBase* pPane,
												CXTPDockingPaneBase* pReplace)
{
...
void CXTPDockingPaneSplitterContainer::_InsertPane(CXTPDockingPaneBase* pPane,
												   CXTPDockingPaneBase* pNeighbour, BOOL bAfter)
{
	...
	{
		if (pos == NULL)
			pos = m_lstPanes.GetTailPosition();
...
void CXTPDockingPaneSplitterContainer::OnSizeParent(CWnd* pParent, CRect rect, LPVOID lParam)
{
	...
	while (pos)
	{
		...
		if (m_bHoriz)
		{
			...
			rcSplitter.SetRect(rcPane.right - nSplitterIndent, rect.top,
							   rcPane.right + nSplitterSize + nSplitterIndent, rect.bottom);
		}
		else
		{
			rcPane.bottom = pos == NULL ? rect.bottom : rcPane.top + nLength;
			rcSplitter.SetRect(rect.left, rcPane.bottom - nSplitterIndent, rect.right,
							   rcPane.bottom + nSplitterSize + nSplitterIndent);
		}
		...
		if (lpLayout->hDWP != NULL && posSplitter)
		{
			...
			if (pSplitter)
			{
				...
				pSplitter->SetWindowPos(&CWnd::wndBottom, rcSplitter.left, rcSplitter.top,
										rcSplitter.Width(), rcSplitter.Height(), 0);
...
void CXTPDockingPaneSplitterContainer::FindChildPane(XTPDockingPaneType type,
...
void CXTPDockingPaneSplitterContainer::Copy(CXTPDockingPaneBase* pCloneBase,
											CXTPPaneToPaneMap* pMap, DWORD /*dwIgnoredOptions*/)
{
...
void CXTPDockingPaneSplitterContainer::_AdjustPanesLength(CXTPDockingPaneManager* pManager,
														  CXTPDockingPaneBaseList& lst, CRect rect,
														  BOOL bHoriz, BOOL bApply,
														  int& nTotalLength, int& nLengthAvail)
{
	nTotalLength = 0;
	nLengthAvail = 0;
	...
	while (pos)
	{
		...
		pPane->m_nLength = max(GetMinSize(&mmi, bHoriz),
							   min(GetMaxSize(&mmi, bHoriz), pPane->m_nLength));
		...
		if (pClientPane == NULL && pPane->ContainPane(pClient))
		{
			if (!pManager->m_bHideClient || pPane->m_szDocking == CSize(0, 0))
			{
				pClientPane = pPane;
				if (bApply)
					pClientPane->m_szDocking = CSize(0, 0);
				pClientPane->m_nLength = 0;
			}
		}
		nTotalLength += pPane->m_nLength;
	}
	nLengthAvail = (bHoriz ? rect.Width() : rect.Height())
				   - (nSplitterSize * ((int)lst.GetCount() - 1));
	...
	if (pClientPane && nTotalLength < nLengthAvail)
	{
		...
		{
			if (bHoriz)
				pClientPane->m_szDocking.cx = pClientPane->m_nLength;
			else
				pClientPane->m_szDocking.cy = pClientPane->m_nLength;
		}
...
CRect CXTPDockingPaneSplitterContainer::_CalculateResultDockingRect(BOOL bHoriz,
																	CXTPDockingPaneBaseList& lst,
																	CRect rect,
																	CXTPDockingPaneBase* pPaneI)
{
...
CRect CXTPDockingPaneSplitterContainer::_CalculateResultDockingRectSelf(
	CXTPDockingPaneBase* pPaneI, XTPDockingPaneDirection direction, CXTPDockingPaneBase* pNeighbour)
{
...
CRect CXTPDockingPaneSplitterContainer::_CalculateResultDockingRectChild(
	CXTPDockingPaneBase* pSplitter, CXTPDockingPaneBase* pPane, XTPDockingPaneDirection direction)
{
	BOOL bAfter = (direction == xtpPaneDockRight || direction == xtpPaneDockBottom);
...
void CXTPDockingPaneSplitterContainer::GetMinMaxInfo(LPMINMAXINFO pMinMaxInfo) const
{
	...
	while (pos)
	{
		...
		else
		{
			...
			GetMinSize(pMinMaxInfo, !m_bHoriz) = max(GetMinSize(pMinMaxInfo, !m_bHoriz),
													 GetMinSize(&info, !m_bHoriz));
			GetMaxSize(pMinMaxInfo, !m_bHoriz) = min(GetMaxSize(pMinMaxInfo, !m_bHoriz),
													 GetMaxSize(&info, !m_bHoriz));
		}
	}
...
BEGIN_DISPATCH_MAP(CXTPDockingPaneSplitterContainer, CXTPCmdTarget)
	...
	DISP_FUNCTION_ID(CXTPDockingPaneSplitterContainer, "Pane", DISPID_VALUE, OleGetPane,
					 VT_DISPATCH, VTS_I4)
	DISP_FUNCTION_ID(CXTPDockingPaneSplitterContainer, "_NewEnum", DISPID_NEWENUM, OleNewEnum,
					 VT_UNKNOWN, VTS_NONE)
	DISP_PROPERTY_EX_ID(CXTPDockingPaneSplitterContainer, "ParentContainer", 20, OleGetContainer,
						SetNotSupported, VT_DISPATCH)
	DISP_PROPERTY_EX_ID(CXTPDockingPaneSplitterContainer, "Type", 21, OleGetType, SetNotSupported,
						VT_I4)
	DISP_PROPERTY_EX_ID(CXTPDockingPaneSplitterContainer, "hWnd", DISPID_HWND, OleGetHwnd,
						SetNotSupported, VT_HANDLE)
	DISP_PROPERTY_EX_ID(CXTPDockingPaneSplitterContainer, "IsEmpty", 22, OleIsEmpty,
						SetNotSupported, VT_BOOL)
	DISP_PROPERTY_EX_ID(CXTPDockingPaneSplitterContainer, "Position", 23, OleGetPosition,
