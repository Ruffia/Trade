...
//     show a window selector for Ctrl+F9 and Alt+F7 handlers.
//===========================================================================
...
public:
	...
	enum ItemType
	{
		itemPane,	 // Item is a docking pane.
		itemMDIFrame, // Item is a handle of a MDI child.
		itemSDIFrame  // Item is a handle of a SDI frame.
	};
...
public:
	//-----------------------------------------------------------------------
	// Summary:
	//     Call this method to determine which item, if any, is at the specified
	//     position.
	// Parameters:
	//     pt - Client coordinates to test.
	// Returns:
	//     A pointer to a CItem object if successful, or 0 if no item is at the
	//     specified position.
	//-----------------------------------------------------------------------
	...
	//-----------------------------------------------------------------------
	// Summary:
	//     Call this method to select a specified item.
	// Parameters:
	...
	//-----------------------------------------------------------------------
	// Summary:
	//     Call this method to select an item by its index.
	// Parameters:
...
public:
	//-----------------------------------------------------------------------
	// Summary:
	//     This method is called to retrieve an item's caption.
	// Parameters:
	//     pItem - Pointer to the item with the caption to retrieve.
	// Returns:
	//     The item's caption.
	//-----------------------------------------------------------------------
	...
	//-----------------------------------------------------------------------
	// Summary:
	//     This method is called to retrieve an item's description.
	// Parameters:
	//     pItem - Pointer to the item with the description to retrieve.
	// Returns:
	...
	//-----------------------------------------------------------------------
	// Summary:
	//     This method is called to retrieve an item's path string.
	// Parameters:
	//     pItem - Pointer to the item with the path string to retrieve.
	// Returns:
	//     The item's path string.
	//-----------------------------------------------------------------------
...
public:
	//-----------------------------------------------------------------------
	// Summary:
	//     This method is called to update the position of all items inside the dialog.
	// Returns:
	//     TRUE is successful.
	//-----------------------------------------------------------------------
...
private:
	...
	void FillItems(HWND hWndClient, int& nItemHeight, int& nItemWidth, int& x, int& y, int& nRow,
				   int& nColumn, int& nTotalRow, int& nFirstColumn);

private:
	CXTPDockingPaneManager* m_pManager;		// Owner DockingPaneManager class.
