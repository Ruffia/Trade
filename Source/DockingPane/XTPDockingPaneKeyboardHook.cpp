...
CXTPDockingPaneWindowSelect::CXTPDockingPaneWindowSelect()
{
	...
	XTPResourceManager()->LoadString(&m_strActiveTools,
									 XTP_IDS_DOCKINGPANE_SELECTWINDOW_ACTIVETOOLS);
	XTPResourceManager()->LoadString(&m_strActiveFiles,
									 XTP_IDS_DOCKINGPANE_SELECTWINDOW_ACTIVEFILES);
...
CString CXTPDockingPaneWindowSelect::GetItemDescription(CItem* pItem) const
{
	...
	CString strTypeName;
	if (pDocument->GetDocTemplate() != NULL)
		pDocument->GetDocTemplate()->GetDocString(strTypeName, CDocTemplate::regFileTypeName);
...
void CXTPDockingPaneWindowSelect::FillItems(HWND hWndClient, int& nItemHeight, int& nItemWidth,
											int& x, int& y, int& nRow, int& nColumn, int& nTotalRow,
											int& nFirstColumn)
{
	...
	if (hWndActive)
	{
		...
		while (hWndFrame)
		{
			DWORD dwStyle = XTPToDWORD(GetWindowLong(hWndFrame, GWL_STYLE));
...
BOOL CXTPDockingPaneWindowSelect::Reposition()
{
	...
	if (m_bActivatePanes && m_nPaneCount > 0)
	{
		...
		else
		{
			...
			{
				m_pSelected =
					m_arrItems[m_pSelected->nIndex >= nRow - 1 ? 0 : m_pSelected->nIndex + 1];
			}
			else
			{
				m_pSelected =
					m_arrItems[m_pSelected->nIndex > 0 ? m_pSelected->nIndex - 1 : nRow - 1];
			}
		}
	}
	...
	{
		FillItems(hWndClient, nItemHeight, nItemWidth, x, y, nRow, nColumn, nTotalRow,
				  nFirstColumn);

#ifdef _XTP_INCLUDE_COMMANDBARS
		CXTPTabClientWnd* pTabClient = DYNAMIC_DOWNCAST(CXTPTabClientWnd,
														CWnd::FromHandle(hWndClient));
		if (pTabClient && pTabClient->IsTearOffEnabled())
		{
			...
			for (int i = 0; i < tabClients.GetSize(); i++)
			{
				...
				FillItems(tabClients.GetAt(i)->m_hWnd, nItemHeight, nItemWidth, x, y, nRow, nColumn,
						  nTotalRow, nFirstColumn);
			}
		}
#endif
	}
	...
	CSize sz(XTP_DPI_X(4) + (nItemWidth + XTP_DPI_X(12)) * nTotalColumns,
			 nItemHeight + XTP_DPI_Y(13) + nTotalRow * (nItemHeight + XTP_DPI_Y(1))
				 + XTP_DPI_Y(110));
...
BOOL CXTPDockingPaneWindowSelect::DoModal()
{
	...
	UINT nClassStyle = UINT(bLayoutRTL ? 0 : CS_SAVEBITS | CS_OWNDC);
	...
	if (!CreateEx(DWORD(WS_EX_TOOLWINDOW | (bLayoutRTL ? WS_EX_LAYOUTRTL : 0)),
				  AfxRegisterWndClass(nClassStyle, m_hArrowCursor), 0, WS_POPUP | MFS_SYNCACTIVE,
				  CRect(0, 0, 0, 0), pSite, 0))
		return FALSE;
...
void CXTPDockingPaneWindowSelect::OnPaint()
{
	...
	dc.Draw3dRect(rc, GetXtremeColor(XPCOLOR_HIGHLIGHT_BORDER),
	...
	if (m_nPaneCount > 0)
	{
		CRect rcActiveTools(XTP_DPI_X(9), 0, XTP_DPI_X(9) + XTP_DPI_X(163),
							nItemHeight + XTP_DPI_Y(12));
		...
		if (!m_pManager->IsClientHidden())
		{
			CRect rcActiveFiles(XTP_DPI_X(9) + XTP_DPI_X(13) + XTP_DPI_X(163), 0,
								XTP_DPI_X(9) + XTP_DPI_X(13) + 2 * XTP_DPI_X(163),
								nItemHeight + XTP_DPI_Y(12));
	...
	{
		CRect rcActiveFiles(XTP_DPI_X(9), 0, XTP_DPI_X(9) + 2 * XTP_DPI_X(163),
							nItemHeight + XTP_DPI_Y(12));
		dc.DrawText(m_strActiveFiles, rcActiveFiles, DT_SINGLELINE | DT_VCENTER | DT_NOPREFIX);
	}
	...
	for (int i = 0; i < (int)m_arrItems.GetSize(); i++)
	{
		...
		if (m_pSelected == pItem)
		{
			...
			dc.Draw3dRect(pItem->rc, GetXtremeColor(XPCOLOR_HIGHLIGHT_BORDER),
						  GetXtremeColor(XPCOLOR_HIGHLIGHT_BORDER));
		}
		...
		else
		{
			...
			if (hIcon == NULL)
				hIcon = (HICON)::SendMessage(pItem->hWndFrame, WM_GETICON, ICON_BIG, 0);
			if (hIcon == NULL)
				hIcon = (HICON)(ULONG_PTR)::GetClassLongPtr(pItem->hWndFrame, GCLP_HICONSM);

			if (hIcon)
			{
				if (GetExStyle() & WS_EX_LAYOUTRTL)
					::DrawIconEx(dc, ptIcon.x + szIcon.cx, ptIcon.y, hIcon, -szIcon.cx, szIcon.cy,
								 0, 0, DI_NORMAL);
				else
					::DrawIconEx(dc, ptIcon.x, ptIcon.y, hIcon, szIcon.cx, szIcon.cy, 0, 0,
								 DI_NORMAL);
			}
		}
		...
		dc.DrawText(GetItemCaption(pItem), rcText,
					DT_SINGLELINE | DT_VCENTER | DT_END_ELLIPSIS | DT_NOPREFIX);
	}

	CRect rcInfoPane(XTP_DPI_X(11), rc.bottom - XTP_DPI_Y(100), rc.right - XTP_DPI_X(11),
					 rc.bottom - XTP_DPI_Y(10));
	dc.Draw3dRect(rcInfoPane, GetXtremeColor(XPCOLOR_HIGHLIGHT_BORDER),
				  GetXtremeColor(XPCOLOR_HIGHLIGHT_BORDER));
	...
	CRect rcCaption(rcInfoPane.left, rcInfoPane.top, rcInfoPane.right,
					rcInfoPane.top + rcInfoPane.Height() / 3);
	dc.DrawText(GetItemCaption(m_pSelected), rcCaption,
				DT_SINGLELINE | DT_VCENTER | DT_END_ELLIPSIS | DT_NOPREFIX);

	font.SetFont(&m_fnt);

	CRect rcTypeName(rcInfoPane.left, rcCaption.bottom, rcInfoPane.right,
					 rcCaption.bottom + rcInfoPane.Height() / 3);
	dc.DrawText(GetItemDescription(m_pSelected), rcTypeName,
				DT_SINGLELINE | DT_VCENTER | DT_END_ELLIPSIS | DT_NOPREFIX);

	CRect rcPathName(rcInfoPane.left, rcTypeName.bottom, rcInfoPane.right, rcInfoPane.bottom);
	dc.DrawText(GetItemPath(m_pSelected), rcPathName,
...
LRESULT CALLBACK CXTPDockingPaneKeyboardHook::KeyboardProc(int code, WPARAM wParam, LPARAM lParam)
{
	...
	if ((!(HIWORD(lParam) & KF_UP)))
	{
		if (wParam == VK_TAB && GetKeyState(VK_CONTROL) < 0 && !(HIWORD(lParam) & KF_ALTDOWN))
		{
			...
			{
				pWindowSelect = (CXTPDockingPaneWindowSelect*)pManager
									->GetKeyboardWindowSelectClass()
									->CreateObject();
		...
		if ((HIWORD(lParam) & KF_ALTDOWN) && (wParam == VK_F6)
			&& GetKeyState(VK_CONTROL) >= 0) // Alt + F6
		{
			SAFE_MANAGE_STATE(pKeyboardManager->m_pModuleState);
			...
			if (pManager && (pManager->IsKeyboardNavigateEnabled() & xtpPaneKeyboardUseAltF6))
			{
				pManager->PostMessage(WM_SYSCOMMAND,
									  (UINT)(GetKeyState(VK_SHIFT) >= 0 ? SC_NEXTWINDOW
																		: SC_PREVWINDOW),
									  0);
				return TRUE;
			}
