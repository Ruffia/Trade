...
BOOL CXTPDockingPaneSidePanel::Init(CXTPDockingPaneBase* pBasePane,
...
BOOL CXTPDockingPaneSidePanel::Create(CRect rc)
{
	...
	if (!CreateEx(0, _T("XTPDockingPaneSidePanel"), _T(""),
				  WS_CLIPCHILDREN | WS_CLIPSIBLINGS | WS_CHILD, rc,
				  GetDockingPaneManager()->GetSite(), 0))
	{
		return FALSE;
	}
...
void AFX_CDECL CXTPDockingPaneSidePanel::SortLength(LENGTH* pLength, int nFirstIndex,
													int nLastIndex)
{
	if (nLastIndex - nFirstIndex < 1)
		return;

	qsort(pLength + nFirstIndex, XTPToSizeTChecked(nLastIndex - nFirstIndex + 1), sizeof(LENGTH),
		  CompareLength);
...
void CXTPDockingPaneSidePanel::MovePanel(LENGTH* pLength, CRect rect)
{
	...
	CSize sz		 = bHorizontal ? CSize(pLength->nSize, pLength->nHeight)
						   : CSize(pLength->nHeight, pLength->nSize);
	...
	{
		if (bHorizontal)
			sz.cy = max(sz.cy, nMinHeight);
		else
			sz.cx = max(sz.cx, nMinHeight);
	}
...
void CXTPDockingPaneSidePanel::OnSizeParentEx(CSidePanelArray& arrSide, CWnd* /*pParent*/,
											  CRect rect)
{
	...
	LENGTH* pLength = new LENGTH[XTPToUIntChecked(nCount)];
	...
	for (i = 0; i < nCount; i++)
	{
		...
		if (rcPanel.Height() > mmi.ptMaxTrackSize.y)
			rcPanel.bottom = rcPanel.top + mmi.ptMaxTrackSize.y;
		if (rcPanel.Height() < mmi.ptMinTrackSize.y)
			rcPanel.bottom = rcPanel.top + mmi.ptMinTrackSize.y;
		if (rcPanel.Width() > mmi.ptMaxTrackSize.x)
			rcPanel.right = rcPanel.left + mmi.ptMaxTrackSize.x;
		if (rcPanel.Width() < mmi.ptMinTrackSize.x)
			rcPanel.right = rcPanel.left + mmi.ptMinTrackSize.x;
		...
		pLength[i].nHeight = bHorizontal ? min(rect.Height(), rcPanel.Height())
										 : min(rect.Width(), rcPanel.Width());
	}
...
void CXTPDockingPaneSidePanel::OnSizeParentEx(CXTPDockingPaneManager* pManager, CWnd* pParent,
											  CRect rect)
{
...
void CXTPDockingPaneSidePanel::Copy(CXTPDockingPaneBase* pCloneBase, CXTPPaneToPaneMap* pMap,
...
void CXTPDockingPaneSidePanel::OnCaptionButtonClick(CXTPDockingPaneCaptionButton* pButton)
{
	...
	switch (pButton->GetID())
	{
		...
		case XTP_IDS_DOCKINGPANE_AUTOHIDE: OnPinButtonClick(); break;
	}
}
...
void CXTPDockingPaneSidePanel::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	{
		TRACKMOUSEEVENT tme = { sizeof(TRACKMOUSEEVENT), TME_HOVER, m_hWnd,
...
void CXTPDockingPaneSidePanel::OnPinButtonClick()
{
	...
	else
	{
		...
		{
			SetWindowPos(0, 0, 0, m_rcWindow.Width(), m_rcWindow.Height(),
...
void CXTPDockingPaneSidePanel::DoSlideStep(BOOL bActivate)
{
	...
	int nSize = max(nMinHeight, m_nSlideStep
									* (bHorizontal ? m_rcWindow.Height() : m_rcWindow.Width())
									/ m_nStepsCount);
	...
	if (nIDEvent == TID_CHECKACTIVE)
	{
			...
			&& ::GetCapture() == NULL)
		{
			if (--m_nDeactivationCount <= 0)
			{
				...
				m_bSlideOut = TRUE;
				SetTimer(TID_SLIDEOUT,
						 XTPToUIntChecked(CXTPDockingPaneAutoHideWnd::m_nAnimationInterval), NULL);
...
void CXTPDockingPaneSidePanel::GetMinMaxInfo(LPMINMAXINFO pMinMaxInfo) const
{
	...
	if (IsHorizontal())
		szBorder.cy += nCaptionHeight;
	else
		szBorder.cx += nCaptionHeight;
...
BOOL CXTPDockingPaneSidePanel::IsResizable(int nHit)
{
	...
	{
		if (nHit == HTTOP)
			return m_direction == xtpPaneDockBottom;
		if (nHit == HTBOTTOM)
			return m_direction == xtpPaneDockTop;
		if (nHit == HTLEFT || nHit == HTRIGHT)
			return TRUE;
	}
	else
	{
		if (nHit == HTLEFT)
			return m_direction == xtpPaneDockRight;
		if (nHit == HTRIGHT)
			return m_direction == xtpPaneDockLeft;
		if (nHit == HTTOP || nHit == HTBOTTOM)
			return TRUE;
	}
...
void CXTPDockingPaneSidePanel::OnNcLButtonDown(UINT nHitTest, CPoint point)
{
	if (nHitTest >= HTSIZEFIRST && nHitTest <= HTSIZELAST)
	{
		...
		BOOL bResizeBottom = nHitTest == HTBOTTOM || nHitTest == HTBOTTOMLEFT
							 || nHitTest == HTBOTTOMRIGHT;
		BOOL bResizeRight = nHitTest == HTRIGHT || nHitTest == HTTOPRIGHT
							|| nHitTest == HTBOTTOMRIGHT;
		...
		while (::GetCapture() == m_hWnd)
		{
			...
			if (msg.message == WM_MOUSEMOVE)
			{
				...
				{
					if (bResizeTop)
						m_rcWindow.top = m_rcWindow.bottom - mmi.ptMaxTrackSize.y;
					else
						m_rcWindow.bottom = m_rcWindow.top + mmi.ptMaxTrackSize.y;
				}
				if (m_rcWindow.Height() < mmi.ptMinTrackSize.y)
				{
					if (bResizeTop)
						m_rcWindow.top = m_rcWindow.bottom - mmi.ptMinTrackSize.y;
					else
						m_rcWindow.bottom = m_rcWindow.top + mmi.ptMinTrackSize.y;
				}

				if (m_rcWindow.Width() > mmi.ptMaxTrackSize.x)
				{
					if (bResizeLeft)
						m_rcWindow.left = m_rcWindow.right - mmi.ptMaxTrackSize.x;
					else
						m_rcWindow.right = m_rcWindow.left + mmi.ptMaxTrackSize.x;
				}
				if (m_rcWindow.Width() < mmi.ptMinTrackSize.x)
				{
					if (bResizeLeft)
						m_rcWindow.left = m_rcWindow.right - mmi.ptMinTrackSize.x;
					else
						m_rcWindow.right = m_rcWindow.left + mmi.ptMinTrackSize.x;
				}
...
BOOL CXTPDockingPaneSidePanel::OnWndMsg(UINT message, WPARAM wParam, LPARAM lParam,
										LRESULT* pResult)
{
	CXTPDockingPaneManager* pManager = GetDockingPaneManager();
...
BEGIN_DISPATCH_MAP(CXTPDockingPaneSidePanel, CMiniFrameWnd)
	...
	DISP_FUNCTION_ID(CXTPDockingPaneSidePanel, "Pane", DISPID_VALUE, OleGetPane, VT_DISPATCH,
					 VTS_I4)
	DISP_FUNCTION_ID(CXTPDockingPaneSidePanel, "_NewEnum", DISPID_NEWENUM, OleNewEnum, VT_UNKNOWN,
					 VTS_NONE)
	DISP_PROPERTY_EX_ID(CXTPDockingPaneSidePanel, "ParentContainer", 20, OleGetContainer,
						SetNotSupported, VT_DISPATCH)
	DISP_PROPERTY_EX_ID(CXTPDockingPaneSidePanel, "Type", 21, OleGetType, SetNotSupported, VT_I4)
	DISP_PROPERTY_EX_ID(CXTPDockingPaneSidePanel, "hWnd", DISPID_HWND, OleGetHwnd, SetNotSupported,
						VT_HANDLE)
	DISP_PROPERTY_EX_ID(CXTPDockingPaneSidePanel, "IsEmpty", 22, OleIsEmpty, SetNotSupported,
						VT_BOOL)
	DISP_PROPERTY_EX_ID(CXTPDockingPaneSidePanel, "Position", 23, OleGetPosition, SetNotSupported,
						VT_I4)
END_DISPATCH_MAP()
