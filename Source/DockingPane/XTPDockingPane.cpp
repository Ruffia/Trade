...
#ifndef _XTP_ACTIVEX
	CString strTitle;
	if (strTitle.LoadString(XTPToUInt(nID)))
	{
		SetTitle(strTitle);
	}
#endif
...
void CXTPDockingPane::SetEnabled(XTPDockingPaneEnableOptions nState)
{
	if (m_bEnabled != nState)
	{
		m_bEnabled = nState;
		InvalidatePane(TRUE);
	}
...
void CXTPDockingPane::OnSizeParent(CWnd* pParent, CRect rect, LPVOID lParam)
{
	...
	if (lpLayout == 0 || lpLayout->hDWP != NULL)
	{
		...
		{
			MapWindowPoints(pParent->GetSafeHwnd(), m_pParentContainer->GetPaneHwnd(),
							(LPPOINT)&rect, 2);
...
void CXTPDockingPane::Copy(CXTPDockingPaneBase* pCloneBase, CXTPPaneToPaneMap* /*pMap*/,
...
CWnd* CXTPDockingPane::AttachView(CWnd* pParentWnd, CRuntimeClass* pViewClass,
								  ...
								  CCreateContext* pContext /*= NULL*/)
{
	...
	if (!pFrame->Create(NULL, NULL, WS_CHILD | WS_CLIPCHILDREN | WS_CLIPSIBLINGS, CRect(0, 0, 0, 0),
...
BOOL CXTPDockingPane::IsTabsVisible() const
{
	...
	return (XTP_SAFE_GET1(((CXTPDockingPaneTabbedContainer*)m_pParentContainer), IsTabsVisible(),
						  FALSE));
}

CXTPImageManagerIcon* CXTPDockingPane::GetIcon(int nWidth) const
{
	return GetDockingPaneManager()->GetIcon(XTPToUInt(GetIconID()), nWidth);
}
...
COLORREF CXTPDockingPane::GetItemColor() const
{
	...
	return (COLORREF)(xtpTabColorBlue + GetID() % 8);
}
...
HRESULT CXTPDockingPane::GetAccessibleParent(IDispatch* FAR* ppdispParent)
{
	...
	*ppdispParent = NULL;
	...
	if (m_pParentContainer)
	{
		*ppdispParent = ((CXTPDockingPaneTabbedContainer*)m_pParentContainer)->CreateAccProxy();
		return S_OK;
	}
	return E_FAIL;
}
...
HRESULT CXTPDockingPane::GetAccessibleChild(VARIANT varChild, IDispatch* FAR* ppdispChild)
{
	...
	*ppdispChild = NULL;
	...
	{
		return AccessibleObjectFromWindow(m_hwndChild, OBJID_WINDOW, IID_IDispatch,
										  (void**)ppdispChild);
	}
...
HRESULT CXTPDockingPane::GetAccessibleState(VARIANT varChild, VARIANT* pvarState)
{
	...
	if (m_pParentContainer
		&& ((CXTPDockingPaneTabbedContainer*)m_pParentContainer)->GetSelected() == this)
...
HRESULT CXTPDockingPane::AccessibleLocation(long* pxLeft, long* pyTop, long* pcxWidth,
	...
	else
	{
		...
		if (pContainer->GetItemCount() > 1)
		{
			for (int i = 0; i < pContainer->GetItemCount(); i++)
			{
				...
				if (NULL != pItem && pItem->GetData() == (DWORD_PTR)this)
				{
...
BEGIN_DISPATCH_MAP(CXTPDockingPane, CXTPCmdTarget)
	...
	DISP_PROPERTY_EX_ID(CXTPDockingPane, "Hidden", dispidHidden, OleGetHidden, OleSetHidden,
						VT_BOOL)
	DISP_PROPERTY_EX_ID(CXTPDockingPane, "Closed", dispidClosed, IsClosed, OleSetClosed, VT_BOOL)
	DISP_PROPERTY_EX_ID(CXTPDockingPane, "Selected", dispidSelected, IsSelected, OleSetSelected,
						VT_BOOL)
	DISP_PROPERTY_ID(CXTPDockingPane, "Options", dispidOptions, m_dwOptions, VT_I4)

	DISP_PROPERTY_EX_ID(CXTPDockingPane, "Floating", dispidFloating, IsFloating, OleSetFloating,
						VT_BOOL)
	...
	DISP_FUNCTION_ID(CXTPDockingPane, "AttachTo", dispidAttachTo, OleAttachTo, VT_EMPTY,
					 VTS_DISPATCH)

	DISP_PROPERTY_ID(CXTPDockingPane, "Tag", dispidTag, m_dwData, VT_I4)
	DISP_PROPERTY_EX_ID(CXTPDockingPane, "MinTrackSize", 15, OleGetMinTrackSize, SetNotSupported,
						VT_DISPATCH)
	DISP_PROPERTY_EX_ID(CXTPDockingPane, "MaxTrackSize", 16, OleGetMaxTrackSize, SetNotSupported,
						VT_DISPATCH)
	...
	DISP_PROPERTY_EX_ID(CXTPDockingPane, "ParentContainer", 20, OleGetContainer, SetNotSupported,
						VT_DISPATCH)
	...
	DISP_FUNCTION_ID(CXTPDockingPane, "GetClientRect", 25, OleGetClientRect, VT_EMPTY,
					 VTS_PI4 VTS_PI4 VTS_PI4 VTS_PI4)

	DISP_PROPERTY_EX_ID(CXTPDockingPane, "TabCaption", 26, OleGetTabCaption, SetTabCaption, VT_BSTR)
	DISP_PROPERTY_EX_ID(CXTPDockingPane, "TitleToolTip", 27, OleGetTitleToolTip, SetTitleToolTip,
						VT_BSTR)
	//}}AFX_DISPATCH_MAP
END_DISPATCH_MAP()
...
CXTPDockingPane* AFX_CDECL CXTPDockingPane::FromDispatch(LPDISPATCH pDisp)
{
	...
	if (NULL != pDisp)
	{
		pInst = XTP_DYNAMIC_DOWNCAST_REMOTE_(CXTPDockingPane,
											 CXTPDockingPane::FromIDispatchSafe(pDisp));
		if (NULL == pInst)
		{
...
{
	if ((m_hwndChild != NULL) && (m_hwndChild != reinterpret_cast<HWND>((LONG_PTR)hWnd)))
	{
		Detach();
	}
	...
	if (m_hwndChild)
	{
		...
		if (m_pParentContainer)
		{
			SetWindowLong(m_hwndChild, GWL_STYLE, GetWindowLong(m_hwndChild, GWL_STYLE) | WS_CHILD);
...
{
	return (long)reinterpret_cast<LONG_PTR>(m_hwndChild);
}
...
void CXTPDockingPane::OleAttachTo(LPDISPATCH pNeighbourDisp)
{
	...
	GetDockingPaneManager()->AttachPane(this, CXTPDockingPane::FromDispatch(pNeighbourDisp));
}
...
void CXTPDockingPane::OleSetHidden(BOOL bHidden)
{
	if (bHidden)
		...
		GetDockingPaneManager()->ToggleAutoHide(this);
}
...
{
	if (bSelected)
		GetDockingPaneManager()->ShowPane(this, FALSE);
}
...
BOOL CXTPDockingPane::OleGetMaximized()
{
	return m_pParentContainer
			   ? ((CXTPDockingPaneTabbedContainer*)m_pParentContainer)->IsPaneMaximized()
			   : FALSE;
}
