...
void CXTPDockingPaneAutoHideWnd::GetMinMaxInfo(LPMINMAXINFO pMinMaxInfo,
											   BOOL bIncludeSplitter /*= TRUE*/) const
{
...
CXTPDockingPaneAutoHideWnd::CXTPDockingPaneAutoHideWnd(CXTPDockingPaneAutoHidePanel* pPanel,
	...
	, m_pPanel(pPanel)
{
	...
	switch (pManager->GetRTLDirection(m_direction))
	{
		case xtpPaneDockLeft:
			szDocking.cx = min(szDocking.cx,
							   rcAvail.right + XTP_DPI_X(SPLITTER_GAP_X) - rcClient.left);
			break;
		...
		case xtpPaneDockTop:
			szDocking.cy = min(szDocking.cy,
							   rcAvail.bottom + XTP_DPI_Y(SPLITTER_GAP_Y) - rcClient.top);
			break;
	...
	HCURSOR hCursor = !bResizable || pManager->IsSplittersLocked()
						  ? AfxGetApp()->LoadStandardCursor(IDC_ARROW)
						  : XTPResourceManager()->LoadCursor(
								UINT(!m_bHoriz ? XTP_IDC_HSPLITBAR : XTP_IDC_VSPLITBAR));
	m_rcWindow = rcClient;
	...
	if (m_dAnimationDelay != .0 && m_nAnimationInterval != 0)
	{
		if (m_dAnimationDelay > 0)
		{
			...
			// implementation
			m_nStepsCount = int((m_bHoriz ? m_rcWindow.Width() : m_rcWindow.Height())
								* m_dAnimationDelay * 50 / m_nAnimationInterval);
		}
		else
		{
	...
	m_bSlideOut = FALSE;
	if (m_nStepsCount > 1)
		SetTimer(TID_SLIDEIN, XTPToUIntChecked(m_nAnimationInterval), NULL);
	DoSlideStep();
...
void CXTPDockingPaneAutoHideWnd::RecalcLayout(BOOL /*bNotify*/)
{
	...
	switch (m_direction)
	{
		...
		case xtpPaneDockRight: rcPane.left += XTP_DPI_X(SPLITTER_GAP_X); break;

		case xtpPaneDockBottom: rcPane.top += XTP_DPI_Y(SPLITTER_GAP_Y); break;
	}
...
void CXTPDockingPaneAutoHideWnd::OnTimer(UINT_PTR nIDEvent)
{
	...
	if (nIDEvent == TID_CHECKACTIVE)
	{
		...
		CPoint pt;
		GetCursorPos(&pt);
		if (!m_pPane->IsActive() && !CXTPWindowRect(this).PtInRect(pt)
			&& !CXTPWindowRect(m_pPanel).PtInRect(pt))
		{
			if (!m_bSlideOut && CXTPDrawHelpers::IsTopParentActive(m_hWnd))
			{
				if (--m_nDeactivationCount <= 0)
				{
					...
					if (!CXTPDockingPaneAutoHidePanel::m_bCloseActiveWindow
						|| GetDockingPaneManager()->NotifyAction(xtpPaneActionCollapsing,
																 m_pPane->GetSelected()))
					{
						m_nDeactivationCount = 6;
					}
					else
					{
						m_bSlideOut = TRUE;
						if (m_hWnd)
							SetTimer(TID_SLIDEOUT, XTPToUIntChecked(m_nAnimationInterval), NULL);
					}
...
void CXTPDockingPaneAutoHideWnd::Reposition(CRect rc, CRect rcClient)
{
	...
	if (!m_bHoriz)
		m_pPane->m_pSelectedPane->m_szDocking.cx = rc.Width();
	else
		m_pPane->m_pSelectedPane->m_szDocking.cy = rc.Height();
...
public:
								  ...
								  CPoint point)
	{
		...
		else
		{
			...
			if (pPane)
			{
				if (!(pPanel->m_pActiveWnd
					  && pPanel->m_pActiveWnd->GetPane()->GetSelected() == pPane))
				{
...
public:
	...
	{
		if (GetPanel()->m_hWnd)
			GetPanel()->InvalidateRect(lpRect, FALSE);
	}
...
class CXTPDockingPaneAutoHidePanel::CAutoHidePanelTabManagersArray
	: public CArray<CAutoHidePanelTabManager*, CAutoHidePanelTabManager*>
{
public:
...
CXTPDockingPaneAutoHidePanel*
	CXTPDockingPaneAutoHidePanel::CAutoHidePanelTabManager::GetPanel() const
{
...
BOOL CXTPDockingPaneAutoHidePanel::DrawIcon(CDC* pDC, CPoint pt, CXTPTabManagerItem* pItem,
...
int CXTPDockingPaneAutoHidePanel::GetPanelHeight() const
{
	...
	return pAppearanceSet->GetButtonHeight(NULL) + pAppearanceSet->m_rcHeaderMargin.bottom
		   + pAppearanceSet->m_rcHeaderMargin.top;
}
...
void CXTPDockingPaneAutoHidePanel::RefreshManagersArray(CDC* pDC, CRect rcClient)
{
	...
	BOOL bIndent   = bVertical
				   && (m_pLayout->m_wndPanels[xtpPaneDockTop] != NULL
					   && !m_pLayout->m_wndPanels[xtpPaneDockTop]->IsEmpty());
	...
	while (pos)
	{
		...
		while (posPane)
		{
			...
			if (pContainer->GetSelected() == pPane
				&& pPaneManager->GetPaintManager()->m_bAutoHidePanelHighlightSelected)
	...
	{
		m_nItemsLength = (!bVertical ? rcClient.left - rcWindow.left : rcClient.top - rcWindow.top)
						 - nMarginIndent;
		...
		BOOL bAddButton = !bVertical ? rcClient.left > rcClient.right
		...
		if (bAddButton)
		{
			if (m_nHeaderOffset < 0)
			{
				...
				if (m_nItemsLength + m_nHeaderOffset
					< nWindowEnd - XTP_DPI_Y(24) - nMarginIndent - nPanelIndent)
				{
					m_nHeaderOffset = min(0, nWindowEnd - XTP_DPI_Y(24) - nMarginIndent
												 - m_nItemsLength - nPanelIndent);
				}
			}

			if (bVertical)
				rcClient.top += m_nHeaderOffset;
			else
				rcClient.left += m_nHeaderOffset;
...
void CXTPDockingPaneAutoHidePanel::_InsertPane(CXTPDockingPaneBase* pPane)
{
	...
	if (pPane->GetType() == xtpPaneTypeTabbedContainer)
	{
		...
		if (((CXTPDockingPaneTabbedContainer*)pPane)->GetSafeHwnd() && pFocus->GetSafeHwnd()
			&& (pFocus == ((CXTPDockingPaneTabbedContainer*)pPane)
				|| ((CXTPDockingPaneTabbedContainer*)pPane)->IsChild(pFocus)))
...
{
	CWnd::Create(_T("XTPDockingPaneAutoHidePanel"), NULL,
				 WS_CHILD | WS_VISIBLE | WS_CLIPCHILDREN | WS_CLIPSIBLINGS, CXTPEmptyRect(),
				 pParent, 0);
...
void CXTPDockingPaneAutoHidePanel::CloseActiveWindow(BOOL bDelayed)
{
	...
	if (m_pActiveWnd)
	{
		...
		{
			if (GetDockingPaneManager()->NotifyAction(xtpPaneActionCollapsing,
...
void CXTPDockingPaneAutoHidePanel::ShowPane(CXTPDockingPane* pPane, BOOL bSetFocus)
{
	...
	{
		if (bSetFocus)
			m_pActiveWnd->m_pPane->SelectPane(pPane, TRUE, FALSE);
	...
	CXTPDockingPaneTabbedContainer* pContainer =
		((CXTPDockingPaneTabbedContainer*)pPane->m_pParentContainer);
	...
	if (bSetFocus)
		pContainer->SelectPane(pPane, TRUE, FALSE);
...
void CXTPDockingPaneAutoHidePanel::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	if (pPane)
	{
		TRACKMOUSEEVENT tme = { sizeof(TRACKMOUSEEVENT), TME_HOVER | TME_LEAVE, m_hWnd,
								CXTPDockingPaneAutoHideWnd::m_nMouseHoverDelay };
...
LRESULT CXTPDockingPaneAutoHidePanel::OnMouseHover(WPARAM, LPARAM lParam)
{
	...
	if (pPane)
	{
		if (!CXTPDrawHelpers::IsTopParentActive(m_hWnd)
			|| (m_pActiveWnd && m_pActiveWnd->m_pPane->GetSelected() == pPane))
			return 1;
...
void CXTPDockingPaneAutoHidePanel::Copy(CXTPDockingPaneBase* pCloneBase, CXTPPaneToPaneMap* pMap,
...
BEGIN_DISPATCH_MAP(CXTPDockingPaneAutoHidePanel, CWnd)
	...
	DISP_FUNCTION_ID(CXTPDockingPaneAutoHidePanel, "Pane", DISPID_VALUE, OleGetPane, VT_DISPATCH,
					 VTS_I4)
	DISP_FUNCTION_ID(CXTPDockingPaneAutoHidePanel, "_NewEnum", DISPID_NEWENUM, OleNewEnum,
					 VT_UNKNOWN, VTS_NONE)
	DISP_PROPERTY_EX_ID(CXTPDockingPaneAutoHidePanel, "ParentContainer", 20, OleGetContainer,
						SetNotSupported, VT_DISPATCH)
	DISP_PROPERTY_EX_ID(CXTPDockingPaneAutoHidePanel, "Type", 21, OleGetType, SetNotSupported,
						VT_I4)
	DISP_PROPERTY_EX_ID(CXTPDockingPaneAutoHidePanel, "hWnd", DISPID_HWND, OleGetHwnd,
						SetNotSupported, VT_HANDLE)
	DISP_PROPERTY_EX_ID(CXTPDockingPaneAutoHidePanel, "IsEmpty", 22, OleIsEmpty, SetNotSupported,
						VT_BOOL)
	DISP_PROPERTY_EX_ID(CXTPDockingPaneAutoHidePanel, "Position", 23, OleGetPosition,
