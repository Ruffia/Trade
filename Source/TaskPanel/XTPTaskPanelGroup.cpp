...
CXTPTaskPanelGroupItem* CXTPTaskPanelGroup::AddLinkItem(UINT nID, int nImage)
{
	...
	CXTPTaskPanelGroupItem* pItem = (CXTPTaskPanelGroupItem*)
										m_pItems->Add(new CXTPTaskPanelGroupItem(), nID);
...
CXTPTaskPanelGroupItem* CXTPTaskPanelGroup::AddLinkItem(UINT nID, LPCTSTR lpCaption,
														LPCTSTR lpTooltip /*= NULL*/,
														int nImage /*= -1*/)
{
	CXTPTaskPanel::CRepositionContext context(m_pPanel);

	CXTPTaskPanelGroupItem* pItem = (CXTPTaskPanelGroupItem*)m_pItems->Add(
		new CXTPTaskPanelGroupItem(), nID, lpCaption, lpTooltip);
	pItem->SetIconIndex(nImage);
	pItem->SetType(xtpTaskItemTypeLink);
...
CXTPTaskPanelGroupItem* CXTPTaskPanelGroup::AddTextItem(LPCTSTR strText)
{
	...
	CXTPTaskPanelGroupItem* pItem = (CXTPTaskPanelGroupItem*)
										m_pItems->Add(new CXTPTaskPanelGroupItem(), 0);
...
{
	return (CXTPTaskPanelGroupItem*)m_pItems->Find(XTPToUInt(nID));
}
...
void CXTPTaskPanelGroup::OnCalcClientHeight(CDC* pDC, CRect rc)
{
	...
	// atsevtkov
	// Need to set right value for the left member and for the right member of m_rcGroupTarget and
	// m_rcGroupCurrent, like how in CXTPTaskPanelGroup::OnReposition
	m_rcGroupTarget		   = rc;
	...
	if (GetItemLayout() == xtpTaskItemLayoutImages)
	{
		...
		for (int i = 0; i < nItemCount; i++)
		{
			...
			CRect rcItem(CPoint(rcItems.left + x * szButton.cx, rcItems.top + y * szButton.cy),
						 szButton);
	...
	else
	{
		...
		for (int i = 0; i < nItemCount; i++)
		{
			...
			if (bMultiColumn)
			{
				...
				CRect rcItem = pItem->OnReposition(
					CRect(rcItems.left + x * nColumnWidth, rcItems.top,
						  rcItems.left + (x + 1) * nColumnWidth, rcItems.bottom));
...
void CXTPTaskPanelGroup::RepositionScrollOffset()
{
	...
	if (nScrollOffset > 0)
	{
		...
		while ((nScrollOffset > 0)
			   && ((rcItem.bottom - GetAt(nScrollOffset - 1)->GetItemRect().top + nTopMargin
					< m_nExpandedClientHeight)
				   || !GetAt(nScrollOffset - 1)->IsVisible()))
		{
			nScrollOffset--;
		}
	}
	...
	for (int i = 0; i < nItemCount; i++)
	{
		...
		if (pItem->IsItemFocused()
			|| (pItem->IsItemSelected() && GetTaskPanel()->m_bSelectItemOnFocus))
		{
...
void CXTPTaskPanelGroup::RepositionAutoHeightControls(int nHeightOffset)
{
	...
	for (i = 0; i < nItemCount; i++)
	{
		...
		if (pItem->IsAutoHeight() && pItem->IsVisible())
		{
			...
			int nItemHeightOffset = nCount > 1 ? (int)(nItemHeight * nHeightOffset / nTotalHeight)
											   : nHeightOffset;
...
CRect CXTPTaskPanelGroup::OnReposition(CRect rc, BOOL bRecalcOnly)
{
	...
	if (!m_pPanel->IsExplorerBehaviour() && (m_pPanel->GetActiveGroup() == this || IsExpanding()))
	{
		...
		int nButtonHeight	= __max(XTPSystemMetrics()->GetIconSize().cy,
									 GetItemIconSize().cy + rcMarginsInner.bottom);

		int nExpandedClientHeight = max(nButtonHeight + rcMarginsInner.top + XTP_DPI_X(2),
										nInsideHeight);
	...
	if (!bRecalcOnly || !m_pPanel->m_bAnimation)
		OnAnimate(0);
	else if (!IsDirty())
		OnAnimate(0);
...
#define MOVETO(A, B, Step)                                                                         \
	if (A != B)                                                                                    \
		A += max(1, abs(A - B) / Step) * (A > B ? -1 : 1);
...
CRect CXTPTaskPanelGroup::GetCaptionRect() const
{
	...
	if (m_pPanel->GetScrollButton(TRUE)->pGroupCaption == this)
		nCaptionRight = m_pPanel->GetScrollButton(TRUE)->rcButton.left
						- GetPaintManager()->GetGroupSpacing(m_pPanel) - 1;
	if (m_pPanel->GetScrollButton(FALSE)->pGroupCaption == this)
		nCaptionRight = m_pPanel->GetScrollButton(FALSE)->rcButton.left
						- GetPaintManager()->GetGroupSpacing(m_pPanel) - 1;

	return CRect(m_rcGroupCurrent.left, m_rcGroupCurrent.top, nCaptionRight,
				 m_rcGroupCurrent.top + m_nCaptionHeight);
}
...
CRect CXTPTaskPanelGroup::GetTargetCaptionRect() const
{
	...
	return CRect(m_rcGroupTarget.left, m_rcGroupTarget.top, nCaptionRight,
				 m_rcGroupTarget.top + m_nCaptionHeight);
}

CRect CXTPTaskPanelGroup::GetClientRect() const
{
	return CRect(m_rcGroupCurrent.left, m_rcGroupCurrent.top + m_nCaptionHeight,
				 m_rcGroupCurrent.right, m_rcGroupCurrent.bottom);
}

CRect CXTPTaskPanelGroup::GetTargetClientRect() const
{
	return CRect(m_rcGroupTarget.left, m_rcGroupTarget.top + m_nCaptionHeight,
				 m_rcGroupTarget.right, m_rcGroupTarget.bottom);
}
...
void CXTPTaskPanelGroup::SetClientBitmap(UINT nID, COLORREF clrTransparent,
										 int nBackgroundAlignemnt)
{
	...
	m_nBackgroundImage	 = XTPToInt(nID);
...
void CXTPTaskPanelGroup::OnFillClient(CDC* pDC, CRect rc)
{
	if (m_nBackgroundImage != -1)
	{
		CXTPImageManagerIcon* pIcon =
			m_pPanel->GetImageManager()->GetImage(XTPToUInt(m_nBackgroundImage), ICON_BIG);
...
CXTPTaskPanelGroupItem* CXTPTaskPanelGroup::HitTest(CPoint pt, CRect* lpRect) const
{
	...
	int nOffset = m_pPanel->GetScrollOffset() - m_nCaptionHeight - m_rcGroupCurrent.top
				  + GetScrollOffsetPos();
	pt.y += nOffset;
...
{
	CXTPTaskPanelGroupItem* pLastVisibleItem = (CXTPTaskPanelGroupItem*)
												   m_pItems->GetLastVisibleItem();
...
int CXTPTaskPanelGroup::GetScrollOffsetPos() const
{
	...
	{
		return GetAt(nScrollOffset)->GetItemRect().top
			   - GetPaintManager()->GetGroupInnerMargins(this).top;
	}
	return 0;
}
...
BOOL CXTPTaskPanelGroup::IsItemVisible(CXTPTaskPanelGroupItem* pItem, BOOL bAllowPart) const
{
	...
	CRect rcItem(pItem->GetItemRect());
	if ((bAllowPart ? rcItem.top : rcItem.bottom) - GetScrollOffsetPos()
		> m_rcGroupTarget.Height() - m_nCaptionHeight)
		return FALSE;
...
void CXTPTaskPanelGroup::EnsureVisible(CXTPTaskPanelGroupItem* pItem,
									   BOOL bUpdateScrollButtons /*= TRUE*/)
{
...
BEGIN_DISPATCH_MAP(CXTPTaskPanelGroup, CXTPTaskPanelItem)
	...
	DISP_PROPERTY_EX_ID(CXTPTaskPanelGroup, "Special", dispidSpecial, IsSpecialGroup,
						SetSpecialGroup, VT_BOOL)

	DISP_PROPERTY_EX_ID(CXTPTaskPanelGroup, "CaptionVisible", dispidCaptionVisible,
						IsCaptionVisible, ShowCaption, VT_BOOL)
	DISP_PROPERTY_EX_ID(CXTPTaskPanelGroup, "Expandable", dispidExpandable, IsExpandable,
						SetExpandable, VT_BOOL)
	DISP_PROPERTY_EX_ID(CXTPTaskPanelGroup, "Expanded", dispidExpanded, IsExpanded, SetExpanded,
						VT_BOOL)
	DISP_PROPERTY_NOTIFY_ID(CXTPTaskPanelGroup, "ItemLayout", 15, m_itemLayout, RepositionPanel,
							VT_I4)
	DISP_FUNCTION_ID(CXTPTaskPanelGroup, "SetIconSize", 104, OleSetIconSize, VT_EMPTY,
					 VTS_I4 VTS_I4)
	DISP_PROPERTY_ID(CXTPTaskPanelGroup, "MinimumClientHeight", 106, m_nMinClientHeight, VT_I4)

	DISP_FUNCTION_ID(CXTPTaskPanelGroup, "SetBackgroundImage", 108, OleSetBackgroundImage, VT_EMPTY,
					 VTS_I4 VTS_I4)
