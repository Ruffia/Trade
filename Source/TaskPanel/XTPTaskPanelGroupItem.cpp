...
IMPLEMENT_SERIAL(CXTPTaskPanelGroupItem, CXTPTaskPanelItem,
...
void CXTPTaskPanelGroupItem::OnDrawItem(CDC* pDC, CRect rc)
{
	if ((m_typeItem != xtpTaskItemTypeControl)
		|| (GetItemGroup()->GetItemLayout() == xtpTaskItemLayoutImages))
...
void CXTPTaskPanelGroupItem::GetPreviewBitmap(CWnd* pWnd, CBitmap& bmp)
{
...

	{
		...
		VERIFY(
			bitmapNonClient.CreateCompatibleBitmap(&dc, rectWindow.Width(), rectWindow.Height()));
...
void CXTPTaskPanelGroupItem::OnAnimate(int /*nStep*/)
{
	...
	rc.OffsetRect(m_rcItem.left - rc.left,
				  m_rcItem.top - m_pPanel->GetScrollOffset() - pGroup->GetScrollOffsetPos());

	CRect rcMargins		= GetPaintManager()->GetItemOuterMargins(this);
	CRect rcGroupMargin = GetPaintManager()->GetGroupInnerMargins(pGroup);

	int nWidth = rc.Width() - rcGroupMargin.left - rcGroupMargin.right - rcMargins.right
				 - rcMargins.left;

	if (m_szItem.cx != 0)
		nWidth = min(m_szItem.cx, nWidth);

	if (!IsVisible() || !pGroup->IsVisible()
		|| (pGroup->GetItemLayout() == xtpTaskItemLayoutImages))
	{
		...
		|| (pGroup->GetClientRect().Height() != pGroup->GetTargetClientRect().Height()))
	{
			...
			|| m_szPreview != m_rcItem.Size())
		{
			...
			{
				SetWindowPos(m_hWnd, 0, rc.left, -m_rcItem.Height(), nWidth, m_rcItem.Height(),
	...
	else
	{
		...
		{
			SetWindowPos(m_hWnd, 0, rc.left, rc.top, nWidth, m_rcItem.Height(),
						 SWP_NOZORDER | SWP_SHOWWINDOW | SWP_NOACTIVATE);
		}
...
{
	XTP_TASKPANEL_GROUPCOLORS* pColors = GetPaintManager()->GetGroupColors(
		GetItemGroup()->IsSpecialGroup());
...
void CXTPTaskPanelGroupItem::SetItemSelected(BOOL bSelected)
{
	...
	{
		if (bSelected)
			m_pPanel->SetFocusedItem(this, FALSE, FALSE);
		return;
	}
...
BOOL CXTPTaskPanelGroupItem::PrepareDrag(COleDataSource& srcItem, BOOL bCacheTextData)
{
	...
	if (!strText.IsEmpty())
	{
		HGLOBAL hGlobalBuff = ::GlobalAlloc(GMEM_MOVEABLE | GMEM_DDESHARE,
...
BEGIN_DISPATCH_MAP(CXTPTaskPanelGroupItem, CXTPTaskPanelItem)
	...
	DISP_FUNCTION_ID(CXTPTaskPanelGroupItem, "SetMargins", dispidSetMargins, OleSetMargins,
					 VT_EMPTY, VTS_I4 VTS_I4 VTS_I4 VTS_I4)
	DISP_PROPERTY_EX_ID(CXTPTaskPanelGroupItem, "Control", dispidControl, OleGetControl,
						OleSetControl, VT_DISPATCH)
	DISP_FUNCTION_ID(CXTPTaskPanelGroupItem, "BackColor", DISPID_BACKCOLOR, GetBackColor, VT_I4,
					 VTS_NONE)
	DISP_PROPERTY_EX_ID(CXTPTaskPanelGroupItem, "Selected", 13, IsItemSelected, SetItemSelected,
						VT_BOOL)
	DISP_PROPERTY_EX_ID(CXTPTaskPanelGroupItem, "Handle", 14, OleGetHandle, OleSetHandle, VT_HANDLE)
	DISP_PROPERTY_EX_ID(CXTPTaskPanelGroupItem, "DragText", 50, OlegGetDragText, SetDragText,
						VT_BSTR)
	DISP_FUNCTION_ID(CXTPTaskPanelGroupItem, "SetSize", 104, OleSetSize, VT_EMPTY,
					 VTS_I4 VTS_I4 VTS_BOOL)
	DISP_PROPERTY_NOTIFY_ID(CXTPTaskPanelGroupItem, "TextColor", DISPID_FORECOLOR, m_clrText,
							OleRedrawPanel, VT_COLOR)
	DISP_FUNCTION_ID(CXTPTaskPanelGroupItem, "SetHandle", 51, OleSetHandle, VT_EMPTY, VTS_HANDLE)
	DISP_FUNCTION_ID(CXTPTaskPanelGroupItem, "GetRect", 53, OleGetRect, VT_EMPTY,
