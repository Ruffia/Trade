...
const CRect CXTPTaskPanelPaintManager::rectDefault(CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT,
												   CW_USEDEFAULT);
...
CFont* CXTPTaskPanelPaintManager::GetItemFont(CXTPTaskPanelItem* pItem)
{
	...
	return (((CXTPTaskPanelGroupItem*)pItem)->IsItemHot()
			&& (pItem->GetTaskPanel()->GetHotTrackStyle() == xtpTaskPanelHighlightText))
			   ? (bBold ? &m_xtpFontIconHotBold : &m_xtpFontIconHot)
			   : (bBold ? &m_xtpFontIconBold : &m_xtpFontIcon);
}

CXTPTaskPanelPaintManager::~CXTPTaskPanelPaintManager()
{
	if (m_hGripperNormal)
		DestroyIcon(m_hGripperNormal);
	if (m_hGripperSpecial)
		DestroyIcon(m_hGripperSpecial);
	FreeBitmaps();
}
...
void CXTPTaskPanelPaintManager::DrawScrollArrow(CDC* pDC, CPoint pt0, CPoint pt1, CPoint pt2,
												BOOL bEnabled)
{
	if (bEnabled)
	{
...
void CXTPTaskPanelPaintManager::DrawScrollButton(CDC* pDC, CXTPTaskPanelScrollButton* pScrollButton)
{
	...
	if (m_bOfficeHighlight && !m_bCaptionScrollButton)
	{
		...
		COLORREF clrBrush  = pScrollButton->bPressed
								? (COLORREF)m_clrHighlightPressed
								: pScrollButton->bHot ? (COLORREF)m_clrHighlightHot
													  : (clrNormal == COLORREF_TRANSPARENT
	...
	if (!pScrollButton->bScrollUp)
	{
		DrawScrollArrow(pDC, CPoint(pt.x - XTP_DPI_X(5), pt.y - XTP_DPI_Y(2)),
						...
						CPoint(pt.x + XTP_DPI_X(3), pt.y - XTP_DPI_Y(2)), pScrollButton->bEnabled);
	}
	else
	{
		DrawScrollArrow(pDC, CPoint(pt.x - XTP_DPI_X(5), pt.y + XTP_DPI_Y(1)),
						CPoint(pt.x - XTP_DPI_X(1), pt.y - XTP_DPI_Y(3)),
						CPoint(pt.x + XTP_DPI_X(3), pt.y + XTP_DPI_Y(1)), pScrollButton->bEnabled);
	}
}

void CXTPTaskPanelPaintManager::CreateGripperIcon(HICON& hIcon, UINT nIDResource,
												  COLORREF clrGripperBack)
{
	if (hIcon)
	{
		DestroyIcon(hIcon);
...
void CXTPTaskPanelPaintManager::CreateGripperBitmaps()
{
	...
	if (systemTheme != xtpSystemThemeUnknown)
	{
		...
		if (SUCCEEDED(
				theme.GetCurrentThemeName(pszThemeFileName, MAX_PATH, NULL, MAX_PATH, NULL, 0))
		...
		CString strColorName = systemTheme == xtpSystemThemeOlive
								   ? _T("Homestead")
								   : systemTheme == xtpSystemThemeSilver ? _T("Metallic")
																		 : _T("NormalColor");
		...
		if (hLib)
		{
			...
			for (int i = 0; i < 8; i++)
			{
				m_bmpGrippers[i] = (HBITMAP)LoadImage(hLib, MAKEINTRESOURCE(100 + i), IMAGE_BITMAP,
	...
	{
		CreateGripperIcon(m_hGripperSpecial, XTP_IDB_TASKPANEL_GRIPPER_SPECIAL,
						  m_groupSpecial.clrHead.clrDark);
		CreateGripperIcon(m_hGripperNormal, XTP_IDB_TASKPANEL_GRIPPER,
						  m_groupNormal.clrHead.clrDark);
	}
}

void CXTPTaskPanelPaintManager::DrawCaptionGripperSigns(CDC* pDC, CPoint pt, COLORREF clr,
														BOOL bExpanded)
{
	const int nHalfSide = XTP_DPI_X(14) / 2;
	pt.y -= XTP_DPI_Y(1);
	CRect rc(pt.x - nHalfSide, pt.y - nHalfSide, pt.x + nHalfSide, pt.y + nHalfSide);
...
CRect CXTPTaskPanelPaintManager::DrawCaptionGripperBitmap(CDC* pDC, CXTPTaskPanelGroup* pGroup,
														  BOOL bExpanded, BOOL bHot, CRect rc)
{
	int nTop = max(rc.top, rc.CenterPoint().y - XTP_DPI_Y(10));
	CRect rcGripper(rc.right - XTP_DPI_X(3) - XTP_DPI_X(20), nTop, rc.right - XTP_DPI_X(3),
					nTop + XTP_DPI_Y(20));
	...
	if (m_bUseBitmapGrippers)
	{
		...
		if (XTPDrawHelpers()->m_pfnAlphaBlend)
		{
			...
			XTPDrawHelpers()->m_pfnAlphaBlend(pDC->GetSafeHdc(), rcGripper.left, rcGripper.top,
											  bmpInfo.bmWidth, bmpInfo.bmHeight, dcGripper, 0, 0,
	...
	else
	{
		...
		::DrawIconEx(*pDC, ptIcon.x, ptIcon.y, hIcon, XTP_DPI_X(19), XTP_DPI_Y(19), 0, NULL,
					 DI_NORMAL);

		XTP_TASKPANEL_GROUPCOLORS* pGroupColors = GetGroupColors(pGroup);

		DrawCaptionGripperSigns(pDC, rcGripper.CenterPoint(),
								bHot ? pGroupColors->clrHeadTextHot
									 : pGroupColors->clrHeadTextNormal,
								bExpanded);
	}
...
CRect CXTPTaskPanelPaintManager::DrawCaptionGripper(CDC* pDC, CXTPTaskPanelGroup* pGroup, CRect rc)
{
	...
	if (m_eGripper == xtpTaskPanelGripperPlain)
	{
		...
		if (pGroup->IsItemHot())
		{
			pDC->Draw3dRect(rcGripper, GetXtremeColor(COLOR_3DHIGHLIGHT),
							GetXtremeColor(COLOR_3DSHADOW));
		}

		DrawCaptionGripperSigns(pDC, rcGripper.CenterPoint(),
								pGroup->IsItemHot() ? pGroupColors->clrHeadTextHot
													: pGroupColors->clrHeadTextNormal,
								pGroup->IsExpanded());
	...
	if (m_eGripper == xtpTaskPanelGripperTriangle)
	{
		...
		CPoint pt(rc.right - XTP_DPI_X(6) - XTP_DPI_X(8) - XTP_DPI_X(5),
				  rc.CenterPoint().y + XTP_DPI_Y(2));

		const int nHalfSide = XTP_DPI_X(22) / 2;
		CRect rcArrow(pt.x - nHalfSide, pt.y - nHalfSide, pt.x + nHalfSide, pt.y + nHalfSide);
...
{
	return RGB(NORMCOLOR((GetRValue(clrFrom) * nAlpha + GetRValue(clrTo) * (255 - nAlpha)) / 255),
...
void CXTPTaskPanelPaintManager::SetItemsFontIndirect(LOGFONT* pLogFont,
													 BOOL bUseStandardItemsFont /*= FALSE*/)
{
...
void CXTPTaskPanelPaintManager::SetCaptionFontIndirect(LOGFONT* pLogFont,
													   BOOL bUseStandardCaptionFont /*= FALSE*/)
{
...
void CXTPTaskPanelPaintManager::RefreshMetrics()
{
	...
	m_groupSpecial.clrHead.SetStandardValue(GetXtremeColor(COLOR_ACTIVECAPTION),
		...
		if (m_bOfficeHighlight == XTP_TASKPANEL_HIGHLIGHT_OFFICE2003)
		{
			...
			switch (systemTheme)
			{
				...
				case xtpSystemThemeAero:
					m_clrHighlightBorder.SetStandardValue(RGB(0, 0, 128));
					break;
				case xtpSystemThemeOlive:
					m_clrHighlightBorder.SetStandardValue(RGB(63, 93, 56));
					break;
...
CRect CXTPTaskPanelPaintManager::MergeRect(CRect rc, CRect rcDefault)
{
	if (rc.left == CW_USEDEFAULT)
		rc.left = rcDefault.left;
	if (rc.top == CW_USEDEFAULT)
		rc.top = rcDefault.top;
	if (rc.right == CW_USEDEFAULT)
		rc.right = rcDefault.right;
	if (rc.bottom == CW_USEDEFAULT)
		rc.bottom = rcDefault.bottom;
...
{
	return MergeRect(const_cast<CXTPTaskPanelGroupItem*>(pItem)->GetMargins(),
					 m_rcItemOuterMargins);
}
...
{
	return MergeRect(const_cast<CXTPTaskPanelGroup*>(pGroup)->GetOuterMargins(),
					 m_rcGroupOuterMargins);
}

CRect CXTPTaskPanelPaintManager::GetGroupInnerMargins(const CXTPTaskPanelGroup* pGroup)
{
	return MergeRect(const_cast<CXTPTaskPanelGroup*>(pGroup)->GetInnerMargins(),
					 m_rcGroupInnerMargins);
}
...
void CXTPTaskPanelPaintManager::DrawGroupItemFrame(CDC* pDC, CXTPTaskPanelGroupItem* pItem,
												   CRect rc)
{
	if (m_bOfficeHighlight)
	{
		...
		else if (pItem->IsItemSelected() && pItem->IsItemHot() && !pItem->IsItemPressed())
...
void CXTPTaskPanelPaintManager::DrawDragArrow(CDC* pDC, CXTPTaskPanelItem* pItem, CRect rc,
											  BOOL bAfter)
{
	...
	else
	{
		...
		for (int i = nArrowLength; i >= 0;
			 --i, --y, nArrowHeight += 2) // DPI scaling don't need in this cycle
		{
...
CRect CXTPTaskPanelPaintManager::DrawGroupItem(CDC* pDC, CXTPTaskPanelGroupItem* pItem, CRect rc,
											   BOOL bDraw)
{
	XTPTaskPanelItemLayout itemLayout = pItem->GetItemGroup()->GetItemLayout();

	CRect rcItem = itemLayout == xtpTaskItemLayoutImagesWithTextBelow
					   ? DrawGroupItemImageWithTextBelow(pDC, pItem, rc, bDraw)
					   : itemLayout == xtpTaskItemLayoutImages
							 ? DrawGroupItemImage(pDC, pItem, rc, bDraw)
							 : DrawGroupItemImageWithText(pDC, pItem, rc, bDraw);
...
CRect CXTPTaskPanelPaintManager::DrawGroupItemImageWithText(CDC* pDC, CXTPTaskPanelGroupItem* pItem,
															CRect rc, BOOL bDraw)
{
	ASSERT_VALID(pItem);
	...
	if (bDraw)
	{
		...
		rc.DeflateRect(rcInnerMargins.left, rcInnerMargins.top, rcInnerMargins.right,
					   rcInnerMargins.bottom);
		...
		if (bDrawIcon)
		{
			if (hotTrackStyle == xtpTaskPanelHighlightImage)
			{
				CRect rcImage(rc.left - rcInnerMargins.left, rc.top - rcInnerMargins.top,
							  rc.left + szIcon.cx + XTP_DPI_X(2) + rcIconPadding.left
								  + rcIconPadding.right,
							  rc.top + szIcon.cy + XTP_DPI_Y(2) + rcIconPadding.top
	...
	if (rcText.Width() > 0)
	{
		CSize szText  = pItem->GetCaptionTextSize(pDC, rcText,
												  (static_cast<UINT>(m_nItemTextFormat) | DT_LEFT)
	...
	return CRect(rc.left, rc.top,
				 hotTrackStyle == xtpTaskPanelHighlightItem
					 ? rc.right
					 : min(rc.right, (rcText.right + rcInnerMargins.right)),
				 rc.top + max(nImageHeight, rcText.Height()) + rcInnerMargins.top
					 + rcInnerMargins.bottom);
}

CRect CXTPTaskPanelPaintManager::DrawGroupItemImageWithTextBelow(CDC* pDC,
																 ...
																 CRect rc, BOOL bDraw)
{
			...
			if (bDrawIcon)
			{
				...
				if (hotTrackStyle == xtpTaskPanelHighlightImage)
				{
					CRect rcImage(ptIcon.x - XTP_DPI_X(2), rc.top - XTP_DPI_Y(2),
								  ptIcon.x + szIcon.cx + XTP_DPI_X(2),
								  rc.top + szIcon.cy + XTP_DPI_Y(2));
					DrawGroupItemFrame(pDC, pItem, rcImage);
				}
	...
	else
	{
		...
		{
			CSize szText  = pItem->GetCaptionTextSize(pDC, rcText,
													  static_cast<UINT>(m_nItemTextFormat)
														  | DT_LEFT);
			rcText.bottom = rcText.top + szText.cy;
			rcText.right  = rcText.left + szText.cx;
		}
		int nHeight = rcText.Height() + (bDrawIcon ? (szIcon.cy + XTP_DPI_Y(3)) : 0);
		int nWidth  = max(bDrawIcon ? (szIcon.cx + XTP_DPI_X(4)) : 0, rcText.Width())
					 + rcInnerMargins.right + +rcInnerMargins.left;
		nWidth = min(nWidth, rc.Width());

		if (hotTrackStyle != xtpTaskPanelHighlightItem)
			return CRect(rc.CenterPoint().x - nWidth + nWidth / 2, rc.top,
						 rc.CenterPoint().x + nWidth / 2,
						 rc.top + nHeight + rcInnerMargins.top + rcInnerMargins.bottom);

		return CRect(rc.left, rc.top, rc.right,
					 rc.top + nHeight + rcInnerMargins.top + rcInnerMargins.bottom);
	}
}

CRect CXTPTaskPanelPaintManager::DrawGroupItemImage(CDC* pDC, CXTPTaskPanelGroupItem* pItem,
													CRect rc, BOOL bDraw)
{
	CXTPTaskPanelGroup* pGroup = pItem->GetItemGroup();
	ASSERT(pGroup);
	...
	if (bDraw)
	{
		CPoint ptIcon(rc.left + m_rcImageLayoutIconPadding.left,
					  rc.top + m_rcImageLayoutIconPadding.top);
	...
	else
	{
		...
		return CRect(rc.CenterPoint().x - nWidth / 2, rc.top,
					 rc.CenterPoint().x - nWidth / 2 + nWidth, rc.top + nHeight);
	}
}
...
void CXTPTaskPanelPaintManager::DrawGroupClient(CDC* pDC, CXTPTaskPanelGroup* pGroup)
{
	...
	{
		dc.BitBlt(rc.left, rc.top + nOffset, rc.Width(), rc.Height(), pDC, rc.left, rc.top,
				  SRCCOPY);
	}
	else
	{
	...
	{
		if (!XTPDrawHelpers()->m_pfnAlphaBlend
			|| (XTPDrawHelpers()->IsContextRTL(pDC) && XTPSystemVersion()->IsWin9x()))
		{
			pDC->BitBlt(rc.left, rc.top, rc.Width(), rcExpaded.Height() - nOffset, &dc, rc.left,
						nOffset, SRCCOPY);
		}
		else
		{
			BLENDFUNCTION bf = { AC_SRC_OVER, 0, (BYTE)nAlpha, 0 };
			XTPDrawHelpers()->m_pfnAlphaBlend(pDC->GetSafeHdc(), rc.left, rc.top, rc.Width(),
											  rc.Height(), dc, rc.left, nOffset, rc.Width(),
											  rcExpaded.Height() - nOffset, bf);
		}
	}
	else
	{
...
void CXTPTaskPanelPaintManager::DrawItemCaption(CDC* pDC, CXTPTaskPanelItem* pItem, CRect rcText,
												UINT nFormat)
{
	if (!pItem->IsGroup())
	{
		...
		if (clrNormal == COLORREF_NULL)
			clrNormal = pItem->GetType() == xtpTaskItemTypeLink ? pGroupColors->clrClientLink
																: pGroupColors->clrClientText;

		pDC->SetTextColor(
			!pItem->GetEnabled()
				? GetXtremeColor(COLOR_GRAYTEXT)
				: pItem->GetType() == xtpTaskItemTypeLink
					  ? (pItem->IsItemHot()
								 && (hotTrackStyle == xtpTaskPanelHighlightText
									 || pGroupColors->clrClientLinkHot.IsCustomValue())
							 ? pGroupColors->clrClientLinkHot.operator COLORREF()
							 : (hotTrackStyle == xtpTaskPanelHighlightItem)
									   && (((CXTPTaskPanelGroupItem*)pItem)->IsItemSelected()
										   || (pItem->IsItemHot()))
								   ? pGroupColors->clrClientLinkSelected.operator COLORREF()
								   : clrNormal)
					  : clrNormal);
	}
...
void CXTPTaskPanelPaintManager::DrawItemImage(CDC* pDC, CXTPTaskPanelItem* pItem, CPoint ptIcon,
											  CXTPImageManagerIcon* pImage, CSize szIcon)
{
	...
	XTPImageState imageState = (!pItem->GetEnabled()
									? xtpImageDisabled
									: bHighlight ? xtpImageHot
												 : bChecked ? xtpImageChecked : xtpImageNormal);
	pImage->Draw(pDC, ptIcon, imageState, szIcon);
}
	...
	if (bDraw)
	{
		...
		if (pGroup->IsCaptionVisible())
		{
			...
			pDC->SetTextColor(!pGroup->GetEnabled()
								  ? GetXtremeColor(COLOR_GRAYTEXT)
								  : pGroup->IsItemHot()
										? pGroupColors->clrHeadTextHot.operator COLORREF()
