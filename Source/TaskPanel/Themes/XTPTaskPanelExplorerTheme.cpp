...
void CXTPTaskPanelExplorerTheme::RefreshMetrics()
{
	...
	switch (systemTheme)
	{
		...
		default:
		{
			m_clrBackground.SetStandardValue(BlendColor(GetXtremeColor(COLOR_INACTIVECAPTION),
														GetXtremeColor(COLOR_WINDOW), 128),
											 BlendColor(GetXtremeColor(COLOR_ACTIVECAPTION),
														GetXtremeColor(COLOR_WINDOW), 192));

			COLORREF clrClient = BlendColor(GetXtremeColor(COLOR_3DFACE),
											GetXtremeColor(COLOR_WINDOW), 24);

			m_groupSpecial.clrHead.SetStandardValue(GetXtremeColor(COLOR_ACTIVECAPTION),
													BlendColor(GetXtremeColor(COLOR_ACTIVECAPTION),
															   GetXtremeColor(COLOR_WINDOW), 92));

			m_groupSpecial.clrHeadTextNormal.SetStandardValue(GetXtremeColor(COLOR_CAPTIONTEXT));
			m_groupSpecial.clrHeadTextHot.SetStandardValue(
				BlendColor(GetXtremeColor(COLOR_3DHIGHLIGHT), GetXtremeColor(COLOR_HIGHLIGHT),
						   128));
			m_groupSpecial.clrClient.SetStandardValue(clrClient);

			m_groupNormal.clrHead.SetStandardValue(GetXtremeColor(COLOR_WINDOW),
												   BlendColor(GetXtremeColor(COLOR_INACTIVECAPTION),
															  GetXtremeColor(COLOR_WINDOW), 92));

			m_groupNormal.clrHeadTextNormal.SetStandardValue(
				BlendColor(GetXtremeColor(COLOR_HIGHLIGHT), GetXtremeColor(COLOR_BTNTEXT), 192));
			m_groupNormal.clrHeadTextHot.SetStandardValue(
				BlendColor(GetXtremeColor(COLOR_HIGHLIGHT), GetXtremeColor(COLOR_BTNTEXT), 240));
			m_groupNormal.clrClient.SetStandardValue(
				BlendColor(clrClient, GetXtremeColor(COLOR_ACTIVECAPTION), 230));
