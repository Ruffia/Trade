// XTPTaskPanelVisualStudio2012Theme.cpp : implementation of the CXTPTaskPanelVisualStudio2012Theme
// class.
//
...
void CXTPTaskPanelVisualStudio2012Theme::FillGroupClientFace(CDC* pDC, CXTPTaskPanelGroup* pGroup,
															 CRect rc)
{
	Rectangle(pDC, rc, GetGroupColors(pGroup)->clrClient, GetGroupColors(pGroup)->clrClient);
}

void CXTPTaskPanelVisualStudio2012Theme::DrawGroupClientFace(CDC* pDC, CXTPTaskPanelGroup* pGroup,
															 CRect rc)
{
	DrawGroupClientItems(pDC, pGroup, rc);
}

void CXTPTaskPanelVisualStudio2012Theme::Triangle(CDC* pDC, POINT* pPoints, COLORREF clr,
												  BOOL bFilled)
{
	CXTPPenDC pen(*pDC, clr);

	HBRUSH hBrush = bFilled ? (::CreateSolidBrush(clr))
	...
	::DeleteObject(hBrush); // It is not necessary (but it is not harmful) to delete stock objects
							// by calling DeleteObject.
}

CXTPPaintManagerColor CXTPTaskPanelVisualStudio2012Theme::GetTriangleColorAndState(
	CXTPTaskPanelGroup* pGroup, BOOL& bFilled)
{
...
CRect CXTPTaskPanelVisualStudio2012Theme::DrawExpandButton(CDC* pDC, CXTPTaskPanelGroup* pGroup,
														   CRect rc)
{
	int nExpandButtonHalfWidth  = XTP_DPI_X(10);
	int nExpandButtonHalfHeight = XTP_DPI_Y(10);

	CRect rcButton(rc.left, rc.CenterPoint().y - nExpandButtonHalfHeight,
				   rc.left + nExpandButtonHalfWidth * 2,
				   rc.CenterPoint().y + nExpandButtonHalfHeight);
...
void CXTPTaskPanelVisualStudio2012Theme::DrawGroupItemFrame(CDC* pDC, CXTPTaskPanelGroupItem* pItem,
															CRect rc)
{
...
CRect CXTPTaskPanelVisualStudio2012Theme::DrawGroupCaptionBackground(CDC* pDC,
...
void CXTPTaskPanelVisualStudio2012Theme::DrawItemCaption(CDC* pDC, CXTPTaskPanelItem* pItem,
														 CRect rcText, UINT nFormat)
{
	if ((pItem->IsGroup() && pItem->IsItemFocused())
		|| (!pItem->IsGroup() && static_cast<CXTPTaskPanelGroupItem*>(pItem)->IsItemSelected()))
	{
