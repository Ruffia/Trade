...
void CAppearanceSetVisio::RepositionTabControl(CXTPTabManager* pTabManager, CDC* /*pDC*/,
											   CRect rcClient)
{
	CXTPTabPaintManagerTheme::DeflateRectEx(rcClient, m_pPaintManager->m_rcControlMargin,
	...
	for (int i = 0; i < pTabManager->GetItemCount(); i++)
	{
		...
		if (pItem->IsSelected())
		{
			int nBottom = max(rcItem.top + nButtonHeight,
							  rc.bottom - (pTabManager->GetItemCount() - i - 1) * nButtonHeight);
...
CRect CAppearanceSetVisio::GetClientRect(CRect rcControl, CXTPTabManager* pTabManager)
{
	...
	if (pItem)
	{
		...
		if (pItem->GetIndex() == pTabManager->GetItemCount() - 1
			&& m_pPaintManager->m_clientFrame != xtpTabFrameNone)
			rcControl.bottom--;
	}
