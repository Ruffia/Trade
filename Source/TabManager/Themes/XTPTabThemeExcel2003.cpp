...
void CXTPTabThemeExcel2003::DrawSingleButton(CDC* pDC, CXTPTabManagerItem* pItem)
{
	...
	CXTPTabPaintManagerColorSet::COLORSET_TAB_EXCEL& cs = pItem->IsSelected()
															  ? GetColorSet()->m_csExcelSelected
															  : GetColorSet()->m_csExcelNormal;
	...
	switch (pItem->GetTabManager()->GetPosition())
	{
		case xtpTabPositionTop:
		{
			...
			POINT ptsFill[]  = { { rc.left, rc.bottom },
								 { nMargin, -nMargin * 2 },
								 { rc.Width() - nMargin, 0 },
								 { nMargin, nMargin * 2 } };
			COLORREF clrDark = FillButton(pDC, pItem, ptsFill, _countof(ptsFill),
										  pItem->GetTabManager()->GetPosition(), FALSE);

			LineEx(pDC, cs.clrLeftOuterBorder, rc.left + 1, rc.bottom - 1, rc.left + nMargin + 1,
				   rc.top);
			LineEx(pDC, cs.clrLeftOuterBorder, rc.left + nMargin, rc.top + 1, rc.right, rc.top + 1);
			LineEx(pDC, cs.clrRightOuterBorder, rc.right - 1, rc.top, rc.right + nMargin - 1,
				   rc.bottom);
		...
		case xtpTabPositionLeft:
		{
			...
			LineEx(pDC, cs.clrLeftOuterBorder, rc.right - 1, rc.top + 1, rc.left,
				   rc.top + nMargin + 1);
			LineEx(pDC, cs.clrLeftOuterBorder, rc.left + 1, rc.top + nMargin, rc.left + 1,
				   rc.bottom);
			LineEx(pDC, cs.clrRightOuterBorder, rc.left, rc.bottom - 1, rc.right,
				   rc.bottom + nMargin - 1);
		...
		case xtpTabPositionBottom:
		{
			...
			POINT ptsFill[]  = { { rc.left, rc.top + 1 },
								 { nMargin, nMargin * 2 },
								 { rc.Width() - nMargin, 0 },
								 { nMargin, -nMargin * 2 } };
			COLORREF clrDark = FillButton(pDC, pItem, ptsFill, _countof(ptsFill),
			...
			LineEx(pDC, cs.clrLeftOuterBorder, rc.left + 1, rc.top + 1, rc.left + nMargin + 1,
				   rc.bottom);
			LineEx(pDC, cs.clrTopOuterBorder, rc.left + nMargin, rc.bottom - 1, rc.right,
				   rc.bottom - 1);
			LineEx(pDC, cs.clrRightOuterBorder, rc.right - 1, rc.bottom, rc.right + nMargin - 1,
				   rc.top);
		...
		case xtpTabPositionRight:
		{
			...
			POINT ptsFill[]  = { { rc.left + 1, rc.top },
								 { nMargin * 2, nMargin },
								 { 0, rc.Height() - nMargin },
								 { -nMargin * 2, nMargin } };
			COLORREF clrDark = FillButton(pDC, pItem, ptsFill, _countof(ptsFill),
			...
			LineEx(pDC, cs.clrLeftOuterBorder, rc.left + 1, rc.top + 1, rc.right,
				   rc.top + nMargin + 1);
			LineEx(pDC, cs.clrLeftOuterBorder, rc.right - 1, rc.top + nMargin, rc.right - 1,
				   rc.bottom);
			LineEx(pDC, cs.clrRightOuterBorder, rc.right, rc.bottom - 1, rc.left,
				   rc.bottom + nMargin - 1);
