...
void CAppearanceSetPropertyPageFlat::BlurTab(CDC* pDC, CRect rcTab, COLORREF clrBackground,
											 COLORREF clrBorder, XTPTabPosition tabPosition)
{
	switch (tabPosition)
	{
...
void CAppearanceSetPropertyPageFlat::DrawSingleButton(CDC* pDC, CXTPTabManagerItem* pItem)
{
	...
	if (m_bVisualStudio2005Style)
	{
		if (pItem->IsSelected())
		{
			...
			if (!pItem->IsSelected())
				DrawSingleLineBorder(pDC, rcItem, XTPTabPosition((tabPosition + 2) % 4),
									 clrFrameBorder, clrFrameBorder);
		...
		else
		{
			if (pItem->GetIndex() < pItem->GetTabManager()->GetItemCount() - 1)
			{
				if (IsHorizontalTabPosition(tabPosition))
					pDC->FillSolidRect(rcItem.right - 1, rcItem.top + 3, 1, rcItem.Height() - 6,
									   clrFrameBorder);
				else
					pDC->FillSolidRect(rcItem.left + 3, rcItem.bottom - 1, rcItem.Width() - 6, 1,
									   clrFrameBorder);
			}
		}
	}
