...
CRect CXTPTabThemeOffice2000::GetHeaderClipBox(CDC* pDC, CXTPTabManager* pTabManager)
{
	...
	if (pTabManager->GetPosition() == xtpTabPositionBottom
		|| pTabManager->GetPosition() == xtpTabPositionRight)
...
CRect CXTPTabThemeOffice2000::FillTabControl(CXTPTabManager* pTabManager, CDC* pDC, CRect rcControl)
{
	...
	if (m_pPaintManager->m_clientFrame == xtpTabFrameBorder)
	{
		...
		Draw3dRect(pDC, rcClient, GetColorSet()->m_csPropertyPage.clrHighlight,
				   GetColorSet()->m_csPropertyPage.clrDarkShadow);

		HorizontalLine(pDC, rcClient.left + 1, rcClient.bottom - 2, rcClient.Width() - 2,
					   GetColorSet()->m_csPropertyPage.clrShadow);
		VerticalLine(pDC, rcClient.right - 2, rcClient.top + 1, rcClient.Height() - 2,
					 GetColorSet()->m_csPropertyPage.clrShadow);
	}
	if (m_pPaintManager->m_clientFrame == xtpTabFrameSingleLine)
	{
		switch (pTabManager->GetPosition())
		{
			case xtpTabPositionTop:
				HorizontalLine(pDC, rcClient.left, rcClient.top, rcClient.Width(),
							   GetColorSet()->m_csPropertyPage.clrHighlight);
				break;
			case xtpTabPositionLeft:
				VerticalLine(pDC, rcClient.left, rcClient.top, rcClient.Height(),
							 GetColorSet()->m_csPropertyPage.clrHighlight);
				break;
			case xtpTabPositionBottom:
				HorizontalLine(pDC, rcClient.left, rcClient.bottom - 1, rcClient.Width(),
							   GetColorSet()->m_csPropertyPage.clrDarkShadow);
				HorizontalLine(pDC, rcClient.left, rcClient.bottom - 2, rcClient.Width(),
							   GetColorSet()->m_csPropertyPage.clrShadow);
				break;
			case xtpTabPositionRight:
				VerticalLine(pDC, rcClient.right - 1, rcClient.top, rcClient.Height(),
							 GetColorSet()->m_csPropertyPage.clrDarkShadow);
				VerticalLine(pDC, rcClient.right - 2, rcClient.top, rcClient.Height(),
							 GetColorSet()->m_csPropertyPage.clrShadow);
				break;
		}
	}
...
void CXTPTabThemeOffice2000::DrawSingleButton(CDC* pDC, CXTPTabManagerItem* pItem)
{
	...
	COLORREF clrShadow	 = GetColorSet()->m_csPropertyPage.clrShadow,
			 clrHighlight  = GetColorSet()->m_csPropertyPage.clrHighlight,
			 clrDarkShadow = GetColorSet()->m_csPropertyPage.clrDarkShadow;
	...
	switch (pItem->GetTabManager()->GetPosition())
	{
		case xtpTabPositionTop:
		{
			...
			COLORREF clr = GetColorSet()->FillPropertyButton(
				pDC, CRect(rc.left + 1, rc.top + 1, rc.right - 1, rc.bottom - 1), pItem);
