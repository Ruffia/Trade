...
{
	if (!CWnd::Create(AfxRegisterWndClass(CS_DBLCLKS, AfxGetApp()->LoadStandardCursor(IDC_ARROW)),
...
CXTPTabManagerItem* CXTPTabControl::InsertItem(int nItem, LPCTSTR lpszItem,
											   HWND hwndChild /*= NULL*/, int nImage /*= -1*/,
											   CXTPTabManagerItem* pAddItem /* = NULL */)
{
...
void CXTPTabControl::SetSelectedItem(CXTPTabManagerItem* pItem)
{
	...
	nmhdr.idFrom   = XTPToUIntPtr(GetDlgCtrlID());
...
{
	HBRUSH hBrush = (HBRUSH)GetParent()->SendMessage(WM_CTLCOLORSTATIC, (WPARAM)pDC->GetSafeHdc(),
													 (LPARAM)m_hWnd);
	if (hBrush)
	{
...
void CXTPTabControl::OnNavigateButtonClick(CXTPTabManagerNavigateButton* pButton)
{
	...
	if (pButton->GetID() == xtpTabNavigateButtonLeft
		|| pButton->GetID() == xtpTabNavigateButtonRight)
	{
		//
	}
}
...
BOOL CXTPTabControl::DrawIcon(CDC* pDC, CPoint pt, CXTPTabManagerItem* pItem, BOOL bDraw,
							  CSize& szIcon) const
{
	if (!pItem)
	...
	CXTPImageManagerIcon* pImage = m_pImageManager->GetImage(XTPToUInt(pItem->GetImageIndex()),
															 szIcon.cx);
...
void CXTPTabControl::SetFocusedItem(CXTPTabManagerItem* pItem)
{
	...
	{
		::SendMessage(::GetParent(m_hWnd), WM_CHANGEUISTATE, MAKEWPARAM(UIS_CLEAR, UISF_HIDEFOCUS),
					  0);
	}
}
