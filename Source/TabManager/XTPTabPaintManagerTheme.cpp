...
void CXTPTabPaintManagerTheme::DrawText(CDC* pDC, CXTPTabManager* /*pManager*/, const CString& str,
...
int CXTPTabPaintManagerTheme::GetButtonHeight(const CXTPTabManager* pTabManager)
{
	...
	if (pTabManager)
		pTabManager->GetItemMetrics(NULL, &szButtonMin, NULL);
	...
	if (pTabManager)
		pTabManager->DrawIcon(NULL, 0, NULL, FALSE, szIcon);
...
void CXTPTabPaintManagerTheme::InflateRectEx(CRect& rc, CRect rcInflate, XTPTabPosition position)
{
	switch (position)
	{
		case xtpTabPositionTop:
			rc.top -= rcInflate.top;
			rc.left -= rcInflate.left;
			rc.bottom += rcInflate.bottom;
			rc.right += rcInflate.right;
			break;
		case xtpTabPositionLeft:
			rc.left -= rcInflate.top;
			rc.bottom += rcInflate.right;
			rc.right += rcInflate.bottom;
			rc.top -= rcInflate.left;
			break;
		case xtpTabPositionBottom:
			rc.bottom += rcInflate.top;
			rc.right += rcInflate.right;
			rc.top -= rcInflate.bottom;
			rc.left -= rcInflate.left;
			break;
		case xtpTabPositionRight:
			rc.right += rcInflate.top;
			rc.top -= rcInflate.left;
			rc.left -= rcInflate.bottom;
			rc.bottom += rcInflate.right;
			break;
	}
}
...
{
	InflateRectEx(rc, CRect(-rcDeflate.left, -rcDeflate.top, -rcDeflate.right, -rcDeflate.bottom),
				  position);
}

void CXTPTabPaintManagerTheme::DrawSingleLineBorder(CDC* pDC, CRect rc, XTPTabPosition position,
...
COLORREF CXTPTabPaintManagerTheme::FillButton(CDC* pDC, CXTPTabManagerItem* pItem, LPPOINT pts,
											  int nCount, XTPTabPosition /*tabPosition*/,
											  BOOL bAbsolute)
{
	LPPOINT lpPointRgn = new POINT[XTPToUIntChecked(nCount)];
	CPoint pt(0);
	...
	COLORREF clrFill = GetColorSet()->FillPropertyButton(
		&dcButton, CRect(0, 0, rcFill.Width(), rcFill.Height()), pItem);
...
void CXTPTabPaintManagerTheme::DrawWorkspacePart(CDC* pDC, LPRECT lpRect,
												 XTPTabWorkspacePart workspacePart)
{
	if (!m_pPaintManager->m_bStaticFrame)
	{
		...
		{
			case xtpTabWorkspacePartBorder: break;
		...
		switch (workspacePart)
		{
			case xtpTabWorkspacePartBorder:
				pDC->Draw3dRect(XTP_DPI_X(1), 0, lpRect->right - lpRect->left - XTP_DPI_X(2),
								lpRect->bottom - lpRect->top - XTP_DPI_Y(1),
								GetXtremeColor(COLOR_3DFACE), GetXtremeColor(COLOR_3DFACE));
				pDC->Draw3dRect(0, 0, lpRect->right - lpRect->left, lpRect->bottom - lpRect->top,
								GetColorSet()->m_clrFrameBorder, GetColorSet()->m_clrFrameBorder);
				break;

			case xtpTabWorkspacePartWidth: lpRect->top -= XTP_DPI_Y(1); break;

			case xtpTabWorkspacePartVSplitter:
				pDC->FillSolidRect(lpRect, GetColorSet()->m_clrControlFace);
				pDC->FillSolidRect(lpRect->right - XTP_DPI_X(1), lpRect->top, XTP_DPI_X(1),
								   lpRect->bottom - lpRect->top, GetColorSet()->m_clrFrameBorder);
				break;

			case xtpTabWorkspacePartHSplitter:
				pDC->FillSolidRect(lpRect, GetColorSet()->m_clrControlFace);
				pDC->FillSolidRect(lpRect->left, lpRect->bottom - XTP_DPI_Y(1),
								   lpRect->right - lpRect->left, XTP_DPI_Y(1),
...
CRect CXTPTabPaintManagerTheme::FillTabControl(CXTPTabManager* /*pTabManager*/, CDC* /*pDC*/,
											   CRect /*rcControl*/)
{
...
void CXTPTabPaintManagerTheme::DrawTabControl(CXTPTabManager* pTabManager, CDC* pDC,
											  CRect rcControl)
{
	m_pPaintManager->DrawTabControlEx(pTabManager, pDC, rcControl);
}

void CXTPTabPaintManagerTheme::RepositionTabControl(CXTPTabManager* pTabManager, CDC* pDC,
													CRect rcClient)
{
