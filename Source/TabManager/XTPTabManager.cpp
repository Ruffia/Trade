...
CXTPTabManager::ROW_ITEMS* CXTPTabManager::CRowIndexer::CreateIndexer(int nRowCount)
{
	...
	{
		SAFE_DELETE_AR(m_pRowItems);
		m_pRowItems = new ROW_ITEMS[XTPToUIntChecked(nRowCount)];
...
{
protected:
	CNavigateButtonArrow(CXTPTabManager* pManager, XTPTabNavigateButton nID,
...
void CXTPTabManager::CNavigateButtonClose::DrawEntry(CDC* pDC, CRect rc, COLORREF clr,
													 BOOL bEnabled)
{
	XTPPrimitiveDrawer()->DrawSymbol(pDC, xtpPrimitiveSymbolTabClose, rc, clr, COLORREF_NULL,
									 !bEnabled);
}

void CXTPTabManager::CNavigateButtonArrowRight::DrawEntry(CDC* pDC, CRect rc, COLORREF clr,
														  BOOL bEnabled)
{
	ASSERT(NULL != pDC);
	...
	if (m_pManager->IsHorizontalPosition())
	{
		...
		if (bEnabled)
			XTPPrimitiveDrawer()->DrawSymbol(pDC,
											 !bRTL ? xtpPrimitiveSymbolTabPlayRight
												   : xtpPrimitiveSymbolTabPlayLeft,
											 rc, clr, COLORREF_NULL);
		else
			XTPPrimitiveDrawer()->DrawSymbol(pDC,
											 !bRTL ? xtpPrimitiveSymbolTabPlayRightDisabled
												   : xtpPrimitiveSymbolTabPlayLeftDisabled,
											 rc, clr, COLORREF_NULL);
	}
	else
	{
		XTPPrimitiveDrawer()->DrawSymbol(pDC, xtpPrimitiveSymbolPlayDown, rc, clr, COLORREF_NULL,
										 !bEnabled);
	}
}
...
void CXTPTabManager::CNavigateButtonArrow::Reposition(CRect& rcNavigateButtons)
{
	...
	if (m_dwFlags == xtpTabNavigateButtonAutomatic)
	{
		...
		if (!(m_pManager->GetItemsLength() + m_pManager->GetHeaderOffset() - rc.left - 1
				  > m_pManager->GetRectLength(rcNavigateButtons) - (rc.left + rc.right)
...
{
	XTPResourceManager()->LoadString(&m_strToolTip, UINT(m_pManager->IsHorizontalPosition()
															 ? XTP_IDS_TABNAVIGATEBUTTON_RIGHT
															 : XTP_IDS_TABNAVIGATEBUTTON_DOWN));
	...
	m_bEnabled = m_pManager->GetItemsLength() + m_pManager->GetHeaderOffset()
				 > m_pManager->GetRectLength(rcNavigateButtons)
					   - (rcHeaderMargin.left + rcHeaderMargin.right) - 28;
...
void CXTPTabManager::CNavigateButtonArrowRight::OnExecute(BOOL bTick)
{
	...
	CNavigateButtonArrow::OnExecute(bTick);
}
...
void CXTPTabManager::CNavigateButtonArrowLeft::DrawEntry(CDC* pDC, CRect rc, COLORREF clr,
														 BOOL bEnabled)
{
	ASSERT(NULL != pDC);
	...
	if (m_pManager->IsHorizontalPosition())
	{
		...
		if (bEnabled)
			XTPPrimitiveDrawer()->DrawSymbol(pDC,
											 !bRTL ? xtpPrimitiveSymbolTabPlayLeft
												   : xtpPrimitiveSymbolTabPlayRight,
											 rc, clr, COLORREF_NULL);
		else
			XTPPrimitiveDrawer()->DrawSymbol(pDC,
											 !bRTL ? xtpPrimitiveSymbolTabPlayLeftDisabled
												   : xtpPrimitiveSymbolTabPlayRightDisabled,
											 rc, clr, COLORREF_NULL);
	}
	else
	{
		XTPPrimitiveDrawer()->DrawSymbol(pDC, xtpPrimitiveSymbolPlayUp, rc, clr, COLORREF_NULL,
										 !bEnabled);
	}
}
...
{
	XTPResourceManager()->LoadString(&m_strToolTip, UINT(m_pManager->IsHorizontalPosition()
															 ? XTP_IDS_TABNAVIGATEBUTTON_LEFT
															 : XTP_IDS_TABNAVIGATEBUTTON_UP));
...
void CXTPTabManager::EnableTabThemeTexture(HWND hWnd, BOOL bEnable)
{
	...
	themeTabControl.EnableThemeDialogTexture(hWnd,
											 DWORD(bEnable ? ETDT_ENABLETAB
														   : ETDT_DISABLE | ETDT_USETABTEXTURE));
}
...
int CXTPTabManager::GetItemsLength() const
{
	...
	{
		nLength = GetPaintManager()->GetAppearanceSet()->GetButtonHeight(this) * GetItemCount();
	}
	else
	{
...
CXTPTabManagerItem* CXTPTabManager::AddItem(int nItem, CXTPTabManagerItem* pItem /*= NULL*/)
{
	...
	pItem->m_clrItem = static_cast<COLORREF>(xtpTabColorBlue
											 + (GetPaintManager()->m_nItemColor++ % 8));
			...
			case WM_MOUSEMOVE:
			{
				...
				for (int i = 0; i < arrRects.GetSize(); i++)
				{
					if (i != pItem->GetIndex() && arrRects[i].PtInRect(pt))
					{
						...
						{
							int j = i < pItem->GetIndex() ? pItem->GetIndex() - 1
														  : pItem->GetIndex() + 1;
...
BOOL CXTPTabManager::PerformKeyDown(HWND hWnd, UINT nChar)
{
	...
	switch (nChar)
	{
		case VK_HOME: SetFocusedItem(FindNextFocusable(-1, +1)); return TRUE;

		case VK_END: SetFocusedItem(FindNextFocusable(nCount, -1)); return TRUE;
...
CXTPTabManagerNavigateButton* CXTPTabManager::HitTestNavigateButton(CPoint point, BOOL bHeaderOnly,
																	int* pnIndex) const
{
	int i;
...
INT_PTR CXTPTabManager::PerformToolHitTest(HWND hWnd, CPoint point, TOOLINFO* pTI) const
{
	...
	if (pItem)
	{
		...
		CXTPToolTipContext::FillInToolInfo(pTI, hWnd, pItem->GetRect(), pItem->GetIndex(), strTip,
										   pItem->GetCaption(), strTip);
...
void CXTPTabManager::GetItemMetrics(CSize* lpszNormal, CSize* lpszMin /*= NULL*/,
									CSize* lpszMax /*= NULL*/) const
{
	CXTPTabPaintManager* pPaintManager = GetPaintManager();
	if (lpszNormal)
		*lpszNormal = CSize(pPaintManager->m_nFixedTabWidth, 0);
	if (lpszMin)
		*lpszMin = CSize(pPaintManager->m_nMinTabWidth, 0);
	if (lpszMax)
		*lpszMax = CSize(pPaintManager->m_nMaxTabWidth, 0);
}

void CXTPTabManager::SetItemMetrics(CSize szNormal, CSize szMin /*= CSize(0, 0)*/,
									CSize szMax /*= CSize(0, 0)*/)
{
	CXTPTabPaintManager* pPaintManager = GetPaintManager();
...
BEGIN_DISPATCH_MAP(CXTPTabManagerItem, CXTPCmdTarget)
	...
	DISP_FUNCTION_ID(CXTPTabManagerItem, "GetRect", 13, OleGetRect, VT_EMPTY,
...
void CXTPTabManagerItem::OleSetHandle(long hwndChild)
{
	...
	m_hWnd = reinterpret_cast<HWND>((LONG_PTR)hwndChild);
...
void CXTPTabManagerItem::OleGetRect(long* Left, long* Top, long* Right, long* Bottom)
{
	...
	if (Top)
		*Top = m_rcItem.top;
	if (Right)
		*Right = m_rcItem.right;
	if (Bottom)
		*Bottom = m_rcItem.bottom;
}
