...
int CXTPChartTitle::GetTextAngle() const
{
	...
	switch (m_nDocking)
	{
		...
		case xtpChartDockBottom: nAngle = 0; break;
		case xtpChartDockRight: nAngle = 90; break;
		case xtpChartDockLeft: nAngle = 270; break;
	}
...
CPoint CXTPChartTitle::CalcOrigin(CRect bounds, CRect startBounds, CRect rect)
{
	...
	{
		int y = m_nDocking == xtpChartDockTop ? bounds.top + m_nInnerIndent
											  : bounds.bottom - m_nInnerIndent;
...
{
	if (m_nDocking == xtpChartDockLeft)
		return xtpChartTextNearRight;
	else if (m_nDocking == xtpChartDockRight)
		return xtpChartTextNearLeft;
	else if (m_nDocking == xtpChartDockTop)
		return xtpChartTextNearBottom;
	return xtpChartTextNearTop;
}

CXTPChartElementView* CXTPChartTitle::CreateView(CXTPChartDeviceContext* /*pDC*/,
...
void CXTPChartTitleCollection::CreateView(CXTPChartDeviceContext* pDC,
										  CXTPChartElementView* pParentView)
{
	for (int i = 0; i < GetCount(); i++)
	{
...
void CXTPChartTitleCollection::CalculateView(CXTPChartDeviceContext* pDC, CRect& rcChart,
											 CXTPChartElementView* pParentView)
{
	CRect rc(rcChart);
...
BEGIN_DISPATCH_MAP(CXTPChartTitle, CXTPChartElement)
	...
	DISP_PROPERTY_EX_ID(CXTPChartTitle, "Antialiasing", 4, GetAntialiasing, SetAntialiasing,
						VT_BOOL)
	DISP_PROPERTY_EX_ID(CXTPChartTitle, "Font", DISPID_FONT, OleGetFont, SetNotSupported,
						VT_DISPATCH)
...
BEGIN_DISPATCH_MAP(CXTPChartTitleCollection, CXTPChartElement)
	...
	DISP_FUNCTION_ID(CXTPChartTitleCollection, "Item", DISPID_VALUE, OleGetItem, VT_DISPATCH,
					 VTS_I4)
	DISP_FUNCTION_ID(CXTPChartTitleCollection, "_NewEnum", DISPID_NEWENUM, OleNewEnum, VT_UNKNOWN,
					 VTS_NONE)
