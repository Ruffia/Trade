...
public:
	...
	//     an argument and a pair of values.
	// Parameters:
	...
	CXTPChartSeriesPoint(double dArgument, double dValue1, double dValue2, double dValue3,
						 double dValue4); // <combine
										  // CXTPChartSeriesPoint::CXTPChartSeriesPoint@const
										  // CXTPChartString&@double@double@double@double>
...
public:
	//-------------------------------------------------------------------------
	// Summary:
	//     Gets/sets the value at a specified index.
	// Parameters:
	...
	void SetValue(int nIndex, double dValue); // <combine CXTPChartSeriesPoint::GetValue@int@const>
	...
	//-------------------------------------------------------------------------
	// Summary:
	//     Gets/sets the double value of the argument of a point.
	// Parameters:
	//     dArgument - Double value to be set.
	// Returns:
	//     A double value argument of the point.
	//-------------------------------------------------------------------------
	double GetArgumentValue() const;
	void SetArgumentValue(double dArgument); // <combine
											 // CXTPChartSeriesPoint::GetArgumentValue@const>

	//-------------------------------------------------------------------------
	// Summary:
	//     Gets/sets the double value of the string argument of a point.
	// Parameters:
	//     dInternalArgument - Double value to be set.
	// Returns:
	//     A double value corresponding to the string argument of the point.
	//-------------------------------------------------------------------------
	double GetInternalArgumentValue() const;
	void SetInternalArgumentValue(
		double dInternalArgument); // <combine CXTPChartSeriesPoint::GetInternalArgumentValue@const>
	...
	//-----------------------------------------------------------------------
	// Summary:
	//      Sets new point legend text.
	// Parameters:
	//      lpszLegendText - New legend text pointer.
	// See Also:
	// 	   GetLegendText
	//-----------------------------------------------------------------------
	...
	//{{AFX_CODEJOCK_PRIVATE
	_XTP_DEPRECATED_IN_FAVOR(SetLegendText)
	void SetLegentText(LPCTSTR lpszLegendText)
	{
		SetLegendText(lpszLegendText);
	}
	//}}AFX_CODEJOCK_PRIVATE

	//-----------------------------------------------------------------------
	// Summary:
	//      Obtains current point legend text.
	// Returns:
	//      Current point legent text.
	// See also:
	//      SetLegendText
	//-----------------------------------------------------------------------
	...
	//-----------------------------------------------------------------------
	// Summary:
	//     Gets/sets the current value length in characters.
	// Parameters:
	// 	   nLength - New length of the value.
	// Returns:
	//     Number of characters of the current value.
	//-----------------------------------------------------------------------
	int GetValueLength() const;
	void SetValueLength(int nLength); // <combine CXTPChartSeriesPoint::GetValueLength@const>
	...
	//-----------------------------------------------------------------------
	// Summary:
	//      Gets/sets point label text.
	// Parameters:
	//      lpszLabelText - Label text pointer.
	// Returns:
	//      Current label text string.
	//-----------------------------------------------------------------------
	void SetLabelText(LPCTSTR lpszLabelText);
	CXTPChartString GetLabelText() const; // <combine CXTPChartSeriesPoint::SetLabelText@LPCTSTR>
...
public:
	//-------------------------------------------------------------------------
	// Summary:
	//     This function adds a point represented by the class CXTPChartSeriesPoint
	//     to the internal array of points.
	// Parameters:
	//     pPoint - The pointer to a CXTPChartSeriesPoint object.
	// Returns:
	//     The pointer to the CXTPChartSeriesPoint object, which is added to the object array.
	//-------------------------------------------------------------------------
	...
	// Returns:
	//     The pointer to the CXTPChartSeriesPoint object, which stays at the index nIndex.
	//-------------------------------------------------------------------------
	virtual CXTPChartSeriesPoint* GetAt(int nIndex) const;

	//-------------------------------------------------------------------------
	// Summary:
	//     Call this method to add a batch of points.
	// Parameters:
	//     nCount - Number of points to add.
	// Remarks:
	//     This method is an optimized way to add a large number of points;
	//     memory will be allocated once for all of them.
	//-------------------------------------------------------------------------
...
AFX_INLINE void CXTPChartSeriesPoint::SetLegendText(LPCTSTR lpszLegendText)
{
