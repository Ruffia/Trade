	...
	else
	{
		...
		if (nIndex >= 0 && nIndex < pContent->GetPanels()->GetCount())
		{
			CXTPChartDiagram* pDiagram = DYNAMIC_DOWNCAST(CXTPChartDiagram,
														  pContent->GetPanels()->GetAt(nIndex));
...
BEGIN_DISPATCH_MAP(CXTPChartSeries, CXTPChartElement)
	...
	DISP_PROPERTY_EX_ID(CXTPChartSeries, "PointLegendFormat", 8, OleGetPointLegendFormat,
						SetNotSupported, VT_DISPATCH)
	DISP_PROPERTY_EX_ID(CXTPChartSeries, "LegendItem", 12, OleGetLegendItem, SetNotSupported,
						VT_DISPATCH)

	DISP_PROPERTY_EX_ID(CXTPChartSeries, "ArgumentScaleType", 4, OleGetArgumentScaleType,
						OleSetArgumentScaleType, VT_I4)
	DISP_PROPERTY_EX_ID(CXTPChartSeries, "ValueScaleType", 5, OleGetValueScaleType,
						OleSetValueScaleType, VT_I4)

	DISP_PROPERTY_NOTIFY_ID(CXTPChartSeries, "Name", 6, m_strName, OleChartChanged, VT_BSTR)
	DISP_PROPERTY_NOTIFY_ID(CXTPChartSeries, "LegendText", 7, m_strLegendText, OleChartChanged,
							VT_BSTR)
	DISP_PROPERTY_NOTIFY_ID(CXTPChartSeries, "Visible", 9, m_bVisible, OleChartChanged, VT_BOOL)

	DISP_PROPERTY_EX_ID(CXTPChartSeries, "LegendVisible", 10, OleGetLegendVisible,
						OleSetLegendVisible, VT_BOOL)

	DISP_FUNCTION_ID(CXTPChartSeries, "EnableVirtualMode", 11, OleEnableVirtualMode, VT_EMPTY,
...
void CXTPChartSeries::OleSetStyle(LPDISPATCH lpStyle)
{
	if (lpStyle)
	{
		...
		CXTPChartSeriesStyle* pStyle = XTP_DYNAMIC_DOWNCAST_REMOTE_(
			CXTPChartSeriesStyle, CXTPChartSeriesStyle::FromIDispatchSafe(lpStyle));
		if (NULL == pStyle)
		{
...
void CXTPChartSeries::OleSetDiagram(LPDISPATCH lpDiagram)
{
	if (lpDiagram)
	{
		CXTPChartDiagram* pDiagram = XTP_DYNAMIC_DOWNCAST_REMOTE_(
			CXTPChartDiagram, CXTPChartDiagram::FromIDispatchSafe(lpDiagram));
		if (NULL == pDiagram)
		{
			AfxThrowOleException(E_INVALIDARG);
		}
...
double CALLBACK OleGetPointValue(CXTPChartSeries* pSeries, double x)
{
	...
	double y = 0;
	pOleControl->FireEvent(10, EVENT_PARAM(VTS_DISPATCH VTS_R8 VTS_PR8),
						   XTPGetDispatch(pSeries, FALSE), x, &y);
...
{
	SetPoints(
		new CXTPChartSeriesVirtualPointCollection(MinRange, MaxRange, Step, OleGetPointValue));
}
