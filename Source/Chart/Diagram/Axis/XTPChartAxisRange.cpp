	...
	for (int i = 0; i < pView->m_arrSeries.GetSize(); i++)
	{
		...
		for (int j = 0; j < pSeriesView->GetPointsView()->GetCount(); j++)
		{
			CXTPChartSeriesPointView* pPointView =
				(CXTPChartSeriesPointView*)pSeriesView->GetPointsView()->GetAt(j);
			CXTPChartErrorBarView* pErrorBarView = static_cast<CXTPChartErrorBarView*>(
				pSeriesView->GetErrorBarsView()->GetAt(j));
...
void CXTPChartAxisRange::Correct(CXTPChartAxisView* pView)
{
	...
	if (m_bSideMargins)
	{
		...
		for (int i = 0; i < arrSeries.GetSize(); i++)
		{
			...
			pSeriesView->GetSeries()->GetStyle()->CorrectAxisSideMargins(m_pAxis, m_nMinValue,
...
BEGIN_DISPATCH_MAP(CXTPChartAxisRange, CXTPChartElement)
	DISP_PROPERTY_EX_ID(CXTPChartAxisRange, "ShowZeroLevel", 1, OleGetShowZeroLevel,
						OleSetShowZeroLevel, VT_BOOL)
	DISP_PROPERTY_EX_ID(CXTPChartAxisRange, "SideMargins", 2, OleGetSideMargins, OleSetSideMargins,
						VT_BOOL)
	...
	DISP_PROPERTY_EX_ID(CXTPChartAxisRange, "AutoRange", 5, OleGetAutoRange, OleSetAutoRange,
						VT_BOOL)

	DISP_PROPERTY_EX_ID(CXTPChartAxisRange, "ViewMinValue", 6, GetViewMinValue, SetViewMinValue,
						VT_R8)
	DISP_PROPERTY_EX_ID(CXTPChartAxisRange, "ViewMaxValue", 7, GetViewMaxValue, SetViewMaxValue,
						VT_R8)
	DISP_PROPERTY_EX_ID(CXTPChartAxisRange, "ViewAutoRange", 8, IsViewAutoRange, SetViewAutoRange,
						VT_BOOL)
