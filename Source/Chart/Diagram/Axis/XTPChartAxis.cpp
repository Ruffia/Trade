...
void CXTPChartAxis::UpdateRange(CXTPChartDeviceContext* pDC, CXTPChartAxisView* pView)
{
	if (pView->m_arrSeries.GetSize() > 0)
	{
		...
		m_nScaleType = IsValuesAxis() ? pSeries->GetValueScaleType()
									  : pSeries->GetArgumentScaleType();
	}
...
CXTPChartAxisView* CXTPChartAxis::CreateView(CXTPChartDeviceContext* /*pDC*/,
...
CXTPChartAxisView* CXTPChartAxisX::CreateView(CXTPChartDeviceContext* pDC,
											  CXTPChartElementView* pParentView)
{
...
CXTPChartAxisView* CXTPChartAxisY::CreateView(CXTPChartDeviceContext* pDC,
											  CXTPChartElementView* pParentView)
{
	UNREFERENCED_PARAMETER(pDC);
...
double CXTPChartAxisX::CalcSideMarginsValue(double nMinValue, double nMaxValue) const
{
	...
	{
		double nDelta = nMaxValue - nMinValue;
		return nDelta / (sqrt(nDelta) * 5);
	}
...
BEGIN_DISPATCH_MAP(CXTPChartAxis, CXTPChartElement)
	...
	DISP_PROPERTY_EX_ID(CXTPChartAxis, "ConstantLines", 6, OleGetConstantLines, SetNotSupported,
						VT_DISPATCH)
	...
	DISP_PROPERTY_EX_ID(CXTPChartAxis, "TickMarks", 9, OleGetTickMarks, SetNotSupported,
						VT_DISPATCH)
	DISP_PROPERTY_EX_ID(CXTPChartAxis, "Title", 10, OleGetTitle, SetNotSupported, VT_DISPATCH)
	DISP_PROPERTY_EX_ID(CXTPChartAxis, "GridLines", 11, OleGetGridLines, SetNotSupported,
						VT_DISPATCH)
	...
	DISP_PROPERTY_EX_ID(CXTPChartAxis, "InterlacedFillStyle", 14, OleGetInterlacedFillStyle,
						SetNotSupported, VT_DISPATCH)

	DISP_PROPERTY_EX_ID(CXTPChartAxis, "Color", 15, OleGetColor, OleSetColor, VT_COLOR)
	DISP_PROPERTY_EX_ID(CXTPChartAxis, "InterlacedColor", 16, OleGetInterlacedColor,
						OleSetInterlacedColor, VT_COLOR)
	DISP_PROPERTY_EX_ID(CXTPChartAxis, "InterlacedColor2", 17, OleGetInterlacedColor2,
						OleSetInterlacedColor2, VT_COLOR)
	DISP_PROPERTY_EX_ID(CXTPChartAxis, "GridSpacing", 18, GetGridSpacing, SetGridSpacing, VT_R8)
	DISP_PROPERTY_EX_ID(CXTPChartAxis, "GridSpacingAuto", 19, GetGridSpacingAuto,
						SetGridSpacingAuto, VT_BOOL)

	DISP_PROPERTY_EX_ID(CXTPChartAxis, "AllowZoom", 20, IsAllowZoom, SetAllowZoom, VT_BOOL)

	DISP_PROPERTY_EX_ID(CXTPChartAxis, "CustomLabels", 21, OleGetCustomLabels, SetNotSupported,
						VT_DISPATCH)

	DISP_PROPERTY_EX_ID(CXTPChartAxis, "Logarithmic", 22, IsLogarithmic, SetLogarithmic, VT_BOOL)
	DISP_PROPERTY_EX_ID(CXTPChartAxis, "LogarithmicBase", 23, GetLogarithmicBase,
						SetLogarithmicBase, VT_R8)

	DISP_PROPERTY_EX_ID(CXTPChartAxis, "DateTimeScaleUnit", 24, GetDateTimeScaleUnit,
						SetDateTimeScaleUnit, VT_I4)
