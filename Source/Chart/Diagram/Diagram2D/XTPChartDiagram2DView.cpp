...
CXTPChartDiagram2DView::CXTPChartDiagram2DView(CXTPChartDiagram* pDiagram,
...
void CXTPChartDiagram2DView::AddAxisView(CXTPChartDeviceContext* pDC,
										 CXTPChartElementView* pParentView, CXTPChartAxis* pAxis)
{
	CXTPChartDiagram2DAxisView* pAxisView = (CXTPChartDiagram2DAxisView*)
												pAxis->CreateView(pDC, pParentView);
...
void CXTPChartDiagram2DView::UpdateRange(CXTPChartDeviceContext* pDC)
{
	...
	for (i = 0; i < GetSeriesView()->GetCount(); i++)
	{
		CXTPChartDiagram2DSeriesView* pSeriesView =
			(CXTPChartDiagram2DSeriesView*)GetSeriesView()->GetAt(i);
	...
	for (i = 0; i < m_pAxisViews->GetCount(); i++)
	{
		...
		for (int j = 0; j < GetSeriesView()->GetCount(); j++)
		{
			...
			CXTPChartDiagram2DSeriesStyle* pStyle = DYNAMIC_DOWNCAST(
				CXTPChartDiagram2DSeriesStyle, pSeriesView->GetSeries()->GetStyle());
			...
			if (pAxisView->GetAxis()->GetIndex()
				!= (bAxisX ? pStyle->GetAxisIndexX() : pStyle->GetAxisIndexY()))
				continue;

			if (pAxisView->GetAxis()->IsSecondary()
				!= (bAxisX ? pStyle->IsSecondaryAxisX() : pStyle->IsSecondaryAxisY()))
				continue;
	...
	for (int nUpdate = 0; nUpdate < 2; nUpdate++)
	{
		...
		for (i = 0; i < m_pAxisViews->GetCount(); i++)
		{
			...
			if (pAxisView->GetAxis()->IsVertical())
			{
				...
				{
					if (rcPane.left != rcBounds.left)
						rcPane.left += AXIS_GAP;
					rcPane.left += pAxisView->GetSize();
				}
				else
				{
					if (rcPane.right != rcBounds.right)
						rcPane.right -= AXIS_GAP;
			...
			else
			{
				if (pAxisView->GetAxis()->GetAlignment() == xtpChartAxisNear)
				{
					if (rcPane.bottom != rcBounds.bottom)
						rcPane.bottom -= AXIS_GAP;
					rcPane.bottom -= pAxisView->GetSize();
				}
				else
				{
					if (rcPane.top != rcBounds.top)
						rcPane.top += AXIS_GAP;
		...
		for (i = m_pAxisViews->GetCount() - 1; i >= 0; i--)
		{
			...
			if (pAxisView->GetAxis()->IsVertical())
			{
				if (pAxisView->GetAxis()->GetAlignment() == xtpChartAxisNear)
				{
					if (rcBounds.left != m_rcBounds.left)
						rcBounds.left += AXIS_GAP;
					pAxisView->SetBounds(pDC, CRect(rcBounds.left, rcPane.top,
													rcBounds.left + nSize, rcPane.bottom));
					rcBounds.left += nSize;
				}
				else
				{
					if (rcBounds.right != m_rcBounds.right)
						rcBounds.right -= AXIS_GAP;
					pAxisView->SetBounds(pDC, CRect(rcBounds.right - nSize, rcPane.top,
													rcBounds.right, rcPane.bottom));
			...
			else
			{
				if (pAxisView->GetAxis()->GetAlignment() == xtpChartAxisNear)
				{
					if (rcBounds.bottom != m_rcBounds.bottom)
						rcBounds.bottom -= AXIS_GAP;
					pAxisView->SetBounds(pDC, CRect(rcPane.left, rcBounds.bottom - nSize,
													rcPane.right, rcBounds.bottom));
					rcBounds.bottom -= nSize;
				}
				else
				{
					if (rcBounds.top != m_rcBounds.top)
						rcBounds.top += AXIS_GAP;
					pAxisView->SetBounds(pDC, CRect(rcPane.left, rcBounds.top, rcPane.right,
													rcBounds.top + nSize));
					rcBounds.top += nSize;
				}
			}
		...
		{
			CXTPChartDiagram2DSeriesLabelView* pLabelView = (CXTPChartDiagram2DSeriesLabelView*)
