...
BOOL CXTPTabBase::SetTheme(XTPControlTheme eTheme)
{
	...
	switch (eTheme)
	{
		case xtpControlThemeOfficeXP: m_pTheme = new CXTPTabBaseThemeOfficeXP; break;

		case xtpControlThemeOffice2003: m_pTheme = new CXTPTabBaseThemeOffice2003; break;

		default: m_pTheme = new CXTPTabBaseTheme; break;
	}
...
void CXTPTabExBase::OnRButtonDown(UINT /*nFlags*/, CPoint point)
{
	if (m_popupMenuID)
	{
		...
		pPopup->TrackPopupMenu(TPM_LEFTALIGN | TPM_RIGHTBUTTON, pt.x, pt.y, m_pParentFrame);
	}
}
...
BOOL CXTPTabExBase::OnSelchange(NMHDR* pNMHDR, LRESULT* pResult)
{
	*pResult = 0;
	...
	{
		*pResult = pOwner->SendMessage(TCN_SELCHANGE, pNMHDR->idFrom, (LPARAM)m_pTabCtrl);
	}
...
BOOL CXTPTabExBase::OnSelchanging(NMHDR* pNMHDR, LRESULT* pResult)
{
	*pResult = 0;
	...
	if (::IsWindow(pOwner->GetSafeHwnd()))
	{
		*pResult = pOwner->SendMessage(TCN_SELCHANGING, pNMHDR->idFrom, (LPARAM)m_pTabCtrl);
	}
...
void CXTPTabExBase::UpdateToolTip(int nIDTab, LPCTSTR lpszText)
{
	...
	if (pos != NULL)
	{
		...
		if (pToolTips != NULL)
		{
			...
			pToolTips->UpdateTipText(pMember->szToolTipLabel, m_pTabCtrl, pMember->uiToolTipId);
		}
	}
}
...
void CXTPTabExBase::ResetToolTips()
{
	...
	if (pToolTips != NULL)
	{
		...
		for (pos = m_tcbItems.GetHeadPosition(); pos; m_tcbItems.GetNext(pos))
		{
			...
			// Update the tooltip for the view
			pMember->uiToolTipId = XTPToUIntChecked(nIndex);

			pToolTips->UpdateTipText(pMember->szToolTipLabel, m_pTabCtrl,
									 XTPToUIntPtrChecked(nIndex));
...
CWnd* CXTPTabExBase::CreateTabView(CRuntimeClass* pViewClass, CDocument* pDocument,
								   CCreateContext* pContext)
{
	...
	CRect rect(0, 0, 0, 0);
	if (!pWnd->Create(NULL, NULL, dwStyle, rect, m_pTabCtrl, XTPToUInt(AFX_IDW_PANE_FIRST + nTab),
					  pContext))
	{
	...
	// Bind document
	if (NULL != pDocument && NULL != pViewClass && pViewClass->IsDerivedFrom(RUNTIME_CLASS(CView)))
	{
...
BOOL CXTPTabExBase::AddView(LPCTSTR lpszLabel, CRuntimeClass* pViewClass,
							CDocument* pDoc /*= NULL*/, CCreateContext* pContext /*= NULL*/,
							int iIndex /*= -1*/, int iIconIndex /*= -1*/, LPARAM lParam)
{
	if (!pContext && !pViewClass)
		return NULL;
...
BOOL CXTPTabExBase::AddView(LPCTSTR lpszLabel, CView* pView, int iIndex /*= -1*/,
							int iIconIndex /*= -1*/, LPARAM lParam)
{
	ASSERT(pView != NULL);
	if (!pView)
		return FALSE;
...
BOOL CXTPTabExBase::AddControl(LPCTSTR lpszLabel, CWnd* pWnd, int iIndex /*= -1*/,
							   int iIconIndex /*= -1*/, LPARAM lParam)
{
	ASSERT(m_pTabCtrl);
	ASSERT(pWnd);
	...
	pMember->uiToolTipId	= XTPToUIntChecked(m_pTabCtrl->GetItemCount());
	pMember->dwStyle		= XTPToDWORD(::GetWindowLong(pWnd->GetSafeHwnd(), GWL_STYLE));
	pMember->dwExStyle		= XTPToDWORD(::GetWindowLong(pWnd->GetSafeHwnd(), GWL_EXSTYLE));
...
BOOL CXTPTabExBase::UpdateTabLabel(int iTab, CXTPTcbItem* pMember, LPCTSTR lpszLabel)
{
	...
	pToolTips->UpdateTipText(strLabel, m_pTabCtrl, XTPToUIntPtrChecked(iTab));
...
void CXTPTabExBase::Condense()
{
	...
	const bool bMultiLine = bVertical
							|| ((m_pTabCtrl->GetStyle() & TCS_MULTILINE) == TCS_MULTILINE);
