...
BOOL CXTPExcelTabCtrl::SetTheme(XTPControlTheme eTheme)
{
	...
	switch (eTheme)
	{
		...
		case xtpControlThemeVisualStudio2015:
			m_pTheme = new CXTPExcelTabCtrlThemeVisualStudio2015;
			break;
		case xtpControlThemeVisualStudio2017:
			m_pTheme = new CXTPExcelTabCtrlThemeVisualStudio2017;
			break;
		case xtpControlThemeVisualStudio2019:
			m_pTheme = new CXTPExcelTabCtrlThemeVisualStudio2019;
			break;
		case xtpControlThemeVisualStudio2022:
			m_pTheme = new CXTPExcelTabCtrlThemeVisualStudio2022;
			break;
		case xtpControlThemeNativeWindows10:
			m_pTheme = new CXTPExcelTabCtrlThemeNativeWindows10;
			break;
...
BOOL CXTPExcelTabCtrl::Init()
{
	if (SetTheme(xtpControlThemeDefault))
	{
		if (m_bSubclassed)
		{
			m_dwStyle = XTPToDWORD(::GetWindowLong(m_hWnd, GWL_STYLE));
...
int CXTPExcelTabCtrl::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_dwStyle		= XTPToDWORD(lpCreateStruct->style);
...
void CXTPExcelTabCtrl::SetGripperPosition(int x, bool bPercent)
{
	...
	else
	{
		...
		m_rectSB_H = CRect(m_rectGripper.right, m_rectTabs.top - !isbottom, m_rectTabs.right - m_cy,
...
void CXTPExcelTabCtrl::SetTabStyle(DWORD dwStyle)
{
	...
	if (m_dwStyle & FTS_XTP_HASARROWS)
	{
		...
		if (m_dwStyle & FTS_XTP_HASHOMEEND)
		{
			m_buttons[nIndex].SetInfo(rectDummy, XTP_IDC_BTN_HOME, xtpArrowIconLeftHome);
			m_iBtnHome = nIndex;
			++nIndex;
		}
		else
		...
		if (m_dwStyle & FTS_XTP_HASHOMEEND)
		{
			m_buttons[nIndex].SetInfo(rectDummy, XTP_IDC_BTN_END, xtpArrowIconRightHome);
			m_iBtnEnd = nIndex;
			++nIndex;
		}
		else
	...
	if (m_dwStyle & FTS_XTP_HSCROLL)
	{
		...
		{
			CRect rc(0, 0, 0, 0);
			VERIFY(m_wndHScrollBar.Create(WS_VISIBLE | WS_CHILD | SBS_HORZ, rc, this,
...
int CXTPExcelTabCtrl::GetTabWidth(int nItem) const
{
	...
	CFont* pOldFont = pDC->SelectObject((m_nCurSel == nItem) ? m_pBoldFont : m_pNormFont);
...
void CXTPExcelTabCtrl::EnableButtons()
{
	if (m_dwStyle & FTS_XTP_HASARROWS)
	{
		...
		m_buttons[m_iBtnRight].m_bEnabled = (GetTotalTabWidth() - m_nOffset
											 > GetTotalTabAreaWidth() - 1);
...
int CXTPExcelTabCtrl::InsertItem(int nItem, LPCTSTR lpszItem, CWnd* pWndControl)
{
	...
	if (pWndControl)
	{
		...
		ASSERT((::GetWindowLong(pWndControl->m_hWnd, GWL_STYLE) & (WS_VISIBLE | WS_CHILD))
			   == WS_CHILD);
		...
		pWndControl->SetWindowPos(NULL, m_rectViews.left, m_rectViews.top, m_rectViews.Width(),
								  m_rectViews.Height(), SWP_NOZORDER | SWP_NOREDRAW);
	}
	...
	pMember->uiToolTipId	= XTPToUIntChecked(GetItemCount());
...
void CXTPExcelTabCtrl::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	if ((iTab != -1) && (iTab != m_nCurSel))
	{
		...
		NMHDR hdr;
		hdr.hwndFrom = m_hWnd;
		hdr.idFrom   = XTPToUIntPtr(id);
		hdr.code	 = TCN_SELCHANGING;
		if (GetOwner()->SendMessage(WM_NOTIFY, XTPToWPARAM(id), (LPARAM)&hdr) == 0)
		{
			...
			GetOwner()->SendMessage(WM_NOTIFY, XTPToWPARAM(id), (LPARAM)&hdr);
		}
	}
...
void CXTPExcelTabCtrl::OnLButtonUp(UINT nFlags, CPoint point)
{
	...
	for (i = 0; i < BUTTON_COUNT; ++i)
	{
		...
		{
			PostMessage(WM_COMMAND, XTPToWPARAM(b.m_iCommand), NULL);
			b.m_bPressed = false;
			RedrawWindow(b.m_rect);
			ReleaseCapture();
			return;
...
void CXTPExcelTabCtrl::OnDraw(CDC* pDC)
{
	...
	if (rectPaint.right >= rcArrows.right)
	{
		...
		for (i = 0; i < cItems; i++)
		{
			if (i != m_nCurSel)
			{
				const int cx = m_pTheme->DrawTab(&tabDC, this, CPoint(x, y), false, m_tcbItems[i]);
				x += cx;
			}
			else
			{
		...
		{
			m_pTheme->DrawTab(&tabDC, this, CPoint(iSelX, y), true, m_tcbItems[m_nCurSel]);
		}

		// blit the bitmap onto the pDC->
		pDC->BitBlt(iTotalArrowWidth, rectPaint.top, GetTotalTabAreaWidth(), m_iTabHeight, &tabDC,
					m_nOffset, 0, SRCCOPY);
		// cleanup.
...
void CXTPExcelTabCtrl::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* /*pScrollBar*/)
{
	...
	if (pWndControl->GetSafeHwnd())
	{
		...
		{
			int dx = XTPToInt(nPos - pWndControl->GetScrollPos(SB_HORZ));
			if (dx)
			{
				pWndControl->SendMessage(LVM_SCROLL, XTPToWPARAM(dx), 0);
			}
		}

		// pass message to control's scroll bar
		_AFX_THREAD_STATE* pThreadState = AfxGetThreadState();
		pWndControl->SendMessage(pThreadState->m_lastSentMsg.message,
...
void CXTPExcelTabCtrl::SyncScrollBar()
{
	...
	if (pWndControl && pWndControl->GetSafeHwnd())
	{
		...
		if (!pWndControl->GetScrollInfo(SB_HORZ, &si) || si.nPage == 0
...
void CXTPExcelTabCtrl::RecalcLayout()
{
	...
	if (m_bManagingViews)
	{
		...
		for (nIndex = 0; nIndex < cItems; ++nIndex)
		{
			...
			m_tcbItems[nIndex]->pWnd->SetWindowPos(NULL, m_rectViews.left, m_rectViews.top,
	...
	if (m_dwStyle & FTS_XTP_HASARROWS)
	{
		...
		CRect btnRects[] = { CRect(0, iTop, m_cx, iBottom), CRect(m_cx, iTop, m_cx * 2, iBottom),
...
void CXTPExcelTabCtrl::SetTabHighlightColor(COLORREF crHighlight)
{
	m_pTheme->m_clr3DHilight.SetCustomValue(crHighlight);
}

COLORREF CXTPExcelTabCtrl::GetTabHighlightColor() const
{
