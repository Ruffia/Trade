...
bool CXTPColorBase::Init()
{
	if (::IsWindow(m_hWnd))
	{
		...
		::SetWindowLong(m_hWnd, GWL_STYLE, lStyle | SS_NOTIFY);
...
void CXTPColorBase::OnLButtonDblClk(UINT nFlags, CPoint point)
{
	...
	if (pWnd)
	{
		pWnd->SendMessage(WM_XTP_COLORDBLCLK, (WPARAM)0,
...
void CXTPColorWnd::OnPaint()
{
	...
	if (m_bmpPicker.GetSafeHandle() == NULL)
	{
		...
		for (x = 0; x < cx; x++)
		{
			...
			for (y = 0; y < cy; y++)
			{
				COLORREF clr = HLStoRGB((double)x / (double)cx, DEFAULT_LUMINANCE,
										(double)y / (double)cy);
...
void CXTPColorWnd::UpdateCursorPos(CPoint point)
{
	...
	if (pWnd)
	{
		pWnd->SendMessage(WM_XTP_UPDATECOLOR, (WPARAM)HLStoRGB(m_nHue, m_nLum, m_nSat),
						  (LPARAM)m_hWnd);
	}
...
void CXTPColorWnd::DrawCrossHair(CDC* pDC)
{
	...
	COLORREF crStick = (m_eHasFocus == focusColorWheel ? RGB(0xff, 0xff, 0xff)
													   : RGB(0x00, 0x00, 0x00));

	CRect rcStick(x - XTP_DPI_X(5), y - XTP_DPI_Y(1), x - XTP_DPI_X(10), y + XTP_DPI_Y(1));
	pDC->FillSolidRect(rcStick, crStick);

	rcStick.SetRect(x + XTP_DPI_X(5), y - XTP_DPI_Y(1), x + XTP_DPI_X(10), y + XTP_DPI_Y(1));
	pDC->FillSolidRect(rcStick, crStick);

	rcStick.SetRect(x - XTP_DPI_X(1), y - XTP_DPI_Y(5), x + XTP_DPI_X(1), y - XTP_DPI_Y(10));
	pDC->FillSolidRect(rcStick, crStick);
...
void CXTPColorLum::DrawSliderArrow(CDC* pDC)
{
	...
	CSize sz = XTPSystemMetrics()->GetSmallIconSize();
...
LRESULT CXTPColorPageCustom::OnUpdateColor(WPARAM wParam, LPARAM lParam)
{
	...
	else
	{
		...
		if (::IsWindow(hWnd))
		{
			if ((hWnd == m_colorLum.m_hWnd) || (hWnd == m_colorWnd.m_hWnd))
			{
				::SetFocus(hWnd);
			}
		}
