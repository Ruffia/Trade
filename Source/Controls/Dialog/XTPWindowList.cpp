...
BOOL CXTPWindowList::OnInitDialog()
{
	...
	if (!m_hIcon)
		m_hIcon = (HICON)::SendMessage(hWndParent, WM_GETICON, ICON_SMALL, 0);
	if (!m_hIcon)
		m_hIcon = (HICON)::SendMessage(hWndParent, WM_GETICON, ICON_BIG, 0);
	if (!m_hIcon)
		m_hIcon = (HICON)(ULONG_PTR)::GetClassLongPtr(hWndParent, GCLP_HICONSM);
	if (!m_hIcon)
		m_hIcon = (HICON)(ULONG_PTR)::GetClassLongPtr(hWndParent, GCLP_HICON);
#else
...
void CXTPWindowList::FillMDIClientWindowList(HWND hWndMDIClient)
{
	// Retrieve the current active MDI child window.
	HWND hwndActiveChild = ::GetWindow(hWndMDIClient, GW_CHILD);
	if (hwndActiveChild != NULL)
	{
		...
		while (hwndChildFrame != NULL)
		{
			if (::IsWindowVisible(hwndChildFrame)
				&& (GetWindowLong(hwndChildFrame, GWL_EXSTYLE) & WS_EX_MDICHILD))
			{
...
void CXTPWindowList::FillMDIWindowList()
{
	...
	FillMDIClientWindowList(m_hWndMDIClient);

#ifdef _XTP_INCLUDE_COMMANDBARS
	CXTPTabClientWnd* pTabClient = DYNAMIC_DOWNCAST(CXTPTabClientWnd,
													CWnd::FromHandle(m_hWndMDIClient));
	if (pTabClient)
	{
		...
		for (int i = 0; i < tabClients.GetSize(); i++)
		{
			...
			FillMDIClientWindowList(tabClients.GetAt(i)->m_hWnd);
		}
	}
#endif
}
...
#ifdef _XTP_INCLUDE_COMMANDBARS
		CXTPTabClientWnd* pTabClientWnd = DYNAMIC_DOWNCAST(CXTPTabClientWnd,
														   CWnd::FromHandle(hWnd)->GetParent());
		if (pTabClientWnd)
		{
			CXTPTearOffFrame* pTearOffFrame = DYNAMIC_DOWNCAST(CXTPTearOffFrame,
															   pTabClientWnd->GetParentFrame());
			XTP_SAFE_CALL1(pTearOffFrame, SetForegroundWindow());

			::SendMessage(pTabClientWnd->m_hWnd, WM_MDIACTIVATE, (WPARAM)hWnd, 0);
		}
#endif
...
void CXTPWindowList::OnBtnMinimize()
{
	...
	if (iCount != LB_ERR && iCount > 0)
	{
		...
		for (iItem = iCount - 1; iItem >= 0; iItem--)
		{
			...
			if (m_lboxWindows.GetSel(iItem) > 0 && (lStyle & WS_MINIMIZEBOX))
			{
				::ShowWindow(hWnd, SW_MINIMIZE);
			}
		}
...
void CXTPWindowList::OnDrawItem(int nIDCtl, LPDRAWITEMSTRUCT lpDIS)
{
	if (nIDCtl == XTP_IDC_LBOX_WINDOWS)
	{
		...
		CString strBuffer;
		m_lboxWindows.GetText(static_cast<int>(lpDIS->itemID), strBuffer);

		pDC->SetBkMode(TRANSPARENT);
		pDC->SetTextColor(crText);

		rcItem.left += sizeIcon.cx + XTP_DPI_X(4);
		pDC->DrawText(strBuffer, -1, &rcItem, DT_LEFT | DT_VCENTER | DT_NOPREFIX | DT_SINGLELINE);
		...
		if (!hIcon)
			hIcon = (HICON)::SendMessage((HWND)lpDIS->itemData, WM_GETICON, ICON_BIG, 0);
		...
		if (!hIcon)
			hIcon = (HICON)(ULONG_PTR)::GetClassLongPtr((HWND)lpDIS->itemData, GCLP_HICON);
		...
		::DrawIconEx(pDC->m_hDC, rcItem.left, rcItem.top, hIcon, sizeIcon.cx, sizeIcon.cy, 0, NULL,
					 DI_NORMAL);
