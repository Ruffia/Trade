...
CXTPTaskDialogClient::CXTPTaskDialogClient()
	...
	, m_szIconLinkGlyph(XTP_DPI_SIZE(CSize(20, 20)))
...
HRESULT CXTPTaskDialogClient::SendNotify(UINT uNotify, WPARAM wParam /*=0*/, LPARAM lParam /*=0*/)
{
	...
	{
		return m_pConfig->pfCallback(m_hWnd, uNotify, wParam, lParam, m_pConfig->lpCallbackData);
	}
	return E_FAIL;
}
...
void CXTPTaskDialogClient::OnMarkupHyperlinkClick(CXTPMarkupObject* pSender,
												  CXTPMarkupRoutedEventArgs* pArgs)
{
	...
	if (pSender->IsKindOf(MARKUP_TYPE(CXTPMarkupHyperlink)))
	{
		CXTPMarkupString* pTag = MARKUP_STATICCAST(CXTPMarkupString,
												   ((CXTPMarkupHyperlink*)pSender)->GetTag());
		SendNotify(TDN_HYPERLINK_CLICKED, (WPARAM)0,
				   (LPARAM)(pTag ? (LPCWSTR)*pTag : (LPCWSTR)NULL));
...
BOOL CXTPTaskDialogClient::OnWndMsg(UINT message, WPARAM wParam, LPARAM lParam, LRESULT* pResult)
{
	...
	if (m_pMarkupContext)
	{
		...
		{
			if (XTPMarkupRelayMessage(m_strMainInstruction.m_pUIElement, message, wParam, lParam,
									  pResult))
				return TRUE;
		}
		...
		{
			if (XTPMarkupRelayMessage(m_strExpandedInformation.m_pUIElement, message, wParam,
									  lParam, pResult))
				return TRUE;
		}
		else
	...
	switch (message)
	{
		case WM_COMMAND:
		{
			if (HIWORD(wParam) == BN_CLICKED)
			{
				...
				if (hWnd
					&& (hWnd == m_wndFooter.GetSafeHwnd() || hWnd == m_wndContent.GetSafeHwnd()
				...
				if (hWnd && m_pBtnVerify->GetSafeHwnd() == hWnd)
				{
					...
					SendNotify(TDN_VERIFICATION_CLICKED, XTPToWPARAM(m_bVerification));
					return 0;
				}
				...
				if (hWnd && m_pBtnExpando->GetSafeHwnd() == hWnd)
				{
					...
					SendNotify(TDN_EXPANDO_BUTTON_CLICKED, XTPToWPARAM(m_bExpanded));
...
void CXTPTaskDialogClient::RefreshMetrics()
{
	...
	m_sizeIconFooter = XTPSystemMetrics()->GetSmallIconSize();
	m_sizeIconMain   = XTPSystemMetrics()->GetIconSize();
	...
	m_ptBtnSpacing = CPoint(XTPDlu2Pix(XTP_DPI_X(xtpDluBtnSpacing), XTP_DPI_X(xtpDluBtnSpacing)).x,
							0);
...
CString CXTPTaskDialogClient::LoadItemString(PCWSTR pszBuffer)
{
	...
	if (pszBuffer && XTP_IS_INTRESOURCE(pszBuffer))
	{
	...
	if (!m_strExpandedInformation.IsEmpty())
	{
		...
		if (m_strExpandedControlText.IsEmpty() && m_strCollapsedControlText.IsEmpty())
		{
			CXTPResourceManager::AssertValid(
				XTPResourceManager()->LoadString(&m_strExpandedControlText.m_strCaption,
												 XTP_IDS_TASK_HIDE_DETAILS));
			CXTPResourceManager::AssertValid(
				XTPResourceManager()->LoadString(&m_strCollapsedControlText.m_strCaption,
...
{
	if ((m_pConfig->dwCommonButtons & TDCBF_OK_BUTTON)
		|| (!m_pConfig->pButtons && !m_pConfig->dwCommonButtons))
	{
...
BOOL CXTPTaskDialogClient::CreateLinkButtons()
{
	...
	for (UINT i = 0; i < m_pConfig->cButtons; ++i)
	{
		...
		if (IsLinkButtonsUsed())
		{
			...
			CButton* pLinkButton = CreateButton(m_pConfig->pButtons[i].nButtonID, strButtonText,
												CSize(0, 0),
												BS_PUSHBUTTON | BS_MULTILINE | WS_TABSTOP, pTheme);
			...
			if (m_pIconLinkGlyph)
			{
				...
				pButton->SetIcon(m_szIconLinkGlyph, m_pIconLinkGlyph);
			}
		...
		else
		{
			...
			if (AddCommandButton(m_pConfig->pButtons[i].nButtonID, strButtonText, sizeButton)
				== NULL)
			{
				TRACE0("Failed to create OK button.\n");
				return FALSE;
...
BOOL CXTPTaskDialogClient::CreateProgressBar()
{
	...
	if (!m_wndProgressBar.Create(
			DWORD(WS_CHILD | WS_VISIBLE
				  | (m_pConfig->dwFlags & TDF_SHOW_MARQUEE_PROGRESS_BAR ? PBS_MARQUEE : 0)),
...
BOOL CXTPTaskDialogClient::CreateInputBox()
{
	...
	if (!m_wndInputBox.CreateEx(WS_EX_CLIENTEDGE, _T("EDIT"), m_strInputBoxText,
								WS_CHILD | WS_VISIBLE | WS_TABSTOP | ES_AUTOHSCROLL,
								CRect(0, 0, 0, 0), this, IDC_INPUTBOX))
		return FALSE;
...
BOOL CXTPTaskDialogClient::CreateRadioButtons()
{
	...
	for (UINT i = 0; i < m_pConfig->cRadioButtons; ++i)
	{
		...
		CButton* pButton = CreateButton(nButtonID, strButton, CalcButtonSize(strButton, &point),
										BS_AUTORADIOBUTTON | BS_TEXT | BS_MULTILINE | BS_TOP);
...
BOOL CXTPTaskDialogClient::CreateVerifyExpandButtons()
{
	...
	if (!m_strExpandedInformation.IsEmpty())
	{
		CString strButtonText = IsExpandedByDefault() ? m_strExpandedControlText
													  : m_strCollapsedControlText;

		CString strButtonSize = m_strExpandedControlText.m_strCaption.GetLength()
										> m_strCollapsedControlText.m_strCaption.GetLength()
									? m_strExpandedControlText
									: m_strCollapsedControlText;

		CSize sizeButton = CalcButtonSize(strButtonSize, NULL,
										  XTPDlu2Pix(XTP_DPI_X(xtpDluMaxVerify), 0).x);

		CButton* pButton = CreateButton(0, strButtonText, sizeButton,
										BS_PUSHBUTTON | BS_MULTILINE | WS_TABSTOP,
										new CXTPTaskButtonThemeExpando);
	...
	if (!m_strVerificationText.IsEmpty())
	{
		...
		CPoint point(XTP_DPI_X(10), XTP_DPI_Y(2));
		CSize sizeButton = CalcButtonSize(strButtonText, &point,
										  XTPDlu2Pix(XTP_DPI_X(xtpDluMaxVerify), 0).x);

		CButton* pButton = CreateButton(0, strButtonText, sizeButton,
										BS_AUTOCHECKBOX | BS_MULTILINE | BS_TOP | BS_LEFT
											| WS_TABSTOP);
...
BOOL CXTPTaskDialogClient::CreateClient(BOOL bMoveWindow)
{
	...
	if (IsHyperlinksEnabled())
	{
		...
		m_wndExpandedInformation.Create(rect, m_strExpandedInformation.m_strCaption, &m_xtpFont,
										this);
		m_wndFooter.Create(rect, m_strFooter.m_strCaption, &m_xtpFont, this);
	}

	if (!CreateProgressBar() || !CreateRadioButtons() || // create radio buttons.
...
HICON CXTPTaskDialogClient::LoadSysIcon(PCWSTR lpwzIcon, CSize sizeIcon)
{
	...
	if (m_bUseSysIcons)
	{
		...
		{
			hNewIcon = XTPResourceManager()->CreateIconFromResource(modUser32Dll,
...
HICON CXTPTaskDialogClient::CreateIcon(HICON hIcon, PCWSTR lpwzIcon, CSize sizeIcon,
									   BOOL bUseHandle)
{
	HICON hNewIcon = NULL;
	...
	{
		if (m_pConfig->hInstance == NULL || lpwzIcon == NULL || !XTP_IS_INTRESOURCE(lpwzIcon))
		{
			return FALSE;
		}

		if (lpwzIcon == TD_ERROR_ICON || lpwzIcon == TD_WARNING_ICON
			|| lpwzIcon == TD_INFORMATION_ICON || lpwzIcon == TD_SHIELD_ICON
		...
		{
			hNewIcon = XTPResourceManager()->CreateIconFromResource(m_pConfig->hInstance,
																	(LPTSTR)lpwzIcon, sizeIcon);
		}
	}
...
void CXTPTaskDialogClient::CreateIcons()
{
	...
	// create small icon.
	SAFE_DELETE_HICON(m_hIconFooter);
	m_hIconFooter = CreateIcon(m_pConfig->hFooterIcon, m_pConfig->pszFooterIcon, m_sizeIconFooter,
							   IsFooterIconHandleUsed());

	// create large icon.
	SAFE_DELETE_HICON(m_hIconMain);
	m_hIconMain = CreateIcon(m_pConfig->hMainIcon, m_pConfig->pszMainIcon, m_sizeIconMain,
							 IsMainIconHandleUsed());

	if (IsLinkButtonsUsed() && IsLinkButtonsIconVisible() && m_pIconLinkGlyph == 0)
	{
		UINT nID = 0;
		BOOL ret = m_pImageManager->SetVectorIcon(XTPResourceManager()->GetResourceHandle(),
												  _T("RT_XAML"), IDR_XAML_ICON_TASKDLG_ARROW_RIGHT,
												  nID, XTPToUIntChecked(m_szIconLinkGlyph.cx));
		if (ret)
			m_pIconLinkGlyph = m_pImageManager->GetImage(nID);
	}
...
CButton* CXTPTaskDialogClient::CreateButton(int nButtonID, LPCTSTR lpszLabel, CSize sizeButton,
											...
											CXTPButtonTheme* pTheme /*=NULL*/)
{
	...
	if (!pButton->Create(lpszLabel, WS_CHILD | WS_VISIBLE | dwButtonStyle, rcButton, this,
						 static_cast<UINT>(nButtonID)))
	{
		SAFE_DELETE(pButton);
		return NULL;
	}
...
{
	CString strButton;
	XTPResourceManager()->LoadString(&strButton, XTPToUInt(nIDLabel));
...
{
	CButton* pButton = CreateButton(nButtonID, lpszLabel, sizeButton,
									BS_PUSHBUTTON | BS_MULTILINE | WS_TABSTOP);
	if (pButton)
	{
...
CSize CXTPTaskDialogClient::CalcButtonSize(CString strButton, CPoint* pMarginButton /*=NULL*/,
										   int nMaxWidth /*=0*/)
{
	...
	if (!strCaptText.IsEmpty())
	{
		...
		if (nMaxWidth == 0)
			XTPCalcIdealTextSize(strCaptText, sizeCaptText, m_xtpFont, XTP_DPI_X(330),
								 XTP_DPI_X(7));
		else
	...
	if (!strNoteText.IsEmpty())
	{
		...
		if (nMaxWidth == 0)
			XTPCalcIdealTextSize(strNoteText, sizeNoteText, m_xtpFont, XTP_DPI_X(330),
								 XTP_DPI_X(7));
		else
...
CSize CXTPTaskDialogClient::CalcCommandButtonSize()
{
	...
	for (UINT nIDString = UINT(XTP_IDS_TASK_OK); nIDString <= UINT(XTP_IDS_TASK_CLOSE); ++nIDString)
	{
		CString strButton;
...
int CXTPTaskDialogClient::CalcLinkButtonIdealWidth(CString strButton, int cxWidth)
{
	...
	if (m_pIconLinkGlyph)
		cxWidth -= m_szIconLinkGlyph.cx - m_nPadding;
	...
	else
	{
		...
		if (XTPCalcIdealTextSize(strCaptText, sizeText, m_xtpFontLarge, XTP_DPI_X(120),
								 XTP_DPI_X(30)))
		{
	...
	if (m_pIconLinkGlyph)
		cxWidth += m_szIconLinkGlyph.cx;

	return cxWidth + m_ptMargin.x * 2 + m_nPadding;
}

CSize CXTPTaskDialogClient::CalcLinkButtonSize(CXTPButton* pButton, CString strButton,
											   int cxMaxWidth)
{
	if (strButton.IsEmpty() || !HasLinkButtons())
	...
	if (m_pIconLinkGlyph)
		cxMaxWidth -= m_szIconLinkGlyph.cx;
	...
	{
		XTPMarkupSetDefaultFont(pButton->GetMarkupContext(), (HFONT)m_xtpFontLarge.GetSafeHandle(),
								COLORREF_NULL);
...
BOOL CXTPTaskDialogClient::MoveButton(CButton* pButton, int x, int y, int cx, int cy,
									  BOOL bRedraw /*=FALSE*/)
{
	// position the button.
	if (IsWindow(pButton)
		&& pButton->SetWindowPos(NULL, x, y, cx, cy,
...
void CXTPTaskDialogClient::SetLinkButtonPos()
{
	...
	for (int i = 0; i < nCount; ++i)
	{
		...
		{
			MoveButton(m_arrLinkButtons[i], m_rcLinkButtons.left, nTop, m_rcLinkButtons.Width(),
					   sizeButton.cy);
			nTop += sizeButton.cy;
		}
	}
...
void CXTPTaskDialogClient::SetLinkCtrlPos()
{
	...
	{
		if (m_wndContent)
			m_wndContent.MoveWindow(m_rcContent);
		if (m_wndExpandedInformation)
			m_wndExpandedInformation.MoveWindow(m_rcExpandedInformation);
		if (m_wndFooter)
			m_wndFooter.MoveWindow(m_rcFooter);

		if (m_wndExpandedInformation)
			m_wndExpandedInformation.ShowWindow(m_bExpanded ? SW_SHOWNOACTIVATE : SW_HIDE);
	}
}
...
int CXTPTaskDialogClient::CalcIdealTextWidth(const CStringMarkup& strItemText, int nMinWidth,
											 BOOL bLargeFont)
{
	...
	if (strItemText.m_pUIElement)
	{
		if (XTPCalcIdealTextSize(m_pMarkupContext, strItemText.m_pUIElement, sizeText,
								 ...
								 XTP_DPI_X(bLargeFont ? 275 : 330), XTP_DPI_X(bLargeFont ? 14 : 7)))
		{
	...
	{
		if (XTPCalcIdealTextSize(strItemText, sizeText, bLargeFont ? m_xtpFontLarge : m_xtpFont,
								 XTP_DPI_X(bLargeFont ? 275 : 330), XTP_DPI_X(bLargeFont ? 14 : 7)))
		{
			nMinWidth = max(nMinWidth, sizeText.cx);
		}
...
CRect CXTPTaskDialogClient::CalcTextRect(const CStringMarkup& strItemText, int nWidth,
										 BOOL bLargeFont)
{
	...
	if (strItemText.m_pUIElement)
	{
		XTPMarkupSetDefaultFont(m_pMarkupContext, bLargeFont ? m_xtpFontLarge : m_xtpFont,
								COLORREF_NULL);
...
int CXTPTaskDialogClient::CalcClientWidth()
{
	...
	if (HasFixedWidth())
	{
		CSize sizeBase = XTPToDWORDChecked(::GetDialogBaseUnits());
		int cxWidth	= ::MulDiv(XTPToIntChecked(m_pConfig->cxWidth), sizeBase.cx, 4);
...
CSize CXTPTaskDialogClient::CalcDynamicLayout(int nIdealWidth)
{
	...
	if (!IsExpandFooterArea())
	{
		if (!m_strExpandedInformation.IsEmpty())
		{
			if (m_bExpanded)
			{
				m_rcExpandedInformation = CalcTextRect(m_strExpandedInformation,
													   nIdealWidth - nMainIconMargin, FALSE);
				m_rcExpandedInformation.OffsetRect(m_ptMargin.x + nMainIconMargin,
												   nLinkTop + m_ptPadding.y);
			}
			else
			{
				m_rcExpandedInformation.SetRect(m_rcContent.left, nLinkTop, m_rcContent.right,
												nLinkTop + m_ptPadding.y);
			}
	...
	{
		CRect rcProgressBar(m_rcContent.left, nLinkTop + m_ptMargin.y, nIdealWidth + m_ptMargin.x,
							nLinkTop + m_ptMargin.y * 5 / 2);
	...
	{
		m_rcRadioButtons = CRect(m_rcContent.left + XTP_DPI_X(1), nLinkTop, m_rcContent.right,
								 nLinkTop);
	}

	if (m_wndInputBox.GetSafeHwnd())
	{
		CRect rcInputBox(m_rcContent.left, nLinkTop + m_ptMargin.y, nIdealWidth + m_ptMargin.x,
						 nLinkTop + m_ptMargin.y + m_ptMargin.y * 2);
	...
	for (int i = 0; i < (int)m_arrLinkButtons.GetSize(); ++i)
	{
		...
		CSize sizeButton = CalcLinkButtonSize((CXTPButton*)pButton, strButton,
											  m_rcLinkButtons.Width());
	...
	if (m_arrLinkButtons.GetSize() > 0)
		m_rcLinkButtons.bottom += m_ptMargin.y;

	m_rcCommandButtons = CalcCommandRect(nIdealWidth);
	m_rcCommandButtons.OffsetRect(m_ptMargin.x,
								  m_rcLinkButtons.bottom + m_ptMargin.y + m_ptPadding.y);
...
BOOL CXTPTaskDialogClient::ResizeParentToFit(CSize szClient, BOOL bMoveWindow)
{
	...
	CRect rcParent = IsPositionRelativeToWindow()
						 ? CXTPWindowRect(m_pConfig->hwndParent)
						 : XTPMultiMonitor()->GetWorkArea(m_pConfig->hwndParent);
	...
	if (bMoveWindow)
		...
		SetWindowPos(0, 0, 0, rcWindow.Width(), rcWindow.Height(),
...
void CXTPTaskDialogClient::DrawBackground(CDC* pDC, CRect rect)
{
	...
	if (m_bThemeReady)
	{
		...
		{
			pDC->FillSolidRect(rect.left, m_rcCommandButtons.top, rect.Width(), XTP_DPI_Y(1),
							   m_cr3DLight);
		}

		if (!m_rcFooter.IsRectEmpty())
		{
			pDC->FillSolidRect(rect.left, m_rcCommandButtons.bottom, rect.Width(), XTP_DPI_Y(1),
							   m_cr3DLight);
			pDC->FillSolidRect(rect.left, m_rcCommandButtons.bottom + XTP_DPI_Y(1), rect.Width(),
							   XTP_DPI_Y(1), m_cr3DHighLight);
		}

		if (m_bExpanded && !m_strExpandedInformation.IsEmpty() && IsExpandFooterArea()
			&& !m_rcExpandedInformation.IsRectEmpty())
		{
			pDC->FillSolidRect(rect.left, m_rcExpandedInformation.top - m_ptMargin.y, rect.Width(),
							   XTP_DPI_Y(1), m_cr3DLight);
...
void CXTPTaskDialogClient::DrawIcon(CDC* pDC, HICON hIcon, CPoint pt, CSize sz)
{
	...
	if (dwLayout & LAYOUT_RTL)
		XTPDrawHelpers()->SetLayout(pDC, dwLayout | LAYOUT_BITMAPORIENTATIONPRESERVED);

	::DrawIconEx(*pDC, pt.x, pt.y, hIcon, sz.cx, sz.cy, 0, NULL, DI_NORMAL);

	if (dwLayout & LAYOUT_RTL)
		XTPDrawHelpers()->SetLayout(pDC, dwLayout);
}

void CXTPTaskDialogClient::DrawText(CDC* pDC, const CStringMarkup& strItemText, CRect rcItem,
									COLORREF clrText, CFont* pFont)
{
	if (!rcItem.IsRectEmpty() && !strItemText.IsEmpty())
	{
		if (strItemText.m_pUIElement)
		{
			...
			XTPMarkupRenderElement(strItemText.m_pUIElement, pDC->GetSafeHdc(),
								   strItemText.m_rcFinalRect);
		}
		else
		{
...
void CXTPTaskDialogClient::DrawExpandedInfo(CDC* pDC)
{
	...
	{
		DrawText(pDC, m_strExpandedInformation, m_rcExpandedInformation,
				 IsExpandFooterArea() ? m_crFooterText : m_crContentText, &m_xtpFont);
	}
}
...
void CXTPTaskDialogClient::DrawFootNote(CDC* pDC)
{
	...
	if (m_hIconFooter)
		DrawIcon(pDC, m_hIconFooter, CPoint(m_ptMargin.x + XTP_DPI_X(2), m_rcFooter.top),
				 m_sizeIconFooter);
