...
int CXTPTaskDialogLinkCtrl::DrawTextPartText(CDC* pDC, CString strBuffer, int x, int y,
											 LINKITEM* pItem)
{
	...
	pDC->DrawText(strBuffer, CRect(x, y, x + sz.cx + XTP_DPI_X(10), y + sz.cy),
...
void CXTPTaskDialogLinkCtrl::DrawTextPart(CDC* pDC, int& x, int& y, int nWidth, CString strBuffer,
										  LINKITEM* pItem)
{
	CXTPFontDC fontDC(pDC, pItem && m_xtpFontUL.GetSafeHandle() ? &m_xtpFontUL : GetFont(),
					  pItem ? m_crTextLink : m_crText);
...
void CXTPTaskDialogLinkCtrl::DrawText(CDC* pDC, CRect rClient)
{
	...
	for (int i = 0; i < m_arrLinks.GetSize(); i++)
	{
		...
		int iStart = XTPToIntChecked(pItem->nStart);
...
BOOL CXTPTaskDialogLinkCtrl::Create(const CRect& rect, CString& strBuffer, CFont* pFont,
									CWnd* pParentWnd)
{
	RemoveAllLinks();
...
DWORD CXTPTaskDialogLinkCtrl::GetLinkPos(CString strBuffer, int iPos /*= 0*/)
{
	...
	return static_cast<DWORD>(MAKELONG(iStart, (iEnd + 4) - iStart));
}
...
BOOL CXTPTaskDialogLinkCtrl::ExtractLinks(CString& strBuffer)
{
	...
	for (DWORD dwPos = GetLinkPos(strBuffer); dwPos;)
	{
		...
		pItem->nStart = XTPToDWORDChecked(iStart);
...
void CXTPTaskButtonTheme::DrawButton(CDC* pDC, CXTPButton* pButton)
{
	...
	if (::GetFocus() == pButton->GetSafeHwnd()
		&& ((pButton->SendMessage(WM_QUERYUISTATE) & UISF_HIDEFOCUS) == 0))
	{
...
BOOL CXTPTaskButtonTheme::DrawWinThemeBackground(CDC* pDC, CXTPButton* pButton)
{
	if (m_bThemeReady)
	{
		...
		if (m_themeButton->IsAppThemeActive() && XTPSystemVersion()->IsWinVistaOrGreater())
		{
			...
			int iStateId = (pButton->IsPushed())
							   ? PBS_PRESSED
							   : pButton->IsHighlighted()
									 ? PBS_HOT
									 : pButton->GetButtonStyle() == BS_DEFPUSHBUTTON ? PBS_DEFAULTED
																					 : PBS_NORMAL;

			if (!pButton->IsWindowEnabled())
				iStateId = PBS_DISABLED;

			if (SUCCEEDED(m_themeButton->DrawThemeBackground(pDC->GetSafeHdc(), 6, iStateId,
															 &rcButton, NULL)))
			{
				return TRUE;
			}
		...
		if (bSelected)
		{
			...
			rcItem.DeflateRect(1, 1);
			XTPDrawHelpers()->DrawLine(pDC, rcItem.left, rcItem.top, rcItem.left + rcItem.Width(),
									   rcItem.top, m_crBorder3DShadow);
...
void CXTPTaskButtonTheme::DrawButtonIcon(CDC* pDC, CXTPButton* pButton)
{
	...
	if (pIcon != NULL)
	{
		...
		CPoint point(XTP_DPI_X(7), m_ptMargin.y - pButton->GetImageSize().cy / 2 + size.cy / 2);

		if (!pButton->IsWindowEnabled())
		{
			pIcon->Draw(pDC, point, xtpImageDisabled, pButton->GetImageSize());
		}
		else if (pButton->IsHighlighted())
		{
			pIcon->Draw(pDC, point, xtpImageHot, pButton->GetImageSize());
		}
		else
		{
			pIcon->Draw(pDC, point, xtpImageNormal, pButton->GetImageSize());
		}
	}
}
...
void CXTPTaskButtonTheme::DrawButtonText(CDC* pDC, CXTPButton* pButton)
{
	...
	if (pButton->GetMarkupUIElement())
	{
		XTPMarkupSetDefaultFont(pButton->GetMarkupContext(),
								m_pFontTitle ? (HFONT)m_pFontTitle->GetSafeHandle()
											 : (HFONT)pFontNote->GetSafeHandle(),
								COLORREF_NULL);

		sizeCaptText = XTPMarkupMeasureElement(pButton->GetMarkupUIElement(), rcText.Width(),
											   INT_MAX);
	}
...
/*
CPoint CXTPTaskButtonTheme::CalculateImagePosition(CDC* pDC, UINT nState, CRect& rcItem, bool
bHasPushedImage, CXTPButton* pButton)
{
	//CPoint point = CXTPButtonTheme::CalculateImagePosition(pDC, nState, rcItem, bHasPushedImage,
pButton); CPoint point = 0;
...
void CXTPTaskButtonThemeExpando::DrawButton(CDC* pDC, CXTPButton* pButton)
{
	...
	if (m_themeButton->IsAppThemeActive())
	{
		...
		if (themeTaskDialog.IsAppThemeActive())
		{
			...
			if (SUCCEEDED(themeTaskDialog.DrawThemeBackground(pDC->GetSafeHdc(), 13, iStateId,
															  &rcButton, NULL)))
			{
				return;
			}
		...
		int iStateId = pButton->IsPushed() ? PBS_PRESSED
										   : pButton->IsHighlighted() ? PBS_HOT : PBS_NORMAL;

		if (SUCCEEDED(m_themeButton->DrawThemeBackground(pDC->GetSafeHdc(), BP_PUSHBUTTON, iStateId,
														 &rc, NULL)))
		{
			bDrawStandard = FALSE;
			pt.x += XTP_DPI_X(1);
	...
	if (bDrawStandard)
	{
		pDC->DrawFrameControl(&rcItem, DFC_BUTTON,
							  UINT((pButton->IsPushed() ? DFCS_PUSHED : 0) | DFCS_BUTTONPUSH));
		if (pButton->IsPushed())
			pt.Offset(1, 1);
	}
	...
	XTPPrimitiveDrawer()->DrawSymbol(pDC,
									 !pButton->GetChecked() ? xtpPrimitiveSymbolPlayDown
