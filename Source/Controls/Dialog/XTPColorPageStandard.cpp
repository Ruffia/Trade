...
void AFX_CDECL CXTPColorHex::CRgnCell::ComputeHexagon(const CTX_DATA::HEXAGON_DATA& data,
...
void AFX_CDECL CXTPColorHex::CRgnCell::ConvertFloatToIntPoints(const CXTPPoint2f (&from)[6],
															   CPoint (&to)[6])
{
...
CXTPColorHex::CRgnCell::CRgnCell(const CTX_DATA& data, CXTPRectF& rect, COLORREF color, float u,
								 float d)
	: m_color(color)
{
...
CXTPColorHex::CRgnCell::CRgnCell(const CTX_DATA& data, CXTPRectF& rect, COLORREF color, float u,
								 float d, float x, float y, BOOL bLarge /*= FALSE*/)
	: m_color(color)
{
	m_iCell[0] = static_cast<int>(u);
	m_iCell[1] = static_cast<int>(d);
...
BOOL CXTPColorHex::CRgnCell::DrawSelection(CDC* pDC)
{
	...
	if (pDC->FrameRgn(this, &brush, 4, 2))
	{
		...
		for (i = 0; i < 6; ++i)
		{
			...
			pDC->MoveTo(static_cast<int>(m_pts[i].X), static_cast<int>(m_pts[i].Y));
			pDC->LineTo(static_cast<int>(m_pts[toIndex].X), static_cast<int>(m_pts[toIndex].Y));
...
void CXTPColorHex::OnPaint()
{
	...
	if (m_bmpPicker.GetSafeHandle() == NULL)
	{
		...
		m_bmpPicker.CreateCompatibleBitmap(&memDC, rc.Width(), rc.Height());
...
void CXTPColorHex::CreateCellArray()
{
	...
	ctxData.sm.width = (static_cast<float>(XTP_CELL_WIDTH) * static_cast<float>(nDPI))
					   / XTP_DEFAULT_DISPLAY_DPI;
	ctxData.sm.half = ctxData.sm.width / 2;
	ctxData.sm.roof = TanPi6 * ctxData.sm.half;
	ctxData.sm.side = static_cast<float>(
		sqrt(ctxData.sm.half * ctxData.sm.half + ctxData.sm.roof * ctxData.sm.roof));
	...
	float x  = static_cast<float>(static_cast<float>(rcClient.Width()) - ctxData.sm.width * 13) / 2;
	float y  = 10;
...
#define XOFFSET(x, cx) (x + ((ctxData.sm.width / 2) * cx))
	...
	y += dy; // row 10
	m_rgnList.AddTail(
		new CRgnCell(ctxData, rect, RGB(0x3d, 0x80, 0x00), 82, 103, XOFFSET(x, 3), y));
	...
	y += dy; // row 11
	m_rgnList.AddTail(
		new CRgnCell(ctxData, rect, RGB(0x56, 0x80, 0x00), 93, 112, XOFFSET(x, 4), y));
	...
	y += dy; // row 12
	m_rgnList.AddTail(
		new CRgnCell(ctxData, rect, RGB(0x6b, 0x80, 0x00), 103, 120, XOFFSET(x, 5), y));
	...
	y += dy; // row 13
	m_rgnList.AddTail(
		new CRgnCell(ctxData, rect, RGB(0x80, 0x80, 0x00), 112, 121, XOFFSET(x, 6), y));
	...
	m_rgnList.AddTail(
		new CRgnCell(ctxData, rect, RGB(0xf8, 0xf8, 0xf8), 127, 129, XOFFSET(x, 5), sy1));
	m_rgnList.AddTail(
		new CRgnCell(ctxData, rect, RGB(0xea, 0xea, 0xea), 128, 130, XOFFSET(x, 6), sy2));
	m_rgnList.AddTail(
		new CRgnCell(ctxData, rect, RGB(0xdd, 0xdd, 0xdd), 129, 131, XOFFSET(x, 5) + sx, sy1));
	m_rgnList.AddTail(
		new CRgnCell(ctxData, rect, RGB(0xc0, 0xc0, 0xc0), 130, 132, XOFFSET(x, 6) + sx, sy2));
	m_rgnList.AddTail(
		new CRgnCell(ctxData, rect, RGB(0xb2, 0xb2, 0xb2), 131, 133, XOFFSET(x, 5) + sx * 2, sy1));
	m_rgnList.AddTail(
		new CRgnCell(ctxData, rect, RGB(0x96, 0x96, 0x96), 132, 134, XOFFSET(x, 6) + sx * 2, sy2));
	m_rgnList.AddTail(
		new CRgnCell(ctxData, rect, RGB(0x80, 0x80, 0x80), 133, 135, XOFFSET(x, 5) + sx * 3, sy1));
	m_rgnList.AddTail(
		new CRgnCell(ctxData, rect, RGB(0x77, 0x77, 0x77), 134, 136, XOFFSET(x, 6) + sx * 3, sy2));
	m_rgnList.AddTail(
		new CRgnCell(ctxData, rect, RGB(0x5f, 0x5f, 0x5f), 135, 137, XOFFSET(x, 5) + sx * 4, sy1));
	m_rgnList.AddTail(
		new CRgnCell(ctxData, rect, RGB(0x4d, 0x4d, 0x4d), 136, 138, XOFFSET(x, 6) + sx * 4, sy2));
	m_rgnList.AddTail(
		new CRgnCell(ctxData, rect, RGB(0x33, 0x33, 0x33), 137, 139, XOFFSET(x, 5) + sx * 5, sy1));
	m_rgnList.AddTail(
		new CRgnCell(ctxData, rect, RGB(0x29, 0x29, 0x29), 138, 140, XOFFSET(x, 6) + sx * 5, sy2));
	m_rgnList.AddTail(
		new CRgnCell(ctxData, rect, RGB(0x1c, 0x1c, 0x1c), 139, 141, XOFFSET(x, 5) + sx * 6, sy1));
	m_rgnList.AddTail(
		new CRgnCell(ctxData, rect, RGB(0x11, 0x11, 0x11), 140, 142, XOFFSET(x, 6) + sx * 6, sy2));
	m_rgnList.AddTail(
		new CRgnCell(ctxData, rect, RGB(0x08, 0x08, 0x08), 141, 143, XOFFSET(x, 5) + sx * 7, sy1));

	// large black cell
	m_rgnList.AddTail(
		new CRgnCell(ctxData, rect, RGB(0x00, 0x00, 0x00), 142, -1, XOFFSET(x, 22), y, TRUE));
...
BOOL CXTPColorHex::UpdateSelection(CRgnCell* pCell, BOOL bNotify /*=TRUE*/)
{
	if (pCell)
	{
		...
		if (m_clrColor != color)
		{
			...
			if (pWnd && bNotify)
			{
				pWnd->SendMessage(WM_XTP_UPDATECOLOR, (WPARAM)m_clrColor, (LPARAM)m_hWnd);
			}
		}
	}
	else
...
void CXTPColorHex::OnLButtonDblClk(UINT nFlags, CPoint point)
{
	...
	if (UpdateSelection(point))
	{
		...
		if (pWnd)
		{
			pWnd->SendMessage(WM_XTP_COLORDBLCLK, (WPARAM)m_clrColor,
							  (LPARAM)MAKELPARAM(LOWORD(point.x), HIWORD(point.y)));
		}
	}
