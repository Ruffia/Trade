...
static void XTPTaskDialogSetConfigText(UNALIGNED PCWSTR& lpszConfig, LPCTSTR strText)
{
	if (lpszConfig)
	{
		if (!XTP_IS_INTRESOURCE(lpszConfig))
	...
	lpszConfig = (PCWSTR) new WCHAR[XTPToUIntChecked(nLength + 1)];

#ifdef _UNICODE
	STRCPY_S((LPWSTR)lpszConfig, XTPToSizeTChecked(nLength + 1), strText);
#else
...
HRESULT CALLBACK CXTPTaskDialog::TaskDialogCallbackProc(HWND hwnd, UINT uNotification,
...
HRESULT CXTPTaskDialog::OnDialogNotify(UINT uNotification, WPARAM wParam, LPARAM lParam)
{
	...
	switch (uNotification)
	{
		...
		{
			TRACE0(
				"CXTPTaskDialog::TaskDialogCallbackProc -- Unhandled notification encountered.\n");
			return E_NOTIMPL;
		}
	}
	return hResult;
...
void CXTPTaskDialog::SetWindowTitle(LPCTSTR pszText)
{
	...
	if (pszText && XTP_IS_INTRESOURCE(pszText))
	{
...
BOOL CXTPTaskDialog::SetMainInstruction(LPCTSTR pszText)
{
	...
	if (::IsWindow(m_hwndTaskDialog))
	{
		return (::SendMessage(m_hwndTaskDialog, TDM_SET_ELEMENT_TEXT, TDE_MAIN_INSTRUCTION,
							  (LPARAM)(LPCWSTR)m_config.pszMainInstruction)
				!= 0);
	}
...
BOOL CXTPTaskDialog::SetContent(LPCTSTR pszText)
{
	...
	if (::IsWindow(m_hwndTaskDialog))
	{
		return (::SendMessage(m_hwndTaskDialog, TDM_SET_ELEMENT_TEXT, TDE_CONTENT,
							  (LPARAM)(LPCWSTR)m_config.pszContent)
				!= 0);
	}
...
BOOL CXTPTaskDialog::SetExpandedInformation(LPCTSTR pszText)
{
	...
	if (::IsWindow(m_hwndTaskDialog))
	{
		return (::SendMessage(m_hwndTaskDialog, TDM_SET_ELEMENT_TEXT, TDE_EXPANDED_INFORMATION,
							  (LPARAM)(LPCWSTR)m_config.pszExpandedInformation)
				!= 0);
	}
...
BOOL CXTPTaskDialog::SetFooter(LPCTSTR pszText)
{
	...
	if (::IsWindow(m_hwndTaskDialog))
	{
		return (::SendMessage(m_hwndTaskDialog, TDM_SET_ELEMENT_TEXT, TDE_FOOTER,
							  (LPARAM)(LPCWSTR)m_config.pszFooter)
				!= 0);
	}
...
BOOL CXTPTaskDialog::SetMainIcon(HICON hIcon)
{
	...
	if (::IsWindow(m_hwndTaskDialog))
	{
		return (::SendMessage(m_hwndTaskDialog, TDM_UPDATE_ICON, TDIE_ICON_MAIN, (LPARAM)hIcon)
				!= 0);
	}
...
{
	ASSERT(XTP_IS_INTRESOURCE(pszResource));
	...
	{
		return (
			::SendMessage(m_hwndTaskDialog, TDM_UPDATE_ICON, TDIE_ICON_MAIN, (LPARAM)pszResource)
			!= 0);
	}
...
BOOL CXTPTaskDialog::SetFooterIcon(HICON hIcon)
{
	...
	if (::IsWindow(m_hwndTaskDialog))
	{
		return (::SendMessage(m_hwndTaskDialog, TDM_UPDATE_ICON, TDIE_ICON_FOOTER, (LPARAM)hIcon)
				!= NULL);
	}
...
BOOL CXTPTaskDialog::SetFooterIcon(LPCWSTR pszResource)
{
	...
	{
		return (
			::SendMessage(m_hwndTaskDialog, TDM_UPDATE_ICON, TDIE_ICON_FOOTER, (LPARAM)pszResource)
			!= 0);
	}
...
INT_PTR CXTPTaskDialog::DoModal()
{
	...
	if (bUseComCtl32 && XTPSystemVersion()->IsWinVistaOrGreater())
	{
		...
		if (modComCtl32Dll.LoadLibrary(_T("ComCtl32.dll"))
			&& modComCtl32Dll.GetProcAddress((FARPROC*)&pfnTaskDialogIndirect, "TaskDialogIndirect",
											 MAKELONG(0, 6)))
		{
			PCWSTR pswModifiedMainIcon   = 0;
			PCWSTR pswModifiedFooterIcon = 0;

			if (((m_config.dwFlags & TDF_USE_HICON_MAIN) == 0)
				&& (m_config.pszMainIcon == TD_QUESTION_ICON))
			{
				m_config.dwFlags |= TDF_USE_HICON_MAIN;
				m_config.hMainIcon  = LoadSysIcon(TD_QUESTION_ICON,
												  CSize(GetSystemMetrics(SM_CXICON),
														GetSystemMetrics(SM_CYICON)));
				pswModifiedMainIcon = TD_QUESTION_ICON;
			}

			if (((m_config.dwFlags & TDF_USE_HICON_FOOTER) == 0)
				&& (m_config.pszFooterIcon == TD_QUESTION_ICON))
			{
				m_config.dwFlags |= TDF_USE_HICON_FOOTER;
				m_config.hFooterIcon  = LoadSysIcon(TD_QUESTION_ICON,
													CSize(GetSystemMetrics(SM_CXSMICON),
			...
			if (FAILED(pfnTaskDialogIndirect(&m_config, &m_nSelButtonID, &m_nSelRadioButtonID,
											 &m_bVerification)))
			{
				nSelButtonID = 0;
			}
...
void CXTPTaskDialog::SetCommonButtons(BOOL bOk, BOOL bYes, BOOL bNo, BOOL bCancel, BOOL bRetry,
									  BOOL bClose)
{
	m_config.dwCommonButtons = 0;
...
AFX_INLINE CString GetPropExchangeString(CXTPPropExchange* pPX, LPCTSTR lpsz, LPCTSTR const* rglpsz,
										 int nString)
{
	CString str;
...
BOOL CXTPTaskDialog::CreateFromResourceParam(HINSTANCE hInstance, UINT nIDResource,
											 LPCTSTR lpszDialog, LPCTSTR const* rglpsz, int nString)
{
...
BOOL CXTPTaskDialog::CreateFromResourceParam(CXTPPropExchangeXMLNode* pPX, LPCTSTR const* rglpsz,
											 int nString)
{
	if (!pPX)
		return FALSE;
	...
	while (pos)
	{
		...
		else if (strSection.CompareNoCase(_T("CommandLink")) == 0 && (nID != 0))
		{
			...
			CString strExplanation = GetPropExchangeString(&secButton, _T("Explanation"), rglpsz,
														   nString);
	...
	if (pPX->IsSectionExists(_T("ExpandedInformation")))
	{
		...
		SetExpandedInformation(
			GetPropExchangeString(&pxExpandedInformation, NULL, rglpsz, nString));
