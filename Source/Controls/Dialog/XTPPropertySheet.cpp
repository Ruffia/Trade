...
{
	return m_hWnd ? m_pActivePage : GetPage(XTPToIntChecked(m_psh.nStartPage));
}

int CXTPPropertySheet::GetActiveIndex() const
{
	return m_hWnd ? GetPageIndex(m_pActivePage) : XTPToIntChecked(m_psh.nStartPage);
}
...
void CXTPPropertySheet::CommonConstruct(CWnd* pParentWnd, UINT iSelectPage)
{
	...
	m_rcMinPage.SetRectEmpty();
	m_rcMaxPage.SetRectEmpty();
	m_bAllowApply = TRUE;
	m_bModeless   = FALSE;
	m_bResizable  = FALSE;
	...
	m_lpDlgTemplate->style = WS_POPUP | WS_BORDER | WS_CAPTION | WS_SYSMENU | DS_NOIDLEMSG
							 | DS_3DLOOK | DS_MODALFRAME | WS_CLIPCHILDREN | WS_CLIPSIBLINGS;
	...
	if (m_pParentWnd && m_pParentWnd->GetExStyle() & WS_EX_LAYOUTRTL)
	{
...
{
	return style == xtpPageBorderFrame
			   ? CSize(4, 4)
...
void CXTPPropertySheet::SetPageSizeMin(CSize szPage)
{
	m_rcMinPage = CRect(0, 0, szPage.cx, szPage.cy);
}

void CXTPPropertySheet::SetPageSizeMax(CSize szPage)
{
	m_rcMaxPage = CRect(0, 0, szPage.cx, szPage.cy);
}
...
BOOL CXTPPropertySheet::Create(CWnd* pParentWnd, DWORD dwStyle, DWORD dwExStyle)
{
	if (dwStyle == (DWORD)-1)
	{
		dwStyle = DS_MODALFRAME | DS_3DLOOK | WS_POPUP | WS_VISIBLE | WS_CAPTION | WS_CLIPCHILDREN
				  | WS_CLIPSIBLINGS;
	...
	return CXTPDialogEx::CreateDlgIndirect(m_lpDlgTemplate, pParentWnd, NULL);
}
...
{
	CXTPDialogEx::DoDataExchange(pDX);
...
{
	if (CXTPDialogEx::OnCreate(lpCreateStruct) == -1)
		return -1;
...
#ifdef _XTP_INCLUDE_COMMANDBARS
	if (GetNavigator()->GetTheme() == xtpControlThemeResource
		|| GetNavigator()->GetTheme() >= xtpControlThemeOffice2013)
	{
		InitCommandBars();
		GetCommandBars()->EnableFrameTheme();

		m_scSizeIcon.SetTheme(GetNavigator()->GetTheme());

#	ifdef _XTP_ACTIVEX
		if (GetNavigator()->GetTheme() == xtpControlThemeOffice2013)
			GetCommandBars()->SetTheme(xtpThemeOffice2013);

		if (GetNavigator()->GetTheme() == xtpControlThemeOffice2016)
			GetCommandBars()->SetTheme(xtpThemeOffice2016);

		if (GetNavigator()->GetTheme() == xtpControlThemeVisualStudio2015)
			GetCommandBars()->SetTheme(xtpThemeVisualStudio2015);

		if (GetNavigator()->GetTheme() == xtpControlThemeVisualStudio2017)
			GetCommandBars()->SetTheme(xtpThemeVisualStudio2017);

		if (GetNavigator()->GetTheme() == xtpControlThemeVisualStudio2019)
			GetCommandBars()->SetTheme(xtpThemeVisualStudio2019);

		if (GetNavigator()->GetTheme() == xtpControlThemeVisualStudio2022)
			GetCommandBars()->SetTheme(xtpThemeVisualStudio2022);

		if (GetNavigator()->GetTheme() == xtpControlThemeNativeWindows10)
...
{
	CRect rcParent = IsPositionRelativeToWindow() ? CXTPWindowRect(m_pParentWnd)
												  : XTPMultiMonitor()->GetWorkArea(m_pParentWnd);
...
CXTPButton* CXTPPropertySheet::CreateButton()
{
	return new CXTPButton();
}
...
BOOL CXTPPropertySheet::AddButton(UINT nIDCaption, UINT nID)
{
	...
	CXTPButton* pButton = CreateButton();

	if (!pButton->Create(strButton, WS_CHILD | BS_PUSHBUTTON | WS_TABSTOP | WS_VISIBLE,
...
void CXTPPropertySheet::PageInfoChanged(CXTPPropertyPage* pPage, BOOL bModified)
{
	...
	if (m_bAllowApply && GetDlgItem(ID_APPLY_NOW))
		GetDlgItem(ID_APPLY_NOW)->EnableWindow(bModified);
}
...
void CXTPPropertySheet::RecalcLayout(LPSIZE pPageSize, BOOL bMoveWindow /*= FALSE*/)
{
	...
	CRect rcPage;
	CSize szClient;
	m_pNavigator->PageToSize(&layout, pPageSize ? CSize(*pPageSize) : m_rcPage.Size(), rcPage,
							 szClient);

	if (m_arrButtons.GetSize() > 0)
	{
		MoveButtons(&layout, szClient);
		szClient.cy += m_szButton.cy + m_pNavigator->m_rcButtonMargin.top
					   + m_pNavigator->m_rcButtonMargin.bottom;
	}
	else
	{
		m_szButton = CSize(0, 0);
	...
	{
		CRect rcSizeIcon(szClient.cx - GetSystemMetrics(SM_CXVSCROLL),
						 szClient.cy - GetSystemMetrics(SM_CYHSCROLL), szClient.cx, szClient.cy);
...
BOOL CXTPPropertySheet::CreateClient()
{
	...
	{
		CXTPPropertyPage* pPage = m_arrPages[XTPToIntPtrChecked(i)];
...
BOOL CXTPPropertySheet::SetActivePage(int nPage)
{
	if (m_hWnd == NULL)
	{
		m_psh.nStartPage = XTPToUIntChecked(nPage);
		return TRUE;
	}
...
BOOL CXTPPropertySheet::SetActivePage(CXTPPropertyPage* pPage)
{
	...
	if (!m_hWnd)
	{
		m_psh.nStartPage = XTPToUIntChecked(GetPageIndex(pPage));
		return TRUE;
	}
	...
	pPage->SetWindowPos(&CWnd::wndTop, m_rcPage.left, m_rcPage.top, m_rcPage.Width(),
...
BOOL CXTPPropertySheet::OnButtonClick(UINT nButton)
{
	...
	switch (nButton)
	{
		case IDOK: lParam = TRUE; bExit = TRUE;
...
LRESULT CXTPPropertySheet::WindowProc(UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	return CXTPDialogEx::WindowProc(message, wParam, lParam);
}
...
LRESULT CXTPPropertySheet::SendPageNotify(CXTPPropertyPage* pPage, int code, LPARAM lParam)
{
	...
	pshn.hdr.idFrom   = XTPToUIntPtr(GetDlgCtrlID());
	pshn.hdr.code	 = XTPToUInt(code);
...
BOOL CXTPPropertySheet::OnCommand(WPARAM wParam, LPARAM lParam)
{
	...
	if (hWndCtrl != NULL && nCode == BN_CLICKED)
	{
		if (::SendMessage(hWndCtrl, WM_GETDLGCODE, 0, 0) & (DLGC_BUTTON | DLGC_DEFPUSHBUTTON))
		{
			...
			if (lStyle == BS_PUSHBUTTON || lStyle == BS_DEFPUSHBUTTON || lStyle == BS_USERBUTTON
				|| lStyle == BS_OWNERDRAW)
			{
				m_nModalResult = XTPToInt(nID);
			}
		}
	}
...
BOOL CXTPPropertySheet::PreTranslateMessage(MSG* pMsg)
{
	...
	if (pMsg->message < WM_KEYFIRST || pMsg->message > WM_KEYLAST)
		return CXTPDialogEx::PreTranslateMessage(pMsg);

	HWND hWndFocusOld = ::GetFocus();

	if (!CXTPDialogEx::PreTranslateMessage(pMsg))
		return FALSE;
...
{
	CXTPDialogEx::OnSize(nType, cx, cy);
	...
	if (/*m_szWindow != szWindow &&*/ m_bResizable && m_pActivePage)
	{
		...
		{
			CRect rcSizeIcon(szClient.cx - GetSystemMetrics(SM_CXVSCROLL),
							 szClient.cy - GetSystemMetrics(SM_CYHSCROLL), szClient.cx,
							 szClient.cy);
			AfxRepositionWindow(&layout, m_scSizeIcon, rcSizeIcon);
		}
...
void CXTPPropertySheet::SetTheme(XTPControlTheme theme)
{
	if (theme == xtpControlThemeResource)
	{
		GetNavigator()->SetTheme(theme);

		m_brBack.CreateSolidBrush(
			XTPIniColor(_T("Ribbon"), _T("RibbonFace"), GetXtremeColor(COLOR_WINDOW)));
	}

	if (theme >= xtpControlThemeOffice2013)
	{
		GetNavigator()->SetTheme(theme);

		m_brBack.CreateSolidBrush(
			XTPIniColor(_T("CommandBars"), _T("Back"), GetXtremeColor(COLOR_WINDOW)));
	}
}

XTPControlTheme CXTPPropertySheet::GetTheme()
{
	return GetNavigator()->GetTheme();
}

BOOL CXTPPropertySheet::OnEraseBkgnd(CDC* pDC)
{
	if (m_brBack.GetSafeHandle())
	{
		CXTPClientRect rc(this);
		::FillRect(pDC->GetSafeHdc(), rc, m_brBack);
		return TRUE;
	}

	return CXTPDialogEx::OnEraseBkgnd(pDC);
}

void CXTPPropertySheet::OnGetMinMaxInfo(MINMAXINFO* pMMI)
{
	// TODO: Add your message handler code here and/or call default

	if (!m_rcMinPage.IsRectEmpty())
	{
		pMMI->ptMinTrackSize.x = m_rcMinPage.Width();
		pMMI->ptMinTrackSize.y = m_rcMinPage.Height();
	}

	if (!m_rcMaxPage.IsRectEmpty())
	{
		pMMI->ptMaxTrackSize.x = m_rcMaxPage.Width();
		pMMI->ptMaxTrackSize.y = m_rcMaxPage.Height();
	}

	CXTPDialogEx::OnGetMinMaxInfo(pMMI);
}
