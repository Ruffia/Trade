...
CXTPTipOfTheDay::CXTPTipOfTheDay(LPCTSTR lpszTipFile /*= NULL*/, CWnd* pParent /*= NULL*/)
{
	...
	CXTPResourceManager::AssertValid(
		XTPResourceManager()->LoadString(&m_strTipTitle, XTP_IDS_DIDYOUKNOW));
	...
	m_pLightBulbIcon = 0;
	{
		...
		iconHandle.Attach(
			CXTPImageManagerIcon::LoadBitmapFromResource(MAKEINTRESOURCE(XTP_IDI_LIGHTBULB), NULL),
			FALSE);
		...
		{
			m_pLightBulbIcon = new CXTPImageManagerIcon(0, iconHandle.GetExtent().cx,
														iconHandle.GetExtent().cy);
...
CXTPTipOfTheDay::~CXTPTipOfTheDay()
{
	...
	CXTPRegistryManager regManager;
	regManager.WriteProfileInt(szSection, szIntFilePos, XTPToIntChecked(m_nNextTip));
...
void CXTPTipOfTheDay::OnPaint()
{
	...
	if (m_bClassicStyle)
	{
		...
		dc.FillSolidRect(rcBorder.left + XTP_DPI_X(50), rcBorder.top,
						 rcBorder.Width() - XTP_DPI_X(50), rcBorder.Height(),
						 GetXtremeColor(COLOR_WINDOW));
		dc.FillSolidRect(rcBorder.left, rcBorder.top, XTP_DPI_X(50), rcBorder.Height(),
						 RGB(128, 128, 128));
		...
		if (m_pLightBulbIcon)
		{
			m_pLightBulbIcon->Draw(&dc,
								   CPoint(rcBorder.left + XTP_DPI_X(8),
										  rcBorder.top + XTP_DPI_Y(8)),
		...
		dc.TextOut(XTP_DPI_X(52) + XTP_DPI_X(10) + XTP_DPI_X(5), XTP_DPI_Y(14) + XTP_DPI_Y(5),
				   m_strTipTitle);
	}
	else
	{
		dc.FillSolidRect(m_rcBorder, GetXtremeColor(COLOR_WINDOW));
		dc.FillSolidRect(m_rcBorder.left, m_rcBorder.bottom, m_rcBorder.Width(), 1,
						 GetXtremeColor(COLOR_3DSHADOW));

		// Draw the light bulb bitmap to the screen.
		if (m_pLightBulbIcon)
		{
			m_pLightBulbIcon->Draw(&dc, XTP_DPI(CPoint(8, 8)),
								   XTP_DPI(m_pLightBulbIcon->GetExtent()));
		}
	...
	if (m_pMarkupTipText)
	{
		XTPMarkupSetDefaultFont(m_pMarkupContext, (HFONT)m_pFontTip->GetSafeHandle(),
								dc.GetTextColor());
...
BOOL CXTPTipOfTheDay::OnInitDialog()
{
	...
	if (m_pStream)
	{
		if (fseek(m_pStream, XTPToLongChecked(m_nNextTip), SEEK_SET) != 0)
		{
	...
	if (!m_bClassicStyle)
	{
		m_rcTipText.SetRect(XTP_DPI_X(52), XTP_DPI_Y(44), m_rcBorder.right - XTP_DPI_X(6),
							m_rcBorder.bottom - XTP_DPI_Y(6));
	}
	else
	{
		m_rcTipText.SetRect(XTP_DPI_X(52) + XTP_DPI_X(10) + XTP_DPI_X(5),
							XTP_DPI_Y(44) + XTP_DPI_Y(5),
							m_rcBorder.right - XTP_DPI_X(6) - XTP_DPI_X(10),
							m_rcBorder.bottom - XTP_DPI_Y(6));
	}
...
void CXTPTipOfTheDay::GetNextTipString(CString& strNext)
{
	if (!m_pStream)
	{
		...
		{
			strNext = m_arrTips[XTPToIntPtrChecked(m_nNextTip) % m_arrTips.GetSize()];
		...
		CXTPResourceManager::AssertValid(
			XTPResourceManager()->LoadString(&strNext, XTP_IDS_FILE_ABSENT));
		return;
	}
	...
	m_nNextTip = XTPToUIntChecked(ftell(m_pStream));
}
