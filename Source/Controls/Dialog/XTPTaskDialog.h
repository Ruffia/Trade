...
public:
	...
	// Parameters:
	//     pWndParent - Pointer to the parent or owner window object.
	//-----------------------------------------------------------------------
	CXTPTaskDialog(CWnd* pWndParent = NULL);

	//-----------------------------------------------------------------------
	// Summary:
	//     Destroys a CXTPTaskDialog object, handles cleanup and deallocation.
	//-----------------------------------------------------------------------
	virtual ~CXTPTaskDialog();

	//-----------------------------------------------------------------------
	// Summary:
	//     Call this member function to set the window title for the task dialog.
	// Parameters:
	...
	//     macro. If this parameter is NULL, then the filename of the executable
	//     program is used.
	// See Also:
	...
	//     The parameter <i>pszText</i> can be either a NULL-terminated string
	//     or an integer resource identifier passed to the MAKEINTRESOURCE
	//     macro. If this parameter is NULL, then the verification checkbox is not
	...
	//-----------------------------------------------------------------------
	// Summary:
	//     Call this member function to set the content text for the task dialog.
	// Parameters:
	//     pszText - Pointer that references the string to be used for the
	...
	//                   dialog Main Instruction area. If this member is NULL,
	...
	//                   NULL, then no icon will be displayed.
	// Remarks:
	...
	//-----------------------------------------------------------------------
	// Summary:
	//     Call this member function to set the icon for the Footer area of the
	//     task dialog.
	// Parameters:
	//     hIcon       - A handle to an Icon that is to be displayed in the task
	//                   dialog footer area. If this member is NULL, then
	//                   no icon will be displayed.
	//     pszResource - Pointer that references the icon to be displayed in
	//                   the task dialog footer area. If this parameter
	//                   is NULL, then no icon will be displayed.
	// Remarks:
	//     The parameter <i>pszResource</i> must be an integer resource
	//     identifier passed to the MAKEINTRESOURCE macro, or one of the
	//     following predefined values:
	...
	//                   in dialog units. If 0, then task dialog will calculate
	//                   the ideal width.
	//     bPixelToDLU - TRUE indicates that the size specified by cxWidth are pixel
	...
	//     task dialog's command area.
	// Remarks:
	//     Call SetInputBoxText to set the default value for an input box field.
	// See Also:
	...
	// ----------------------------------------------------------------------
	// Summary:
	//     Call this member function to set the default value for an
	//     input box field.
	// Parameters:
	//     lpszInputBoxText - Text to display in the input box.
	// Remarks:
	//     Call AddInputBox to add a user input box to the task dialog's command area.
	// See Also:
	...
	//-----------------------------------------------------------------------
	// Summary:
	//     Call this member function to get the text entered by a user in
	//     an input box field.
	// Remarks:
	//     Call AddInputBox to add a user input box to the task dialog's command area.
	// See Also:
	...
	//-----------------------------------------------------------------------
	// Summary:
	//     Call this member function to indicate that the buttons specified with
	...
	//-----------------------------------------------------------------------
	// Summary:
	//     Call this method to enable markup for TaskDialog.
	// Parameters:
	...
	//     task dialog.
	// Parameters:
	//     bEnable - TRUE to allow dialog cancellation, otherwise FALSE.
	// Remarks:
	//     When <i>bAllow</i> is TRUE, this indicates that the dialog should
	//     be able to be closed using Alt-F4, Escape, and the title bar's
	//     close button even if no cancel button is specified by the
	...
	//     parent window.
	// Parameters:
	//     bEnable - TRUE to reposition the task dialog, otherwise FALSE.
	// Remarks:
	...
	//     is FALSE, or hWndParent is NULL, then the task dialog is positioned
	...
	//-----------------------------------------------------------------------
	// Summary:
	//     Call this member function to enable right-to-left (RTL) text display
	//     for the task dialog.
	// Parameters:
	...
	//     the AddButton member function, then the task dialog will contain
	...
	//
	//     * <b>TDCBF_OK_BUTTON</b>:     The task dialog contains the <b>OK</b> push button.
	//     * <b>TDCBF_YES_BUTTON</b>:    The task dialog contains the <b>Yes</b> push button.
	//     * <b>TDCBF_NO_BUTTON</b>:     The task dialog contains the <b>No</b> push button.
	//     * <b>TDCBF_CANCEL_BUTTON</b>: The task dialog contains the <b>Cancel</b> push button.
	//     * <b>TDCBF_RETRY_BUTTON</b>   The task dialog contains the <b>Retry</b> push button.
	//     * <b>TDCBF_CLOSE_BUTTON</b>:  The task dialog contains the <b>Close</b> push button.
	//
	//     If the Cancel button is specified, then the task dialog will respond to
	...
	//     the OK button by default.
	// Parameters:
	//     bOk       - TRUE if the task dialog contains the <b>OK</b> push button.
	//     bYes      - TRUE if the task dialog contains the <b>Yes</b> push button.
	//     bNo       - TRUE if the task dialog contains the <b>No</b> push button.
	//     bCancel   - TRUE if the task dialog contains the <b>Cancel</b> push button.
	//     bRetry    - TRUE if the task dialog contains the <b>Retry</b> push button.
	//     bClose    - TRUE if the task dialog contains the <b>Close</b> push button.
	// Remarks:
	//     Any of the following flags can be used with the dwButtons parameter:
	//
	//     * <b>TDCBF_OK_BUTTON</b>:     The task dialog contains the <b>OK</b> push button.
	...
	// Parameters:
	//     bUseComCtl32 - TRUE to use the ComCtl32.dll version of Task Dialog. This
	...
	//     * <b>IDCANCEL</b>: Cancel button was selected, Alt-F4 was pressed, Escape was
...
#	endif
	...
	// Parameters:
	//     bChecked     - TRUE to set the state of the checkbox to be checked, or
	//                    FALSE to set it to be unchecked.
	//     bSetKeyFocus - TRUE to set the keyboard focus to the checkbox, or
	//                    FALSE otherwise.
	// Remarks:
	//     This member function should only be called from a derived class,
	//     and only after the task dialog has been initialized.
	...
	//     bEnable   - TRUE to enable the radio button, FALSE to disable the radio button.
	// Remarks:
	//     This member function should only be called from a derived class,
	//     and only after the task dialog has been initialized.
	// Example:
	...
	//     bRequired - TRUE to designate that the action invoked by the button requires
	...
	// Parameters:
	//     bEnable - TRUE to enable the style, FALSE to disable.
	// ----------------------------------------------------------------------
	...
	//     rglpsz      - Array of strings that will be replaced with XML strings.
...
protected:
	...
	// Parameters:
	//     uNotification - Notification identifier.
...
AFX_INLINE void CXTPTaskDialog::ClickButton(int nButtonID)
{
	...
	::SendMessage(m_hwndTaskDialog, TDM_CLICK_BUTTON, XTPToWPARAM(nButtonID), 0);
}
AFX_INLINE void CXTPTaskDialog::ClickRadioButton(int nButtonID)
{
	ASSERT(::IsWindow(m_hwndTaskDialog));
	::SendMessage(m_hwndTaskDialog, TDM_CLICK_RADIO_BUTTON, XTPToWPARAM(nButtonID), 0);
}
...
{
	ASSERT(::IsWindow(m_hwndTaskDialog));
	::SendMessage(m_hwndTaskDialog, TDM_ENABLE_BUTTON, XTPToWPARAM(nButtonID), bEnable);
}
AFX_INLINE void CXTPTaskDialog::EnableRadioButton(int nButtonID, BOOL bEnable)
{
	ASSERT(::IsWindow(m_hwndTaskDialog));
	::SendMessage(m_hwndTaskDialog, TDM_ENABLE_RADIO_BUTTON, XTPToWPARAM(nButtonID), bEnable);
}
...
{
	ASSERT(::IsWindow(m_hwndTaskDialog));
	::SendMessage(m_hwndTaskDialog, TDM_SET_PROGRESS_BAR_STATE, XTPToWPARAM(nState), 0);
}
AFX_INLINE void CXTPTaskDialog::SetProgressBarPos(int nPosition)
{
	ASSERT(::IsWindow(m_hwndTaskDialog));
	::SendMessage(m_hwndTaskDialog, TDM_SET_PROGRESS_BAR_POS, XTPToWPARAM(nPosition), 0);
}
...
{
	ASSERT(::IsWindow(m_hwndTaskDialog));
	::SendMessage(m_hwndTaskDialog, TDM_SET_BUTTON_ELEVATION_REQUIRED_STATE, XTPToWPARAM(nButtonID),
				  bRequired);
}
