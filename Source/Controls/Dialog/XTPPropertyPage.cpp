...
CXTPPropertyPage::CXTPPropertyPage()
	: CPropertyPage()
	, CXTPResize(this, xtpResizeNoClipChildren | xtpResizeNoSizeIcon)
{
	CommonConstruct();
}
...
BOOL CXTPPropertyPage::MeasureTemplate(SIZE& szDlg)
{
	...
	do
	{
		...
		LPCTSTR lpDlgParentClassName =
			AfxRegisterWndClass(CS_VREDRAW | CS_HREDRAW, ::LoadCursor(NULL, IDC_ARROW),
...
HBRUSH CXTPPropertyPage::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
	...
	if ((nCtlColor == CTLCOLOR_BTN || nCtlColor == CTLCOLOR_DLG || nCtlColor == CTLCOLOR_STATIC)
		&& m_pSheet)
	{
		HBRUSH hPageBrush = m_pSheet->GetNavigator()->GetPageBrush(pDC, this);
		if (hPageBrush)
		{
			CXTPWindowRect rcPaint(pWnd);
			CXTPWindowRect rcBrush(m_hWnd);

			::SetBrushOrgEx(pDC->GetSafeHdc(), rcBrush.left - rcPaint.left,
							rcBrush.top - rcPaint.top, NULL);
...
void CXTPPropertyPage::GetScrollBarState(CSize sizeClient, CSize& needSb, CSize& sizeRange,
										 CPoint& ptMove, BOOL bInsideClient)
{
...
void CXTPPropertyPage::ScrollPage(int xAmount, int yAmount)
{
	...
	for (int i = 0; i < (int)m_arrItems.GetSize(); i++)
	{
		...
		rrcWindow.left += static_cast<XTP_RESIZE>(xAmount);
		rrcWindow.right += static_cast<XTP_RESIZE>(xAmount);
		rrcWindow.top += static_cast<XTP_RESIZE>(yAmount);
		rrcWindow.bottom += static_cast<XTP_RESIZE>(yAmount);
	}
}
...
void CXTPPropertyPage::UpdateBars()
{
	...
	if (needSb.cx)
	{
		info.nPage = XTPToUIntChecked(sizeClient.cx);
	...
	if (needSb.cy)
	{
		info.nPage = XTPToUIntChecked(sizeClient.cy);
...
BOOL CXTPPropertyPage::DoMouseWheel(UINT fFlags, short zDelta, CPoint point)
{
	...
	if (bHasVertBar)
	{
		bResult = OnScrollBy(CSize(0, (zDelta < 0 ? 1 : -1) * m_lineDev.cy
										  * XTPToIntChecked(uWheelScrollLines)),
							 TRUE);
	}
	else if (bHasHorzBar)
	{
		bResult = OnScrollBy(
			CSize((zDelta < 0 ? 1 : -1) * m_lineDev.cy * XTPToIntChecked(uWheelScrollLines), 0),
			TRUE);
	}
...
BOOL CXTPPropertyPage::OnScroll(UINT nScrollCode, UINT nPos, BOOL bDoScroll)
{
	...
	switch (LOBYTE(nScrollCode))
	{
		...
		case SB_BOTTOM: x = INT_MAX; break;
		case SB_LINEUP: x -= m_lineDev.cx; break;
		case SB_LINEDOWN: x += m_lineDev.cx; break;
		case SB_PAGEUP: x -= CXTPClientRect(this).Width(); break;
		case SB_PAGEDOWN: x += CXTPClientRect(this).Width(); break;
		case SB_THUMBTRACK: x = XTPToIntChecked(nPos); break;
	}
	...
	switch (HIBYTE(nScrollCode))
	{
		...
		case SB_BOTTOM: y = INT_MAX; break;
		case SB_LINEUP: y -= m_lineDev.cy; break;
		case SB_LINEDOWN: y += m_lineDev.cy; break;
		case SB_PAGEUP: y -= CXTPClientRect(this).Height(); break;
		case SB_PAGEDOWN: y += CXTPClientRect(this).Height(); break;
		case SB_THUMBTRACK: y = XTPToIntChecked(nPos); break;
	}
...
void CXTPPropertyPage::OnNcPaint()
{
	...
	COLORREF clrListBoxBack   = RGB(0, 0, 0);
	COLORREF clrListBoxBorder = RGB(0, 0, 0);
	BOOL bThemeEnabled = m_pSheet->GetNavigator()->GetTheme() != xtpControlThemeDefault ? TRUE
																						: FALSE;
	if (bThemeEnabled)
	{
		clrListBoxBack   = XTPIniColor(_T("Controls.ListBox"), _T("Back"),
									   GetXtremeColor(COLOR_WINDOW));
		clrListBoxBorder = XTPIniColor(_T("Controls.ListBox"), _T("Border"),
	...
	if (m_pSheet->GetPageBorderStyle() == xtpPageBorderFrame)
	{
		...
		if (wrpTreeView.IsAppThemeActive())
		{
			...
			{
				dc.Draw3dRect(rcPage, bThemeEnabled ? clrListBoxBorder : clr,
							  bThemeEnabled ? clrListBoxBorder : clr);
				rcPage.DeflateRect(1, 1);
				dc.Draw3dRect(rcPage, bThemeEnabled ? clrListBoxBack : GetXtremeColor(COLOR_WINDOW),
							  bThemeEnabled ? clrListBoxBack : GetXtremeColor(COLOR_WINDOW));
				rcPage.DeflateRect(1, 1);
				bFrame = TRUE;
			}
		}
	...
	{
		dc.FillSolidRect(rcPage.left, rcPage.bottom - 2, rcPage.Width(), 1,
						 bThemeEnabled ? clrListBoxBack : GetXtremeColor(COLOR_BTNSHADOW));
		dc.FillSolidRect(rcPage.left, rcPage.bottom - 1, rcPage.Width(), 1,
						 bThemeEnabled ? clrListBoxBorder : GetXtremeColor(COLOR_BTNHIGHLIGHT));
	...
	if ((GetStyle() & (WS_HSCROLL | WS_VSCROLL)) == (WS_HSCROLL | WS_VSCROLL))
	{
		CRect rcSizeIcon(rcPage.right - GetSystemMetrics(SM_CXVSCROLL),
						 rcPage.bottom - GetSystemMetrics(SM_CYHSCROLL), rcPage.right,
						 rcPage.bottom);
