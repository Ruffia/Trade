...
BOOL CXTPTaskDialogFrame::OnInitDialog()
{
	...
	if (pWnd)
	{
		SetDefID(XTPToUInt(nDefaultButton));
	}
...
BOOL CXTPTaskDialogFrame::Create(CWnd* pParentWnd)
{
	...
	m_lpDlgTemplate->dwExtendedStyle = DWORD(m_pConfig->dwFlags & TDF_RTL_LAYOUT ? WS_EX_LAYOUTRTL
																				 : 0);
...
CString CXTPTaskDialogFrame::GetFrameTitle() const
{
	...
	{
		if (XTP_IS_INTRESOURCE(m_pConfig->pszWindowTitle))
		{
			XTPLoadStringInst(m_pConfig->hInstance, (UINT)(UINT_PTR)m_pConfig->pszWindowTitle,
							  &strWindowTitle);
		}
		else
		{
...
{
	DWORD dwStyle = WS_POPUP | WS_BORDER | WS_CAPTION | DS_ABSALIGN | DS_NOIDLEMSG | DS_3DLOOK
					| DS_MODALFRAME;
	...
	if ((m_pConfig->dwFlags & TDF_ALLOW_DIALOG_CANCELLATION)
		|| (m_pConfig->dwCommonButtons & TDCBF_CANCEL_BUTTON))
	{
		dwStyle |= WS_SYSMENU;
	}
	return dwStyle;
...
{
	CButton* pWnd = DYNAMIC_DOWNCAST(CButton, GetDlgItem((int)wParam));
	if (!pWnd)
		return FALSE;
	...
	m_nSelRadioButtonID = (int)wParam;
...
{
	CWnd* pWnd = DYNAMIC_DOWNCAST(CButton, GetDlgItem((int)wParam));
	if (!pWnd)
		return FALSE;
...
LRESULT CXTPTaskDialogFrame::OnDialogUpdateIcon(WPARAM wParam, LPARAM lParam)
{
	if (wParam == TDIE_ICON_MAIN)
	{
		...
		m_hIconMain = CreateIcon((HICON)lParam, (PCWSTR)lParam, m_sizeIconMain,
	...
	if (wParam == TDIE_ICON_FOOTER)
	{
		...
		m_hIconFooter = CreateIcon((HICON)lParam, (PCWSTR)lParam, m_sizeIconFooter,
...
LRESULT CXTPTaskDialogFrame::OnDialogSetMarqueeProgressBar(WPARAM wParam, LPARAM /*lParam*/)
{
	...
	m_wndProgressBar.ModifyStyle(PBS_MARQUEE, DWORD(wParam ? PBS_MARQUEE : 0));
...
LRESULT CXTPTaskDialogFrame::OnDialogSetProgressBarRange(WPARAM /*wParam*/, LPARAM lParam)
{
	...
	m_wndProgressBar.SetRange(static_cast<short>(LOWORD(lParam)),
