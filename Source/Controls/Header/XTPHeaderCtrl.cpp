...
void CXTPHeaderCtrl::FitFieldWidths(int iNewWidth)
{
	...
	DWORD dwStyle = XTPToDWORD(::GetWindowLong(::GetParent(m_hWnd), GWL_STYLE));
...
BOOL CXTPHeaderCtrl::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message)
{
	if (m_arFrozenCols.GetCount())
	{
		...
		int nIndex = (int)::SendMessage(GetSafeHwnd(), HDM_HITTEST, 0L, (LPARAM)&hti);
		...
		if (nIndex > -1)
		{
			...
			else
			{
				...
				// if columns are resized by dragging to the right, test for the frozen column on
				// the left. if columns are resized by dragging to the left, test for the frozen
				// column on the right.
				...
				if ((iAdjIndex > -1) && IsColFrozen(iAdjIndex))
				{
					...
					// if columns resize to the right and the point is the right half of the header
					// item...
					...
					// if columns resize to the left and the point is the left half of the header
					// item...
...
BOOL CXTPHeaderCtrl::IsDpiBitmapScalingEnabled() const
{
	return (m_bDpiBitmapScalingModified ? m_bDpiBitmapScaling
										: XTPDpiHelper()->IsDpiBitmapScalingEnabled());
}

void CXTPHeaderCtrl::EnableDpiBitmapScaling(BOOL bEnable /*= TRUE*/)
{
...
LRESULT CXTPHeaderCtrl::OnLayout(WPARAM wParam, LPARAM lParam)
{
	if (IsThemeValid())
	{
		return (LRESULT)m_pTheme->Layout((LPHDLAYOUT)lParam, this);
	}
...
BOOL CXTPHeaderCtrl::RecalcLayout()
{
	...
	pWndParent->SetWindowPos(NULL, 0, 0, cx, cy + 1, SWP_NOMOVE | SWP_FRAMECHANGED);

	pWndParent->SetWindowPos(NULL, 0, 0, cx, cy, SWP_NOMOVE | SWP_FRAMECHANGED);
...
void CXTPHeaderCtrl::OnRButtonDown(UINT nFlags, CPoint point)
{
	...
	if (!m_popupMenuID)
	{
		...
		if (iIndex != -1)
		{
			...
			CXTPImageManager* pImgMgr = new CXTPImageManager();
			if (pImgMgr)
			{
				CString type	= _T("RT_XAML");
				HMODULE hModule = XTPResourceManager()->GetResourceHandle();

				pImgMgr->SetVectorIcon(hModule, type, IDR_XAML_ICON_HEADERCTRL_SORTAZ,
									   XTP_IDC_SORTASC, XTPToUInt(GetSystemMetrics(SM_CXSMICON)));
				pImgMgr->SetVectorIcon(hModule, type, IDR_XAML_ICON_HEADERCTRL_SORTZA,
									   XTP_IDC_SORTDSC, XTPToUInt(GetSystemMetrics(SM_CXSMICON)));
				pImgMgr->SetVectorIcon(hModule, type, IDR_XAML_ICON_HEADERCTRL_LEFT,
									   XTP_IDC_ALIGNLEFT, XTPToUInt(GetSystemMetrics(SM_CXSMICON)));
				pImgMgr->SetVectorIcon(hModule, type, IDR_XAML_ICON_HEADERCTRL_CENTER,
									   XTP_IDC_ALIGNCENTER,
									   XTPToUInt(GetSystemMetrics(SM_CXSMICON)));
				pImgMgr->SetVectorIcon(hModule, type, IDR_XAML_ICON_HEADERCTRL_RIGHT,
									   XTP_IDC_ALIGNRIGHT,
									   XTPToUInt(GetSystemMetrics(SM_CXSMICON)));
			}

			XTPContextMenu(pPopup, TPM_LEFTALIGN | TPM_RIGHTBUTTON, pt.x, pt.y, this, pImgMgr,
						   TRUE);
		}
	}
	else
	{
		CMenu menu;
		VERIFY(XTPResourceManager()->LoadMenu(&menu, m_popupMenuID));
		...
		pPopup->TrackPopupMenu(TPM_LEFTALIGN | TPM_RIGHTBUTTON, pt.x, pt.y, GetOwner());
	}
}
...
void CXTPHeaderCtrl::SendNotify(int iIndex)
{
		...
		|| pParentWnd->IsKindOf(RUNTIME_CLASS(CListView)))
	{
		...
		HDITEM hdi;
		hdi.mask = HDI_TEXT | HDI_BITMAP | HDI_FORMAT | HDI_IMAGE | HDI_LPARAM | HDI_ORDER
				   | HDI_WIDTH;
		...
		nmh.hdr.idFrom   = XTPToUIntPtr(GetDlgCtrlID());
		...
		pParentWnd->SendMessage(WM_NOTIFY, (WPARAM)(int)nmh.hdr.idFrom, (LPARAM)(NMHEADER*)&nmh);

		// then forward to the descendants.
		pParentWnd->SendMessageToDescendants(WM_NOTIFY, (WPARAM)(int)nmh.hdr.idFrom,
											 (LPARAM)(NMHEADER*)&nmh);
	}
}
