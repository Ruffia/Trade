...
BOOL CXTPHeaderCtrlTheme::OnDrawBackgroundWinThemed(LPDRAWITEMSTRUCT lpDIS)
{
	...
	if (m_themeHeader->DrawThemeBackground(lpDIS->hDC, 0, HIS_NORMAL, &lpDIS->rcItem, NULL) != S_OK)
	{
...
BOOL CXTPHeaderCtrlTheme::OnDrawItemBackgroundWinThemed(LPDRAWITEMSTRUCT lpDIS)
{
	...
	if (m_themeHeader->DrawThemeBackground(lpDIS->hDC, HP_HEADERITEM, iState, &lpDIS->rcItem, NULL)
		!= S_OK)
	{
		TRACE0("Error drawing background using WinTheme API.\n");
		return FALSE;
...
void CXTPHeaderCtrlTheme::OnDrawItemText(LPDRAWITEMSTRUCT lpDIS, CXTPHeaderCtrl* pHeaderCtrl)
{
	...
	BOOL bSortAsc  = FALSE;
	UINT nSortItem = XTPToUInt(pHeaderCtrl->GetSortedCol(&bSortAsc));
	...
	rcText.top	= (m_sizePadding.cy > 0) ? m_sizePadding.cy : (rcItem.Height() - sizeText.cy) / 2;
	...
	{
		case HDF_LEFT: rcText.right = rcText.left + sizeText.cx; break;
		case HDF_CENTER:
			rcText.left += (rcItem.Width() - sizeText.cx) / 2;
			rcText.right = rcText.left + sizeText.cx;
			break;
		case HDF_RIGHT: rcText.left = rcText.right - sizeText.cx; break;
	}
	...
	if (rcText.Width() > 0)
	{
		...
		// draw text.
		pDC->DrawText(lpHDI->pszText, &rcText, DT_SINGLELINE | DT_VCENTER | DT_END_ELLIPSIS);
	}
}
...
void CXTPHeaderCtrlTheme::DrawHeader(CDC* pDC, CXTPHeaderCtrl* pHeaderCtrl)
{
	...
	int* plItems   = new int[XTPToUIntChecked(iItemCount)];
	...
	for (iItem = 0; iItem < iItemCount; ++iItem)
	{
		...
		dis.itemID		 = XTPToUInt(plItems[iItem]);
		dis.itemState	= 0;
		dis.rcItem		 = rcClient;
		dis.rcItem.left  = cxy;
...
{
	ASSERT(pHeaderCtrl);
	return (BOOL)pHeaderCtrl->DefWindowProc(HDM_LAYOUT, (WPARAM)0, (LPARAM)lpHDL);
}

BOOL CXTPHeaderCtrlTheme::SetBitmap(int iCol, UINT uBitmapID, BOOL bRemove, COLORREF crMask,
									CXTPHeaderCtrl* pHeaderCtrl)
{
	...
	if (!bRemove)
	{
		HBITMAP hBitmap = (HBITMAP)::LoadImage(AfxGetResourceHandle(), MAKEINTRESOURCE(uBitmapID),
		...
		HIMAGELIST hImageList = ::ImageList_Create(bmpInfo.bmWidth, bmpInfo.bmHeight,
												   ILC_MASK | ILC_COLOR24, 0, 1);
...
void CXTPHeaderCtrlTheme::OnDrawItemBitmap(LPDRAWITEMSTRUCT lpDIS, CRect& rcText,
										   CXTPHeaderCtrl* pHeaderCtrl)
{
	...
	pHeaderCtrl->GetItem(XTPToInt(lpDIS->itemID), &hdi);
	...
	{
		CXTPImageManagerIcon* pIcon = pHeaderCtrl->GetImageManager()->GetImage(XTPToUInt(nIndex),
																			   0);

		if (pIcon)
		{
			CSize szIcon = pIcon->GetExtent();
			if (pIcon->IsVectorIcon() || pHeaderCtrl->IsDpiBitmapScalingEnabled())
			...
			XTPImageState imageState = (lpDIS->itemState & ODS_SELECTED)
										   ? xtpImageChecked
										   : (lpDIS->itemState & ODS_HOTLIGHT) ? xtpImageHot
																			   : xtpImageNormal;
			...
			pIcon->Draw(CDC::FromHandle(lpDIS->hDC), point, imageState, szIcon);
	...
	if (hImageList)
	{
		...
		if (::ImageList_GetIconSize(hImageList, &cx, &cy)
			&& (nIndex < ImageList_GetImageCount(hImageList)))
		{
			if (pHeaderCtrl->IsDpiBitmapScalingEnabled())
			{
				cx = XTP_DPI_X(cx);
				cy = XTP_DPI_X(cy);
			}
			...
			CXTPDrawHelpers::StretchImageListImage(CDC::FromHandle(lpDIS->hDC),
												   CRect(point, CSize(cx, cy)),
												   CImageList::FromHandle(hImageList), nIndex);
		}
	}
}
...
void CXTPHeaderCtrlThemeResource::RefreshMetrics(CXTPHeaderCtrl* pHeader)
{
	...
	if (pImages)
	{
		m_crText	   = pImages->GetImageColor(_T("GridControl"), _T("HeaderText"), m_crText);
		m_cr3DShadow   = pImages->GetImageColor(_T("GridControl"), _T("ColumnSeparator"), m_crText);
		m_cr3DDkShadow = pImages->GetImageColor(_T("GridControl"), _T("ColumnShadow"), m_crText);

		m_crgBack.SetStandardValue(pImages->GetImageColor(_T("GridControl"),
														  _T("NormalColumnLight"), m_cr3DFace),
								   pImages->GetImageColor(_T("GridControl"), _T("NormalColumnDark"),
														  m_cr3DFace));

		m_crgBackPressed.SetStandardValue(
			pImages->GetImageColor(_T("GridControl"), _T("PressedColumnLight"), m_cr3DFace),
			pImages->GetImageColor(_T("GridControl"), _T("PressedColumnDark"), m_cr3DFace));
	}
	else
	{
...
void CXTPHeaderCtrlThemeResource::OnDrawItemText(LPDRAWITEMSTRUCT lpDIS,
												 CXTPHeaderCtrl* pHeaderCtrl)
{
	ASSERT(pHeaderCtrl);
	...
	BOOL bSortAsc;
	UINT nSortItem = XTPToUIntChecked(pHeaderCtrl->GetSortedCol(&bSortAsc));
	if (nSortItem == lpDIS->itemID)
		rcItem.right -= m_sizeArrow.cx + m_iArrowPadding * 2;
