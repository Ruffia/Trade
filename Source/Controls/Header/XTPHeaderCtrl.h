...
public:
	...
	//--------------------------------------------------------------------
	// Summary:
	//     Call this member function to thaw all columns in the header control.
	...
	//--------------------------------------------------------------------
	// Summary:
	//     Call this member function to determine if a specified column is
	//     frozen.
	// Parameters:
	...
	//     specified point.
	// Parameters:
	//     pt -      POINT structure that contains the point to be hit test,
	...
	//                             header items.
	//     * <b>HHT_ONDIVOPEN</b>: The point is on the divider of an item that has a
	...
	//                           control's bounding rectangle.
	// Returns:
	//     If the hit test is successful, returns the index of the item at
	//     the hit test point, if any. Otherwise, returns a value of -1.
	// -------------------------------------------------------------------
	...
	// Parameters:
	//     popupMenuID - Resource ID for the pop-up menu used with the header.
	...
	//--------------------------------------------------------------------
	// Summary:
	//     Call this member function to enable/disable the column sort arrow.
	// Parameters:
	//     bSortArrow - TRUE to draw the column sort arrow.
	//--------------------------------------------------------------------
	...
	//--------------------------------------------------------------------
	// Summary:
	//     Call this member function to determine if the current sorting order
	//     is ascending.
	// Returns:
	...
	//--------------------------------------------------------------------
	// Summary:
	//     Call this member function to enable/disable the pop-up menu display whenever
	//     a user right clicks on the header control.
	// Parameters:
	//     bEnableMenus - TRUE to enable pop-up menus, FALSE to disable.
	//--------------------------------------------------------------------
	...
	//     a sort arrow.
	// Returns:
	//     TRUE if the header control displays a sort arrow, otherwise FALSE.
	//--------------------------------------------------------------------
	BOOL HasSortArrow();
	...
	//--------------------------------------------------------------------
	// Summary:
	//     Retrieves the image manager associated with the header.
	// Returns:
	//     Pointer to the image manager object that stores images in
	//     the header.
	//--------------------------------------------------------------------
	...
	//--------------------------------------------------------------------
	// Summary:
	//     Call this method to assign a new image manager.
	// Parameters:
	//     pImageManager - Pointer to the new image manager object that will
	//                     store images in the header.
	//--------------------------------------------------------------------
	...
	// Parameters:
	//     eTheme  - New window theme. Can be any of the values listed in
	//               the Remarks section.
	//     eScheme - Color scheme used by the theme. Can be any of the
	...
	BOOL SetTheme(CXTPHeaderCtrlTheme* pTheme); // <combine
												// CXTPHeaderCtrl::SetTheme@XTPControlTheme>

	//--------------------------------------------------------------------
	// Summary:
	//     Call this member function to get a pointer to the currently selected theme.
	// Returns:
	//     A pointer to a CXTPHeaderCtrlTheme object representing the currently
	//     selected theme.
	//--------------------------------------------------------------------
	...
	void EnableDpiBitmapScaling(
		BOOL bEnable = TRUE); // <combine CXTPHeaderCtrl::IsDpiBitmapScalingEnabled@const>
...
protected:
	...
	// Returns:
	//     true if the text alignment is right-to-left (RTL), or
	//     false if the text alignment is left-to-right (LTR).
	//--------------------------------------------------------------------
	...
	//--------------------------------------------------------------------
	// Summary:
	//     This member function is used by the header control to determine the
	//     new width for auto-sized columns and to resize the columns accordingly.
	// Parameters:
	//     iNewWidth - New width to resize all columns to. If iNewWidth
	//                 is '0', then columns are equally sized to fit the
	...
	//--------------------------------------------------------------------
	// Summary:
	//     Called to send WM_NOTIFY to tell the parent's owner that a column needs
	//     to be sorted.
	// Parameters:
...
protected:
	...
	int m_iOverIndex; // Column that the mouse is currently hovering over.
	...
	BOOL m_bEnableMenus;			   // TRUE to disable the pop-up menu display.
	...
	CXTPHeaderCtrlTheme* m_pTheme;	 // Pointer to the active theme.
	CXTPImageManager* m_pImageManager; // Pointer to the ImageManager associated with the header.
	BOOL m_bDpiBitmapScaling;		   // DPI bitmap scaling flag.
	BOOL m_bDpiBitmapScalingModified;  // Indicates that m_bDpiBitmapScaling has been modified.
