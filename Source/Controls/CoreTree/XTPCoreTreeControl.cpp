...
CLIPFORMAT CXTPCoreTreeControl::m_cfItem = (CLIPFORMAT)::RegisterClipboardFormat(
	_T("CoreTreeItem"));
								  ...
								  CPoint point)
	{
		...
		if (nTimerID == nTimerHover)
		{
			...
			if (point != m_ptLastPoint)
			{
				m_ptLastPoint  = point;
				m_dwLastTick   = dwTick;
				m_nScrollDelay = XTPToUIntChecked(m_nHoverTime);
			}
...
CXTPCoreTreeControl::CXTPCoreTreeControl()
{
	...
	m_clrBorder1   = GetSysColor(COLOR_BTNSHADOW);
	m_clrBorder2   = GetSysColor(COLOR_WINDOW);
}
...
BOOL CXTPCoreTreeControl::IsDpiBitmapScalingEnabled() const
{
	return (m_bDpiBitmapScalingModified ? m_bDpiBitmapScaling
										: XTPDpiHelper()->IsDpiBitmapScalingEnabled());
}

void CXTPCoreTreeControl::EnableDpiBitmapScaling(BOOL bEnable /*= TRUE*/)
{
...
CXTPCoreTreeItem* CXTPCoreTreeControl::HitTestTreeItems(CXTPCoreTreeItems* pItems,
														CPoint point) const
{
...
int CXTPCoreTreeControl::RecalcTreeItems(CXTPCoreTreeItems* pItems, CDC* pDC, int nLevel,
										 LPCRECT lprcMargin)
{
	...
	while (pos)
	{
		...
		pItem->SetRect(CRect(m_rcControl.left + lprcMargin->left, nTop,
							 m_rcControl.right - lprcMargin->right, nTop + sz.cy));
...
void CXTPCoreTreeControl::SetDragOverItem(CXTPCoreTreeItem* pDragOverItem,
										  XTPCoreTreeDropPosition nDropPosition)
{
...
DROPEFFECT CXTPCoreTreeControl::OnDragOver(COleDataObject* /*pDataObject*/, DWORD /*dwKeyState*/,
										   CPoint /*point*/)
{
	return DROPEFFECT_NONE;
}

void CXTPCoreTreeControl::OnDragHover(COleDataObject* /*pDataObject*/, DWORD /*dwKeyState*/,
									  CPoint point)
{
...
BOOL CXTPCoreTreeControl::OnDrop(COleDataObject* pDataObject, DROPEFFECT dropEffect,
								 CPoint /*point*/)
{
	if (!m_pDragOverItem)
		return FALSE;
...
BOOL CXTPCoreTreeControl::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt)
{
	if (m_bScrollVisible)
	{
		...
		if (zDelta > 0)
			nCurPos = max(nCurPos - nDelta, 0);
		else
			nCurPos = min(nCurPos + nDelta, GetScrollLimit(SB_VERT));
...
void CXTPCoreTreeControl::OnNcPaint()
{
	...
	if (GetPaintManager()->IsAppThemed() && GetExStyle() & WS_EX_CLIENTEDGE)
	{
		...
		dc.Draw3dRect(rc, m_clrBorder1, m_clrBorder1);
		rc.DeflateRect(1, 1);
		dc.Draw3dRect(rc, m_clrBorder2, m_clrBorder2);
	}
}
...
void CXTPCoreTreeControl::EnableTheme(BOOL bEnable)
{
	m_bEnableTheme = bEnable;

	if (m_bEnableTheme)
	{
		m_clrBorder1 = XTPIniColor(_T("Controls.Tree"), _T("Border"), GetSysColor(COLOR_BTNSHADOW));
		m_clrBorder2 = XTPIniColor(_T("Controls.Tree"), _T("Back"), GetSysColor(COLOR_WINDOW));
	}
	else
	{
		m_clrBorder1 = GetSysColor(COLOR_BTNSHADOW);
	...
	if (m_pPaintManager)
		m_pPaintManager->EnableTheme(bEnable);
}
