...
//     even if it is a multi-line control. Also, if an edit control is multi-line,
...
//                            text in an edit control. Unlike the EN_UPDATE
//                            notification message, this notification message is
...
//     * <b>ON_EN_HSCROLL</b>: The user clicks an edit control's horizontal
//                             scrollbar. The parent window is notified before
...
//                             number of characters for the edit control and has
//                             been truncated. Also sent when an edit control
//                             does not have the ES_AUTOHSCROLL style and the
//                             number of characters to be inserted would exceed
//                             the width of the edit control. Also sent when an
//                             edit control does not have the ES_AUTOVSCROLL
//                             style and the total number of lines resulting
//                             from a text insertion would exceed the height of
//                             the edit control.
...
//                            Sent after the control has formatted the text but
//                            before it screens the text so that the window size
//                            can be altered, if necessary.
//     * <b>ON_EN_VSCROLL</b>: The user clicks an edit control's vertical
//                             scrollbar. The parent window is notified before
//                             the screen is updated.
//
//     If you create a CXTPEdit object within a dialog box, then the CXTPEdit object
...
//     editor, then the CXTPEdit object is automatically destroyed when the user
...
public:
	...
	//                   If NULL, then 'lpszLiteral' is used to initialize
	//                   the edit text.
	// Remarks:
	...
	//-----------------------------------------------------------------------
	// Summary:
	//     Call this member function to set the pattern for the edit mask.
	// Parameters:
	//     lpszPattern - Text pattern to set.
	//-----------------------------------------------------------------------
	...
	//-----------------------------------------------------------------------
	void AdjustBuddyRect(CWnd* pBuddy, CRect& rcUpDown, int nAlignment);
	...
	//     nAlignment - Side of the edit box that thee buddy is attached to
	//                  (either UDS_ALIGNRIGHT or UDS_ALIGNLEFT).
	//-----------------------------------------------------------------------
	void DrawBuddyBorders(CWnd* pBuddy, CDC* pDC, CRect rc, int nAlignment);

	// -----------------------------------------------------------------
	// Summary:
	//     This member is called to update the color, text and other
	...
	//-----------------------------------------------------------------------
	// Summary:
	//     Call this member to retrieve a pointer to the current theme.
	// Returns:
	//     A CXTPEditTheme object that represents the current theme.
	//-----------------------------------------------------------------------
	...
	//-----------------------------------------------------------------------
	// Summary:
	//     Called by the framework to determine if the pattern set using
	//     SetPattern() is valid.
	// Returns:
	//     TRUE if the previously set pattern is valid.
	//-----------------------------------------------------------------------
	BOOL IsPatternValid();

	//-----------------------------------------------------------------------
	// Summary:
	//     Call this member function to enable/disable theme border display.
	// Parameters:
	//     bShowThemedBorder - TRUE to enable themed borders,
	//                         FALSE to disable themed borders.
	//-----------------------------------------------------------------------
	void ShowThemedBorder(BOOL bShowThemedBorder);

	//-----------------------------------------------------------------------
	// Summary:
	//     Call this member function to determine if the edit state is highlighted,
	//     meaning that the mouse cursor is currently hovering over the control.
	// Returns:
	//     TRUE if the edit state is highlighted, otherwise FALSE.
	//-----------------------------------------------------------------------
	BOOL IsHighlighted() const;

	//-----------------------------------------------------------------------
	// Summary:
	//     Call this member function to determine if the edit-box has input focus.
	// Returns:
	//     TRUE if the edit-box has input focus, otherwise FALSE.
	//-----------------------------------------------------------------------
	BOOL IsFocused() const;

	//-----------------------------------------------------------------------
	// Summary:
	//     Gets the flat style appearance of the edit control.
	// Remarks:
	...
	//-----------------------------------------------------------------------
	// Summary:
	//     Call this member to determine if non-client borders are displayed.
	// Returns:
	//     TRUE if non-client borders are displayed.
	//-----------------------------------------------------------------------
...
protected:
	//-----------------------------------------------------------------------
	// Summary:
	//     Retrieves a handle to the client background brush.
	// Parameters:
	//     pDC - Pointer to a valid device context.
	// Returns:
	...
	//-----------------------------------------------------------------------
	// Summary:
	//     Called by the framework to redraw the edit control.
	//-----------------------------------------------------------------------
	void RedrawEdit();

	//-----------------------------------------------------------------------
	// Summary:
	//     Called by the framework to redraw the focus rectangle for the edit control.
	//-----------------------------------------------------------------------
	void RedrawFocusedFrame();

	CXTPEditTheme* m_pTheme; // Pointer to the current edit theme.
	...
	BOOL m_bHighlighted;	 // TRUE when the mouse is hovering over the control.
	BOOL m_bFocused;		 // TRUE when the control has input focus.
	CXTPRegExp* m_pRegExp;   // Pointer to the CXTPRegExp object for this control.
};
...
//===========================================================================
// Summary:
//     An adaptor for any CEdit derived control that overrides standard scrollbars
...
public:
	...
	using CXTPScrollable<EditBase>::ModifyStyle;
	using CXTPScrollable<EditBase>::ModifyStyleEx;
	using CXTPScrollable<EditBase>::GetRuntimeClass;
...
//---------------------------------------------------------------------------
// Summary:
//     Type alias for a CXTPEdit derived scrollable control.
//---------------------------------------------------------------------------
...
AFX_INLINE void CXTPScrollableEditT<EditBase>::DisableScrollbars(CWnd& wnd)
{
	...
	// Do nothing as EDIT behaves improperly if scrollbars gets disabled repeatedly.
}
...
public:
	//-----------------------------------------------------------------------
	// Summary:
	//     This member is called to update the color, text, and other visual elements
	//     of the control.
	//-----------------------------------------------------------------------
	virtual void RefreshMetrics();
};

_XTP_DEPRECATED_IN_FAVOR(CXTPEdit)
