...
const int CX_BUTTON = (::GetSystemMetrics(SM_CXHTHUMB) + (::GetSystemMetrics(SM_CXEDGE) * 2));
...
BOOL CXTPBrowseButton::Create(CXTPBrowseEdit* pEditWnd, DWORD dwBStyle, UINT nID,
							  UINT nMenu /*= -1*/, UINT nSubMenuIndx /*= -1*/)
{
	...
	DWORD dwStyle = (m_pEditWnd->GetStyle() & WS_VISIBLE) | WS_CHILD | WS_TABSTOP | BS_CENTER
					| BS_VCENTER;
	...
	if ((m_dwBStyle & BES_XTP_POPUPMENU) == 0)
	{
		CXTPResourceManager::AssertValid(
			XTPResourceManager()->LoadString(&strButtonText, XTP_IDS_DOT));
	}
	else
	{
		hPopupIcon = XTPResourceManager()->LoadIcon(MAKEINTRESOURCE(XTP_IDS_ARROW), CSize(16, 16));
		dwStyle |= BS_ICON;
	}

	if (!CXTPButton::Create(strButtonText, dwStyle, CRect(0, 0, 0, 0), m_pEditWnd->GetParent(),
							nID))
	{
...
void CXTPBrowseButton::SetBrowseStyle(DWORD dwxStyle)
{
	...
	{
		CSize sz(GetSystemMetrics(SM_CXSMICON), GetSystemMetrics(SM_CYSMICON));
		HICON hIcon = XTPResourceManager()->LoadIcon(XTP_IDS_ARROW, sz);
		if (hIcon)
			SetIcon(sz, hIcon);
	}
}
...
CXTPBrowseEdit::CXTPBrowseEdit()
{
	...
	CXTPResourceManager::AssertValid(
		XTPResourceManager()->LoadString(&m_strDefExt, XTP_IDS_DEFEXT));
	CXTPResourceManager::AssertValid(
		XTPResourceManager()->LoadString(&m_strFilter, XTP_IDS_DEFFILT));
	CXTPResourceManager::AssertValid(
		XTPResourceManager()->LoadString(&m_strTitle, XTP_IDS_DEFTITLE));
}
...
bool CXTPBrowseEdit::Initialize(CWnd* pParentWnd, DWORD dwBStyle, UINT nMenu /*= 0*/,
								int nSubMenuIndx /*= 0*/)
{
	...
	if (m_dwBStyle & (BES_XTP_CHOOSEDIR | BES_XTP_CHOOSEFILE | BES_XTP_POPUPMENU | BES_XTP_BROWSE))
	{
		...
		m_nMenu		   = nMenu;
		m_nSubMenuIndx = XTPToUIntChecked(nSubMenuIndx);

		// Create the browse button associated with the browse edit control.
		if (!m_btnBrowse.Create(this, m_dwBStyle, XTP_IDC_BTN_BROWSE, m_nMenu, m_nSubMenuIndx))
		{
		...
		if (m_nGap > 0)
		{
			...
			::SetWindowPos(m_hWnd, NULL, rcWindow.left, rcWindow.top, rcWindow.Width(),
						   rcWindow.Height(), SWP_FRAMECHANGED | SWP_NOZORDER);
		}
		else
		{
			...
			// set the window position to accommodate for the browse button.
			::SetWindowPos(m_btnBrowse, NULL, rcWindow.left, rcWindow.top, rcWindow.Width(),
						   rcWindow.Height(), SWP_FRAMECHANGED | SWP_NOZORDER);
...
void CXTPBrowseEdit::PositionBrowseButton()
{
	if (::IsWindow(m_btnBrowse.GetSafeHwnd()))
	{
		...
		::SetWindowPos(m_btnBrowse.m_hWnd, m_hWnd, rcButton.left, rcButton.top, rcButton.Width(),
					   rcButton.Height(), SWP_FRAMECHANGED);
...
void CXTPBrowseEdit::ChooseFile()
{
	...
	{
		CFileDialog dlg(m_bOpenFileDialog, m_strDefExt, m_strFileName, m_dwFlags, m_strFilter,
						GetParent());
...
#pragma warning(disable : 4127) // C4127: conditional expression is constant
		if (sizeof(OPENFILENAME) < 88 && XTPSystemVersion()->IsWin2KOrGreater()
...
void CXTPBrowseEdit::PopupMenu()
{
	...
	CMenu* pPopup = menu.GetSubMenu(XTPToIntChecked(m_nSubMenuIndx));
	ASSERT(pPopup != NULL);
	...
	XTPContextMenu(pPopup, TPM_LEFTALIGN | TPM_LEFTBUTTON | TPM_VERTICAL, rect.right, rect.top,
				   pWndPopupOwner, 0);
...
void CXTPBrowseEdit::OnBrowse()
{
	...
	switch (m_dwBStyle
			& (BES_XTP_CHOOSEDIR | BES_XTP_CHOOSEFILE | BES_XTP_POPUPMENU | BES_XTP_BROWSE))
	{
		case BES_XTP_CHOOSEDIR: ChooseDirectory(); break;
		case BES_XTP_CHOOSEFILE: ChooseFile(); break;
		case BES_XTP_POPUPMENU: PopupMenu(); break;
		case BES_XTP_BROWSE:
			GetParent()->SendMessage(WM_COMMAND, MAKEWPARAM(GetDlgCtrlID(), LBN_XTP_ONBROWSE),
									 (LPARAM)m_hWnd);
			break;
	}
...
BOOL CXTPItemEdit::Create(CWnd* pParent, const CRect& rect, CString& strWindowText,
						  DWORD dwBStyle /*= BES_XTP_CHOOSEDIR*/, bool bAutoDelete /*= true*/)
{
	...
	if (!CXTPBrowseEdit::CreateEx(WS_EX_CLIENTEDGE, _T("EDIT"), 0,
								  WS_BORDER | WS_CHILD | WS_VISIBLE | ES_AUTOHSCROLL, rect, pParent,
								  XTP_IDC_LBOX_EDIT))
		return FALSE;
...
void CXTPItemEdit::EndLabelEdit()
{
	...
	if (m_bModified == false)
	{
		...
		if (m_bEscapeKey == false)
		{
			...
			pOwner->SendMessage(WM_COMMAND, MAKEWPARAM(GetDlgCtrlID(), BEN_XTP_LABELEDITEND),
								(LPARAM)GetSafeHwnd());
		}
		else
		{
			// Notify of label edit.
			pOwner->SendMessage(WM_COMMAND, MAKEWPARAM(GetDlgCtrlID(), BEN_XTP_LABELEDITCANCEL),
								(LPARAM)GetSafeHwnd());
		}
	}
}
