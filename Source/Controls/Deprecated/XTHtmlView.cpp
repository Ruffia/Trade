...
ON_EVENT(CXTHtmlView, AFX_IDW_PANE_FIRST, 105 /* CommandStateChange */, OnCommandStateChange,
		 VTS_I4 VTS_BOOL)
...
ON_EVENT(CXTHtmlView, AFX_IDW_PANE_FIRST, 252 /* NavigateComplete2 */, NavigateComplete2,
		 VTS_DISPATCH VTS_PVARIANT)
ON_EVENT(CXTHtmlView, AFX_IDW_PANE_FIRST, 250 /* BeforeNavigate2 */, BeforeNavigate2,
...
ON_EVENT(CXTHtmlView, AFX_IDW_PANE_FIRST, 251 /* NewWindow2 */, OnNewWindow2,
		 VTS_PDISPATCH VTS_PBOOL)
ON_EVENT(CXTHtmlView, AFX_IDW_PANE_FIRST, 259 /* DocumentComplete */, DocumentComplete,
		 VTS_DISPATCH VTS_PVARIANT)
ON_EVENT(CXTHtmlView, AFX_IDW_PANE_FIRST, 271, /* NavigateError */ NavigateError,
...
void CXTHtmlView::OnSize(UINT nType, int cx, int cy)
{
	...
	if (::IsWindow(m_wndBrowser.m_hWnd))
	{
		...
		m_wndBrowser.SetWindowPos(NULL, -icx, -icy, cx + (icx * 2), cy + (icy * 2),
								  SWP_NOACTIVATE | SWP_NOZORDER);
	}
}

BOOL CXTHtmlView::Create(LPCTSTR lpszClassName, LPCTSTR lpszWindowName, DWORD dwStyle,
						 const RECT& rect, CWnd* pParentWnd, UINT nID, CCreateContext* pContext)
{
	...
	if (!m_wndBrowser.CreateControl(CLSID_WebBrowser, lpszWindowName, WS_VISIBLE | WS_CHILD,
...
void CXTHtmlView::ExecWB(OLECMDID cmdID, OLECMDEXECOPT cmdexecopt, VARIANT* pvaIn, VARIANT* pvaOut)
{
	ASSERT(m_pBrowserApp != NULL);
	if (!m_pBrowserApp)
		return;
						   ...
						   DWORD dwPostDataLen /* = 0 */)
{
	...
	{
		if (dwPostDataLen == 0)
			dwPostDataLen = XTPToDWORDChecked(lstrlen((LPCTSTR)lpvPostData));
...
void CXTHtmlView::Navigate2(LPITEMIDLIST pIDL, DWORD dwFlags, LPCTSTR lpszTargetFrameName)
{
	...
	m_pBrowserApp->Navigate2(vPIDL, COleVariant((long)dwFlags, VT_I4),
							 COleVariant(lpszTargetFrameName, VT_BSTR), empty, empty);
}
...
void CXTHtmlView::Navigate2(LPCTSTR lpszURL, DWORD dwFlags, CByteArray& baPostData,
							LPCTSTR lpszTargetFrameName /* = NULL */,
...
COleVariant CXTHtmlView::GetProperty(LPCTSTR lpszProperty)
{
	...
	m_wndBrowser.InvokeHelper(0x12f, DISPATCH_METHOD, VT_VARIANT, (void*)&result, parms,
							  lpszProperty);
...
void CXTHtmlView::BeforeNavigate2(LPDISPATCH /* pDisp */, VARIANT* URL, VARIANT* Flags,
								  ...
								  BOOL* Cancel)
{
	...
	if (V_VT(vtPostedData) & VT_ARRAY)
	{
		...
		array.SetSize(XTPToIntPtrChecked(dwSize));
	...
	DWORD nFlags = XTPToDWORD(V_I4(Flags));
...
void CXTHtmlView::NavigateError(LPDISPATCH /*pDisp*/, VARIANT* pvURL, VARIANT* pvFrame,
								VARIANT* pvStatusCode, VARIANT_BOOL* pvbCancel)
{
	...
	DWORD dwError = XTPToDWORD(V_I4(pvStatusCode));
...
void CXTHtmlView::OnNavigateError(LPCTSTR /*lpszURL*/, LPCTSTR /*lpszFrame*/, DWORD dwError,
								  BOOL* pbCancel)
{
	// default to continuing
	*pbCancel = FALSE;
