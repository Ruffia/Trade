...
int CXTComboBoxEx::InsertItem(int iItem, UINT nStringID, int iIndent, int iImage,
							  int iSelectedImage, UINT mask)
{
	CString strItem;
...
int CXTComboBoxEx::InsertItem(int iItem, LPCTSTR lpszItem, int iIndent, int iImage,
							  int iSelectedImage, UINT mask)
{
	...
	if (pEditCtrl && ::IsWindow(pEditCtrl->m_hWnd))
	{
		if (pMsg->message == WM_KEYDOWN)
		{
			if (::GetKeyState(VK_CONTROL) < 0)
			{
				...
				{
					case 'a':
					case 'A': pEditCtrl->SetSel(0, -1); return TRUE;
					case 'c':
					case 'C': pEditCtrl->Copy(); return TRUE;
					case 'x':
					case 'X': pEditCtrl->Cut(); return TRUE;
					case 'v':
					case 'V': pEditCtrl->Paste(); return TRUE;
				}
			}
			else
			{
