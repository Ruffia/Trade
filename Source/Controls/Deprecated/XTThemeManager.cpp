...
void CXTThemeManagerStyleFactory::SetTheme(CXTThemeManagerStyle* pTheme)
{
	...
	if (pTheme)
	{
		...
		for (CXTThemeManagerStyleHost* pHost = m_hostList.GetHead(); pHost != NULL;
			 pHost							 = pHost->m_pNextHost)
		{
...
CXTThemeManagerStyleFactory*
	CXTThemeManager::GetDefaultThemeFactory(CRuntimeClass* pThemeFactoryClass)
{
	ASSERT(pThemeFactoryClass);
	if (!pThemeFactoryClass)
		return 0;
...
void CXTThemeManager::RefreshMetrics()
{
	...
	for (CXTThemeManagerStyleFactory* pFactory = m_factoryList.GetHead(); pFactory != NULL;
