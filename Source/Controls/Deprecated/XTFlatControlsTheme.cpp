...
void CXTFlatComboBoxTheme::DrawFlatComboBox(CDC* pDC, CXTFlatComboBox* pCombo, int eState)
{
	...
	if (pCombo->IsWindowEnabled())
	{
		...
		{
			rFillColor.right--;
			pDC->Draw3dRect(&rcItem, GetXtremeColor(COLOR_3DSHADOW),
...
void CXTFlatComboBoxTheme::DrawDownArrow(CDC* pDC, const CRect& rcItem, int eState,
										 CXTFlatComboBox* pCombo)
{
	...
	COLORREF cr = pCombo->IsWindowEnabled() ? GetXtremeColor(COLOR_WINDOWTEXT)
...
void CXTFlatComboBoxTheme::DrawArrowBack(CDC* pDC, CRect& rcArrow, int eState,
										 CXTFlatComboBox* /*pCombo*/)
{
...
void CXTFlatComboBoxThemeOfficeXP::DrawArrowBack(CDC* pDC, CRect& rcArrow, int eState,
												 CXTFlatComboBox* /*pCombo*/)
{
...
void CXTFlatComboBoxThemeOfficeXP::DrawDownArrow(CDC* pDC, const CRect& rcItem, int eState,
												 CXTFlatComboBox* pCombo)
{
	// determine background size.
	CRect rcArrow(rcItem);
...
void CXTFlatComboBoxThemeOffice2003::DrawArrowBack(CDC* pDC, CRect& rcArrow, int eState,
												   CXTFlatComboBox* pCombo)
{
	rcArrow.left = rcArrow.right - (m_cxArrow - 2);
	...
	if (eState == xtMouseHover)
	{
		XTPDrawHelpers()->GradientFill(pDC, &rcArrow, XTPColorManager()->grcLunaSelected, FALSE);
	...
	{
		XTPDrawHelpers()->GradientFill(pDC, &rcArrow, XTPColorManager()->grcLunaPushed, FALSE);
	...
	{
		rcArrow.DeflateRect(1, 1);
		XTPDrawHelpers()->GradientFill(pDC, &rcArrow, XTPColorManager()->grcMenuItemPopup, FALSE);
	}
}
...
void CXTFlatEditTheme::DrawBorders(CDC* pDC, CXTFlatEdit* pEdit, const CRect& rWindow, int eState)
{
	if (pEdit->IsWindowEnabled())
	{
		...
		if (eState == xtMouseHover || eState == xtMouseSelect)
		{
			pDC->Draw3dRect(&rWindow, GetXtremeColor(COLOR_3DSHADOW),
							GetXtremeColor(COLOR_3DHILIGHT));
		}
	}
	else
...
void CXTFlatEditThemeOfficeXP::DrawBorders(CDC* pDC, CXTFlatEdit* pEdit, const CRect& rWindow,
										   int eState)
{
	if (pEdit->IsWindowEnabled())
	{
...
void CXTFlatEditThemeOffice2003::RefreshMetrics()
{
	...
	switch (systemTheme)
	{
		...
		case xtpSystemThemeSilver:
			...
			break;
		default: m_clrFrameNormal.SetStandardValue(GetXtremeColor(COLOR_3DSHADOW));
	}
	m_clrFrameEdit.SetStandardValue(m_clrFrameNormal);
}

void CXTFlatEditThemeOffice2003::DrawBorders(CDC* pDC, CXTFlatEdit* pEdit, const CRect& rWindow,
											 int eState)
{
	if (pEdit->IsWindowEnabled())
	{
