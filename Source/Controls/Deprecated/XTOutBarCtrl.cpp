...
BOOL CXTOutBarEditItem::PreTranslateMessage(MSG* pMsg)
{
	switch (pMsg->wParam)
	{
		case VK_ESCAPE: { m_bEscapeKey = TRUE; // fall thru
		}
		case VK_RETURN:
		{
			PostMessage(WM_CLOSE);
...
BOOL CXTOutBarEditItem::Create(LPCTSTR lpszText, DWORD dwStyle, const RECT& rect, CWnd* pParentWnd,
							   UINT nID, UINT nIndex, bool bIsFolder, bool bSmallIcons)
{
	...
	m_iIndex	  = XTPToIntChecked(nIndex);
	m_strText	 = lpszText;
...
void CXTOutBarCtrl::DrawItemIcon(CDC* pDC, CPoint pt, CXTOutBarItem* pBarItem,
...
LRESULT CXTOutBarCtrl::NotifyOwner(UINT nCode, XT_OUTBAR_INFO* pobi)
{
	...
	return pOwner->SendMessage(XTWM_OUTBAR_NOTIFY, (WPARAM)nCode, (LPARAM)pobi);
}
...
BOOL CXTOutBarCtrl::Create(DWORD dwStyle, const RECT& rect, CWnd* pParentWnd, UINT nID,
						   const DWORD dwFlags)
{
...
BOOL CXTOutBarCtrl::CreateEx(DWORD dwExStyle, DWORD dwStyle, const RECT& rect, CWnd* pParentWnd,
							 UINT nID, const DWORD dwFlags)
{
...
void CXTOutBarCtrl::OnDraw(CDC* pDC)
{
	...
	if (!GetFolderChild())
	{
		...
		m_rcDownArrow.OffsetRect(rc.right - 5 - GetSystemMetrics(SM_CXVSCROLL),
								 rc.bottom - 5 - GetSystemMetrics(SM_CXVSCROLL));
	...
	// dc.BitBlt(rectClient.left, rectClient.top, rectClient.Width(), rectClient.Height(), pDC, 0,
	// 0, SRCCOPY);
...
int CXTOutBarCtrl::AddFolder(LPCTSTR lpszFolderName, const DWORD dwData)
{
	...
	pBarFolder->m_bSmallIcons = (0 != (m_dwFlags & OBS_XT_SMALLICON));
...
void CXTOutBarCtrl::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	if (ht == hitFolder)
		nFolderHilighted = iIndex;
	if (ht == hitItem)
		nItemHilighted = iIndex;
...
CXTOutBarCtrl::HitTestCode CXTOutBarCtrl::HitTestEx(const CPoint& point, int& iIndex)
{
	if (m_arFolder.GetSize() != 0)
	{
		if (m_bUpArrow && m_rcUpArrow.PtInRect(point))
			return hitUpScroll;
		if (m_bDownArrow && m_rcDownArrow.PtInRect(point))
			return hitDnScroll;
		...
		if (::GetCapture() == NULL)
		{
			...
			for (;;)
			{
				...
				switch (msg.message)
				{
					...
					case WM_SETCURSOR: ::SetCursor(hCur); break;
						...
						if (!bDragging)
						{
							if (ht1 == hitItem && idx == iIndex)
							{
								...
								if (lResult == TRUE && nSelFolder == GetSelFolder())
								{
									...
									if ((m_dwFlags & OBS_XT_SELHIGHLIGHT)
										&& (m_nLastItemSelected >= 0))
									{
										bInvalidate = TRUE;
									}
						...
						else
						{
							if (ht1 == hitItem)
							{
								if (idx != iIndex)
								{
									...
									CXTOutBarFolder* pBarFolder =
										(CXTOutBarFolder*)m_arFolder.GetAt(m_nSelFolder);
									POSITION pos1 = pBarFolder->m_barItems.FindIndex(iIndex);

									CXTOutBarItem* pItemFrom =
										(CXTOutBarItem*)pBarFolder->m_barItems.GetAt(pos1);
									...
									if (lResult == TRUE)
									{
										...
										else if (((iIndex <= m_nLastItemSelected)
												  && (idx <= m_nLastItemSelected))
												 || ((iIndex > m_nLastItemSelected)
													 && (idx > m_nLastItemSelected)))
										{
											// do nothing...
										}
							...
							else
							{
								...
								if (pt.y > rcItem.bottom && pt.y < inRc.bottom)
								{
									...
									CXTOutBarItem* pItemFrom =
										(CXTOutBarItem*)pBarFolder->m_barItems.GetAt(pos1);
									POSITION pos2 = pBarFolder->m_barItems.FindIndex(
										pBarFolder->m_barItems.GetCount());
				...
				switch (msg.message)
				{
					case WM_MOUSEMOVE:
					{
						...
						if (m_rcDownArrow.PtInRect(pt))
						{
							if (bHigh == FALSE)
							{
								DrawScrollButton(&dc, m_rcDownArrow, DFC_SCROLL,
				...
				switch (msg.message)
				{
					case WM_MOUSEMOVE:
					{
						...
						if (m_rcUpArrow.PtInRect(pt))
						{
							if (bHigh == FALSE)
							{
								DrawScrollButton(&dc, m_rcUpArrow, DFC_SCROLL,
...
int CXTOutBarCtrl::InsertItem(const int iFolder, const int iIndex, LPCTSTR lpszItemName,
							  const int iImage, const DWORD dwData)
{
	if (m_arFolder.GetSize() != 0)
	{
...
CImageList* CXTOutBarCtrl::SetFolderImageList(const int iFolder, CImageList* pImageList,
											  DWORD dwImageList)
{
	if (m_arFolder.GetSize() != 0)
	{
	...
	if (m_arFolder.GetSize() != 0)
	{
		...
		if (pBarItem->GetName().GetLength())
		{
			if (iType != rectItemIcon)
			{
				...
				if (IsSmallIconView(iFolder))
				{
					szLabel = dc.GetTextExtent(pBarItem->GetName(),
											   pBarItem->GetName().GetLength());
				}
				else
				{
					CRect rc;
		...
		if (IsSmallIconView(iFolder))
		{
			if (iType == rectItemIcon)
				szAll = szImage;
			else if (iType == rectItemLabel)
				szAll = szLabel;
			else if (iType == rectItemBoth)
				szAll = CSize(szImage.cx + szLabel.cx + m_sizeOffset.cx,
		...
		else
		{
			...
			else if (iType == rectItemBoth)
			{
				szAll = CSize(szImage.cx > szLabel.cx ? szImage.cx : szLabel.cx,
							  szLabel.cy + szImage.cy + m_sizeOffset.cy + m_nIconSpacingLarge);
			}
		}
		return szAll;
	}
...
void CXTOutBarCtrl::DrawItem(CDC* pDC, const int iFolder, CRect rc, const int iIndex,
								...
								CRect* pInsideRect /*= NULL*/)
{
	...
	{
		rect.SetRect(rc.left + (rc.Width() - sz.cx) / 2, top,
					 rc.left + (rc.Width() - sz.cx) / 2 + sz.cx, top + sz.cy);
		rect.top -= y + 3;
		rect.bottom -= y + 2;
	}
}
...
void CXTOutBarCtrl::StartGroupEdit(const int iIndex)
{
	...
	if (m_arFolder.GetSize() != 0)
	{
		...
		if (pEdit->Create(pBarFolder->GetName(), WS_CHILD | WS_VISIBLE | ES_CENTER | ES_AUTOHSCROLL,
						  rc, this, 0xFF, XTPToUInt(iIndex), true, false))
		{
...
void CXTOutBarCtrl::StartItemEdit(const int iIndex)
{
	if (m_arFolder.GetSize() != 0)
	{
		...
		CXTOutBarEditItem* pEdit = new CXTOutBarEditItem;
		if (pEdit->Create(pBarItem->GetName(), dwStyle, rcLabel, this, 0xFF, XTPToUInt(iIndex),
						  false, bNoDown))
		{
			pEdit->Initialize(this);
			pEdit->SetFocus();
...
void CXTOutBarCtrl::DrawPreviewBitmap(CWnd* pWnd, CDC* pDC, int nOffset)
{
	...
	dcMemSourceNonClient.FillSolidRect(0, 0, rectWindow.Width(), rectWindow.Height(),
									   GetXtremeColor(COLOR_WINDOW));
	pWnd->Print(&dcMemSourceNonClient, PRF_NONCLIENT);

	pDC->BitBlt(0, 0, rectWindow.Width(), rectWindow.Height(), &dcMemSourceNonClient, 0, 0,
				SRCCOPY);
...
void CXTOutBarCtrl::AnimateFolderScroll(const int iFrom, const int iTo)
{
	...
	bmpFrom.CreateCompatibleBitmap(&dc, rc.Width(),
								   rc.Height() + nFolderHeight * (nBottomItem - nTopItem));
	bmpTo.CreateCompatibleBitmap(&dc, rc.Width(),
								 rc.Height() + nFolderHeight * (nBottomItem - nTopItem));
	...
	if (pWnd1)
	{
		...
		PaintItems(&memDC, nTopItem, rcFrom);
	...
	if (pWnd2)
	{
		...
		PaintItems(&memDC, nBottomItem, rcTo);
	...
	if (iTo > iFrom)
	{
		...
		for (int y = rcTo.top - nFolderHeight; y > rcFrom.bottom; y -= nFolderHeight)
		{
			...
			memDC.SelectObject(&bmpTo);
			dc.BitBlt(rc.left, y, rc.Width(), rc.bottom - y + (iTo - iFrom) * nFolderHeight, &memDC,
					  0, 0, SRCCOPY);
			Sleep(XTPToDWORDChecked(m_nAnimationTickCount));
		}
	}
	else
	{
...
void CXTOutBarCtrl::SetAnimSelHighlight(const int iTime)
{
	if (iTime <= 0)
		...
		SetTimer(3, XTPToUInt(iTime), NULL);
	...
	if (lResult == TRUE)
	{
			...
			if ((m_dwFlags & OBS_XT_REMOVEITEMS) || (m_dwFlags & OBS_XT_EDITITEMS))
			{
				...
				if (m_dwFlags & OBS_XT_REMOVEITEMS)
				{
					CXTPResourceManager::AssertValid(
						XTPResourceManager()->LoadString(&string, XTP_IDC_REMOVEITEM));
					pPopup->AppendMenu(MF_STRING, XTP_IDC_REMOVEITEM, string);
				}
				if (m_dwFlags & OBS_XT_EDITITEMS)
				{
					CXTPResourceManager::AssertValid(
						XTPResourceManager()->LoadString(&string, XTP_IDC_RENAMEITEM));
		...
		else if (m_nHitInternal1 == hitFolder)
		{
			if ((m_dwFlags & OBS_XT_REMOVEGROUPS) || (m_dwFlags & OBS_XT_EDITGROUPS))
			{
				...
				{
					CXTPResourceManager::AssertValid(
						XTPResourceManager()->LoadString(&string, XTP_IDC_REMOVEGROUP));
					pPopup->AppendMenu(MF_STRING, XTP_IDC_REMOVEITEM, string);
				}
				if (m_dwFlags & OBS_XT_EDITGROUPS)
				{
					CXTPResourceManager::AssertValid(
						XTPResourceManager()->LoadString(&string, XTP_IDC_RENAMEGROUP));
					pPopup->AppendMenu(MF_STRING, XTP_IDC_RENAMEITEM, string);
				}
			}
		}

		pPopup->CheckMenuItem(UINT(IsSmallIconView(m_nSelFolder) ? XTP_IDC_SMALLICON
																 : XTP_IDC_LARGEICON),
		...
		if (pImageManager)
		{
			...
			pImageManager->SetVectorIcon(hModule, type, IDR_XAML_ICON_OUTBAR_SMALL,
										 XTP_IDC_SMALLICON,
										 XTPToUInt(GetSystemMetrics(SM_CXSMICON)));
			pImageManager->SetVectorIcon(hModule, type, IDR_XAML_ICON_OUTBAR_LARGE,
										 XTP_IDC_LARGEICON,
										 XTPToUInt(GetSystemMetrics(SM_CXSMICON)));
		}

		XTPContextMenu(pPopup, TPM_LEFTALIGN | TPM_RIGHTBUTTON, pt.x, pt.y, pWndPopupOwner,
					   pImageManager);
	}
							 ...
							 CRect* pInsideRect /*= NULL*/)
{
	...
	if ((m_bUpArrow && iIndex == m_iFirst) || (m_bDownArrow && iIndex == m_iLast)
...
void CXTToolBox::DrawIcon(CDC* pDC, int iIcon, int iFolder, bool bHilight)
{
	...
	if (bHilight)
	{
		if (m_bIconPressed)
		{
			pDC->Draw3dRect(rcIcon, GetXtremeColor(COLOR_3DSHADOW),
							GetXtremeColor(COLOR_3DHILIGHT));
		}
		else
		{
			pDC->Draw3dRect(rcIcon, GetXtremeColor(COLOR_3DHILIGHT),
...
void CXTToolBox::DrawItem(CDC* pDC, const int iFolder, CRect rc, const int iIndex,
						  const BOOL bOnlyImage)
{
	if (m_arFolder.GetSize() != 0)
	{
...
void CXTToolBox::OnDraw(CDC* pDC)
{
	...
	if (!GetFolderChild())
	{
		...
		if (GetItemCount() > 0 && m_iLast < GetItemCount() - 1
			&& m_rcDownArrow.IntersectRect(rc, m_rcDownArrow))
		{
			if (m_bDownPressed)
			{
				DrawScrollButton(pDC, m_rcDownArrow, DFC_SCROLL, DFCS_SCROLLDOWN | DFCS_PUSHED);
	...
	if (m_arFolder.GetSize() != 0)
	{
		...
		switch (eHilight)
		{
			...
			case xtMouseHover:
			{
				...
				pDC->Draw3dRect(rcFolder, GetXtremeColor(COLOR_3DHILIGHT),
								GetXtremeColor(COLOR_3DSHADOW));
				pDC->SetBkMode(TRANSPARENT);
			...
			{
				pDC->FillSolidRect(rcFolder, GetXtremeColor(COLOR_3DFACE));
				pDC->Draw3dRect(rcFolder, GetXtremeColor(COLOR_3DSHADOW),
								GetXtremeColor(COLOR_3DHILIGHT));
				pDC->SetBkMode(TRANSPARENT);
				pDC->SelectObject(GetFontX());
...
BOOL CXTToolBox::CreateEx(DWORD dwExStyle, DWORD dwStyle, const RECT& rect, CWnd* pParentWnd,
						  UINT nID, const DWORD dwFlags)
{
