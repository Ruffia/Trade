...
void CXTButtonTheme::SetAlternateColors(COLORREF clr3DFace, COLORREF clr3DHilight,
										COLORREF clr3DShadow, COLORREF clrBtnText)
{
...
CPoint CXTButtonTheme::GetTextPosition(UINT /*nState*/, CRect& rcItem, CSize& sizeText,
									   CXTButton* pButton)
{
	CRect rcText = rcItem;
	...
	else
	{
		...
		switch (pButton->GetStyle() & (BS_CENTER | BS_LEFT | BS_RIGHT))
		{
			...
			case BS_RIGHT: point.x = rcItem.right - pButton->GetBorderGap() - sizeText.cx; break;
...
CPoint CXTButtonTheme::CalculateImagePosition(CDC* pDC, UINT /*nState*/, CRect& rcItem,
	...
	else
	{
		...
		if (pButton->GetXButtonStyle() & BS_XT_TWOROWS)
		{
			...
			switch (pButton->GetStyle() & (BS_CENTER | BS_LEFT | BS_RIGHT))
			{
				...
				default:
					point.x = rcItem.left
							  + __max(0, ((rcItem.Width() - pButton->GetImageSize().cx) / 2));
					break;
			}
		}
		else
...
void CXTButtonTheme::DrawButtonIcon(CDC* pDC, UINT nState, CRect& rcItem, CXTButton* pButton)
{
	...
	CPoint pt = CalculateImagePosition(pDC, nState, rcItem, (pButton->GetSelectedIcon() != NULL),
									   pButton);
	...
	else if (pButton->GetHilite() || bSelected)
	{
		pIcon->Draw(pDC, pt,
					pButton->GetChecked()
						? pIcon->GetCheckedIcon()
						: (nState & ODS_SELECTED) ? pIcon->GetPressedIcon() : pIcon->GetHotIcon(),
					pButton->GetImageSize());
	}
	else
	{
...
BOOL CXTButtonTheme::DrawWinThemeBackground(LPDRAWITEMSTRUCT lpDIS, CXTButton* pButton)
{
	if (UseWinXPThemes(pButton))
	{
		...
		int nState   = XTPToIntChecked(lpDIS->itemState);
		...
		HBRUSH hBrush = (HBRUSH)pButton->GetParent()->SendMessage(WM_CTLCOLORBTN,
																  (WPARAM)pDC->GetSafeHdc(),
																  (LPARAM)pButton->GetSafeHwnd());
		if (hBrush)
		{
		...
		if (m_themeWrapper->DrawThemeBackground(pDC->GetSafeHdc(), BP_PUSHBUTTON, iStateId, &rcItem,
												NULL)
			!= S_OK)
		{
...
BOOL CXTButtonTheme::DrawButtonThemeBackground(LPDRAWITEMSTRUCT lpDIS, CXTButton* pButton)
{
	...
	if (dwxStyle & BS_XT_FLAT)
	{
		...
		{
			pDC->Draw3dRect(rcItem, bSelected ? m_crBorderShadow : m_crBorderHilite,
	...
	else // classic button
	{
		...
		pDC->Draw3dRect(rcItem, bSelected ? m_crBorder3DShadow : m_crBorder3DHilite,
...
{
	CXTPContextTextColorHandler foreHandler(pDC, XTPToIntChecked(m_crText));
...
void CXTButtonTheme::DrawItem(LPDRAWITEMSTRUCT lpDIS, CXTButton* pButton)
{
	...
	CDC* pDC	 = CDC::FromHandle(lpDIS->hDC);
	CRect rcItem = lpDIS->rcItem;
	int nState   = XTPToInt(lpDIS->itemState);

	// Set the background mode to transparent.
	CXTPContextBkModeHandler modeHandler(pDC, TRANSPARENT);

	// draw the button text, icon or bitmap.
	DrawButtonText(pDC, XTPToUInt(nState), rcItem, pButton);
	DrawButtonIcon(pDC, XTPToUInt(nState), rcItem, pButton);
	...
	if ((pButton->GetXButtonStyle() & BS_XT_SHOWFOCUS))
	{
		...
		{
			DrawFocusRect(pDC, XTPToUInt(nState), rcItem, pButton);
		}
	}
}
...
void CXTButtonThemeOfficeXP::RefreshMetrics()
{
	...
	m_crBack.SetStandardValue(
		GetXtremeColor(UINT(m_bWordTheme ? XPCOLOR_TOOLBAR_FACE : COLOR_BTNFACE)));
...
void CXTButtonThemeOfficeXP::DrawButtonIcon(CDC* pDC, UINT nState, CRect& rcItem,
											CXTButton* pButton)
{
	if (pButton == NULL || m_bShowIcon == FALSE)
		return;
	...
	CPoint pt = CalculateImagePosition(pDC, nState, rcItem, FALSE, pButton);
...
BOOL CXTButtonThemeOfficeXP::DrawButtonThemeBackground(LPDRAWITEMSTRUCT lpDIS, CXTButton* pButton)
{
	...
	if (pButton->GetHilite() || bPressed || pButton->GetChecked())
	{
		pDC->FillSolidRect(rcItem, bPressed ? m_crBackPushed
											: pButton->GetChecked()
												  ? (pButton->GetHilite() ? m_crBackPushed
																		  : m_crBackChecked)
												  : m_crBackHilite);
...
void CXTButtonThemeOffice2003::DrawButtonIcon(CDC* pDC, UINT nState, CRect& rcItem,
											  CXTButton* pButton)
{
	if (pButton == NULL || m_bShowIcon == FALSE)
		return;
...
BOOL CXTButtonThemeOffice2003::DrawButtonThemeBackground(LPDRAWITEMSTRUCT lpDIS, CXTButton* pButton)
{
	...
	if (bChecked || bHilight || bPressed)
	{
		if (bPressed)
		{
			XTPDrawHelpers()->GradientFill(pDC, &rcItem, XTPColorManager()->grcLunaPushed, FALSE);
		}
		else if (bHilight)
		{
			XTPDrawHelpers()->GradientFill(pDC, &rcItem, XTPColorManager()->grcLunaSelected, FALSE);
		}
		else if (bChecked)
		{
			XTPDrawHelpers()->GradientFill(pDC, &rcItem, XTPColorManager()->grcLunaChecked, FALSE);
		}
...
void CXTDisabledButtonTheme::DrawButtonIcon(CDC* pDC, UINT nState, CRect& rcItem,
											CXTButton* pButton)
{
	if (pButton == NULL || m_bShowIcon == FALSE)
		return;
	...
	if (nState & ODS_DISABLED)
	{
		pDC->DrawState(pt, pIconManager->GetExtent(), pIconManager->GetIcon(),
					   DSS_NORMAL | DSS_DISABLED, (HBRUSH)0);
	}
	else if (pButton->GetHilite() || bSelected)
	{
		pDC->DrawState(pt, pIconManager->GetExtent(),
					   (nState & ODS_SELECTED) ? pIconManager->GetPressedIcon()
											   : pIconManager->GetHotIcon(),
					   DSS_NORMAL, (HBRUSH)0);
	}
	else
	{
		pDC->DrawState(pt, 0, pIconManager->GetIcon(), DSS_NORMAL, (HBRUSH)0);
	}
}
