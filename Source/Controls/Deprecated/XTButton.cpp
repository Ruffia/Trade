...
void CXTButton::OnDraw(CDC* pDC)
{
	...
	if (dwState & UISF_HIDEACCEL)
		dis.itemState |= ODS_NOACCEL;
	if (dwState & UISF_HIDEFOCUS)
...
LRESULT CXTButton::OnSetStyle(WPARAM wParam, LPARAM lParam)
{
	if (m_bOwnerDraw)
	{
		...
		ModifyStyle(0, DWORD(wParam | BS_OWNERDRAW), 0);
...
{
	return CXTButton::SetIcon(size, MAKEINTRESOURCE(nID), MAKEINTRESOURCE(nHotID), bRedraw);
}

BOOL CXTButton::SetIcon(CSize size, LPCTSTR lpszID, LPCTSTR lpszHotID /*= NULL*/,
						BOOL bRedraw /*= TRUE*/)
{
...
BOOL CXTButton::SetIcon(CSize size, const CXTPImageManagerIconHandle& hIcon,
...
void CXTButton::SetAlternateColors(COLORREF clr3DFace, COLORREF clr3DHilight, COLORREF clr3DShadow,
								   COLORREF clrBtnText)
{
	if (IsThemeValid())
	{
		GetTheme()->SetAlternateColors(clr3DFace, clr3DHilight, clr3DShadow, clrBtnText);
	}
}
