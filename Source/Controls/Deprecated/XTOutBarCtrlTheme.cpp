...
void CXTOutBarCtrlTheme::DrawItemIcon(CDC* pDC, CPoint pt, CXTOutBarItem* pBarItem,
									  CImageList* pImageList)
{
	...
	else
	{
		...
		if (m_bFlatIcon)
		{
			...
			pDC->DrawState(pt, CSize(0, 0), hIcon, DST_ICON | DSS_MONO, (HBRUSH)brush);
			brush.DeleteObject();
		}
		else
		{
			pDC->DrawState(pt, CSize(0, 0), hIcon, DSS_DISABLED | DST_ICON, (HBRUSH)NULL);
		}
...
void CXTOutBarCtrlTheme::DrawFolder(CDC* pDC, CRect rect, CXTOutBarFolder* pBarFolder,
...
void CXTOutBarCtrlTheme::DrawIconFrame(CDC* pDC, CRect rcIcon, BOOL bSelected, BOOL bPressed)
{
	...
	{
		if (bPressed)
			pDC->Draw3dRect(rcIcon, GetXtremeColor(COLOR_WINDOWFRAME),
							GetXtremeColor(COLOR_3DFACE));
		else
			pDC->Draw3dRect(rcIcon, GetXtremeColor(COLOR_3DFACE),
...
void CXTOutBarCtrlThemeOfficeXP::DrawIconFrame(CDC* pDC, CRect rcIcon, BOOL bSelected,
											   BOOL bPressed)
{
	if (!bSelected)
	{
		pDC->FillSolidRect(rcIcon, GetXtremeColor(UINT(bPressed ? XPCOLOR_HIGHLIGHT_PUSHED
																: XPCOLOR_HIGHLIGHT)));
...
void CXTOutBarCtrlThemeOffice2003::RefreshMetrics()
{
	...
	if (!XTPColorManager()->IsLunaColorsDisabled())
	{
		...
		switch (systemTheme)
		{
			...
			case xtpSystemThemeAero: m_clrBack.SetStandardValue(RGB(216, 231, 252)); break;
			case xtpSystemThemeOlive: m_clrBack.SetStandardValue(RGB(226, 231, 191)); break;
			case xtpSystemThemeSilver: m_clrBack.SetStandardValue(RGB(223, 223, 234)); break;
		}
...
void CXTOutBarCtrlThemeOffice2003::DrawFolder(CDC* pDC, CRect rect, CXTOutBarFolder* pBarFolder,
											  const XTMouseState eHilight)
{
	CRect rcFolder(rect);
	...
	{
		case xtMouseNormal:
			XTPDrawHelpers()->GradientFill(pDC, rect, XTPColorManager()->grcDockBar.clrDark,
										   XTPColorManager()->grcDockBar.clrLight, FALSE);
			break;

		case xtMouseSelect: XTPDrawHelpers()->GradientFill(pDC, rect, m_grcPushed, FALSE); break;

		case xtMouseHover: XTPDrawHelpers()->GradientFill(pDC, rect, m_grcSelected, FALSE); break;
	}
...
void CXTOutBarCtrlThemeOffice2003::DrawIconFrame(CDC* pDC, CRect rcIcon, BOOL bSelected,
												 BOOL bPressed)
{
	if (!m_bLunaTheme)
	{
