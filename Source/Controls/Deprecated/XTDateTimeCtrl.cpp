...
{
	ASSERT(::IsWindow(m_hWnd));
	WPARAM wParam = XTPToWPARAM((pTimeNew == NULL) ? GDT_NONE : GDT_VALID);
...
BOOL CXTDateTimeCtrl::SetTime(const COleDateTime& timeNew)
{
	...
	if (timeNew.GetStatus() == COleDateTime::valid && GetAsSystemTime(timeNew, sysTime))
	{
		wParam = GDT_VALID;
	}

	bRetVal = (BOOL)::SendMessage(m_hWnd, DTM_SETSYSTEMTIME, wParam, (LPARAM)&sysTime);
...
{
	SYSTEMTIME sysTime;
	DWORD dwResult = (DWORD)::SendMessage(m_hWnd, DTM_GETSYSTEMTIME, 0, (LPARAM)&sysTime);
...
{
	BOOL bWasVisible = (0 != (dwStyle & WS_VISIBLE));
	dwStyle &= ~WS_VISIBLE;
...
BOOL CXTMonthCalCtrl::SetCurSel(const COleDateTime& refTime)
{
	...
	if (GetAsSystemTime(refTime, sysTime) && refTime.GetStatus() == COleDateTime::valid)
	{
		bRetVal = (BOOL)::SendMessage(m_hWnd, MCM_SETCURSEL, 0, (LPARAM)&sysTime);
	}
...
BOOL CXTMonthCalCtrl::GetToday(COleDateTime& refTime) const
{
	...
	SYSTEMTIME sysTime;
	BOOL bResult = (BOOL)::SendMessage(m_hWnd, MCM_GETTODAY, 0, (LPARAM)&sysTime);
...
BOOL CXTMonthCalCtrl::SetFirstDayOfWeek(int iDay, int* lpnOld /* = NULL */)
{
	ASSERT(::IsWindow(m_hWnd));
	DWORD dwResult;
	dwResult = (DWORD)::SendMessage(m_hWnd, MCM_SETFIRSTDAYOFWEEK, 0, (LPARAM)iDay);
...
BOOL CXTMonthCalCtrl::SetRange(const COleDateTime* pMinRange, const COleDateTime* pMaxRange)
{
	...
	return (BOOL)::SendMessage(m_hWnd, MCM_SETRANGE, wFlags, (LPARAM)&sysTimes);
}
...
DWORD CXTMonthCalCtrl::GetRange(COleDateTime* pMinRange, COleDateTime* pMaxRange) const
{
	...
	DWORD dwRanges = (DWORD)::SendMessage(m_hWnd, MCM_GETRANGE, 0, (LPARAM)&sysTimes);
...
int CXTMonthCalCtrl::GetMonthRange(COleDateTime& refMinRange, COleDateTime& refMaxRange,
								   DWORD dwFlags) const
{
	ASSERT(::IsWindow(m_hWnd));
	...
	SYSTEMTIME sysTimes[2];
	memset(sysTimes, 0, sizeof(sysTimes));
	int nCount = (int)::SendMessage(m_hWnd, MCM_GETMONTHRANGE, (WPARAM)dwFlags, (LPARAM)&sysTimes);
...
int CXTMonthCalCtrl::GetMonthRange(LPSYSTEMTIME pMinRange, LPSYSTEMTIME pMaxRange,
								   DWORD dwFlags) const
{
	ASSERT(::IsWindow(m_hWnd));
...
BOOL CXTMonthCalCtrl::GetSelRange(LPSYSTEMTIME pMinRange, LPSYSTEMTIME pMaxRange) const
{
	...
	SYSTEMTIME sysTimes[2];
	BOOL bReturn = (BOOL)::SendMessage(m_hWnd, MCM_GETSELRANGE, 0, (LPARAM)&sysTimes);
...
BOOL CXTMonthCalCtrl::SetSelRange(const LPSYSTEMTIME pMinRange, const LPSYSTEMTIME pMaxRange)
{
	...
	return (BOOL)::SendMessage(m_hWnd, MCM_SETSELRANGE, 0, (LPARAM)&sysTimes);
}
...
BOOL CXTMonthCalCtrl::SetSelRange(const COleDateTime& refMinRange, const COleDateTime& refMaxRange)
{
	...
	{
		if (GetAsSystemTime(refMinRange, sysTimes[0]) && GetAsSystemTime(refMaxRange, sysTimes[1]))
		{
			bResult = (BOOL)::SendMessage(m_hWnd, MCM_SETSELRANGE, 0, (LPARAM)sysTimes);
		}
	}
...
BOOL CXTMonthCalCtrl::GetSelRange(COleDateTime& refMinRange, COleDateTime& refMaxRange) const
{
	...
	SYSTEMTIME sysTimes[2];
	memset(sysTimes, 0, sizeof(sysTimes));
	BOOL bResult = (BOOL)::SendMessage(m_hWnd, MCM_GETSELRANGE, 0, (LPARAM)&sysTimes);
												...
												const CTime* pMinRange, const CTime* pMaxRange)
{
	...
	CXTDateTimeCtrl* pWnd = (CXTDateTimeCtrl*)pDX->m_pDlgWnd->GetDlgItem(
		static_cast<int>(pDX->m_idLastControl));
#else
	CXTDateTimeCtrl* pWnd = (CXTDateTimeCtrl*)CWnd::FromHandle(pDX->m_hWndLastControl);
#endif // MFC 7.0
	...
	if (!pDX->m_bSaveAndValidate)
	{
		...
		{
#pragma warning(push)
// C4365: 'argument': conversion from 'unsigned int' to 'int', signed/unsigned mismatch
#pragma warning(disable : 4365)
#if _MSC_VER > 1200 // MFC 7.0
			TRACE(traceAppMsg, 0,
				  "Warning: initial dialog data is out of range in "
				  "control ID %d.\n",
				  pDX->m_idLastControl);
#else
#	ifdef _DEBUG
											 ...
											 const CTime* pMinRange, const CTime* pMaxRange)
{
	ASSERT(pMinRange == NULL || pMaxRange == NULL || *pMinRange <= *pMaxRange);
#if _MSC_VER > 1200 // MFC 7.0
	CXTMonthCalCtrl* pWnd = (CXTMonthCalCtrl*)pDX->m_pDlgWnd->GetDlgItem(
		static_cast<int>(pDX->m_idLastControl));
#else
	CXTMonthCalCtrl* pWnd = (CXTMonthCalCtrl*)CWnd::FromHandle(pDX->m_hWndLastControl);
#endif // MFC 7.0
