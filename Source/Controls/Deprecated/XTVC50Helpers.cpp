...
CXTStringHelper::CXTStringHelper(CString strIn)
	: CString(strIn)
{
}
...
int CXTStringHelper::Insert(int nIndex, LPCTSTR pstr)
{
	...
	if (nInsertLength > 0)
	{
		...
		// move existing bytes down
		memmove(m_pchData + nIndex + nInsertLength, m_pchData + nIndex,
				(nNewLength - nIndex - nInsertLength + 1) * sizeof(TCHAR));
		memmove(m_pchData + nIndex, pstr, nInsertLength * sizeof(TCHAR));
		GetData()->nDataLength = nNewLength;
	}
...
int CXTStringHelper::Delete(int nIndex, int nCount)
{
	...
	if (nCount > 0 && nIndex < nNewLength)
	{
		...
		memcpy(m_pchData + nIndex, m_pchData + nIndex + nCount, nBytesToCopy * sizeof(TCHAR));
...
BOOL CXTHeaderCtrlHelper::GetOrderArray(LPINT piArray, int iCount /* = -1 */)
{
	...
	return (BOOL)::SendMessage(m_hWnd, HDM_GETORDERARRAY, (WPARAM)iCount, (LPARAM)piArray);
}
