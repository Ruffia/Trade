										   ...
										   CPoint point, BOOL bInColor)
{
	...
	pDC->BitBlt(point.x + 1, point.y + 1, rcImage.Width(), rcImage.Height(), &memDC, 0, 0,
				0xB8074A);
										  ...
										  UINT nID /*= AFX_IDW_PANE_FIRST*/)
{
	...
	CRect rect(0, 0, 0, 0);
	if (!pWnd->Create(NULL, NULL, dwStyle, rect, pParentWnd, nID, pContext))
	{
...
_XTP_EXT_CLASS BOOL AFXAPI XTPContextMenu(CMenu* pPopup, UINT nFlags, int x, int y, CWnd* pWnd,
										  int nIDBitmap, BOOL bNoNotify)
{
	BOOL bResult = FALSE;
	nIDBitmap;
	bNoNotify;
	...
	if (nIDBitmap > 0)
	{
		...
		pImageManager->SetIcons(XTPToUInt(nIDBitmap));
	...
	bResult = CXTPCommandBars::TrackPopupMenu(pPopupBar, nFlags | (bNoNotify ? TPM_NONOTIFY : 0), x,
											  y, pWnd);
	pPopupBar->InternalRelease();
...
_XTP_EXT_CLASS BOOL AFXAPI XTPContextMenu(CMenu* pPopup, UINT nFlags, int x, int y, CWnd* pWnd,
										  CXTPImageManager* pImageManager, BOOL bNoNotify)
{
	ASSERT_VALID(pPopup);
