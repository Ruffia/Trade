...
enum XTPAnimationDCType
{
	...
	xtpAniSlide,   // Animates a window to slide in from the left.
...
public:
	...
	//     device context used to draw an off-screen bitmap.
	// Parameters:
	//     hDestDC - Handle to the destination device context the memory
	//               device is BitBlt to.
	//     rcPaint - Size of the area to paint.
	//     clrBack - Represents background color for fill; can be COLORREF or
	//               gradient values using CXTPPaintManagerColorGradient.
	//     bHorz   - Used when drawing a gradient background. TRUE to draw
	//               gradient from left-to-right, otherwise drawn top-to-bottom.
	// ---------------------------------------------------------------------------
	...
	CXTPAnimationDC(HDC hDestDC, const CRect& rcPaint, const CXTPPaintManagerColorGradient& clrBack,
					const BOOL bHorz = FALSE); // <combine
											   // CXTPAnimationDC::CXTPAnimationDC@HDC@const CRect&>

	// ---------------------------------------------------------------------------
	// Summary:
	//     Destroys a CXTPAnimationDC object, handles cleanup and deallocation.
	// ---------------------------------------------------------------------------
	...
	//     pDestDC -         [in] Pointer to the device context you must draw to.
	...
	//     nSteps -          [in] Number of steps to take during the animation.
	//     nAnimationTime -  [in] Amount of time to rest, in milliseconds, between
	//                       each step.
	// Remarks:
	//     You can add new animation effects to call the SetCustomAnimation member.
	// Example:
