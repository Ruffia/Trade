...
//     DLL, then add the following two lines to your stdafx.h file. This will
...
// ------------------------------------------------------------------------------
// Summary:
//     The XTPDrawEmbossed function will draw an embossed icon onto the specified
//     device context from the image list <i>imageList</i>. It is typically used
//     by toolbars and menus to draw a disabled icon in color.
// Parameters:
...
//     file or sub-directory whose name matches the specified name.
// Parameters:
//     lpszFileName -  [in] A pointer to a NULL-terminated string that specifies a
//                     valid directory or path and file name, which can contain
//                     wild card characters (* and ?). If the string ends with a
//                     wild card, a period, or a directory name, then the user must
//                     have access to the root and all sub-directories on the path.
// Returns:
//     TRUE if the file or sub-directory exists, otherwise FALSE.
// ------------------------------------------------------------------------------
...
//     pOwnerWnd -   Owner of the view. If NULL, then 'pParentWnd' is used.
...
//                  screen-position flag can be one of the following:
...
//     nIDBitmap -  Toolbar resource ID that contains images to be displayed with menu
//                  commands.
