	...
	: m_hDestDC(hDestDC)
{
	...
	m_bitmap.Attach(::CreateCompatibleBitmap(m_hDestDC, m_rect.Width(), m_rect.Height()));
	...
	: m_hDestDC(hDestDC)
{
	...
	if (!clrBack.IsNull())
	{
		XTPDrawHelpers()->GradientFill(this, CRect(0, 0, m_rect.Width(), m_rect.Height()), clrBack,
									   bHorz);
	}
}
...
CXTPAnimationDC::~CXTPAnimationDC()
{
	...
	if (m_hDestDC)
	{
		::BitBlt(m_hDestDC, m_rect.left, m_rect.top, m_rect.Width(), m_rect.Height(), m_hDC, 0, 0,
				 SRCCOPY);
	}
...
void CXTPAnimationDC::Animate(int nType, int nSteps, int nAnimationTime)
{
	...
	switch (nType)
	{
		case xtpAniDefault:
		{
			...
			// ?url=/library/en-us/regentry/regazbd.asp

			HKEY hKey	= NULL;
			DWORD dwType = 0, dwValue = 0, dwSize = sizeof(dwValue);
			::RegOpenKey(HKEY_CURRENT_USER, _T("Control Panel\\Desktop"), &hKey);
			::RegQueryValueEx(hKey, _T("UserPreferencesMask"), NULL, &dwType, (LPBYTE)&dwValue,
							  &dwSize);
			::RegCloseKey(hKey);
		...
		case xtpAniRandom:
		{
			switch (RAND_S() % 3)
			{
				case 0: nType = xtpAniFade; break;

				case 1: nType = xtpAniSlide; break;

				default: nType = xtpAniUnfold; break;
			}
		}
		break;
	}
...
void CXTPAnimationDC::DefaultAnimation(CRect rc, CDC* pDestDC, CDC* pSrcDC, int nType, int nSteps,
									   int nAnimationTime)
{
	CRect r;
	switch (nType)
	{
		case xtpAniSlide:
		{
			...
			int i;
			for (i = 0; i < rc.Height(); i += (1 + (rc.Height() / nSteps)))
			{
				pDestDC->BitBlt(rc.left, rc.top, rc.Width(), i, pSrcDC, 0, bottom - i, SRCCOPY);
		...
		case xtpAniUnfold:
		{
			...
			for (i = 0; i < nSteps; ++i)
			{
				...
				pDestDC->BitBlt(r.left, r.top, r.Width(), r.Height(), pSrcDC, 0, 0, SRCCOPY);
				Sleep(XTPToDWORDChecked(nAnimationTime / nSteps));
			}
		}
		break;
...
void AFX_CDECL CXTPAnimationDC::AlphaBlendU(PBYTE pDest, PBYTE pSrcBack, int cx, int cy, PBYTE pSrc,
											BYTE byAlpha)
{
