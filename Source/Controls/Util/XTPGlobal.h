...
//=============================================================================
// Summary:
//     The CXTPTcbItem class is used to maintain information for a particular
//     tab item in a CXTPExcelTabCtrl, CXTPTabCtrl, or CXTPTabView object.
// See Also:
...
public:
	//-------------------------------------------------------------------------
	// Summary:
	//     Constructs a CXTPTcbItem object.
	//-------------------------------------------------------------------------
	CXTPTcbItem();
...
public:
	//-------------------------------------------------------------------------
	// Summary:
	//     Constructs a CXTPLogFont object.
	// Parameters:
	//     rhs - The CXTPLogFont object to copy from.
	//     logFont - The LOGFONT structure to initialize the class object with.
	//-------------------------------------------------------------------------
	CXTPLogFont();
	CXTPLogFont(const CXTPLogFont& rhs); //<combine CXTPLogFont::CXTPLogFont>
	CXTPLogFont(const LOGFONT& logFont); //<combine CXTPLogFont::CXTPLogFont>
...
public:
	...
	// Parameters:
	//     rhs - The CXTPLogFont object to copy from.
	//     logFont - The LOGFONT structure to copy from.
	//-------------------------------------------------------------------------
	CXTPLogFont& operator=(const CXTPLogFont& rhs);
	CXTPLogFont& operator=(const LOGFONT& logFont);

public:
	DWORD dwType; // Used to hold the font type, i.e. TT_FONT, DEVICE_FONT.
};

AFX_INLINE CXTPLogFont::CXTPLogFont()
{
	memset(this, 0, sizeof(*this));
}

AFX_INLINE CXTPLogFont::CXTPLogFont(const CXTPLogFont& rhs)
{
	*this = rhs;
}

AFX_INLINE CXTPLogFont::CXTPLogFont(const LOGFONT& logFont)
{
	*this = logFont;
}

AFX_INLINE CXTPLogFont& CXTPLogFont::operator=(const CXTPLogFont& rhs)
{
	memcpy(this, &rhs, sizeof(*this));
	return *this;
}

AFX_INLINE CXTPLogFont& CXTPLogFont::operator=(const LOGFONT& logFont)
{
	memcpy(static_cast<LOGFONT*>(this), &logFont, sizeof(logFont));
	return *this;
}
...
//=============================================================================
// Summary:
//     CXTPAuxData is a standalone global item data structure
...
public:
	//-------------------------------------------------------------------------
	// Summary:
	//     Destroys an CXTPAuxData object, handles cleanup and deallocation.
	//-------------------------------------------------------------------------
	virtual ~CXTPAuxData();
...
public:
	//-------------------------------------------------------------------------
	// Summary:
	//     Initializes the cursors used by the library.
	// Returns:
	...
	//-------------------------------------------------------------------------
	// Summary:
	//     Initializes the fonts used by the library.
	// Parameters:
	//     lpszHorzFaceName -  [in] Pointer to a NULL-terminated font name that is used
	//                         to create the horizontal fonts used by the library.
	//     lpszVertFaceName -  [in] Pointer to a NULL-terminated font name that is used
	//                         to create the vertical fonts used by the library.
	// Remarks:
	//     Call this member function to update all of the fonts used by the library.
	// Returns:
	//     TRUE if successful, otherwise FALSE if a font fails to load.
	//-------------------------------------------------------------------------
	...
	// Returns:
	//     TRUE if the version is 4.72 or higher, otherwise FALSE.
	//-------------------------------------------------------------------------
	...
	//-------------------------------------------------------------------------
	// Summary:
	//     Enables/disables Tahoma font usage for the library.
	// Parameters:
	...
	// Parameters:
	//     lpszFaceName - [in] A NULL-terminated string that represents the font
	//                    name to search for.
	// Remarks:
	//     Call this member function to determine if the font specified by
	//     <i>strFaceName</i> has been installed on the operating system.
	// Returns:
	...
	//     additional values to set that will override what is specified
	//     in the LOGFONT structure passed in.
	// Returns:
	...
	BOOL CreateSysFont(const CXTPLogFont& lf, CXTPFont& font, long lfWeight = -1,
					   ...
					   long lfEscapement = -1); // <combine CXTPAuxData::CreateSysFont@const
	...
	//     Initializes the fonts used by the library.
	// Parameters:
	//     pHorzFont -         [in] A pointer to a valid CFont object that is used to
	//                         define the fonts that the library will use.
	//     pVertFont -         [in] A pointer to a valid CFont object that is used to
	//                         define the vertical fonts used by the library. If NULL,
	//                         then 'pFont' will be used.
	//     lpszFaceName -      [in] A pointer to a NULL-terminated font name that is
	//                         used to create the horizontal fonts used by the library.
	//     lpszVertFaceName -  [in] A pointer to a NULL-terminated font name that is
	//                         used to create the vertical fonts used by the library.
	// Remarks:
	//     Call this member function to update the fonts used by the library.
	...
	// Returns:
	//     TRUE if successful, otherwise returns FALSE if a font fails to load.
	//-------------------------------------------------------------------------
	...
	// Parameters:
	//     bVerNumOnly -  [in] true to return the version number only
	//                    (excluding "Xtreme Toolkit v").
	// Remarks:
...
public:
	...
	int cxHThumb;   // Width, in pixels, of the thumb box in a horizontal scrollbar.
	int cyVThumb;   // Height, in pixels, of the thumb box in a vertical scrollbar.
