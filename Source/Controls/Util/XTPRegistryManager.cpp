...
HKEY CXTPRegistryManager::GetAppRegistryKey(REGSAM samDesired)
{
	...
	m_lResult = ::RegOpenKeyEx(m_hKeyBase, _T("Software"), 0, samDesired, hSoftKey);

	if (m_lResult != ERROR_SUCCESS)
		return NULL;

	DWORD dw  = 0;
	m_lResult = ::RegCreateKeyEx(hSoftKey, m_strRegistryKey, 0, REG_NONE, REG_OPTION_NON_VOLATILE,
								 samDesired, NULL, hCompanyKey, &dw);
...
HKEY CXTPRegistryManager::RecurseOpenKey(LPCTSTR lpszSection, REGSAM samDesired)
{
	...
	while (nIndex != -1)
	{
		...
		m_lResult = ::RegCreateKeyEx(hParentKey.operator HKEY() == NULL
										 ? hKeyBase
										 : hParentKey.operator HKEY(),
									 strKey, 0, REG_NONE, REG_OPTION_NON_VOLATILE, samDesired, NULL,
									 &hKey, &dw);
	...
	HKEY hAppKey = NULL;
	m_lResult = ::RegCreateKeyEx(hParentKey, strRegistryKey, 0, REG_NONE, REG_OPTION_NON_VOLATILE,
...
HKEY CXTPRegistryManager::GetSectionKey(LPCTSTR lpszSection, REGSAM samDesired)
{
	...
	DWORD dw  = 0;
	m_lResult = ::RegCreateKeyEx(hAppKey, lpszSection, 0, REG_NONE, REG_OPTION_NON_VOLATILE,
...
BOOL CXTPRegistryManager::WriteProfileInt(LPCTSTR lpszSection, LPCTSTR lpszEntry, int nValue)
{
	...
	if (m_strINIFileName.IsEmpty())
	{
		...
		m_lResult = ::RegSetValueEx(hSecKey, lpszEntry, NULL, REG_DWORD, (LPBYTE)&nValue,
									sizeof(nValue));
	...
	else
	{
		...
		return ::WritePrivateProfileString(lpszSection, lpszEntry, szT, m_strINIFileName);
	}
}

BOOL CXTPRegistryManager::WriteProfileBinary(LPCTSTR lpszSection, LPCTSTR lpszEntry, LPBYTE pData,
											 UINT nBytes)
{
	ASSERT(lpszSection != NULL);
	ASSERT(lpszEntry != NULL);
	...
	if (m_strINIFileName.IsEmpty())
	{
		...
		m_lResult = ::RegSetValueEx(hSecKey, lpszEntry, NULL, REG_BINARY, pData, nBytes);
...
BOOL CXTPRegistryManager::WriteProfileString(LPCTSTR lpszSection, LPCTSTR lpszEntry,
											 LPCTSTR lpszValue)
{
	ASSERT(lpszSection != NULL);
	if (!lpszSection)
	...
	if (m_strINIFileName.IsEmpty())
	{
		...
		else
		{
			...
			m_lResult = ::RegSetValueEx(hSecKey, lpszEntry, NULL, REG_SZ, (LPBYTE)lpszValue,
...
UINT CXTPRegistryManager::GetProfileInt(LPCTSTR lpszSection, LPCTSTR lpszEntry, int nDefault)
{
	...
	if (m_strINIFileName.IsEmpty()) // use registry
	{
		...
		if (hSecKey == NULL)
			return XTPToUInt(nDefault);

		DWORD dwValue;
		DWORD dwType;
		DWORD dwCount = sizeof(DWORD);

		m_lResult = ::RegQueryValueEx(hSecKey, (LPTSTR)lpszEntry, NULL, &dwType, (LPBYTE)&dwValue,
									  &dwCount);
	...
	{
		ASSERT(m_strINIFileName.IsEmpty() == FALSE);
		return ::GetPrivateProfileInt(lpszSection, lpszEntry, nDefault, m_strINIFileName);
	}
}

BOOL CXTPRegistryManager::GetProfileBinary(LPCTSTR lpszSection, LPCTSTR lpszEntry, BYTE** ppData,
										   UINT* pBytes)
{
	ASSERT(lpszSection != NULL);
	ASSERT(lpszEntry != NULL);
	...
	*pBytes = 0;
	...
	if (m_strINIFileName.IsEmpty())
	{
		...
		m_lResult = ::RegQueryValueEx(hSecKey, (LPTSTR)lpszEntry, NULL, &dwType, NULL, &dwCount);
		...
		*ppData = new BYTE[*pBytes];
		...
		m_lResult = ::RegQueryValueEx(hSecKey, (LPTSTR)lpszEntry, NULL, &dwType, *ppData, &dwCount);
	...
	else
	{
		...
		*pBytes  = XTPToUIntChecked(nLen / 2);
		*ppData  = new BYTE[*pBytes];
		int i;
		for (i = 0; i < nLen; i += 2)
		{
...
CString CXTPRegistryManager::GetProfileString(LPCTSTR lpszSection, LPCTSTR lpszEntry,
											  LPCTSTR lpszDefault)
{
	ASSERT(lpszSection != NULL);
	ASSERT(lpszEntry != NULL);
	if (m_strINIFileName.IsEmpty())
	{
		...
		m_lResult = ::RegQueryValueEx(hSecKey, (LPTSTR)lpszEntry, NULL, &dwType,
									  (LPBYTE)strValue.GetBuffer(XTPToInt(dwCount / sizeof(TCHAR))),
									  &dwCount);
	...
	else
	{
		...
		TCHAR szT[4096];
		DWORD dw = ::GetPrivateProfileString(lpszSection, lpszEntry, lpszDefault, szT,
											 _countof(szT), m_strINIFileName);
		ASSERT(dw < 4095);
		UNUSED(dw);
		return szT;
	}
...
int CXTPRegistryManager::EnumValues(LPCTSTR lpszSection,
									...
									CStringArray* arrayNames)
{
	...
	while (::RegEnumValue(hKey, XTPToDWORD(index++), szValue, &dwLen, NULL, &dwType, NULL, NULL)
		   == ERROR_SUCCESS)
	{
		if (mapItems)
...
int CXTPRegistryManager::EnumKeys(LPCTSTR lpszSection, CStringArray& arrayKeys)
{
	...
	while (::RegEnumKeyEx(hKey, XTPToDWORD(index++), szValue, &dwLen, NULL, NULL, NULL, NULL)
		   == ERROR_SUCCESS)
	{
...
LONG CXTPRegistryManager::RecurseDeleteKey(HKEY hKey, LPCTSTR lpszKey)
{
	...
	while (::RegEnumKeyEx(hSubKey, 0, szBuffer, &dwSize, NULL, NULL, NULL, &time) == ERROR_SUCCESS)
	{
...
BOOL CXTPRegistryManager::WriteProfilePoint(LPCTSTR lpszSection, LPCTSTR lpszEntry, CPoint* pValue)
{
	...
	if (m_strINIFileName.IsEmpty())
	{
		...
		m_lResult = ::RegSetValueEx(hSecKey, lpszEntry, NULL, REG_BINARY, (LPBYTE)pValue,
									sizeof(CPoint));
...
BOOL CXTPRegistryManager::GetProfilePoint(LPCTSTR lpszSection, LPCTSTR lpszEntry, CPoint* ptResult)
{
	...
	if (m_strINIFileName.IsEmpty())
	{
		...
		ASSERT(dwType == REG_BINARY);
		m_lResult = ::RegQueryValueEx(hSecKey, (LPTSTR)lpszEntry, NULL, &dwType, (LPBYTE)ptResult,
									  &dwCount);
...
BOOL CXTPRegistryManager::GetProfileRect(LPCTSTR lpszSection, LPCTSTR lpszEntry, CRect* rcResult)
{
	...
	if (m_strINIFileName.IsEmpty())
	{
		...
		ASSERT(dwType == REG_BINARY);
		m_lResult = ::RegQueryValueEx(hSecKey, (LPTSTR)lpszEntry, NULL, &dwType, (LPBYTE)rcResult,
									  &dwCount);
	...
	else
	{
		...
		SCANF_S(str, _T("%ld,%ld,%ld,%ld"), &rcResult->left, &rcResult->top, &rcResult->right,
				&rcResult->bottom);
		return TRUE;
	}
}
...
BOOL CXTPRegistryManager::GetProfileSize(LPCTSTR lpszSection, LPCTSTR lpszEntry, CSize* szResult)
{
	...
	if (m_strINIFileName.IsEmpty())
	{
		...
		ASSERT(dwType == REG_BINARY);
		m_lResult = ::RegQueryValueEx(hSecKey, (LPTSTR)lpszEntry, NULL, &dwType, (LPBYTE)szResult,
									  &dwCount);
	...
	else
	{
		...
		SCANF_S(str, _T("%li,%li"), &szResult->cx, &szResult->cy);
		return TRUE;
	}
}
...
BOOL CXTPRegistryManager::GetProfileDouble(LPCTSTR lpszSection, LPCTSTR lpszEntry, double* dResult)
{
	...
	if (m_strINIFileName.IsEmpty())
	{
		...
		ASSERT(dwType == REG_BINARY);
		m_lResult = ::RegQueryValueEx(hSecKey, (LPTSTR)lpszEntry, NULL, &dwType, (LPBYTE)dResult,
									  &dwCount);
...
BOOL CXTPRegistryManager::WriteProfileDword(LPCTSTR lpszSection, LPCTSTR lpszEntry, DWORD* pValue)
{
	...
	if (m_strINIFileName.IsEmpty())
	{
		...
		m_lResult = ::RegSetValueEx(hSecKey, lpszEntry, NULL, REG_DWORD, (LPBYTE)pValue,
									sizeof(*pValue));
...
BOOL CXTPRegistryManager::GetProfileDword(LPCTSTR lpszSection, LPCTSTR lpszEntry, DWORD* dwResult)
{
	...
	if (m_strINIFileName.IsEmpty())
	{
		...
		ASSERT(dwType == REG_DWORD);
		m_lResult = ::RegQueryValueEx(hSecKey, (LPTSTR)lpszEntry, NULL, &dwType, (LPBYTE)dwResult,
									  &dwCount);
...
BOOL CXTPRegistryManager::WriteProfileColor(LPCTSTR lpszSection, LPCTSTR lpszEntry,
											COLORREF* pValue)
{
	return WriteProfileDword(lpszSection, lpszEntry, pValue);
}

BOOL CXTPRegistryManager::GetProfileColor(LPCTSTR lpszSection, LPCTSTR lpszEntry,
										  COLORREF* rgbResult)
{
