...
DWORD CXTPMemFile::Insert(CFile* fSrc, DWORD dwSourcePos, DWORD dwDestPos, DWORD dwBytes)
{
	...
	Extract(&fPostIns, dwDestPos,
			(DWORD)((DWORD)GetLength() - dwDestPos)); // Extract Data AFTER intertion point
	...
	// Extract the "to be inserted" Data from the (fSrc) Source file
	{
		...
		while (((fSrc->GetPosition() - dwSourcePos) < dwBytes)
			   && (fSrc->GetPosition() < fSrc->GetLength()))
		{
