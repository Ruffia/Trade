...
//===========================================================================
// Summary:
//     CXTPRegistryManager is a standalone registry management class. You
...
public:
	...
	// Parameters:
	//     hKeyBase - Current key to be used in the registry. By default,
	//                this parameter is HKEY_CURRENT_USER.
	//-----------------------------------------------------------------------
...
public:
	...
	// Parameters:
	//     lpszRegistryKey - Used to determine the full registry key for storing
...
public:
	...
	// Parameters:
	//     lpszSection - Pointer to a NULL-terminated string that specifies
	//                   the section containing the entry. If the section
	//                   does not exist, then it is created. The name of the
	//                   section is case independent. The string may be
	//                   any combination of uppercase and lowercase letters.
	//     lpszEntry   - Pointer to a NULL-terminated string that contains
	//                   the entry into which the value is to be written. If
	//                   the entry does not exist in the specified section,
	//                   then it is created.
	//     nValue      - Contains the value to be written.
	// Returns:
	//     Nonzero if successful, otherwise zero. If the user does not have
	//     access rights to modify the system registry, then this function
	//     will return -1.
	//-----------------------------------------------------------------------
	...
	//     pValue      - Contains the value to be written.
	// Remarks:
	//     Since COLORREF is a typedef for a DWORD, WriteProfileColor calls
	//     this function.
	// Returns:
	//     Nonzero if successful, otherwise zero. If the user does not have
	//     access rights to modify the system registry, then this function
	...
	//     .INI file.
	// Parameters:
	//     lpszSection - Pointer to a NULL-terminated string that specifies the
	//                   section containing the entry.
	//     lpszEntry   - Pointer to a NULL-terminated string that contains the
	...
	// Remarks:
	//     This member function supports hexadecimal notation for the value in
	//     the .INI file. When you retrieve a signed integer, you should cast
	//     the value into an int.
	// Returns:
	//     The integer value of the string that follows the specified entry if
	//     the function is successful, the value of the 'nDefault' parameter
	//     if the function does not find the entry, or zero if the value that
	//     corresponds to the specified entry is not an integer.
	//-----------------------------------------------------------------------
	...
	//     ppData      - Address of a LPBYTE data member to receive the value.
	//                   It is the caller's responsibility to free the memory
	//                   allocated for 'ppData'.
	// Example:
	...
	//     lpszEntry   - Pointer to a NULL-terminated string that contains the
	//                   entry whose value is to be retrieved.
	//     lpszDefault - Pointer to the default string value for the given entry
	...
	//     lpszEntry   - Pointer to a NULL-terminated string that contains the
	//                   entry whose value is to be retrieved.
	//     ptResult    - Pointer to the variable to be filled from the registry.
	// Returns:
	...
	//     lpszEntry   - Pointer to a NULL-terminated string that contains the
	//                   entry whose value is to be retrieved.
	//     rcResult    - Pointer to the variable to be filled from the registry.
	// Returns:
	//     TRUE if successful, otherwise FALSE.
	//-----------------------------------------------------------------------
	...
	//     * <b>REG_DWORD_LITTLE_ENDIAN</b>: A 32-bit number in little-endian format.
	...
	//                                       systems.
	//     * <b>REG_DWORD_BIG_ENDIAN</b>: A 32-bit number in big-endian format. In
	...
	//     * <b>REG_EXPAND_SZ</b>: A NULL-terminated string that contains unexpanded
	...
	//     * <b>REG_QWORD_LITTLE_ENDIAN</b>: A 64-bit number in little-endian format.
	//                                       This is equivalent to REG_QWORD.
	//     * <b>REG_RESOURCE_LIST</b>: A device-driver resource list.
	//     * <b>REG_SZ</b>: A NULL-terminated string. It will be either a Unicode or
	...
	//     lpszSection - Pointer to a NULL-terminated string that specifies the
	//                   section containing the entry.
	//     arrayKeys   - A reference to a CStringArray, which will contain the keys
	//                   in the registry under the above key.
	//-----------------------------------------------------------------------
	...
	//     lpszSection - Pointer to a NULL-terminated string that specifies the
	//                   section containing the entry.
	//     lpszKey     - Pointer to a NULL-terminated string that specifies the
	//                   subkey to delete.
	// Returns:
	//     true if successful, otherwise false.
	//-----------------------------------------------------------------------
	...
	// Returns:
	//     If the last operation was successful, then the return value is ERROR_SUCCESS.
	...
	// Returns:
	//    A CString object that contains the error message that describes the
	//    last error that occurred while accessing the system registry.
	//-----------------------------------------------------------------------
...
protected:
	...
	// Parameters:
	//     samDesired - An access mask that specifies the desired access rights
	//                  to the key.
	// Returns:
	//     An HKEY handle to the application's registry key.
	//-----------------------------------------------------------------------
	virtual HKEY GetAppRegistryKey(REGSAM samDesired = KEY_WRITE | KEY_READ);

	//-----------------------------------------------------------------------
	// Summary:
	//     Called to get a handle to the specified section from the application's
	//     registry key.
	// Parameters:
	...
	//                   to the key.
	// Returns:
	//     An HKEY handle to the specified section from the application's registry key.
	//-----------------------------------------------------------------------
