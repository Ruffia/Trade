...
int CXTPListBase::CopyRow(int nFrom, int nTo)
{
	...
	for (i = 1; i < GetColumnCount(); i++)
	{
		...
		if (!strLabel.IsEmpty())
		{
			m_pListCtrl->SetItemText(nTo, i, strLabel);
		}
	}
...
BOOL CXTPListBase::ModifyExtendedStyle(DWORD dwRemove, DWORD dwAdd)
{
	...
	::SendMessage(m_pListCtrl->m_hWnd, LVM_SETEXTENDEDLISTVIEWSTYLE, 0, XTPToLPARAM(dwNewStyle));
	return TRUE;
}
...
void CXTPListBase::OnCustomDraw(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	*pResult			   = CDRF_DODEFAULT;
	...
	switch (lpLVCD->nmcd.dwDrawStage)
	{
		...
		case CDDS_PREPAINT | CDDS_ITEM: *pResult = CDRF_NOTIFYSUBITEMDRAW; break;
		...
		case CDDS_PREPAINT | CDDS_ITEM | CDDS_SUBITEM:
		{
			...
			if (bUseRowColor
				&& m_pListCtrl->GetItemState((int)lpLVCD->nmcd.dwItemSpec, LVIS_SELECTED)
					   == LVIS_SELECTED)
			{
				bUseRowColor = FALSE;
			}
...
int CXTPListBase::AddColumn(LPCTSTR lpszColHeading, int nWidth /*= -1*/,
...
bool CXTPListBase::BuildColumns(int nCols, int* nWidth, int* nColString)
{
	...
	{
		CString strColumn;
		if (!strColumn.LoadString(XTPToUInt(nColString[i])))
		{
		}
...
int CXTPListBase::HitTestEx(CPoint& point, int* nCol) const
{
	...
	for (; row <= bottom; row++)
	{
		...
		if (rect.PtInRect(point))
		{
			...
			{
				int colwidth = m_pListCtrl->GetColumnWidth(
					Header_OrderToIndex(pHeaderCtrl->m_hWnd, colnum));
...
void CXTPListBase::SaveColumnWidths()
{
	...
	for (i = 0; i < pHeaderCtrl->GetItemCount(); ++i)
	{
		CString strTemp;
		strTemp.Format(_T("%d,"),
					   m_pListCtrl->GetColumnWidth(Header_OrderToIndex(pHeaderCtrl->m_hWnd, i)));
		strValue += strTemp;
	}
...
void CXTPListBase::LoadColumnWidths()
{
	...
	DWORD dwStyle = XTPToDWORD(::GetWindowLong(hWndList, GWL_STYLE));
...
void CXTPListBase::AutoSizeColumn(int nCol, BOOL bUseHeader)
{
	...
	{
		m_pListCtrl->SetColumnWidth(Header_OrderToIndex(pHeaderCtrl->m_hWnd, nCol),
...
void CXTPListBase::OnPaint()
{
	...
	bool bReportView = ((::GetWindowLong(m_pListCtrl->m_hWnd, GWL_STYLE) & LVS_TYPEMASK)
						== LVS_REPORT);
	...
	if (pHeaderCtrl && ::IsWindow(pHeaderCtrl->m_hWnd))
	{
		...
		{
			int iColWidth = m_pListCtrl->GetColumnWidth(
				Header_OrderToIndex(pHeaderCtrl->m_hWnd, iCol));
...
CHeaderCtrl* CXTPListBase::_xtGetHeaderCtrl() const
{
	...
	CHeaderCtrl* const pHeaderCtrl = (CHeaderCtrl*)m_pListCtrl->GetDlgItem(0);
...
DWORD CXTPListBase::GetExtendedStyle()
{
	ASSERT(::IsWindow(m_pListCtrl->GetSafeHwnd()));
	return (DWORD)m_pListCtrl->SendMessage(LVM_GETEXTENDEDLISTVIEWSTYLE, 0, 0);
}

void CXTPListBase::SetExtendedStyle(DWORD dwExStyle)
{
	ASSERT(::IsWindow(m_pListCtrl->GetSafeHwnd()));
	m_pListCtrl->SendMessage(LVM_SETEXTENDEDLISTVIEWSTYLE, 0,
							 XTPToLPARAM(GetExtendedStyle() | dwExStyle));
}

void CXTPListBase::SetMinimumColSize(int nSize)
{
	m_nMinColWidth = nSize;
	XTPGetHeaderCtrl()->SetMinSize(nSize);
}

void CXTPListBase::SetRowColors(COLORREF clrText, COLORREF clrTextBk)
{
...
COLORREF CXTPListBase::GetListBackColor() const
{
	return m_bListColor
...
COLORREF CXTPListBase::GetListTextColor() const
{
	return m_bListColor
...
COLORREF CXTPListBase::GetSortBackColor() const
{
	return (m_crSortBack == (COLORREF)-1) ? GetListBackColor() : m_crSortBack;
}

COLORREF CXTPListBase::GetSortTextColor() const
{
