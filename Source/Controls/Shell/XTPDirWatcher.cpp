...
{
	DWORD dw = ::WaitForMultipleObjects(2, &m_dwMonitorEvents[2], FALSE, INFINITE);
...
BOOL CXTPDirWatcher::MonitorNotifications()
{
	...
	m_dwMonitorEvents[0] =
		::FindFirstChangeNotification(m_strFolderPath,				 // directory to watch
...
#if !defined(_UNICODE)
		::MultiByteToWideChar(CP_ACP, MB_PRECOMPOSED, lpszFolderPath, -1, szOleChar, MAX_PATH);
#else
		STRCPY_S(szOleChar, MAX_PATH, lpszFolderPath);
#endif

		// Convert the path to an ITEMIDLIST.
		if (SUCCEEDED(pDesktopFolder->ParseDisplayName(NULL, NULL, szOleChar, &chEaten, &pidl,
													   &dwAttributes)))
		{
			...
			pDesktopFolder->BindToObject(lpTVID.lpifq, NULL, IID_IShellFolder,
										 (LPVOID*)&psfMyFolder);
...
{
	m_pMainWnd->SendMessage(WM_XTP_SHELL_NOTIFY, SHN_XTP_REFRESHFOLDER, (LPARAM)&m_tvid);
}

void CXTPDirWatcher::RefreshTree()
{
	m_pMainWnd->SendMessage(WM_XTP_SHELL_NOTIFY, SHN_XTP_REFRESHTREE, (LPARAM)&m_tvid);
}
