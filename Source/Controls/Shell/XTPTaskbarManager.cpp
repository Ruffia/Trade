...
const IID IID_IXTPTaskbarList3 = { 0xea1afb91,
								   0x9e28,
								   0x4b86,
								   { 0x90, 0xe9, 0x9e, 0x9f, 0x8a, 0x5e, 0xef, 0xaf } };

const CLSID CLSID_XTPDestinationList = { 0x77f10cf0,
										 0x3db5,
										 0x4966,
...
const IID IID_IXTPObjectArray = { 0x92CA9DCD,
								  0x5622,
								  0x4bba,
								  { 0xa8, 0x05, 0x5E, 0x9F, 0x54, 0x1B, 0xD8, 0xC9 } };

const IID IID_IXTPCustomDestinationList = { 0x6332debf,
											0x87b5,
											0x4670,
											{ 0x90, 0xc0, 0x5e, 0x57, 0xb4, 0x08, 0xa4, 0x9e } };

const IID IID_IXTPObjectCollection = { 0x5632b1a4,
									   0xe38a,
									   0x400a,
									   { 0x92, 0x8a, 0xd4, 0xcd, 0x63, 0x23, 0x02, 0x95 } };

const IID IID_IXTPShellItem = { 0x43826d1e,
								0xe718,
								0x42ee,
								{ 0xbc, 0x55, 0xa1, 0xe2, 0x61, 0xc3, 0x7b, 0xfe } };

const IID IID_IXTPPropertyStore = { 0x886d8eeb,
									0x8cf2,
									0x4446,
...
{
	HRESULT hr = CoCreateInstance(CLSID_XTPDestinationList, NULL, CLSCTX_INPROC_SERVER,
								  IID_IXTPCustomDestinationList, (void**)&m_pcdl);
	if (SUCCEEDED(hr))
	{
		// Added by CPN
		if (!m_sAppID.IsEmpty())
...
void CXTPTaskbarManagerJumpList::AddTask(LPCTSTR lpszPath, LPCTSTR lpszArgument, LPCTSTR lpszTitle,
										 LPCTSTR lpszIconPath, int nIconIndex /*= -1*/)
{
	CTask task;
...
HRESULT CXTPTaskbarManagerJumpList::AddUserTasks()
{
	...
	HRESULT hr = CoCreateInstance(CLSID_XTPEnumerableObjectCollection, NULL, CLSCTX_INPROC,
								  IID_IXTPObjectCollection, (void**)(&poc));
	if (SUCCEEDED(hr))
	{
		for (int i = 0; i < m_arrTask.GetSize(); i++)
		{
			...
			hr = CreateShellLink(XTP_CT2CW(task.m_strPath), XTP_CT2CW(task.m_strArguments),
								 XTP_CT2CW(task.m_strTitle), XTP_CT2CW(task.m_strIconPath),
								 task.m_nIconIndex, &psl);
...
HRESULT CXTPTaskbarManagerJumpList::CreateShellLink(LPCWSTR lpszApp, LPCWSTR lpszCommandLine,
													LPCWSTR lpszTitle, LPCWSTR lpszIconPath,
													int nIconIndex, IShellLinkW** ppSL)
{
	IShellLinkW* psl;
...
BOOL CXTPTaskbarManagerJumpList::IsItemInArray(IShellItem* pSI, IObjectArray* pArray)
{
	...
	if (SUCCEEDED(pArray->GetCount(&cItems)))
	{
		...
		for (UINT i = 0; !fRet && i < cItems; i++)
		{
			...
			{
				int iOrder;
				fRet = SUCCEEDED(psiCompare->Compare(pSI, SICHINT_CANONICAL, &iOrder))
					   && (0 == iOrder);
				psiCompare->Release();
			}
		}
...
CXTPTaskbarManager::CXTPTaskbarManager()
{
	...
	HRESULT hr = CoCreateInstance(CLSID_TaskbarList, NULL, CLSCTX_INPROC_SERVER,
								  IID_IXTPTaskbarList3, (void**)(&m_pTaskbarList3));
...
BOOL CXTPTaskbarManager::SetOverlayIcon(HWND hWndParent, HICON hIcon, LPCTSTR lpszDescription)
{
	...
	if (FAILED(m_pTaskbarList3->SetOverlayIcon(hWndParent, hIcon,
											   lpszDescription ? XTP_CT2CW(lpszDescription) : L"")))
		return FALSE;
...
{
	HICON hIcon = (HICON)LoadImage(AfxGetResourceHandle(), MAKEINTRESOURCE(nIDResource), IMAGE_ICON,
								   16, 16, LR_LOADMAP3DCOLORS);
...
void CXTPTaskbarManager::SetProgressBarPosition(HWND hWndParent, int nProgressPos)
{
	...
	m_pTaskbarList3->SetProgressValue(hWndParent,
									  XTPToUIntChecked(nProgressPos - m_nProgressBarRangeMin),
									  XTPToUIntChecked(m_nProgressBarRangeMax
													   - m_nProgressBarRangeMin));
}
...
BEGIN_DISPATCH_MAP(CXTPTaskbarManagerJumpList, CXTPCmdTarget)
	...
	DISP_FUNCTION_ID(CXTPTaskbarManagerJumpList, "AddTask", 1, AddTask, VT_EMPTY,
