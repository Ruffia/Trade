...
// ----------------------------------------------------------------------
// Summary:
//     The XTP_LVITEMDATA structure is used to maintain information for
...
// ----------------------------------------------------------------------
// Summary:
//     The XTP_TVITEMDATA structure is used to maintain information for
...
//===========================================================================
// Summary:
//     CXTPShellPidl is a standalone base class. This class is used by the
...
public:
	...
	public:
		...
		// Returns:
		//     If successful, then Alloc returns a pointer to the allocated memory
		//     block. If insufficient memory is available, then Alloc returns NULL.
		//-------------------------------------------------------------------
		...
		//-------------------------------------------------------------------
		// Summary:
		//     Retrieves a pointer to Shell's IMalloc interface for this object.
		// Returns:
		//     A pointer to Shell's IMalloc interface for this object.
		//-------------------------------------------------------------------
		operator LPMALLOC()
		{
			return m_lpMalloc;
	...
	protected:
		LPMALLOC m_lpMalloc; // Pointer to Shell's IMalloc interface for this object.
	};

	//=======================================================================
	// Summary:
	//     CXTPShellPidl::CShellSpecialFolder is a helper class that is used by
	//     CXTPShellPidl to initialize a pointer to an ITEMIDLIST structure of
	//     a special folder.
	//=======================================================================
	...
	public:
		...
		// Parameters:
		//     nFolder - A value that identifies the folder of interest. See
		...
		//-------------------------------------------------------------------
		// Summary:
		//     Retrieves a pointer to the interface for the desktop folder,
		//     which is the root of the Shell's namespace.
		// Returns:
		//     A pointer to the interface for the desktop folder.
		//-------------------------------------------------------------------
	...
	protected:
		LPSHELLFOLDER m_lpFolder; // Pointer to the interface for the desktop folder.
	};
	...
	// Returns:
	//     A pointer to an item ID list if successful, otherwise NULL.
	//-----------------------------------------------------------------------
	...
	// Parameters:
	//     lpMalloc - Pointer to the shell's IMalloc interface.
	...
	// Parameters:
	//     lpMalloc - Pointer to the shell's IMalloc interface.
	//     lpi      - Pointer to the item ID that is to be copied.
	// Returns:
	//     A pointer to an item ID list.
	//-----------------------------------------------------------------------
	...
	//     lpsf           - Pointer to the parent shell folder.
	//     lpi            - Pointer to the item ID that is relative to 'lpsf'.
	//     dwFlags        - Flags to determine which value to return. See SHGNO
	//                      for more details.
	...
	//-----------------------------------------------------------------------
	// Summary:
	//     This member function calculates the number of item IDs in an
	//     item ID list.
	// Parameters:
	...
	//-----------------------------------------------------------------------
	// Summary:
	//     This member function displays a pop-up context menu if given a parent
	...
	//     lppt       - Screen location of where to pop-up the menu.
	// Returns:
	//     TRUE if successful, otherwise FALSE.
	//-----------------------------------------------------------------------
	...
	//-----------------------------------------------------------------------
	// Summary:
	//     This member function gets the index for the current icon. The
	//     index is the index into the system image list.
	// Parameters:
	//     lpi    - Fully qualified item ID list for the current item.
	//     uFlags - Flags that specify what the image list retrieves. Can
	//              be a combination of the following values:<p/>
	...
	//     uFlags - Flags that specify what the image list retrieves. Can
	//              be a combination of the following values:<p/>
	//              * <b>SHGFI_SMALLICON</b>: Return the handle to the system image
	//                                        list that contains small icon images.
	//              * <b>SHGFI_LARGEICON</b>: Return the handle to the system image
	//                                        list that contains large icon images.
	// Returns:
	//     A handle to the system image list that contains the specified icon images.
	//-----------------------------------------------------------------------
	...
	//     lpsf   - Pointer to the parent shell folder.
	//     lpi    - Fully qualified item ID list for the current item.
	//     uFlags - Single ULONG value that contains the attributes that
	...
	//     pcmType   - A pointer to a valid int data type that represents the
	//                 version number of the context menu.
	// Returns:
	//     A HRESULT value.
	//-----------------------------------------------------------------------
	...
	//                   for the two items being compared.
	//     lparamSort -  Corresponds to the lParam member of TV_SORTCB.
	// Remarks:
	...
	//                   for the two items being compared.
	//     lparamSort -  Corresponds to the lParam member of LV_SORTCB.
	// Remarks:
	//     This member function is a callback function used by
	...
	// ---------------------------------------------------------------------------
	// Summary:
	//     Call this member function to override the shortcut link display for each item.
	// Parameters:
	//     bShowLinkIcons - TRUE to enable shortcut link display, FALSE to disable.
	// ---------------------------------------------------------------------------
	void ShowShellLinkIcons(BOOL bShowLinkIcons = TRUE);

	//-----------------------------------------------------------------------
	// Summary:
	//     Calculates the number of item IDs in the list.
	// Parameters:
	//     pidl - Pointer to an item ID list.
	// Returns:
	//     The number of item IDs in the list.
	...
	//-----------------------------------------------------------------------
	// Summary:
	//     This member function frees memory allocated for a PIDL.
	// Parameters:
	//     pidl - Pointer to an item ID list.
	//-----------------------------------------------------------------------
	...
	//-----------------------------------------------------------------------
	// Summary:
	//     This member function copies a single PIDL out of a list. Remember
	//     to Free() the old one if it is no longer needed.
	// Parameters:
	//     pidl - Pointer to an ITEMIDLIST.
	...
	//     pidl1        - Pointer to an item ID list.
	//     pidl2        - Pointer to an item ID list.
	//     pShellFolder - Pointer to the shell folder to use for the comparison
	//                    (default Desktop).
	// Returns:
	//     TRUE if successful, otherwise FALSE.
	//-----------------------------------------------------------------------
...
protected:
	//-----------------------------------------------------------------------
	// Summary:
	//     This method is called when an item of a context menu is executed.
	// Parameters:
	//     idCmd - Identifier of the item that was executed.
	//     cmi   - A CMINVOKECOMMANDINFO structure reference containing additional
	//             information about the item that was executed.
	//-----------------------------------------------------------------------
