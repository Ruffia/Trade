...
// Remarks:
//     Whenever an item is created, deleted or renamed, the thread will send a
//     notification to its owner in the form of a WM_XTP_SHELL_NOTIFY message.
//     The WPARAM value for the notification will be set to SHN_XTP_TREESELCHANGE
//     if the folder list contents require updating or SHN_XTP_CONTENTSCHANGED
//     if the tree contents require updating. The LPARAM value is set to the address
//     of an XTP_TVITEMDATA structure that contains information that the path
//     monitored.
// Example:
//     To use CXTPDirWatcher, add a CXTPDirWatcher* m_pDirThread member
...
// </code>
//
//     You can add a message handler to the window receiving messages and handle
//     your change event notifications. The thread will send two notifications.
//     The wParam value indicates what event has occurred. If wParam equals
//     HN_XT_REFRESHFOLDER, then this indicates that the folder list has changed
//     and needs to be updated. If wParam equals SHN_XTP_REFRESHTREE, then
//     this indicates the folder tree has changed and needs to be updated. The
//     lParam value represents a pointer to an XTP_TVITEMDATA structure for the
//     folder:
//
// <code>
...
// </code>
//
//     When you are finished using the CXTPDirWatcher thread, you will need to
//     free the memory associated with the object. To do so, you can use the
//     SAFE_DELETE macro like so:
//
// <code>
...
//     CXTPShellListView classes to handle refreshing the list when a file or
...
public:
	...
	// Parameters:
	//     pMainWnd       - Pointer to the window to receive change notifications.
	//     lpszFolderPath - Full path of the directory to monitor.
	//     bWatchSubtree  - Monitors the directory tree rooted at the specified
	//                      directory.
	// Returns:
	...
	//-----------------------------------------------------------------------
	// Summary:
	//     Call this member function to retrieve the full path to the currently
	//     monitored directory.
	// Returns:
	//     A CString object representing the full path to the currently monitored
	//     directory.
	//-----------------------------------------------------------------------
	...
	//     to an XTP_TVITEMDATA pointer.
	// Parameters:
	//     pMainWnd - Pointer to the window to receive update notifications.
	//     lpTVID   - Pointer to the tree view item data.
	// Returns:
	//     TRUE if successful, otherwise FALSE.
	//-----------------------------------------------------------------------
	...
	// Remarks:
	//     In the first version, this member function is called to return a pointer
	//     to the XTP_TVITEMDATA structure for the directory that is currently
	//     monitored. In the second version, this function constructs an
	//     XTP_TVITEMDATA structure from the specified path.
	// Returns:
	//     In the first version; a pointer to an XTP_TVITEMDATA structure.
	//     In the second version; TRUE if successful, otherwise FALSE.
	// ----------------------------------------------------------------------
...
protected:
	...
	// Returns:
	//     TRUE if succcessful, otherwise FALSE.
	//-----------------------------------------------------------------------
