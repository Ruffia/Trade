...
LPITEMIDLIST CXTPShellPidl::IDLFromPath(LPCTSTR strPath)
{
	...
	// Get the folders IDL
	psfDeskTop->ParseDisplayName(NULL, NULL, (LPOLESTR)XTP_CT2CW(strPath), NULL, &pidlPath, NULL);
...
LPITEMIDLIST CXTPShellPidl::CopyPidlItem(LPITEMIDLIST pidl, UINT nItem)
{
	...
	int nSize			  = XTPToIntChecked(pidl->mkid.cb + sizeof(pidl->mkid.cb));
	LPITEMIDLIST pidlCopy = CreatePidl(CShellMalloc(), XTPToUIntChecked(nSize));

	if (pidlCopy)
	{
		ZeroMemory(pidlCopy, XTPToSizeTChecked(nSize));
...
BOOL CXTPShellPidl::ComparePidls(LPCITEMIDLIST pidl1, LPCITEMIDLIST pidl2,
...
BOOL CXTPShellPidl::GetName(LPSHELLFOLDER lpsf, LPITEMIDLIST lpi, DWORD dwFlags,
							CString& strFriendlyName)
{
	...
	if (lpsf->GetDisplayNameOf(lpi, dwFlags, &str) == NOERROR)
	{
		switch (str.uType)
		{
			...
			case STRRET_CSTR: strFriendlyName = (LPSTR)str.cStr; break;
...
LPITEMIDLIST CXTPShellPidl::GetFullyQualPidl(LPSHELLFOLDER lpsf, LPITEMIDLIST lpi)
{
	...
	if (FAILED(lpsfDeskTop->ParseDisplayName(NULL, NULL, (LPOLESTR)XTP_CT2CW(strBuff), &ulEaten,
											 &lpifq, &ulAttribs)))
	{
		lpsfDeskTop->Release();
		return NULL;
...
BOOL CXTPShellPidl::ShowContextMenu(HWND hwnd, LPSHELLFOLDER lpsfParent, LPITEMIDLIST lpi,
									LPPOINT lppt)
{
...
BOOL CXTPShellPidl::ShowContextMenu(HWND hwnd, LPSHELLFOLDER lpsfParent, LPCITEMIDLIST* lpi,
									int nCount, LPPOINT lppt)
{
	LPCONTEXTMENU lpcm;
	...
	if (SUCCEEDED(hr))
	{
		...
		if (hMenu != NULL)
		{
			...
			if (SUCCEEDED(hr))
			{
				...
				if (cmType > 1)
				{
					m_pOldWndProc = (WNDPROC)SetWindowLongPtr(hwnd, GWLP_WNDPROC,
...
HIMAGELIST CXTPShellPidl::GetSystemImageList(UINT uFlags) const
{
	...
	for (TCHAR cDrive = _T('C'); cDrive <= _T('Z'); cDrive++)
	{
		...
		{
			return (HIMAGELIST)::SHGetFileInfo(CString(cDrive) + _T(":\\"), 0, &sfi,
											   sizeof(SHFILEINFO), SHGFI_SYSICONINDEX | uFlags);
		}
	}
...
HRESULT CXTPShellPidl::GetSHContextMenu(LPSHELLFOLDER psfFolder, LPCITEMIDLIST* localPidl,
										int nCount, void** ppCM, int* pcmType)
{
	*ppCM				= NULL;
	...
	{
		hr = psfFolder->GetUIObjectOf(NULL, XTPToUIntChecked(nCount), localPidl, IID_IContextMenu,
									  NULL, (void**)&pICv1);
	}
	...
	switch (msg)
	{
		...
		case WM_MENUSELECT:
		{
			...
			if (0 == (MF_POPUP & HIWORD(wp)) && uItem >= 1 && uItem <= 0x7fff)
			{
				...
				if (SUCCEEDED(m_pIContext2->GetCommandString(uItem - 1, GCS_HELPTEXT, NULL,
															 (LPSTR)szBuf, _countof(szBuf))))
...
int CXTPShellPidl::TreeViewCompareProc(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort)
{
	...
	HRESULT hr = lptvid1->lpsfParent->CompareIDs(MAKELPARAM(LOWORD(iIndex),
															HIWORD(SHCIDS_ALLFIELDS)),
...
int CXTPShellPidl::ListViewCompareProc(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort)
{
	...
	HRESULT hr = lplvid1->lpsfParent->CompareIDs(MAKELPARAM(LOWORD(iIndex),
															HIWORD(SHCIDS_ALLFIELDS)),
...
void CXTPShellPidl::MapShellFlagsToItemAttributes(CTreeCtrl* pTreeCtrl, HTREEITEM hItem,
												  DWORD dwAttributes)
{
	// Display with 'cut' feedback if the item is ghosted (about to be moved to a different location
	// or a hidden one)
	...
	else if (dwAttributes & SFGAO_SHARE)
	{
...
void CXTPShellPidl::MapShellFlagsToItemAttributes(CListCtrl* pListCtrl, int iItem,
												  DWORD dwAttributes)
{
	// Display with 'cut' feedback if the item is ghosted (about to be moved to a different location
	// or a hidden one)
