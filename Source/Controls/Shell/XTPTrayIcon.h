...
//     It is used to display a system tray icon with an animation.
//===========================================================================
...
private:
	//-----------------------------------------------------------------------
	// Summary:
	//     TRAYICONDATA is a standalone helper structure. It is used by CXTPTrayIcon
	...
	struct NOTIFYICONDATAEX
	{
		...
		GUID guidItem;
		HICON hBalloonIcon; // Supported in Windows XP as well, but displayed as a small icon.
							// Windows Vista and later display a bigger icon in a balloon.
	};
...
public:
	//-----------------------------------------------------------------------
	// Summary:
	//     Determines if shell32.dll has Major version 5 or above.
	// Remarks:
	//     shell32.dll Major version 5 and above provide extended functionality.
	// Returns:
	//     TRUE if shell32.dll has Major version 5 or above.
	//-----------------------------------------------------------------------
...
public:
	...
	// Parameters:
	//     lpszCaption       - A NULL-terminated string that contains the tooltip text
	//                         displayed when the cursor is over the tray icon.
	...
	//     uMenuID           - Pop-up menu resource identifier.
	//     uDefMenuItemID    - Command ID that represents the default item for the menu.
	//     bDefMenuItemByPos - true if the default is defined by its position,
	//                         false if it is defined by its command ID.
	// Returns:
	...
	//                     It is placed to the left of the title. If the 'lpszInfoTitle'
	//                     member is zero-length, then the icon is not shown and can
	//                     be any of the values listed in the Remarks section.
	//     uTimeout      - The timeout value in seconds for a balloon ToolTip. The system
	//                     enforces minimum and maximum timeout values. 'uTimeout' values
	//                     that are too large default to the maximum value, and values that
	//                     are too small default to the minimum value. The system minimum
	//                     and maximum timeout values are currently set at 10 seconds and
	...
	//     tray icons. To use, call SetAnimationIcons and then SetTimer.
	// Parameters:
	//     lpIDArray     - [in] An array of resource IDs that represent the icons to
	//                     display in the caption area.
	//     nIDCount      - [in] Zero-based size of the array passed. <i>lpIDArray</i>
	//                     and <i>lpStrTipArray</i> must contain the same number of
	//                     items and must match value specified by <i>nIDCount</i>.
	//     lpStrTipArray - [in] An array of tooltips that match the icons passed in as
	//                     'lpStrTipArray'. If NULL, the default tooltip will be used.
	//-----------------------------------------------------------------------
	...
	// Parameters:
	//     lpszTipText - A NULL-terminated string that represents the tooltip
	//                   text to display for the icon.
	...
	//     pop-up menu if the tray icon handles notification messages.
	// Parameters:
	//     uItem  - Command identifier or position of the default menu item.
	//     bByPos - true if 'uItem' is the menu item's index,
	//              false if it is the menu item's command ID.
	// Returns:
	//     true if successful, otherwise false.
	//-----------------------------------------------------------------------
	...
	//     for the tray icon.
	// Parameters:
	//     uItem  - Reference to a UINT that is to receive the default
	//              menu item ID.
	//     bByPos - Reference to a bool that is to receive the default
	//              menu item position flag.
	//-----------------------------------------------------------------------
	void GetDefaultMenuItem(UINT& uItem, bool& bByPos);

	//-----------------------------------------------------------------------
	// Summary:
	//     Call this member function to set the window that is to receive
	//     notification messages from the tray icon.
	// Parameters:
	//     pWndNotify - Pointer to a valid CWnd object.
	// Returns:
	//     true if successful, otherwise false.
	//-----------------------------------------------------------------------
	...
	//-----------------------------------------------------------------------
	// Summary:
	//     Call this member function to retrieve a handle to the icon displayed
	//     in the system tray.
	// Returns:
	...
	//-----------------------------------------------------------------------
	bool ShowIcon();
	BOOL ShowIcon(BOOL bShow); // <combine CXTPTrayIcon::ShowIcon>

	//-----------------------------------------------------------------------
	// Summary:
	//     Call this member function to minimize the specified window to the
	//     system tray. If window animation is supported, then the window
	//     will glide down to the system tray.
	// Parameters:
	...
	//     system tray. If window animation is supported, then the window
	//     will glide up from the system tray.
	// Parameters:
	...
	// Returns:
	//     TRUE if successful, otherwise FALSE. If dwMessage is set to NIM_SETVERSION,
	//     then this function returns TRUE if the version was successfully changed,
	...
	//-----------------------------------------------------------------------
	// Summary:
	//     Creates the tray icon's min/max window.
	// Remarks:
	//     Called to create the minimize window used during minimize/maximize
	//     operations.
	// Parameters:
	//     pWndApp - Pointer to the calling application's main window.
	// Returns:
	...
	//-------------------------------------------------------------------------
	// Summary:
	//     Clears the animated icon array.
	// Remarks:
	...
	// Parameters:
	//     strToolTip - A NULL-terminated string that contains the tooltip text
	//                  displayed when the cursor is over the tray icon.
	// Remarks:
...
protected:
	...
	UINT m_uDefMenuItemID;		 // Default pop-up menu item ID.
	bool m_bDefMenuItemByPos;	// Determines if the default menu item is a command or index.
	bool m_bHidden;				 // State of the icon. true to indicate that the icon is hidden.
	...
	HWND m_hWndNotify;			 // Handle to the window that receives command notifications.
	CWnd m_wndMinimize;			 // Hidden window used during minimize and restore functions.
	size_t m_iMaxTipSize;		 // Maximum size for a tooltip string.
