...
void CXTPShellListCtrl::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	if (nChar == VK_RETURN)
	{
		...
		{
			m_pTreeCtrl->SendMessage(WM_XTP_SHELL_NOTIFY, (WPARAM)SHN_XTP_SELECTCHILD,
									 (LPARAM)(XTP_LVITEMDATA*)GetItemData(iIndex));
		}
	}
...
bool CXTPShellListCtrlEx::BrowseToFolder(LPCTSTR lpszPath)
{
	...
	if (SUCCEEDED(::SHGetDesktopFolder(&pDesktopFolder)))
	{
		...
		hr = pDesktopFolder->ParseDisplayName(NULL, NULL, szOleChar, &chEaten, &pidl,
											  &dwAttributes);
...
void CXTPShellListCtrlEx::OnLButtonDblClk(UINT nFlags, CPoint point)
{
	...
	if (iItem >= 0)
	{
		...
		if (lplvid)
		{
			ULONG ulAttrs = GetAttributes(lplvid->lpsfParent, lplvid->lpi, SHCIDS_ALLFIELDS);
