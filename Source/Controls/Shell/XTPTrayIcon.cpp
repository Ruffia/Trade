...
bool CXTPTrayIcon::SetIcon(LPCTSTR lpszIconName)
{
	...
	m_hIcon = (HICON)::LoadImage(AfxGetResourceHandle(), lpszIconName, IMAGE_ICON, cx, cy,
								 LR_DEFAULTCOLOR);
...
bool CXTPTrayIcon::SetAnimationIcons(const UINT* lpIDArray, int nIDCount,
									 const CString* lpStrTipArray /*= NULL*/)
{
	...
	for (iIcon = 0; iIcon < nIDCount; ++iIcon)
	{
		...
		ti.hIcon = (HICON)::LoadImage(AfxGetResourceHandle(), MAKEINTRESOURCE(lpIDArray[iIcon]),
...
bool CXTPTrayIcon::SetDefaultMenuItem(UINT uItem, bool bByPos)
{
	...
	CMenu menu;
	if (!XTPResourceManager()->LoadMenu(&menu, m_niData.uID))
	{
		return false;
	}
...
void CXTPTrayIcon::OnTimer(UINT_PTR nIDEvent)
{
	...
	{
		POSITION pos = m_arTrayIcons.FindIndex(XTPToIntPtrChecked(m_nCounter));
		if (pos != NULL)
		{
...
bool CXTPTrayIcon::Create(LPCTSTR lpszCaption, CWnd* pParentWnd, UINT nIconID, UINT uMenuID /*= 0*/,
...
bool CXTPTrayIcon::ShowBalloonTip(LPCTSTR lpszInfo, LPCTSTR lpszInfoTitle /*= NULL*/,
...
LRESULT CXTPTrayIcon::OnTrayNotification(WPARAM wParam, LPARAM lParam)
{
	...
	switch (LOWORD(lParam))
	{
		...
		case NIN_SELECT:
			...
			// if a user selects a notify icon with the keyboard and activates it with the space bar
			// or ENTER key
		...
		case WM_RBUTTONUP:
		{
			...
			::SetMenuDefaultItem(pSubMenu->m_hMenu, m_uDefMenuItemID, m_bDefMenuItemByPos);
		...
		case WM_LBUTTONDBLCLK:
		{
			...
			if (m_bDefMenuItemByPos)
			{
				...
				uItem = pSubMenu->GetMenuItemID(XTPToInt(m_uDefMenuItemID));
...
bool CXTPTrayIcon::CreateMinimizeWnd(CWnd* pWndApp)
{
	...
	{
		if (!m_wndMinimize.CreateEx(0, AfxRegisterWndClass(0), _T(""), WS_POPUP, CW_USEDEFAULT,
...
void CXTPTrayIcon::MaximizeFromTray(CWnd* pWndApp)
{
	...
	pWndApp->SetWindowPos(0, 0, 0, 0, 0,
