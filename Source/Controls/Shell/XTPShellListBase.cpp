...
CString CXTPShellListBase::m_strDateFormat;
CString CXTPShellListBase::m_strTimeFormat;
	...
	: m_pDirThread(0)
{
	...
	m_ulSFGAOFlags = SFGAO_FOLDER | SFGAO_STREAM | SFGAO_FILESYSANCESTOR | SFGAO_DISPLAYATTRMASK
					 | SFGAO_REMOVABLE | SFGAO_COMPRESSED | SFGAO_ENCRYPTED;
...
BOOL CXTPShellListBase::PopulateListView(XTP_TVITEMDATA* lptvid, LPSHELLFOLDER lpsf)
{
	...
	{
		SortList((m_nSortedCol > -1) ? m_nSortedCol : 0, (m_nSortedCol > -1) ? m_bAscending : 1);
			...
			if (iIndex >= 0)
			{
				...
				if (handle != INVALID_HANDLE_VALUE)
				{
					if (!ISFOLDER(ulAttrs) || ISZIPFILE(ulAttrs))
					{
						...
						TCHAR szBuffer[16];
						CString strSize;
						strSize.Format(_T("%s KB"),
									   InsertCommas((fsize + 1024) / 1024, szBuffer, 15));
					...
					{
						CTime cTime;
						cTime = CTime(time.wYear, time.wMonth, time.wDay, time.wHour, time.wMinute,
									  time.wSecond);

						TCHAR szDate[0x100];
						GetDateFormat(LOCALE_USER_DEFAULT, 0, &time, m_strDateFormat, szDate,
									  _countof(szDate));

						TCHAR szTime[0x100];
						GetTimeFormat(LOCALE_USER_DEFAULT, 0, &time, m_strTimeFormat, szTime,
									  _countof(szTime));

						m_pListCtrl->SetItemText(iIndex, 3, CString(szDate) + _T(" ") + szTime);
					}
				...
				SHFILEINFO sfi;
				::SHGetFileInfo((TCHAR*)lpifqThisItem, 0, &sfi, sizeof(SHFILEINFO),
...
TCHAR* CXTPShellListBase::InsertCommas(LONGLONG value, TCHAR* szBufferOut, UINT nSize)
{
	...
	fmt.LeadingZero = XTPToUIntChecked(_ttoi(szBufferIn));
	fmt.Grouping	= 3;
	...
	fmt.NegativeOrder = XTPToUIntChecked(_ttoi(szBufferIn));
	strValue.Format(_T("%I64d"), value);
	::GetNumberFormat(LOCALE_USER_DEFAULT, 0, strValue, &fmt, szBufferOut, XTPToIntChecked(nSize));
...
void CXTPShellListBase::GetNormalAndSelectedIcons(LPITEMIDLIST lpifq, LPTV_ITEM lptvitem)
{
	...
	lptvitem->iSelectedImage = GetItemIcon(lpifq, SHGFI_PIDL | SHGFI_SYSICONINDEX | SHGFI_SMALLICON
													  | SHGFI_OPENICON);
}
...
void CXTPShellListBase::ShowShellContextMenu(CPoint point)
{
	...
	int nIndex = point == CPoint(-1, -1) ? m_pListCtrl->GetNextItem(-1, LVNI_FOCUSED)
										 : m_pListCtrl->HitTest(ptClient);
	...
	if (nIndex != -1)
	{
		...
		CXTPShellPidl::ShowContextMenu(m_pListCtrl->m_hWnd, lplvid->lpsfParent,
									   (LPCITEMIDLIST*)arrItems.GetData(), (int)arrItems.GetSize(),
									   &point);
	}
	...
	switch (nMessage)
	{
		...
		case SHN_XTP_TREESELCHANGE:
		{
			...
			if (lptvid->lpsfParent == NULL)
			{
				...
				{
					lpShellFolder->BindToObject(lptvid->lpifq, 0, IID_IShellFolder,
												(LPVOID*)&pFolder);
					lpShellFolder->Release();
				}
				ASSERT(pFolder != 0);
			...
			else
			{
				...
				if (SUCCEEDED(lptvid->lpsfParent->BindToObject(lptvid->lpi, 0, IID_IShellFolder,
															   (LPVOID*)&lpsf)))
				{
...
BOOL CXTPShellListBase::RefreshRowColors()
{
	...
	for (int i = 0; i < m_pListCtrl->GetItemCount(); ++i)
	{
		...
		if (lplvid)
		{
			if ((lplvid->ulAttribs & SFGAO_COMPRESSED) || (lplvid->ulAttribs & SFGAO_ENCRYPTED))
			{
				ROWCOLOR rowColor;
				...
				rowColor.lParam = XTPToLPARAM(lplvid->ulAttribs);
...
bool CXTPShellListBase::SortList(int nCol, bool bAscending)
{
	...
	m_pListCtrl->SortItems(ListViewCompareProc, XTPToULongPtr((nCol + 1) * (bAscending ? 1 : -1)));
...
void CXTPShellListBase::OnDragDrop(NM_LISTVIEW* /*pNMListView*/)
{
	...
	static CLIPFORMAT clpFormat = (CLIPFORMAT)::RegisterClipboardFormat(
		_T("{B0D76F7A-B5D9-436c-8F10-BA16AEE69D42}"));
	...
	switch (dwEffect)
	{
		case DROPEFFECT_MOVE:
		{
			...
			for (nSelItem = m_pListCtrl->GetNextItem(-1, LVNI_SELECTED); nSelItem != -1;
...
void CXTPShellListBase::InitLocaleFormats()
{
	if (m_strDateFormat.IsEmpty())
	{
		int cchData = ::GetLocaleInfo(LOCALE_USER_DEFAULT, LOCALE_SSHORTDATE, NULL, 0);
		if (0 < cchData)
		{
			LPTSTR lpLCData = new TCHAR[XTPToUInt(cchData)];
			::GetLocaleInfo(LOCALE_USER_DEFAULT, LOCALE_SSHORTDATE, lpLCData, cchData);
			m_strDateFormat = lpLCData;
			delete[] lpLCData;
		}
	}
	...
	{
		int cchData = ::GetLocaleInfo(LOCALE_USER_DEFAULT, LOCALE_STIMEFORMAT, NULL, 0);
		if (0 < cchData)
		{
			LPTSTR lpLCData = new TCHAR[XTPToUInt(cchData)];
			::GetLocaleInfo(LOCALE_USER_DEFAULT, LOCALE_STIMEFORMAT, lpLCData, cchData);
			m_strTimeFormat = lpLCData;
			delete[] lpLCData;
		}
	}
}
