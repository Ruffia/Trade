...
void CXTPShellSettings::RefreshSettings()
{
		...
		&& modShell32Dll.GetProcAddress((FARPROC*)&SHGetSettings, "SHGetSettings"))
	{
		SHGetSettings((SHELLFLAGSTATE*)this,
					  SSF_DESKTOPHTML | SSF_DONTPRETTYPATH | SSF_DOUBLECLICKINWEBVIEW
						  | SSF_HIDEICONS | SSF_MAPNETDRVBUTTON | SSF_NOCONFIRMRECYCLE
						  | SSF_SHOWALLOBJECTS | SSF_SHOWATTRIBCOL | SSF_SHOWCOMPCOLOR
						  | SSF_SHOWEXTENSIONS | SSF_SHOWINFOTIP | SSF_SHOWSYSFILES
						  | SSF_WIN95CLASSIC);
	}
	...
	if (::RegOpenKeyEx(HKEY_CURRENT_USER, COLORKEY, 0, KEY_QUERY_VALUE, &hKey) == ERROR_SUCCESS)
	{
		...
		if (::RegQueryValueEx(hKey, _T("AltColor"), NULL, &dwType, (LPBYTE)&crCompColor, &uBytes)
			== ERROR_SUCCESS)
		{
		...
		if (::RegQueryValueEx(hKey, _T("AltEncryptionColor"), NULL, &dwType, (LPBYTE)&crEncrColor,
							  &uBytes)
			== ERROR_SUCCESS)
		{
