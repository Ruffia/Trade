	...
	: m_bTunneling(false)
{
	...
	m_ulSFGAOFlags = SFGAO_HASSUBFOLDER | SFGAO_FOLDER | SFGAO_DISPLAYATTRMASK | SFGAO_REMOVABLE
					 | SFGAO_COMPRESSED | SFGAO_ENCRYPTED;
...
void CXTPShellTreeBase::OnFolderExpanding(NM_TREEVIEW* pNMTreeView)
{
	if (!(pNMTreeView->itemNew.state & TVIS_EXPANDEDONCE))
	{
		...
		if (lptvid != NULL && lptvid->lpsfParent != NULL)
		{
			...
			m_pTreeCtrl->SetItemState(pNMTreeView->itemNew.hItem, TVIS_EXPANDEDONCE,
									  TVIS_EXPANDEDONCE);
		}
	}
}
...
HTREEITEM CXTPShellTreeBase::GetContextMenu()
{
	...
	if (tvhti.flags & (TVHT_ONITEMLABEL | TVHT_ONITEMICON))
	{
		...
		if (lptvid->lpsfParent == NULL)
		{
			...
			ShowContextMenu(m_pTreeCtrl->m_hWnd, lpShellFolder, lptvid->lpi, &point);
		...
		{
			ShowContextMenu(m_pTreeCtrl->m_hWnd, lptvid->lpsfParent, lptvid->lpi, &point);
		}
...
BOOL CXTPShellTreeBase::OnFolderSelected(NM_TREEVIEW* pNMTreeView, CString& strFolderPath)
{
	...
	if (lptvid && lptvid->lpi && lptvid->lpsfParent)
	{
		LPSHELLFOLDER lpsf = NULL;
		if (SUCCEEDED(
				lptvid->lpsfParent->BindToObject(lptvid->lpi, 0, IID_IShellFolder, (LPVOID*)&lpsf)))
		{
...
BOOL CXTPShellTreeBase::FindTreeItem(HTREEITEM hItem, XTP_LVITEMDATA* lplvid, BOOL bRecursively)
{
	...
	while (hItem)
	{
		...
		if (lptvid)
		{
			if (SCODE_CODE(GetScode(lplvid->lpsfParent->CompareIDs(0, lplvid->lpi, lptvid->lpi)))
				== 0)
			{
...
void CXTPShellTreeBase::OnSelchanged(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	if (pNMTreeView)
	{
		...
		if (XTPSystemVersion()->IsWinXPOrGreater())
		{
			...
			{
				HTREEITEM hItem = m_pTreeCtrl->GetChildItem(m_pTreeCtrl->GetParentItem(tvi.hItem));
...
BOOL CXTPShellTreeBase::InitializeTree(
...
BOOL CXTPShellTreeBase::TunnelTree(CString strFindPath)
{
	...
	if (pidlPath != NULL)
	{
		...
		int nItems = XTPToIntChecked(GetPidlCount(pidlPath));
		...
		{
			pPartPidl = CopyPidlItem(pidlPath, XTPToUIntChecked(i));
	...
	{
		// Must not be a special folder (stand alone) or a path that ParseDisplayName() doesn't
		// recognize so look through tree directly to see if we can find it that way. Start at
		// Desktop ...
		HTREEITEM hItemRoot = m_pTreeCtrl->GetRootItem();
		TCHAR* pszNext		= NULL;
		TCHAR szBuff[MAX_PATH];
		...
		if (pszNext != NULL)
		{
			...
			while (pszNext && hItemRoot)
			{
				...
				if (strItemText.CompareNoCase(pszNext) == 0)
				{
					...
					if (!(uState & TVIS_EXPANDEDONCE))
					{
						InitTreeNode(hItemRoot,
									 (XTP_TVITEMDATA*)m_pTreeCtrl->GetItemData(hItemRoot));
						m_pTreeCtrl->SetItemState(hItemRoot, TVIS_EXPANDEDONCE, TVIS_EXPANDEDONCE);
					}
					pszNext = STRTOK_S(NULL, _T("\\/"), &lpszContext);
					if (pszNext)
...
BOOL CXTPShellTreeBase::GetFolderItemPath(HTREEITEM hItem, CString& strFolderPath)
{
	...
	if (lptvid && lptvid->lpi)
	{
		if (lptvid->lpsfParent)
		{
			...
			lptvid->lpsfParent->GetAttributesOf(1, (const struct _ITEMIDLIST**)&lptvid->lpi,
												&ulAttrs);
		...
		else
		{
			...
			::SHGetFileInfo((TCHAR*)lptvid->lpi, 0, &sfi, sizeof(SHFILEINFO),
...
void CXTPShellTreeBase::PopulateTree(LPCTSTR lpszPath)
{
	...
	do
	{
		...
		{
			strPath = strFolder.Left(strFolder.GetLength() - strNextFolder.GetLength());
		}
		else
		{
			strPath = strFolder;
		...
		// Get ShellFolder Pidl
		ULONG eaten;
		if (FAILED(lpSF->ParseDisplayName(NULL, NULL, (LPOLESTR)XTP_CT2CW(strPath), &eaten, &lpIDL,
										  NULL)))
		{
			break;
		}
...
void CXTPShellTreeBase::BeginDrag(NM_TREEVIEW* pNMTreeView)
{
	...
	if (lplvid && lplvid->lpsfParent)
	{
		...
		HRESULT hResult = lplvid->lpsfParent->GetUIObjectOf(AfxGetMainWnd()->m_hWnd, 1,
															(const struct _ITEMIDLIST**)&lplvid->lpi,
															IID_IDataObject, 0, (LPVOID*)&lpdo);
		...
		if (SUCCEEDED(hResult))
		{
			...
			DWORD dwEffect;
			::DoDragDrop(lpdo, lpds, DROPEFFECT_COPY | DROPEFFECT_MOVE | DROPEFFECT_LINK,
						 &dwEffect);
