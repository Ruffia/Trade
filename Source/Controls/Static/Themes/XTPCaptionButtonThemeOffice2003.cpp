// XTPCaptionButtonThemeOffice2003.cpp: implementation of the CXTPCaptionButtonThemeOffice2003
// class.
//
	...
	if (::IsWindow(pCaption->GetSafeHwnd()))
	{
		...
		if (bSelected || bHilite)
		{
			XTPDrawHelpers()->GradientFill(pDC, rc,
										   bSelected ? XTPColorManager()->grcLunaPushed
													 : XTPColorManager()->grcLunaSelected,
										   FALSE);
		...
		else
		{
			...
			XTPDrawHelpers()->GradientFill(pDC, rcCaption,
										   pCaption->IsAppCaption() ? XTPColorManager()->grcCaption
																	: XTPColorManager()->grcToolBar,
										   FALSE, rc);
		}
