...
void CXTPHyperLink::ReportError(int iErrorNo)
{
	...
	switch (iErrorNo)
	{
		...
		case SE_ERR_ACCESSDENIED:
			XTPResourceManager()->LoadString(&strMessage, XTP_IDS_SE_ERR_ACCESSDENIED);
			break;
		case SE_ERR_OOM: XTPResourceManager()->LoadString(&strMessage, XTP_IDS_SE_ERR_OOM); break;
		case ERROR_BAD_FORMAT:
			XTPResourceManager()->LoadString(&strMessage, XTP_IDS_SE_ERR_BAD_FORMAT);
			break;
		case SE_ERR_SHARE:
			XTPResourceManager()->LoadString(&strMessage, XTP_IDS_SE_ERR_SHARE);
			break;
		case SE_ERR_ASSOCINCOMPLETE:
			XTPResourceManager()->LoadString(&strMessage, XTP_IDS_SE_ERR_ASSOCINCOMPLETE);
			break;
		case SE_ERR_DDETIMEOUT:
			XTPResourceManager()->LoadString(&strMessage, XTP_IDS_SE_ERR_DDETIMEOUT);
			break;
		case SE_ERR_DDEFAIL:
			XTPResourceManager()->LoadString(&strMessage, XTP_IDS_SE_ERR_DDEFAIL);
			break;
		case SE_ERR_DDEBUSY:
			XTPResourceManager()->LoadString(&strMessage, XTP_IDS_SE_ERR_DDEBUSY);
			break;
		case SE_ERR_NOASSOC:
			XTPResourceManager()->LoadString(&strMessage, XTP_IDS_SE_ERR_NOASSOC);
			break;
		case SE_ERR_DLLNOTFOUND:
			XTPResourceManager()->LoadString(&strMessage, XTP_IDS_SE_ERR_DLLNOTFOUND);
			break;
		default:
		{
			CString str;
...
bool CXTPHyperLink::GotoURL(LPCTSTR lpszLink /*=NULL*/, int nShowCmd /*=SW_SHOW*/,
							LPCTSTR lpszParams /*=NULL*/, LPCTSTR lpszDir /*=NULL*/)
{
	if (!lpszLink)
		lpszLink = m_strLink;

	// open the URL
	int iRetVal = (int)(INT_PTR)::ShellExecute(NULL, _T("open"), lpszLink, lpszParams, lpszDir,
											   nShowCmd);
...
HBRUSH CXTPHyperLink::CtlColor(CDC* pDC, UINT nCtlColor)
{
	...
	if (IsTextControl())
	{
		...
		{
			hbr = (HBRUSH)::DefWindowProc(::GetParent(m_hWnd), WM_CTLCOLORSTATIC,
...
void CXTPHyperLink::OnPaint()
{
	...
	if (((::GetWindowLong(m_hWnd, GWL_EXSTYLE) & WS_EX_TRANSPARENT) != 0)
		|| (m_clrBack == COLORREF_NULL))
	{
		Default();
	}
...
bool CXTPHyperLink::IsLinkHot()
{
	...
	{
		case SS_LEFT: { rHot.right = rHot.left + size.cx;
		}
		break;
		...
		case SS_RIGHT: { rHot.left = rHot.right - size.cx;
		}
		break;
	}
