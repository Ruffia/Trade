...
{
	return XTPDrawHelpers()->RegisterWndClass(0, XTCAPTIONPOPUP_CLASSNAME, CS_HREDRAW | CS_VREDRAW,
...
BOOL CXTPCaptionPopupWnd::Create(const CRect& rect, CWnd* pParentWnd, CWnd* pChildWnd)
{
	...
	CXTPResourceManager::AssertValid(
		XTPResourceManager()->LoadString(&string, XTP_IDS_CLICKKEEPOPEN));
...
void CXTPCaptionPopupWnd::OnPaint()
{
	...
	dc.Draw3dRect(rc, GetXtremeColor(COLOR_3DSHADOW), GetXtremeColor(COLOR_3DHILIGHT));
}
