...
void CXTPSearchOptionsCtrl::Expand()
{
	...
	if (pOwner != NULL)
	{
		pOwner->SendMessage(WM_COMMAND, MAKEWPARAM(GetDlgCtrlID(), SON_XTP_ITEMEXPANDING),
							(LPARAM)m_hWnd);
	}
	...
	if (pOwner != NULL)
	{
		pOwner->SendMessage(WM_COMMAND, MAKEWPARAM(GetDlgCtrlID(), SON_XTP_ITEMEXPAND),
							(LPARAM)m_hWnd);
	}
}
...
void CXTPSearchOptionsCtrl::Contract()
{
	...
	if (pOwner != NULL)
	{
		pOwner->SendMessage(WM_COMMAND, MAKEWPARAM(GetDlgCtrlID(), SON_XTP_ITEMCONTRACTING),
							(LPARAM)m_hWnd);
	}
	...
	if (pOwner != NULL)
	{
		pOwner->SendMessage(WM_COMMAND, MAKEWPARAM(GetDlgCtrlID(), SON_XTP_ITEMCONTRACT),
							(LPARAM)m_hWnd);
	}
}
...
void CXTPSearchOptionsCtrl::OnPaint()
{
	...
	HBRUSH hBrush = (HBRUSH)GetParent()->SendMessage(WM_CTLCOLORSTATIC, (WPARAM)memDC.GetSafeHdc(),
													 (LPARAM)GetSafeHwnd());
	if (hBrush)
	{
	...
	memDC.DrawText(m_bExpanded ? m_strExpandLabel : m_strContractLabel, &m_rcLabel,
...
CXTPSearchOptionsView::CResizeWnd::CResizeWnd(CWnd* pWndParent, HWND hWndChild,
											  const XTP_RESIZEPOINT& ptTopLeft,
...
bool CXTPSearchOptionsView::CResizeWnd::IsGroupBox()
{
	...
	if (0 != ::GetClassName(m_hWndChild, szClassName, _countof(szClassName)))
	{
		...
		{
			DWORD dwStyle = XTPToDWORD(::GetWindowLong(m_hWndChild, GWL_STYLE));
...
bool CXTPSearchOptionsView::CResizeWnd::Resize(HDWP& hDWP, float dx)
{
	...
	if (dx > 0)
	{
		...
		if (rrcItem != rrcWindow)
		{
			...
			::DeferWindowPos(hDWP, m_hWndChild, 0, x, y, cx, cy, uFlags);
...
void CXTPSearchOptionsView::SetResize(int nID, const XTP_RESIZEPOINT& ptTopLeft,
									  const XTP_RESIZEPOINT& ptTopRight)
{
	...
	{
		pResizeWnd->m_rcSizing += CXTPResizeRect(ptTopLeft.x, ptTopLeft.y, ptTopRight.x,
												 ptTopRight.y);
	}
}
