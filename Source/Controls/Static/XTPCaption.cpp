	...
	: m_pTheme(NULL)
{
	...
	m_nOffset	  = 0;
	m_nBorder	  = 0;
	m_sizeIcon	 = XTPSystemMetrics()->GetSmallIconSize();
	m_dwExStyle	= 0L;
...
BOOL CXTPCaptionButton::SetTheme(XTPControlTheme eTheme)
{
	...
	switch (eTheme)
	{
		case xtpControlThemeOfficeXP: m_pTheme = new CXTPCaptionButtonThemeOfficeXP(); break;
		case xtpControlThemeOffice2003: m_pTheme = new CXTPCaptionButtonThemeOffice2003(); break;
		default: m_pTheme = new CXTPCaptionButtonTheme();
	}
...
BOOL CXTPCaption::SetTheme(XTPControlTheme eTheme)
{
	switch (eTheme)
	{
		case xtpControlThemeOfficeXP: return SetTheme(new CXTPCaptionThemeOfficeXP());

		case xtpControlThemeOffice2003: return SetTheme(new CXTPCaptionThemeOffice2003());
	}
...
BOOL CXTPCaption::Create(CWnd* pParentWnd, LPCTSTR lpszWindowName, DWORD dwExStyle, DWORD dwStyle,
						 const CRect& rect, UINT nID)
{
	...
	if (HasCloseButton())
	{
		...
		CString xaml;
		int id = XTPColorManager()->GetCurrentSystemTheme() == xtpSystemThemeAero
					 ? IDR_XAML_ICON_CAPTION_CLOSE_2
					 : IDR_XAML_ICON_CAPTION_CLOSE_1;
		VERIFY(XTPResourceManager()->LoadXAML(xaml, XTPToUInt(id)));
		m_btnCaption.EnableMarkup();
		m_btnCaption.SetWindowText(xaml);
	}
	else
	{
		// Create the button to be used with child window.
		if (!m_btnCaption.Create(m_strCaption, BS_ICON | BS_VCENTER, CRect(0, 0, 0, 0), this,
								 XTP_IDC_BTN_CLOSE))
		{
			TRACE0("Unable to create caption button.\n");
			return FALSE;
	...
	SetWindowPos(NULL, 0, 0, 0, 0, SWP_NOSIZE | SWP_NOMOVE | SWP_FRAMECHANGED);

	return TRUE;
}

void CXTPCaption::ModifyCaptionStyle(int nBorderSize, CFont* pFont, LPCTSTR lpszWindText,
									 HICON hIcon)
{
...
void CXTPCaption::OnCaptButton()
{
	if (HasCloseButton())
	{
		...
		if (pOwner)
		{
			...
			pOwner->SendMessage(WM_COMMAND, MAKEWPARAM(GetDlgCtrlID(), BN_CLICKED), (LPARAM)m_hWnd);
		}
	}
	else
	{
...
CRect CXTPCaption::GetButtonRect() const
{
	...
	CRect rcButton;
	rcButton.top	= (rcClient.Height() - m_sizeIcon.cy) / 2;
	rcButton.bottom = rcButton.top + m_sizeIcon.cy;

	// ensure offset is the same for top and right edge.
	int x = rcButton.top - rcClient.top;

	rcButton.left  = (rcClient.Width() - m_sizeIcon.cx) - x;
	rcButton.right = rcButton.left + m_sizeIcon.cx;
