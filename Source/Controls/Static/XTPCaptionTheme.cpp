...
void CXTPCaptionTheme::DrawCaptionBack(CDC* pDC, CXTPCaption* pCaption, CRect& rcItem)
{
	...
	else if (pCaption->m_dwExStyle & CPWS_EX_RAISED_EDGE)
	{
		pDC->Draw3dRect(rc, m_clrEdgeLight, m_clrEdgeShadow);
...
void CXTPCaptionTheme::DrawCaptionText(CDC* pDC, CXTPCaption* pCaption)
{
	...
	CXTPClientRect rc(pCaption);
	if (pCaption->HasCloseButton())
		rc.right -= pCaption->GetButtonRect().Width();

	switch (pCaption->GetStyle() & (SS_LEFT | SS_CENTER | SS_RIGHT))
	{
		case SS_LEFT: rcText.OffsetRect(0, 0); break;

		case SS_CENTER: rcText.OffsetRect((rc.Width() - rcText.Width()) / 2, 0); break;

		case SS_RIGHT: rcText.OffsetRect(rc.Width() - rcText.Width(), 0); break;
	}
...
void CXTPCaptionTheme::DrawCaptionIcon(CDC* pDC, CXTPCaption* pCaption, CRect& rcItem)
{
	...
	if (rc.right < rcItem.right - pCaption->m_nBorder)
	{
		...
		::DrawIconEx(pDC->GetSafeHdc(), rc.left, rc.top, pCaption->m_hIcon, rc.Width(), rc.Height(),
