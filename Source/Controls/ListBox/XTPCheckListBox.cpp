...
CXTPCheckListBox::CCheckListState::CCheckListState(BOOL bListBox3D)
	: m_sizeCheck(XTP_DPI_X(12), XTP_DPI_Y(12))
	, idUnchecked(bListBox3D ? IDR_XAML_ICON_CHECKBOX3D_UNCHECKED
							 : IDR_XAML_ICON_CHECKBOX_UNCHECKED)
	, idChecked(bListBox3D ? IDR_XAML_ICON_CHECKBOX3D_CHECKED : IDR_XAML_ICON_CHECKBOX_CHECKED)
	, idMixed(bListBox3D ? IDR_XAML_ICON_CHECKBOX3D_MIXED : IDR_XAML_ICON_CHECKBOX_MIXED)
	, m_bIconsSet(FALSE)
{
...
void CXTPCheckListBox::CCheckListState::Draw(CDC* pDC, int nCheck, CPoint pt, CSize sz)
{
	if (!m_bIconsSet)
	{
		XTPImageManager()->SetVectorIcon(_T("RT_XAML"), XTPToUIntChecked(idChecked),
										 XTPToUIntChecked(idChecked),
										 XTPToUIntChecked(m_sizeCheck.cx));
		XTPImageManager()->SetVectorIcon(_T("RT_XAML"), XTPToUIntChecked(idUnchecked),
										 XTPToUIntChecked(idUnchecked),
										 XTPToUIntChecked(m_sizeCheck.cx));
		XTPImageManager()->SetVectorIcon(_T("RT_XAML"), XTPToUIntChecked(idMixed),
										 XTPToUIntChecked(idMixed),
										 XTPToUIntChecked(m_sizeCheck.cx));
	...
	{
		case 0:
			pIcon = XTPImageManager()->GetImage(XTPToUIntChecked(idUnchecked), m_sizeCheck.cx);
			break;
		case 1:
			pIcon = XTPImageManager()->GetImage(XTPToUIntChecked(idChecked), m_sizeCheck.cx);
			break;
		case 2:
			pIcon = XTPImageManager()->GetImage(XTPToUIntChecked(idMixed), m_sizeCheck.cx);
			break;
...
void CXTPCheckListBox::SetCheck(int nIndex, int nCheck)
{
	...
	LRESULT lResult = DefWindowProc(LB_GETITEMDATA, XTPToWPARAM(nIndex), 0);
	if (lResult != LB_ERR)
	{
		...
		VERIFY(DefWindowProc(LB_SETITEMDATA, XTPToWPARAM(nIndex), XTPToLPARAM(pState)) != LB_ERR);
...
void CXTPCheckListBox::DrawItem(LPDRAWITEMSTRUCT lpDIS)
{
	...
	if (((LONG)(lpDIS->itemID) >= 0) && (lpDIS->itemAction & (ODA_DRAWENTIRE | ODA_SELECT)))
	{
		...
		pDC->ExtTextOut(rcItem.left + 2, rcItem.top, ETO_OPAQUE, &rcItem, strText,
...
{
	lpMIS->itemHeight = XTPToUIntChecked(m_checkListState.m_sizeCheck.cy
...
bool CXTPCheckListBox::PreDrawItemThemed(CDC* pDC, DRAWITEMSTRUCT& drawItem, int nCheck, int cyItem)
{
	...
	{
		BOOL bEnabled = IsWindowEnabled() && IsEnabled(XTPToInt(drawItem.itemID));
		...
		if (SUCCEEDED(m_themeHelper->GetThemePartSize(pDC->m_hDC, BP_CHECKBOX, nState, NULL,
													  TS_TRUE, &size)))
		{
			...
			if (SUCCEEDED(m_themeHelper->DrawThemeBackground(pDC->m_hDC, BP_CHECKBOX, nState,
															 &rectCheckBox, NULL)))
			{
...
void CXTPCheckListBox::PreDrawItemNonThemed(CDC* pDC, DRAWITEMSTRUCT& drawItem, int nCheck,
											int cyItem)
{
	CDC bitmapDC;
	...
	{
		// redraw background.
		pDC->FillSolidRect(&drawItem.rcItem, ::GetSysColor(COLOR_WINDOW));
		...
		m_checkListState.Draw(pDC, nCheck, rectCheckBox.TopLeft(), rectCheckBox.Size());
...
void CXTPCheckListBox::PreDrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	...
	{
		int cyItem = GetItemHeight(XTPToInt(drawItem.itemID));

		CDC* pDC = CDC::FromHandle(drawItem.hDC);

		COLORREF newBkColor = GetSysColor(COLOR_WINDOW);

		BOOL fDisabled = !IsWindowEnabled() || !IsEnabled(XTPToInt(drawItem.itemID));
		if ((drawItem.itemState & ODS_SELECTED) && !fDisabled)
			newBkColor = GetSysColor(COLOR_HIGHLIGHT);

		COLORREF oldBkColor = pDC->SetBkColor(newBkColor);

		int nCheck = GetCheck(XTPToInt(drawItem.itemID));
	...
	drawItem.rcItem.left = drawItem.rcItem.left
						   + (m_bLastItemDrawnThemed ? m_checkListState.m_sizeCheckThemed.cx
													 : m_checkListState.m_sizeCheck.cx)
						   + 3;
...
void CXTPCheckListBox::PreDrawItem(LPDRAWITEMSTRUCT lpDIS)
{
		...
		((drawItem.itemAction & (ODA_DRAWENTIRE | ODA_SELECT)) != 0))
	{
		...
		if (bitmapDC.CreateCompatibleDC(pDC))
		{
			...
			HBITMAP hOldBitmap = (HBITMAP)::SelectObject(bitmapDC.m_hDC,
m_checkListState.m_hbitmapCheck);
...
void CXTPCheckListBox::PreMeasureItem(LPMEASUREITEMSTRUCT lpMeasureItemStruct)
{
	...
	measureItem.itemHeight = XTPToUIntChecked(cyItem);
...
BOOL CXTPCheckListBox::OnChildNotify(UINT message, WPARAM wParam, LPARAM lParam, LRESULT* pResult)
{
	switch (message)
	{
		...
		case WM_DELETEITEM:
			...
			break;
		default: return CXTPListBox::OnChildNotify(message, wParam, lParam, pResult);
	}
	return TRUE;
}
		...
		if (itemRect.PtInRect(point) && IsEnabled(nIndex))
		{
			if (m_nStyle != BS_CHECKBOX && m_nStyle != BS_3STATE)
			{
				if (point.x - itemRect.left < m_checkListState.m_sizeCheck.cx + 2)
				{
					...
					// Inform of check
					pParent->SendMessage(WM_COMMAND, MAKEWPARAM(GetDlgCtrlID(), CLBN_CHKCHANGE),
										 (LPARAM)m_hWnd);
					return;
				}
			}
...
int CXTPCheckListBox::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	{
		SetItemHeight(0, XTPToUIntChecked(CalcMinimumItemHeight()));
	}
...
LRESULT CXTPCheckListBox::OnLBAddString(WPARAM wParam, LPARAM lParam)
{
	...
	if (!(GetStyle() & LBS_HASSTRINGS))
	{
		...
		pState->m_dwUserData = XTPToUIntPtr(lParam);
		lParam				 = XTPToLPARAM(pState);
	}
...
LRESULT CXTPCheckListBox::OnLBGetItemData(WPARAM wParam, LPARAM lParam)
{
	...
	if (lResult != LB_ERR)
	{
		...
		lResult = XTPToLRESULT(pState->m_dwUserData);
	}
	return lResult;
}
...
LRESULT CXTPCheckListBox::OnLBGetText(WPARAM wParam, LPARAM lParam)
{
	...
	if (lResult != LB_ERR)
	{
		...
		if (pState != NULL)
			lParam = XTPToLPARAM(pState->m_dwUserData);
	}
	return lResult;
}
...
{
	LRESULT lResult = DefWindowProc(LB_GETITEMDATA, XTPToWPARAM(wParam), 0);
	...
	if (lResult != LB_ERR)
	{
		...
		pState->m_dwUserData = XTPToUIntPtr(lParam);
		lResult				 = DefWindowProc(LB_SETITEMDATA, wParam, XTPToLPARAM(pState));
