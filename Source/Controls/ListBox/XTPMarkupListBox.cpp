...
{
	XTP_MARKUP_LISTITEM* pListItem = reinterpret_cast<XTP_MARKUP_LISTITEM*>(
...
void CXTPMarkupListBox::MeasureItem(LPMEASUREITEMSTRUCT lpMeasureItemStruct)
{
	...
	lpMeasureItemStruct->itemHeight = XTPToUIntChecked(pListItem->pItem->GetDesiredSize().cy);
}
...
void CXTPMarkupListBox::OnSize(UINT nType, int cx, int cy)
{
	...
	for (int i = 0; i < GetCount(); i++)
	{
		...
		pListItem->pItem->Measure(&dc, CSize(rc.Width(), rc.Height()));
		SetItemHeight(i, XTPToUIntChecked(pListItem->pItem->GetDesiredSize().cy));
	}
}
...
BOOL CXTPMarkupListBox::OnWndMsg(UINT message, WPARAM wParam, LPARAM lParam, LRESULT* pResult)
{
	if ((NULL != m_hWnd))
	{
		...
		{
			XTP_MARKUP_LISTITEM* pItem = reinterpret_cast<XTP_MARKUP_LISTITEM*>(
				GetItemDataPtr(m_nHotItem));
