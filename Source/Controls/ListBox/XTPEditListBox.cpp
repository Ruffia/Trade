...
{
	return IsWindowEnabled() ? GetXtremeColor(COLOR_WINDOW) : GetXtremeColor(COLOR_3DFACE);
}
...
void CXTPEditListBox::OnEndLabelEdit()
{
	...
	if (m_bNewItem)
	{
		...
		DeleteString(static_cast<UINT>(m_nIndex));

		// If the text string is empty, then send a cancel message.
		if (m_strItemText.IsEmpty())
		{
			pOwner->SendMessage(WM_COMMAND, MAKEWPARAM(GetDlgCtrlID(), LBN_XTP_LABELEDITCANCEL),
								XTPToLPARAM(m_hWnd));
		...
		else
		{
			...
			// NB: SetTopIndex below causes some confusion if you have editing session, change
			// content and then double click on another item - the result looks like misplaced edit
			// control
			// SetTopIndex(m_nIndex);

			pOwner->SendMessage(WM_COMMAND, MAKEWPARAM(GetDlgCtrlID(), LBN_XTP_NEWITEM),
								XTPToLPARAM(m_hWnd));
		}
	}
	else
	{
		...
		else
		{
			...
			pOwner->SendMessage(WM_COMMAND, MAKEWPARAM(GetDlgCtrlID(), LBN_XTP_LABELEDITEND),
								XTPToLPARAM(m_hWnd));
		}
	}
...
void CXTPEditListBox::OnItemBrowse()
{
	...
	if (pOwner)
		pOwner->SendMessage(WM_COMMAND, MAKEWPARAM(GetDlgCtrlID(), LBN_XTP_ONBROWSE),
							XTPToLPARAM(m_hWnd));
}
...
void CXTPEditListBox::EditListItem(BOOL bNewItem)
{
	...
	{
		pOwner->SendMessage(WM_COMMAND, MAKEWPARAM(GetDlgCtrlID(), LBN_XTP_PRENEWITEM),
							XTPToLPARAM(m_hWnd));
...
void CXTPEditListBox::DeleteItem()
{
	...
	pOwner->SendMessage(WM_COMMAND, MAKEWPARAM(GetDlgCtrlID(), LBN_XTP_PREDELETEITEM),
						XTPToLPARAM(m_hWnd));
	...
	if (nCurIndex != LB_ERR)
	{
		...
		int nCount = DeleteString(static_cast<UINT>(nCurIndex));
	...
	// Send notification to owner.
	pOwner->SendMessage(WM_COMMAND, MAKEWPARAM(GetDlgCtrlID(), LBN_XTP_DELETEITEM),
						XTPToLPARAM(m_hWnd));
}
...
void CXTPEditListBox::MoveItemUp(int nIndex)
{
	...
	if ((m_nIndex != LB_ERR) && (m_nIndex != 0))
	{
		...
		// index items...
		DeleteString(static_cast<UINT>(nNewIndex));
	...
	// Send notification to owner.
	pOwner->SendMessage(WM_COMMAND, MAKEWPARAM(GetDlgCtrlID(), LBN_XTP_MOVEITEMUP),
						XTPToLPARAM(m_hWnd));
}
...
void CXTPEditListBox::MoveItemDown(int nIndex)
{
	...
	// Send notification to owner.
	pOwner->SendMessage(WM_COMMAND, MAKEWPARAM(GetDlgCtrlID(), LBN_XTP_MOVEITEMDOWN),
						XTPToLPARAM(m_hWnd));
}
...
void CXTPEditListBox::Initialize(bool bAutoFont /*= true*/)
{
	...
	::SetWindowPos(GetSafeHwnd(), NULL, rcWindow.left, rcWindow.top, rcWindow.Width(),
...
BOOL CXTPEditListBox::CreateEditGroup(bool bAutoFont)
{
	...
	// list edit control.
	DWORD dwStyle = (GetStyle() & WS_VISIBLE) | WS_CHILD | SS_CENTERIMAGE | SS_LEFT | SS_NOTIFY;

	if (!m_editGroup.CreateEx(WS_EX_STATICEDGE, _T("Static"), m_strTitle, dwStyle,
							  CRect(0, 0, 0, 0), GetParent(), XTP_IDC_GROUP_EDIT))
	{
...
BOOL CXTPEditListBox::PreTranslateMessage(MSG* pMsg)
{
	...
	{
		if ((pMsg->wParam == VK_UP) && (::GetKeyState(VK_MENU) < 0) && GetCurSel() != LB_ERR)
		{
			OnMoveItemUp();
		}
		if ((pMsg->wParam == VK_DOWN) && (::GetKeyState(VK_MENU) < 0) && GetCurSel() != LB_ERR)
		{
			OnMoveItemDown();
		}
	}
...
void CXTPEditListBox::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	if (m_nIndex >= GetCount())
	{
		...
		GetOwner()->SendMessage(WM_COMMAND, MAKEWPARAM(GetDlgCtrlID(), LBN_SELCHANGE),
								XTPToLPARAM(m_hWnd));
	}
	...
	if (GetItemRect(m_nIndex, rcItem) != LB_ERR)
	{
		if (!rcItem.PtInRect(point))
		{
			...
			SetCurSel(-1);
			pOwner->SendMessage(WM_COMMAND, MAKEWPARAM(GetDlgCtrlID(), LBN_SELCHANGE),
								XTPToLPARAM(m_hWnd));
		}
	}
}
...
void CXTPEditListBox::RecalcLayout()
{
	if (HasToolbar() && ::IsWindow(m_editGroup.m_hWnd))
	{
		...
		::SetWindowPos(m_editGroup.m_hWnd, NULL, rcWindow.left, rcWindow.top, rcWindow.Width(),
					   rcWindow.Height(), SWP_FRAMECHANGED);
	}
}
...
void CXTPEditListBoxToolBar::Initialize(bool bAutoFont /*= true*/)
{
	...
	for (int i = 0; i < _countof(m_arButton); ++i)
	{
		...
		CXTPResourceManager::AssertValid(
			XTPResourceManager()->LoadString(&strToolTip, _arButtonID[i]));
...
void CXTPEditListBoxToolBar::SendCommand(UINT nCmdID)
{
	if (m_bEnableEdit)
	{
		...
		if (pOwner)
			pOwner->SendMessage(WM_COMMAND, MAKEWPARAM(GetDlgCtrlID(), nCmdID),
								XTPToLPARAM(m_hWnd));
	}
}
...
void CXTPEditListBoxToolBar::DrawText(CDC* pDC, CRect& rcClient)
{
	...
	pDC->SetTextColor(
		GetXtremeColor(IsWindowEnabled() ? UINT(COLOR_BTNTEXT) : UINT(COLOR_GRAYTEXT)));
...
void CXTPEditListBoxToolBar::OnNcPaint()
{
	...
	if (wrpTreeView.IsAppThemeActive() && (GetOwner()->GetExStyle() & WS_EX_CLIENTEDGE))
	{
			...
			&& SUCCEEDED(wrpTreeView.GetThemeColor(0, 0, TMT_BORDERCOLOR, &clr)))
		{
			...
			dc.FillSolidRect(XTP_DPI_X(1), rc.Height() - XTP_DPI_Y(1), rc.Width() - XTP_DPI_X(2),
