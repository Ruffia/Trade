...
void CXTPShortcutListBoxTheme::RefreshMetrics(CXTPShortcutListBox* /*pList*/)
{
	...
	m_crgItemBorder.SetStandardValue(GetXtremeColor(COLOR_3DDKSHADOW),
...
void CXTPShortcutListBoxTheme::DrawItem(CXTPShortcutListBox* pMenuListBox, LPDRAWITEMSTRUCT lpDIS)
{
	...
	int iItemID  = static_cast<int>(lpDIS->itemID);
	...
	if ((lpDIS->itemState & ODS_SELECTED) && pCI->m_bEnabled)
	{
		...
		if (::GetKeyState(VK_LBUTTON) < 0)
		{
			...
			pDC->Draw3dRect(&rcBorder, m_crgItemBorder.clrLight, m_crgItemBorder.clrDark);
		...
		else
		{
			...
			pDC->Draw3dRect(&rcBorder, m_crgItemBorder.clrDark, m_crgItemBorder.clrLight);
...
void CXTPShortcutListBoxTheme::DrawImage(CDC* pDC, CRect& rcIcon, CXTPImageManagerIcon* pImage,
										 BOOL bEnabled, BOOL /*bSelected*/, BOOL /*bPressed*/)
{
	...
	{
		if (pImage->HasDisabledIcon() || pImage->IsAlpha() || !pImage->IsRasterIcon())
		{
			pImage->Draw(pDC, pt, xtpImageDisabled, szIcon);
		}
		else
		{
			pDC->DrawState(pt, rcIcon.Size(), pImage->GetIcon(), DST_ICON | DSS_DISABLED,
						   (HBRUSH)NULL);
		}
	}
}

void CXTPShortcutListBoxTheme::DrawText(CDC* pDC, CRect& rcText,
										CXTPShortcutListBox::CONTENT_ITEM* pCI)
{
	if (!pCI->m_bEnabled)
	{
		CRect rc = rcText;
		...
		pDC->DrawText(pCI->m_strText, rc, DT_CENTER | DT_TOP | DT_END_ELLIPSIS);
...
void CXTPShortcutListBoxThemeOfficeXP::RefreshMetrics(CXTPShortcutListBox* pList)
{
	...
	m_crgItemBorder.SetStandardValue(GetXtremeColor(XPCOLOR_HIGHLIGHT_BORDER),
									 GetXtremeColor(XPCOLOR_HIGHLIGHT_BORDER));
	m_crItemTextDisabled.SetStandardValue(GetXtremeColor(XPCOLOR_GRAYTEXT));

	// hot, pressed colors
	m_crgItemBackHot.SetStandardValue(GetXtremeColor(XPCOLOR_HIGHLIGHT),
									  GetXtremeColor(XPCOLOR_HIGHLIGHT));
	m_crgItemBackPressed.SetStandardValue(GetXtremeColor(XPCOLOR_HIGHLIGHT_PUSHED),
										  GetXtremeColor(XPCOLOR_HIGHLIGHT_PUSHED));
}

void CXTPShortcutListBoxThemeOfficeXP::DrawImage(CDC* pDC, CRect& rcIcon,
												 CXTPImageManagerIcon* pImage, BOOL bEnabled,
												 BOOL bSelected, BOOL bPressed)
{
	...
	if (bEnabled)
	{
		if (!bSelected && !bPressed)
		{
			...
			{
				pImage->Draw(pDC, pt, xtpImageNormal, szIcon);
			}
		}
		else if (bSelected && !bPressed)
		{
			...
			pImage->Draw(pDC, CPoint(pt.x - 1, pt.y - 1), xtpImageHot, szIcon);
		}
		else
		{
...
void CXTPShortcutListBoxThemeOfficeXP::DrawText(CDC* pDC, CRect& rcText,
												CXTPShortcutListBox::CONTENT_ITEM* pCI)
{
...
void CXTPShortcutListBoxThemeOffice2003::DrawImage(CDC* pDC, CRect& rcIcon,
												   CXTPImageManagerIcon* pImage, BOOL bEnabled,
												   BOOL /*bSelected*/, BOOL /*bPressed*/)
{
	CPoint pt(rcIcon.TopLeft());
