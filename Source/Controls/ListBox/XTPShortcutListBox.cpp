...
BOOL CXTPShortcutListBox::SetTheme(XTPControlTheme eTheme)
{
	...
	switch (eTheme)
	{
		case xtpControlThemeOfficeXP: m_pTheme = new CXTPShortcutListBoxThemeOfficeXP; break;

		case xtpControlThemeOffice2003: m_pTheme = new CXTPShortcutListBoxThemeOffice2003; break;

		default: m_pTheme = new CXTPShortcutListBoxTheme; break;
	}
...
{
	BOOL bOutSide;
	int nIndex = static_cast<int>(ItemFromPoint(point, bOutSide));
...
void CXTPShortcutListBox::OnLButtonUp(UINT nFlags, CPoint point)
{
	...
	if (pWndOwner)
		pWndOwner->SendMessage(WM_XTP_SHORTCUTLBOX_NOTIFY, XTPToWPARAM(GetCurSel()),
							   static_cast<LPARAM>(GetDlgCtrlID()));
}
...
void CXTPShortcutListBox::MeasureItem(LPMEASUREITEMSTRUCT lpMIS)
{
	...
	lpMIS->itemHeight = XTPToUIntChecked(GetTheme()->m_cyEdge + GetTheme()->m_cyIcon
										 + GetTheme()->m_cyEdge + iTextHeight
										 + GetTheme()->m_cyEdge);
}
...
int CXTPShortcutListBox::InsertMenuItem(int iIndex, UINT nIconID, LPCTSTR lpszText,
										BOOL bEnabled /*= TRUE*/)
{
	CONTENT_ITEM* pCI = new CONTENT_ITEM;
	ASSERT(pCI != NULL);
...
void CXTPShortcutListBox::SetHighlightCursor(UINT nIDCursor, bool bNoBorder /*= false*/,
											 bool bNoUnderline /*= false*/)
{
	// load the new cursor.
...
void CXTPShortcutListBox::SetColors(COLORREF clrText, COLORREF clrBack,
									COLORREF clrBackDark /*= COLORREF_NULL*/)
{
	GetTheme()->m_crItemText.SetCustomValue(
		clrText); // Must be custom value used to prevent reset color in RefreshMetrics.
