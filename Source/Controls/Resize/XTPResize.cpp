...
BOOL CXTPResize::RemoveResize(UINT nID)
{
	// search for an item with the given id

	for (INT_PTR iItem = m_arrItems.GetSize() - 1; iItem >= 0; iItem--)
	{
		CWnd* pWnd = m_arrItems[iItem]->m_pWnd;
		ASSERT(pWnd);

		if (pWnd->GetDlgCtrlID() == XTPToInt(nID))
		{
...
void CXTPResize::RemoveAllControls()
{
	...
	for (INT_PTR iIndex = 0; iIndex < m_arrItems.GetSize(); ++iIndex)
	{
...
void CXTPResize::SetResize(UINT nID, HWND hWnd, const XTP_RESIZERECT& rrcSizing)
{
	CXTPResizeItem* pItem = 0;

	ASSERT("Window has ID different from the provided one"
		   && (0 != nID && NULL != hWnd ? static_cast<UINT>(::GetWindowLong(hWnd, GWL_ID)) == nID
										: TRUE));

	if (0 != nID)
	{
		// Search for an item with the given ID.
		for (INT_PTR i = m_arrItems.GetSize() - 1; i >= 0; i--)
		{
			CWnd* pWnd = m_arrItems[i]->m_pWnd;
			if (pWnd && ::IsWindow(pWnd->m_hWnd) && (pWnd->GetDlgCtrlID() == XTPToInt(nID)))
			{
				pItem = m_arrItems[i];
			}
		}
	...
	if (NULL == pItem)
	{
		...
		if (NULL == hWndChild)
		{
			CWnd* pWnd = m_pWnd->GetDlgItem(XTPToInt(nID));
...
void CXTPResize::Init()
{
	...
	if (!HasFlag(xtpResizeNoSizeIcon))
	{
		...
		if (!::IsWindow(m_scSizeIcon.m_hWnd))
		{
			...
			m_scSizeIcon.Create(WS_CHILD | WS_VISIBLE | SBS_SIZEGRIP, rcIcon, m_pWnd, IDC_SIZEICON);
		}
...
void CXTPResize::Offset(CPoint ptOffset)
{
	...
	{
		HDWP hDWP = ::BeginDeferWindowPos(XTPToIntChecked(m_arrItems.GetSize()));
...
BOOL CXTPResize::Defer(HDWP& hDWP, CXTPResizeItem* pItem, int dx, int dy)
{
	...
	if (::IsWindow(hWnd))
	{
		...
		rrcItem.left += static_cast<XTP_RESIZE>(dx) * pItem->m_rrcSizing.left;
		rrcItem.top += static_cast<XTP_RESIZE>(dy) * pItem->m_rrcSizing.top;
		rrcItem.right += static_cast<XTP_RESIZE>(dx) * pItem->m_rrcSizing.right;
		rrcItem.bottom += static_cast<XTP_RESIZE>(dy) * pItem->m_rrcSizing.bottom;

		if (rrcItem != pItem->m_rrcWindow)
		{
			int x  = static_cast<int>(rrcItem.left);
			int y  = static_cast<int>(rrcItem.top);
			int cx = static_cast<int>(rrcItem.Width());
			int cy = static_cast<int>(rrcItem.Height());
			...
			hDWP = ::DeferWindowPos(hDWP, hWnd, NULL, x, y, cx, cy, uFlags);
			if (hDWP == NULL)
			{
...
void CXTPResize::GetMinMaxInfo(MINMAXINFO* pMMI)
{
	...
	if (m_szMin.cx != 0)
		pMMI->ptMinTrackSize.x = m_szMin.cx + szBorder.cx;
	if (m_szMin.cy != 0)
		pMMI->ptMinTrackSize.y = m_szMin.cy + szBorder.cy;
	if (m_szMax.cx != 0)
		pMMI->ptMaxTrackSize.x = m_szMax.cx + szBorder.cx;
	if (m_szMax.cy != 0)
		pMMI->ptMaxTrackSize.y = m_szMax.cy + szBorder.cy;
}
...
void CXTPResize::UpdateControlRect(CWnd* pWnd)
{
	...
	for (INT_PTR i = m_arrItems.GetSize(); i--;)
	{
...
CXTPResizeItem::CXTPResizeItem(CWnd* pWnd, const CXTPResizeRect& rrcSizing, CRect& rcWindow,
							   BOOL bAutoDelete)
	: m_bIsGroupBox(FALSE)
...
bool CXTPResizeItem::MakeTransparent(CXTPResize* pXTPResize)
{
	...
	if (m_pWnd && ::IsWindow(m_pWnd->m_hWnd))
	{
		...
		DWORD dwStyle = XTPToDWORD(::GetWindowLong(m_pWnd->m_hWnd, GWL_STYLE));
		if ((dwStyle & (BS_GROUPBOX | WS_TABSTOP)) == BS_GROUPBOX)
		{
			...
			DWORD dwStyleEx = XTPToDWORD(::GetWindowLong(m_pWnd->m_hWnd, GWL_EXSTYLE));

			// add the WS_EX_TRANSPARENT flag to the group box.
			::SetWindowLong(m_pWnd->m_hWnd, GWL_EXSTYLE, LONG(dwStyleEx | WS_EX_TRANSPARENT));

			// apply the style for the window.
			::SetWindowPos(m_pWnd->m_hWnd, NULL, 0, 0, 0, 0,
