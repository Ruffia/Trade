...
IMPLEMENT_DYNAMIC(CXTPResizeGroupBox, CXTPButton)

#include "Common/Base/Diagnostic/XTPBeginAfxMap.h"
BEGIN_MESSAGE_MAP(CXTPResizeGroupBox, CXTPButton)
...
void CXTPResizeGroupBox::Exclude(CDC* pDC, CRect& rcClient)
{
	...
	while (pChildWnd != NULL)
	{
		...
		if (pChildWnd != this && pChildWnd->IsWindowVisible())
		{
			...
			// it from painting.
			if (rcClient.PtInRect(rc.TopLeft()) || rcClient.PtInRect(rc.BottomRight()))
			{
...
void CXTPResizeGroupBox::OnPaint()
{
	...
	// CXTPButton::DefWindowProc(WM_PAINT, (WPARAM)memDC.m_hDC, 0);
	CXTPButton::OnDraw(&memDC);
}
