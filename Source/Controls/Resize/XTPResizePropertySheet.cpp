...
CXTPResizePropertySheet::CXTPResizePropertySheet(const UINT nCaption, CWnd* pParent,
												 const UINT nSelectPage, const UINT nFlags)
	: CPropertySheet(nCaption, pParent, nSelectPage)
	, CXTPResize(this, nFlags)
{
}

CXTPResizePropertySheet::CXTPResizePropertySheet(LPCTSTR pszCaption, CWnd* pParent,
												 const UINT nSelectPage, const UINT nFlags)
...
BOOL CXTPResizePropertySheet::OnInitDialog()
{
	...
	::SetWindowLong(m_hWnd, GWL_STYLE, LONG(GetStyle() | WS_THICKFRAME));
	...
	if (pSysMenu)
	{
		...
		if (szMaximize.IsEmpty())
			szMaximize = _T("Ma&ximize");
		if (szMinimize.IsEmpty())
			szMinimize = _T("Mi&nimize");
		if (szSize.IsEmpty())
			szSize = _T("&Size");
		if (szRestore.IsEmpty())
			szRestore = _T("&Restore");
