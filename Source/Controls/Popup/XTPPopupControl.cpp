	...
	, m_bAutoDelete(bAutoDelete)
{
	...
	m_bDpiBitmapScalingModified = FALSE;
}
...
CPoint CXTPPopupControl::GetPopupPos() const
{
	...
	{
		return CPoint(rcDeskWnd.CenterPoint().x + m_szPopup.cx / 2,
					  rcDeskWnd.CenterPoint().y + m_szPopup.cy / 2);
	}
...
void CXTPPopupControl::SetRegionAlphaLayer(CXTPImageManagerIcon* pIcon)
{
	...
	MEMCPY_S(pvBits, pBits, XTPToSizeTChecked(cx * cy * 4));

	HBITMAP hOld = (HBITMAP)SelectObject(dc, hBitmap);

	ModifyStyleEx(0, WS_EX_LAYERED);
	m_pfnUpdateLayeredWindow((HWND)GetSafeHwnd(), (HDC)0, 0, &szIcon, dc.GetSafeHdc(), &pt, 0, &bf,
							 0x02);
	...
	for (int y = 0; y < nHeight; y++)
	{
		for (int x = 0; x < nWidth; x++)
		{
			...
			if (x > x0)
			{
				...
				{
					mMaxRects += nAlloc;
					RGNDATA* pReData = (RGNDATA*)realloc(pData, sizeof(RGNDATAHEADER)
																	+ (sizeof(RECT) * mMaxRects));
					if (!pReData)
					{
						DeleteObject(hbm32);
						free(pData);
				...
				if (pData->rdh.nCount > 2000)
				{
					HRGN h = ExtCreateRegion(NULL,
											 sizeof(RGNDATAHEADER) + (sizeof(RECT) * mMaxRects),
											 pData);
					if (hRgn)
					{
...
void CXTPPopupControl::UpdateBitmapRegion()
{
	...
	CXTPImageManagerIcon* pImage = m_pImageManager->GetImage(XTPToUInt(m_nBackgroundBitmap), 0);
	if (!pImage)
	{
		SetWindowRgn(NULL, FALSE);
		return;
...
BOOL CXTPPopupControl::IsDpiBitmapScalingEnabled() const
{
	return (m_bDpiBitmapScalingModified ? m_bDpiBitmapScaling
										: XTPDpiHelper()->IsDpiBitmapScalingEnabled());
}

void CXTPPopupControl::EnableDpiBitmapScaling(BOOL bEnable /*= TRUE*/)
{
	m_bDpiBitmapScaling			= bEnable;
	m_bDpiBitmapScalingModified = TRUE;
}
...
BOOL CXTPPopupControl::Create(CWnd* pParentWnd)
{
	...
	DWORD dwExStyle = m_pfnSetLayeredWindowAttributes
							  && (m_nTransparency < 255
								  || m_popupAnimation == xtpPopupAnimationFade)
						  ? DWORD(WS_EX_LAYERED)
						  : 0;
	...
	if (!CreateEx(dwExStyle | WS_EX_TOOLWINDOW | WS_EX_TOPMOST
				  ...
				  AfxRegisterWndClass(NULL, AfxGetApp()->LoadStandardCursor(IDC_ARROW)), NULL,
				  WS_POPUP, CRect(0, 0, 0, 0), pParentWnd, NULL))
		return FALSE;
...
void CXTPPopupControl::SetTheme(XTPPopupPaintTheme theme)
{
	...
	switch (theme)
	{
		case xtpPopupThemeMSN: SetTheme(new CXTPPopupThemeMSN()); break;
		case xtpPopupThemeOffice2003: SetTheme(new CXTPPopupThemeOffice2003()); break;
		case xtpPopupThemeOffice2013: SetTheme(new CXTPPopupThemeOffice2013()); break;
		case xtpPopupThemeResource: SetTheme(new CXTPPopupThemeResource()); break;
		case xtpPopupThemeOfficeXP: SetTheme(new CXTPPopupThemeOfficeXP()); break;
		case xtpPopupThemeOffice2000: SetTheme(new CXTPPopupThemeOffice2000()); break;
...
void CXTPPopupControl::OnShow()
{
	...
	m_stateCurrent.rcPopup		 = CRect(CPoint(ptPopup.x - m_szPopup.cx, ptPopup.y - m_szPopup.cy),
									 m_szPopup);
...
void CXTPPopupControl::OnCollapsing()
{
	...
	// calc step
	m_nStep = XTPToIntChecked(max(1, m_uAnimationDelay / m_nAnimationInterval));
...
void CXTPPopupControl::OnExpanding(BOOL bUpdateCurrent)
{
	...
	// reinit target rect
	m_stateTarget.rcPopup		= CRect(CPoint(ptPopup.x - m_szPopup.cx, ptPopup.y - m_szPopup.cy),
									m_szPopup);
	...
	if (bUpdateCurrent)
	{
		...
		{
			// set rectangular of target
			m_stateCurrent.rcPopup = CRect(ptPopup.x - m_szPopup.cx, ptPopup.y, ptPopup.x,
										   ptPopup.y);
		}
		// if Unfold animation mode
		else if (m_popupAnimation == xtpPopupAnimationUnfold)
	...
	{
		// calc step
		m_nStep = XTPToIntChecked(max(1, m_uAnimationDelay / m_nAnimationInterval - m_nStep));
	}
...
void CXTPPopupControl::TrackMove()
{
	...
	while (GetCapture() == this)
	{
		...
		else if (msg.message == WM_MOUSEMOVE)
		{
			...
			if (rc.left < rcDeskWnd.left)
				rc.OffsetRect(rcDeskWnd.left - rc.left, 0);
			if (rc.top < rcDeskWnd.top)
				rc.OffsetRect(0, rcDeskWnd.top - rc.top);
			if (rc.right > rcDeskWnd.right)
				rc.OffsetRect(rcDeskWnd.right - rc.right, 0);
			if (rc.bottom > rcDeskWnd.bottom)
				rc.OffsetRect(0, rcDeskWnd.bottom - rc.bottom);
...
void CXTPPopupControl::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	if (m_bAllowMove
		&& (!pPressed
			|| (!pPressed->GetID() && !pPressed->IsButton()
				&& (pPressed->GetCaption().IsEmpty() || !pPressed->IsHyperLink()))))
	{
		TrackMove();
		return;
	}
...
void CXTPPopupControl::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	if (pSelected != m_pSelected)
	{
		...
		m_pSelected = (m_pPressed == 0 || m_pPressed == pSelected || pSelected == NULL) ? pSelected
																						: NULL;
...
{
	SetPopupAnimation(m_pfnSetLayeredWindowAttributes ? xtpPopupAnimationFade
													  : xtpPopupAnimationSlide);
}
