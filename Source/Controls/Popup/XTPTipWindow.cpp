...
BOOL CXTPTipWindow::Create(CWnd* pParentWnd)
{
	...
	XTPDrawHelpers()->RegisterWndClass(0, _T("XTPTipWindow"),
									   CS_SAVEBITS | CS_HREDRAW | CS_VREDRAW);

	// call the base class for creation.
	if (!CWnd::CreateEx(0, _T("XTPTipWindow"), _T(""), WS_POPUP, 0, 0, 0, 0, pParentWnd->m_hWnd, 0,
						NULL))
	{
...
{
	if (((m_dwTipStyle & (TWS_XTP_ALPHASHADOW | TWS_XTP_DROPSHADOW))
		 == (TWS_XTP_ALPHASHADOW | TWS_XTP_DROPSHADOW))
		&& XTPSystemVersion()->IsWinXPOrGreater()) // Windows XP only
	{
		SetClassLong(m_hWnd, GCL_STYLE, XTPToLong(GetClassLong(m_hWnd, GCL_STYLE)) | CS_DROPSHADOW);
		m_bSystemShadow = TRUE;
	}
	else
	{
		SetClassLong(m_hWnd, GCL_STYLE,
					 XTPToLong(GetClassLong(m_hWnd, GCL_STYLE)) & ~CS_DROPSHADOW);
		m_bSystemShadow = FALSE;
	}
}

BOOL CXTPTipWindow::ShowTipWindow(const CPoint& point, DWORD dwTipStyle /*= TWS_XTP_DROPSHADOW*/,
								  UINT nDelay /*= 0*/, UINT nElapse /*= 5000*/,
								  BOOL bCenterHorz /*= FALSE*/)
{
	...
	if (!nDelay)
	{
		...
		SetWindowPos(&wndTop, m_rcScreenWindow.left, m_rcScreenWindow.top,
					 m_rcShadow.right - m_rcWindow.left, m_rcShadow.bottom - m_rcWindow.top,
					 SWP_SHOWWINDOW | SWP_NOACTIVATE); // see also InitializeSize()
	...
	if (nElapse || nDelay)
	{
		if (nDelay)
		{
			SetTimer(m_nDelayTimerEventID, nDelay, NULL); // if timer runns out, window will be
														  // shown
			m_bDelayTimerRunning = TRUE;
		}

		if (nElapse)
			SetTimer(m_nElapseTimerEventID, nDelay + nElapse,
					 NULL); // if timer runns out, window will be hide
	}
...
//
BOOL CXTPTipWindow::ShowTipWindow(const CRect& rectCtrl, const CPoint& pointMousePos,
								  DWORD dwTipStyle /*= TWS_XTP_DROPSHADOW*/, UINT nDelay /*= 0*/,
								  UINT nElapse /*= 5000*/, BOOL bCenterHorz /*= FALSE*/,
								  BOOL bShowTipAtCursorPos /*= FALSE*/)
{
	// Check for valid tip text
...
void CXTPTipWindow::InitializeSize(const CPoint& point, BOOL bCenterHorz /*FALSE*/)
{
	...
	else
	{
		...
		for (nStrings = 0; nStrings < m_arStrings.GetSize(); ++nStrings)
		{
			...
			{
				sizeTemp = dc.GetTabbedTextExtent(m_arStrings[nStrings], m_nTabPositions,
												  m_pnTabStopPositions);
	...
	int nHeight = (m_sizeMargin.cy * 2
				   + ((m_strTitle.GetLength() > 0) ? m_sizeTitle.cy + m_nLineSpace : 0)
				   + sizeDescrip.cy);
...
{
	return RGB(CheckValue(GetRValue(crPixel) - i), CheckValue(GetGValue(crPixel) - i),
...
void CXTPTipWindow::OnPaint()
{
	...
	dc.Draw3dRect(m_rcWindow, GetXtremeColor(COLOR_3DDKSHADOW), GetXtremeColor(COLOR_3DDKSHADOW));
	...
	if (m_dwTipStyle & TWS_XTP_THICKBORDER)
	{
		...
		dc.Draw3dRect(rect, GetXtremeColor(COLOR_3DSHADOW), GetXtremeColor(COLOR_3DSHADOW));
	}
	...
	if (m_arStrings.GetSize() == 0)
		dc.TabbedTextOut(m_sizeMargin.cx + XTP_DPI_X(1),
						 m_sizeMargin.cy
							 + ((m_strTitle.GetLength() > 0) ? m_sizeTitle.cy + m_nLineSpace : 0),
						 m_strDescrip, m_nTabPositions, m_pnTabStopPositions,
						 m_sizeMargin.cx + XTP_DPI_X(1));
	else
	{
		// Get the string height for stepping through
		CSize sizeDesc = dc.GetTabbedTextExtent(m_arStrings[0], m_nTabPositions,
												m_pnTabStopPositions);

		// The second parameter is long because we add the height of the strings already done, and
		// also we don't add anything to get below the title if the length of the title is 0.
		int nStrings;
		for (nStrings = 0; nStrings < m_arStrings.GetSize(); ++nStrings)
			dc.TabbedTextOut(m_sizeMargin.cx + XTP_DPI_X(1),
							 m_sizeMargin.cy + (nStrings * sizeDesc.cy)
								 + ((m_strTitle.GetLength() > 0) ? m_sizeTitle.cy + m_nLineSpace
																 : 0),
							 m_arStrings[nStrings], m_nTabPositions, m_pnTabStopPositions,
							 m_sizeMargin.cx + XTP_DPI_X(1));
	}
...
void CXTPTipWindow::OnTimer(UINT_PTR nIDEvent)
{
	...
	if (m_nDelayTimerEventID == nIDEvent)
	{
		...
		SetWindowPos(&wndTop, m_rcScreenWindow.left, m_rcScreenWindow.top, m_rcScreenWindow.Width(),
					 m_rcScreenWindow.Height(),
					 SWP_SHOWWINDOW | SWP_NOACTIVATE); //+5 because of the shadow
		SetCapture();
		m_bDelayTimerRunning = FALSE;
	}
...
void CXTPTipWindow::ForwardMessage(UINT uiMsg, UINT nFlags, CPoint point)
{
	...
	if (!pClick->IsChild(this) && pClick != this)
	{
		...
		if (AfxGetMainWnd() != NULL)
			pClick->SendMessage(WM_MOUSEACTIVATE, XTPToWPARAM(AfxGetMainWnd()->m_hWnd),
								MAKELPARAM(iHitTest, uiMsg));
