...
void CXTPColorPopup::OnDestroy()
{
	...
	m_pParentWnd->SendMessage(CPN_XTP_CLOSEUP, (WPARAM)m_clrColor, m_callerParam);
}
...
int CXTPColorPopup::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	SetWindowPos(pTopParent->GetExStyle() & WS_EX_TOPMOST ? &CWnd::wndTopMost : &CWnd::wndTop,
				 m_rcWnd.left, m_rcWnd.top, m_rcWnd.Width(), m_rcWnd.Height(),
	...
	m_pParentWnd->SendMessage(CPN_XTP_DROPDOWN, (WPARAM)m_clrColor, m_callerParam);
...
void CXTPColorPopup::OnNcPaint()
{
	if (GetTheme()->m_bThinBorder)
	{
		...
		dc.Draw3dRect(0, 0, rc.Width(), rc.Height(), GetTheme()->m_crPopupBorder,
					  GetTheme()->m_crPopupBorder);
	}
	else
	{
		Default();
