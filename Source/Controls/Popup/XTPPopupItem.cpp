...
CXTPPopupItem::CXTPPopupItem(LPRECT rcItem, LPCTSTR lpszCaption, COLORREF clrBackground,
							 COLORREF clrBorder)
	: m_nID(0)
	, m_nIndex(0)
	, m_nIconIndex((int)-1)
...
void CXTPPopupItem::Draw(CDC* pDC)
{
	if (m_pUIElement)
	{
		XTPMarkupSetDefaultFont(m_pControl->GetMarkupContext(),
								(HFONT)GetTextFont()->GetSafeHandle(),
								m_clrText == (COLORREF)-1 ? m_pControl->GetPaintManager()->m_clrText
														  : m_clrText);
...
CFont* CXTPPopupItem::GetTextFont()
{
	...
	return (m_pControl && m_xtpFontText.m_hObject == NULL)
			   ? &m_pControl->GetPaintManager()->m_xtpFontText
			   : &m_xtpFontText;
}
...
void CXTPPopupItem::CalculateHeight()
{
	...
	{
		XTPMarkupSetDefaultFont(m_pControl->GetMarkupContext(),
								(HFONT)GetTextFont()->GetSafeHandle(), COLORREF_NULL);
...
BOOL CXTPPopupItem::SetIcon(HICON hIcon, XTPPopupItemIcon itemIcon)
{
	...
	return m_pControl->GetImageManager()->SetIcon(hIcon, XTPToUInt(GetIconIndex()), 0,
												  itemIcon == xtpPopupItemIconNormal
													  ? xtpImageNormal
													  : itemIcon == xtpPopupItemIconSelected
															? xtpImageHot
															: xtpImageChecked);
}
...
{
	return m_pControl->GetImageManager()->SetIcon(nIDIcon, XTPToUInt(GetIconIndex()), 0,
												  itemIcon == xtpPopupItemIconNormal
													  ? xtpImageNormal
													  : itemIcon == xtpPopupItemIconSelected
															? xtpImageHot
...
BOOL CXTPPopupItem::SetIcons(UINT nIDBitmap, COLORREF clrTransparent, int itemIcon)
{
	...
	HBITMAP hBitmap = CXTPImageManagerIcon::LoadBitmapFromResource(MAKEINTRESOURCE(nIDBitmap),
																   &bAlphaBitmap);
	if (!hBitmap)
		return FALSE;
							 ...
							 BOOL bAlphaBitmap)
{
	...
	if ((itemIcon & xtpPopupItemIconNormal) != 0)
	{
		...
		nIDs[nIndex++] = XTPToUInt(GetIconIndex());

		m_pControl->GetImageManager()->SetIcons(bmp, nIDs, nCount,
												CSize(bmpInfo.bmWidth / nCount, bmpInfo.bmHeight),
												xtpImageNormal, bAlphaBitmap);
	}
...
{
	return m_pControl->GetImageManager()->GetImage(XTPToUInt(GetIconIndex()), m_rcItem.Width());
}
...
void CXTPPopupItem::CenterIcon()
{
	...
	if (rc.Width() > sz.cx)
		m_ptOffset.x = rc.Width() / 2 - sz.cx / 2;
	if (rc.Height() > sz.cy)
		m_ptOffset.y = rc.Height() / 2 - sz.cy / 2;
}
...
{
	SetIcon(reinterpret_cast<HICON>((LONG_PTR)hIcon), static_cast<XTPPopupItemIcon>(itemIcon));
}

void CXTPPopupItem::OleSetIcons(int hBitmap, COLORREF clrTransparent, int itemIcon)
{
	SetIcons(reinterpret_cast<HBITMAP>((INT_PTR)hBitmap), clrTransparent, itemIcon);
}
