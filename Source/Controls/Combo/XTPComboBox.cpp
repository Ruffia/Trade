...
#include "Controls/Combo/Themes/XTPComboBoxThemeVisualStudio2017.h"
#include "Controls/Combo/Themes/XTPComboBoxThemeVisualStudio2019.h"
#include "Controls/Combo/Themes/XTPComboBoxThemeVisualStudio2022.h"
...
BOOL CXTPComboBox::SetTheme(XTPControlTheme nTheme)
{
	...
	switch (nTheme)
	{
		...
		case xtpControlThemeNativeWinXP: m_pTheme = new CXTPComboBoxThemeDefault(); break;

		case xtpControlThemeResource: m_pTheme = new CXTPComboBoxThemeResource(); break;

		case xtpControlThemeOffice2000: m_pTheme = new CXTPComboBoxThemeOffice2000(); break;

		case xtpControlThemeOfficeXP:
		case xtpControlThemeFlat:
		case xtpControlThemeUltraFlat: m_pTheme = new CXTPComboBoxThemeOfficeXPFlat(nTheme); break;

		case xtpControlThemeOffice2013: m_pTheme = new CXTPComboBoxThemeOffice2013(); break;

		case xtpControlThemeOffice2003: m_pTheme = new CXTPComboBoxThemeOffice2003(); break;

		case xtpControlThemeVisualStudio2005:
		case xtpControlThemeVisualStudio2008: m_pTheme = new CXTPComboBoxThemeVisualStudio(); break;
		...
		case xtpControlThemeVisualStudio2012Dark:
			m_pTheme = new CXTPComboBoxThemeVisualStudio2012(
				nTheme == xtpControlThemeVisualStudio2012Light);
			break;
		...
		case xtpControlThemeVisualStudio2017:
			m_pTheme = new CXTPComboBoxThemeVisualStudio2017();
			break;

		case xtpControlThemeVisualStudio2019:
			m_pTheme = new CXTPComboBoxThemeVisualStudio2019();
			break;

		case xtpControlThemeVisualStudio2022:
			m_pTheme = new CXTPComboBoxThemeVisualStudio2022();
			break;

		case xtpControlThemeNativeWindows10:
			m_pTheme = new CXTPComboBoxThemeNativeWindows10();
			break;

		default: m_pTheme = new CXTPComboBoxTheme();
	}

	m_bUseVisualStyle = (nTheme == xtpControlThemeDefault || nTheme == xtpControlThemeNativeWinXP);
...
HBRUSH CXTPComboBox::GetClientBrush(CDC* pDC)
{
	...
	HBRUSH hBrush = (HBRUSH)::SendMessage(::GetParent(m_hWnd),
										  UINT(bDisabled ? WM_CTLCOLORSTATIC : WM_CTLCOLOREDIT),
...
void CXTPComboBox::OnMouseLeave()
{
	...
	if (m_pWndEdit && m_pWndEdit->GetSafeHwnd())
	{
		...
		{
			TRACKMOUSEEVENT tme = { sizeof(TRACKMOUSEEVENT), TME_LEAVE, m_pWndEdit->GetSafeHwnd(),
									HOVER_DEFAULT };
...
BOOL CXTPComboBox::PreTranslateMessage(MSG* pMsg)
{
	...
	if (pMsg->message == WM_KEYDOWN || pMsg->message == WM_KEYUP)
	{
		...
		switch (pMsg->wParam)
		{
			...
			case VK_TAB:
			case VK_RETURN:
			case VK_ESCAPE: { return CComboBox::PreTranslateMessage(pMsg);
			}
		}
	...
	if (m_bAutoComp)
	{
		...
		else
		{
			...
			if (nLength >= 1)
			{
				...
				{
					SetWindowText(
						strItemTyped); // No text selected, so restore what was there before
...
HBRUSH CXTPComboBox::CtlColor(CDC* pDC, UINT nCtlColor)
{
	...
	switch (nCtlColor)
	{
		// in mode DropList when combobox without focus, in this mode combobox does not have edit
		// control
		case CTLCOLOR_EDIT:
		{
		...
		// when combobox is disabled in DropList mode, in this mode combobox does not have edit
		// control
