// XTPComboBoxThemeVisualStudio2012.cpp: implementation of the CXTPComboBoxThemeVisualStudio20122012
// class.
//
...
void CXTPComboBoxThemeVisualStudio2012::RefreshMetrics(CXTPComboBox* pComboBox)
{
	...
	if (m_bLight)
	{
		...
		m_clrButtonNormal.SetStandardValue(
			RGB(239, 239, 242)); // color of button when combobox is disabled
	...
	else
	{
		...
		// button
		m_clrButtonNormal.SetStandardValue(
			RGB(45, 45, 48)); // color of button when combobox is disabled
...
void CXTPComboBoxThemeVisualStudio2012::DrawButton(CDC* pDC, CXTPComboBox* pComboBox, CRect rcBtn)
{
	...
	if (pComboBox->IsHighlighted())
	{
		...
		CXTPPenDC penHoverLine(pDC->GetSafeHdc(),
							   bEnabled ? clrHoverLine : GetXtremeColor(COLOR_3DSHADOW));
