...
void CXTPComboBoxThemeOffice2003::RefreshMetrics(CXTPComboBox* pComboBox)
{
	...
	switch (systemTheme)
	{
		...
		default:
			...
			m_grcButton.SetStandardValue(XTPColorManager()->LightColor(GetXtremeColor(COLOR_3DFACE),
...
void CXTPComboBoxThemeOffice2003::DrawBackground(CDC* pDC, CXTPComboBox* pComboBox)
{
	...
	{
		pDC->Draw3dRect(rc, bEnabled ? clrBorders : GetXtremeColor(COLOR_3DSHADOW),
						bEnabled ? clrBorders : GetXtremeColor(COLOR_3DSHADOW));
	}
}
...
void CXTPComboBoxThemeOffice2003::DrawButton(CDC* pDC, CXTPComboBox* pComboBox, CRect rcBtn)
{
	...
	else
	{
		if (bEnabled)
		{
			CXTPPaintManagerColorGradient& clrButton = bPressed ? m_grcButtonPushed
																: bSelected ? m_grcButtonHilite
																			: m_grcButton;
		...
		{
			COLORREF clrButton = bPressed ? m_clrButtonPressed
