...
void CXTPComboBoxThemeResource::RefreshMetrics(CXTPComboBox* pComboBox)
{
	...
	const COLORREF clrBorderHilite = XTPResourceImages()->GetImageColor(_T("RibbonComboBox"),
																		_T("BorderHilite"));
	m_clrBorderHot.SetStandardValue(clrBorderHilite);

	const COLORREF clrBorder = XTPResourceImages()->GetImageColor(_T("RibbonComboBox"),
																  _T("Border"));
	m_clrBorderNormal.SetStandardValue(clrBorder);

	const COLORREF clrBorderDisabled = XTPResourceImages()->GetImageColor(_T("RibbonComboBox"),
																		  _T("BorderDisabled"));
...
BOOL CXTPComboBoxThemeResource::DrawComboBox(CDC* pDC, CXTPComboBox* pComboBox)
{
	...
	else
	{
		...
		{
			FillRect(pDC->GetSafeHdc(),
					 CRect(rc.right - GetSystemMetrics(SM_CXVSCROLL) - XTP_DPI_X(2), rc.top,
						   rc.right, rc.bottom),
					 hBrush);
		}
		...
		if (!bSelected && !bPressed)
		{
			if (pComboBox->GetFlatStyle())
			{
				CXTPPenDC pen(pDC->GetSafeHdc(),
							  bEnabled ? ::GetSysColor(COLOR_WINDOW) : ::GetSysColor(COLOR_3DFACE));
			...
			{
				CXTPPenDC pen(pDC->GetSafeHdc(),
							  bEnabled ? m_clrBorderNormal : m_clrBorderDisabled);
				pDC->MoveTo(rcBtn.left, rcBtn.top + 1);
				pDC->LineTo(rcBtn.left, rcBtn.bottom - 1);
			}
		}
