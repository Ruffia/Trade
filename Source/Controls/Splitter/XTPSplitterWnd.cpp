...
BOOL CXTPSplitterWnd::SetTheme(XTPControlTheme eTheme)
{
	...
	switch (eTheme)
	{
		...
		case xtpControlThemeVisualStudio2008: pTheme = new CXTPSplitterWndThemeOfficeXP(); break;
		...
		case xtpControlThemeVisualStudio2017:
			pTheme = new CXTPSplitterWndThemeVisualStudio2017();
			break;

		case xtpControlThemeVisualStudio2019:
			pTheme = new CXTPSplitterWndThemeVisualStudio2019();
			break;

		case xtpControlThemeVisualStudio2022:
			pTheme = new CXTPSplitterWndThemeVisualStudio2022();
			break;

		case xtpControlThemeOffice2003: pTheme = new CXTPSplitterWndThemeOffice2003(); break;

		case xtpControlThemeOffice2013: pTheme = new CXTPSplitterWndThemeOffice2013(); break;

		case xtpControlThemeResource: pTheme = new CXTPSplitterWndThemeResource(); break;

		case xtpControlThemeDefault:
		default: pTheme = new CXTPSplitterWndTheme(); break;
	}
	...
	switch (eTheme)
	{
		...
		case xtpControlThemeVisualStudio2017:
			SetScrollBarTheme(xtpScrollBarThemeVisualStudio2017);
			break;

		case xtpControlThemeVisualStudio2019:
			SetScrollBarTheme(xtpScrollBarThemeVisualStudio2019);
			break;

		case xtpControlThemeVisualStudio2022:
			SetScrollBarTheme(xtpScrollBarThemeVisualStudio2022);
			break;
		...
		default: SetScrollBarTheme(xtpScrollBarThemeWindowsDefault); break;
	}
...
BOOL CXTPSplitterWnd::OnWndMsg(UINT message, WPARAM wParam, LPARAM lParam, LRESULT* pResult)
{
	switch (message)
	{
		...
		case WM_SYSCOLORCHANGE: RefreshMetrics(); break;

		case WM_PRINTCLIENT: PrintClient(CDC::FromHandle((HDC)wParam)); return TRUE;
	...
	if (pDC)
	{
		...
		if (m_bHasVScroll && m_nRows < m_nMaxRows)
		{
			OnDrawSplitter(pDC, splitBox,
						   CRect(rectInside.right, rClient.top, rClient.right,
								 rClient.top + m_cySplitter));
		}

		if (m_bHasHScroll && m_nCols < m_nMaxCols)
		{
			OnDrawSplitter(pDC, splitBox,
						   CRect(rClient.left, rectInside.bottom, rClient.left + m_cxSplitter,
								 rClient.bottom));
		}
...
CView* CXTPSplitterWnd::ReplaceView(int nRow, int nCol, CRuntimeClass* pViewClass)
{
	...
	if (!pWnd->Create(NULL, NULL, WS_CHILD | WS_VISIBLE, CRect(0, 0, 0, 0), this,
					  XTPToUInt(nCtrlID), &contextT))
	{
...
{
	if (m_htTrack == hSplitterBox || m_htTrack == vSplitterBox)
	{
...
void CXTPSplitterWndEx::LayoutRowCol(CSplitterWnd::CRowColInfo* pInfoArray, int nMax, int nSize,
									 int nSizeSplitter)
{
...
void CXTPSplitterWndEx::DeferClientPos(AFX_SIZEPARENTPARAMS* lpLayout, CWnd* pWnd, int x, int y,
									   int cx, int cy, BOOL bScrollBar)
{
	...
	if (bScrollBar)
	{
		...
		pWnd->ModifyStyle(DWORD(bNeedBorder ? 0 : WS_BORDER), DWORD(bNeedBorder ? WS_BORDER : 0));
	}
...
void CXTPSplitterWndEx::RecalcLayout()
{
	...
	if (m_bHasHScroll && m_bHasVScroll)
	{
		...
		if (pScrollBar->ModifyStyle(SBS_SIZEGRIP | SBS_SIZEBOX,
									DWORD(bSizingParent ? SBS_SIZEGRIP : SBS_SIZEBOX)))
			pScrollBar->Invalidate();
		pScrollBar->EnableWindow(bSizingParent);

		// reposition the size box
		DeferClientPos(&layout, pScrollBar, rectInside.right, rectInside.bottom, cx, cy, TRUE);
	}
