...
//---------------------------------------------------------------------------
// Summary:
//     CXTPSplitterWnd is a CSplitterWnd derived class. It adds the ability
...
public:
	...
	//-----------------------------------------------------------------------
	// Summary:
	//     Destroys a CXTPSplitterWnd object, handles cleanup and deallocation.
	//-----------------------------------------------------------------------
...
public:
	...
	BOOL SetTheme(CXTPSplitterWndTheme* pTheme); // <combine
												 // CXTPSplitterWnd::SetTheme@XTPControlTheme>

	//-----------------------------------------------------------------------
	// Summary:
	//     Call this member function to get a pointer to the currently selected theme.
	// Returns:
	//     A pointer to a CXTPSplitterWndTheme object representing the currently
	//     selected theme.
	//-----------------------------------------------------------------------
	...
	//     pNewView -    Pointer to an already existing view.
	// Returns:
	//     A CView pointer to the view that was replaced, otherwise NULL.
	// See Also:
	//     SwitchView
	//-----------------------------------------------------------------------
	...
	// Parameters:
	//     nFullDrag - 1 to enable full drag, 0 to disable, or
	//                -1 to use system settings for full drag.
	// Remarks:
	//     This member function will enable/disable the display of windows contents
	//     for each child pane while the splitter is resized if nFullDrag is set to 1.
	//-----------------------------------------------------------------------
	void EnableFullDrag(int nFullDrag);

	//-----------------------------------------------------------------------
	// Summary:
	//     Call this member function to enable dotted tracker display rather than
	//     the default.
	// Parameters:
	...
	//-----------------------------------------------------------------------
	// Summary:
	//     Call this member function to show/hide the borders for the splitter
	//     client area.
	// Parameters:
	//     bBorder - TRUE to show borders, FALSE to hide.
	//-----------------------------------------------------------------------
	void EnableBorder(BOOL bBorder);

	//-----------------------------------------------------------------------
	// Summary:
	//     Call this member function to determine the visibility of borders
	//     for the splitter client area.
	// Returns:
	//     TRUE if borders are shown, FALSE if borders are hidden.
	//-----------------------------------------------------------------------
	...
	//-----------------------------------------------------------------------
	// Summary:
	//     Call this member function to retrieve the index of the column that is
	//     currently hidden.
	// Returns:
	...
	//
	//     Specify the pane by providing either a row and column, or by providing
	//     <i>pWnd</i>.
	// Parameters:
	...
	//-----------------------------------------------------------------------
	// Summary:
	//     Call this member function to check if full window dragging is enabled.
	// Returns:
	//     TRUE if full window dragging is enabled, otherwise FALSE.
	//-----------------------------------------------------------------------
	...
	_XTP_DEPRECATED_IN_FAVOR(EnableFullDrag)
	void SetFullDrag(BOOL bFullDrag);

	_XTP_DEPRECATED_IN_FAVOR_("EnableFullDrag(), EnableDotTracker or EnableNoSize()")
	virtual void SetSplitterStyle(DWORD dwxStyle);

	_XTP_DEPRECATED virtual DWORD GetSplitterStyle();

	_XTP_DEPRECATED_IN_FAVOR(SetTheme)
...
protected:
	...
	int m_nFullDrag;  // 1 to enable full drag, 0 to disable, or -1 to use system settings for full
					  // drag.
	...
	BOOL m_bShowBorder;				// TRUE to show a border around the client area.
...
public:
	...
	//-----------------------------------------------------------------------
	// Summary:
	//     Destroys a CXTPSplitterWndEx object, handles cleanup and deallocation.
	//-----------------------------------------------------------------------
...
public:
	...
	// Parameters:
	//     bShowTopBorder -  true to show a top border for the splitter
	//                       window, similar to Outlook. By default, this
	//                       parameter is true.
	//     cyTopBorderGap -  The size, in pixels, for the top border. By
	//                       default, this parameter is 7.
	//-----------------------------------------------------------------------
...
protected:
	//-----------------------------------------------------------------------
	// Summary:
	//     Called to retrieve the client area excluding the inside 3D borders.
	// Remarks:
	//     This member function is called by the splitter window to retrieve
	//     the size of the client area excluding any 3D borders.
	// Parameters:
	//     rect -  A reference to a CRect object to receive the size of the
	//             client area excluding borders.
	//-----------------------------------------------------------------------
...
public:
	...
	//     a part of the creation process before the splitter window is visible,
	//     then it is not necessary to call this member function.<p/>
	// Remarks:
	...
	//                window is drawn.
	//     nType   -  A value of the enum ESplitType. See the remarks section for a list
	//                of values.
	...
	//     article Multiple Document Types, Views, and Frame Windows in Visual C++
