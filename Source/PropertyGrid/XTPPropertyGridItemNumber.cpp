...
CXTPPropertyGridItemNumber::CXTPPropertyGridItemNumber(LPCTSTR strCaption, long nValue,
													   long* pBindNumber)
...
CXTPPropertyGridItemDouble::CXTPPropertyGridItemDouble(LPCTSTR strCaption, double fValue,
													   LPCTSTR strFormat, double* pBindDouble)
	: CXTPPropertyGridItem(strCaption)
{
...
CXTPPropertyGridItemDouble::CXTPPropertyGridItemDouble(UINT nID, double fValue, LPCTSTR strFormat,
													   double* pBindDouble)
	: CXTPPropertyGridItem(nID)
{
	m_pBindDouble			  = pBindDouble;
...
BEGIN_DISPATCH_MAP(CXTPPropertyGridItemDouble, CXTPPropertyGridItem)
	DISP_PROPERTY_ID(CXTPPropertyGridItemDouble, "UseSystemDecimalSymbol", 50,
					 m_bUseSystemDecimalSymbol, VT_BOOL)
END_DISPATCH_MAP()
...
void CXTPPropertyGridItemDouble::OleSetValue(const VARIANT* varValue)
{
	...
	else if (varValue->vt == VT_CY)
	{
		...
		SetDouble(V_R8(varValue));
	else if (varValue->vt == VT_R4)
		SetDouble((double)V_R4(varValue));
	else if (varValue->vt == (VT_R8 | VT_BYREF))
		SetDouble(*V_R8REF(varValue));
	else if (varValue->vt == (VT_R4 | VT_BYREF))
		SetDouble((double)*V_R4REF(varValue));
	else
	{
		VARIANT var;
		::VariantInit(&var);
