...
CXTPPropertyGridItemOption::CXTPPropertyGridItemOption(LPCTSTR strCaption, int nValue,
													   int* pBindOption)
...
void CXTPPropertyGridItemOption::MeasureItem(LPMEASUREITEMSTRUCT lpMeasureItemStruct)
{
	...
	lpMeasureItemStruct->itemHeight = (lpMeasureItemStruct->itemHeight - XTP_DPI_Y(3)) * nLinesCount
									  + 4;
}
...
BOOL CXTPPropertyGridItemOption::OnDrawItemValue(CDC& dc, CRect rcValue)
{
	...
	{
		CRect rcItem(rcValue.left, rcValue.top + i * nHeight, rcValue.right,
					 rcValue.top + (i + 1) * nHeight);
		...
		if (m_bCheckBoxStyle)
		{
			pPaintManager->DrawCheckMark(&dc, rcSample, bEnabled && pConstraint->m_bEnabled,
										 m_nValue & (int)pConstraint->m_dwData);
		}
		else
		{
			pPaintManager->DrawRadioMark(&dc, rcSample, bEnabled && pConstraint->m_bEnabled,
										 m_nValue == (int)pConstraint->m_dwData);
		}
		...
		if (m_pConstraints->GetCurrent() == i && IsSelected()
			&& ::GetFocus() == m_pGrid->GetSafeHwnd())
		{
			rcText.left--;
			rcText.right = rcText.left + dc.GetTextExtent(pConstraint->m_strConstraint).cx
						   + XTP_DPI_X(2);
...
{
	if (IsAllowEdit() && m_pConstraints->GetCurrent() != -1
