...
void CXTPPropertyGridInplaceEdit::Create(CXTPPropertyGridItem* pItem, CRect rect)
{
	...
	if (m_hWnd)
	{
		DWORD dwStyle = DWORD((WS_CHILD | WS_VISIBLE | ES_READONLY)
							  | (m_pGrid->GetExStyle() & WS_EX_LAYOUTRTL ? ES_RIGHT : 0));
	...
	{
		CEdit::SetLimitText(XTPToUInt(m_pItem->GetMetrics(TRUE, FALSE)->m_nMaxLength));
	}
	else
	{
	...
	SetMargins(XTPToUIntChecked(XTP_DPI_X(3)), XTPToUIntChecked(XTP_DPI_Y(3)));
		...
		if (!pList->IsEmpty())
		{
			...
			do
			{
				...
				if (strSeach.CompareNoCase(str.Left(1)) == 0
					&& pList->GetConstraintAt(nIndex)->m_bEnabled)
				{
					SetWindowText(str);
...
void CXTPPropertyGridInplaceEdit::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	{
		m_pGrid->GetPropertyGrid()->OnNavigate(xtpPropertyGridUIInplaceEdit,
											   GetKeyState(VK_SHIFT) >= 0, m_pItem);
		return;
	}
	...
	{
		CXTPPropertyGridInplaceButton* pButton = m_pItem->GetInplaceButtons()->Find(
...
BOOL CXTPPropertyGridInplaceEdit::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt)
{
	if (m_pGrid && m_pItem != NULL && m_pItem->IsAllowEdit())
	{
		...
		CXTPPropertyGridInplaceControl* pControl = nCount > 0
													   ? m_pItem->GetInplaceControls()->GetAt(nCount
																							  - 1)
													   : NULL;
...
void CXTPPropertyGridInplaceEdit::OnDestroy()
{
	// Setting default font on closing is necessary in order to
	// prevent assertion from GDI cache on deleting a cached object.
	SetFont(CFont::FromHandle((HFONT)::GetStockObject(DEFAULT_GUI_FONT)), FALSE);

	CXTPMaskEditT<CEdit>::OnDestroy();
}
...
BOOL CXTPPropertyGridInplaceEdit::SelectConstraint(int nDirection, BOOL bCycle)
{
	CXTPPropertyGridItemConstraints* pList = m_pItem->GetConstraints();
	if (pList->IsEmpty())
		return FALSE;
	...
	if (nIndex >= pList->GetCount())
		nIndex = bCycle ? 0 : pList->GetCount() - 1;
	if (nIndex < 0)
		nIndex = bCycle ? pList->GetCount() - 1 : 0;
...
void CXTPPropertyGridInplaceControl::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	{
		m_pItem->GetGrid()->GetPropertyGrid()->OnNavigate(xtpPropertyGridUIInplaceControl,
														  GetKeyState(VK_SHIFT) >= 0, m_pItem);
		return;
	}
...
BOOL CXTPPropertyGridInplaceSlider::OnMouseWheel(UINT /*nFlags*/, short zDelta, CPoint /*pt*/)
{
	...
	OnScroll(UINT(zDelta > 0 ? SB_LEFT : SB_RIGHT), XTPToUInt(nValue));
...
void CXTPPropertyGridInplaceSlider::OnCreateWindow(CRect& rcValue)
{
	...
	if (!m_hWnd)
	{
		...
		Create(TRACKBAR_CLASS, NULL, WS_CHILD | TBS_HORZ | WS_TABSTOP, rcSlider,
			   (CWnd*)m_pItem->GetGrid(), 0);
	}
...
void CXTPPropertyGridInplaceSpinButton::OnCreateWindow(CRect& rcValue)
{
	...
	if (!m_hWnd)
	{
		...
		Create(UPDOWN_CLASS, NULL, WS_CHILD | UDS_ARROWKEYS, rcButton, (CWnd*)m_pItem->GetGrid(),
			   0);
	}
...
BEGIN_INTERFACE_MAP(CXTPPropertyGridInplaceSlider, CCmdTarget)
	INTERFACE_PART(CXTPPropertyGridInplaceSlider, XTPDIID_IPropertyGridInplaceSliderControl,
				   Dispatch)
...
BEGIN_INTERFACE_MAP(CXTPPropertyGridInplaceSpinButton, CCmdTarget)
	INTERFACE_PART(CXTPPropertyGridInplaceSpinButton, XTPDIID_IPropertyGridInplaceSpinButton,
				   Dispatch)
