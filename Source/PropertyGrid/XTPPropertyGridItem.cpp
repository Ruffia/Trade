...
CXTPPropertyGridItem::CXTPPropertyGridItem(LPCTSTR lpszCaption, LPCTSTR strValue,
										   CString* pBindString)
{
	Init();
...
void CXTPPropertyGridItem::Init()
{
	...
	m_bCategory		   = FALSE;
	m_nID			   = 0;
	m_nFlags		   = xtpPropertyGridItemHasEdit;
...
CXTPPropertyGridItemMetrics* CXTPPropertyGridItem::GetMetrics(BOOL bValueMetrics,
															  BOOL bCreate /*= TRUE*/) const
{
	if (bCreate)
...
void CXTPPropertyGridItem::SetEditText(const CString& str)
{
	...
	if (wndEdit.GetSafeHwnd() && wndEdit.m_pItem == this)
	{
		...
		wndEdit.SetMargins(XTPToUIntChecked(XTP_DPI_X(3)), XTPToUIntChecked(XTP_DPI_Y(3)));
	}
}
...
{
	if (IsVisible() && m_pGrid->GetSafeHwnd() && m_pGrid->GetStyle() & LBS_OWNERDRAWVARIABLE
		&& m_nIndex >= 0)
	{
		MEASUREITEMSTRUCT measureItemStruct = { ODT_LISTBOX, 0, static_cast<UINT>(m_nIndex),
												0,			 0, reinterpret_cast<ULONG_PTR>(this) };
...
void CXTPPropertyGridItem::OnSelect()
{
	...
	if ((m_nFlags & xtpPropertyGridItemHasEdit) && !m_pGrid->m_bTracking)
	{
		CXTPPropertyGridInplaceEdit& wndEdit = GetInplaceEdit();
...
{
	return WS_CHILD | (m_bPassword ? ES_PASSWORD : 0) | (IsMultiLine() ? ES_MULTILINE : 0)
		   | m_dwEditStyle;
}
...
void CXTPPropertyGridItem::OnInplaceButtonDown(CXTPPropertyGridInplaceButton* pButton)
{
	...
	if ((pButton->GetID() == XTP_ID_PROPERTYGRID_COMBOBUTTON) && !m_pConstraints->IsEmpty()
		&& OnRequestEdit())
	{
...
CRect CXTPPropertyGridItem::GetInplaceWindowsRect()
{
	...
	if (pPaintManager->GetItemsBorderHeight() > XTP_DPI_Y(1))
	{
		rcValue.top += pPaintManager->GetItemsBorderHeight() / 2;
		rcValue.bottom -= pPaintManager->GetItemsBorderHeight() / 2
						  + pPaintManager->GetItemsBorderHeight() % 2;
	}
...
BOOL CXTPPropertyGridItem::OnChar(UINT nChar)
{
	...
	if ((m_nFlags & xtpPropertyGridItemHasEdit) && (nChar != VK_ESCAPE))
	{
		OnSelect();
		...
		if (wndEdit.GetSafeHwnd() && wndEdit.IsWindowVisible() && wndEdit.GetItem() == this)
		{
			wndEdit.SetFocus();
			wndEdit.SetSel(0, -1);
			if ((nChar != VK_TAB) && (wndEdit.GetItem() == this))
				wndEdit.SendMessage(WM_CHAR, nChar);
		}
	...
	else if (nChar == VK_TAB)
	{
		...
		{
			m_pGrid->GetPropertyGrid()->OnNavigate(xtpPropertyGridUIInplaceEdit, TRUE, this);
		}
	}
	return FALSE;
}
...
void CXTPPropertyGridItem::OnLButtonDblClk(UINT /*nFlags*/, CPoint point)
{
	...
	else
	{
		...
		if (m_nFlags & xtpPropertyGridItemHasEdit)
		{
			CXTPPropertyGridInplaceEdit& wndEdit = GetInplaceEdit();
			if (wndEdit.GetSafeHwnd() && wndEdit.IsWindowVisible() && wndEdit.GetItem() == this)
			{
				...
				if (wndEdit.GetItem() == this && IsAllowEdit()
					&& wndEdit.SelectConstraint(+1, TRUE))
				{
					OnValidateEdit();
				}
				else
...
BOOL CXTPPropertyGridItem::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	if ((point.x >= nOffset && point.x <= XTP_DPI_X(XTP_PGI_EXPAND_BORDER) + nOffset)
		&& IsExpandable())
	{
		if (m_bExpanded)
			Collapse();
	...
	if ((m_nFlags & xtpPropertyGridItemHasEdit) && PtInValueRect(point))
	{
	...
	if (m_nFlags == xtpPropertyGridItemHasComboButton && PtInValueRect(point) && IsAllowEdit()
...
{
	if (m_pGrid)
		return *m_pGrid->m_pGrid->m_pInplaceEdit;
...
{
	if (m_pGrid)
		return *m_pGrid->m_pGrid->m_pInplaceListBox;
...
void CXTPPropertyGridItem::SetMask(LPCTSTR strMask, LPCTSTR strLiteral,
								   TCHAR chPrompt /*= _T('_')*/)
{
	m_strMask	= strMask;
...
void CXTPPropertyGridItem::MeasureItem(LPMEASUREITEMSTRUCT lpMeasureItemStruct)
{
	...
	if (m_pGrid && (GetMarkupValue() || GetMarkupCaption()))
	{
		CRect rc = GetItemRect();
		...
		if (GetMarkupValue())
		{
			XTPMarkupSetDefaultFont(XTPMarkupElementContext(GetMarkupValue()),
									(HFONT)pPaintManager->GetItemFont(this, FALSE)->GetSafeHandle(),
									pPaintManager->GetItemTextColor(this, TRUE));
	...
	else
	{
		if (m_nHeight != -1)
		{
			lpMeasureItemStruct->itemHeight = XTPToUIntChecked(m_nHeight);
		}
		else if (m_nLinesCount > 1)
		{
			lpMeasureItemStruct->itemHeight = (lpMeasureItemStruct->itemHeight - XTP_DPI_Y(4))
												  * m_nLinesCount
											  + 4;
		}
	}
	...
	if (pPaintManager->GetItemsBorderHeight() != XTP_DPI_Y(1))
		lpMeasureItemStruct->itemHeight += pPaintManager->GetItemsBorderHeight() * 2 - XTP_DPI_Y(1);
}
...
void CXTPPropertyGridItem::SetDescription(LPCTSTR lpszDescription)
{
	...
	{
		m_pMarkupDescription = XTPMarkupParseText(m_pGrid->m_pGrid->GetMarkupContext(),
												  m_strDescription);
	}

	if (IsSelected())
		GetGrid()->GetPropertyGrid()->Invalidate(FALSE);
}
...
void CXTPPropertyGridItem::OnDrawItemConstraint(CDC* pDC,
...
CSize CXTPPropertyGridItem::OnMergeItemConstraint(CDC* pDC,
												  CXTPPropertyGridItemConstraint* pConstraint)
{
...
HRESULT CXTPPropertyGridItem::GetAccessibleParent(IDispatch* FAR* ppdispParent)
{
	...
	*ppdispParent = m_pGrid->CreateAccProxy();
	return S_OK;
}
...
HRESULT CXTPPropertyGridItem::AccessibleLocation(long* pxLeft, long* pyTop, long* pcxWidth,
...
int CXTPPropertyGridItems::CompareFunc(const CXTPPropertyGridItem** ppItem1,
...
void CXTPPropertyGridItems::Sort()
{
	...
	qsort(m_arrItems.GetData(), XTPToSizeTChecked(m_arrItems.GetSize()),
		  sizeof(CXTPPropertyGridItem*), (GENERICCOMPAREFUNC)CompareFunc);
}
...
CXTPImageManagerIcon* CXTPPropertyGridItemConstraint::GetImage() const
{
	...
	return m_pItem->GetGrid()->GetImageManager()->GetImage(XTPToUInt(m_nImage), 0);
}
...
CXTPPropertyGridItemConstraint* CXTPPropertyGridItemConstraints::AddConstraint(LPCTSTR str,
																			   DWORD_PTR dwData,
																			   int nImage)
{
	CXTPPropertyGridItemConstraint* pConstraint = new CXTPPropertyGridItemConstraint();

	pConstraint->m_strConstraint = str;
	pConstraint->m_dwData		 = dwData;
	pConstraint->m_nIndex		 = (int)m_arrConstraints.Add(pConstraint);
	pConstraint->m_pItem		 = m_pItem;
	pConstraint->m_nImage		 = nImage;
...
{
	return nIndex >= 0 && nIndex < GetCount() ? m_arrConstraints.GetAt(nIndex)->m_strConstraint
											  : CString();
}
...
int CXTPPropertyGridItemConstraints::CompareFunc(
	const CXTPPropertyGridItemConstraint** ppConstraint1,
...
void CXTPPropertyGridItemConstraints::Sort()
{
	...
	qsort(m_arrConstraints.GetData(), (size_t)m_arrConstraints.GetSize(),
		  sizeof(CXTPPropertyGridItemConstraint*), (GENERICCOMPAREFUNC)CompareFunc);
}
...
BEGIN_DISPATCH_MAP(CXTPPropertyGridItemConstraint, CXTPCmdTarget)
	DISP_PROPERTY_ID(CXTPPropertyGridItemConstraint, "Caption", DISPID_VALUE, m_strConstraint,
					 VT_BSTR)
	DISP_PROPERTY_ID(CXTPPropertyGridItemConstraint, "Data", 2, m_dwData, VT_I4)
	DISP_PROPERTY_EX_ID(CXTPPropertyGridItemConstraint, "Index", 3, GetIndex, SetNotSupported,
						VT_I4)
...
BEGIN_DISPATCH_MAP(CXTPPropertyGridItemConstraints, CXTPCmdTarget)
	DISP_FUNCTION_ID(CXTPPropertyGridItemConstraints, "Count", dispidCount, OleGetItemCount, VT_I4,
					 VTS_NONE)
	DISP_FUNCTION_ID(CXTPPropertyGridItemConstraints, "Remove", dispidRemove, OleRemove, VT_EMPTY,
					 VTS_I4)
	DISP_FUNCTION_ID(CXTPPropertyGridItemConstraints, "Add", dispidAdd, OleAdd, VT_EMPTY,
					 VTS_BSTR VTS_VARIANT)
	DISP_FUNCTION_ID(CXTPPropertyGridItemConstraints, "Clear", dispidClear, OleClear, VT_EMPTY,
					 VTS_NONE)
	DISP_FUNCTION_ID(CXTPPropertyGridItemConstraints, "Item", DISPID_VALUE, OleItem, VT_DISPATCH,
					 VTS_I4)
	DISP_FUNCTION_ID(CXTPPropertyGridItemConstraints, "_NewEnum", DISPID_NEWENUM, OleNewEnum,
					 VT_UNKNOWN, VTS_NONE)
END_DISPATCH_MAP()
...
BEGIN_DISPATCH_MAP(CXTPPropertyGridItems, CXTPCmdTarget)
	...
	DISP_FUNCTION_ID(CXTPPropertyGridItems, "_NewEnum", DISPID_NEWENUM, OleNewEnum, VT_UNKNOWN,
					 VTS_NONE)
END_DISPATCH_MAP()
...
BEGIN_DISPATCH_MAP(CXTPPropertyGridItem, CXTPCmdTarget)
	...
	//}}AFX_DISPATCH_MAP
	DISP_PROPERTY_EX_ID(CXTPPropertyGridItem, "Expanded", dispidExpanded, OleGetExpanded,
						OleSetExpanded, VT_BOOL)
	DISP_PROPERTY_EX_ID(CXTPPropertyGridItem, "ReadOnly", dispidReadOnly, GetReadOnly, SetReadOnly,
						VT_BOOL)
	DISP_PROPERTY_EX_ID(CXTPPropertyGridItem, "Selected", dispidSelected, OleGetSelected,
						OleSetSelected, VT_BOOL)
	DISP_PROPERTY_EX_ID(CXTPPropertyGridItem, "Caption", dispidCaption, OleGetCaption,
						OleSetCaption, VT_BSTR)
	DISP_PROPERTY_EX_ID(CXTPPropertyGridItem, "Description", dispidDescription, OleGetDescription,
						SetDescription, VT_BSTR)
	DISP_PROPERTY_EX_ID(CXTPPropertyGridItem, "MarkupTemplate", dispidMarkupTemplate,
						OleGetMarkupTemplate, OleSetMarkupTemplate, VT_BSTR)
	...
	DISP_PROPERTY_ID(CXTPPropertyGridItem, "ConstraintEdit", dispidConstraintEdit,
					 m_bConstraintEdit, VT_BOOL)
	...
	DISP_PROPERTY_EX_ID(CXTPPropertyGridItem, "Value", DISPID_VALUE, OleGetValue, OleSetValue,
						VT_VARIANT)

	DISP_FUNCTION_ID(CXTPPropertyGridItem, "Constraints", dispidConstraints, OleGetConstraints,
					 VT_DISPATCH, VTS_NONE)
	DISP_FUNCTION_ID(CXTPPropertyGridItem, "Childs", dispidChilds, OleGetChilds, VT_DISPATCH,
					 VTS_NONE)
	DISP_FUNCTION_ID(CXTPPropertyGridItem, "Parent", dispidParent, OleGetParent, VT_DISPATCH,
					 VTS_NONE)
	DISP_FUNCTION_ID(CXTPPropertyGridItem, "AddChildItem", dispidAddChildItem, OleAddChildItem,
					 VT_DISPATCH, VTS_I4 VTS_BSTR VTS_VARIANT)
	DISP_FUNCTION_ID(CXTPPropertyGridItem, "InsertChildItem", 105, OleInsertChildItem, VT_DISPATCH,
					 VTS_I4 VTS_I4 VTS_BSTR VTS_VARIANT)

	DISP_FUNCTION_ID(CXTPPropertyGridItem, "InsertChildItemBinded", 118, OleInsertChildItemBinded,
					 VT_DISPATCH, VTS_I4 VTS_I4 VTS_DISPATCH VTS_BSTR)
	DISP_FUNCTION_ID(CXTPPropertyGridItem, "AddChildItemBinded", dispidAddChildItemBinded,
					 OleAddChildItemBinded, VT_DISPATCH, VTS_I4 VTS_DISPATCH VTS_BSTR)
	...
	DISP_FUNCTION_ID(CXTPPropertyGridItem, "SetMask", 103, OleSetMask, VT_EMPTY,
					 VTS_BSTR VTS_BSTR VTS_VARIANT)
	DISP_PROPERTY_ID(CXTPPropertyGridItem, "PasswordMask", 10, m_bPassword, VT_BOOL)
	DISP_PROPERTY_EX_ID(CXTPPropertyGridItem, "MaskedText", 104, OleGetMaskedText, SetMaskedText,
						VT_BSTR)
	...
	DISP_PROPERTY_EX_ID(CXTPPropertyGridItem, "ValueMetrics", 110, OleGetValueMetrics,
						SetNotSupported, VT_DISPATCH)
	DISP_PROPERTY_EX_ID(CXTPPropertyGridItem, "CaptionMetrics", 111, OleGetCaptionMetrics,
						SetNotSupported, VT_DISPATCH)
	...
	DISP_PROPERTY_EX_ID(CXTPPropertyGridItem, "MultiLinesCount", 15, OleGetMultiLinesCount,
						SetMultiLinesCount, VT_I4)
	...
	DISP_FUNCTION_ID(CXTPPropertyGridItem, "AddInplaceButton", 114, OleAddInplaceButton,
					 VT_DISPATCH, VTS_I4)
	DISP_FUNCTION_ID(CXTPPropertyGridItem, "AddSliderControl", 115, OleAddSliderControl,
					 VT_DISPATCH, VTS_NONE)
	DISP_FUNCTION_ID(CXTPPropertyGridItem, "AddSpinButton", 116, OleAddSpinButton, VT_DISPATCH,
					 VTS_NONE)
	DISP_FUNCTION_ID(CXTPPropertyGridItem, "InplaceButtons", 117, OleGetButtons, VT_DISPATCH,
					 VTS_NONE)
	DISP_FUNCTION_ID(CXTPPropertyGridItem, "GetRect", 119, OleGetRect, VT_EMPTY,
...
LPDISPATCH CXTPPropertyGridItem::OleAddChildItem(long nType, LPCTSTR strTitle,
...
LPDISPATCH CXTPPropertyGridItem::OleInsertChildItem(int nIndex, long nType, LPCTSTR strTitle,
													const VARIANT* varValue)
{
	...
	switch (nType)
	{
		...
		case 11: /*PropertyItemMultilineString*/
			pItem = InsertChildItem(nIndex,
									new CXTPPropertyGridItemMultilineString(strTitle, _T("")));
			break;
...
LPDISPATCH CXTPPropertyGridItem::OleAddChildItemBinded(long nType, LPDISPATCH lpHandler,
													   LPCTSTR lpzsProperty)
{
	return OleInsertChildItemBinded(m_pChilds->GetCount(), nType, lpHandler, lpzsProperty);
}

LPDISPATCH CXTPPropertyGridItem::OleInsertChildItemBinded(long nIndex, long nType,
														  LPDISPATCH lpHandler,
														  LPCTSTR lpzsProperty)
{
	if (lpHandler == NULL)
		return 0;
	...
	if (FAILED(
			lpHandler->GetIDsOfNames(IID_NULL, &szMember, 1, LOCALE_SYSTEM_DEFAULT, &dispidBinded)))
		return 0;
	...
	switch (nType)
	{
		...
		case 11: /*PropertyItemMultilineString*/
		{
			...
			pItem = InsertChildItem(nIndex, new CXTPPropertyGridItemMultilineString(lpzsProperty,
																					strValue));
		}
		break;
...
{
	if (m_pParent)
		return m_pParent->GetIDispatch(TRUE);
