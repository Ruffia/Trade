...
CXTPPropertyGridItemFlags::CXTPPropertyGridItemFlags(LPCTSTR strCaption, int nValue,
													 int* pBindFlags)
...
BOOL CXTPPropertyGridItemFlags::HasFlag(const CString& strValue, CString strFlag) const
{
	...
	do
	{
		...
		TCHAR chLast = nIndex + strFlag.GetLength() == strValue.GetLength()
						   ? _T(']')
						   : strValue[strFlag.GetLength() + nIndex];
...
void CXTPPropertyGridItemFlags::OnConstraintsChanged()
{
	...
	{
		AddChildItem(
			new CXTPPropertyGridItemFlag(pConstraints->GetAt(i),
										 XTPToDWORD(pConstraints->GetConstraintAt(i)->m_dwData)));
	}
