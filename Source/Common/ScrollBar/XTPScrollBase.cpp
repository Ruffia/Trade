...
int AFX_CDECL SBPosFromPx(XTP_SCROLLBAR_POSINFO* pSBInfo, int px)
{
	...
	if (pSBInfo->cpx)
		return (
			pSBInfo->posMin
			+ MulDiv(pSBInfo->posMax - pSBInfo->posMin - (pSBInfo->page ? pSBInfo->page - 1 : 0),
...
void CXTPScrollBase::CalcScrollBarInfo(LPRECT lprc, XTP_SCROLLBAR_POSINFO* pSBInfo, SCROLLINFO* pSI)
{
	...
	if (pPaintManager->m_cThumb > 0)
		pSBInfo->cpxThumb = pPaintManager->m_cThumb;
...
CRect CXTPScrollBase::GetScrollBarPartRect(int nPart) const
{
	...
	if (!m_spi.fSizebox)
	{
		...
		{
			case XTP_HTSCROLLUP:
				return m_spi.fVert ? CRect(rc.left, rc.top, rc.right, rc.top + m_spi.pxUpArrow)
								   : CRect(rc.left, rc.top, rc.left + m_spi.pxUpArrow, rc.bottom);
			case XTP_HTSCROLLDOWN:
				return m_spi.fVert
						   ? CRect(rc.left, rc.bottom - m_spi.pxDownArrow, rc.right, rc.bottom)
						   : CRect(rc.right - m_spi.pxDownArrow, rc.top, rc.right, rc.bottom);
			case XTP_HTSCROLLUPPAGE:
				return m_spi.fVert ? CRect(rc.left, rc.top + m_spi.pxUpArrow, rc.right,
										   rc.top + m_spi.pxThumbTop)
								   : CRect(rc.left + m_spi.pxUpArrow, rc.top,
										   rc.left + m_spi.pxThumbTop, rc.bottom);
			case XTP_HTSCROLLDOWNPAGE:
				return m_spi.fVert ? CRect(rc.left, rc.top + m_spi.pxThumbBottom, rc.right,
										   rc.top + m_spi.pxDownArrow)
								   : CRect(rc.left + m_spi.pxThumbBottom, rc.top,
										   rc.left + m_spi.pxDownArrow, rc.bottom);
			case XTP_HTSCROLLTHUMB:
				return m_spi.fVert ? CRect(rc.left, rc.top + m_spi.pxThumbTop, rc.right,
										   rc.top + m_spi.pxThumbBottom)
								   : CRect(rc.left + m_spi.pxThumbTop, rc.top,
										   rc.left + m_spi.pxThumbBottom, rc.bottom);
		}
	}
	...
	switch (message)
	{
		...
		case WM_MOUSEMOVE:
			if (fHit && fHitChanged)
			{
				...
				pSBTrack->hTimerSB = SetTimer(m_pSBTrack->hWndTrack, XTPToUInt(IDSYS_SCROLL),
											  XTPToUInt(m_nTimerElapse), NULL);
...
void CXTPScrollBase::PerformTrackInit(HWND hWnd, CPoint point, XTP_SCROLLBAR_POSINFO* pSBInfo,
									  BOOL bDirect)
{
...
HBRUSH CXTPScrollBase::GetCltColor(CDC* pDC) const
{
	...
	{
		LRESULT lResult = ::DefWindowProc(pWndParent->GetSafeHwnd(), WM_CTLCOLORSCROLLBAR,
										  WPARAM(pDC->GetSafeHdc()),
										  LPARAM(pWndParent->GetSafeHwnd()));
