...
public:
	...
	{
		return m_bUseMask && TBase::m_hWnd && ((TBase::GetStyle() & ES_READONLY) == 0)
			   && !m_strMask.IsEmpty();
	}
	...
	{
		if (!CanUseMask())
			return static_cast<BOOL>(TBase::DefWindowProc(WM_CUT, 0, 0));
	...
	{
		if (!CanUseMask())
			return static_cast<BOOL>(TBase::DefWindowProc(WM_COPY, 0, 0));
	...
	{
		if (!CanUseMask())
			return static_cast<BOOL>(TBase::DefWindowProc(WM_PASTE, 0, 0));

		GetMaskState();

		if (!TBase::OpenClipboard())
			return FALSE;
...
#	endif
	...
	{
		if (!CanUseMask())
			return static_cast<BOOL>(TBase::DefWindowProc(WM_CLEAR, 0, 0));
	...
	afx_msg BOOL MaskUndo()
	{
		if (!CanUseMask())
		{
			return static_cast<BOOL>(TBase::DefWindowProc(EM_UNDO, 0, 0));
		}
		else
		{
			MaskGetSel();
			...
			if (m_bRedo)
			{
				TBase::SetWindowText(m_strRedoBuffer);
			}
			else
			{
				TBase::GetWindowText(m_strRedoBuffer);
				TBase::SetWindowText(m_strUndoBuffer);
			}
			...
			m_nEndChar = m_nStartChar;
			TBase::SetSel(m_nStartChar, m_nEndChar);
		}
	...
	virtual BOOL SetEditMask(LPCTSTR lpszMask, LPCTSTR lpszLiteral, LPCTSTR lpszDefault = NULL)
	{
		...
		m_bRedo		= FALSE;
		m_bModified = FALSE;
		TBase::SetWindowText(m_strWindowText);
...
protected:
	...
	void GetMaskState(BOOL bCorrectSelection = TRUE)
	{
		...
		MaskGetSel();
		TBase::GetWindowText(m_strWindowText);
	...
	void MaskGetSel() // To allow CEdit and CRichEditCtrl
	{
		if (TBase::m_hWnd)
		{
			TBase::SendMessage(EM_GETSEL, (WPARAM)&m_nStartChar, (LPARAM)&m_nEndChar);
		}
	}
	...
	void SetMaskState(BOOL bUpdateUndo = TRUE)
	{
		...
		TBase::GetWindowText(strWindowText);
