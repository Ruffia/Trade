...
CLIPFORMAT CXTPImageManager::m_nImageClipFormat		 = (CLIPFORMAT)RegisterClipboardFormat(
	_T("Xtreme ToolBar Image"));
CLIPFORMAT CXTPImageManager::m_nAlphaClipFormat = (CLIPFORMAT)RegisterClipboardFormat(
	_T("Alpha Bitmap Image"));
										...
										int nHeightSrc, UINT crTransparent)
{
	...
	if (!::BitBlt(hdcTmpSrc, 0, 0, nWidthSrc, nHeightSrc, hdcSrc, nXOriginSrc, nYOriginSrc,
				  SRCCOPY))
	{
		goto ClearUp;
	}
	...
	{
		if (!::BitBlt(hdcDest, nXOriginDest, nYOriginDest, nWidthSrc, nHeightSrc, hdcMask, 0, 0,
					  SRCAND))
		{
			goto ClearUp;
		}
		if (!::BitBlt(hdcDest, nXOriginDest, nYOriginDest, nWidthSrc, nHeightSrc, hdcTmpSrc, 0, 0,
					  SRCPAINT))
		{
			goto ClearUp;
		}
	...
	{
		if (!::StretchBlt(hdcDest, nXOriginDest, nYOriginDest, nWidthDest, nHeightDest, hdcMask, 0,
						  0, nWidthSrc, nHeightSrc, SRCAND))
		{
			goto ClearUp;
		}
		if (!::StretchBlt(hdcDest, nXOriginDest, nYOriginDest, nWidthDest, nHeightDest, hdcTmpSrc,
									  ...
									  const CRect& rcSrc, UINT crTransparent) const
{
	...
	{
		return XTPDrawHelpers()->m_pfnTransparentBlt(hdcDest, rcDest.left, rcDest.top,
													 rcDest.Width(), rcDest.Height(), hdcSrc,
													 rcSrc.left, rcSrc.top, rcSrc.Width(),
													 rcSrc.Height(), crTransparent);
	}
...
// Based on http://support.microsoft.com/kb/303916/en-us
BOOL CXTPImageManager::BlendImages(HBITMAP hbmSrc1, BOOL bRTL1, HBITMAP hbmSrc2, BOOL bRTL2,
								   HBITMAP hbmDst) const
{
	...
	if (!GetObject(hbmSrc1, sizeof(BITMAP), &bmSrc1))
		return FALSE;
	if (!GetObject(hbmSrc2, sizeof(BITMAP), &bmSrc2))
		return FALSE;
	if (!GetObject(hbmDst, sizeof(BITMAP), &bmDst))
		return FALSE;
...
// Based on http://support.microsoft.com/kb/303916/en-us
HBITMAP AFX_CDECL CXTPImageManager::Create32BPPDIBSection(HDC hDC, int iWidth, int iHeight,
														  LPBYTE* lpBits)
{
	BITMAPINFO bmi;
	HBITMAP hbm;
									...
									) const
{
	...
	if (!StretchBlt(hdcSrc1, 0, 0, nWidthDest, nHeightDest, hdcSrc, nXOriginSrc, nYOriginSrc,
					nWidthSrc, nHeightSrc, SRCCOPY))
		goto HANDLEERROR;

	// Capture a copy of the destination area.
	if (!BitBlt(hdcSrc2, 0, 0, nWidthDest, nHeightDest, hdcDest, nXOriginDest, nYOriginDest,
				SRCCOPY))
		goto HANDLEERROR;
	...
	if (bReturn)
	{
		...
		BitBlt(hdcDest, nXOriginDest, nYOriginDest, nWidthDest, nHeightDest, hdcSrc1, 0, 0,
			   SRCCOPY | (bRTLDest ? NOMIRRORBITMAP : 0));
...
BOOL CXTPImageManager::AlphaBlend(HDC hdcDest, const CRect& rcDest, HDC hdcSrc,
								  const CRect& rcSrc) const
{
	...
	if (XTPDrawHelpers()->m_pfnAlphaBlend
		&& (IsWindow2000() || (rcSrc.top == 0 && rcSrc.left == 0 && rcDest.top >= 0)))
	{
		BLENDFUNCTION bf;
		...
		return XTPDrawHelpers()->m_pfnAlphaBlend(hdcDest, rcDest.left, rcDest.top, rcDest.Width(),
												 rcDest.Height(), hdcSrc, rcSrc.left, rcSrc.top,
												 rcSrc.Width(), rcSrc.Height(), bf);
	}
	return DoAlphaBlend(hdcDest, rcDest.left, rcDest.top, rcDest.Width(), rcDest.Height(), hdcSrc,
						rcSrc.left, rcSrc.top, rcSrc.Width(), rcSrc.Height());
}

BOOL CXTPImageManager::AlphaBlend2(HDC hdcDest, const CRect& rcDest, HDC hdcSrc, const CRect& rcSrc,
								   BYTE Transparency) const
{
	...
	if (XTPDrawHelpers()->m_pfnAlphaBlend
		&& (IsWindow2000() || (rcSrc.top == 0 && rcSrc.left == 0)))
	{
		BLENDFUNCTION bf;
		ZeroMemory(&bf, sizeof(BLENDFUNCTION));
...
HICON CXTPImageManagerImageList::GetIconHandle(UINT nCommand) const
{
	return ImageList_GetIcon(m_hImageList, XTPToInt(nCommand), ILD_NORMAL);
}

HBITMAP CXTPImageManagerImageList::GetBitmapHandle(UINT nCommand) const
{
	IMAGEINFO img = { 0 };
	if (!ImageList_GetImageInfo(m_hImageList, XTPToInt(nCommand), &img))
	{
		return NULL;
	}
...
void CXTPImageManagerImageList::Draw(CDC* pDC, CPoint pt, CXTPImageManagerIcon* pIcon, CSize szIcon,
									 COLORREF clrBK, COLORREF clrFG, UINT uiFlags)
{
	ImageList_DrawEx(m_hImageList, XTPToInt(pIcon->GetID() - m_nBaseCommand), pDC->GetSafeHdc(),
					 pt.x, pt.y, szIcon.cx, szIcon.cy, clrBK, clrFG,
					 ILD_TRANSPARENT | ILD_NORMAL | uiFlags);
}
...
BOOL CXTPImageManagerIcon::IsAlphaBitmapFile(LPCTSTR pszFileName)
{
	...
	if (!ReadFile(hFile, &fileHeader, sizeof(BITMAPFILEHEADER), &dwBytesRead, 0)
	...
	if (!ReadFile(hFile, &infoHeader, sizeof(BITMAPINFOHEADER), &dwBytesRead, 0)
...
BOOL CXTPImageManagerIcon::IsPngBitmapFile(LPCTSTR pszFileName)
{
	...
	if (!ReadFile(hFile, &fileHeader, sizeof(fileHeader), &dwBytesRead, 0)
		|| dwBytesRead != sizeof(fileHeader))
	{
		CloseHandle(hFile);
		return FALSE;
	}
...
HBITMAP AFX_CDECL CXTPImageManagerIcon::LoadBitmapFromFile(LPCTSTR lpszFileName,
														   BOOL* lbAlphaBitmap)
{
#ifdef _XTP_INCLUDE_GRAPHICLIBRARY
	...
	HBITMAP hBmp = (HBITMAP)LoadImage(0, lpszFileName, UINT(IMAGE_BITMAP), 0, 0,
									  UINT(LR_DEFAULTSIZE | (bAlphaBitmap ? LR_CREATEDIBSECTION : 0)
										   | LR_LOADFROMFILE));
	...
	if (OleLoadPicturePath((LPOLESTR)XTP_CT2CW(lpszFileName), NULL, 0, 0, IID_IPicture,
						   (LPVOID*)&pPict)
		== S_OK)
	{
		CDC dc;
...
HBITMAP AFX_CDECL CXTPImageManagerIcon::LoadBitmapFromResource(LPCTSTR lpszResource,
															   BOOL* lbAlphaBitmap)
{
	HMODULE hModule;
...
HBITMAP AFX_CDECL CXTPImageManagerIcon::LoadBitmapFromResource(HMODULE hModule,
...
HBITMAP CXTPImageManagerIcon::LoadAlphaIcon(LPCTSTR pszFileName, int nWidth)
{
	...
	DWORD dwBytesRead = 0;
	if (!ReadFile(hFile, &idh, sizeof(ICONDIRHEADER), &dwBytesRead, NULL)
		|| dwBytesRead != sizeof(ICONDIRHEADER))
	{
		CloseHandle(hFile);
		return FALSE;
	}
	...
	for (int i = 0; i < idh.idCount; i++)
	{
		...
		{
			if (!SetFilePointer(hFile, XTPToLongChecked(pEntries[i].dwImageOffset), NULL,
								FILE_BEGIN))
				continue;
			...
			dwBytesRead  = 0;
			BOOL bResult = ReadFile(hFile, pIconImage, pEntries[i].dwBytesInRes, &dwBytesRead,
									NULL);

			if (!bResult || (dwBytesRead != pEntries[i].dwBytesInRes)
				|| (pIconImage->icHeader.biBitCount != 32))
			{
				free(pIconImage);
				continue;
			}
			...
			pIconImage->icHeader.biSizeImage = XTPToUIntChecked(pIconImage->icHeader.biHeight
																* pIconImage->icHeader.biWidth * 4);

			BYTE* pDest  = NULL;
			HBITMAP hBmp = CreateDIBSection(dcSrc, (BITMAPINFO*)&pIconImage->icHeader,
...
BOOL CXTPImageManager::SetIconFromIcoFile(LPCTSTR pszFileName, UINT nIDCommand, CSize szIcon,
										  XTPImageState imageState)
{
	...
	DWORD dwBytesRead = 0;
	if (!ReadFile(hFile, &idh, sizeof(CXTPImageManagerIcon::ICONDIRHEADER), &dwBytesRead, NULL)
		|| dwBytesRead != sizeof(CXTPImageManagerIcon::ICONDIRHEADER))
	{
		CloseHandle(hFile);
		return FALSE;
	}
	...
	CXTPImageManagerIcon::ICONDIRENTRY* pEntries = (CXTPImageManagerIcon::ICONDIRENTRY*)malloc(
		sizeof(CXTPImageManagerIcon::ICONDIRENTRY) * idh.idCount);
	...
	if (!ReadFile(hFile, pEntries, idh.idCount * sizeof(CXTPImageManagerIcon::ICONDIRENTRY),
				  &dwBytesRead, NULL)
	...
	for (int i = 0; i < idh.idCount; i++)
	{
		...
		if ((szIcon == szResourceIcon) || (szIcon == CSize(0, 0)))
		{
			...
			CXTPImageManagerIcon::ICONIMAGE* pIconImage = (CXTPImageManagerIcon::ICONIMAGE*)malloc(
				pEntries[i].dwBytesInRes);
			if (pIconImage == NULL)
				continue;

			dwBytesRead	= 0;
			BOOL bReadFile = ReadFile(hFile, pIconImage, pEntries[i].dwBytesInRes, &dwBytesRead,
									  NULL);
			...
			if (pIconImage->icHeader.biBitCount == 32)
			{
				...
				pIconImage->icHeader.biHeight /= 2;
				pIconImage->icHeader.biSizeImage = XTPToUIntChecked(
					pIconImage->icHeader.biHeight * pIconImage->icHeader.biWidth * 4);

				BYTE* pDest		= NULL;
				HBITMAP hBitmap = CreateDIBSection(dcSrc, (BITMAPINFO*)&pIconImage->icHeader,
												   DIB_RGB_COLORS, (void**)&pDest, NULL, 0);
			...
			{
				hIcon.Attach(CreateIconFromResourceEx((PBYTE)pIconImage, pEntries[i].dwBytesInRes,
													  TRUE, 0x00030000, szResourceIcon.cx,
													  szResourceIcon.cy, LR_DEFAULTCOLOR),
							 FALSE);
			}
...
BOOL AFX_CDECL CXTPImageManagerIcon::IsAlphaIconHandle(HICON hIcon)
{
	...
	if ((CXTPImageManager::IsAlphaIconsImageListSupported()
		 || XTPSystemVersion()->IsWinXPOrGreater())
		&& GetIconInfo(hIcon, &iconInfo))
	{
		LPBYTE pBitsI   = NULL;
		BOOL bAlpha		= -1;
		HBITMAP hBitmap = CXTPImageManagerIcon::PreMultiplyAlphaBitmap(iconInfo.hbmColor, &bAlpha,
																	   &pBitsI);
	...
	: m_pResource(NULL)
{
	...
	m_pResource->cRefs					  = 1;
	m_pResource->nType					  = IMAGE_RESOURCE::MarkupType;
	...
	: m_pResource(NULL)
{
	...
	{
		pSerializationDataStream->AddRef();
	}
}
...
CXTPImageManagerVectorImageHandle::CXTPImageManagerVectorImageHandle(
	const CXTPImageManagerVectorImageHandle& rhs)
...
CXTPImageManagerVectorImageHandle& CXTPImageManagerVectorImageHandle::
	operator=(const CXTPImageManagerVectorImageHandle& rhs)
{
	Reset();
...
CXTPImageManagerVectorImageHandle::operator BOOL() const
{
	return NULL != m_pResource;
}

CXTPImageManagerVectorImageHandle CXTPImageManagerVectorImageHandle::CreateMarkupHandle(
	CXTPMarkupContext* pContext, BOOL bSharedContext, IStream* pStream)
{
	...
	return CXTPImageManagerVectorImageHandle(pContext, bSharedContext, pMarkupElement, pStream);
}
	...
	CXTPMarkupContext* pContext, BOOL bSharedContext, HGLOBAL SerializationData)
{
	...
	{
		CXTPImageManagerVectorImageHandle imgHandle = CreateMarkupHandle(pContext, bSharedContext,
																		 pStream);
		return imgHandle;
	}
...
void CXTPImageManagerVectorImageHandle::Reset()
{
	if (NULL != m_pResource)
	{
		if (0 == --m_pResource->cRefs)
		{
			if (IsMarkupImage())
			{
				XTPMarkupReleaseElement(m_pResource->u.markup.pMarkupElement);

				if (FALSE == m_pResource->u.markup.bSharedContext)
				{
					XTPMarkupReleaseContext(m_pResource->u.markup.pContext);
				}
			}

			if (NULL != m_pResource->SerializationData)
			{
				GlobalFree(m_pResource->SerializationData);
			}

			if (NULL != m_pResource->pSerializationDataStream)
			{
				m_pResource->pSerializationDataStream->Release();
			}
...
BOOL CXTPImageManagerVectorImageHandle::IsMarkupImage() const
{
	return NULL != m_pResource && IMAGE_RESOURCE::MarkupType == m_pResource->nType;
}

void CXTPImageManagerVectorImageHandle::Draw(CDC* pDC, CPoint pt, CSize szIcon)
{
	ASSERT_VALID(pDC);
	ASSERT(NULL != m_pResource);

	CRect rc;
	rc.left   = pt.x;
	rc.top	= pt.y;
	rc.right  = rc.left + szIcon.cx;
	rc.bottom = rc.top + szIcon.cy;
...
HGLOBAL CXTPImageManagerVectorImageHandle::GetSerializationData() const
{
	ASSERT(NULL != m_pResource);

	if (NULL != m_pResource->SerializationData)
		return m_pResource->SerializationData;

	if (NULL != m_pResource->pSerializationDataStream)
	{
		STATSTG sStats = { 0 };
		HRESULT hr	 = m_pResource->pSerializationDataStream->Stat(&sStats, 0);
		if (FAILED(hr))
			return NULL;

		if (0 == sStats.cbSize.LowPart || 0 != sStats.cbSize.HighPart)
			return NULL;

		LARGE_INTEGER pos;
		ZeroMemory(&pos, sizeof(pos));

		hr = m_pResource->pSerializationDataStream->Seek(pos, STREAM_SEEK_SET, 0);
		if (FAILED(hr))
			return NULL;

		HGLOBAL hGlobal = ::GlobalAlloc(GMEM_FIXED, sStats.cbSize.LowPart);
		if (NULL == hGlobal)
			return NULL;

		CHAR* szBuffer = (CHAR*)GlobalLock(hGlobal);
		if (NULL == szBuffer)
		{
			::GlobalFree(hGlobal);
			return NULL;
		...
		ULONG pcbRead = 0;
		hr = m_pResource->pSerializationDataStream->Read(szBuffer, sStats.cbSize.LowPart, &pcbRead);
		::GlobalUnlock(hGlobal);
		...
		hGlobal = m_pResource->SerializationData;
		return hGlobal;
	}
...
void CXTPImageManagerIconHandle::CopyHandle(const CXTPImageManagerIconHandle& hHandle)
{
	...
	if (m_pImageManager && !hHandle.IsEmpty() && m_pImageManager->m_bUseResources
		&& hHandle.m_bUseResources)
	{
	...
	{
		if (hHandle.m_hIcon)
			m_hIcon = CopyIcon(hHandle.m_hIcon);
		if (hHandle.m_hBitmap)
			m_hBitmap = CXTPImageManagerIcon::CopyAlphaBitmap(hHandle.m_hBitmap,
															  hHandle.m_pBits ? &m_pBits : NULL);
		if (hHandle.m_pRInfo)
		{
...
void CXTPImageManagerIconHandle::Draw(CDC* pDC, CPoint pt, CSize szIcon, BOOL bDrawComposited)
{
	...
	else if (!IsAlpha())
	{
		...
		if (!bDrawComposited)
		{
			DrawIconEx(pDC->GetSafeHdc(), pt.x, pt.y, GetIcon(), szIcon.cx, szIcon.cy, 0, 0,
					   DI_NORMAL);
		}
		else
		{
		...
		if (pt.y < 0 && (szIcon.cy == (int)m_szIcon.cy))
		{
			if (pt.y + m_szIcon.cy > 0)
			{
				CXTPImageManagerIcon::DrawAlphaBitmap(pDC, m_hBitmap, CPoint(pt.x, 0),
													  CSize(szIcon.cx, m_szIcon.cy + pt.y),
													  CPoint(0, -pt.y),
													  CSize(m_szIcon.cx, m_szIcon.cy + pt.y));
			}
		}
		else
		{
			CXTPImageManagerIcon::DrawAlphaBitmap(pDC, m_hBitmap, pt, szIcon, CPoint(0, 0),
												  m_szIcon);
		}
	}
}
...
HICON CXTPImageManagerIconHandle::GetDpiScaled(int nDpi /*= 0*/) const
{
	...
	if (NULL != m_hIcon)
	{
		...
		int nWidth   = (0 != nDpi ? XTPDpiHelper()->ScaleX(static_cast<int>(szSize.cx), nDpi)
								  : XTP_DPI_X(static_cast<int>(szSize.cx)));
...
BOOL CXTPImageManagerIconHandle::CreateIconFromResource(
	LPCTSTR lpszResourceName, CSize szIcon, BOOL bGroupResource /*= TRUE*/,
...
BOOL CXTPImageManagerIconHandle::CreateIconFromResource(
	HINSTANCE hInst, LPCTSTR lpszResourceName, CSize szIcon, BOOL bGroupResource,
	XTPIconSizeMatching nSizeMatching /*= xtpIconSizeExact*/)
{
	...
	if (hRsrc != NULL && bGroupResource)
	{
		...
		{
			case xtpIconSizeAtLeast: szMatched = CSize(INT_MAX, INT_MAX); break;
			case xtpIconSizeAtMost: szMatched = CSize(INT_MIN, INT_MIN); break;
		...
		for (int i = (int)lpGrpIconDir->idCount - 1; i >= 0; i--)
		{
			...
			switch (nSizeMatching)
			{
				case xtpIconSizeExact:
					...
					{
						if (CreateIconFromResource(hInst,
												   MAKEINTRESOURCE(lpGrpIconDir->idEntries[i].nID),
												   szIcon, FALSE))
							return TRUE;
					}
					break;
				case xtpIconSizeAtLeast:
					if (szIcon.cx <= sz.cx && szIcon.cy <= sz.cy && sz.cx < szMatched.cx
						&& sz.cy < szMatched.cy)
					{
				...
				case xtpIconSizeAtMost:
					if (sz.cx <= szIcon.cx && sz.cy <= szIcon.cy && szMatched.cx < sz.cx
						&& szMatched.cy < sz.cy)
					{
						nMatchedID   = lpGrpIconDir->idEntries[i].nID;
						szMatched	= sz;
	...
	if (!hGlobal)
		return FALSE;
	CXTPImageManagerIcon::ICONIMAGE* lpIconImage = (CXTPImageManagerIcon::ICONIMAGE*)LockResource(
		hGlobal);
	...
	if (lpIconImage->icHeader.biBitCount == 32)
	{
		...
		BITMAPINFOHEADER* pBitmapInfo = (BITMAPINFOHEADER*)malloc(sizeof(BITMAPINFOHEADER)
																  + sizeof(COLORREF) * 3);
		if (!pBitmapInfo)
			return FALSE;
		...
		*pBitmapInfo = lpIconImage->icHeader;
		...
		pBitmapInfo->biSizeImage = XTPToUIntChecked(pBitmapInfo->biHeight * pBitmapInfo->biWidth
													* 4);

		BYTE* pDest		= NULL;
		HBITMAP hBitmap = CreateDIBSection(dcSrc, (BITMAPINFO*)pBitmapInfo, DIB_RGB_COLORS,
										   (void**)&pDest, NULL, 0);
	...
	{
		Attach(CreateIconFromResourceEx((PBYTE)lpIconImage, dwResourceSize, TRUE, 0x00030000,
...
HBITMAP CXTPImageManagerIcon::PreMultiplyAlphaBitmap(HBITMAP hBitmap, BOOL* pbAlpha, LPBYTE* lpBits)
{
	...
	TRY
	{
		...
		hBitmapAlpha = CreateDIBSection(dcSrc, pBitmapInfo, DIB_RGB_COLORS, (void**)&pBitsDest,
										NULL, 0);
...
BOOL CXTPImageManagerIcon::GetBitmapBits(CDC& dcSrc, HBITMAP hBitmap, PBITMAPINFO& pBitmapInfo,
										 LPVOID& pBits, UINT& nSize)
{
	...
	if ((pBitmapInfo = (PBITMAPINFO)malloc(sizeof(BITMAPINFOHEADER) + 3 * sizeof(COLORREF)))
		== NULL)
	{
		FREE(pBits);
		return FALSE;
	...
	if (GetDIBits(dcSrc, hBitmap, 0, XTPToUIntChecked(bmi.bmiHeader.biHeight), pBits, pBitmapInfo,
				  UINT(DIB_RGB_COLORS))
		== NULL)
	{
		FREE(pBits);
...
HBITMAP CXTPImageManagerIcon::CopyAlphaBitmap(HBITMAP hBitmap, LPBYTE* lpBits)
{
	...
	BYTE* pBitsDest = NULL;
	HBITMAP hBmp	= CreateDIBSection(dcSrc, pBitmapInfo, DIB_RGB_COLORS, (void**)&pBitsDest, NULL,
									   0);
...
//////////////////////////////////////////////////////////////////////////
// CXTPImageManagerIcon
CXTPImageManagerIcon::CXTPImageManagerIcon(UINT nID, int nWidth, int nHeight,
										   CXTPImageManagerIconSet* pIconSet)
	: m_nID(nID)
	, m_nType(RasterIcon)
	, m_nWidth(nWidth)
	, m_nHeight(nHeight)
	, m_szRecommendedExtent(nWidth, nHeight)
	, m_pIconSet(pIconSet)
	, m_pImageList(NULL)
...
CXTPImageManagerIcon::CXTPImageManagerIcon(UINT nID, int nWidth, int nHeight,
										   CXTPImageManagerImageList* pImageList)
	: m_nID(nID)
	, m_nType(RasterIcon)
	, m_nWidth(nWidth)
	, m_nHeight(nHeight)
...
CXTPImageManagerIcon::CXTPImageManagerIcon(UINT nID, CXTPImageManagerIconSet* pIconSet,
										   CSize szRecommendedExtent /*= CSize(0, 0)*/)
	: m_nID(nID)
	, m_nType(UndefinedIcon)
	, m_nWidth(0)
	, m_nHeight(0)
	, m_szRecommendedExtent(szRecommendedExtent)
	, m_pIconSet(pIconSet)
	, m_pImageList(NULL)
	, m_pMarkupContext(NULL)
{
	if (pIconSet && pIconSet->GetIcons()->GetCount())
	{
		BOOL isRaster = FALSE, isVector = FALSE, isUndefined = FALSE;
		...
		while (pos)
		{
			...
			pIconSet->GetIcons()->GetNextAssoc(pos, nWidth, pIcon);
			ASSERT(pIcon);

			if (pIcon->IsUndefinedIcon())
				isUndefined = TRUE;
		...
		ASSERT(!isUndefined);
		ASSERT((isRaster && !isVector) || (isVector && !isRaster));
...
void CXTPImageManagerIcon::Clear(BOOL bIcon /* = FALSE */)
{
	...
	m_VectorImageNormal   = CXTPImageManagerVectorImageHandle();
	m_VectorImageHot	  = CXTPImageManagerVectorImageHandle();
	m_VectorImageChecked  = CXTPImageManagerVectorImageHandle();
	m_VectorImagePressed  = CXTPImageManagerVectorImageHandle();
	m_VectorImageDisabled = CXTPImageManagerVectorImageHandle();
...
CXTPImageManagerIconHandle& CXTPImageManagerIcon::GetIcon(XTPImageState imageState)
{
	...
	switch (imageState)
	{
		case xtpImageNormal: return GetIcon();
		case xtpImageDisabled: return GetDisabledIcon();
		case xtpImageHot: return GetHotIcon();
		case xtpImageChecked: return GetCheckedIcon();
		case xtpImagePressed: return GetPressedIcon();
		default: ASSERT(!"Unexpected type"); break;
	}
...
BOOL CXTPImageManagerIcon::HasIcon(XTPImageState imageState) const
{
	switch (imageState)
	{
		case xtpImageNormal: return HasNormalIcon();
		case xtpImageDisabled: return HasDisabledIcon();
		case xtpImageHot: return HasHotIcon();
		case xtpImageChecked: return HasCheckedIcon();
		case xtpImagePressed: return HasPressedIcon();
		default: ASSERT(!"Unexpected type"); return FALSE;
	}
}

BOOL CXTPImageManagerIcon::HasNormalIcon() const
{
	if (IsRasterIcon())
	{
		if (!m_hIcon.IsEmpty())
	...
	else if (IsVectorIcon())
	{
		if (m_VectorImageNormal)
			return TRUE;
	}
...
BOOL CXTPImageManagerIcon::HasCheckedIcon() const
{
	if (IsRasterIcon())
	{
		if (!m_hChecked.IsEmpty())
			return TRUE;
	}
	else if (IsVectorIcon())
	{
		if (m_VectorImageChecked)
			return TRUE;
	}
...
BOOL CXTPImageManagerIcon::HasPressedIcon() const
{
	if (IsRasterIcon())
	{
		if (!m_hPressed.IsEmpty())
			return TRUE;
	}
	else if (IsVectorIcon())
	{
		if (m_VectorImagePressed)
			return TRUE;
	}
...
BOOL CXTPImageManagerIcon::HasHotIcon() const
{
	if (IsRasterIcon())
	{
		if (!m_hHot.IsEmpty())
...
CXTPImageManagerIconHandle&
	CXTPImageManagerIcon::GetDisabledIcon(BOOL bCreateIfNotExists /*= TRUE*/)
{
	ASSERT(IsRasterIcon());
...
BOOL CXTPImageManagerIcon::SetIcon(const CXTPImageManagerIconHandle& iconHandle,
								   XTPImageState imageState)
{
	ASSERT(IsRasterIcon() || IsUndefinedIcon());
	...
	switch (imageState)
	{
		case xtpImageNormal: SetNormalIcon(iconHandle); break;
		case xtpImageHot: SetHotIcon(iconHandle); break;
		case xtpImageChecked: SetCheckedIcon(iconHandle); break;
		case xtpImageDisabled: SetDisabledIcon(iconHandle); break;
		case xtpImagePressed: SetPressedIcon(iconHandle); break;
...
CXTPMarkupContext* CXTPImageManagerIcon::CreateMarkupContext()
{
	ASSERT(IsVectorIcon() || IsUndefinedIcon());
...
CXTPMarkupContext* CXTPImageManagerIcon::GetMarkupContext()
{
	return m_pMarkupContext;
}

BOOL CXTPImageManagerIcon::SetVectorIcon(CXTPImageManagerVectorImageHandle hIcon)
{
	ASSERT(IsVectorIcon() || IsUndefinedIcon());
										 ...
										 XTPImageState imageState)
{
	...
	switch (imageState)
	{
		case xtpImageNormal: m_VectorImageNormal = hIcon; return TRUE;
		case xtpImageDisabled: m_VectorImageDisabled = hIcon; return TRUE;
		case xtpImageHot: m_VectorImageHot = hIcon; return TRUE;
		case xtpImageChecked: m_VectorImageChecked = hIcon; return TRUE;
		case xtpImagePressed: m_VectorImagePressed = hIcon; return TRUE;
		default: ASSERT(!"Unexpected type"); return FALSE;
	}
}

CXTPImageManagerVectorImageHandle CXTPImageManagerIcon::GetVectorIcon() const
{
	ASSERT(IsVectorIcon());
	...
	return m_VectorImageNormal;
}

CXTPImageManagerVectorImageHandle CXTPImageManagerIcon::GetVectorIcon(XTPImageState imageState) const
{
	ASSERT(IsVectorIcon());
	...
	switch (imageState)
	{
		case xtpImageNormal: return m_VectorImageNormal;
		case xtpImageDisabled:
			return (NULL != m_VectorImageDisabled ? m_VectorImageDisabled : m_VectorImageNormal);
		case xtpImageHot:
			return (NULL != m_VectorImageHot ? m_VectorImageHot : m_VectorImageNormal);
		case xtpImageChecked:
			return (NULL != m_VectorImageChecked ? m_VectorImageChecked : m_VectorImageNormal);
		case xtpImagePressed:
			return (NULL != m_VectorImagePressed ? m_VectorImagePressed : m_VectorImageNormal);
		default: ASSERT(!"Unexpected type"); break;
	}
...
{
	return RGB(factor * 255 + (1.0 - factor) * GetRValue(clr),
...
void CXTPImageManagerIcon::CreateShadowIcon()
{
	...
	COLORREF clrShadow	 = RGB(GetRValue(clrBackground) * .75, GetGValue(clrBackground) * .75,
...
void CXTPImageManagerIcon::CreateDisabledIcon(COLORREF clrDisabledLight /*= (COLORREF)-1*/,
				...
				for (int i = 0; i < bmp.bmWidth; i++)
					for (int j = 0; j < bmp.bmHeight; j++)
					{
						...
						if (clrMask == 0)
						{
							...
							{
								double dGray = (GetRValue(clr) * 0.299 + GetGValue(clr) * 0.587
												+ GetBValue(clr) * 0.114)
											   / 255;
								double dLight = 1.0 - dGray;

								clr = RGB(dLight * GetRValue(clrDisabledDark)
											  + dGray * GetRValue(clrDisabledLight),
										  dLight * GetGValue(clrDisabledDark)
											  + dGray * GetGValue(clrDisabledLight),
										  dLight * GetBValue(clrDisabledDark)
											  + dGray * GetBValue(clrDisabledLight));
							...
							else
							{
								...
								int nGray = (BYTE)(
									pow(dGray / 255.0,
										CXTPImageManager::m_dDisabledBrightnessFactor)
									* 255.0);
		...
		for (int i = 0; i < nCount; i++)
		{
			...
			if (nAlpha != 0)
			{
				...
				if (bOfficeDisabledIcons)
				{
					...
					R = BYTE(dLight * GetBValue(clrDisabledDark)
							 + dGray * GetBValue(clrDisabledLight));
					G = BYTE(dLight * GetGValue(clrDisabledDark)
							 + dGray * GetGValue(clrDisabledLight));
					B = BYTE(dLight * GetRValue(clrDisabledDark)
							 + dGray * GetRValue(clrDisabledLight));
				}
				else
				{
					...
					R = G = B = (BYTE)(
						pow(dGray / 255.0, CXTPImageManager::m_dDisabledBrightnessFactor) * 255.0);
...
BOOL CXTPImageManagerIcon::GetDIBBitmap(HBITMAP hBitmap, PBYTE& pBits, UINT& nBitsSize,
										PBITMAPINFO& pBitmapInfo, UINT& nBitmapInfoSize)
{
	ASSERT(IsRasterIcon());
	...
	TRY
	{
		...
		nBitsSize = bmi.bmiHeader.biSizeImage != 0
		...
		int nColorTableSize = bmi.bmiHeader.biBitCount == 4
								  ? 16
								  : bmi.bmiHeader.biBitCount == 8 ? 256 : 3;
		...
		if (GetDIBits(dcSrc, hBitmap, 0, XTPToUIntChecked(bmi.bmiHeader.biHeight), pBits,
					  pBitmapInfo, UINT(DIB_RGB_COLORS))
			== NULL)
			AfxThrowMemoryException();
	}
	CATCH(CMemoryException, e)
	...
	else
	{
		...
		if (hbmColor)
		{
			...
			else
			{
				...
				il.Add(CBitmap::FromHandle(hbmColor),
					   hbmMask == 0 ? (CBitmap*)NULL : CBitmap::FromHandle(hbmMask));
...
void CXTPImageManagerIcon::SerializeIcon(CXTPImageManagerVectorImageHandle& hIcon, CArchive& ar,
										 long nSchema)
{
	UNREFERENCED_PARAMETER(nSchema);

	if (ar.IsStoring())
	{
		if (hIcon && hIcon.IsMarkupImage())
		{
			HGLOBAL hGlobal = hIcon.GetSerializationData();
			if (hGlobal)
			{
				...
				if (pBuff)
				{
					...
					ar.WriteCount(sz);
					ar.Write(pBuff, XTPToUIntChecked(sz));
					GlobalUnlock(hGlobal);
				}
				else
				{
	...
	{
		UINT sz = XTPToUIntChecked(ar.ReadCount());
		if (sz)
		{
			...
			if (hGlobal)
			{
				...
				if (pBuff)
				{
					...
					hIcon = CXTPImageManagerVectorImageHandle::CreateMarkupHandle(
						CreateMarkupContext(), TRUE, hGlobal);
					return;
				}
			...
			BYTE* pBuff = new BYTE[XTPToUIntChecked(sz)];
			ar.Read(pBuff, XTPToUIntChecked(sz));
			SAFE_DELETE(pBuff);
		}

		hIcon = CXTPImageManagerVectorImageHandle();
	}
}

void CXTPImageManagerIcon::Serialize(CArchive& ar)
{
	ASSERT(!IsUndefinedIcon());

	long nSchema	   = _XTP_SCHEMA_CURRENT;
	IconType nFileType = UndefinedIcon;
	...
	else
	{
		...
		if (nSchema > _XTP_SCHEMA_1700 && HIWORD(m_nHeight) == 0xFFFF
			&& LOWORD(m_nHeight) == VectorIcon)
		{
			nFileType = VectorIcon;
			m_nHeight = 0;
		}
		else
		{
			nFileType = RasterIcon;
		}
	}

	if (ar.IsLoading() && nFileType != m_nType)
	{
		Clear();

		if (nFileType == VectorIcon)
			m_nHeight = m_nWidth = 0;

		if (nFileType == RasterIcon)
			m_nWidth = m_nHeight;
	...
	{
		if (nSchema > _XTP_SCHEMA_1700) // _XTP_SCHEMA_1900+
		{
			SerializeIcon(m_VectorImageNormal, ar, nSchema);
			SerializeIcon(m_VectorImageHot, ar, nSchema);
			SerializeIcon(m_VectorImageChecked, ar, nSchema);
			SerializeIcon(m_VectorImagePressed, ar, nSchema);
			SerializeIcon(m_VectorImageDisabled, ar, nSchema);
		}
	}
	else
	{
		ASSERT(FALSE); // UndefinedIcon
	}
}
...
{
	ASSERT_VALID(pIcon);
	ASSERT(IsRasterIcon() == pIcon->IsRasterIcon());
	ASSERT(IsVectorIcon() == pIcon->IsVectorIcon());
	...
	if (this != pIcon)
	{
		...
		if (IsVectorIcon())
		{
			m_pMarkupContext = CreateMarkupContext();
			SetVectorIcon(pIcon->GetVectorIcon());
			SetVectorDisabledIcon(pIcon->GetVectorDisabledIcon());
			SetVectorHotIcon(pIcon->GetVectorHotIcon());
			SetVectorCheckedIcon(pIcon->GetVectorCheckedIcon());
			SetVectorPressedIcon(pIcon->GetVectorPressedIcon());
		}
	}
}

void CXTPImageManagerIcon::DrawAlphaBitmap(CDC* pDC, HBITMAP hBitmap, CPoint ptDest, CSize szDest,
										   CPoint ptSrc, CSize szSrc)
{
	if (szSrc == CSize(0))
		szSrc = szDest;
	...
	XTPImageManager()->AlphaBlend(pDC->GetSafeHdc(), CRect(ptDest, szDest), dc,
								  CRect(ptSrc, szSrc));
}
...
void CXTPImageManagerIcon::DrawMono(CDC* pDC, CPoint pt, CXTPImageManagerIconHandle& hIcon,
									CSize szIcon, COLORREF clrBrush)
{
	ASSERT(IsRasterIcon());
...
{
	Draw(pDC, pt, xtpImageNormal, szIcon);
}

void CXTPImageManagerIcon::Draw(CDC* pDC, CPoint pt)
{
	Draw(pDC, pt, xtpImageNormal);
}

void CXTPImageManagerIcon::Draw(CDC* pDC, CPoint pt, XTPImageState imageState)
{
	ASSERT(!IsUndefinedIcon());

	if (IsRasterIcon())
	{
		Draw(pDC, pt, GetIcon(imageState));
	}
	else if (IsVectorIcon())
	{
		Draw(pDC, pt, GetVectorIcon(imageState));
	}
}

void CXTPImageManagerIcon::Draw(CDC* pDC, CPoint pt, XTPImageState imageState, CSize szIcon)
{
	ASSERT(!IsUndefinedIcon());

	if (IsRasterIcon())
	{
		Draw(pDC, pt, GetIcon(imageState), szIcon);
	}
	else if (IsVectorIcon())
	{
		Draw(pDC, pt, GetVectorIcon(imageState), szIcon);
	}
}
...
void CXTPImageManagerIcon::Draw(CDC* pDC, CPoint pt, CXTPImageManagerIconHandle& hIcon,
								CSize szIcon /*= 0*/, COLORREF clrBK, COLORREF clrFG, UINT uiFlags)
{
	ASSERT(IsRasterIcon());
...
void CXTPImageManagerIcon::Draw(CDC* pDC, CPoint pt, CXTPImageManagerVectorImageHandle hIcon,
								CSize szIcon /*= 0*/)
{
	ASSERT_VALID(pDC);
	ASSERT(hIcon);
...
CXTPImageManagerIcon* CXTPImageManagerIconSet::CreateVectorIcon(UINT nWidth)
{
	CXTPImageManagerIcon* pIcon = 0;
	if (m_mapIcons.Lookup(nWidth, pIcon))
	{
	...
	{
		pIcon = new CXTPImageManagerIcon(m_nID, this, CSize(XTPToIntChecked(nWidth), 0));
		m_mapIcons.SetAt(nWidth, pIcon);
	}
...
BOOL CXTPImageManagerIconSet::SetVectorIcon(IStream* pStream, UINT nWidth)
{
	ASSERT(pStream);

	CXTPImageManagerIcon* pIcon = CreateVectorIcon(nWidth);
	if (!pIcon)
		return FALSE;

	CXTPMarkupContext* pMarkupContext = pIcon->GetMarkupContext();
	if (!pMarkupContext)
		pMarkupContext = pIcon->CreateMarkupContext();

	CXTPImageManagerVectorImageHandle hVectorIcon =
		CXTPImageManagerVectorImageHandle::CreateMarkupHandle(pMarkupContext, TRUE, pStream);
	if (!hVectorIcon)
		return FALSE;

	return pIcon->SetVectorIcon(hVectorIcon);
}

BOOL CXTPImageManagerIconSet::SetVectorDisabledIcon(IStream* pStream, UINT nWidth)
{
	ASSERT(pStream);
	...
	return pIcon->SetVectorDisabledIcon(hVectorIcon);
}

BOOL CXTPImageManagerIconSet::SetVectorHotIcon(IStream* pStream, UINT nWidth)
{
	ASSERT(pStream);
	...
	return pIcon->SetVectorHotIcon(hVectorIcon);
}

BOOL CXTPImageManagerIconSet::SetVectorCheckedIcon(IStream* pStream, UINT nWidth)
{
	ASSERT(pStream);
	...
	return pIcon->SetVectorCheckedIcon(hVectorIcon);
}

BOOL CXTPImageManagerIconSet::SetVectorPressedIcon(IStream* pStream, UINT nWidth)
{
	ASSERT(pStream);
	...
	return pIcon->SetVectorPressedIcon(hVectorIcon);
}

BOOL CXTPImageManagerIconSet::SetVectorIcon(CXTPImageManagerVectorImageHandle hIcon, UINT nWidth)
{
	if (!hIcon)
		return FALSE;
	...
	return pIcon->SetVectorIcon(hIcon);
}

BOOL CXTPImageManagerIconSet::SetVectorDisabledIcon(CXTPImageManagerVectorImageHandle hIcon,
													UINT nWidth)
{
	if (!hIcon)
		return FALSE;
	...
	return pIcon->SetVectorDisabledIcon(hIcon);
}

BOOL CXTPImageManagerIconSet::SetVectorHotIcon(CXTPImageManagerVectorImageHandle hIcon, UINT nWidth)
{
	if (!hIcon)
		return FALSE;
	...
	return pIcon->SetVectorHotIcon(hIcon);
}

BOOL CXTPImageManagerIconSet::SetVectorCheckedIcon(CXTPImageManagerVectorImageHandle hIcon,
												   UINT nWidth)
{
	if (!hIcon)
		return FALSE;
	...
	return pIcon->SetVectorCheckedIcon(hIcon);
}

BOOL CXTPImageManagerIconSet::SetVectorPressedIcon(CXTPImageManagerVectorImageHandle hIcon,
												   UINT nWidth)
{
	if (!hIcon)
		return FALSE;
	...
	return pIcon->SetVectorPressedIcon(hIcon);
}
...
CXTPImageManagerIcon* CXTPImageManagerIconSet::GetIcon(UINT nWidth, BOOL bScaled /* = TRUE*/)
{
	...
	while (pos)
	{
		...
		{
			if (((nDelta == 0)
				 || ((nWidth == ICON_SMALL && nWidthScale < nDelta)
...
{
	return &CXTPSingleton<CXTPImageManager,
						  CXTPSingletonDependencies<CXTPSingleton<CXTPGdiPlus> > >::Instance();
}
...
BOOL CXTPImageManager::LoadToolbar(UINT nIDResourceToolBar, UINT*& pItems, int& nCount,
								   CSize& szIcon) const
{
...
BOOL CXTPImageManager::SetIcons(UINT nIDResourceToolBar, CBitmap& bmpIcons,
								XTPImageState imageState)
{
	UINT* pItems = NULL;
	CSize szIcon(0);
...
BOOL CXTPImageManager::SetIcons(UINT nIDResourceToolBar, UINT nIDResourceBitmap,
								XTPImageState imageState)
{
	UINT* pItems = NULL;
	CSize szIcon(0);
...
BOOL CXTPImageManager::SetIcons(UINT nIDResourceToolBar, CImageList& imlIcons,
								XTPImageState imageState)
{
	UINT* pItems = NULL;
	CSize szIcon(0);
...
BOOL CXTPImageManager::SetIcon(const CXTPImageManagerIconHandle& hIcon, UINT nIDCommand,
							   CSize szIcon, XTPImageState imageState)
{
	...
	if (m_mapImages.Lookup(nIDCommand, pIconSet))
	{
		switch (imageState)
		{
			case xtpImageNormal: pIconSet->SetIcon(hIcon, XTPToUIntChecked(szIcon.cx)); break;
			case xtpImageDisabled:
				pIconSet->SetDisabledIcon(hIcon, XTPToUIntChecked(szIcon.cx));
				break;
			case xtpImageHot: pIconSet->SetHotIcon(hIcon, XTPToUIntChecked(szIcon.cx)); break;
			case xtpImageChecked:
...
BOOL CXTPImageManager::SetIcon(UINT nIDResourceIcon, UINT nIDCommand, CSize szIcon,
							   XTPImageState imageState)
{
	return SetIcon(MAKEINTRESOURCE(nIDResourceIcon), nIDCommand, szIcon, imageState);
}

BOOL CXTPImageManager::SetIconFromResource(HINSTANCE hInst, LPCTSTR lpszResourceName,
										   UINT nIDCommand, CSize szIcon, XTPImageState imageState)
{
	// determine location of the bitmap in resource fork
	HRSRC hRsrc = ::FindResource(hInst, lpszResourceName, RT_GROUP_ICON);
	if (hRsrc != NULL)
	{
		...
		{
			CXTPImageManagerIconHandle hIcon;
			CSize szResourceIcon(lpGrpIconDir->idEntries[i].bWidth,
								 lpGrpIconDir->idEntries[i].bHeight);

			if ((szIcon == szResourceIcon) || (szIcon == CSize(0, 0)))
			{
				if (!hIcon.CreateIconFromResource(hInst,
												  MAKEINTRESOURCE(lpGrpIconDir->idEntries[i].nID),
...
BOOL CXTPImageManager::SetIcon(LPCTSTR lpszResourceName, UINT nIDCommand, CSize szIcon,
							   XTPImageState imageState)
{
	// determine location of the bitmap in resource fork
	HINSTANCE hInst = AfxFindResourceHandle(lpszResourceName, RT_GROUP_ICON);
...
BOOL CXTPImageManager::SetIcons(CImageList& imlIcons, UINT* pCommands, int nCount, CSize szIcon,
								XTPImageState imageState)
{
	...
	for (int i = 0; i < nCount; i++)
	{
		if ((pCommands == NULL) || bIncludeAll
			|| (pCommands[i] != 0 && pCommands[i] != IMAGE_PLACEHOLDER))
		{
...
BOOL CXTPImageManager::SplitBitmap(HBITMAP hbmSource, int nCount, HBITMAP* pDest) const
{
	...
	TRY
	{
		...
		for (i = 0; i < nCount; i++)
		{
			pDest[i] = CreateDIBSection(dcSrc, pBitmapInfo, DIB_RGB_COLORS, (void**)&pDestBits[i],
										NULL, 0);
			if (!pDest[i])
			{
				for (int j = 0; j < i; j++)
...
BOOL CXTPImageManager::ResourceToStream(HMODULE hModule, LPCTSTR lpResource, LPCTSTR lpResourceType,
										IStream** ppStream)
{
	return S_OK
		   == XTPCreateReadOnlyResourceStream(hModule, lpResource, lpResourceType, FALSE, ppStream);
}

BOOL CXTPImageManager::FileToStream(LPCTSTR path, IStream** ppStream)
{
	return S_OK == XTPCreateReadOnlyFileStream(path, ppStream);
}

BOOL CXTPImageManager::SetVectorIcon(IStream* pStream, UINT nIDCommand, UINT nWidth,
									 XTPImageState imageState)
{
	ASSERT(pStream);
	...
	if (m_mapImages.Lookup(nIDCommand, pIconSet))
	{
		switch (imageState)
		{
			case xtpImageNormal: bRes = pIconSet->SetVectorIcon(pStream, nWidth); break;
			case xtpImageDisabled: bRes = pIconSet->SetVectorDisabledIcon(pStream, nWidth); break;
			case xtpImageHot: bRes = pIconSet->SetVectorHotIcon(pStream, nWidth); break;
			case xtpImageChecked: bRes = pIconSet->SetVectorCheckedIcon(pStream, nWidth); break;
			case xtpImagePressed: bRes = pIconSet->SetVectorPressedIcon(pStream, nWidth); break;
	...
	else if (imageState == xtpImageNormal)
	{
		...
		bRes = pIconSet->SetVectorIcon(pStream, nWidth);
		if (!bRes)
		{
			SAFE_DELETE(pIconSet);
			return bRes;
		}
...
BOOL CXTPImageManager::SetVectorIcon(LPCTSTR lpResourceType, UINT nIDResource, UINT nIDCommand,
									 UINT nWidth, XTPImageState imageState)
{
	return SetVectorIcon(NULL, lpResourceType, MAKEINTRESOURCE(nIDResource), nIDCommand, nWidth,
						 imageState);
}

BOOL CXTPImageManager::SetVectorIcon(LPCTSTR lpResourceType, LPCTSTR lpResource, UINT nIDCommand,
									 UINT nWidth, XTPImageState imageState)
{
	return SetVectorIcon(NULL, lpResourceType, lpResource, nIDCommand, nWidth, imageState);
}

BOOL CXTPImageManager::SetVectorIcon(HMODULE hModule, LPCTSTR lpResourceType, UINT nIDResource,
									 UINT nIDCommand, UINT nWidth, XTPImageState imageState)
{
	return SetVectorIcon(hModule, lpResourceType, MAKEINTRESOURCE(nIDResource), nIDCommand, nWidth,
						 imageState);
}

BOOL CXTPImageManager::SetVectorIcon(HMODULE hModule, LPCTSTR lpResourceType, LPCTSTR lpResource,
									 UINT nIDCommand, UINT nWidth, XTPImageState imageState)
{
	IStreamPtr pStream;
	BOOL bRes = ResourceToStream(hModule, lpResource, lpResourceType, &pStream);
	if (!bRes)
		return bRes;

	return SetVectorIcon(pStream, nIDCommand, nWidth, imageState);
}

BOOL CXTPImageManager::SetVectorIcon(CXTPImageManagerVectorImageHandle hImage, UINT nIDCommand,
									 UINT nWidth, XTPImageState imageState)
{
	if (!hImage)
		return FALSE;
	...
	if (m_mapImages.Lookup(nIDCommand, pIconSet))
	{
		switch (imageState)
		{
			case xtpImageNormal: bRes = pIconSet->SetVectorIcon(hImage, nWidth); break;
			case xtpImageDisabled: bRes = pIconSet->SetVectorDisabledIcon(hImage, nWidth); break;
			case xtpImageHot: bRes = pIconSet->SetVectorHotIcon(hImage, nWidth); break;
			case xtpImageChecked: bRes = pIconSet->SetVectorCheckedIcon(hImage, nWidth); break;
			case xtpImagePressed: bRes = pIconSet->SetVectorPressedIcon(hImage, nWidth); break;
			default: ASSERT(FALSE); break;
		}
	}
	else if (imageState == xtpImageNormal)
	{
		...
		bRes = pIconSet->SetVectorIcon(hImage, nWidth);
		if (!bRes)
		{
			SAFE_DELETE(pIconSet);
			return FALSE;
...
BOOL CXTPImageManager::SetVectorIcon(LPCTSTR lpResourceType, UINT nIDResourceNormal,
									 UINT nIDResourceDisabled, UINT nIDResourceHot,
									 UINT nIDResourceChecked, UINT nIDResourcePressed,
									 UINT nIDCommand, UINT nWidth)
{
	return SetVectorIcon(NULL, lpResourceType, MAKEINTRESOURCE(nIDResourceNormal),
						 MAKEINTRESOURCE(nIDResourceDisabled), MAKEINTRESOURCE(nIDResourceHot),
						 MAKEINTRESOURCE(nIDResourceChecked), MAKEINTRESOURCE(nIDResourcePressed),
						 nIDCommand, nWidth);
}

BOOL CXTPImageManager::SetVectorIcon(LPCTSTR lpResourceType, LPCTSTR lpIDResourceNormal,
									 LPCTSTR lpIDResourceDisabled, LPCTSTR lpIDResourceHot,
									 LPCTSTR lpIDResourceChecked, LPCTSTR lpIDResourcePressed,
									 UINT nIDCommand, UINT nWidth)
{
	return (
		SetVectorIcon(NULL, lpResourceType, lpIDResourceNormal, nIDCommand, nWidth, xtpImageNormal)
		&& SetVectorIcon(NULL, lpResourceType, lpIDResourceDisabled, nIDCommand, nWidth,
						 xtpImageDisabled)
		&& SetVectorIcon(NULL, lpResourceType, lpIDResourceHot, nIDCommand, nWidth, xtpImageHot)
		&& SetVectorIcon(NULL, lpResourceType, lpIDResourceChecked, nIDCommand, nWidth,
						 xtpImageChecked)
		&& SetVectorIcon(NULL, lpResourceType, lpIDResourcePressed, nIDCommand, nWidth,
						 xtpImagePressed));
}

BOOL CXTPImageManager::SetVectorIcon(HMODULE hModule, LPCTSTR lpResourceType,
									 UINT nIDResourceNormal, UINT nIDResourceDisabled,
									 UINT nIDResourceHot, UINT nIDResourceChecked,
									 UINT nIDResourcePressed, UINT nIDCommand, UINT nWidth)
{
	return SetVectorIcon(hModule, lpResourceType, MAKEINTRESOURCE(nIDResourceNormal),
						 MAKEINTRESOURCE(nIDResourceDisabled), MAKEINTRESOURCE(nIDResourceHot),
						 MAKEINTRESOURCE(nIDResourceChecked), MAKEINTRESOURCE(nIDResourcePressed),
						 nIDCommand, nWidth);
}
...
BOOL CXTPImageManager::SetVectorIcon(HMODULE hModule, LPCTSTR lpResourceType,
									 LPCTSTR lpIDResourceNormal, LPCTSTR lpIDResourceDisabled,
									 LPCTSTR lpIDResourceHot, LPCTSTR lpIDResourceChecked,
									 LPCTSTR lpIDResourcePressed, UINT nIDCommand, UINT nWidth)
{
	return (
		SetVectorIcon(hModule, lpResourceType, lpIDResourceNormal, nIDCommand, nWidth,
					  xtpImageNormal)
		&& SetVectorIcon(hModule, lpResourceType, lpIDResourceDisabled, nIDCommand, nWidth,
						 xtpImageDisabled)
		&& SetVectorIcon(hModule, lpResourceType, lpIDResourceHot, nIDCommand, nWidth, xtpImageHot)
		&& SetVectorIcon(hModule, lpResourceType, lpIDResourceChecked, nIDCommand, nWidth,
						 xtpImageChecked)
		&& SetVectorIcon(hModule, lpResourceType, lpIDResourcePressed, nIDCommand, nWidth,
						 xtpImagePressed));
}

BOOL CXTPImageManager::SetVectorIcon(CXTPImageManagerVectorImageHandle hImageNormal,
									 ...
									 CXTPImageManagerVectorImageHandle hImagePressed,
									 UINT nIDCommand, UINT nWidth)
{
	return (SetVectorIcon(hImageNormal, nIDCommand, nWidth, xtpImageNormal)
			&& SetVectorIcon(hImageDisabled, nIDCommand, nWidth, xtpImageDisabled)
			&& SetVectorIcon(hImageHot, nIDCommand, nWidth, xtpImageHot)
			&& SetVectorIcon(hImageChecked, nIDCommand, nWidth, xtpImageChecked)
			&& SetVectorIcon(hImagePressed, nIDCommand, nWidth, xtpImagePressed));
}
...
HBITMAP CXTPImageManager::ResampleAlphaLayer(HBITMAP bmpAlpha, COLORREF clrMask)
{
	...
	HBITMAP hBitmapResult = CreateDIBSection(dcSrc, pBitmapInfo, DIB_RGB_COLORS, (void**)&pTarget,
											 NULL, 0);
...
BOOL CXTPImageManager::SetIcons(CBitmap& bmpIcons, UINT* pCommands, int nCount, CSize szIcon,
								XTPImageState imageState, BOOL bAlpha)
{
	if (szIcon == CSize(0) && nCount > 0)
	{
		BITMAP bmpInfo;
	...
	{
		HBITMAP hBitmap = ResampleAlphaLayer(bmpIcons, m_clrMask == COLORREF(-1) ? RGB(0, 0xFF, 0)
																				 : m_clrMask);
		if (!hBitmap)
			return FALSE;
...
BOOL CXTPImageManager::SetIcons(UINT nIDResourceBitmap, UINT* pCommands, int nCount, CSize szIcon,
								XTPImageState imageState)
{
	return SetIcons(MAKEINTRESOURCE(nIDResourceBitmap), pCommands, nCount, szIcon, imageState);
}

BOOL CXTPImageManager::SetIcons(LPCTSTR lpszResourceBitmap, UINT* pCommands, int nCount,
								CSize szIcon, XTPImageState imageState)
{
...
BOOL CXTPImageManager::SetImageList(HIMAGELIST hImageList, int nBaseCommand, BOOL bDestoryImageList)
{
	...
	pImageList->m_nBaseCommand = XTPToUInt(nBaseCommand);
...
{
	BOOL bAlpha		= FALSE;
	HBITMAP hBitmap = CXTPImageManagerIcon::LoadBitmapFromResource(MAKEINTRESOURCE(nBitmap),
																   &bAlpha);
	if (!hBitmap)
		return FALSE;
	...
	HIMAGELIST hImageList = ImageList_Create(cx, bmpInfo.bmHeight,
											 UINT((bAlpha ? ILC_COLOR32 : ILC_COLOR24)
												  | (clrMask == COLORREF_NULL ? 0 : ILC_MASK)),
											 0, 1);
...
BOOL CXTPImageManager::IsPngBitmapResource(UINT nIDResourceBitmap)
{
	return CXTPImageManagerIcon::IsPngBitmapResource(
		AfxFindResourceHandle(MAKEINTRESOURCE(nIDResourceBitmap), _T("PNG")),
		MAKEINTRESOURCE(nIDResourceBitmap));
}

BOOL CXTPImageManager::IsAlphaBitmapResource(UINT nIDResourceBitmap)
{
	return CXTPImageManagerIcon::IsAlphaBitmapResource(
		AfxFindResourceHandle(MAKEINTRESOURCE(nIDResourceBitmap), RT_BITMAP),
		MAKEINTRESOURCE(nIDResourceBitmap));
}
...
BOOL CXTPImageManager::IsPrimaryImageExists(UINT nCommand, int nWidth) const
{
	...
	if (pIconSet)
	{
		...
		pIcon = pIconSet->GetIcon(0, FALSE); // Attempt to get XAML icon
...
void CXTPImageManager::AddIcons(CXTPImageManagerIconSet* pIconSetAdd)
{
	...
	UINT nWidth					   = 0;
	CXTPImageManagerIcon* pIconAdd = NULL;
...
BOOL CXTPImageManager::DoDisableBitmap(HBITMAP hBmp, COLORREF clrDisabledLight,
									   COLORREF clrDisabledDark, int nBlackAndWhiteContrast)
{
	...
	for (y = 0; y < bmBitmap.bmHeight; y++)
	{
		for (x = 0; x < bmBitmap.bmWidth; x++)
		{
			if (bOfficeDisabledIcons)
			{
				double dGray = (lprgbBitmap[x].rgbBlue * 0.114 + lprgbBitmap[x].rgbGreen * 0.587
								+ lprgbBitmap[x].rgbRed * 0.299)
							   / 255.0;
				double dLight = 1.0 - dGray;

				lprgbBitmap[x].rgbBlue  = BYTE(dLight * GetBValue(clrDisabledDark)
											   + dGray * GetBValue(clrDisabledLight));
				lprgbBitmap[x].rgbGreen = BYTE(dLight * GetGValue(clrDisabledDark)
											   + dGray * GetGValue(clrDisabledLight));
				lprgbBitmap[x].rgbRed   = BYTE(dLight * GetRValue(clrDisabledDark)
											   + dGray * GetRValue(clrDisabledLight));
			}
			else if (nBlackAndWhiteContrast != -1)
			{
				int nGray = (lprgbBitmap[x].rgbBlue * 114 + lprgbBitmap[x].rgbGreen * 587
				...
				lprgbBitmap[x].rgbReserved = BYTE(lprgbBitmap[x].rgbReserved
												  / CXTPImageManager::m_dDisabledAlphaFactor);
			}
			else
			{
				double dGray = (lprgbBitmap[x].rgbBlue * 0.114 + lprgbBitmap[x].rgbGreen * 0.587
				...
				lprgbBitmap[x].rgbBlue = (BYTE)(
					pow(dGray, CXTPImageManager::m_dDisabledBrightnessFactor) * 255.0);
...
BOOL CXTPImageManager::DisableBitmap(HDC hDC, const CRect& rcRect, COLORREF clrDisabledLight,
									 COLORREF clrDisabledDark)
{
	HDC hdcDst		= NULL;
	HBITMAP hbmDst  = NULL;
	...
	// Capture a copy of the source area.
	if (!BitBlt(hdcDst, 0, 0, rcRect.Width(), rcRect.Height(), hDC, rcRect.left, rcRect.top,
				SRCCOPY))
		goto HANDLEERROR;
	...
	if (bReturn)
	{
		BitBlt(hDC, rcRect.left, rcRect.top, rcRect.Width(), rcRect.Height(), hdcDst, 0, 0,
			   SRCCOPY);
	}
...
CXTPImageManager* AFX_CDECL CXTPImageManager::FromDispatch(LPDISPATCH pDisp)
{
	...
	if (NULL != pDisp)
	{
		pInst = XTP_DYNAMIC_DOWNCAST_REMOTE_(CXTPImageManager,
											 CXTPImageManager::FromIDispatchSafe(pDisp));
		if (NULL == pInst)
		{
...
BEGIN_DISPATCH_MAP(CXTPImageManagerIcon, CXTPCmdTarget)
	...
	DISP_FUNCTION_ID(CXTPImageManagerIcon, "Draw", 7, OleDraw, VT_EMPTY,
					 VTS_I4 VTS_I4 VTS_I4 VTS_I4 VTS_I4 VTS_I4)
	DISP_FUNCTION_ID(CXTPImageManagerIcon, "CreatePicture", 8, OleCreatePicture, VT_DISPATCH,
					 VTS_I4)
...
long CXTPImageManagerIcon::OleGetHandle()
{
	...
	if (m_hIcon.m_pRInfo && m_hIcon.m_pRInfo->pResource && m_hIcon.m_pRInfo->pResource->m_hbmpImage)
		return (long)reinterpret_cast<LONG_PTR>(m_hIcon.m_pRInfo->pResource->m_hbmpImage);

	if (IsAlpha())
	{
		return (long)reinterpret_cast<LONG_PTR>(m_hIcon.GetBitmap());
	}

	return (long)reinterpret_cast<LONG_PTR>(m_hIcon.operator HICON());
}
...
LPDISPATCH CXTPImageManagerIcon::OleCreatePicture(int nState)
{
	...
	{
		HBITMAP hbmpMask = CreateBitmap(iconCopy.GetExtent().cx, iconCopy.GetExtent().cy, 1, 1,
										NULL);
	...
	if (FAILED(::OleCreatePictureIndirect(&pdesc, IID_IPictureDisp, TRUE, (LPVOID*)&lpPict)))
	{
		return NULL;
	}
...
{
	Draw(CDC::FromHandle(reinterpret_cast<HDC>((LONG_PTR)hDC)), CPoint(x, y),
		 static_cast<XTPImageState>(nState), CSize(cx, cy));
}
...
BEGIN_DISPATCH_MAP(CXTPImageManager, CXTPCmdTarget)
	...
	DISP_FUNCTION_ID(CXTPImageManager, "AddIcon", 2, OleAddIcon, VT_EMPTY,
					 GDI_HANDLE_TYPE VTS_I4 VTS_I4)
	DISP_FUNCTION_ID(CXTPImageManager, "LoadIcon", 3, OleLoadIcon, VT_EMPTY, VTS_BSTR VTS_I4 VTS_I4)
	DISP_FUNCTION_ID(CXTPImageManager, "LoadBitmap", 4, OleLoadBitmap, VT_EMPTY,
					 VTS_BSTR VTS_VARIANT VTS_I4)
	DISP_FUNCTION_ID(CXTPImageManager, "LoadBitmapFromResource", 14, OleLoadBitmapFromResource,
					 VT_EMPTY, MODULE_HANDLE_TYPE VTS_I4 VTS_VARIANT VTS_I4)
	DISP_FUNCTION_ID(CXTPImageManager, "LoadIconFromResource", 16, OleLoadIconFromResource,
					 VT_EMPTY, MODULE_HANDLE_TYPE VTS_I4 VTS_I4 VTS_I4)
	...
	DISP_FUNCTION_ID(CXTPImageManager, "AddBitmap", 8, OleAddBitmap, VT_EMPTY,
					 GDI_HANDLE_TYPE VTS_VARIANT VTS_I4 VTS_BOOL)
	...
	DISP_FUNCTION_ID(CXTPImageManager, "IsAlphaIconsSupported", 11, IsAlphaIconsSupported, VT_BOOL,
					 VTS_NONE)
	...
	DISP_FUNCTION_ID(CXTPImageManager, "DoPropExchange", 15, OleDoPropExchange, VT_EMPTY,
					 VTS_DISPATCH)

	DISP_FUNCTION_ID(CXTPImageManager, "_NewEnum", DISPID_NEWENUM, OleNewEnum, VT_UNKNOWN, VTS_NONE)

	DISP_FUNCTION_ID(CXTPImageManager, "LoadVectorIcon", 17, OleLoadVectorIcon, VT_BOOL,
					 VTS_BSTR VTS_I4 VTS_I4 VTS_I4)
	DISP_FUNCTION_ID(CXTPImageManager, "LoadVectorIconCode", 18, OleLoadVectorIconCode, VT_BOOL,
					 VTS_BSTR VTS_I4 VTS_I4 VTS_I4)
	DISP_FUNCTION_ID(CXTPImageManager, "LoadVectorIconFromResource", 19,
					 OleLoadVectorIconFromResource, VT_BOOL,
					 MODULE_HANDLE_TYPE VTS_BSTR VTS_I4 VTS_I4 VTS_I4 VTS_I4)
...
void CXTPImageManager::OleAddBitmap(GDI_HANDLE Handle, const VARIANT& Commands, long imageState,
									BOOL bAlphaBitmap)
{
	if (!Handle)
		return;
...
void CXTPImageManager::OleLoadBitmapFromResource(MODULE_HANDLE Module, long Resource,
												 const VARIANT& Commands, long imageState)
{
	BOOL bAlphaBitmap = FALSE;
	HBITMAP hBmp = CXTPImageManagerIcon::LoadBitmapFromResource(reinterpret_cast<HMODULE>(Module),
																MAKEINTRESOURCE(Resource),
																&bAlphaBitmap);

	if (hBmp)
	{
		OleAddBitmap((OLE_HANDLE) reinterpret_cast<LONG_PTR>(hBmp), Commands, imageState,
					 bAlphaBitmap);
		return;
	}
}

void CXTPImageManager::OleLoadIconFromResource(MODULE_HANDLE Module, long Resource, long Command,
											   long imageState)
{
	SetIconFromResource(reinterpret_cast<HMODULE>(Module), MAKEINTRESOURCE(Resource), Command,
						CSize(0, 0), static_cast<XTPImageState>(imageState));
}
...
void CXTPImageManager::OleLoadBitmap(LPCTSTR Path, const VARIANT& Commands, long imageState)
{
	...
	if (SUCCEEDED(OleLoadPicturePath((LPOLESTR)XTP_CT2CW(Path), NULL, 0, 0, IID_IPicture,
									 (LPVOID*)&pPict)))
	{
		CBitmap bmp;
		...
		UINT* pSrcBits = NULL;
		if (bmp.Attach(CreateDIBSection(NULL, (BITMAPINFO*)&BMI, DIB_RGB_COLORS, (void**)&pSrcBits,
										0, 0l)))
		{
		...
		OleAddBitmap((OLE_HANDLE) reinterpret_cast<LONG_PTR>(bmp.Detach()), Commands, imageState,
					 FALSE);
...
BOOL CXTPImageManager::OleLoadVectorIcon(LPCTSTR Path, long nIDCommand, long nWidth,
										 XTPImageState imageState)
{
	IStreamPtr pStream;
	BOOL bRes = FileToStream(Path, &pStream);
	if (!bRes)
		return bRes;
...
BOOL CXTPImageManager::OleLoadVectorIconCode(LPCTSTR Code, long nIDCommand, long nWidth,
											 XTPImageState imageState)
{
	CXTPMarkupContext* pMarkupContext = XTPMarkupCreateContext(NULL, TRUE);
	if (NULL == pMarkupContext)
		return FALSE;

	CXTPImageManagerVectorImageHandle hImage =
		CXTPImageManagerVectorImageHandle::CreateMarkupHandle(pMarkupContext, FALSE, Code);
	if (!hImage)
	{
	...
	return SetVectorIcon(hImage, nIDCommand, nWidth, imageState);
}

BOOL CXTPImageManager::OleLoadVectorIconFromResource(MODULE_HANDLE Module, LPCTSTR ResourceType,
													 long Resource, long nIDCommand, long nWidth,
													 XTPImageState imageState)
{
	IStreamPtr pStream;
	BOOL bRes = ResourceToStream((HMODULE)Module, MAKEINTRESOURCE(Resource), ResourceType,
								 &pStream);
	if (!bRes)
		return bRes;
...
BOOL CXTPImageManager::OleGetNextItem(/* [in, out] */ POSITION& rPos,
									  /* [out] */ VARIANT* pVar)
{
	ASSERT(pVar);
	if (!rPos || !pVar)
...
BEGIN_DISPATCH_MAP(CXTPImageManagerIconSet, CXTPCmdTarget)
	...
	DISP_FUNCTION_ID(CXTPImageManagerIconSet, "_NewEnum", DISPID_NEWENUM, OleNewEnum, VT_UNKNOWN,
					 VTS_NONE)
...
BOOL CXTPImageManagerIconSet::OleGetNextItem(/* [in, out] */ POSITION& rPos,
											 /* [out] */ VARIANT* pVar)
{
	ASSERT(pVar);
	if (!rPos || !pVar)
...
BOOL AFX_CDECL XTPScaleBitmap(CBitmap* src, CBitmap* dst)
{
	ASSERT_VALID(src);
	ASSERT_VALID(dst);
	...
	if (src && dst)
	{
		...
		if (srcDC.CreateCompatibleDC(NULL) && destDC.CreateCompatibleDC(NULL))
		{
			...
			{
				int width  = XTP_DPI_X(static_cast<int>(bmpInfo.bmWidth));
