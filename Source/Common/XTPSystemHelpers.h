...
public:
	...
	// Parameters:
	//	   dwConditionMask   - A variable to be passed as the condition mask,
	//                         can be either VER_EQUAL or VER_GREATER_EQUAL.
	...
	//     Call this member function to check whether or not the
	//     operating system is of the Windows NT family, and if it is
	//     Windows 11 or a later version.
	// Returns:
	//     true if the OS is of the Windows NT family, and is Windows 11
	//     or a later version, otherwise returns false.
	//-----------------------------------------------------------------------
...
//===========================================================================
// Summary:
//     Critical Sections Lock/Unlock helper
//===========================================================================
...
class _XTP_EXT_CLASS CXTPAccessibleProxy;
class _XTP_EXT_CLASS CXTPAccessible
{
	friend class CXTPAccessibleProxy;

public:
	CXTPAccessible();
	virtual ~CXTPAccessible();

	LRESULT LresultFromProxyObject(WPARAM wParam);
	IAccessible* CreateAccProxy();
...
private:
	...
	struct SharedHandle
	{
		SharedHandle(CXTPAccessible* pAcc);
		void AddRef();
		void Release();
	...
	SharedHandle* m_pHandle;
};

class _XTP_EXT_CLASS CXTPAccessibleProxy : public IAccessible
{
public:
	explicit CXTPAccessibleProxy(CXTPAccessible::SharedHandle* pAccHandle);
	virtual ~CXTPAccessibleProxy();
...
private:
	CXTPAccessible::SharedHandle* m_pAccHandle;
	ULONG m_cRefs;
};
...
enum XTPSoundManagerState
{
	...
	xtpSoundMenuPopup,   // Menu pop-up system sound
};

//===========================================================================
// Summary:
//     CXTPSoundManager is a standalone class. It used to play system sound.
// See Also:
//     XTPSoundManagerState, XTPSoundManager
//===========================================================================
...
public:
	...
	//     * <b>xtpSoundMenuPopup</b>: Play menu pop-up system sound.
	//-----------------------------------------------------------------------
...
public:
	BOOL m_bSoundAvailable; // FALSE to disable sound.  you can use "m_bSoundAvailable =
							// waveOutGetNumDevs()"
private:
...
// Returns:
//     S_OK if successful, otherwise a standard error code that indicates
...
template<class T>
struct XTPDefaultInitProcs
{
	static T* AFX_CDECL Create()
	{
		return new T();
	}

	static void AFX_CDECL Destroy(T* p)
	{
		ASSERT(NULL != p);
		delete p;
	}
...
//===========================================================================
// Summary:
//     Implements thread local variable adapter. By default it is assumed that
//     the type of the adaptee is default constructible and
//     can be destroyed using the delete operator, otherwise the custom
//     object creation and destruction callbacks must be provided.
// Parameter:
//     T - Type of thread local variable.
//===========================================================================
template<class T>
class CXTPThreadLocal
{
public:
	// A custom object instance allocation and initialization callback type.
	typedef T*(AFX_CDECL* CreateProc)();

	// A custom object instance destruction and deallocation callback type.
	typedef void(AFX_CDECL* DestroyProc)(T*);

	//{{AFX_CODEJOCK_PRIVATE
	typedef XTPDefaultInitProcs<T> DefaultProcs;
	//}}AFX_CODEJOCK_PRIVATE
	...
	// Parameters:
	//     pfnCreate - A custom object instance allocation and initialization callback pointer. If
	//                 not specified then the default implementation is used.
	//     pfnDestroy - A custom object instance destruction and deallocation callback pointer. If
	//                 not specified then the default implementation is used.
	//-----------------------------------------------------------------------
	CXTPThreadLocal(CreateProc pfnCreate   = DefaultProcs::Create,
					DestroyProc pfnDestroy = DefaultProcs::Destroy);
...
private:
	...
	CreateProc m_pfnCreate;
	DestroyProc m_pfnDestroy;
};

template<class T>
AFX_INLINE CXTPThreadLocal<T>::CXTPThreadLocal(CreateProc pfnCreate /*= DefaultProcs::Create*/,
											   DestroyProc pfnDestroy /*= DefaultProcs::Destroy*/)
	: m_pfnCreate(pfnCreate)
	, m_pfnDestroy(pfnDestroy)
{
	ASSERT(NULL != pfnCreate);
	ASSERT(NULL != pfnDestroy);
