...
BOOL AFX_CDECL PX_DPI_X_Long(CXTPPropExchange* pPX, LPCTSTR pszPropName, long& lValue)
{
	ASSERT_POINTER(pPX, CXTPPropExchange);
	ASSERT_POINTER(&lValue, long);
	if (!pPX)
	...
	if (pPX->IsLoading())
	{
		...
		if (bResult)
		{
			lValue = XTP_DPI_X(val);
		}
	}
	else
	{
		val		= XTP_UNDPI_X(lValue);
		bResult = pPX->ExchangeProp(pszPropName, VT_I4, &val);
	}
							 ...
							 long lDefault)
{
	...
	if (pPX->IsLoading())
	{
		bResult = pPX->ExchangeProp(pszPropName, VT_I4, &val, &lDefault);
		if (bResult)
		{
			lValue = XTP_DPI_X(val);
		}
...
BOOL AFX_CDECL PX_DPI_Y_Long(CXTPPropExchange* pPX, LPCTSTR pszPropName, long& lValue)
{
	ASSERT_POINTER(pPX, CXTPPropExchange);
	ASSERT_POINTER(&lValue, long);
	if (!pPX)
	...
	if (pPX->IsLoading())
	{
		...
		if (bResult)
		{
			lValue = XTP_DPI_Y(val);
		}
	}
	else
	{
		val		= XTP_UNDPI_Y(lValue);
		bResult = pPX->ExchangeProp(pszPropName, VT_I4, &val);
	}
