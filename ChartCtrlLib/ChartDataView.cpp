...
void CChartDataView::InitParams( CChart* chartPtr, bool bClearMap, const CChartContainer* pHost)
{
  ChartProperty* pAttr = chartPtr->GetProperty();
  m_chartIdx      = chartPtr->GetChartIdx();
  m_precision     = pHost->m_pProperty->GetContainerPrecisionX();
  m_precisionY    = pAttr->GetPrecisionY();
  m_label         = pAttr->GetChartName();
  string_t tmpStr = pHost->GetAxisXName();
  m_labelX        = tmpStr.empty() ? string_t(_T("X")) : tmpStr;
  tmpStr          = pAttr->GetAxisYName();
  m_labelY        = tmpStr.empty() ? string_t(_T("Y")) : tmpStr;
  m_pXLabelStrFn  = pHost->GetLabXValStrFnPtr();
  m_pYLabelStrFn  = pAttr->GetLabYValStrFnPtr(); 
...
bool CChartDataView::UpdateParams(CChart* chartPtr, int flagsData) // TODO: Param bClearMap
{
  bool bRes = false;
  int flags = 0;
  size_t dataOffset = 0;
  ...
  if (chartIdx == m_chartIdx)
  {
	...
	ChartProperty* pAttr = chartPtr->GetProperty();
    m_label = pAttr->GetChartName();    // Page header must be changed with new date stamp

    int precisionX = pHost->m_pProperty->GetContainerPrecisionX();
    ...
    val_label_str_fn pYLabelStrFn = pAttr->GetLabYValStrFnPtr(); 
...
// Get the dpiRatio
  ...
  FontFamily fontFamily(A2W(m_fontName.c_str()));
...
// Number rect
  ...
  gr.MeasureString(A2W(strNmb.c_str()), -1, &textFont, rGdiF, &strFormat, &nmbRF);
...
// Number rect
  ...
  if ((flags & F_NAMEX) && !(flags & F_VALX))
    gr.MeasureString(A2W(m_labelX.c_str()), -1, &textFont, rGdiF, &strFormat, &maxXRF);
  if ((flags & F_NAMEY) && !(flags & F_VALY))
    gr.MeasureString(A2W(m_labelY.c_str()), -1, &textFont, rGdiF, &strFormat, &maxYRF);
...
void CChartDataView::ShowWaitMessage(int chartIdx, size_t newSize)
{
  if ((chartIdx != m_chartIdx) &&(newSize > 10000))
  {
    ...
    gr.DrawString(L"Calculating Layout", -1, &headerFont, boundingRect, &strFormat, &br);
  }
}
...
void CChartDataView::DrawPageHeader(float offsetX, Graphics* grPtr, size_t pageNmb, float dpiRatio)
{
  ...
  grPtr->DrawString(A2W(m_header.c_str()), -1, &headerFont,  headerRF, &strFormat, &txtBr);
  sstream_t stream;
  stream << _T("Page ") << pageNmb << _T(" of ") << m_nPages;
  strFormat.SetAlignment(StringAlignmentNear);
  grPtr->DrawString(A2W(stream.str().c_str()), -1, &headerFont, headerRF, &strFormat, &txtBr);
}
...
float CChartDataView::DrawTableHeader(float offsetX, float offsetY, Graphics* grPtr, float dpiRatio)
{
  ...
  for (size_t colCnt = 0; colCnt < m_cols; ++colCnt)
  {
    ...
    grPtr->DrawString(A2W(m_labelX.c_str()), -1, &txtFont, textRF, &strFormat, &cellBr);
    dataXRF.Offset(deltaX, 0.0f);

    grPtr->DrawRectangle(&cellPen, dataYRF);
    textRF = dataYRF;
    textRF.Width -= m_dpiRatio*DATA_ENDOFFS;
    grPtr->DrawString(A2W(m_labelY.c_str()), -1, &txtFont, textRF, &strFormat, &cellBr);
...
// Draw rows
  ...
  for (size_t colCnt = 0; colCnt < m_cols; ++colCnt)
  {
    ...
    for (size_t rowCnt = 0; rowCnt < rowNmb; ++rowCnt)
    {
      ...
      grPtr->DrawString(A2W(nmbStr.c_str()), -1, &txtFont, txtRF, &strFormat, &cellBr);
      ...
      txtRF = dataXRF;
      txtRF.Width -= txtEndOffsX;
      grPtr->DrawString(A2W(valXStr.c_str()), -1, &txtFont, txtRF,  &strFormat, &cellBr);
      ...
      txtRF = dataYRF;
      txtRF.Width -= txtEndOffsX;
      grPtr->DrawString(A2W(valYStr.c_str()), -1, &txtFont, txtRF,  &strFormat, &cellBr);
