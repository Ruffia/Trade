...
// Add container name, name_x, and precision
    ...
    if (hr == S_OK)
    {
      ...
      pe->setAttribute(_bstr_t(_T("Precision_X")), variant_t(pContainer->m_pProperty->GetContainerPrecisionX()));
...
// Get and save chart attributes
      string_t name = pAttr->GetChartName();
      string_t nameY = pAttr->GetAxisYName();
      Color chartColor = pAttr->GetChartColor();
      bool bVisible  = chartPtr->IsChartVisible();
      DashStyle dashStyle = pAttr->GetChartDashStyle();
      float penWidth = pAttr->GetPenWidth();
      float tension  = pAttr->GetTension();
      int precisionX = pContainer->m_pProperty->GetContainerPrecisionX();
      ...
      string_t sampleDataY = pAttr->GetLabYValStrFnPtr()(pntD.Y, precisionY, false);
...
// Get the chart data points
          ...
          if (bClearCharts)
          {
            pContainer->m_pProperty->SetContainerPrecision(precisionX);
            pContainer->SetAxisXName(nameX);
            pContainer->m_pProperty->SetContainerPrecision(precisionX);
          }
		  ChartProperty* pAttr = new ChartProperty(true, chName, nameY, precisionY, DashStyle(dashStyle), 
			  penWidth, tension, chColor);
		  CChart* pChart = new CChart;
          pContainer->AddChart(pAttr,pChart, vData);
      }
      pNodeList->Release();
    }
    catch(_TCHAR* str)
