...
//坐标区间
typedef struct AxisSection
{
	double  m_dValue;           //区间值
	double  m_dScreenValueMin;  //区间值对应的坐标最小值
	double  m_dScreenValueMax;  //区间值对应的坐标最大值
	AxisSection(double dValue,double dScreenValueMin,double dScreenValueMax)
	{
		m_dValue = dValue;
		m_dScreenValueMin = dScreenValueMin;
		m_dScreenValueMax = dScreenValueMax;
	}
}AXISSECTION;
...
typedef std::tuple<float, float, float, float, float, float,float, float,int> TUPLE_PRNLEGLAYOUT; 

typedef std::tuple<string_t, string_t, string_t, string_t, string_t, string_t, string_t, Gdiplus::Color, Gdiplus::DashStyle, float> TUPLE_PRINT;
